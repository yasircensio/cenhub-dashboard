...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),y=$.summary||$;l=y.runId,c=0,setFbLeadProgress({processed:0,total:y.inWindow||0,label:`Matching ${y.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:b||g?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:r})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const y=$.inWindow||c||b;a?(setFbLeadProgress({processed:c,total:y,complete:!$.hasMore}),s&&(s.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:y,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const w=d.summary||d,m=w.inWindow||c||b,f=w.nextBatchOffset??c;if(a)setFbLeadProgress({processed:f,total:m,complete:!0}),markFbLeadPreviewReady(e,t,w.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(w.updated||0)>0),showToast((w.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:f,total:m,written:w.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(w){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${w.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((w.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(s=>s.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null,settings:o=null}={}){const s=n||e?.monthKey||null,r=s?metaReportMonthBounds(s):null,i=o||a?.settings||{},l=isMetaReportGhlListClient(i),c=resolveClientToplineMode(i),d=resolveMetaToplineSource(e,i),u=l&&c==="cenhub"&&d==="ghl";let p="";if(t&&e&&!e.meta?.emptyMonth&&r){const R=u?" readonly":"",L=renderMetaReportMonthManualRow(e,i);p=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        ...
        <div class="meta-report-edit-grid">
          ...
          <label class="meta-cv-value-field${u?" is-locked":""}">Number of leads<input type="number" step="any" id="meta-report-leads" value="${esc(u?e.ghlTotals?.leads??e.topline?.leads??"":e.topline?.leads??"")}"${R}${u?' disabled tabindex="-1" aria-readonly="true"':""} /></label>
          <label class="meta-cv-value-field${u?" is-locked":""}">Won leads<input type="number" step="any" id="meta-report-won-leads" value="${esc(e.topline?.wonLeads??"")}"${R}${u?' disabled tabindex="-1" aria-readonly="true"':""} /></label>
          <label>Avg lead value<input type="number" step="any" id="meta-report-avg-lead-value" value="${esc(e.topline?.avgLeadValue??"")}"${R} /></label>
          <label>Avg profit per won<input type="number" step="any" id="meta-report-avg-profit" value="${esc(e.inputs?.avgProfitPerWon??"")}"${R} /></label>
        </div>
        ${L}
  ...
  `}function updateGoogleAdsHubDom(e){const t=document.getElementById("google-ads-banner"),a=document.getElementById("google-ads-cards"),n=document.getElementById("google-ads-count"),o=e.google||{};if(t&&(t.innerHTML=o.partnerFetchError?`<div class="admin-banner admin-banner--warn">${esc(o.partnerFetchError)}</div>`:""),n){const s=(e.clients||[]).length;n.textContent=`${s} account${s===1?"":"s"}`}a&&(a.innerHTML=(e.clients||[]).length?e.clients.map(renderGoogleAdsHubCard).join(""):'<div class="sync-history-empty" style="padding:24px">No Google Ads accounts found.</div>'),bindGoogleAdsHubRowEvents()}function bindGoogleAdsHubRowEvents(){document.querySelectorAll("[data-google-ads-enable]").forEach(e=>{e.onclick=async()=>{const t=e.getAttribute("data-google-ads-enable"),a=e.getAttribute("data-google-ads-name")||t;e.disabled=!0;try{const n=await adminFetch("/api/google-ads-reports/provision",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({googleCustomerId:t,accountName:a})});if(showToast(`Enabled ${a}`,"success"),n?.clientId){window.location.href=`/admin/google-ads/${encodeURIComponent(n.clientId)}`;return}await loadGoogleAdsHubPage()}catch(n){showToast(n.message||"Enable failed","error"),e.disabled=!1}}})}function bindGoogleAdsHubEvents(){const e=document.getElementById("google-ads-search");e&&(e.oninput=()=>{googleAdsReportsState.searchQuery=e.value,updateGoogleAdsHubDom(getGoogleAdsHubView(googleAdsReportsState.dashboardData,googleAdsReportsState.filter,googleAdsReportsState.searchQuery))}),document.querySelectorAll("[data-google-ads-filter]").forEach(a=>{a.onclick=()=>{googleAdsReportsState.filter=a.getAttribute("data-google-ads-filter")||"all",document.querySelectorAll("[data-google-ads-filter]").forEach(n=>{n.classList.toggle("is-active",n===a)}),updateGoogleAdsHubDom(getGoogleAdsHubView(googleAdsReportsState.dashboardData,googleAdsReportsState.filter,googleAdsReportsState.searchQuery))}});const t=document.getElementById("google-ads-refresh");t&&(t.onclick=()=>loadGoogleAdsHubPage()),bindGoogleAdsHubRowEvents()}async function loadGoogleAdsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
    ...
    `);try{const a=await adminFetch("/api/google-ads-reports?filter=all");fetch("http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c",{method:"POST",headers:{"Content-Type":"application/json","X-Debug-Session-Id":"f182bb"},body:JSON.stringify({sessionId:"f182bb",runId:"post-fix-v25",hypothesisId:"F",location:"index.html:loadGoogleAdsHubPage",message:"Google Ads hub API payload",data:{partnerFetchError:a?.google?.partnerFetchError||null,debug:a?.google?.debug||null,accountCount:(a?.clients||[]).length},timestamp:Date.now()})}).catch(()=>{}),googleAdsReportsState.dashboardData=a;const n=getGoogleAdsHubView(a,googleAdsReportsState.filter,googleAdsReportsState.searchQuery);googleAdsReportsState.hubMounted?updateGoogleAdsHubDom(n):(e.innerHTML=renderGoogleAdsHubPage(n),googleAdsReportsState.hubMounted=!0,bindGoogleAdsHubEvents())}catch(a){e.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("google-ads"))}
      ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(a.message)}</div>`)}
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(E=>E.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label class="meta-cv-value-field${C}">Avg profit per won${r?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(R)}"${L} />
      </label>
    </div>
    ${M}
