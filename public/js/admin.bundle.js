(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){if(t[1]==="sync-history"&&(t[2]==="ghl"||t[2]==="meta")){document.body.dataset.dashboardMode=`sync-history-${t[2]}`,delete document.body.dataset.clientSlug;return}if(t[1]==="fb-lead-sync"){document.body.dataset.dashboardMode="fb-lead-sync",delete document.body.dataset.clientSlug;return}document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_ADMIN_SYNC_HISTORY_GHL=DASHBOARD_MODE==="sync-history-ghl",IS_ADMIN_SYNC_HISTORY_META=DASHBOARD_MODE==="sync-history-meta",IS_ADMIN_FB_LEAD_SYNC=DASHBOARD_MODE==="fb-lead-sync",IS_ADMIN_SYNC_HISTORY=IS_ADMIN_SYNC_HISTORY_GHL||IS_ADMIN_SYNC_HISTORY_META,IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);return e[0]==="admin"&&e[1]?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){return`
        ...
        <div class="auth-card">
          <div class="auth-card-header">
            <div class="auth-card-title-row">
              <div class="auth-card-icon" aria-hidden="true">${c}</div>
  ...
  `}function renderSyncHistoryRows(e,t){if(!e.length)return'<div class="sync-history-empty">No sync runs logged yet.</div>';const n=t==="meta"?"Spend / details":"Details",a=e.map(s=>{let i="\u2014";if(t==="meta"){const o=[];if(s.thisMonthSpend!=null&&o.push(`${fmtDkk(s.thisMonthSpend)} DKK this month`),s.spendDateStop){const c=formatSyncHistoryDate(s.spendDateStop);c&&o.push(`through ${c}`)}s.errorMessage&&o.push(s.errorMessage),i=o.join(" \xB7 ")||"\u2014"}else s.opportunityCount!=null?(i=`${s.opportunityCount} opportunities`,s.errorMessage&&(i+=` \xB7 ${s.errorMessage}`)):s.errorMessage&&(i=s.errorMessage);return`
      <tr>
...
${o} contact(s) will get Fb Lead id written in GHL. This writes live data.`)&&(fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:!0,previewRunId:n},await runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!1}))}async function refreshFbLeadClientRowsUi(){const e=document.getElementById("fb-lead-clients-mount");e&&(e.innerHTML=renderFbLeadClientRows(fbLeadSyncState.clients)),bindFbLeadSyncToggles()}function fbLeadRunHasLaterApply(e,t){const n=Date.parse(t.startedAt||"");return Number.isFinite(n)?(e||[]).some(a=>{if(a.id===t.id||a.clientId!==t.clientId||(a.mode||"recent")!==(t.mode||"recent")||a.dryRun)return!1;const s=String(a.status||"").toLowerCase();return s!=="success"&&s!=="ok"?!1:Date.parse(a.startedAt||"")>n}):!1}function renderFbLeadClientActions(e){const t=esc(e.clientId),n=e.metaPageId?"":`<a class="admin-btn admin-btn--ghost admin-btn--small" href="/admin/${encodeURIComponent(e.clientId)}">Setup</a>`;return`
  ...
  `:'<div class="sync-history-empty">No clients configured yet.</div>'}function renderFbLeadClientRowsWrapper(e){return`<div id="fb-lead-clients-mount">${renderFbLeadClientRows(e)}</div>`}function renderFbLeadHistoryRows(e,t={}){return e.length?`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table">
        ...
        <tbody>${e.map(a=>{const s=[a.mode,a.dryRun?"dry-run":"apply",`${a.updated||0} updated`,`${a.skippedNoMatch||0} no match`,a.errors?`${a.errors} errors`:null].filter(Boolean).join(" \xB7 "),i=dryRunIsReadyForApply(a,e),o=fbLeadSyncState.applyingRunId===a.id;return`
      <tr class="fb-lead-audit-row">
        ...
        <td>
          <div class="fb-lead-history-actions">
            ${i?`<button class="admin-btn admin-btn--primary admin-btn--small" type="button" data-fb-apply-run="${Number(a.id)}" data-apply-count="${Number(a.updated||0)}"${o?" disabled":""} onclick="applyFbLeadSyncFromHistory(${Number(a.id)})">${o?"Applying\u2026":`Apply (${a.updated})`}</button>`:""}
  ...
  `}function renderGlobalApplyProgress(e,t){const n=fbLeadSyncState.clients.find(s=>s.clientId===e);return`
    <div class="fb-lead-apply-progress-card" id="fb-lead-global-progress-card">
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start">
        <div>
          <strong>Applying ${esc(t==="backfill"?"90-day backfill":"recent sync")} to GHL</strong>
          <div style="color:var(--text-soft);font-size:13px;margin-top:4px">${esc(n?.accountName||e)}</div>
        </div>
        <span class="fb-lead-badge is-ok" id="fb-lead-apply-status-badge">Running</span>
      </div>
      <div class="fb-lead-progress-wrap" id="fb-lead-global-progress-wrap">
        <div class="fb-lead-progress-label" id="fb-lead-global-progress-label">Starting\u2026</div>
        <div class="fb-lead-progress is-visible">
          <div class="fb-lead-progress-bar" id="fb-lead-global-progress-bar"></div>
        </div>
      </div>
    </div>
  `}function showGlobalApplyProgress(e,t){const n=document.getElementById("fb-lead-apply-progress-mount");n&&(n.innerHTML=renderGlobalApplyProgress(e,t),n.scrollIntoView({behavior:"smooth",block:"nearest"}))}function finishGlobalApplyProgress(e){const t=document.getElementById("fb-lead-apply-status-badge");t&&(t.textContent=e?"Complete":"Failed",t.classList.toggle("is-ok",e),t.classList.toggle("is-missing",!e)),window.setTimeout(()=>{const n=document.getElementById("fb-lead-apply-progress-mount");n&&fbLeadSyncState.applyingRunId==null&&(n.innerHTML="")},e?5e3:1e4)}function getFbLeadProgressElements(e=!1){return e?{wrap:document.getElementById("fb-lead-global-progress-wrap"),bar:document.getElementById("fb-lead-global-progress-bar"),label:document.getElementById("fb-lead-global-progress-label")}:{wrap:document.getElementById("fb-lead-progress-wrap"),bar:document.getElementById("fb-lead-progress-bar"),label:document.getElementById("fb-lead-progress-label")}}function setHistoryApplyButtonState(e,t){const n=document.querySelector(`[data-fb-apply-run="${Number(e)}"]`);if(!n)return;n.disabled=t;const a=n.dataset.applyCount||"0";n.textContent=t?"Applying\u2026":`Apply (${a})`}function setFbLeadProgress({processed:e=0,total:t=0,complete:n=!1,label:a="",global:s=!1}={}){const{wrap:i,bar:o,label:c}=getFbLeadProgressElements(s);if(!i||!o)return;i.hidden=!1,i.classList.toggle("is-complete",n);const r=t>0?Math.min(100,Math.round(e/t*100)):n?100:0;o.style.width=`${r}%`,c&&(c.textContent=a||(n?`Complete \u2014 ${e} / ${t} leads (${r}%)`:`${s?"Writing to GHL":"Processing"} \u2014 ${e} / ${t} leads (${r}%)`))}function hideFbLeadProgress(e=!0,t=!1){const{wrap:n,bar:a}=getFbLeadProgressElements(t);n&&e&&(n.hidden=!0,n.classList.remove("is-complete"),a&&(a.style.width="0%"))}async function refreshFbLeadHistoryUi(){await loadFbLeadHistory();const e=document.getElementById("fb-lead-history-mount");if(!e)return;const t=Object.fromEntries(fbLeadSyncState.clients.map(n=>[n.clientId,n.locationId]));e.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,t)}async function clearFbLeadSyncHistory(){if(!(!isStaffAdmin()||!window.confirm(`Delete all FB lead sync run history?

This only clears the log \u2014 it does not change GHL contacts or Meta data.`)))try{const t=await adminFetch("/api/fb-lead-sync/history",{method:"DELETE"});showToast(`Cleared ${t.deleted||0} run(s).`,"success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(t){showToast(t.message||"Failed to clear FB lead sync log.","error")}}async function deleteFbLeadRun(e){if(!(!isStaffAdmin()||!window.confirm(`Delete run #${e} from history?`)))try{await adminFetch(`/api/fb-lead-sync/history/${e}`,{method:"DELETE"}),showToast("Run deleted.","success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(n){showToast(n.message||"Failed to delete run.","error")}}async function runFbLeadSyncBatch({clientId:e,mode:t,dryRun:n}){const a=document.getElementById("fb-lead-preview-btn"),s=document.getElementById("fb-lead-run-results");a&&(a.disabled=!0);let i=fbLeadSyncState.activeRun?.runId||null,o=0,c=null,r=[],f=!1;const h=fbLeadSyncState.activeRun?.previewRunId??null;n||(showGlobalApplyProgress(e,t),fbLeadSyncState.applyingRunId=h,h!=null&&setHistoryApplyButtonState(h,!0));try{setFbLeadProgress({processed:0,total:0,label:n?"Starting preview\u2026":"Starting apply\u2026",global:!n});do{c=await adminFetch("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:i,batchOffset:o,batchLimit:25})});const v=c.summary||c;i=v.runId,o=v.nextBatchOffset??v.batchOffset+(v.batchProcessed||0),r=r.concat(v.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=i),setFbLeadProgress({processed:o,total:v.inWindow||o,complete:!v.hasMore,global:!n}),s&&n&&(s.innerHTML=renderFbLeadRunResults({...v,rows:r}))}while((c.summary||c).hasMore);f=!0;const b=c.summary||c;if(setFbLeadProgress({processed:b.nextBatchOffset??o,total:b.inWindow||o,complete:!0,global:!n,label:n?void 0:"Complete \u2014 written to GHL"}),n)markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=!0),showToast("Preview complete \u2014 use Apply in Run history below.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{delete fbLeadSyncState.previewOkByClient[e];try{const v=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete v[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(v))}catch{}finishGlobalApplyProgress(!0),showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success")}await refreshFbLeadHistoryUi();const C=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=C.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){n?hideFbLeadProgress(!0,!1):(finishGlobalApplyProgress(!1),setFbLeadProgress({processed:0,total:0,complete:!1,global:!0,label:`Failed \u2014 ${b.message||"Apply failed"}`})),showToast((b.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress may be saved \u2014 refresh and try Preview again.":""),"error")}finally{a&&(a.disabled=!1),!n&&h!=null&&setHistoryApplyButtonState(h,!1),fbLeadSyncState.applyingRunId=null,!f&&n&&hideFbLeadProgress(!0,!1)}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=fbLeadSyncState.historyRuns.filter(s=>s.clientId===e),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1}={}){const t=document.getElementById("dashboard");if(!t)return;const n=await fetchStaffMe();if(!n){e||(t.innerHTML=`
        ${renderBrandTopbar("")}
        ${wrapDashboardShell(`
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_CHECK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>',ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
    ...
    <div class="date-picker-panel">
      <div class="date-picker-header">
        ...
        <div class="date-picker-nav">
          ...
          <button type="button" class="date-picker-nav-btn" aria-label="Next month" onclick="shiftDatePickerMonth(1)" ${d?"":"disabled"}>${ICON_CHEVRON_RIGHT}</button>
        </div>
      </div>
      <div class="date-picker-weekdays">
        ${i.map(m=>`<div class="date-picker-weekday">${m}</div>`).join("")}
      </div>
      <div class="date-picker-grid">${L}</div>
    ...
    <table>
      ...
      <tbody>
        ${a.length?a.map(o=>`
          <tr>
            <td>${s(o,"label")}</td>
            ${i.map(c=>`<td class="${c.align||""}">${s(o,c.key)}</td>`).join("")}
          </tr>
  ...
  `}function renderDashboardContent(e){const{kpis:t,statusBreakdown:n,sourceReport:a,assigneeReport:s,pipelines:i,filters:o}=e,c=isVisible("sections","statusBreakdown");return`
      ...
      <div class="section-grid">
        ${isVisible("sections","assigneeReport")?`
          <div class="card">
            ...
            ${renderTable("assigneeReport","Assignee",[{key:"won",label:"Won",align:"num"},{key:"totalLeads",label:"Total leads",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"},{key:"totalValue",label:"Total value",align:"num"}],s,(r,f)=>f==="label"?esc(r.assigneeName):f==="wonValue"||f==="totalValue"?`Dkr ${fmt(r[f])}`:fmt(r[f]))}
          </div>
        `:""}
        ${isVisible("sections","pipelineBreakdown")?`
          <div class="card">
            <div class="section-title">Pipeline Breakdown</div>
            ${renderTable("pipelineBreakdown","Pipeline",[{key:"count",label:"Leads",align:"num"},{key:"won",label:"Won",align:"num"},{key:"monetary",label:"Value",align:"num"},{key:"profit",label:"Bundlinje",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"}],i,(r,f)=>f==="label"?esc(r.name):f==="monetary"||f==="profit"||f==="wonValue"?`Dkr ${fmt(r[f])}`:fmt(r[f]))}
          </div>
        `:""}
      </div>
    `:""}
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),c=document.getElementById("apply-filters-btn");if(e||!cachedData){const r=++fetchGeneration;activeFetchController=new AbortController;const f=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const h=document.getElementById("dashboard-content");h&&(h.innerHTML=renderKpiSkeleton()),c&&(c.disabled=!0,c.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let h=null;if(!availablePipelines.length){if(h=await bootstrapDashboardData(f),r!==fetchGeneration)return;availablePipelines=h.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,h.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const C=canReuseBootstrapDashboard(h)&&!s;let v,k;if(C?[k,v]=await Promise.all([fetchFacebookMetrics(f),Promise.resolve(h)]):[v,k]=await Promise.all([fetchDashboardData(availablePipelines,f,{forceFresh:s}),fetchFacebookMetrics(f)]),r!==fetchGeneration)return;cachedFacebookMetrics=k,cachedData=applyMarketingData(v,k),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(h){if(r!==fetchGeneration)return;if(h.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            </div>`)}else o?o&&cachedData?(restoreDashboardContentAfterFailedFetch(o),showToast(h.message||"Failed to load filtered data.","error")):c&&(c.textContent="Apply failed - try again"):i.innerHTML=`
          ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
          ${wrapDashboardShell(`
            <div class="error-state">
              <div>Fejl ved hentning af data</div>
