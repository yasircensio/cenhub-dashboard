...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),w=$.summary||$;l=w.runId,c=0,setFbLeadProgress({processed:0,total:w.inWindow||0,label:`Matching ${w.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:v||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:s})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const w=$.inWindow||c||v;a?(setFbLeadProgress({processed:c,total:w,complete:!$.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:w,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||v,g=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:g,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:g,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const y=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=y.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `:"",p=renderMetaReportMonthTables(e,{editable:t}),h=d?renderMetaReportChartsPanel({editable:t}):"",m=t&&d?renderMetaReportScenarioCard():"",v=d?renderMetaReportViewModeTabs():"";let b;return d?b=`
      ...
      ${m}
    `:b=`<div class="meta-report-content-main">${p}</div>`,`${s}${u}${v}${b}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),v=META_REPORT_FEE_MODE_META.map(({value:y,icon:$,title:w,desc:S})=>`
