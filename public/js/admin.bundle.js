...
Resume the interrupted ${b}?`)&&(l=m.id,d=Number(m.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const m=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),u=m.summary||m;l=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||v?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:I,batchOffset:d,batchLimit:o})});const m=c.summary||c;l=m.runId,d=m.nextBatchOffset??m.batchOffset+(m.batchProcessed||0),y=y.concat(m.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const u=m.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:u,complete:!m.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...m,rows:y}))):updateGlobalApplyProgress({processed:d,total:u,written:m.updated||0,complete:!m.hasMore})}while((c.summary||c).hasMore);g=!0;const $=c.summary||c,f=$.inWindow||d||w,h=$.nextBatchOffset??d;if(n)setFbLeadProgress({processed:h,total:f,complete:!0}),markFbLeadPreviewReady(e,t,$.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=($.updated||0)>0),showToast(($.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:f,written:$.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const m=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete m[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(m))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch($){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${$.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast(($.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!g&&n&&hideFbLeadProgress(!0),!n&&!g&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `).join("")}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;if(!t||!metaReportsState.activeMonthKey)return;t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e;const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e,getMetaReportMonthBodyOptions(t.settings,{editable:!0})),bindMetaReportsClientEditEvents(t.clientId,t),refreshMetaReportBackfillControl(t))}function refreshMetaReportBackfillControl(e){const t=document.querySelector(".meta-report-client-page .meta-report-toolbar-left"),n=document.querySelector(".meta-report-backfill-wrap"),a=metaReportMonthsNeedingBackfill(e);if(!a.length){n&&n.remove();return}const s=`${ICON_SYNC} Backfill ${a.length} month${a.length===1?"":"s"} from Meta`,i=document.getElementById("meta-report-backfill");if(i)i.innerHTML=s,i.disabled=!1;else if(t){const o=document.createElement("div");o.className="meta-report-backfill-wrap",o.innerHTML=`
    ...
    `,t.appendChild(o),bindMetaReportBackfillButton(e.clientId)}}function updateMetaReportsClientContent(e){metaReportsState.clientPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null);const n=document.getElementById("meta-report-year");if(n){const r=[e.year,e.year-1].filter((l,d,c)=>c.indexOf(l)===d);n.innerHTML=r.map(l=>`
      <option value="${l}"${Number(metaReportsState.selectedYear)===Number(l)?" selected":""}>${l}</option>
    `).join("")}const a=document.getElementById("meta-report-copy-link");e.reportUrl?a&&(a.setAttribute("data-copy-report-url",e.reportUrl),a.hidden=!1):a&&(a.hidden=!0);const s=e.settings||{};syncMetaReportSettingsControls(s);const i=document.querySelector(".meta-report-tabs");i&&(i.innerHTML=renderMetaReportsClientTabs(e),bindMetaReportsClientTabEvents(e));const o=document.getElementById("meta-report-month-panel");o&&(o.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(s,{editable:!0})),bindMetaReportsClientEditEvents(e.clientId,e)),refreshMetaReportBackfillControl(e)}function updatePublicMetaReportContent(e){metaReportsState.publicPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null);const n=document.getElementById("meta-report-year");if(n){const i=[e.year,e.year-1].filter((o,r,l)=>l.indexOf(o)===r);n.innerHTML=i.map(o=>`
      <option value="${o}"${Number(metaReportsState.selectedYear)===Number(o)?" selected":""}>${o}</option>
    `).join("")}const a=document.querySelector(".meta-report-tabs");a&&(a.innerHTML=t.map(i=>`
  ...
  `}function normalizeMetaReportTableColumns(e){return Number(e)===2?2:1}function getMetaReportMonthBodyOptions(e={},{editable:t=!1}={}){return{editable:t,showOtherToClient:e.metaReportShowOther!==!1,tableColumns:normalizeMetaReportTableColumns(e.metaReportTableColumns)}}function renderMetaReportMonthBody(e,{editable:t=!1,showOtherToClient:n=!0,tableColumns:a=1}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return`<div class="meta-report-empty">No ad spend recorded for this month yet.${t?' Click "Refresh from Meta" below, or use "Backfill from Meta" above to pull historical months.':""}</div>`;let s="";t&&(s=`
    ...
    `);const i=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["CTR",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],o=[["Leads",metaFmtNum(e.topline?.leads),""],["Cost Per Lead (CPL)",metaFmtKr(e.topline?.cpl),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let r="";if(r+=renderMetaReportMetricTable("Meta ads",i,"meta"),r+=renderMetaReportMetricTable("Topline KPI'er",o,"topline",{highlightLastN:2}),e.bottomline){const y=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];e.bottomline.censioFee>0&&(y.push([`Censio performance fee (${metaFmtNum(e.bottomline.feePercent,0)}%)`,metaFmtKr(e.bottomline.censioFee),""]),y.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),y.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])),r+=renderMetaReportMetricTable("Bottomline KPI'er",y,"bottomline",{highlightLastN:2})}if(!!e.customLineItems?.length&&(t||n)){const y=e.customLineItems.map(g=>[g.label,metaFmtKr(g.amount),""]);y.push(["Total",metaFmtKr(e.customLineItemsTotal),""]),r+=renderMetaReportMetricTable("Other",y,"other",{badge:t&&!n?"Hidden from client":"",highlightLastN:1})}const d=normalizeMetaReportTableColumns(a),c=`
    ${renderMetaReportHighlightStrip(e)}
    <div class="meta-report-groups-grid is-cols-${d}">${r}</div>
  `;return`${s}<div class="meta-report-metrics-card"><div class="meta-report-metrics-card-label">${t?"Report preview":"Report"}</div>${c}</div>`}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      ...
      <div class="meta-report-settings">
        <div class="meta-report-settings-group">
          ...
          <label class="meta-report-layout-field">Table layout
            <select id="meta-report-setting-table-columns" class="admin-select">
              <option value="1"${normalizeMetaReportTableColumns(s.metaReportTableColumns)===1?" selected":""}>1 column</option>
              <option value="2"${normalizeMetaReportTableColumns(s.metaReportTableColumns)===2?" selected":""}>2 columns</option>
            </select>
          </label>
        </div>
