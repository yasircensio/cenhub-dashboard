...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(n?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),w=$.summary||$;l=w.runId,c=0,setFbLeadProgress({processed:0,total:w.inWindow||0,label:`Matching ${w.inWindow||0} Meta leads to GHL\u2026`})}else!n&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:b||g?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:f,batchOffset:c,batchLimit:s})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const w=$.inWindow||c||b;n?(setFbLeadProgress({processed:c,total:w,complete:!$.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:w,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const y=d.summary||d,m=y.inWindow||c||b,h=y.nextBatchOffset??c;if(n)setFbLeadProgress({processed:h,total:m,complete:!0}),markFbLeadPreviewReady(e,t,y.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(y.updated||0)>0),showToast((y.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:m,written:y.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(y){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${y.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((y.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&n&&hideFbLeadProgress(!0),!n&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(r=>r.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `).join(""):'<p class="linked-report-picker-empty">No matching accounts.</p>'}async function saveLinkedReportPair(e,t){const n=`/api/google-ads-reports/clients/${encodeURIComponent(e)}/settings`;return adminFetch(n,{method:"PATCH",body:JSON.stringify({linkedMetaClientId:t})})}async function reloadLinkedAdminPairAndRender(){if(linkedAdminOrigin()==="google-ads"){googleAdsReportsState.clientPageMounted=!1,await loadGoogleAdsClientPage({silent:!0});return}metaReportsState.clientPageMounted=!1,await loadMetaReportsClientPage({silent:!0})}async function reloadCurrentLinkedEditor(){if(isGoogleAdsEditorActive()&&linkedReportsState.googlePayload){if(linkedReportsState.origin==="meta"){const e=linkedReportsState.googlePayload.clientId,t=await adminFetch(`/api/google-ads-reports/clients/${encodeURIComponent(e)}?year=${encodeURIComponent(googleAdsReportsState.selectedYear)}`);linkedReportsState.googlePayload=t,googleAdsReportsState.clientPayload=t,googleAdsReportsState.selectedYear=Number(t.year)||googleAdsReportsState.selectedYear,applyLinkedAdminPlatformState("google-ads");const n=document.querySelector(".meta-report-client-page");n&&(n.innerHTML=renderLinkedAdminEditorInner()),bindLinkedAdminEditorEvents();const a=googleAdsPayloadToMetaUiShape(t);requestAnimationFrame(()=>{mountMetaReportCharts(a,{editable:!0,clientId:e})});return}await loadGoogleAdsClientPage({silent:!0});return}if(linkedReportsState.origin==="google-ads"&&linkedReportsState.metaPayload){const e=linkedReportsState.metaPayload.clientId,t=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`);linkedReportsState.metaPayload=t,applyLinkedAdminPlatformState("meta");const n=document.querySelector(".meta-report-client-page");n&&(n.innerHTML=renderLinkedAdminEditorInner()),bindLinkedAdminEditorEvents(),requestAnimationFrame(()=>{mountMetaReportCharts(t,{editable:!0,clientId:e})});return}await loadMetaReportsClientPage({silent:!0})}async function openLinkedReportPickerModal(){const e=linkedAdminOrigin(),t=linkedReportsState.pair;openMetaReportModal(`
    ...
    ${t?'<div class="linked-report-unlink-row"><button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="linked-report-unlink-btn">Unlink</button></div>':""}
    <input type="search" class="admin-input linked-report-picker-search" id="linked-report-picker-search" placeholder="Search accounts" autocomplete="off" />
    <div class="linked-report-picker-list" id="linked-report-picker-list">Loading\u2026</div>
  `,{wide:!0});const a=document.getElementById("linked-report-picker-list"),o=document.getElementById("linked-report-picker-search"),r=document.getElementById("linked-report-unlink-btn"),s=linkedReportsState.metaPayload?.clientId||(e==="meta"?CLIENT_SLUG:t?.metaClientId),i=linkedReportsState.googlePayload?.clientId||(e==="google-ads"?CLIENT_SLUG:t?.googleClientId),l=c=>{if(!a)return;const d=()=>{a.innerHTML=renderLinkedReportPickerItems(c,o?.value),a.querySelectorAll("[data-linked-pick-id]").forEach(u=>{u.onclick=async()=>{const p=u.getAttribute("data-linked-pick-id");u.disabled=!0;try{e==="meta"?await saveLinkedReportPair(p,s):await saveLinkedReportPair(i,p),closeMetaReportModal(),showToast("Reports linked","success"),await reloadLinkedAdminPairAndRender()}catch(f){showToast(f.message||"Could not link reports","error"),u.disabled=!1}}})};o&&(o.oninput=d),d()};r&&(r.onclick=async()=>{if(i){r.disabled=!0;try{await saveLinkedReportPair(i,null),closeMetaReportModal(),showToast("Reports unlinked","success"),linkedReportsState.pair=null,e==="meta"?linkedReportsState.googlePayload=null:linkedReportsState.metaPayload=null,linkedReportsState.activePlatform=e,await reloadLinkedAdminPairAndRender()}catch(c){showToast(c.message||"Could not unlink","error"),r.disabled=!1}}});try{if(e==="meta"){const d=((await adminFetch("/api/google-ads-reports?filter=enabled")).clients||[]).filter(u=>u.clientId&&u.googleAdsReportEnabled&&!u.needsSetup).filter(u=>!u.linkedMetaClientId||String(u.linkedMetaClientId)===String(s)).map(u=>({id:u.clientId,name:u.accountName,sub:u.googleCustomerLabel||u.googleCustomerId||u.clientId}));l(d)}else{const d=((await adminFetch("/api/meta-reports?filter=enabled")).clients||[]).filter(u=>u.clientId&&u.metaReportEnabled&&!u.needsSetup).filter(u=>!u.linkedGoogleClientId||String(u.linkedGoogleClientId)===String(i)).map(u=>({id:u.clientId,name:u.accountName,sub:u.clientId}));l(d)}}catch(c){a&&(a.innerHTML=`<p class="linked-report-picker-empty">${esc(c.message||"Could not load accounts")}</p>`)}}async function fetchLinkedPartnerPayload(e,t){const n=linkedReportPairFromPayload(e);if(!n)return null;try{if(t==="meta"&&n.googleClientId){const a=linkedReportsState.googlePayload?.year||googleAdsReportsState.selectedYear||e.year;return await adminFetch(`/api/google-ads-reports/clients/${encodeURIComponent(n.googleClientId)}?year=${encodeURIComponent(a)}`)}if(t==="google-ads"&&n.metaClientId){const a=linkedReportsState.metaPayload?.year||metaReportsState.selectedYear||e.year;return await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(n.metaClientId)}?year=${encodeURIComponent(a)}`)}}catch(a){console.warn("Linked partner report failed to load",a)}return null}const META_HUB_FILTER_OPTIONS=[{value:"all",label:"All"},{value:"enabled",label:"Live reports"},{value:"meta-only",label:"Meta only"},{value:"needs-setup",label:"Needs setup"}];function metaHubClientInitial(e){const t=String(e||"?").trim();return esc(t.charAt(0).toUpperCase())}function filterMetaReportsClients(e,t,n=""){let a=e||[];t==="enabled"&&(a=a.filter(r=>r.metaReportEnabled)),t==="meta-only"&&(a=a.filter(r=>!clientUsesCenhubMetaReportTopline(r))),t==="needs-setup"&&(a=a.filter(r=>r.needsSetup));const o=String(n||"").trim().toLowerCase();return o&&(a=a.filter(r=>[r.accountName,r.metaAdAccountId,r.clientId,r.metaName].filter(Boolean).join(" ").toLowerCase().includes(o))),a}function getMetaReportsHubView(e,t,n=metaReportsState.searchQuery){const a=e?.clients||[],o=filterMetaReportsClients(a,t,n),r=e?.summary||{};return{clients:o,allClients:a,summary:{...r,totalListed:a.length},meta:e?.meta||{},filter:t,searchQuery:n}}function renderMetaHubSwitch({label:e,attr:t,value:n,checked:a,disabled:o=!1}){return n?`
