(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){if(t[1]==="sync-history"&&(t[2]==="ghl"||t[2]==="meta")){document.body.dataset.dashboardMode=`sync-history-${t[2]}`,delete document.body.dataset.clientSlug;return}if(t[1]==="fb-lead-sync"){document.body.dataset.dashboardMode="fb-lead-sync",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="custom-values"){document.body.dataset.dashboardMode="meta-reports-custom",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="ghl-clients"){document.body.dataset.dashboardMode="meta-reports-ghl-clients",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]){document.body.dataset.dashboardMode="meta-reports-client",document.body.dataset.clientSlug=t[2];return}if(t[1]==="meta-reports"){document.body.dataset.dashboardMode="meta-reports",delete document.body.dataset.clientSlug;return}document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}if(t[0]==="report"&&t[1]){document.body.dataset.dashboardMode="report",document.body.dataset.reportToken=t[1],delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_ADMIN_SYNC_HISTORY_GHL=DASHBOARD_MODE==="sync-history-ghl",IS_ADMIN_SYNC_HISTORY_META=DASHBOARD_MODE==="sync-history-meta",IS_ADMIN_FB_LEAD_SYNC=DASHBOARD_MODE==="fb-lead-sync",IS_ADMIN_META_REPORTS=DASHBOARD_MODE==="meta-reports",IS_ADMIN_META_REPORTS_CLIENT=DASHBOARD_MODE==="meta-reports-client",IS_ADMIN_META_REPORTS_CUSTOM=DASHBOARD_MODE==="meta-reports-custom",IS_ADMIN_META_REPORTS_GHL_CLIENTS=DASHBOARD_MODE==="meta-reports-ghl-clients",IS_REPORT_VIEW=DASHBOARD_MODE==="report",REPORT_TOKEN=document.body.dataset.reportToken||"",IS_ADMIN_SYNC_HISTORY=IS_ADMIN_SYNC_HISTORY_GHL||IS_ADMIN_SYNC_HISTORY_META,IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN_META_REPORTS_GHL_CLIENTS||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN_META_REPORTS_GHL_CLIENTS||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);return e[0]==="admin"&&e[1]==="meta-reports"&&e[2]&&e[2]!=="custom-values"&&e[2]!=="ghl-clients"?e[2]:e[0]==="admin"&&e[1]&&e[1]!=="meta-reports"?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"&&e[0]!=="report"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){const t=!!(e&&e.includes("staff-admin-chrome")),n=t||e&&e.includes("admin-topbar-link")?"/admin":"",a=n?`<a href="${n}" class="brand-logo-link" aria-label="Cenhub home"><img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" /></a>`:'<img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" />',o=t?`<button type="button" class="staff-nav-toggle" id="staff-nav-toggle" aria-expanded="false" aria-controls="staff-topbar-panel" aria-label="Open menu">
  ...
  `}function renderSyncHistoryRows(e,t){if(!e.length)return'<div class="sync-history-empty">No sync runs logged yet.</div>';const n=t==="meta"?"Spend / details":"Details",a=e.map(o=>{let s="\u2014";if(t==="meta"){const r=[];if(o.thisMonthSpend!=null&&r.push(`${fmtDkk(o.thisMonthSpend)} DKK this month`),o.spendDateStop){const i=formatSyncHistoryDate(o.spendDateStop);i&&r.push(`through ${i}`)}o.errorMessage&&r.push(o.errorMessage),s=r.join(" \xB7 ")||"\u2014"}else o.opportunityCount!=null?(s=`${o.opportunityCount} opportunities`,o.errorMessage&&(s+=` \xB7 ${o.errorMessage}`)):o.errorMessage&&(s=o.errorMessage);return`
      <tr>
...
${n}`))try{const o=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}`,{method:"DELETE"});showToast(`Cleared ${o.deleted||0} log entr${o.deleted===1?"y":"ies"}.`,"success"),await loadSyncHistoryPage(e)}catch(o){showToast(o.message||`Failed to clear ${t} sync log.`,"error")}}function renderSyncHistoryLoginPrompt(e){const t=e==="meta"?"Meta sync log":"GHL sync log";return`
    ${renderBrandTopbar("")}
    ${wrapDashboardShell(`
      <div class="page-hero admin-hub-hero">
        <h1>${esc(t)}</h1>
  ...
  `}async function loadSyncHistoryPage(e,{silent:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){t||(n.innerHTML=renderSyncHistoryLoginPrompt(e));return}currentStaffUser=a,t||(n.innerHTML=`
    ...
    `);try{const o=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}&limit=150`);n.innerHTML=renderSyncHistoryPage(e,o);const s=document.getElementById("sync-history-refresh");s&&(s.onclick=()=>loadSyncHistoryPage(e));const r=document.getElementById("sync-history-clear-log");r&&(r.onclick=()=>clearSyncHistoryLog(e)),syncHistoryRefreshTimer&&(clearInterval(syncHistoryRefreshTimer),syncHistoryRefreshTimer=null),syncHistoryRefreshTimer=window.setInterval(()=>{loadSyncHistoryPage(e,{silent:!0}).catch(()=>{})},6e4)}catch(o){n.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome(e==="meta"?"meta-sync":"ghl-sync"))}
...
${r} contact(s) will get Fb Lead id written in GHL. This writes live data.`)&&(fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:!0,previewRunId:n},await runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!1}))}async function refreshFbLeadClientRowsUi(){const e=document.getElementById("fb-lead-clients-mount"),t=document.getElementById("fb-lead-field-warnings-mount");e&&(e.innerHTML=renderFbLeadClientRows(fbLeadSyncState.clients)),t&&(t.innerHTML=renderFbLeadFieldWarnings(fbLeadSyncState.clients)),bindFbLeadSyncToggles()}function renderFbLeadClientActions(e){const t=esc(e.clientId),n=e.metaPageId?"":`<a class="admin-btn admin-btn--ghost admin-btn--small" href="/admin/${encodeURIComponent(e.clientId)}">Setup</a>`;return`
  ...
  `}function lastRunIsSuccessfulApply(e){if(!e||e.dryRun)return!1;const t=String(e.status||"").toLowerCase();return t==="success"||t==="ok"}function lastRunIsSuccessfulPreview(e){if(!e||!e.dryRun)return!1;const t=String(e.status||"").toLowerCase();return t==="success"||t==="ok"}function resolveFbLeadClientStats(e,t={}){const n=e?.displayStats||{},a=t.metaLeadCount90d??n.metaLeadCount90d??null,o=t.estimatedMissing??n.outstanding??null,s=t.estimatedMissing!=null?"Estimated contacts still missing Fb Lead id (from a Meta sample)":n.outstandingHint||"Estimated contacts still missing Fb Lead id (from a Meta sample)";return{metaLeads:a!=null?String(a):"\u2014",missing:o!=null?String(o):"\u2014",missingTitle:s}}function formatFbLeadLastRunLabel(e){if(!e)return"\u2014";const t=[e.dryRun?"preview":e.status,`${e.updated||0} ${e.dryRun?"would update":"updated"}`];return(e.skippedNoMatch??0)>0&&t.push(`${e.skippedNoMatch} no match`),t.join(" \xB7 ")}function renderFbLeadClientRows(e){return e.length?`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table fb-lead-clients-table">
        ...
        <tbody>${e.map(n=>{const a=fbLeadSyncState.preflightByClient[n.clientId]||{},o=resolveFbLeadClientStats(n,a),s=formatFbLeadLastRunLabel(n.lastRun);return`
      <tr>
        <td>
  ...
  `:'<div class="sync-history-empty">No clients configured yet.</div>'}function renderFbLeadClientRowsWrapper(e){return`<div id="fb-lead-clients-mount">${renderFbLeadClientRows(e)}</div>`}function renderFbLeadHistoryRows(e,t={}){const n=getFbLeadHistoryRunsForDisplay(e);if(!n.length){const o=fbLeadSyncState.showRoutineCronRuns?0:(e||[]).filter(s=>isRoutineSuccessfulCronRun(s)).length;return o>0?`<div class="sync-history-empty">${o} routine auto-sync run(s) hidden \u2014 enable <strong>Show routine auto-sync runs</strong> above to view them.</div>`:'<div class="sync-history-empty">No FB lead sync runs logged yet.</div>'}return e=n,`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table">
        <thead>
          <tr>
  ...
  `}function renderFbLeadAuditTable(e,t){if(!e.length)return'<div class="sync-history-empty" style="padding:12px">No contact-level rows stored for this run.</div>';const n=e.map(o=>{const s=ghlContactUrl(t,o.contactId),r=s?`<a href="${esc(s)}" target="_blank" rel="noopener noreferrer">Open in GHL</a>`:"\u2014";return`
      <tr>
  ...
  `}function closeFbLeadRunPanel(){const e=document.getElementById("fb-lead-run-panel-mount");e&&(e.innerHTML=""),fbLeadSyncState.activeRun=null}function openFbLeadRunPanel(e,t){const n=fbLeadSyncState.clients.find(s=>s.clientId===e);if(!clientFbLeadFieldReady(n)){showToast(getClientFbLeadFieldHint(n),"error");return}const a=document.getElementById("fb-lead-run-panel-mount");if(!a)return;fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:clientHasReadyPreview(fbLeadSyncState.clients.find(s=>s.clientId===e))},a.innerHTML=renderFbLeadRunPanel(e,t);const o=document.getElementById("fb-lead-preview-btn");o&&(o.onclick=()=>runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!0})),a.scrollIntoView({behavior:"smooth",block:"nearest"})}function renderFbLeadRunResults(e){const t=e.rows||[];if(!t.length)return'<div class="sync-history-empty" style="padding:12px">No rows in this batch.</div>';const n=t.map(s=>`
    <tr>
  ...
  `}function renderGlobalApplyProgress(e,t,n=0){const a=fbLeadSyncState.clients.find(r=>r.clientId===e),o=t==="backfill"?"90-day backfill":"recent sync",s=n>0?`${n} contact(s)`:"contacts";return`
    <div class="fb-lead-apply-progress-card" id="fb-lead-global-progress-card">
      <div class="fb-lead-apply-progress-header">
        <div>
          ...
          <div class="fb-lead-apply-progress-sub">${esc(a?.accountName||e)} \xB7 ${esc(s)}</div>
        </div>
...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(n?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),$=w.summary||w;l=$.runId,c=0,setFbLeadProgress({processed:0,total:$.inWindow||0,label:`Matching ${$.inWindow||0} Meta leads to GHL\u2026`})}else!n&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:b||f?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:v,batchOffset:c,batchLimit:r})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const $=w.inWindow||c||b;n?(setFbLeadProgress({processed:c,total:$,complete:!w.hasMore}),s&&(s.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:$,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const h=d.summary||d,m=h.inWindow||c||b,g=h.nextBatchOffset??c;if(n)setFbLeadProgress({processed:g,total:m,complete:!0}),markFbLeadPreviewReady(e,t,h.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(h.updated||0)>0),showToast((h.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:g,total:m,written:h.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const y=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=y.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(h){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${h.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((h.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&n&&hideFbLeadProgress(!0),!n&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(s=>s.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function metaReportFullUrl(e){return e?`${window.location.origin}${e}`:""}function normalizeMetaReportSlugInput(e){return String(e||"").trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function resolveMetaReportShareSlug(e){const t=e.settings||{};if(t.metaReportSlug)return t.metaReportSlug;const a=String(e.reportUrl||"").replace(/^\/report\//,"").match(/^(.+)-(\d{4})$/);return a?a[1]:""}function resolveMetaReportShareCode(e){const n=String(e.reportUrl||"").replace(/^\/report\//,"").match(/-(\d{4})$/);return n?n[1]:""}function renderMetaReportShareSummary(e,{hidden:t=!1}={}){const a=!!(e.settings||{}).metaReportEnabled,o=resolveMetaReportShareSlug(e),s=resolveMetaReportShareCode(e),r=o?`${o}-${s||"\xB7\xB7\xB7\xB7"}`:"Not set";return`
  ...
  `}function renderMetaReportClientControlPanel(e){const t=e.settings||{},n=metaReportsState.clientShareExpanded,a=metaReportsState.clientReportSettingsExpanded,o=!!e.reportUrl,s=n||a,r=metaReportMonthsNeedingBackfill(e),i=r.length?`${ICON_SYNC} Backfill ${r.length} month${r.length===1?"":"s"} from Meta`:`${ICON_SYNC} Re-sync year from Meta`;return`
  ...
  `).join("")}function syncMetaReportToplineSection(e){const t=document.getElementById("meta-report-topline-section");if(!t||!e)return;const n=e.settings||{};if(!isMetaReportGhlListClient(n))return;const a=metaReportsState.activeMonthKey,o=a?e.months?.[a]:null;t.outerHTML=renderMetaToplineModeSection({prefix:"meta-report",monthKey:a,monthPayload:o,settings:n,saving:metaReportsState.clientToplineSaving,uiPreference:metaReportsState.clientToplineUiMode}),bindMetaReportToplineEvents(e.clientId,e)}function bindMetaReportToplineEvents(e,t){bindMetaToplineSourceEvents({clientId:e,prefix:"meta-report",getMonthKey:()=>metaReportsState.activeMonthKey,getYear:()=>metaReportsState.selectedYear,getMonthRecord:n=>t?.months?.[n]||null,getUiPreference:()=>metaReportsState.clientToplineUiMode,setSaving:n=>{metaReportsState.clientToplineSaving=n,syncMetaReportToplineSection(t)},setUiPreference:n=>{metaReportsState.clientToplineUiMode=n},onMonthUpdated:async(n,a,o={})=>{if(o.yearSync){metaReportsState.clientToplineUiMode=null,await loadMetaReportsClientPage({silent:!0});return}n?.monthPayload&&a?(t.months=t.months||{},t.months[a]=n.monthPayload,metaReportsState.clientToplineUiMode=null,refreshMetaReportMonthPanel(n.monthPayload)):o.refreshUiOnly&&refreshMetaReportMonthPanel(t.months?.[metaReportsState.activeMonthKey])}})}function getMetaReportCalendarYear(e){const t=Number(e?.currentYear);return Number.isFinite(t)&&t>=2e3?t:new Date().getFullYear()}function getMetaReportYearOptions(e){const t=getMetaReportCalendarYear(e),n=t-1,a=new Map;Array.isArray(e?.years)&&e.years.forEach(s=>{const r=Number(s?.year);Number.isFinite(r)&&a.set(r,s.available!==!1)});const o=a.has(n)?a.get(n):e?.previousYearHasData===!0;return[{year:t,available:!0},{year:n,available:!!o}]}function renderMetaReportYearSelectOptions(e,{disableUnavailable:t=!1}={}){const n=Number(metaReportsState.selectedYear)||Number(e?.year);return getMetaReportYearOptions(e).map(({year:a,available:o})=>{const s=t&&!o;return`<option value="${a}"${Number(n)===a?" selected":""}${s?" disabled":""}>${a}${o?"":" (no data)"}</option>`}).join("")}function syncMetaReportSelectedYear(e,{disableUnavailable:t=!1}={}){const n=getMetaReportYearOptions(e),a=Number(metaReportsState.selectedYear),o=n.find(l=>l.year===a);if(o&&(!t||o.available)){metaReportsState.selectedYear=o.year;return}const s=Number(e?.year),r=n.find(l=>l.year===s);if(r&&(!t||r.available)){metaReportsState.selectedYear=r.year;return}const i=n.find(l=>l.available)||n[0];i&&(metaReportsState.selectedYear=i.year)}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;!t||!metaReportsState.activeMonthKey||(t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e,!document.getElementById("meta-report-month-panel"))||(syncMetaReportMonthPanelDom(e,getMetaReportMonthBodyOptions(t.settings,{editable:!0,yearPayload:t}),t),bindMetaReportsClientEditEvents(t.clientId,t),syncMetaReportToplineSection(t),refreshMetaReportBackfillControl(t))}function refreshMetaReportBackfillControl(e){const t=document.querySelector(".meta-report-client-page .meta-report-toolbar-left"),n=document.querySelector(".meta-report-backfill-wrap"),a=metaReportMonthsNeedingBackfill(e),o=a.length?`${ICON_SYNC} Backfill ${a.length} month${a.length===1?"":"s"} from Meta`:`${ICON_SYNC} Re-sync year from Meta`,s=document.getElementById("meta-report-backfill");if(s)s.innerHTML=o,s.disabled=!1;else if(t){const r=document.createElement("div");r.className="meta-report-backfill-wrap",r.innerHTML=`
      <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-report-backfill">${o}</button>
      <span class="meta-report-backfill-progress" id="meta-report-backfill-progress"></span>
    `,t.appendChild(r),bindMetaReportBackfillButton(e.clientId)}n&&!s&&!t&&n.remove()}function updateMetaReportsClientContent(e){metaReportsState.clientPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null),syncMetaReportSelectedYear(e,{disableUnavailable:!0});const n=document.getElementById("meta-report-year");n&&(n.innerHTML=renderMetaReportYearSelectOptions(e,{disableUnavailable:!0})),syncMetaReportToolbarCopy(e),refreshMetaReportClientShareUi(e);const a=e.settings||{},o=e.months?.[metaReportsState.activeMonthKey]||null;syncMetaReportSettingsControls(a,o);const s=document.querySelector(".meta-report-tabs");s&&(s.innerHTML=renderMetaReportsClientTabs(e),bindMetaReportsClientTabEvents(e)),syncMetaReportToplineSection(e),document.getElementById("meta-report-month-panel")&&(syncMetaReportMonthPanelDom(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(a,{editable:!0,yearPayload:e}),e),bindMetaReportsClientEditEvents(e.clientId,e)),refreshMetaReportBackfillControl(e),metaReportsState.chartSeries?.length&&ensureMetaReportScenarioSource(e,{editable:!0}).then(()=>{const i=buildMetaReportScenarioProjection(resolveMetaReportScenarioSeries(e),e);metaReportsState.chartProjection=i,syncMetaReportBaselineUi(resolveMetaReportScenarioSeries(e),i,resolveMetaReportScenarioPayload(e)),syncMetaReportScenario(e)})}function updatePublicMetaReportContent(e){metaReportsState.publicPayload=e,syncMetaReportPublicBranding(e);const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null),syncMetaReportSelectedYear(e,{disableUnavailable:!0});const n=document.getElementById("meta-report-year");n&&(n.innerHTML=renderMetaReportYearSelectOptions(e,{disableUnavailable:!0}));const a=document.querySelector(".meta-report-tabs");a&&(a.innerHTML=t.map(s=>`
  ...
  `}function renderMetaReportMetricTable(e,t,n="meta",{badge:a="",highlightLastN:o=0}={}){const s=t.map(([r,i,l],c)=>`
  ...
  `}function isMetaReportCensioClient(e){const t=String(e?.clientId||"").toLowerCase(),n=String(e?.accountName||"");return t==="censio"||/censio/i.test(n)}function monthHasChartData(e){if(!e||e.meta?.emptyMonth)return!1;if(!metaReportMonthPeriodAligned(e))return(Number(e.topline?.leads)||0)>0;const t=Number(e.meta?.spend)||0,n=Number(e.topline?.leads)||0;return t>0||n>0}function buildMetaReportChartSeries(e){const t=e?.monthKeys||[],n=e?.months||{},a=[];for(const o of t){const s=n[o];if(!s||s.meta?.emptyMonth||!monthHasChartData(s))continue;const r=Number(s.topline?.roasKr)||0,i=Number(s.topline?.roasX)||0,l=!!s.bottomline,c=l?Number(s.bottomline.poasKr)||0:null,d=l?Number(s.bottomline.poasX)||0:null;a.push({monthKey:o,label:metaMonthLabel(o),periodEnd:s.periodEnd||null,roasKr:r,roasX:i,poasKr:c,poasX:d,spend:Number(s.meta?.spend)||0,leads:Number(s.topline?.leads)||0,wonLeads:Number(s.topline?.wonLeads)||0,avgLeadValue:Number(s.topline?.avgLeadValue)||0,avgProfitPerWon:Number(s.inputs?.avgProfitPerWon)||0,hasData:!0})}return a}function getMetaReportDemoSeries(e,t,n,a){if(!isMetaReportCensioClient({clientId:e,accountName:t}))return null;const s=new Date,r=s.getFullYear(),i=s.getMonth()+1,l=Number(n)||r;let c=(a||[]).filter(d=>{const u=Number(String(d).slice(0,4)),p=Number(String(d).slice(5,7));return!(u!==l||l===r&&p>i)});if(!c.length){const d=l===r?Math.min(8,i):8;c=Array.from({length:d},(u,p)=>`${l}-${String(p+1).padStart(2,"0")}`)}return c.map((d,u)=>{const p=c.length>1?u/(c.length-1):0,v=Math.round(8e3+p*1e4),f=Math.round(2e3+p*15e3),b=Number((.5+p*2.5).toFixed(2)),h=Math.round(1500+p*12e3),m=Number((.4+p*2.2).toFixed(2)),g=Math.round(20+p*80),y=Math.max(1,Math.round(g*(.08+p*.04)));return{monthKey:d,label:metaMonthLabel(d),roasKr:f,roasX:b,poasKr:h,poasX:m,spend:v,leads:g,wonLeads:y,avgLeadValue:85e3,avgProfitPerWon:42e3,hasData:!0,demo:!0}})}function resolveMetaReportChartSeries(e){const t=buildMetaReportChartSeries(e);if(t.length>=META_REPORT_CHART_MIN_POINTS)return{series:t,demo:!1};if(isMetaReportCensioClient(e)){const n=getMetaReportDemoSeries(e.clientId,e.accountName,e.year||metaReportsState.selectedYear,e.monthKeys);if(n&&n.length>=META_REPORT_CHART_MIN_POINTS)return{series:n,demo:!0}}return{series:t,demo:!1}}function metaReportParseAmount(e){const t=Number.parseFloat(e);return Number.isFinite(t)?t:0}function metaReportRoundMoney(e){return Math.round(metaReportParseAmount(e)*100)/100}function metaReportRoundRatio(e){return Math.round(metaReportParseAmount(e)*1e8)/1e8}function metaReportAggregateSeriesEfficiency(e=[]){let t=0,n=0,a=0,o=0,s=0;for(const i of e){const l=metaReportParseAmount(i.spend),c=metaReportParseAmount(i.leads),d=metaReportParseAmount(i.wonLeads);l<=0&&c<=0||(t+=l,n+=c,a+=d,o+=d*metaReportParseAmount(i.avgLeadValue),s+=d*metaReportParseAmount(i.avgProfitPerWon))}const r=e.filter(i=>metaReportParseAmount(i.spend)>0).length;return{avgCpl:n>0?metaReportRoundMoney(t/n):0,winRate:n>0?metaReportRoundRatio(a/n):0,avgLeadValue:a>0?metaReportRoundMoney(o/a):0,avgProfitPerWon:a>0?metaReportRoundMoney(s/a):0,avgMonthlySpend:r>0?metaReportRoundMoney(t/r):0}}const META_SCENARIO_MIN_EFFICIENCY_MONTHS=2,META_SCENARIO_MIN_TREND_MONTHS=4,META_SCENARIO_MAD_MULTIPLIER=2.5,META_SCENARIO_MAX_MONTHLY_TREND=.03;function metaReportScenarioParseDate(e){if(!e)return null;const t=String(e).slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(t))return null;const n=new Date(`${t}T12:00:00.000Z`);return Number.isNaN(n.getTime())?null:n}function metaReportScenarioIsIncompleteMonth(e,t=new Date){if(!metaReportScenarioParseDate(e.periodEnd))return!0;const a=t.toISOString().slice(0,10);return String(e.periodEnd).slice(0,10)>=a}function metaReportScenarioIsAdActive(e){return metaReportParseAmount(e.spend)>0&&metaReportParseAmount(e.leads)>0}function metaReportScenarioMedian(e=[]){const t=e.filter(a=>Number.isFinite(a)).slice().sort((a,o)=>a-o);if(!t.length)return 0;const n=Math.floor(t.length/2);return t.length%2===0?(t[n-1]+t[n])/2:t[n]}function metaReportScenarioMad(e=[]){const t=metaReportScenarioMedian(e);return metaReportScenarioMedian(e.map(n=>Math.abs(n-t)))}function metaReportScenarioLinearRegression(e=[]){const t=e.length;if(t<2)return{slope:0,intercept:0,r:0};let n=0,a=0,o=0,s=0,r=0;for(const p of e){const v=metaReportParseAmount(p.x),f=metaReportParseAmount(p.y);n+=v,a+=f,o+=v*f,s+=v*v,r+=f*f}const i=t*s-n*n;if(i===0)return{slope:0,intercept:a/t,r:0};const l=(t*o-n*a)/i,c=(a-l*n)/t,d=Math.sqrt((t*s-n*n)*(t*r-a*a)),u=d>0?(t*o-n*a)/d:0;return{slope:l,intercept:c,r:u}}function metaReportScenarioEnrichMonth(e){const t=metaReportParseAmount(e.spend),n=metaReportParseAmount(e.leads),a=metaReportParseAmount(e.wonLeads),o=metaReportParseAmount(e.avgLeadValue),s=metaReportParseAmount(e.avgProfitPerWon),r=metaReportRoundMoney(a*o);return{...e,spend:t,leads:n,wonLeads:a,avgLeadValue:o,avgProfitPerWon:s,cpl:n>0?metaReportRoundMoney(t/n):0,totalLeadValue:r,efficiencyIndex:t>0?r/t:0}}function metaReportPrepareScenarioSeries(e=[],{windowMonths:t="6",asOfDate:n=new Date,smoothOutliers:a=!0}={}){const o=normalizeMetaReportScenarioMonthWindow(t)==="all"?1/0:Number.parseInt(normalizeMetaReportScenarioMonthWindow(t),10)||6,s=e.filter(metaReportScenarioIsAdActive).filter(d=>!metaReportScenarioIsIncompleteMonth(d,n)).map(metaReportScenarioEnrichMonth),r=s.length,i=o===1/0?s:s.slice(-o);let l=0,c=i;if(a&&i.length>=3){const d=i.map(h=>metaReportParseAmount(h.cpl)),u=i.map(h=>metaReportParseAmount(h.leads)),p=metaReportScenarioMedian(d),v=metaReportScenarioMedian(u),f=metaReportScenarioMad(d),b=metaReportScenarioMad(u);c=i.map(h=>{const m={...h};let g=!1;const y=metaReportParseAmount(h.leads);if(f>0){const w=p-META_SCENARIO_MAD_MULTIPLIER*f,$=p+META_SCENARIO_MAD_MULTIPLIER*f,S=Math.min($,Math.max(w,metaReportParseAmount(h.cpl)));S!==metaReportParseAmount(h.cpl)&&(m.cpl=metaReportRoundMoney(S),m.leads=Math.max(1,Math.round(metaReportParseAmount(h.spend)/S)),g=!0)}if(b>0){const w=v-META_SCENARIO_MAD_MULTIPLIER*b,$=v+META_SCENARIO_MAD_MULTIPLIER*b,S=Math.min($,Math.max(w,metaReportParseAmount(h.leads)));Math.round(S)!==Math.round(y)&&(m.leads=Math.max(1,Math.round(S)),g=!0)}if(g){l+=1;const w=y>0?metaReportParseAmount(h.wonLeads)/y:0,$=metaReportParseAmount(m.leads);m.wonLeads=Math.max(0,Math.round($*w)),m.totalLeadValue=metaReportRoundMoney(m.wonLeads*metaReportParseAmount(h.avgLeadValue));const S=metaReportParseAmount(m.spend);m.efficiencyIndex=S>0?m.totalLeadValue/S:0}return m})}return{months:c,monthsAvailable:r,monthsUsed:c.length,outliersAdjusted:l,confidence:metaReportResolveScenarioConfidence(c.length)}}function metaReportScenarioRecencyEfficiency(e=[]){if(e.length<META_SCENARIO_MIN_EFFICIENCY_MONTHS)return null;let t=0,n=0,a=0,o=0,s=0,r=0;return e.forEach((i,l)=>{const c=l+1;t+=metaReportParseAmount(i.spend)*c,n+=metaReportParseAmount(i.leads)*c,a+=metaReportParseAmount(i.wonLeads)*c,o+=metaReportParseAmount(i.wonLeads)*metaReportParseAmount(i.avgLeadValue)*c,s+=metaReportParseAmount(i.wonLeads)*metaReportParseAmount(i.avgProfitPerWon)*c,r+=metaReportParseAmount(i.spend)}),{avgCpl:n>0?metaReportRoundMoney(t/n):0,winRate:n>0?metaReportRoundRatio(a/n):0,avgLeadValue:a>0?metaReportRoundMoney(o/a):0,avgProfitPerWon:a>0?metaReportRoundMoney(s/a):0,avgMonthlySpend:e.length>0?metaReportRoundMoney(r/e.length):0}}function metaReportScenarioPooledEfficiency(e=[]){return metaReportAggregateSeriesEfficiency(e)}function metaReportScenarioBlendEfficiency(e,t,n=.5){if(!e||!t)return e||t;const a=1-n;return{avgCpl:metaReportRoundMoney(e.avgCpl*n+t.avgCpl*a),winRate:metaReportRoundRatio(e.winRate*n+t.winRate*a),avgLeadValue:metaReportRoundMoney(e.avgLeadValue*n+t.avgLeadValue*a),avgProfitPerWon:metaReportRoundMoney(e.avgProfitPerWon*n+t.avgProfitPerWon*a),avgMonthlySpend:metaReportRoundMoney(e.avgMonthlySpend*n+t.avgMonthlySpend*a)}}function metaReportScenarioMonthlyTrend(e=[],t=.75,{dampenHotStreak:n=!1}={}){if(e.length<META_SCENARIO_MIN_TREND_MONTHS)return{trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};const a=e.map(d=>metaReportParseAmount(d.spend)).filter(d=>d>0),o=metaReportScenarioMedian(a)||1,s=e.map((d,u)=>{const p=metaReportParseAmount(d.spend),v=metaReportParseAmount(d.efficiencyIndex),f=v>0?v:1e-6,b=p>0?p:o;return{x:u,y:Math.log(f)-t*Math.log(b/o)}}),{slope:r,r:i}=metaReportScenarioLinearRegression(s);let l=Math.max(-META_SCENARIO_MAX_MONTHLY_TREND,Math.min(META_SCENARIO_MAX_MONTHLY_TREND,r));if(n){const d=e.map(v=>metaReportParseAmount(v.efficiencyIndex)).filter(v=>v>0),u=metaReportScenarioMedian(d),p=e.slice(-3).reduce((v,f)=>v+metaReportParseAmount(f.efficiencyIndex),0)/Math.min(3,e.length);u>0&&p>u*1.15&&l>0&&(l*=.5)}let c="flat";return Math.abs(l)>=.005&&(c=l>0?"up":"down"),{trendRate:l,trendDirection:c,trendRatePct:metaReportRoundRatio(l*100),trendR:metaReportRoundRatio(i)}}function metaReportScenarioCalibrateElasticity(){return META_REPORT_SCENARIO_FIXED_ELASTICITY}function metaReportResolveScenarioConfidence(e=0){const t=Number.parseInt(e,10)||0;return t<4?"low":t<7?"medium":"high"}function metaReportComputeScenarioEfficiency(e=[],{blendHistory:t=!1,includeTrend:n=!1,cautionStrongMonths:a=!1}={}){const o=META_REPORT_SCENARIO_FIXED_ELASTICITY,s=metaReportScenarioCalibrateElasticity(),r=metaReportScenarioRecencyEfficiency(e),i=metaReportScenarioPooledEfficiency(e);if(!r||r.avgCpl<=0)return{efficiency:null,elasticity:s,presetElasticity:o,trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};const l=t?metaReportScenarioBlendEfficiency(i,r,.5):r;let c={trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};if(n){const d=metaReportScenarioMonthlyTrend(e,s,{dampenHotStreak:a});Math.abs(d.trendR)>=META_SCENARIO_TREND_R_THRESHOLD&&(c=d)}return{efficiency:l,elasticity:s,presetElasticity:o,...c}}function metaReportProjectBudgetAtSpend({baselineSpend:e,spend:t,efficiency:n,elasticity:a=.75,trendAdj:o=1,hasBottomline:s=!1}={}){const r=metaReportParseAmount(e),i=metaReportParseAmount(t);if(i<=0||r<=0||!n||n.avgCpl<=0)return{spend:0,leads:0,wonLeads:0,totalLeadValue:0,roasKr:0,roasX:0,poasKr:null,poasX:null};const l=r/n.avgCpl,c=i/r,d=metaReportParseAmount(o)>0?metaReportParseAmount(o):1,u=Math.max(0,Math.round(l*c**a*d)),p=Math.round(u*n.winRate),v=metaReportRoundMoney(p*n.avgLeadValue),f=metaReportRoundMoney(v-i),b=i>0?metaReportRoundRatio(f/i):0;let h=null,m=null;if(s&&n.avgProfitPerWon>0){const g=metaReportRoundMoney(p*n.avgProfitPerWon);h=metaReportRoundMoney(g-i),m=i>0?metaReportRoundRatio(h/i):0}return{spend:i,leads:u,wonLeads:p,totalLeadValue:v,roasKr:f,roasX:b,poasKr:h,poasX:m}}function metaReportSpendFromMonthKey(e=[],t=null,n=null){if(!n)return 0;const a=t?.months?.[n];if(a&&!a.meta?.emptyMonth&&metaReportMonthPeriodAligned(a)){const s=metaReportParseAmount(a.meta?.spend);if(s>0)return s}const o=e.find(s=>s.monthKey===n);return o?metaReportParseAmount(o.spend):0}function buildMetaReportScenarioProjection(e,t){const n=getMetaReportScenarioPillsFromState(),a=resolveMetaReportScenarioPayload(t)||t;return metaReportProjectBudgetScenario({series:e,multiplier:metaReportsState.budgetMultiplier,hasBottomline:payloadHasMetaReportBottomline(a),blendHistory:n.blendHistory,includeTrend:n.includeTrend,monthWindow:metaReportsState.scenarioMonthWindow,baselineMode:metaReportsState.budgetBaseline,activeMonthKey:resolveMetaReportScenarioActiveMonthKey(a),smoothOutliers:n.smoothUneven})}function isMetaReportViewingPastYear(e){const t=Number(e?.year||metaReportsState.selectedYear),n=getMetaReportCalendarYear(e);return Number.isFinite(t)&&t<n}function updateMetaReportScenarioSource(e){if(!e)return;const t=getMetaReportCalendarYear(e);if(Number(e.year)!==t)return;const{series:n}=resolveMetaReportChartSeries(e);n.length&&(metaReportsState.scenarioSourcePayload=e,metaReportsState.scenarioSeries=n)}function resolveMetaReportScenarioPayload(e){return isMetaReportViewingPastYear(e)&&metaReportsState.scenarioSourcePayload?metaReportsState.scenarioSourcePayload:e}function resolveMetaReportScenarioSeries(e){return isMetaReportViewingPastYear(e)?metaReportsState.scenarioSeries||[]:metaReportsState.chartSeries||[]}function resolveMetaReportScenarioActiveMonthKey(e){return isMetaReportViewingPastYear(e)?metaReportMonthKeyFromDate(new Date):metaReportsState.activeMonthKey}async function ensureMetaReportScenarioSource(e,{editable:t=!1}={}){if(!e||!isMetaReportViewingPastYear(e)||metaReportsState.scenarioSeries?.length)return;const n=getMetaReportCalendarYear(e),a=e.clientId;try{if(t&&a){const o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(a)}?year=${encodeURIComponent(n)}`);updateMetaReportScenarioSource(o);return}if(REPORT_TOKEN){const o=await fetch(`/api/meta-reports/public/${encodeURIComponent(REPORT_TOKEN)}?year=${encodeURIComponent(n)}`).then(async s=>{const r=await s.json();if(!s.ok)throw new Error(r.error||"Report not found");return r});updateMetaReportScenarioSource(o)}}catch{}}function resolveMetaReportLastMonthBaseline(e=[],{activeMonthKey:t=null,asOfDate:n=new Date,payload:a=null}={}){const o=e.filter(c=>metaReportParseAmount(c.spend)>0).sort((c,d)=>String(c.monthKey).localeCompare(String(d.monthKey)));if(!o.length)return null;const s=t||metaReportMonthKeyFromDate(n);if(!s)return null;const r=s?metaReportAddMonthsToMonthKey(s,-1):null;if(!r)return null;const i=o.find(c=>c.monthKey===r),l=metaReportSpendFromMonthKey(e,a,r);return!i&&l<=0?null:{monthKey:r,label:metaMonthLabel(r),spend:l>0?l:metaReportParseAmount(i.spend)}}function resolveMetaReportScenarioBaselineSpend(e,t,{baselineMode:n="year",activeMonthKey:a=null,asOfDate:o=new Date,payload:s=null}={}){const r=t?.months||[];if(n==="month"&&a){const i=metaReportSpendFromMonthKey(e,s,a);if(i>0)return i;const l=r.find(c=>c.monthKey===a);return l&&metaReportParseAmount(l.spend)>0?metaReportParseAmount(l.spend):0}if(n==="last"){const i=resolveMetaReportLastMonthBaseline(e,{activeMonthKey:a,asOfDate:o,payload:s});return i?.spend>0?i.spend:0}if(n==="year"&&r.length>0){const i=r.reduce((l,c)=>l+metaReportParseAmount(c.spend),0);return metaReportRoundMoney(i/r.length)}return 0}function metaReportProjectBudgetScenario({series:e=[],baselineSpend:t,multiplier:n=1,hasBottomline:a=!1,blendHistory:o=!1,includeTrend:s=!1,cautionStrongMonths:r=!1,monthWindow:i="6",baselineMode:l="year",activeMonthKey:c=null,asOfDate:d=new Date,smoothOutliers:u=!0}={}){const p=metaReportPrepareScenarioSeries(e,{windowMonths:i,asOfDate:d,smoothOutliers:u}),v=metaReportComputeScenarioEfficiency(p.months,{blendHistory:o,includeTrend:s,cautionStrongMonths:r});let f=metaReportParseAmount(t);f||(f=resolveMetaReportScenarioBaselineSpend(e,p,{baselineMode:l,activeMonthKey:c,asOfDate:d}));const b=metaReportParseAmount(n)||1,{efficiency:h,elasticity:m,trendRate:g}=v;if(!h||h.avgCpl<=0||f<=0||p.monthsUsed<META_SCENARIO_MIN_EFFICIENCY_MONTHS)return{...v,prepared:p,baseline:metaReportProjectBudgetAtSpend({baselineSpend:0,spend:0,efficiency:h}),projected:metaReportProjectBudgetAtSpend({baselineSpend:0,spend:0,efficiency:h}),projectedConservative:null,projectedOptimistic:null,multiplier:b,baselineSpend:f,hasBottomline:a,insufficientData:!0};const y=Math.exp(g*1),w=metaReportProjectBudgetAtSpend({baselineSpend:f,spend:f,efficiency:h,elasticity:m,trendAdj:1,hasBottomline:a}),$=metaReportProjectBudgetAtSpend({baselineSpend:f,spend:metaReportRoundMoney(f*b),efficiency:h,elasticity:m,trendAdj:y,hasBottomline:a}),S=metaReportProjectBudgetAtSpend({baselineSpend:f,spend:metaReportRoundMoney(f*b),efficiency:h,elasticity:m,trendAdj:Math.exp(Math.min(g,0)*1),hasBottomline:a}),I=metaReportProjectBudgetAtSpend({baselineSpend:f,spend:metaReportRoundMoney(f*b),efficiency:h,elasticity:m,trendAdj:Math.exp(Math.max(g,0)*1),hasBottomline:a});return{...v,prepared:p,baseline:w,projected:$,projectedConservative:S,projectedOptimistic:I,multiplier:b,baselineSpend:f,hasBottomline:a,insufficientData:!1}}function metaReportBuildScenarioProjectionMonthKeys(e=META_REPORT_SCENARIO_PROJECTED_MONTHS,t=new Date){const n=metaReportMonthKeyFromDate(t);if(!n||e<=0)return[];const a=[];for(let o=1;o<=e;o+=1)a.push(metaReportAddMonthsToMonthKey(n,o));return a}function buildMetaReportScenarioProjectionSteps(e,{months:t=META_REPORT_SCENARIO_PROJECTED_MONTHS,hasBottomline:n=!1,asOfDate:a=new Date,targetMultiplier:o=null}={}){if(!e||e.insufficientData)return[];const s=metaReportParseAmount(e.baselineSpend),r=metaReportParseAmount(o??e.multiplier)||1,i=e.efficiency,l=e.elasticity||.75,c=e.trendRate||0,d=metaReportBuildScenarioProjectionMonthKeys(t,a),u=[];for(let p=1;p<=t;p+=1){const v=r,f=metaReportRoundMoney(s*v),b=Math.exp(c*p),h=metaReportProjectBudgetAtSpend({baselineSpend:s,spend:f,efficiency:i,elasticity:l,trendAdj:b,hasBottomline:n}),m=d[p-1]||null;u.push({label:m?metaMonthLabel(m):`+${p} mo`,monthKey:m,spendMultiplier:metaReportRoundRatio(v),spend:h.spend,leads:h.leads,wonLeads:h.wonLeads,totalLeadValue:h.totalLeadValue,roasKr:h.roasKr,roasX:h.roasX,poasKr:h.poasKr,poasX:h.poasX})}return u}function metaReportSpendAxisTick(e){const t=Number(e)||0;return Math.abs(t)>=1e6?`${(t/1e6).toFixed(1)}M`:Math.abs(t)>=1e3?`${Math.round(t/1e3)}K`:t}function metaReportSpendMultiplierValue(e,t){const n=metaReportParseAmount(t),a=metaReportParseAmount(e);return n<=0?0:metaReportRoundRatio(a/n)}function resolveMetaReportSpendChartBaseline(e,t=null){const n=metaReportParseAmount(t?.baselineSpend);return n>0?n:metaReportAggregateSeriesEfficiency(e).avgMonthlySpend}function trimMetaReportScenarioSeries(e,t=META_REPORT_SCENARIO_MAX_HISTORICAL){const n=e.filter(a=>metaReportParseAmount(a.spend)>0);return n.length>t?n.slice(-t):n}function metaReportMonthKeyFromDate(e=new Date){const t=e instanceof Date?e:new Date(e);return Number.isNaN(t.getTime())?null:`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}`}function metaReportAddMonthsToMonthKey(e,t=0){const n=String(e||"").trim();if(!/^\d{4}-\d{2}$/.test(n))return null;const[a,o]=n.split("-").map(Number),s=new Date(a,o-1+t,1);return`${s.getFullYear()}-${String(s.getMonth()+1).padStart(2,"0")}`}function metaReportDeltaPct(e,t){const n=metaReportParseAmount(t),a=metaReportParseAmount(e);return n?metaReportRoundRatio((a-n)/Math.abs(n)*100):null}function metaReportResolveBaselineSpend(e,t,n=null){if(n?.baselineSpend>0)return metaReportParseAmount(n.baselineSpend);const a=getMetaReportScenarioPillsFromState(),o=n?.prepared||metaReportPrepareScenarioSeries(t,{windowMonths:metaReportsState.scenarioMonthWindow,smoothOutliers:a.smoothUneven});return resolveMetaReportScenarioBaselineSpend(t,o,{baselineMode:metaReportsState.budgetBaseline,activeMonthKey:resolveMetaReportScenarioActiveMonthKey(e)})}function resolveMetaReportScenarioChartSeries(e){const t=metaReportMonthKeyFromDate(new Date),n=e.filter(a=>!(metaReportParseAmount(a.spend)<=0||t&&a.monthKey&&a.monthKey>t));return trimMetaReportScenarioSeries(n)}function resolveMetaReportScenarioBaselineHints(e=[],t=null,n=null){const a=n||metaReportsState.clientPayload||null,o=getMetaReportScenarioPillsFromState(),s=t?.prepared||metaReportPrepareScenarioSeries(e,{windowMonths:metaReportsState.scenarioMonthWindow,smoothOutliers:o.smoothUneven}),r=resolveMetaReportScenarioActiveMonthKey(a),i=metaReportMonthKeyFromDate(new Date),l=getMetaReportCalendarYear(a)||a?.year||metaReportsState.selectedYear||(r?Number(String(r).slice(0,4)):null),c={activeMonthKey:r,asOfDate:new Date,payload:a},d=resolveMetaReportLastMonthBaseline(e,c),u=resolveMetaReportScenarioBaselineSpend(e,s,{baselineMode:"year",...c}),p=resolveMetaReportScenarioBaselineSpend(e,s,{baselineMode:"last",...c}),v=resolveMetaReportScenarioBaselineSpend(e,s,{baselineMode:"month",...c}),f=!!(i&&l&&String(i).startsWith(`${l}-`));return{windowAverage:u,windowMonthCount:s.monthsUsed||s.months.length||0,selectedMonthSpend:v,selectedMonthLabel:r?metaMonthLabel(r):"",activeKey:r,currentMonthSpend:f?metaReportSpendFromMonthKey(e,a,i):0,currentMonthLabel:f?metaMonthLabel(i):"",currentKey:f?i:null,lastMonthSpend:p,lastMonthLabel:d?.label||"",lastMonthKey:d?.monthKey||null}}function renderMetaReportBaselineSelectOptions(e=null,t="year"){const n=e?.windowAverage>0?metaReportFmtBaselineAmount(e.windowAverage):"\u2014",a=e?.lastMonthSpend>0?metaReportFmtBaselineAmount(e.lastMonthSpend):"\u2014",o=e?.selectedMonthSpend>0?metaReportFmtBaselineAmount(e.selectedMonthSpend):"\u2014",s=normalizeMetaReportBudgetBaseline(t),r=e?.lastMonthLabel?`Last month \xB7 ${e.lastMonthLabel}`:"Last month";return`
    <option value="year"${s==="year"?" selected":""}>Window average \xB7 ${esc(n)}</option>
    <option value="last"${s==="last"?" selected":""}>${esc(r)} \xB7 ${esc(a)}</option>
    <option value="month"${s==="month"?" selected":""}>Selected month \xB7 ${esc(o)}</option>
  `}function syncMetaReportBaselineUi(e=[],t=null,n=null){const a=resolveMetaReportScenarioBaselineHints(e,t,n),o=document.getElementById("meta-report-budget-baseline");if(o){const s=o.value||metaReportsState.budgetBaseline;o.innerHTML=renderMetaReportBaselineSelectOptions(a,s),o.value=normalizeMetaReportBudgetBaseline(s)}}function renderMetaReportScenarioContextStrip(e){if(!e||e.insufficientData)return'<p class="meta-report-scenario-context" id="meta-report-scenario-context">Add at least two completed ad months to model a budget scenario.</p>';const t=describeActiveMetaReportScenarioPills(),n=t.length?t.join(" \xB7 "):"Recent months only",a=e.trendDirection==="up"?`trend up ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:e.trendDirection==="down"?`trend down ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:"trend flat",o=e.prepared?.outliersAdjusted?` \xB7 ${e.prepared.outliersAdjusted} uneven month${e.prepared.outliersAdjusted===1?"":"s"} removed`:"",r={low:"Low confidence",medium:"Medium confidence",high:"High confidence"}[e.prepared?.confidence]||"",i=r?` \xB7 ${r} (${e.prepared?.monthsUsed||0} mo data)`:"";return`<p class="meta-report-scenario-context" id="meta-report-scenario-context"><strong>Active:</strong> ${esc(n)} \xB7 last ${e.prepared?.monthsUsed||0} ad months \xB7 ${esc(a)}${esc(o)}${esc(i)}</p>`}function payloadHasMetaReportBottomline(e){return e?.settings?.metaReportShowBottomline===!1?!1:seriesHasBottomline(metaReportsState.chartSeries||buildMetaReportChartSeries(e))}function seriesHasBottomline(e=[]){return e.some(t=>t.poasKr!=null&&t.poasX!=null)}function renderMetaReportChartsPanel({editable:e=!1}={}){const t=metaReportsState.chartTab||"trend";return`
  ...
  `}function applyMetaReportComparisonMonthChange(e,t,n,{editable:a=!1}={}){if(!n||!/^\d{4}-\d{2}$/.test(n))return;const o=t==="b"?"comparisonPeriodB":"comparisonPeriodA";metaReportsState[o]=metaReportComparisonFullMonthDates(n),metaReportsState.comparisonMode="months",document.querySelectorAll("[data-comparison-preset]").forEach(s=>{const r=s.getAttribute("data-comparison-preset")==="months";s.classList.toggle("active",r),s.setAttribute("aria-pressed",r)}),refreshMetaReportComparison(e,{editable:a})}function positionMetaReportComparisonMonthMenu(e,t,n){const a=e.getBoundingClientRect(),s=(n?.getAttribute("data-comparison-month-part")||"month")==="year"?120:176;t.style.left=`${a.left+a.width/2}px`,t.style.top=`${a.bottom+6}px`,t.style.width=`${Math.max(s,a.width+28)}px`,t.style.minWidth=`${s}px`}function openMetaReportComparisonMonthMenu(e,t,n){closeMetaReportComparisonMonthMenus(t),t.parentElement!==document.body&&(t._metaComparisonOwner=n,document.body.appendChild(t)),t.hidden=!1,n.classList.add("is-open"),e.setAttribute("aria-expanded","true"),positionMetaReportComparisonMonthMenu(e,t,n)}function closeMetaReportComparisonMonthMenus(e=null){document.querySelectorAll(".meta-report-comparison-month-menu").forEach(t=>{if(t===e)return;t.hidden=!0;const n=t._metaComparisonOwner||t.closest(".meta-report-comparison-month-dropdown"),a=n?.querySelector(".meta-report-comparison-month-trigger");n?.classList.remove("is-open"),a?.setAttribute("aria-expanded","false"),t._metaComparisonOwner&&t.parentElement===document.body&&t._metaComparisonOwner.appendChild(t)})}function bindMetaReportComparisonMonthInputs(e,{editable:t=!1}={}){document.querySelectorAll("[data-comparison-month-side]").forEach(n=>{const a=n.getAttribute("data-comparison-month-side");if(!a)return;const o=n.querySelector('[data-comparison-month-part="month"]'),s=n.querySelector('[data-comparison-month-part="year"]'),r=()=>{const l=o?.getAttribute("data-selected-value")||"",c=s?.getAttribute("data-selected-value")||"";!l||!c||applyMetaReportComparisonMonthChange(e,a,`${c}-${l}`,{editable:t})};n.querySelectorAll(".meta-report-comparison-month-dropdown").forEach(l=>{const c=l.querySelector(".meta-report-comparison-month-trigger"),d=l.querySelector(".meta-report-comparison-month-menu");!c||!d||(c.onclick=u=>{if(u.preventDefault(),u.stopPropagation(),!d.hidden){closeMetaReportComparisonMonthMenus();return}openMetaReportComparisonMonthMenu(c,d,l)},l.querySelectorAll(".meta-report-comparison-month-option").forEach(u=>{u.onclick=p=>{p.preventDefault(),p.stopPropagation();const v=u.getAttribute("data-value")||"";l.setAttribute("data-selected-value",v);const f=c.querySelector(".meta-report-comparison-month-trigger-value");f&&(f.textContent=u.textContent||v),l.querySelectorAll(".meta-report-comparison-month-option").forEach(b=>{const h=b===u;b.classList.toggle("is-selected",h),b.setAttribute("aria-selected",String(h))}),closeMetaReportComparisonMonthMenus(),r()}}))});const i=n.closest(".meta-report-comparison-duel-side");i&&(i.onclick=l=>{if(l.target.closest(".meta-report-comparison-month-dropdown"))return;const c=n.getBoundingClientRect();(l.clientX<c.left+c.width/2?o:s)?.querySelector(".meta-report-comparison-month-trigger")?.click()})}),metaReportsState.comparisonMonthMenuListenerBound||(document.addEventListener("click",n=>{n.target.closest(".meta-report-comparison-month-menu")||n.target.closest(".meta-report-comparison-month-trigger")||closeMetaReportComparisonMonthMenus()}),window.addEventListener("resize",()=>closeMetaReportComparisonMonthMenus()),window.addEventListener("scroll",n=>{const a=n.target;a instanceof Element&&a.closest(".meta-report-comparison-month-menu")||closeMetaReportComparisonMonthMenus()},!0),metaReportsState.comparisonMonthMenuListenerBound=!0)}function renderMetaReportComparisonMonthPickersPanel(e,t,n){const a=metaReportComparisonMonthInputBounds(n),o=e?.startMonthKey||metaReportsState.activeMonthKey||a.min?.slice(0,7)||"",s=t?.startMonthKey||metaReportComparisonAddMonths(o,-1)||a.min?.slice(0,7)||"";return`
    <div class="meta-report-comparison-duel is-month-pickers" id="meta-report-comparison-range-panel" role="group" aria-label="Compare months">
      ...
      ${renderMetaReportComparisonMonthPickerSide("b",s,n)}
    </div>
  `}function metaReportComparisonMonthKeyFromDate(e){const t=metaReportComparisonParseDate(e);return t?t.slice(0,7):""}const metaReportComparisonDatePickerState={inputKey:null,side:null,bound:null,anchorEl:null,viewYear:null,viewMonth:null,payload:null,editable:!1};function getMetaReportComparisonDatePickerFieldBounds(e,t,n){const a=metaReportComparisonDateInputBounds(e),s=metaReportsState[t==="b"?"comparisonPeriodB":"comparisonPeriodA"]||{};let r=a.min||void 0,i=a.max||void 0;return n==="start"&&s.endDate&&(i=s.endDate),n==="end"&&s.startDate&&(r=s.startDate),metaReportComparisonSafePickerBounds(r,i)}function renderMetaReportComparisonDateInput(e,t,n,a,o=null){const s=`${e}-${t}`,r=t==="start"?"From date":"To date",i=n?metaReportComparisonFormatDateLabel(n):"Select date";return`
    <button type="button" class="meta-report-comparison-date-trigger${n?" has-value":" is-empty"}" data-comparison-date-trigger="${esc(s)}" aria-label="${esc(r)}" aria-haspopup="dialog">
      <span class="meta-report-comparison-date-trigger-value">${esc(i)}</span>
    </button>
    <input type="hidden" data-comparison-date="${esc(s)}" value="${esc(n||"")}" />
  ...
  `}function renderMetaReportComparisonView(){const e=metaReportsState.clientPayload||metaReportsState.publicPayload,t=metaReportsState.comparisonResult||computeMetaReportComparison(e),{periodA:n,periodB:a}=metaReportComparisonPeriodsFromState(e),o=metaReportsState.comparisonTab||"table",s=metaReportsState.comparisonLoading;return`
  ...
  `}function metaReportFormatDeltaPct(e){return e==null||!Number.isFinite(e)?"":`${e>0?"+":""}${e.toFixed(0)}%`}function metaReportScenarioDeltaTone(e=""){return String(e).startsWith("+")?"positive":String(e).startsWith("-")?"negative":"neutral"}function renderMetaReportScenarioOutcomeCard({label:e,projectedValue:t,projectedSub:n="",baselineValue:a="",delta:o=""}={}){const s=metaReportScenarioDeltaTone(o);return`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:n=null,activeMonthKey:a=null,settings:o=null}={}){const s=a||e?.monthKey||null,r=s?metaReportMonthBounds(s):null,i=o||n?.settings||{},l=isMetaReportGhlListClient(i),c=resolveMetaToplineUiMode(e,metaReportsState.clientToplineUiMode),d=resolveMetaToplineSource(e),u=l&&c==="cenhub"&&d==="ghl",p=l&&c==="cenhub"&&d!=="ghl";let v="";if(t&&e&&!e.meta?.emptyMonth&&r){const A=u?" readonly":"",L=e.manualLeads??e.inputs?.manualLeads??"",k=l?renderMetaToplineModeSection({prefix:"meta-report",monthKey:s,monthPayload:e,settings:i,saving:metaReportsState.clientToplineSaving,uiPreference:metaReportsState.clientToplineUiMode}):"";v=`
    ...
    `}const f=a||e?.monthKey||null,b=t?f?`Report preview \xB7 ${metaMonthLabel(f)}`:"Report preview":f?`Report \xB7 ${metaMonthLabel(f)}`:"Report",h=metaReportsState.reportViewMode||"monthly",m=shouldShowMetaReportYearVisuals(e,n,{editable:t}),g=e&&!e.meta?.emptyMonth?`
  ...
  `:"",y=renderMetaReportMonthTables(e,{editable:t}),w=m?renderMetaReportChartsPanel({editable:t}):"",$=t&&m?renderMetaReportScenarioCard():"",S=m?renderMetaReportViewModeTabs():"";let I;return m?I=`
      ...
      ${$}
    `:I=`<div class="meta-report-content-main">${y}</div>`,`${v}${g}${S}${I}`}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
    ...
    `)}
  `}function renderMetaReportsClientPage(e){const t=e.monthKeys||[],n=metaReportsState.activeMonthKey||t[t.length-1]||"",a=e.months?.[n]||null,o=e.settings||{},s=t.map(r=>`
  ...
  `}function setMetaReportYearLoadingBanner(e,t,n){const a=document.querySelector(e);if(!a)return;let o=a.querySelector(".meta-report-year-loading-banner");n?(o||(o=document.createElement("span"),o.className="meta-report-year-loading-banner",o.setAttribute("aria-live","polite"),a.appendChild(o)),o.innerHTML=`<span class="meta-report-save-indicator-spinner"></span> Loading ${esc(t)}\u2026`,o.hidden=!1):o&&(o.hidden=!0)}function setMetaReportClientYearLoading(e,t){const n=document.querySelector(".meta-report-client-page"),a=document.getElementById("meta-report-year");if(t){a&&(a.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!0);const o=document.querySelector(".meta-report-tabs");o&&(o.innerHTML=renderMetaReportTabsSkeleton(),o.setAttribute("aria-busy","true"));const s=document.getElementById("meta-report-month-panel");s&&(s.innerHTML=renderMetaReportMonthPanelSkeleton(),s.classList.add("is-loading")),destroyMetaReportCharts()}else{a&&(a.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!1);const o=document.querySelector(".meta-report-tabs");o&&o.removeAttribute("aria-busy");const s=document.getElementById("meta-report-month-panel");s&&s.classList.remove("is-loading")}}function setMetaCvYearLoading(e,t){const n=document.querySelector(".sync-history-page.meta-reports-page"),a=document.getElementById("meta-cv-year"),o=document.getElementById("meta-cv-search");if(t){a&&(a.disabled=!0),o&&(o.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!0);const s=document.querySelector(".meta-cv-summary");s&&(s.classList.add("is-skeleton"),s.innerHTML=renderMetaCvSummarySkeleton());const r=document.querySelector(".meta-cv-legend");r&&r.setAttribute("aria-hidden","true");const i=document.getElementById("meta-cv-client-list");i&&(i.innerHTML=renderMetaCvListSkeleton(),i.setAttribute("aria-busy","true"));const l=document.getElementById("meta-cv-editor");l&&(l.innerHTML=renderMetaCvEditorYearSkeleton(),l.setAttribute("aria-busy","true"))}else{a&&(a.disabled=!1),o&&(o.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!1);const s=document.querySelector(".meta-cv-summary");s&&s.classList.remove("is-skeleton");const r=document.querySelector(".meta-cv-legend");r&&r.removeAttribute("aria-hidden");const i=document.getElementById("meta-cv-client-list");i&&i.removeAttribute("aria-busy");const l=document.getElementById("meta-cv-editor");l&&l.removeAttribute("aria-busy")}}function setMetaReportPublicYearLoading(e,t){const n=document.querySelector(".meta-reports-page--public"),a=document.getElementById("meta-report-year");if(t){a&&(a.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!0);const o=document.querySelector(".meta-reports-page--public .meta-report-tabs");o&&(o.innerHTML=renderMetaReportTabsSkeleton(),o.setAttribute("aria-busy","true"));const s=document.getElementById("meta-report-month-panel");s&&(s.innerHTML=renderMetaReportMonthPanelSkeleton(),s.classList.add("is-loading")),destroyMetaReportCharts()}else{a&&(a.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!1);const o=document.querySelector(".meta-reports-page--public .meta-report-tabs");o&&o.removeAttribute("aria-busy");const s=document.getElementById("meta-report-month-panel");s&&s.classList.remove("is-loading")}}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,n=document.getElementById("meta-report-year");n&&(n.onchange=async()=>{if(n.selectedOptions?.[0]?.disabled){n.value=String(metaReportsState.selectedYear);return}const o=metaReportsState.selectedYear,s=Number(n.value);if(s!==o){metaReportsState.selectedYear=s,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportClientYearLoading(s,!0);try{await loadMetaReportsClientPage({silent:!0})}catch(r){metaReportsState.selectedYear=o,n&&(n.value=String(o)),await loadMetaReportsClientPage({silent:!0}),showToast(r.message||"Load failed","error")}finally{setMetaReportClientYearLoading(s,!1)}}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report"),bindMetaReportShareEvents(e)}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e),isMetaReportGhlListClient(e.settings||{})&&bindMetaReportToplineEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const n=document.getElementById("meta-report-refresh-meta");n&&(n.onclick=async()=>{n.disabled=!0;try{const o=metaReportsState.activeMonthKey,s=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(s.monthPayload),showToast("Meta data refreshed","success")}catch(o){showToast(o.message||"Meta refresh failed","error")}finally{n.disabled=!1}});const a=document.getElementById("meta-report-save-month");a&&(a.onclick=async()=>{a.disabled=!0;try{const o=metaReportsState.activeMonthKey,s=t?.months?.[o],r=t?.settings||{},i=resolveMetaToplineUiMode(s,metaReportsState.clientToplineUiMode),l=isMetaReportGhlListClient(r)&&i==="cenhub"&&resolveMetaToplineSource(s)!=="ghl",c={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:document.getElementById("meta-report-won-leads")?.value===""?null:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:document.getElementById("meta-report-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-report-avg-profit")?.value===""?null:Number(document.getElementById("meta-report-avg-profit")?.value)};if(l){c.toplineSource="manual",c.manualOverride=!0;const u=document.getElementById("meta-report-manual-leads");u&&(c.manualLeads=u.value===""?null:Number(u.value))}const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}`,{method:"PUT",body:JSON.stringify(c)});refreshMetaReportMonthPanel(d.monthPayload),showToast("Month saved","success")}catch(o){showToast(o.message||"Save failed","error")}finally{a.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{switchMetaReportMonthTab(t.getAttribute("data-meta-month-tab"),{editable:!1})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}const a=metaReportsState.selectedYear,o=Number(t.value);if(o!==a){metaReportsState.selectedYear=o,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportPublicYearLoading(o,!0);try{await loadPublicMetaReportPage({silent:!0})}catch(s){metaReportsState.selectedYear=a,t&&(t.value=String(a)),await loadPublicMetaReportPage({silent:!0}),showToast(s.message||"Load failed","error")}finally{setMetaReportPublicYearLoading(o,!1)}}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
    ...
    `}).join("")}function isMetaReportGhlListClient(e={}){return!!e.metaReportGhlDataEnabled}function resolveMetaToplineSource(e){return e?.dataSource||e?.toplineSource||"meta"}function classifyMetaReportMonthInputStatus(e,t=!1){if(!e)return"empty";const n=e.topline?.wonLeads!=null||e.inputs?.wonLeads!=null,a=e.topline?.avgLeadValue!=null||e.inputs?.avgLeadValue!=null,o=e.inputs?.avgProfitPerWon!=null;return n&&a&&(!t||o)?"complete":n||a||o?"partial":"empty"}function resolveMetaToplineUiMode(e,t=null){return t==="cenhub"||t==="meta"?t:resolveMetaToplineSource(e)==="meta"?"meta":"cenhub"}function monthHasCenhubSnapshot(e){if(resolveMetaToplineSource(e)==="ghl")return!0;const n=e?.ghlTotals||{};return!!(e?.ghlSyncedAt||n.leads!=null||n.wonLeads!=null)}function renderMetaToplineModeCopy(e,t){const n=resolveMetaToplineSource(e),a=e?.ghlTotals||{},o=a.leads??e?.ghlLeads,s=a.wonLeads??e?.ghlWonLeads,r=a.totalRevenue??e?.ghlTotalRevenue,i=a.totalProfit??e?.ghlTotalProfit,l=e?.ghlSyncedAt?`Last synced ${metaCvFormatUpdatedAt(e.ghlSyncedAt)}.`:"";if(t==="meta")return{status:"Using Meta analytics for topline KPIs.",detail:"Ad spend still comes from Meta. Switch to Cenhub to use dashboard snapshot data."};if(n==="ghl"){const c=`Leads ${o??"\u2014"} \xB7 Won ${s??"\u2014"} \xB7 Revenue ${r??"\u2014"} \xB7 Profit ${i??"\u2014"}`;return{status:"Using Cenhub snapshot for this month.",detail:l?`${l} ${c}`:c}}return n==="manual"?{status:"Manual values for this month.",detail:"Enter won leads and averages below, then save."}:{status:"No Cenhub data for this month yet.",detail:"Sync from Cenhub or enter values manually below, then save."}}function renderMetaToplineModeSection({prefix:e,monthKey:t,monthPayload:n,settings:a={},saving:o=!1,uiPreference:s=null}={}){if(!isMetaReportGhlListClient(a))return"";const r=resolveMetaToplineUiMode(n,s),i=resolveMetaToplineSource(n),l=renderMetaToplineModeCopy(n,r),c=o?" disabled":"",d=a.hasGhl!==!1,u=r==="cenhub",p=r==="cenhub"&&i==="ghl";return`
    <div class="meta-topline-subsection" id="${esc(e)}-topline-section" data-meta-topline-prefix="${esc(e)}">
      ...
      <div class="meta-topline-mode-pills" role="group" aria-label="Topline source">
        <button type="button" class="meta-topline-mode-pill${r==="cenhub"?" is-active":""}" id="${esc(e)}-mode-cenhub" data-meta-topline-mode="cenhub"${c}>Cenhub</button>
        <button type="button" class="meta-topline-mode-pill${r==="meta"?" is-active":""}" id="${esc(e)}-mode-meta" data-meta-topline-mode="meta"${c}>Meta analytics</button>
      </div>
      <p class="meta-topline-mode-status">${esc(l.status)}</p>
      <p class="meta-topline-mode-detail">${esc(l.detail)}</p>
      ${p?`<button type="button" class="meta-topline-manual-link" id="${esc(e)}-use-manual"${c}>Enter manually anyway</button>`:""}
      ${u?`
  ...
  `}function bindMetaToplineSourceEvents({clientId:e,prefix:t,getMonthKey:n,getYear:a,getMonthRecord:o,setSaving:s,getUiPreference:r,setUiPreference:i,onMonthUpdated:l}){if(!e||!t)return;const c=document.getElementById(`${t}-mode-cenhub`);c&&(c.onclick=async()=>{if(c.disabled)return;const b=n?.(),h=o?.(b);resolveMetaToplineUiMode(h,r?.())!=="cenhub"&&(i&&i("cenhub"),l&&await l({},b,{refreshUiOnly:!0}))});const d=document.getElementById(`${t}-mode-meta`);d&&(d.onclick=async()=>{if(d.disabled)return;const b=n?.(),h=o?.(b);if(resolveMetaToplineUiMode(h,r?.())!=="meta"&&b){s&&s(!0);try{const g=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(b)}/use-meta`,{method:"POST",body:"{}"});i&&i(null),l&&await l(g,b),showToast("Using Meta analytics for this month","success")}catch(g){showToast(g.message||"Update failed","error")}finally{s&&s(!1)}}});const u=document.getElementById(`${t}-use-manual`);u&&(u.onclick=async()=>{if(u.disabled)return;const b=n?.();if(b){s&&s(!0);try{const h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(b)}/use-manual`,{method:"POST",body:"{}"});i&&i("cenhub"),l&&await l(h,b),showToast("Manual entry enabled for this month","success")}catch(h){showToast(h.message||"Could not switch to manual mode","error")}finally{s&&s(!1)}}});const p=document.getElementById(`${t}-sync-ghl`);p&&(p.onclick=async()=>{const b=n?.();if(!b)return;const h=o?.(b);let m=!1;if(h?.manualOverride&&(h?.toplineSource==="manual"||h?.dataSource==="manual")){if(!window.confirm("This month has manual values. Overwrite with Cenhub data?"))return;m=!0}s&&s(!0);try{const g=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(b)}/sync-ghl`,{method:"POST",body:JSON.stringify({overwriteManual:m})});i&&i("cenhub"),l&&await l(g,b),showToast("Synced from Cenhub snapshot","success")}catch(g){showToast(g.message||"Sync failed","error")}finally{s&&s(!1)}});const v=document.getElementById(`${t}-sync-ghl-year`);v&&(v.onclick=async()=>{s&&s(!0);try{const b=a?.()||metaReportsState.selectedYear,h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-preview?year=${encodeURIComponent(b)}`);let m=!0,g=!1;if((h.skipManual||[]).length){const w=window.prompt(`${h.skipManual.length} month(s) have manual values (${h.skipManual.join(", ")}).
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!w)return;w.trim().toLowerCase()==="overwrite"&&(m=!1,g=!0)}const y=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(b)}`,{method:"POST",body:JSON.stringify({skipManual:m,overwriteManual:g})});i&&i(null),l&&await l(y,null,{yearSync:!0}),showToast(`Synced ${y.synced?.length||0} month(s)${y.skipped?.length?`, skipped ${y.skipped.length}`:""}`,"success")}catch(b){showToast(b.message||"Year sync failed","error")}finally{s&&s(!1)}});const f=document.getElementById(`${t}-refresh-snapshot`);f&&(f.onclick=async()=>{const b=n?.();if(b){s&&s(!0);try{const h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(b)}/refresh-snapshot`,{method:"POST",body:"{}"});i&&i("cenhub"),l&&await l(h,b),showToast("Cenhub data refreshed and synced","success")}catch(h){showToast(h.message||"Refresh failed","error")}finally{s&&s(!1)}}})}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),n=e.monthKeys||[];return t.length?t.map(a=>`
    ...
    `).join(""):'<div class="meta-report-empty" style="padding:32px 12px">No matching clients.</div>'}function monthStatusIsEmpty(e,t={}){return e?(e.status??classifyMetaCvMonthStatus({wonLeads:e.inputs?.wonLeads??e.topline?.wonLeads??e.wonLeads,avgLeadValue:e.inputs?.avgLeadValue??e.topline?.avgLeadValue??e.avgLeadValue,avgProfitPerWon:e.inputs?.avgProfitPerWon??e.avgProfitPerWon},!!t.metaReportShowBottomline))==="empty":!0}function metaReportEffectiveFeeEnabled(e,t=null){if(!t)return!!e?.metaReportFeeEnabled;const n=!!e?.metaReportShowBottomline;return(t.status??classifyMetaCvMonthStatus({wonLeads:t.inputs?.wonLeads??t.topline?.wonLeads??t.wonLeads,avgLeadValue:t.inputs?.avgLeadValue??t.topline?.avgLeadValue??t.avgLeadValue,avgProfitPerWon:t.inputs?.avgProfitPerWon??t.avgProfitPerWon},n))==="empty"?!1:!!e?.metaReportFeeEnabled}function resolveMetaReportFeeMode(e={}){return e.metaReportFeeEnabled?resolveMetaReportStoredFeeMode(e):""}const META_REPORT_FEE_MODE_META=[{value:"",icon:"\u2013",title:"None",desc:"No Censio fee"},{value:"performance",icon:"%",title:"Performance fee",desc:"% of profit (POAS)"},{value:"marketing",icon:"Dkr",title:"Marketing fee",desc:"Fixed Dkr amount"}];function metaReportFeeValueBlockHtml(e,t,n,a,{inline:o=!1}={}){return o&&t==="performance"?`
      ...
      </label>`:'<p class="meta-report-fee-value-empty">No amount needed \u2014 just save your selection.</p>'}function metaReportUsesInlineFeeValue(e){return e==="meta-cv"}function describeMetaReportFeeModeLabel(e,t,n,{feeEnabled:a=!0}={}){if(!a||e===""||e==null)return"No Censio fee";if(e==="performance"){const o=Number(t);return`Performance fee \xB7 ${Number.isFinite(o)?o:20}%`}if(e==="marketing"){const o=Number(n);return`Marketing fee \xB7 Dkr ${Number.isFinite(o)?o:0}`}return"No Censio fee"}function readMetaReportFeeDraftState(e){const t=document.getElementById(`${e}-fee-nested`);if(!t)return null;const n=t.querySelector(`input[name="${e}-fee-mode"]:checked`),a=n?n.value||"":t.dataset.feeMode||"";let o=Number(t.dataset.feePercent),s=Number(t.dataset.marketingFee);if(a==="performance"){const u=document.getElementById(`${e}-setting-fee-percent`),p=Number(u?.value);Number.isFinite(p)&&(o=p)}else if(a==="marketing"){const u=document.getElementById(`${e}-setting-marketing-fee`),p=Number(u?.value);Number.isFinite(p)&&(s=p)}const r=t.dataset.feeEnabled==="true"?resolveMetaReportStoredFeeMode({metaReportFeeEnabled:!0,metaReportFeeMode:t.dataset.feeMode||null,metaReportMarketingFeeAmount:t.dataset.marketingFee}):"",i=Number(t.dataset.feePercent),l=Number(t.dataset.marketingFee);return{appliedMode:r,appliedPercent:i,appliedMarketing:l,selectedMode:a,selectedPercent:o,selectedMarketing:s,isDirty:a!==r||(a==="performance"?o!==i:a==="marketing"?s!==l:!1),appliedLabel:describeMetaReportFeeModeLabel(r,i,l),selectedLabel:describeMetaReportFeeModeLabel(a,o,s)}}function metaReportFeeStatusHtml(e){return e.isDirty?`
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t,t&&(metaReportsState.clientShareExpanded=!1);const n=document.querySelector(".meta-report-bottomline-fee-settings--client"),a=document.getElementById(`${e}-fee-editor`);n?.classList.toggle("is-editing",t),n?.classList.toggle("is-collapsed",!t),a&&a.classList.toggle("is-hidden",!t);const o=document.getElementById("meta-report-share-summary"),s=document.getElementById("meta-report-share-editor");o&&(o.hidden=metaReportsState.clientShareExpanded),s&&(s.hidden=!metaReportsState.clientShareExpanded),syncMetaReportControlPanelUi()}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const n=document.getElementById(`${t}-fee-summary`);if(!n)return;const a=document.createElement("div");a.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const o=a.firstElementChild;o&&n.replaceWith(o);const s=document.getElementById(`${t}-fee-edit`);s&&(s.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:n=!1,variant:a="default",collapsible:o=!1,externalSummary:s=!1}={}){const r=!!e.metaReportShowBottomline,i=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",c=resolveMetaReportFeeMode(e),d=e.metaReportFeePercent??20,u=e.metaReportMarketingFeeAmount??0,p=metaReportUsesInlineFeeValue(t),v=`data-fee-mode="${esc(l)}" data-fee-enabled="${i?"true":"false"}" data-fee-percent="${esc(d)}" data-marketing-fee="${esc(u)}"`;if(a==="cv"){const y=describeMetaReportFeeModeLabel(c,d,u),w=META_REPORT_FEE_MODE_META.map(({value:$,title:S})=>`
    ...
    <div class="meta-report-bottomline-fee-settings${o?" meta-report-bottomline-fee-settings--client":""}${o?h?" is-editing":" is-collapsed":""}">
      ...
      <div class="meta-report-bottomline-editor${o&&!h?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${r?"":" is-hidden"}" id="${t}-fee-nested" ${v}>
          ...
          <div class="meta-report-fee-mode-grid${n?" meta-report-fee-mode-grid--rows":""}">${b}
          </div>
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find(R=>R.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const o=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,o);const s=t.editorPayload?.months?.[a]||null,r=n.metaReportShowBottomline,i=!!n.metaReportGhlDataEnabled,l=o.toplineSource||s?.dataSource||"meta",c=resolveMetaToplineUiMode(s||{dataSource:l},t.toplineUiMode),d=i&&c==="cenhub"&&l==="ghl",u=i&&c==="cenhub"&&l!=="ghl",p=o.wonLeads!=null?o.wonLeads:s?.inputs?.wonLeads??s?.topline?.wonLeads??"",v=o.avgLeadValue!=null?o.avgLeadValue:s?.inputs?.avgLeadValue??s?.topline?.avgLeadValue??"",f=o.avgProfitPerWon!=null?o.avgProfitPerWon:s?.inputs?.avgProfitPerWon??"",b=o.ghlLeads??s?.ghlTotals?.leads??"",h=o.ghlTotalRevenue??s?.ghlTotals?.totalRevenue??"",m=o.ghlTotalProfit??s?.ghlTotals?.totalProfit??"",g=metaReportsState.customValues.draftInputs,y=g?.wonLeads??p,w=g?.avgLeadValue??v,$=g?.avgProfitPerWon??f,S=g?.manualLeads??o.manualLeads??"",I=d?" readonly":"",A=d?" disabled":"",L=t.settingsSaving&&t.settingsSavingScope==="report",k=i?renderMetaToplineModeSection({prefix:"meta-cv",monthKey:a,monthPayload:s||{dataSource:l,ghlTotals:{leads:b!==""?b:null,totalRevenue:h!==""?h:null,totalProfit:m!==""?m:null},ghlSyncedAt:o.ghlSyncedAt||null},settings:{metaReportGhlDataEnabled:!0,hasGhl:n.hasGhl!==!1},saving:t.saving,uiPreference:t.toplineUiMode}):"",M=u?`
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(R=>`
            <option value="${esc(R.clientId)}"${R.clientId===n.clientId?" selected":""}>${esc(R.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(R=>`
            <option value="${esc(R)}"${R===a?" selected":""}>${esc(metaMonthLabel(R))}</option>
          `).join("")}
        </select>
      </label>
      ${M}
  ...
  `).join("");return`
    ...
    `)}
  `}function syncMetaCvToplineSection(){const e=document.getElementById("meta-cv-topline-section");if(!e)return;const t=metaReportsState.customValues,n=t.overview?.clients?.find(d=>d.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return;const o=n.months?.[a]||{},s=t.editorPayload?.months?.[a]||null,r=o.toplineSource||s?.dataSource||"meta",i=o.ghlLeads??s?.ghlTotals?.leads??"",l=o.ghlTotalRevenue??s?.ghlTotals?.totalRevenue??"",c=o.ghlTotalProfit??s?.ghlTotals?.totalProfit??"";e.outerHTML=renderMetaToplineModeSection({prefix:"meta-cv",monthKey:a,monthPayload:s||{dataSource:r,ghlTotals:{leads:i!==""?i:null,totalRevenue:l!==""?l:null,totalProfit:c!==""?c:null},ghlSyncedAt:o.ghlSyncedAt||null},settings:{metaReportGhlDataEnabled:!0,hasGhl:n.hasGhl!==!1},saving:t.saving,uiPreference:t.toplineUiMode}),bindMetaCvToplineEvents()}function bindMetaCvToplineEvents(){const e=metaReportsState.customValues.selectedClientId;e&&bindMetaToplineSourceEvents({clientId:e,prefix:"meta-cv",getMonthKey:()=>metaReportsState.customValues.selectedMonthKey,getYear:()=>metaReportsState.selectedYear,getMonthRecord:t=>metaReportsState.customValues.overview?.clients?.find(n=>n.clientId===e)?.months?.[t]||null,getUiPreference:()=>metaReportsState.customValues.toplineUiMode,setSaving:t=>{metaReportsState.customValues.saving=t,syncMetaCvToplineSection(),setMetaCvSavePending(t)},setUiPreference:t=>{metaReportsState.customValues.toplineUiMode=t},onMonthUpdated:async(t,n,a={})=>{if(a.refreshUiOnly){refreshMetaCvListAndEditor();return}if(a.yearSync){metaReportsState.customValues.toplineUiMode=null,await refreshMetaCvOverviewSilently(),refreshMetaCvListAndEditor();return}n&&(applyMetaCvSaveResult(e,n,{},t),metaReportsState.customValues.toplineUiMode=null),await refreshMetaCvOverviewSilently(),refreshMetaCvListAndEditor()}})}function refreshMetaCvListAndEditor(){const e=metaReportsState.customValues.overview;if(!e)return;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e));const n=document.getElementById("meta-cv-editor");n&&(n.innerHTML=renderMetaCvEditor(e)),metaReportsState.customValues.draftInputs=null,bindMetaCvEditorEvents(),bindMetaCvListEvents()}function setMetaCvSavePending(e){metaReportsState.customValues.saving=e;const t=document.getElementById("meta-cv-save"),n=document.getElementById("meta-cv-save-indicator");n&&n.classList.toggle("is-visible",e),t&&(t.disabled=e||metaReportsState.customValues.loadingEditor||metaReportsState.customValues.settingsSaving,t.textContent=e?"Saving\u2026":"Save values")}function setMetaCvSettingsSaving(e,t="report"){metaReportsState.customValues.settingsSaving=e,metaReportsState.customValues.settingsSavingScope=e?t:null;const n=e&&t==="report";document.getElementById("meta-cv-settings-status")?.classList.toggle("is-visible",n),document.querySelector(".meta-cv-client-settings")?.classList.toggle("is-saving",n),["meta-cv-setting-bottomline"].forEach(s=>{const r=document.getElementById(s);r&&(r.disabled=n)}),document.querySelectorAll('input[name="meta-cv-fee-mode"]').forEach(s=>{s.disabled=n}),["meta-cv-setting-fee-percent","meta-cv-setting-marketing-fee"].forEach(s=>{const r=document.getElementById(s);r&&(r.disabled=n||r.disabled)});const o=document.getElementById("meta-cv-save");o&&(o.disabled=e||metaReportsState.customValues.saving||metaReportsState.customValues.loadingEditor)}function classifyMetaCvMonthStatus(e,t=!1){if(!e)return"empty";const n=e.wonLeads!=null,a=e.avgLeadValue!=null,o=e.avgProfitPerWon!=null;return n&&a&&(!t||o)?"complete":n||a||o?"partial":"empty"}function applyMetaCvSaveResult(e,t,n,a){const s=metaReportsState.customValues.overview?.clients?.find(l=>l.clientId===e),r=s?.months?.[t],i=a?.month||{};r&&(r.wonLeads=i.wonLeads??n.wonLeads,r.avgLeadValue=i.avgLeadValue??n.avgLeadValue,r.avgProfitPerWon=i.avgProfitPerWon??n.avgProfitPerWon,r.toplineSource=i.toplineSource??n.toplineSource??r.toplineSource,r.manualOverride=i.manualOverride??n.manualOverride??r.manualOverride,r.ghlLeads=i.ghlLeads??r.ghlLeads,r.ghlWonLeads=i.ghlWonLeads??r.ghlWonLeads,r.ghlTotalRevenue=i.ghlTotalRevenue??r.ghlTotalRevenue,r.ghlTotalProfit=i.ghlTotalProfit??r.ghlTotalProfit,r.ghlSyncedAt=i.ghlSyncedAt??r.ghlSyncedAt,r.manualLeads=i.manualLeads??n.manualLeads??r.manualLeads,r.status=classifyMetaCvMonthStatus(r,!!s.metaReportShowBottomline),r.updatedAt=r.status==="empty"?null:i.updatedAt||new Date().toISOString()),a?.monthPayload&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:a.monthPayload}})}async function refreshMetaCvOverviewSilently(){const e=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(e.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=e;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e),bindMetaCvListEvents());const n=document.querySelector(".meta-cv-summary");if(n&&e.summary){const a=e.summary;n.innerHTML=`
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(o){showToast(o.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",o=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",s=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",r=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `:`
    ...
    <div class="field-group">
      ...
      <select id="${e}">
        ...
        ${a.map(s=>`
          <option value="${esc(s.id)}" ${s.id===n?"selected":""}>${esc(s.name)}</option>
        `).join("")}
      </select>
  ...
  `}function getSetupProgressSteps(e){return[{id:"metrics",label:"Metrics",done:!!e.metricsModelSetAt},{id:"ghl",label:"GHL",done:!!(e.hasGhlToken&&e.locationId)},{id:"pipelines",label:"Pipelines",done:!!(e.newLeadsPipelineId&&e.salesPipelineId)},{id:"meta",label:"Meta",done:e.metaSyncStatus==="ok",partial:!!(e.metaAdAccountId&&e.metaSyncStatus!=="ok")}]}function renderSetupProgressStrip(e){const t=getSetupProgressSteps(e),n='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6L9 17l-5-5"/></svg>';return`
    <nav class="setup-progress" aria-label="Setup progress">
      ${t.map((a,o)=>{const s=a.done?" is-done":a.partial?" is-partial":"",r=a.done?`<span class="setup-progress-mark">${n}</span>`:`<span class="setup-progress-mark">${o+1}</span>`;return`
          <button type="button" class="setup-progress-step${s}" onclick="scrollToSetupSection('${a.id}')">
            ${r}
      ...
      <div class="checkbox-list">
        ${o.map(([s,r])=>`
          <label class="checkbox-item">
            <input type="checkbox"
              ${isVisible(t,s,a)?"checked":""}
              onchange="toggleDisplay('${t}', '${s}'${a?`, '${a}'`:""})" />
            <span>${r}</span>
          </label>
        `).join("")}
      </div>
  ...
  `:`
    ...
    `}function renderTable(e,t,n,a,o){const s=n.filter(r=>isVisible("columns",r.key,e));return s.length?`
  ...
  `}function renderDashboardContent(e){const{kpis:t,statusBreakdown:n,sourceReport:a,assigneeReport:o,pipelines:s,filters:r}=e,i=isVisible("sections","statusBreakdown");return`
