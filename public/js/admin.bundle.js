...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),y=w.summary||w;l=y.runId,c=0,setFbLeadProgress({processed:0,total:y.inWindow||0,label:`Matching ${y.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:b||p?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:o})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const y=w.inWindow||c||b;a?(setFbLeadProgress({processed:c,total:y,complete:!w.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:y,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);m=!0;const g=d.summary||d,f=g.inWindow||c||b,v=g.nextBatchOffset??c;if(a)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,g.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(g.updated||0)>0),showToast((g.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:g.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const $=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=$.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(g){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${g.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((g.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!m&&a&&hideFbLeadProgress(!0),!a&&!m&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}const META_REPORT_COMPARISON_PRESETS=[{id:"mom",label:"This month vs last month"},{id:"yoy_month",label:"Same month last year"},{id:"ytd",label:"This year vs last year"},{id:"custom",label:"Custom"}],META_REPORT_COMPARISON_TABLE_GROUPS=[{title:"Meta ads",accent:"meta",rows:[{label:"Total spend",metricId:"spend",format:"kr",higherIsBetter:null},{label:"Cost Pr Mile (CPM)",metricId:"cpm",format:"kr",higherIsBetter:!1},{label:"Impressions",metricId:"impressions",format:"num",higherIsBetter:!0},{label:"Reach",metricId:"reach",format:"num",higherIsBetter:!0},{label:"Click",metricId:"clicks",format:"num",higherIsBetter:!0},{label:"CTR",metricId:"conversionRatePercent",format:"pct",higherIsBetter:!0}]},{title:"Topline KPI'er",accent:"topline",highlightLastN:2,rows:[{label:"Leads",metricId:"leads",format:"num",higherIsBetter:!0},{label:"Cost Per Lead (CPL)",metricId:"cpl",format:"kr",higherIsBetter:!1},{label:"Won leads",metricId:"wonLeads",format:"num",higherIsBetter:!0},{label:"Total Lead Value",metricId:"totalLeadValue",format:"kr",higherIsBetter:!0},{label:"Average Lead Value",metricId:"avgLeadValue",format:"kr",higherIsBetter:!0},{label:"Client acquisition cost (CAC)",metricId:"cac",format:"kr",higherIsBetter:!1},{label:"Return on Ads Spend (ROAS)",metricId:"roasKr",format:"kr",higherIsBetter:!0},{label:"Return on Ads Spend % (ROAS)",metricId:"roasX",format:"x",higherIsBetter:!0}]},{title:"Bottomline KPI'er",accent:"bottomline",highlightLastN:2,bottomlineOnly:!0,rows:[{label:"Leads",metricId:"leads",format:"num",higherIsBetter:!0},{label:"Won leads",metricId:"wonLeads",format:"num",higherIsBetter:!0},{label:"Total Lead Value",metricId:"totalLeadValue",format:"kr",higherIsBetter:!0},{label:"Average Lead Value",metricId:"avgLeadValue",format:"kr",higherIsBetter:!0},{label:"Client acquisition cost (CAC)",metricId:"cac",format:"kr",higherIsBetter:!1},{label:"Avg Total Profit",metricId:"totalProfit",format:"kr",higherIsBetter:!0},{label:"Avg Single Profit Order",metricId:"avgProfitPerWon",format:"kr",higherIsBetter:!0},{label:"Profit on Ads Spend (POAS)",metricId:"poasKr",format:"kr",higherIsBetter:!0},{label:"Profit on Ads Spend % (POAS)",metricId:"poasX",format:"x",higherIsBetter:!0},{label:"Censio fee",metricId:"censioFee",format:"kr",higherIsBetter:!1},{label:"Profit on Investment (POI)",metricId:"poiKr",format:"kr",higherIsBetter:!0},{label:"Profit on Investment % (POI)",metricId:"poiX",format:"x",higherIsBetter:!0}]}];function metaReportComparisonAddMonths(e,t=0){const a=String(e||"").trim();if(!/^\d{4}-\d{2}$/.test(a))return null;const[n,s]=a.split("-").map(Number),r=new Date(n,s-1+t,1);return`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,"0")}`}function metaReportComparisonMonthKeysInRange(e,t){if(!e||!t)return[];const[a,n]=e<=t?[e,t]:[t,e],s=[];let r=a;for(;r&&r<=n&&(s.push(r),r!==n);)r=metaReportComparisonAddMonths(r,1);return s}function metaReportComparisonPeriodLabel(e,t){if(!e||!t)return"\u2014";if(e===t)return`${metaMonthLabel(e)} ${String(e).slice(0,4)}`;const a=String(e).slice(0,4),n=String(t).slice(0,4);return a===n?`${metaMonthLabel(e)}\u2013${metaMonthLabel(t)} ${a}`:`${metaMonthLabel(e)} ${a}\u2013${metaMonthLabel(t)} ${n}`}function metaReportMonthHasComparisonData(e){return!e||e.meta?.emptyMonth?!1:metaReportParseAmount(e.meta?.spend)>0||metaReportParseAmount(e.topline?.leads)>0}function metaReportAggregateComparisonRange(e={},t,a){const n=metaReportComparisonMonthKeysInRange(t,a),s=[];let r=!1,o=0,i=0,l=0,c=0,d=0,u=0,m=0,h=0,p=0,b=0,g=0,f=0,v=0;for(const R of n){const A=e[R];if(!metaReportMonthHasComparisonData(A))continue;s.push(R),A.bottomline&&(r=!0);const k=metaReportParseAmount(A.meta?.spend),D=metaReportParseAmount(A.meta?.impressions),N=metaReportParseAmount(A.topline?.leads),F=metaReportParseAmount(A.topline?.wonLeads);o+=k,i+=D,l+=metaReportParseAmount(A.meta?.reach),c+=metaReportParseAmount(A.meta?.clicks),d+=N,u+=F,m+=metaReportParseAmount(A.topline?.totalLeadValue),F>0&&(b+=metaReportParseAmount(A.topline?.avgLeadValue)*F,A.bottomline&&(g+=metaReportParseAmount(A.bottomline.avgProfitPerWon)*F)),D>0&&(f+=metaReportParseAmount(A.meta?.cpm)*D,v+=D),A.bottomline&&(h+=metaReportParseAmount(A.bottomline.totalProfit),p+=metaReportParseAmount(A.bottomline.censioFee))}const $=d>0?Math.round(o/d*100)/100:0,w=u>0?Math.round(o/u*100)/100:0,y=u>0?Math.round(b/u*100)/100:0,S=u>0?Math.round(g/u*100)/100:0,E=v>0?Math.round(f/v*100)/100:0,B=i>0?Math.round(c/i*100*1e8)/1e8:0,P=Math.round((m-o)*100)/100,I=o>0?Math.round(P/o*1e8)/1e8:0,M=Math.round((h-o)*100)/100,L=o>0?Math.round(M/o*1e8)/1e8:0,C=r?Math.round((M-p)*100)/100:0,T=o>0&&r?Math.round(C/o*1e8)/1e8:0;return{startKey:t,endKey:a,label:metaReportComparisonPeriodLabel(t,a),monthKeys:s,monthCount:s.length,expectedMonthCount:n.length,partialData:s.length>0&&s.length<n.length,hasBottomline:r,metrics:{spend:o,cpm:E,impressions:i,reach:l,clicks:c,conversionRatePercent:B,leads:d,cpl:$,wonLeads:u,totalLeadValue:m,avgLeadValue:y,cac:w,roasKr:P,roasX:I,totalProfit:h,avgProfitPerWon:S,poasKr:M,poasX:L,censioFee:p,poiKr:C,poiX:T}}}function metaReportResolveComparisonPresets({activeMonthKey:e=null,mode:t="mom"}={}){const a=e||null;if(t==="custom"){const n=metaReportsState.comparisonPeriodA?.startMonthKey&&metaReportsState.comparisonPeriodA?.endMonthKey?{...metaReportsState.comparisonPeriodA}:null,s=metaReportsState.comparisonPeriodB?.startMonthKey&&metaReportsState.comparisonPeriodB?.endMonthKey?{...metaReportsState.comparisonPeriodB}:null;return{mode:t,periodA:n,periodB:s}}if(!a)return{mode:t,periodA:null,periodB:null};if(t==="mom"){const n=metaReportComparisonAddMonths(a,-1);return{mode:t,periodA:{startMonthKey:a,endMonthKey:a},periodB:n?{startMonthKey:n,endMonthKey:n}:null}}if(t==="yoy_month"){const n=metaReportComparisonAddMonths(a,-12);return{mode:t,periodA:{startMonthKey:a,endMonthKey:a},periodB:n?{startMonthKey:n,endMonthKey:n}:null}}if(t==="ytd"){const n=String(a).slice(0,4),s=String(a).slice(5,7),r=String(Number(n)-1);return{mode:t,periodA:{startMonthKey:`${n}-01`,endMonthKey:a},periodB:{startMonthKey:`${r}-01`,endMonthKey:`${r}-${s}`}}}return{mode:t,periodA:null,periodB:null}}function metaReportComparisonMetricDefs(e=!1){const t=[{id:"spend",label:"Ad spend",group:"meta",format:"kr",higherIsBetter:null},{id:"cpm",label:"CPM",group:"meta",format:"kr",higherIsBetter:!1},{id:"impressions",label:"Impressions",group:"meta",format:"num",higherIsBetter:!0},{id:"reach",label:"Reach",group:"meta",format:"num",higherIsBetter:!0},{id:"clicks",label:"Clicks",group:"meta",format:"num",higherIsBetter:!0},{id:"conversionRatePercent",label:"CTR",group:"meta",format:"pct",higherIsBetter:!0},{id:"leads",label:"Leads",group:"topline",format:"num",higherIsBetter:!0},{id:"cpl",label:"CPL",group:"topline",format:"kr",higherIsBetter:!1},{id:"wonLeads",label:"Won leads",group:"topline",format:"num",higherIsBetter:!0},{id:"totalLeadValue",label:"Total lead value",group:"topline",format:"kr",higherIsBetter:!0},{id:"avgLeadValue",label:"Average lead value",group:"topline",format:"kr",higherIsBetter:!0},{id:"cac",label:"CAC",group:"topline",format:"kr",higherIsBetter:!1},{id:"roasKr",label:"ROAS",group:"topline",format:"kr",higherIsBetter:!0},{id:"roasX",label:"ROAS %",group:"topline",format:"x",higherIsBetter:!0}];return e&&t.push({id:"totalProfit",label:"Total profit",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"avgProfitPerWon",label:"Avg profit per won",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poasKr",label:"POAS",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poasX",label:"POAS %",group:"bottomline",format:"x",higherIsBetter:!0},{id:"poiKr",label:"POI",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poiX",label:"POI %",group:"bottomline",format:"x",higherIsBetter:!0}),t}function metaReportComparisonHeroIds(e=!1){return e?["spend","leads","wonLeads","roasKr","totalLeadValue","poasKr"]:["spend","leads","wonLeads","roasKr","totalLeadValue"]}function metaReportComparisonChartIds(e=!1,t="kr"){const a=["spend","leads","totalLeadValue"];return t==="x"?[...a,"roasX",...e?["poasX"]:[]]:[...a,"roasKr",...e?["poasKr"]:[]]}function metaReportBuildYtdByMonthComparison(e,t,a,n){if(!a||!n)return[];const s=metaReportComparisonMonthKeysInRange(a.startMonthKey,a.endMonthKey),r=String(n.startMonthKey||"").slice(0,4);return s.map(o=>{const i=String(o).slice(5,7),l=`${r}-${i}`,c=metaReportAggregateComparisonRange(e,o,o),d=metaReportAggregateComparisonRange(t,l,l);return{monthKey:o,label:metaMonthLabel(o),periodA:c.metrics,periodB:d.metrics,hasDataA:c.monthCount>0,hasDataB:d.monthCount>0}}).filter(o=>o.hasDataA||o.hasDataB)}function metaReportBuildComparison({monthsMapA:e={},monthsMapB:t={},periodA:a=null,periodB:n=null,mode:s="mom"}={}){if(!a?.startMonthKey||!a?.endMonthKey||!n?.startMonthKey||!n?.endMonthKey)return{mode:s,insufficientData:!0,samePeriod:!1,periodA:null,periodB:null,deltas:{},heroMetrics:[],detailRows:[],ytdByMonth:[],hasBottomline:!1};const r=a.startMonthKey===n.startMonthKey&&a.endMonthKey===n.endMonthKey,o=metaReportAggregateComparisonRange(e,a.startMonthKey,a.endMonthKey),i=metaReportAggregateComparisonRange(t,n.startMonthKey,n.endMonthKey),l=o.hasBottomline||i.hasBottomline,c=o.monthCount===0&&i.monthCount===0,d=metaReportComparisonMetricDefs(l),u={},m=d.map(b=>{const g=o.metrics[b.id],f=i.metrics[b.id],v=metaReportDeltaPct(g,f);return u[b.id]={pct:v,abs:Math.round((metaReportParseAmount(g)-metaReportParseAmount(f))*100)/100},{...b,valueA:g,valueB:f,deltaPct:v}}),h=metaReportComparisonHeroIds(l).map(b=>{const g=d.find(f=>f.id===b);return{id:b,label:g?.label||b,valueA:o.metrics[b],valueB:i.metrics[b],deltaPct:u[b]?.pct??null,format:g?.format||"num",higherIsBetter:g?.higherIsBetter??null}}),p=s==="ytd"?metaReportBuildYtdByMonthComparison(e,t,a,n):[];return{mode:s,insufficientData:c,samePeriod:r,hasBottomline:l,periodA:o,periodB:i,deltas:u,heroMetrics:h,detailRows:m,ytdByMonth:p}}function metaReportComparisonYearsNeeded(e,t){const a=new Set;for(const n of[e,t])n?.startMonthKey&&(a.add(String(n.startMonthKey).slice(0,4)),a.add(String(n.endMonthKey).slice(0,4)));return[...a].filter(Boolean)}function metaReportComparisonMonthsForYear(e,t){return!e||Number(e.year)===Number(t)?e?.months||{}:metaReportsState.comparisonYearCache?.[t]?.months||{}}function metaReportComparisonAvailableMonthKeys(e){const t=new Set,a=n=>{n&&Object.entries(n.months||{}).forEach(([s,r])=>{metaReportMonthHasComparisonData(r)&&t.add(s)})};return a(e),Object.values(metaReportsState.comparisonYearCache||{}).forEach(a),[...t].sort()}function metaReportComparisonPresetDisabled(e,t){const a=metaReportsState.activeMonthKey;if(!a)return!0;if(e==="mom")return!metaReportComparisonAddMonths(a,-1);if(e==="yoy_month"||e==="ytd"){const n=Number(String(a).slice(0,4))-1,s=(t?.years||[]).find(r=>Number(r.year)===n);return!!(s&&s.available===!1||t?.previousYearHasData===!1)}return!1}function metaReportComparisonFmtValue(e,t="num"){return t==="kr"?metaFmtKr(e):t==="x"?metaFmtX(e):t==="pct"?`${metaFmtNum(e,2)}%`:metaFmtNum(e)}function metaReportComparisonDeltaTone(e,t=null){return e==null||!Number.isFinite(e)||e===0?"neutral":t===!1?e<0?"positive":"negative":e>0?"positive":"negative"}function metaReportComparisonPeriodsFromState(e){const t=metaReportResolveComparisonPresets({activeMonthKey:metaReportsState.activeMonthKey,mode:metaReportsState.comparisonMode});return metaReportsState.comparisonMode!=="custom"&&(metaReportsState.comparisonPeriodA=t.periodA,metaReportsState.comparisonPeriodB=t.periodB),t}function metaReportComparisonMaps(e,t,a){const n=metaReportComparisonYearsNeeded(t,a),s={};n.forEach(o=>{s[o]=metaReportComparisonMonthsForYear(e,o)});const r=o=>{const i=String(o?.startMonthKey||"").slice(0,4);return s[i]||e?.months||{}};return{monthsMapA:r(t),monthsMapB:r(a)}}async function ensureMetaReportComparisonYears(e,{editable:t=!1}={}){const{periodA:a,periodB:n}=metaReportComparisonPeriodsFromState(e),s=metaReportComparisonYearsNeeded(a,n),r=String(e?.year||metaReportsState.selectedYear),o=s.filter(i=>i!==r&&!metaReportsState.comparisonYearCache[i]);if(o.length){metaReportsState.comparisonLoading=!0,syncMetaReportComparisonUi(e,{editable:t});for(const i of o)try{let l;t&&e?.clientId?l=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e.clientId)}?year=${encodeURIComponent(i)}`):REPORT_TOKEN&&(l=await fetch(`/api/meta-reports/public/${encodeURIComponent(REPORT_TOKEN)}?year=${encodeURIComponent(i)}`).then(async c=>{const d=await c.json();if(!c.ok)throw new Error(d.error||"Report not found");return d})),l&&(metaReportsState.comparisonYearCache[i]=l)}catch{}metaReportsState.comparisonLoading=!1}}function computeMetaReportComparison(e){const{periodA:t,periodB:a,mode:n}=metaReportComparisonPeriodsFromState(e),{monthsMapA:s,monthsMapB:r}=metaReportComparisonMaps(e,t,a);return metaReportBuildComparison({monthsMapA:s,monthsMapB:r,periodA:t,periodB:a,mode:n})}function renderMetaReportComparisonRangeOptions(e,t){return t.map(a=>`
    <option value="${esc(a)}"${a===e?" selected":""}>${esc(metaMonthLabel(a))} ${esc(String(a).slice(0,4))}</option>
  `).join("")}function renderMetaReportComparisonRangeGroup(e,t,a,n,s){const r=a?.startMonthKey||n[0]||"",o=a?.endMonthKey||a?.startMonthKey||n[n.length-1]||"";return`
    <div class="meta-report-comparison-range-group is-period-${s}">
      <div class="meta-report-comparison-range-label">
        <span class="meta-report-comparison-badge ${t}">${s==="a"?"A":"B"}</span>
        ${esc(e)}
      </div>
      <div class="meta-report-comparison-range-fields">
        <select class="admin-select" data-comparison-range="${s}-start">${renderMetaReportComparisonRangeOptions(r,n)}</select>
        <span class="meta-report-comparison-range-sep">to</span>
        <select class="admin-select" data-comparison-range="${s}-end">${renderMetaReportComparisonRangeOptions(o,n)}</select>
      </div>
    </div>
  `}function renderMetaReportComparisonChangeCell(e,t){const a=metaReportComparisonDeltaTone(e,t);if(e==null||!Number.isFinite(e))return'<span class="meta-report-comparison-change is-neutral">\u2014</span>';const n=e>0?"\u2191":e<0?"\u2193":"\u2014",s=metaReportFormatDeltaPct(e);return`<span class="meta-report-comparison-change is-${a}">${esc(n)} ${esc(s)}</span>`}function renderMetaReportComparisonGroupTable(e,t,a,n,{highlightLastN:s=0}={}){if(!n||n.insufficientData||n.samePeriod)return"";const r=n.periodA.label,o=n.periodB.label,i=t.map((l,c)=>{const d=n.periodA.metrics[l.metricId],u=n.periodB.metrics[l.metricId],m=metaReportDeltaPct(d,u);return`
      <tr class="${s>0&&c>=t.length-s?`is-highlight accent-${a}`:""}">
        <th scope="row">${esc(l.label)}</th>
        <td class="meta-report-comparison-col-a">${esc(metaReportComparisonFmtValue(d,l.format))}</td>
        <td class="meta-report-comparison-col-change">${renderMetaReportComparisonChangeCell(m,l.higherIsBetter)}</td>
        <td class="meta-report-comparison-col-b">${esc(metaReportComparisonFmtValue(u,l.format))}</td>
      </tr>
    `}).join("");return`
    <section class="meta-report-group">
      <div class="meta-report-group-head">
  ...
  `}function renderMetaReportComparisonTables(e){return metaReportsState.comparisonLoading?'<p class="meta-report-comparison-loading">Loading comparison data\u2026</p>':!e||e.insufficientData?'<p class="meta-report-comparison-empty">Select two periods with ad data to compare performance.</p>':e.samePeriod?'<p class="meta-report-comparison-warning">Both periods are the same. Choose different ranges to compare.</p>':`<div class="meta-report-groups-stack">${META_REPORT_COMPARISON_TABLE_GROUPS.filter(a=>!a.bottomlineOnly||e.hasBottomline).map(a=>renderMetaReportComparisonGroupTable(a.title,a.rows,a.accent,e,{highlightLastN:a.highlightLastN||0})).join("")}</div>`}function renderMetaReportComparisonHeadline(e){if(metaReportsState.comparisonLoading)return'<p class="meta-report-comparison-loading">Loading comparison data\u2026</p>';if(!e||e.insufficientData)return'<p class="meta-report-comparison-empty">Select two periods with ad data to compare.</p>';if(e.samePeriod)return'<p class="meta-report-comparison-warning">Both periods are the same. Choose different ranges to compare.</p>';const t=e.periodA?.partialData||e.periodB?.partialData?'<span class="meta-report-comparison-partial-badge">Partial data</span>':"";return`
    <div class="meta-report-comparison-headline" id="meta-report-comparison-headline">
      <span class="meta-report-comparison-headline-a">${esc(e.periodA.label)}</span>
      <span class="meta-report-comparison-headline-vs">vs</span>
      <span class="meta-report-comparison-headline-b">${esc(e.periodB.label)}</span>
      ${t}
    </div>
  `}function renderMetaReportComparisonCard(){const e=metaReportsState.clientPayload||metaReportsState.publicPayload,t=metaReportsState.comparisonResult||computeMetaReportComparison(e),a=metaReportComparisonAvailableMonthKeys(e),{periodA:n,periodB:s}=metaReportComparisonPeriodsFromState(e),r=metaReportsState.comparisonTab||"table",o=metaReportsState.comparisonMode==="custom";return`
    <section class="meta-report-comparison-card" id="meta-report-comparison-card">
      ...
      <div class="meta-report-comparison-presets" id="meta-report-comparison-presets">
        ${META_REPORT_COMPARISON_PRESETS.map(({id:i,label:l})=>{const c=metaReportComparisonPresetDisabled(i,e),d=c&&i==="yoy_month"||c&&i==="ytd"?"Prior year data is not available on this report":c&&i==="mom"?"No previous month available":"";return`
            <button type="button" class="preset-btn${metaReportsState.comparisonMode===i?" active":""}${c?" is-disabled":""}" data-comparison-preset="${esc(i)}" aria-pressed="${metaReportsState.comparisonMode===i}"${c?" disabled":""}${d?` title="${esc(d)}"`:""}>${esc(l)}</button>
          `}).join("")}
      </div>
      ${o?`
        <div class="meta-report-comparison-range-row" id="meta-report-comparison-range-row">
          ${renderMetaReportComparisonRangeGroup("Period A","is-a",n,a,"a")}
          ${renderMetaReportComparisonRangeGroup("Period B","is-b",s,a,"b")}
        </div>
      `:""}
      <div id="meta-report-comparison-headline-wrap">${renderMetaReportComparisonHeadline(t)}</div>
  ...
  `:"",d=renderMetaReportMonthTables(e,{editable:t}),u=shouldShowMetaReportYearVisuals(e,a,{editable:t}),m=u?renderMetaReportChartsPanel({editable:t}):"",h=u?renderMetaReportComparisonCard():"",p=t&&u?renderMetaReportScenarioCard():"",b=u?`
  ...
  `:`
    <div class="meta-report-content-main">${d}</div>
  `;return`${o}${c}${b}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
      ...
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${o?"":" is-hidden"}" id="${t}-fee-nested" ${h}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
            <span class="meta-report-fee-status-value is-active">${esc($)}</span>
          </div>
  ...
  `}const p=describeMetaReportFeeModeLabel(c,d,u),b=META_REPORT_FEE_MODE_META.map(({value:$,icon:w,title:y,desc:S})=>`
        ...
        </label>`).join(""),g=s?metaReportsState.clientReportSettingsExpanded:!0,f=s?`
    ...
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?g?" is-editing":" is-collapsed":""}">
      ${v}
      <div class="meta-report-bottomline-editor${s&&!g?" is-hidden":""}" id="${t}-fee-editor">
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(b=>`
            <option value="${esc(b.clientId)}"${b.clientId===a.clientId?" selected":""}>${esc(b.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(b=>`
            <option value="${esc(b)}"${b===n?" selected":""}>${esc(metaMonthLabel(b))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
      ...
      <label>Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(m)}" />
      </label>
      <label>Avg profit per won${o?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(h)}" />
      </label>
    </div>
    <div class="meta-report-settings-group meta-report-settings-group--stacked meta-cv-client-settings${p?" is-saving":""}">
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(a){showToast(a.message,"error")}finally{if(t){t.disabled=!1;const a=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${a}`:a}}}function getMetricsModelLabels(e,t=setupPipelines){const a=new Map((t||[]).map(n=>[n.id,n.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),a=!e.metricsModelSetAt||metricsModelChangeMode,n=a?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",r=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return a?`
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:a=!1,forceFresh:n=!1}=t,s=!!n,r=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),i=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const c=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)r.innerHTML=`
        ...
        `)}`;else if(!a){const d=document.getElementById("dashboard-content");d&&(d.innerHTML=renderKpiSkeleton()),i&&(i.disabled=!0,i.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let d=null;if(!availablePipelines.length){if(d=await bootstrapDashboardData(c),l!==fetchGeneration)return;availablePipelines=d.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,d.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const m=canReuseBootstrapDashboard(d)&&!s;let h,p;if(m?[p,h]=await Promise.all([fetchFacebookMetrics(c),Promise.resolve(d)]):[h,p]=await Promise.all([fetchDashboardData(availablePipelines,c,{forceFresh:s}),fetchFacebookMetrics(c)]),l!==fetchGeneration)return;cachedFacebookMetrics=p,cachedData=applyMarketingData(h,p),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(d){if(l!==fetchGeneration)return;if(d.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):r.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
