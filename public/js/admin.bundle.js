...
Resume the interrupted ${$}?`)&&(l=g.id,d=Number(g.batchOffset)||0)}}if(a?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&d===0){const g=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),m=g.summary||g;l=m.runId,d=0,setFbLeadProgress({processed:0,total:m.inWindow||0,label:`Matching ${m.inWindow||0} Meta leads to GHL\u2026`})}else!a&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:y||h?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:k,batchOffset:d,batchLimit:r})});const g=c.summary||c;l=g.runId,d=g.nextBatchOffset??g.batchOffset+(g.batchProcessed||0),u=u.concat(g.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const m=g.inWindow||d||y;a?(setFbLeadProgress({processed:d,total:m,complete:!g.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...g,rows:u}))):updateGlobalApplyProgress({processed:d,total:m,written:g.updated||0,complete:!g.hasMore})}while((c.summary||c).hasMore);f=!0;const C=c.summary||c,p=C.inWindow||d||y,b=C.nextBatchOffset??d;if(a)setFbLeadProgress({processed:b,total:p,complete:!0}),markFbLeadPreviewReady(e,t,C.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(C.updated||0)>0),showToast((C.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:b,total:p,written:C.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const g=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete g[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(g))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(C){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${C.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((C.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!f&&a&&hideFbLeadProgress(!0),!a&&!f&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function isMetaReportCensioClient(e){const t=String(e?.clientId||"").toLowerCase(),a=String(e?.accountName||"");return t==="censio"||/censio/i.test(a)}function monthHasChartData(e){if(!e||e.meta?.emptyMonth)return!1;const t=Number(e.meta?.spend)||0,a=Number(e.topline?.leads)||0;return t>0||a>0}function buildMetaReportChartSeries(e){const t=e?.monthKeys||[],a=e?.months||{},n=[];for(const s of t){const o=a[s];if(!o||o.meta?.emptyMonth||!monthHasChartData(o))continue;const r=Number(o.topline?.roasKr)||0,i=Number(o.topline?.roasX)||0,l=!!o.bottomline,d=l?Number(o.bottomline.poasKr)||0:null,c=l?Number(o.bottomline.poasX)||0:null;n.push({monthKey:s,label:metaMonthLabel(s),roasKr:r,roasX:i,poasKr:d,poasX:c,spend:Number(o.meta?.spend)||0,leads:Number(o.topline?.leads)||0,wonLeads:Number(o.topline?.wonLeads)||0,avgLeadValue:Number(o.topline?.avgLeadValue)||0,avgProfitPerWon:Number(o.inputs?.avgProfitPerWon)||0,hasData:!0})}return n}function getMetaReportDemoSeries(e,t,a,n){if(!isMetaReportCensioClient({clientId:e,accountName:t}))return null;const o=new Date,r=o.getFullYear(),i=o.getMonth()+1,l=Number(a)||r;let d=(n||[]).filter(c=>{const u=Number(String(c).slice(0,4)),f=Number(String(c).slice(5,7));return!(u!==l||l===r&&f>i)});if(!d.length){const c=l===r?Math.min(8,i):8;d=Array.from({length:c},(u,f)=>`${l}-${String(f+1).padStart(2,"0")}`)}return d.map((c,u)=>{const f=d.length>1?u/(d.length-1):0,k=Math.round(8e3+f*1e4),h=Math.round(2e3+f*15e3),y=Number((.5+f*2.5).toFixed(2)),C=Math.round(1500+f*12e3),p=Number((.4+f*2.2).toFixed(2)),b=Math.round(20+f*80),v=Math.max(1,Math.round(b*(.08+f*.04)));return{monthKey:c,label:metaMonthLabel(c),roasKr:h,roasX:y,poasKr:C,poasX:p,spend:k,leads:b,wonLeads:v,avgLeadValue:85e3,avgProfitPerWon:42e3,hasData:!0,demo:!0}})}function resolveMetaReportChartSeries(e){const t=buildMetaReportChartSeries(e);if(t.length>=META_REPORT_CHART_MIN_POINTS)return{series:t,demo:!1};if(isMetaReportCensioClient(e)){const a=getMetaReportDemoSeries(e.clientId,e.accountName,e.year||metaReportsState.selectedYear,e.monthKeys);if(a&&a.length>=META_REPORT_CHART_MIN_POINTS)return{series:a,demo:!0}}return{series:t,demo:!1}}function metaReportParseAmount(e){const t=Number.parseFloat(e);return Number.isFinite(t)?t:0}function metaReportRoundMoney(e){return Math.round(metaReportParseAmount(e)*100)/100}function metaReportRoundRatio(e){return Math.round(metaReportParseAmount(e)*1e8)/1e8}function metaReportAggregateSeriesEfficiency(e=[]){let t=0,a=0,n=0,s=0,o=0;for(const i of e){const l=metaReportParseAmount(i.spend),d=metaReportParseAmount(i.leads),c=metaReportParseAmount(i.wonLeads);l<=0&&d<=0||(t+=l,a+=d,n+=c,s+=c*metaReportParseAmount(i.avgLeadValue),o+=c*metaReportParseAmount(i.avgProfitPerWon))}const r=e.filter(i=>metaReportParseAmount(i.spend)>0).length;return{avgCpl:a>0?metaReportRoundMoney(t/a):0,winRate:a>0?metaReportRoundRatio(n/a):0,avgLeadValue:n>0?metaReportRoundMoney(s/n):0,avgProfitPerWon:n>0?metaReportRoundMoney(o/n):0,avgMonthlySpend:r>0?metaReportRoundMoney(t/r):0}}function metaReportProjectBudgetAtSpend(e,t,a=!1){const n=metaReportParseAmount(e);if(n<=0||t.avgCpl<=0)return{spend:0,leads:0,wonLeads:0,totalLeadValue:0,roasKr:0,roasX:0,poasKr:null,poasX:null};const s=Math.round(n/t.avgCpl),o=Math.round(s*t.winRate),r=metaReportRoundMoney(o*t.avgLeadValue),i=metaReportRoundMoney(r-n),l=n>0?metaReportRoundRatio(i/n):0;let d=null,c=null;if(a&&t.avgProfitPerWon>0){const u=metaReportRoundMoney(o*t.avgProfitPerWon);d=metaReportRoundMoney(u-n),c=n>0?metaReportRoundRatio(d/n):0}return{spend:n,leads:s,wonLeads:o,totalLeadValue:r,roasKr:i,roasX:l,poasKr:d,poasX:c}}function metaReportProjectBudgetScenario({series:e=[],baselineSpend:t,multiplier:a=1,hasBottomline:n=!1}={}){const s=metaReportAggregateSeriesEfficiency(e),o=metaReportParseAmount(t),r=metaReportParseAmount(a)||1,i=metaReportProjectBudgetAtSpend(o,s,n),l=metaReportProjectBudgetAtSpend(metaReportRoundMoney(o*r),s,n);return{efficiency:s,baseline:i,projected:l,multiplier:r,baselineSpend:o,hasBottomline:n,insufficientData:s.avgCpl<=0||o<=0}}function metaReportSpendAxisTick(e){const t=Number(e)||0;return Math.abs(t)>=1e6?`${(t/1e6).toFixed(1)}M`:Math.abs(t)>=1e3?`${Math.round(t/1e3)}K`:t}function metaReportSpendMultiplierValue(e,t){const a=metaReportParseAmount(t),n=metaReportParseAmount(e);return a<=0?0:metaReportRoundRatio(n/a)}function resolveMetaReportSpendChartBaseline(e,t=null){const a=metaReportParseAmount(t?.baselineSpend);return a>0?a:metaReportAggregateSeriesEfficiency(e).avgMonthlySpend}function trimMetaReportScenarioSeries(e,t=META_REPORT_SCENARIO_MAX_HISTORICAL){const a=e.filter(n=>metaReportParseAmount(n.spend)>0);return a.length>t?a.slice(-t):a}function buildMetaReportScenarioProjectionSteps(e,{months:t=META_REPORT_SCENARIO_PROJECTED_MONTHS,hasBottomline:a=!1}={}){if(!e||e.insufficientData)return[];const n=metaReportParseAmount(e.baselineSpend),s=metaReportParseAmount(e.multiplier)||1,o=e.efficiency,r=[];for(let i=1;i<=t;i+=1){const l=metaReportRoundRatio(1+(s-1)*i/t),d=metaReportRoundMoney(n*l),c=metaReportProjectBudgetAtSpend(d,o,a);r.push({label:`+${i} mo`,spendMultiplier:l,spend:c.spend,roasKr:c.roasKr,roasX:c.roasX,poasKr:c.poasKr,poasX:c.poasX})}return r}function metaReportDeltaPct(e,t){const a=metaReportParseAmount(t),n=metaReportParseAmount(e);return a?metaReportRoundRatio((n-a)/Math.abs(a)*100):null}function metaReportResolveBaselineSpend(e,t){if(metaReportsState.budgetBaseline==="month"&&metaReportsState.activeMonthKey){const n=t.find(s=>s.monthKey===metaReportsState.activeMonthKey);if(n&&metaReportParseAmount(n.spend)>0)return metaReportParseAmount(n.spend)}return metaReportAggregateSeriesEfficiency(t).avgMonthlySpend}function payloadHasMetaReportBottomline(e){return e?.settings?.metaReportShowBottomline===!1?!1:seriesHasBottomline(metaReportsState.chartSeries||buildMetaReportChartSeries(e))}function seriesHasBottomline(e=[]){return e.some(t=>t.poasKr!=null&&t.poasX!=null)}function renderMetaReportChartsPanel({editable:e=!1}={}){const t=metaReportsState.chartTab||"trend";return`
    ...
    <section class="meta-report-scenario-card" id="meta-report-scenario-card">
      <div class="meta-report-scenario-head">
        <div>
          ...
          <p class="meta-report-scenario-copy">Estimate what happens if ad spend scales while funnel efficiency stays constant.</p>
        </div>
      </div>
      <div class="meta-report-scenario-controls">
        <label class="meta-report-scenario-field">
          ...
          <div class="meta-report-scenario-slider-row">
            ...
            <output id="meta-report-budget-multiplier-value" class="meta-report-scenario-multiplier-pill">${esc(Number(metaReportsState.budgetMultiplier).toFixed(1))}\xD7</output>
          </div>
        </label>
        <label class="meta-report-scenario-field">
          <span>Baseline spend</span>
          <select id="meta-report-budget-baseline" class="admin-select">
            <option value="year"${metaReportsState.budgetBaseline==="year"?" selected":""}>Year average</option>
      ...
      <p class="meta-report-scenario-disclaimer">Estimate assumes current funnel efficiency (CPL, win rate, lead value) stays constant.</p>
    </section>
  `}const META_SCENARIO_ICON_REVENUE='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><path d="M12 8v8M9 11h6"/></svg>',META_SCENARIO_ICON_ROAS='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M7 14l4-4 3 3 5-6"/></svg>',META_SCENARIO_ICON_PROFIT='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 17l6-6 4 4 8-8"/><path d="M14 7h7v7"/></svg>',META_SCENARIO_ICON_LEADS='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>',META_SCENARIO_ICONS={revenue:META_SCENARIO_ICON_REVENUE,roas:META_SCENARIO_ICON_ROAS,profit:META_SCENARIO_ICON_PROFIT,leads:META_SCENARIO_ICON_LEADS};function metaReportFormatDeltaPct(e){return e==null||!Number.isFinite(e)?"":`${e>0?"+":""}${e.toFixed(0)}%`}function metaReportScenarioDeltaTone(e=""){return String(e).startsWith("+")?"positive":String(e).startsWith("-")?"negative":"neutral"}function metaReportFormatScenarioGain(e,t){const a=metaReportRoundMoney(metaReportParseAmount(e)-metaReportParseAmount(t));return a?`${a>0?"+":"\u2212"}${metaFmtKr(Math.abs(a))} vs baseline`:""}function renderMetaReportScenarioCompareBar(e,t,{compact:a=!1}={}){const n=Math.abs(metaReportParseAmount(e)),s=Math.abs(metaReportParseAmount(t)),o=Math.max(n,s,1),r=Math.max(4,Math.round(n/o*100)),i=Math.max(4,Math.round(s/o*100));return`
    <div class="meta-report-scenario-compare-bar${a?" is-compact":""}" aria-hidden="true">
      <div class="meta-report-scenario-compare-bar-track">
        <div class="meta-report-scenario-compare-bar-base" style="width:${r}%"></div>
        <div class="meta-report-scenario-compare-bar-projected" style="width:${i}%"></div>
      </div>
    </div>
  `}function renderMetaReportScenarioContextBanner(e){const t=Number(e?.multiplier??metaReportsState.budgetMultiplier).toFixed(1),a=metaReportsState.budgetBaseline==="month"?"Selected month baseline":"Year average baseline";return`<div class="meta-report-scenario-context">At ${esc(t)}\xD7 budget \xB7 ${esc(a)}</div>`}function renderMetaReportScenarioOutcomeCard({variant:e="revenue",label:t,projectedValue:a,projectedSub:n="",baselineValue:s="",delta:o="",heroFirst:r=!1,projectedAmount:i=null,baselineAmount:l=null}={}){const d=metaReportScenarioDeltaTone(o),c=i!=null&&l!=null?metaReportFormatScenarioGain(i,l):"",u=r&&n?n:a,f=r&&n?a:n,k=META_SCENARIO_ICONS[e]||META_SCENARIO_ICONS.revenue;return`
    <div class="${["meta-report-scenario-outcome-card",`is-${e}`,d==="negative"?"is-negative":""].filter(Boolean).join(" ")}">
      <div class="meta-report-scenario-outcome-head">
        <div class="meta-report-scenario-outcome-head-left">
          <span class="meta-report-scenario-outcome-icon">${k}</span>
          <div class="meta-report-scenario-outcome-label">${esc(t)}</div>
        </div>
        ${o?`<div class="meta-report-scenario-outcome-delta is-${d}">${esc(o)}</div>`:""}
      </div>
      <div class="meta-report-scenario-outcome-hero">${esc(u)}</div>
      ${f?`<div class="meta-report-scenario-outcome-secondary">${esc(f)}${r?" return":""}</div>`:""}
      ${i!=null&&l!=null?renderMetaReportScenarioCompareBar(l,i):""}
      ${c?`<div class="meta-report-scenario-outcome-gain">${esc(c)}</div>`:""}
      ${s?`<div class="meta-report-scenario-outcome-baseline">Baseline ${esc(s)}</div>`:""}
    </div>
  `}function renderMetaReportScenarioDetailChip(e,t,a,n,s,o){const r=metaReportScenarioDeltaTone(n);return`
    <div class="meta-report-scenario-detail-chip">
      <div class="meta-report-scenario-detail-chip-head">
        <span class="meta-report-scenario-detail-chip-label">${esc(e)}</span>
        <span class="meta-report-scenario-detail-chip-delta is-${r}">${esc(n||"\u2014")}</span>
      </div>
      <div class="meta-report-scenario-detail-chip-values">
        <span>${esc(t)}</span>
        <span class="meta-report-scenario-detail-chip-arrow">\u2192</span>
        <span class="is-projected">${esc(a)}</span>
      </div>
      ${renderMetaReportScenarioCompareBar(s,o,{compact:!0})}
    </div>
  `}function renderMetaReportScenarioMetrics(e){if(!e||e.insufficientData)return'<p class="meta-report-scenario-empty">Add months with ad spend and leads to model a budget scenario.</p>';const t=metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.totalLeadValue,e.baseline.totalLeadValue)),a=metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.roasKr,e.baseline.roasKr)),n=[renderMetaReportScenarioOutcomeCard({variant:"revenue",label:"Projected revenue",projectedValue:metaFmtKr(e.projected.totalLeadValue),baselineValue:metaFmtKr(e.baseline.totalLeadValue),projectedAmount:e.projected.totalLeadValue,baselineAmount:e.baseline.totalLeadValue,delta:t}),renderMetaReportScenarioOutcomeCard({variant:"roas",label:"Projected ROAS",projectedValue:metaFmtKr(e.projected.roasKr),projectedSub:metaFmtX(e.projected.roasX),heroFirst:!0,baselineValue:`${metaFmtKr(e.baseline.roasKr)} \xB7 ${metaFmtX(e.baseline.roasX)}`,projectedAmount:e.projected.roasKr,baselineAmount:e.baseline.roasKr,delta:a})];if(e.hasBottomline){const o=metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.poasKr,e.baseline.poasKr));n.push(renderMetaReportScenarioOutcomeCard({variant:"profit",label:"Projected profit",projectedValue:metaFmtKr(e.projected.poasKr),projectedSub:metaFmtX(e.projected.poasX),baselineValue:`${metaFmtKr(e.baseline.poasKr)} \xB7 ${metaFmtX(e.baseline.poasX)}`,projectedAmount:e.projected.poasKr,baselineAmount:e.baseline.poasKr,delta:o}))}else{const o=metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.wonLeads,e.baseline.wonLeads));n.push(renderMetaReportScenarioOutcomeCard({variant:"leads",label:"Projected won leads",projectedValue:metaFmtNum(e.projected.wonLeads),baselineValue:metaFmtNum(e.baseline.wonLeads),projectedAmount:e.projected.wonLeads,baselineAmount:e.baseline.wonLeads,delta:o}))}const s=[renderMetaReportScenarioDetailChip("Ad spend",metaFmtKr(e.baseline.spend),metaFmtKr(e.projected.spend),metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.spend,e.baseline.spend)),e.baseline.spend,e.projected.spend),renderMetaReportScenarioDetailChip("Leads",metaFmtNum(e.baseline.leads),metaFmtNum(e.projected.leads),metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.leads,e.baseline.leads)),e.baseline.leads,e.projected.leads)];return e.hasBottomline&&s.push(renderMetaReportScenarioDetailChip("Won leads",metaFmtNum(e.baseline.wonLeads),metaFmtNum(e.projected.wonLeads),metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.wonLeads,e.baseline.wonLeads)),e.baseline.wonLeads,e.projected.wonLeads)),`
    ${renderMetaReportScenarioContextBanner(e)}
  ...
  `:"",r=renderMetaReportMonthTables(e,{editable:t}),i=shouldShowMetaReportYearVisuals(e,a,{editable:t}),l=i?renderMetaReportChartsPanel({editable:t}):"",d=i?renderMetaReportScenarioCard():"",c=i?`
  ...
  `).join("");return`
    ...
    `)}
  `}function renderPublicMetaReportPage(e){const t=e.monthKeys||[],a=metaReportsState.activeMonthKey||t[t.length-1]||"",n=e.months?.[a]||null,s=t.map(o=>`
  ...
  `).join("");return`
    ...
    <div class="meta-reports-page meta-reports-page--public">
      ...
      <div class="meta-report-month-panel" id="meta-report-month-panel">
        ${renderMetaReportMonthBody(n,getMetaReportMonthBodyOptions(e.settings,{editable:!1,yearPayload:e}))}
      </div>
    </div>
    `)}
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t,t&&(metaReportsState.clientShareExpanded=!1);const a=document.querySelector(".meta-report-bottomline-fee-settings--client"),n=document.getElementById(`${e}-fee-editor`);a?.classList.toggle("is-editing",t),a?.classList.toggle("is-collapsed",!t),n&&n.classList.toggle("is-hidden",!t);const s=document.getElementById("meta-report-share-summary"),o=document.getElementById("meta-report-share-editor");s&&(s.hidden=metaReportsState.clientShareExpanded),o&&(o.hidden=!metaReportsState.clientShareExpanded),syncMetaReportControlPanelUi()}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const a=document.getElementById(`${t}-fee-summary`);if(!a)return;const n=document.createElement("div");n.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const s=n.firstElementChild;s&&a.replaceWith(s);const o=document.getElementById(`${t}-fee-edit`);o&&(o.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:a=!1,variant:n="default",collapsible:s=!1,externalSummary:o=!1}={}){const r=!!e.metaReportShowBottomline,i=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",d=resolveMetaReportFeeMode(e),c=e.metaReportFeePercent??20,u=e.metaReportMarketingFeeAmount??0,f=metaReportUsesInlineFeeValue(t),k=`data-fee-mode="${esc(l)}" data-fee-enabled="${i?"true":"false"}" data-fee-percent="${esc(c)}" data-marketing-fee="${esc(u)}"`;if(n==="cv"){const v=describeMetaReportFeeModeLabel(d,c,u),g=META_REPORT_FEE_MODE_META.map(({value:m,title:$})=>`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${r?"":" is-hidden"}" id="${t}-fee-nested" ${k}>
  ...
  `}const h=describeMetaReportFeeModeLabel(d,c,u),y=META_REPORT_FEE_MODE_META.map(({value:v,icon:g,title:m,desc:$})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(v)}"${d===v?" checked":""} />
        ...
        </label>`).join(""),C=s?metaReportsState.clientReportSettingsExpanded:!0,p=s?`
    ...
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?C?" is-editing":" is-collapsed":""}">
      ${b}
      <div class="meta-report-bottomline-editor${s&&!C?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${r?"":" is-hidden"}" id="${t}-fee-nested" ${k}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(y=>y.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label>Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(f)}" />
      </label>
      <label>Avg profit per won${r?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(k)}" />
      </label>
    </div>
    <div class="meta-report-settings-group meta-report-settings-group--stacked meta-cv-client-settings${h?" is-saving":""}">
      ${renderMetaReportSaveIndicator("meta-cv-settings-status",h)}
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(a){showToast(a.message,"error")}finally{if(t){t.disabled=!1;const a=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${a}`:a}}}function getMetricsModelLabels(e,t=setupPipelines){const a=new Map((t||[]).map(n=>[n.id,n.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),a=!e.metricsModelSetAt||metricsModelChangeMode,n=a?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",o=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",r=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return a?`
    ...
    <div class="date-picker-panel">
      <div class="date-picker-header">
        ...
        <div class="date-picker-nav">
          ...
          <button type="button" class="date-picker-nav-btn" aria-label="Next month" onclick="shiftDatePickerMonth(1)" ${m?"":"disabled"}>${ICON_CHEVRON_RIGHT}</button>
        </div>
      </div>
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:a=!1,forceFresh:n=!1}=t,s=!!n,o=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const r=!!document.getElementById("dashboard-content"),i=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!r)o.innerHTML=`
        ...
        `)}`;else if(!a){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),i&&(i.disabled=!0,i.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const f=canReuseBootstrapDashboard(c)&&!s;let k,h;if(f?[h,k]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[k,h]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=h,cachedData=applyMarketingData(k,h),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(l!==fetchGeneration)return;if(c.name==="AbortError"){r?restoreDashboardContentAfterFailedFetch(r):o.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
