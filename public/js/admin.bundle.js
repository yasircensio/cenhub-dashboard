...
Resume the interrupted ${g}?`)&&(l=f.id,d=Number(f.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const f=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),u=f.summary||f;l=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||b?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:w,batchOffset:d,batchLimit:i})});const f=c.summary||c;l=f.runId,d=f.nextBatchOffset??f.batchOffset+(f.batchProcessed||0),m=m.concat(f.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const u=f.inWindow||d||$;n?(setFbLeadProgress({processed:d,total:u,complete:!f.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...f,rows:m}))):updateGlobalApplyProgress({processed:d,total:u,written:f.updated||0,complete:!f.hasMore})}while((c.summary||c).hasMore);y=!0;const S=c.summary||c,v=S.inWindow||d||$,h=S.nextBatchOffset??d;if(n)setFbLeadProgress({processed:h,total:v,complete:!0}),markFbLeadPreviewReady(e,t,S.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(S.updated||0)>0),showToast((S.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:v,written:S.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const f=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete f[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(f))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(S){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${S.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((S.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!y&&n&&hideFbLeadProgress(!0),!n&&!y&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(o=>o.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `}function getMetaReportMonthBodyOptions(e={},{editable:t=!1}={}){return{editable:t}}function renderMetaReportMonthBody(e,{editable:t=!1}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return`<div class="meta-report-empty">No ad spend recorded for this month yet.${t?' Click "Refresh from Meta" below, or use "Backfill from Meta" above to pull historical months.':""}</div>`;let n="";t&&(n=`
    ...
    `);const a=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["CTR",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],s=[["Leads",metaFmtNum(e.topline?.leads),""],["Cost Per Lead (CPL)",metaFmtKr(e.topline?.cpl),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let o="";if(o+=renderMetaReportMetricTable("Meta ads",a,"meta"),o+=renderMetaReportMetricTable("Topline KPI'er",s,"topline",{highlightLastN:2}),e.bottomline){const d=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];if(e.bottomline.feeMode){const c=e.bottomline.feeLabel||(e.bottomline.feeMode==="marketing"?"Censio marketing fee":`Censio performance fee (${metaFmtNum(e.bottomline.feePercent,0)}%)`);d.push([c,metaFmtKr(e.bottomline.censioFee),""]),d.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),d.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])}o+=renderMetaReportMetricTable("Bottomline KPI'er",d,"bottomline",{highlightLastN:2})}const r=`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${t?"Report preview":"Report"}</div>
  ...
  `}const b=describeMetaReportFeeModeLabel(d,c,m),$=META_REPORT_FEE_MODE_META.map(({value:p,icon:f,title:u,desc:g})=>`
        <label class="meta-report-fee-mode-card">
          ...
          <span class="meta-report-fee-mode-card-text">
            <span class="meta-report-fee-mode-card-title">${esc(u)}</span>
            <small>${esc(g)}</small>
          </span>
          <span class="meta-report-fee-mode-card-radio" aria-hidden="true">${ICON_CHECK}</span>
        </label>`).join(""),S=s?metaReportsState.clientReportSettingsExpanded:!0,v=s?`
    ...
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?S?" is-editing":" is-collapsed":""}">
      ...
      <div class="meta-report-bottomline-editor${s&&!S?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${i?"":" is-hidden"}" id="${t}-fee-nested" ${w}>
          ...
          <div class="meta-report-fee-value" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,d,c,m,{inline:y})}</div>
          ${v}
        </div>
      </div>
    </div>
  `}function syncMetaReportBottomlineFeeDom(e={},t="meta-report"){const n=!!e.metaReportShowBottomline,a=!!e.metaReportFeeEnabled,s=e.metaReportFeeMode||"",o=resolveMetaReportFeeMode(e),i=e.metaReportFeePercent??20,r=e.metaReportMarketingFeeAmount??0,l=document.getElementById(`${t}-setting-bottomline`);l&&(l.checked=n);const d=document.getElementById(`${t}-fee-nested`);d?.classList.toggle("is-hidden",!n),d&&(d.dataset.feeMode=s,d.dataset.feeEnabled=a?"true":"false",d.dataset.feePercent=String(i),d.dataset.marketingFee=String(r)),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(y=>{y.checked=y.value===o});const c=metaReportUsesInlineFeeValue(t),m=document.getElementById(`${t}-fee-value`);m&&(m.innerHTML=metaReportFeeValueBlockHtml(t,o,i,r,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),syncMetaReportFeeSelectionState(t),t==="meta-report"&&updateMetaReportBottomlineFeeSummary(e,t)}function bindMetaReportFeeValueInputEnterKey(e){const t=document.getElementById(`${e}-setting-fee-percent`)||document.getElementById(`${e}-setting-marketing-fee`);t&&(t.onkeydown=n=>{n.key==="Enter"&&(n.preventDefault(),document.getElementById(`${e}-fee-save`)?.click())})}function bindMetaReportBottomlineFeeEvents(e,t,n){const a=n||((l,d)=>saveMetaReportClientSettings(e,l,d)),s=document.getElementById(`${t}-fee-edit`);s&&(s.onclick=()=>setMetaReportClientSettingsExpanded(t,!0));const o=document.getElementById(`${t}-fee-done`);o&&(o.onclick=()=>{const l=metaReportsState.clientPayload?.settings||{};syncMetaReportBottomlineFeeDom(l,t),setMetaReportClientSettingsExpanded(t,!1),updateMetaReportBottomlineFeeSummary(l,t)});const i=document.getElementById(`${t}-setting-bottomline`);i&&(i.onchange=()=>{const l=i.checked;a({metaReportShowBottomline:l},()=>{i.checked=!l})}),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(l=>{l.onchange=()=>{if(!l.checked)return;const d=document.getElementById(`${t}-fee-nested`),c=metaReportUsesInlineFeeValue(t),m=document.getElementById(`${t}-fee-value`);m&&d&&(m.innerHTML=metaReportFeeValueBlockHtml(t,l.value||"",d.dataset.feePercent??20,d.dataset.marketingFee??0,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),bindMetaReportFeeAmountInputs(t),syncMetaReportFeeSelectionState(t)}}),bindMetaReportFeeValueInputEnterKey(t),bindMetaReportFeeDraftTracking(t);const r=document.getElementById(`${t}-fee-save`);r&&(r.onclick=async()=>{if(r.disabled)return;const l=document.getElementById(`${t}-fee-nested`),d=l?.querySelector(`input[name="${t}-fee-mode"]:checked`),c=d&&d.value||null,m={metaReportFeeMode:c};if(c==="performance"){const w=document.getElementById(`${t}-setting-fee-percent`);let b=Number(w?.value);Number.isFinite(b)||(b=20),b=Math.min(100,Math.max(0,b)),w&&(w.value=String(b)),m.metaReportFeePercent=b}else if(c==="marketing"){const w=document.getElementById(`${t}-setting-marketing-fee`);let b=Number(w?.value);Number.isFinite(b)||(b=0),b=Math.max(0,b),w&&(w.value=String(b)),m.metaReportMarketingFeeAmount=b}const y=r.textContent;r.disabled=!0,r.textContent="Saving\u2026",r.classList.remove("is-dirty");try{await a(m,()=>syncMetaReportBottomlineFeeDom({metaReportShowBottomline:i?i.checked:!0,metaReportFeeMode:l?.dataset.feeMode||null,metaReportFeePercent:Number(l?.dataset.feePercent)||20,metaReportMarketingFeeAmount:Number(l?.dataset.marketingFee)||0},t)),syncMetaReportFeeSelectionState(t)}finally{r.disabled=!1,r.textContent=y,syncMetaReportFeeSelectionState(t)}})}function renderMetaReportSaveIndicator(e,t=!1){return`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find($=>$.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
    ...
    `;const s=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,s);const o=t.editorPayload?.months?.[a]||null,i=n.metaReportShowBottomline,r=s.wonLeads!=null?s.wonLeads:o?.inputs?.wonLeads??o?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:o?.inputs?.avgLeadValue??o?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:o?.inputs?.avgProfitPerWon??"",c=metaReportsState.customValues.draftInputs,m=c?.wonLeads??r,y=c?.avgLeadValue??l,w=c?.avgProfitPerWon??d,b=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label>Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(y)}" />
      </label>
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",o=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",i=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
    ...
    <div class="date-picker-panel">
      <div class="date-picker-header">
        ...
        <div class="date-picker-nav">
          ...
          <button type="button" class="date-picker-nav-btn" aria-label="Next month" onclick="shiftDatePickerMonth(1)" ${u?"":"disabled"}>${ICON_CHEVRON_RIGHT}</button>
        </div>
      </div>
      <div class="date-picker-weekdays">
        ${o.map(g=>`<div class="date-picker-weekday">${g}</div>`).join("")}
      </div>
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,o=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const i=!!document.getElementById("dashboard-content"),r=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!i)o.innerHTML=`
        ...
        `)}`;else if(!n){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),r&&(r.disabled=!0,r.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const y=canReuseBootstrapDashboard(c)&&!s;let w,b;if(y?[b,w]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[w,b]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=b,cachedData=applyMarketingData(w,b),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(l!==fetchGeneration)return;if(c.name==="AbortError"){i?restoreDashboardContentAfterFailedFetch(i):o.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            `)}`;return}if(IS_ADMIN_CLIENT&&i){const m=document.getElementById("dashboard-content");m&&(m.innerHTML=`
            <div class="note admin-setup-placeholder">
              ${esc(c.message)}
