...
Resume the interrupted ${$}?`)&&(l=y.id,d=Number(y.batchOffset)||0)}}if(a?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&d===0){const y=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),b=y.summary||y;l=b.runId,d=0,setFbLeadProgress({processed:0,total:b.inWindow||0,label:`Matching ${b.inWindow||0} Meta leads to GHL\u2026`})}else!a&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||f?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:m,batchOffset:d,batchLimit:r})});const y=c.summary||c;l=y.runId,d=y.nextBatchOffset??y.batchOffset+(y.batchProcessed||0),u=u.concat(y.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const b=y.inWindow||d||w;a?(setFbLeadProgress({processed:d,total:b,complete:!y.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...y,rows:u}))):updateGlobalApplyProgress({processed:d,total:b,written:y.updated||0,complete:!y.hasMore})}while((c.summary||c).hasMore);p=!0;const S=c.summary||c,h=S.inWindow||d||w,v=S.nextBatchOffset??d;if(a)setFbLeadProgress({processed:v,total:h,complete:!0}),markFbLeadPreviewReady(e,t,S.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(S.updated||0)>0),showToast((S.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:h,written:S.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const y=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete y[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(y))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const g=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=g.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(S){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${S.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((S.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function renderMetaReportMetricTable(e,t,a="meta",{badge:n="",highlightLastN:s=0}={}){const o=t.map(([r,i,l],d)=>`
      <tr class="${s>0&&d>=t.length-s?`is-highlight accent-${a}`:""}">
  ...
  `}function getMetaReportMonthBodyOptions(e={},{editable:t=!1,yearPayload:a=null}={}){return{editable:t,yearPayload:a}}function shouldShowMetaReportYearVisuals(e,t,{editable:a=!1}={}){if(!e||e.meta?.emptyMonth||!t)return!1;const{series:n,demo:s}=resolveMetaReportChartSeries(t);return!(n.length<META_REPORT_CHART_MIN_POINTS||s&&!a)}const META_REPORT_CHART_MIN_POINTS=2,META_REPORT_SCENARIO_MAX_HISTORICAL=9,META_REPORT_SCENARIO_PROJECTED_MONTHS=4,META_REPORT_SPEND_CHART_TYPE_OPTIONS=[{value:"area",label:"Area"},{value:"bar",label:"Bar"}],META_REPORT_SCENARIO_TREND_METHOD_OPTIONS=[{value:"recency_weighted",label:"Recency-weighted"},{value:"robust_trend",label:"Robust trend-adjusted"},{value:"conservative_blend",label:"Conservative blend"}],META_REPORT_SCENARIO_MONTH_WINDOW_OPTIONS=[{value:"3",label:"Last 3 ad months"},{value:"6",label:"Last 6 ad months"},{value:"9",label:"Last 9 ad months"},{value:"12",label:"Last 12 ad months"},{value:"all",label:"All ad-active months"}],META_REPORT_SCENARIO_ELASTICITY_OPTIONS=[{value:"conservative",label:"Conservative scaling",description:"Stronger diminishing returns when spend increases (0.70)."},{value:"balanced",label:"Balanced scaling",description:"Default saturation curve for most clients (0.75)."},{value:"optimistic",label:"Optimistic scaling",description:"Milder diminishing returns \u2014 capped for realism (0.80)."}],META_REPORT_SCENARIO_MODEL_PILLS=[{id:"scenarioSmoothUneven",label:"Smooth uneven months",description:"Softens months with unusual CPL or lead spikes so one bad tracking month does not skew the forecast.",defaultOn:!0},{id:"scenarioBlendHistory",label:"Blend with full history",description:"Mixes recent months with the full window average instead of only weighting the latest streak.",defaultOn:!1},{id:"scenarioIncludeTrend",label:"Include trend direction",description:"Adds a small up or down adjustment when efficiency has been moving consistently over time.",defaultOn:!1},{id:"scenarioCautionStrongMonths",label:"Caution after strong months",description:"Pulls projections back when the last 3 months look much better than the full period.",defaultOn:!1}],META_REPORT_SCENARIO_ELASTICITY_VALUES={conservative:.7,balanced:.75,optimistic:.8};function normalizeMetaReportSpendChartType(e){const t=String(e||"area").trim().toLowerCase();return META_REPORT_SPEND_CHART_TYPE_OPTIONS.some(a=>a.value===t)?t:"area"}function hydrateMetaReportSpendChartType(e){metaReportsState.spendChartType=normalizeMetaReportSpendChartType(e?.settings?.metaReportSpendChartType)}function normalizeMetaReportScenarioTrendMethod(e){const t=String(e||"recency_weighted").trim().toLowerCase();return META_REPORT_SCENARIO_TREND_METHOD_OPTIONS.some(a=>a.value===t)?t:"recency_weighted"}function normalizeMetaReportScenarioMonthWindow(e){const t=String(e||"6").trim().toLowerCase();return META_REPORT_SCENARIO_MONTH_WINDOW_OPTIONS.some(a=>a.value===t)?t:"6"}function normalizeMetaReportScenarioElasticity(e){const t=String(e||"balanced").trim().toLowerCase();return META_REPORT_SCENARIO_ELASTICITY_OPTIONS.some(a=>a.value===t)?t:"balanced"}function normalizeMetaReportBudgetBaseline(e){return String(e||"year").trim().toLowerCase()==="month"?"month":"year"}function normalizeMetaReportBudgetMultiplier(e){const t=Number.parseFloat(e);return Number.isFinite(t)?Math.min(3,Math.max(.5,Math.round(t*10)/10)):2}function getMetaReportScenarioPillsFromState(){return{smoothUneven:!!metaReportsState.scenarioSmoothUneven,blendHistory:!!metaReportsState.scenarioBlendHistory,includeTrend:!!metaReportsState.scenarioIncludeTrend,cautionStrongMonths:!!metaReportsState.scenarioCautionStrongMonths}}function deriveMetaReportScenarioPillsFromTrendMethod(e){const t=normalizeMetaReportScenarioTrendMethod(e),a={smoothUneven:!0,blendHistory:!1,includeTrend:!1,cautionStrongMonths:!1};return t==="robust_trend"?a.includeTrend=!0:t==="conservative_blend"&&(a.blendHistory=!0,a.cautionStrongMonths=!0,a.includeTrend=!0),a}function resolveMetaReportTrendMethodFromPills(e={}){return e.blendHistory||e.cautionStrongMonths?"conservative_blend":e.includeTrend?"robust_trend":"recency_weighted"}function applyMetaReportScenarioPillsToState(e={}){metaReportsState.scenarioSmoothUneven=!!e.smoothUneven,metaReportsState.scenarioBlendHistory=!!e.blendHistory,metaReportsState.scenarioIncludeTrend=!!e.includeTrend,metaReportsState.scenarioCautionStrongMonths=!!e.cautionStrongMonths,metaReportsState.scenarioTrendMethod=resolveMetaReportTrendMethodFromPills(e)}function describeActiveMetaReportScenarioPills(){return META_REPORT_SCENARIO_MODEL_PILLS.filter(e=>!!metaReportsState[e.id]).map(e=>e.label)}function hydrateMetaReportScenarioSettings(e){const t=e?.settings||{};metaReportsState.budgetMultiplier=normalizeMetaReportBudgetMultiplier(t.metaReportBudgetMultiplier),metaReportsState.budgetBaseline=normalizeMetaReportBudgetBaseline(t.metaReportBudgetBaseline),metaReportsState.scenarioMonthWindow=normalizeMetaReportScenarioMonthWindow(t.metaReportScenarioMonthWindow),metaReportsState.scenarioElasticity=normalizeMetaReportScenarioElasticity(t.metaReportScenarioElasticity),applyMetaReportScenarioPillsToState(deriveMetaReportScenarioPillsFromTrendMethod(t.metaReportScenarioTrendMethod))}function renderMetaReportScenarioModelPills(){return`
    <div class="meta-report-scenario-pill-section meta-report-scenario-field--wide">
      <span class="meta-report-scenario-field-label">Forecast adjustments</span>
      <p class="meta-report-scenario-pill-lead">Choose how history is interpreted. These replace the old abstract trend method names.</p>
      <div class="meta-report-scenario-pill-grid" role="group" aria-label="Forecast adjustments">
        ${META_REPORT_SCENARIO_MODEL_PILLS.map(({id:e,label:t,description:a})=>{const n=!!metaReportsState[e];return`
            <button type="button" class="meta-report-scenario-pill${n?" is-active":""}" data-scenario-pill="${esc(e)}" aria-pressed="${n}">
              <span class="meta-report-scenario-pill-label">${esc(t)}</span>
              <span class="meta-report-scenario-pill-desc">${esc(a)}</span>
            </button>
          `}).join("")}
      </div>
    </div>
  `}function renderMetaReportScenarioElasticityPills(){const e=metaReportsState.scenarioElasticity||"balanced";return`
    <div class="meta-report-scenario-pill-section meta-report-scenario-field--wide">
      <span class="meta-report-scenario-field-label">Spend scaling</span>
      <p class="meta-report-scenario-pill-lead">How aggressively results grow when ad spend increases.</p>
      <div class="meta-report-scenario-pill-grid meta-report-scenario-pill-grid--compact" role="group" aria-label="Spend scaling">
        ${META_REPORT_SCENARIO_ELASTICITY_OPTIONS.map(({value:t,label:a,description:n})=>`
          <button type="button" class="meta-report-scenario-pill meta-report-scenario-pill--select${t===e?" is-active":""}" data-scenario-elasticity="${esc(t)}" aria-pressed="${t===e}">
            <span class="meta-report-scenario-pill-label">${esc(a)}</span>
            <span class="meta-report-scenario-pill-desc">${esc(n)}</span>
          </button>
        `).join("")}
      </div>
    </div>
  ...
  `}function isMetaReportCensioClient(e){const t=String(e?.clientId||"").toLowerCase(),a=String(e?.accountName||"");return t==="censio"||/censio/i.test(a)}function monthHasChartData(e){if(!e||e.meta?.emptyMonth)return!1;const t=Number(e.meta?.spend)||0,a=Number(e.topline?.leads)||0;return t>0||a>0}function buildMetaReportChartSeries(e){const t=e?.monthKeys||[],a=e?.months||{},n=[];for(const s of t){const o=a[s];if(!o||o.meta?.emptyMonth||!monthHasChartData(o))continue;const r=Number(o.topline?.roasKr)||0,i=Number(o.topline?.roasX)||0,l=!!o.bottomline,d=l?Number(o.bottomline.poasKr)||0:null,c=l?Number(o.bottomline.poasX)||0:null;n.push({monthKey:s,label:metaMonthLabel(s),periodEnd:o.periodEnd||null,roasKr:r,roasX:i,poasKr:d,poasX:c,spend:Number(o.meta?.spend)||0,leads:Number(o.topline?.leads)||0,wonLeads:Number(o.topline?.wonLeads)||0,avgLeadValue:Number(o.topline?.avgLeadValue)||0,avgProfitPerWon:Number(o.inputs?.avgProfitPerWon)||0,hasData:!0})}return n}function getMetaReportDemoSeries(e,t,a,n){if(!isMetaReportCensioClient({clientId:e,accountName:t}))return null;const o=new Date,r=o.getFullYear(),i=o.getMonth()+1,l=Number(a)||r;let d=(n||[]).filter(c=>{const u=Number(String(c).slice(0,4)),p=Number(String(c).slice(5,7));return!(u!==l||l===r&&p>i)});if(!d.length){const c=l===r?Math.min(8,i):8;d=Array.from({length:c},(u,p)=>`${l}-${String(p+1).padStart(2,"0")}`)}return d.map((c,u)=>{const p=d.length>1?u/(d.length-1):0,m=Math.round(8e3+p*1e4),f=Math.round(2e3+p*15e3),w=Number((.5+p*2.5).toFixed(2)),S=Math.round(1500+p*12e3),h=Number((.4+p*2.2).toFixed(2)),v=Math.round(20+p*80),g=Math.max(1,Math.round(v*(.08+p*.04)));return{monthKey:c,label:metaMonthLabel(c),roasKr:f,roasX:w,poasKr:S,poasX:h,spend:m,leads:v,wonLeads:g,avgLeadValue:85e3,avgProfitPerWon:42e3,hasData:!0,demo:!0}})}function resolveMetaReportChartSeries(e){const t=buildMetaReportChartSeries(e);if(t.length>=META_REPORT_CHART_MIN_POINTS)return{series:t,demo:!1};if(isMetaReportCensioClient(e)){const a=getMetaReportDemoSeries(e.clientId,e.accountName,e.year||metaReportsState.selectedYear,e.monthKeys);if(a&&a.length>=META_REPORT_CHART_MIN_POINTS)return{series:a,demo:!0}}return{series:t,demo:!1}}function metaReportParseAmount(e){const t=Number.parseFloat(e);return Number.isFinite(t)?t:0}function metaReportRoundMoney(e){return Math.round(metaReportParseAmount(e)*100)/100}function metaReportRoundRatio(e){return Math.round(metaReportParseAmount(e)*1e8)/1e8}function metaReportAggregateSeriesEfficiency(e=[]){let t=0,a=0,n=0,s=0,o=0;for(const i of e){const l=metaReportParseAmount(i.spend),d=metaReportParseAmount(i.leads),c=metaReportParseAmount(i.wonLeads);l<=0&&d<=0||(t+=l,a+=d,n+=c,s+=c*metaReportParseAmount(i.avgLeadValue),o+=c*metaReportParseAmount(i.avgProfitPerWon))}const r=e.filter(i=>metaReportParseAmount(i.spend)>0).length;return{avgCpl:a>0?metaReportRoundMoney(t/a):0,winRate:a>0?metaReportRoundRatio(n/a):0,avgLeadValue:n>0?metaReportRoundMoney(s/n):0,avgProfitPerWon:n>0?metaReportRoundMoney(o/n):0,avgMonthlySpend:r>0?metaReportRoundMoney(t/r):0}}const META_SCENARIO_MIN_EFFICIENCY_MONTHS=2,META_SCENARIO_MIN_TREND_MONTHS=4,META_SCENARIO_MAD_MULTIPLIER=2.5,META_SCENARIO_MAX_MONTHLY_TREND=.03;function metaReportScenarioParseDate(e){if(!e)return null;const t=String(e).slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(t))return null;const a=new Date(`${t}T12:00:00.000Z`);return Number.isNaN(a.getTime())?null:a}function metaReportScenarioIsIncompleteMonth(e,t=new Date){if(!metaReportScenarioParseDate(e.periodEnd))return!0;const n=t.toISOString().slice(0,10);return String(e.periodEnd).slice(0,10)>=n}function metaReportScenarioIsAdActive(e){return metaReportParseAmount(e.spend)>0&&metaReportParseAmount(e.leads)>0}function metaReportScenarioMedian(e=[]){const t=e.filter(n=>Number.isFinite(n)).slice().sort((n,s)=>n-s);if(!t.length)return 0;const a=Math.floor(t.length/2);return t.length%2===0?(t[a-1]+t[a])/2:t[a]}function metaReportScenarioMad(e=[]){const t=metaReportScenarioMedian(e);return metaReportScenarioMedian(e.map(a=>Math.abs(a-t)))}function metaReportScenarioLinearRegression(e=[]){const t=e.length;if(t<2)return{slope:0,intercept:0,r:0};let a=0,n=0,s=0,o=0,r=0;for(const p of e){const m=metaReportParseAmount(p.x),f=metaReportParseAmount(p.y);a+=m,n+=f,s+=m*f,o+=m*m,r+=f*f}const i=t*o-a*a;if(i===0)return{slope:0,intercept:n/t,r:0};const l=(t*s-a*n)/i,d=(n-l*a)/t,c=Math.sqrt((t*o-a*a)*(t*r-n*n)),u=c>0?(t*s-a*n)/c:0;return{slope:l,intercept:d,r:u}}function metaReportScenarioEnrichMonth(e){const t=metaReportParseAmount(e.spend),a=metaReportParseAmount(e.leads),n=metaReportParseAmount(e.wonLeads),s=metaReportParseAmount(e.avgLeadValue),o=metaReportParseAmount(e.avgProfitPerWon),r=metaReportRoundMoney(n*s);return{...e,spend:t,leads:a,wonLeads:n,avgLeadValue:s,avgProfitPerWon:o,cpl:a>0?metaReportRoundMoney(t/a):0,totalLeadValue:r,efficiencyIndex:t>0?r/t:0}}function metaReportPrepareScenarioSeries(e=[],{windowMonths:t="6",asOfDate:a=new Date,smoothOutliers:n=!0}={}){const s=normalizeMetaReportScenarioMonthWindow(t)==="all"?1/0:Number.parseInt(normalizeMetaReportScenarioMonthWindow(t),10)||6,o=e.filter(metaReportScenarioIsAdActive).filter(c=>!metaReportScenarioIsIncompleteMonth(c,a)).map(metaReportScenarioEnrichMonth),r=o.length,i=s===1/0?o:o.slice(-s);let l=0,d=i;if(n&&i.length>=3){const c=i.map(S=>metaReportParseAmount(S.cpl)),u=i.map(S=>metaReportParseAmount(S.leads)),p=metaReportScenarioMedian(c),m=metaReportScenarioMedian(u),f=metaReportScenarioMad(c),w=metaReportScenarioMad(u);d=i.map(S=>{const h={...S};let v=!1;if(f>0){const g=p-META_SCENARIO_MAD_MULTIPLIER*f,y=p+META_SCENARIO_MAD_MULTIPLIER*f,b=Math.min(y,Math.max(g,metaReportParseAmount(S.cpl)));b!==metaReportParseAmount(S.cpl)&&(h.cpl=metaReportRoundMoney(b),h.leads=Math.max(1,Math.round(metaReportParseAmount(S.spend)/b)),v=!0)}if(w>0){const g=m-META_SCENARIO_MAD_MULTIPLIER*w,y=m+META_SCENARIO_MAD_MULTIPLIER*w,b=Math.min(y,Math.max(g,metaReportParseAmount(S.leads)));Math.round(b)!==Math.round(metaReportParseAmount(S.leads))&&(h.leads=Math.max(1,Math.round(b)),v=!0)}return v&&(l+=1),h})}return{months:d,monthsAvailable:r,monthsUsed:d.length,outliersAdjusted:l}}function metaReportScenarioRecencyEfficiency(e=[]){if(e.length<META_SCENARIO_MIN_EFFICIENCY_MONTHS)return null;let t=0,a=0,n=0,s=0,o=0,r=0;return e.forEach((i,l)=>{const d=l+1;t+=metaReportParseAmount(i.spend)*d,a+=metaReportParseAmount(i.leads)*d,n+=metaReportParseAmount(i.wonLeads)*d,s+=metaReportParseAmount(i.wonLeads)*metaReportParseAmount(i.avgLeadValue)*d,o+=metaReportParseAmount(i.wonLeads)*metaReportParseAmount(i.avgProfitPerWon)*d,r+=metaReportParseAmount(i.spend)}),{avgCpl:a>0?metaReportRoundMoney(t/a):0,winRate:a>0?metaReportRoundRatio(n/a):0,avgLeadValue:n>0?metaReportRoundMoney(s/n):0,avgProfitPerWon:n>0?metaReportRoundMoney(o/n):0,avgMonthlySpend:e.length>0?metaReportRoundMoney(r/e.length):0}}function metaReportScenarioPooledEfficiency(e=[]){return metaReportAggregateSeriesEfficiency(e)}function metaReportScenarioBlendEfficiency(e,t,a=.5){if(!e||!t)return e||t;const n=1-a;return{avgCpl:metaReportRoundMoney(e.avgCpl*a+t.avgCpl*n),winRate:metaReportRoundRatio(e.winRate*a+t.winRate*n),avgLeadValue:metaReportRoundMoney(e.avgLeadValue*a+t.avgLeadValue*n),avgProfitPerWon:metaReportRoundMoney(e.avgProfitPerWon*a+t.avgProfitPerWon*n),avgMonthlySpend:metaReportRoundMoney(e.avgMonthlySpend*a+t.avgMonthlySpend*n)}}function metaReportScenarioMonthlyTrend(e=[],t=.75){if(e.length<META_SCENARIO_MIN_TREND_MONTHS)return{trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};const a=e.map(p=>metaReportParseAmount(p.spend)).filter(p=>p>0),n=metaReportScenarioMedian(a)||1,s=e.map((p,m)=>{const f=metaReportParseAmount(p.spend),w=metaReportParseAmount(p.efficiencyIndex),S=w>0?w:1e-6,h=f>0?f:n;return{x:m,y:Math.log(S)-t*Math.log(h/n)}}),{slope:o,r}=metaReportScenarioLinearRegression(s);let i=Math.max(-META_SCENARIO_MAX_MONTHLY_TREND,Math.min(META_SCENARIO_MAX_MONTHLY_TREND,o));const l=e.map(p=>metaReportParseAmount(p.efficiencyIndex)).filter(p=>p>0),d=metaReportScenarioMedian(l),c=e.slice(-3).reduce((p,m)=>p+metaReportParseAmount(m.efficiencyIndex),0)/Math.min(3,e.length);d>0&&c>d*1.15&&i>0&&(i*=.5);let u="flat";return Math.abs(i)>=.005&&(u=i>0?"up":"down"),{trendRate:i,trendDirection:u,trendRatePct:metaReportRoundRatio(i*100),trendR:metaReportRoundRatio(r)}}function metaReportScenarioCalibrateElasticity(e=[],t=.75){if(e.length<META_SCENARIO_MIN_TREND_MONTHS)return t;const a=e.map(l=>metaReportParseAmount(l.spend)),n=Math.min(...a),s=Math.max(...a);if(n<=0||s<=0||(s-n)/n<.25)return t;const o=e.filter(l=>metaReportParseAmount(l.spend)>0&&metaReportParseAmount(l.leads)>0).map(l=>({x:Math.log(metaReportParseAmount(l.spend)),y:Math.log(metaReportParseAmount(l.leads))}));if(o.length<META_SCENARIO_MIN_TREND_MONTHS)return t;const{slope:r,r:i}=metaReportScenarioLinearRegression(o);return Math.abs(i)<.25?t:metaReportRoundRatio(Math.max(.65,Math.min(.8,r)))}function metaReportComputeScenarioEfficiency(e=[],{trendMethod:t="recency_weighted",elasticityPreset:a="balanced"}={}){const n=normalizeMetaReportScenarioTrendMethod(t),s=META_REPORT_SCENARIO_ELASTICITY_VALUES[normalizeMetaReportScenarioElasticity(a)]||.75,o=metaReportScenarioCalibrateElasticity(e,s),r=metaReportScenarioRecencyEfficiency(e),i=metaReportScenarioPooledEfficiency(e);if(!r||r.avgCpl<=0)return{efficiency:null,elasticity:o,presetElasticity:s,trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0,trendMethod:n};let l=r,d={trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};if(n==="robust_trend")d=metaReportScenarioMonthlyTrend(e,o);else if(n==="conservative_blend"){l=metaReportScenarioBlendEfficiency(i,r,.5);const c=metaReportScenarioMonthlyTrend(e,o);Math.abs(c.trendR)>=.35&&(d=c)}return{efficiency:l,elasticity:o,presetElasticity:s,...d,trendMethod:n}}function metaReportProjectBudgetAtSpend({baselineSpend:e,spend:t,efficiency:a,elasticity:n=.75,trendAdj:s=1,hasBottomline:o=!1}={}){const r=metaReportParseAmount(e),i=metaReportParseAmount(t);if(i<=0||r<=0||!a||a.avgCpl<=0)return{spend:0,leads:0,wonLeads:0,totalLeadValue:0,roasKr:0,roasX:0,poasKr:null,poasX:null};const l=r/a.avgCpl,d=i/r,c=metaReportParseAmount(s)>0?metaReportParseAmount(s):1,u=Math.max(0,Math.round(l*d**n*c)),p=Math.round(u*a.winRate),m=metaReportRoundMoney(p*a.avgLeadValue),f=metaReportRoundMoney(m-i),w=i>0?metaReportRoundRatio(f/i):0;let S=null,h=null;if(o&&a.avgProfitPerWon>0){const v=metaReportRoundMoney(p*a.avgProfitPerWon);S=metaReportRoundMoney(v-i),h=i>0?metaReportRoundRatio(S/i):0}return{spend:i,leads:u,wonLeads:p,totalLeadValue:m,roasKr:f,roasX:w,poasKr:S,poasX:h}}function metaReportProjectBudgetScenario({series:e=[],baselineSpend:t,multiplier:a=1,hasBottomline:n=!1,trendMethod:s="recency_weighted",monthWindow:o="6",elasticityPreset:r="balanced",baselineMode:i="year",activeMonthKey:l=null,asOfDate:d=new Date,smoothOutliers:c=!0}={}){const u=metaReportPrepareScenarioSeries(e,{windowMonths:o,asOfDate:d,smoothOutliers:c}),p=metaReportComputeScenarioEfficiency(u.months,{trendMethod:s,elasticityPreset:r});let m=metaReportParseAmount(t);if(!m){if(i==="month"&&l){const R=u.months.find(L=>L.monthKey===l)||e.find(L=>L.monthKey===l);R&&metaReportParseAmount(R.spend)>0&&(m=metaReportParseAmount(R.spend))}if(!m&&u.months.length>0){const R=u.months.reduce((L,B)=>L+metaReportParseAmount(B.spend),0);m=metaReportRoundMoney(R/u.months.length)}}const f=metaReportParseAmount(a)||1,{efficiency:w,elasticity:S,trendRate:h}=p;if(!w||w.avgCpl<=0||m<=0||u.monthsUsed<META_SCENARIO_MIN_EFFICIENCY_MONTHS)return{...p,prepared:u,baseline:metaReportProjectBudgetAtSpend({baselineSpend:0,spend:0,efficiency:w}),projected:metaReportProjectBudgetAtSpend({baselineSpend:0,spend:0,efficiency:w}),projectedConservative:null,projectedOptimistic:null,multiplier:f,baselineSpend:m,hasBottomline:n,insufficientData:!0};const v=Math.exp(h*1),g=metaReportProjectBudgetAtSpend({baselineSpend:m,spend:m,efficiency:w,elasticity:S,trendAdj:1,hasBottomline:n}),y=metaReportProjectBudgetAtSpend({baselineSpend:m,spend:metaReportRoundMoney(m*f),efficiency:w,elasticity:S,trendAdj:v,hasBottomline:n}),b=metaReportProjectBudgetAtSpend({baselineSpend:m,spend:metaReportRoundMoney(m*f),efficiency:w,elasticity:Math.max(.65,S-.05),trendAdj:Math.exp(Math.min(h,0)*1),hasBottomline:n}),$=metaReportProjectBudgetAtSpend({baselineSpend:m,spend:metaReportRoundMoney(m*f),efficiency:w,elasticity:Math.min(.8,S+.05),trendAdj:Math.exp(Math.max(h,0)*1),hasBottomline:n});return{...p,prepared:u,baseline:g,projected:y,projectedConservative:b,projectedOptimistic:$,multiplier:f,baselineSpend:m,hasBottomline:n,insufficientData:!1}}function metaReportSpendAxisTick(e){const t=Number(e)||0;return Math.abs(t)>=1e6?`${(t/1e6).toFixed(1)}M`:Math.abs(t)>=1e3?`${Math.round(t/1e3)}K`:t}function metaReportSpendMultiplierValue(e,t){const a=metaReportParseAmount(t),n=metaReportParseAmount(e);return a<=0?0:metaReportRoundRatio(n/a)}function resolveMetaReportSpendChartBaseline(e,t=null){const a=metaReportParseAmount(t?.baselineSpend);return a>0?a:metaReportAggregateSeriesEfficiency(e).avgMonthlySpend}function trimMetaReportScenarioSeries(e,t=META_REPORT_SCENARIO_MAX_HISTORICAL){const a=e.filter(n=>metaReportParseAmount(n.spend)>0);return a.length>t?a.slice(-t):a}function buildMetaReportScenarioProjectionSteps(e,{months:t=META_REPORT_SCENARIO_PROJECTED_MONTHS,hasBottomline:a=!1}={}){if(!e||e.insufficientData)return[];const n=metaReportParseAmount(e.baselineSpend),s=metaReportParseAmount(e.multiplier)||1,o=e.efficiency,r=e.elasticity||.75,i=e.trendRate||0,l=[];for(let d=1;d<=t;d+=1){const c=Math.min(s,1+d),u=metaReportRoundMoney(n*c),p=Math.exp(i*d),m=metaReportProjectBudgetAtSpend({baselineSpend:n,spend:u,efficiency:o,elasticity:r,trendAdj:p,hasBottomline:a});l.push({label:`+${d} mo`,spendMultiplier:metaReportRoundRatio(c),spend:m.spend,leads:m.leads,wonLeads:m.wonLeads,totalLeadValue:m.totalLeadValue,roasKr:m.roasKr,roasX:m.roasX,poasKr:m.poasKr,poasX:m.poasX})}return l}function metaReportDeltaPct(e,t){const a=metaReportParseAmount(t),n=metaReportParseAmount(e);return a?metaReportRoundRatio((n-a)/Math.abs(a)*100):null}function metaReportResolveBaselineSpend(e,t,a=null){if(a?.baselineSpend>0)return metaReportParseAmount(a.baselineSpend);if(metaReportsState.budgetBaseline==="month"&&metaReportsState.activeMonthKey){const s=t.find(o=>o.monthKey===metaReportsState.activeMonthKey);if(s&&metaReportParseAmount(s.spend)>0)return metaReportParseAmount(s.spend)}const n=a?.prepared||metaReportPrepareScenarioSeries(t,{windowMonths:metaReportsState.scenarioMonthWindow});if(n.months.length>0){const s=n.months.reduce((o,r)=>o+metaReportParseAmount(r.spend),0);return metaReportRoundMoney(s/n.months.length)}return metaReportAggregateSeriesEfficiency(t).avgMonthlySpend}function resolveMetaReportScenarioChartSeries(e,t){if(t?.prepared?.months?.length){const a=new Map(e.map(n=>[n.monthKey,n]));return t.prepared.months.map(n=>{const s=a.get(n.monthKey)||{};return{...s,...n,label:s.label||metaMonthLabel(n.monthKey)}})}return trimMetaReportScenarioSeries(e.filter(a=>metaReportParseAmount(a.spend)>0))}function renderMetaReportScenarioContextStrip(e){if(!e||e.insufficientData)return'<p class="meta-report-scenario-context" id="meta-report-scenario-context">Add at least two completed ad months to model a budget scenario.</p>';const t=describeActiveMetaReportScenarioPills(),a=t.length?t.join(" \xB7 "):"Recent months only",n=e.trendDirection==="up"?`trend up ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:e.trendDirection==="down"?`trend down ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:"trend flat",s=META_REPORT_SCENARIO_ELASTICITY_OPTIONS.find(r=>r.value===metaReportsState.scenarioElasticity)?.label||"Balanced scaling",o=e.prepared?.outliersAdjusted?` \xB7 ${e.prepared.outliersAdjusted} uneven month${e.prepared.outliersAdjusted===1?"":"s"} smoothed`:"";return`<p class="meta-report-scenario-context" id="meta-report-scenario-context"><strong>Active:</strong> ${esc(a)} \xB7 last ${e.prepared?.monthsUsed||0} ad months \xB7 ${esc(n)} \xB7 ${esc(s)}${esc(o)}</p>`}function payloadHasMetaReportBottomline(e){return e?.settings?.metaReportShowBottomline===!1?!1:seriesHasBottomline(metaReportsState.chartSeries||buildMetaReportChartSeries(e))}function seriesHasBottomline(e=[]){return e.some(t=>t.poasKr!=null&&t.poasX!=null)}function renderMetaReportChartsPanel({editable:e=!1}={}){const t=metaReportsState.chartTab||"trend";return`
  ...
  `}function renderMetaReportScenarioCard(){return`
    <section class="meta-report-scenario-card" id="meta-report-scenario-card">
      ...
      <div class="meta-report-scenario-controls">
        ...
        <label class="meta-report-scenario-field">
          ...
          <select id="meta-report-budget-baseline" class="admin-select">
            <option value="year"${metaReportsState.budgetBaseline==="year"?" selected":""}>Window average</option>
            <option value="month"${metaReportsState.budgetBaseline==="month"?" selected":""}>Selected month</option>
          </select>
        </label>
        <label class="meta-report-scenario-field">
      ...
      ${renderMetaReportScenarioElasticityPills()}
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null}={}){let n="";t&&e&&!e.meta?.emptyMonth&&(n=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        <h3>Edit month inputs</h3>
        <div class="meta-report-edit-grid">
          <label>Period start<input type="date" id="meta-report-period-start" value="${esc(e.periodStart||"")}" /></label>
          <label>Period end<input type="date" id="meta-report-period-end" value="${esc(e.periodEnd||"")}" /></label>
    ...
    `);const s=t?"Report preview":"Report",o=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${s}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",r=renderMetaReportMonthTables(e,{editable:t}),i=shouldShowMetaReportYearVisuals(e,a,{editable:t}),l=i?renderMetaReportChartsPanel({editable:t}):"",d=t&&i?renderMetaReportScenarioCard():"",c=i?`
    <div class="meta-report-content-layout">
      <div class="meta-report-content-main">${r}</div>
      ${l}
    </div>
    ${d}
  `:`
    <div class="meta-report-content-main">${r}</div>
  `;return`${n}${o}${c}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t,t&&(metaReportsState.clientShareExpanded=!1);const a=document.querySelector(".meta-report-bottomline-fee-settings--client"),n=document.getElementById(`${e}-fee-editor`);a?.classList.toggle("is-editing",t),a?.classList.toggle("is-collapsed",!t),n&&n.classList.toggle("is-hidden",!t);const s=document.getElementById("meta-report-share-summary"),o=document.getElementById("meta-report-share-editor");s&&(s.hidden=metaReportsState.clientShareExpanded),o&&(o.hidden=!metaReportsState.clientShareExpanded),syncMetaReportControlPanelUi()}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const a=document.getElementById(`${t}-fee-summary`);if(!a)return;const n=document.createElement("div");n.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const s=n.firstElementChild;s&&a.replaceWith(s);const o=document.getElementById(`${t}-fee-edit`);o&&(o.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:a=!1,variant:n="default",collapsible:s=!1,externalSummary:o=!1}={}){const r=!!e.metaReportShowBottomline,i=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",d=resolveMetaReportFeeMode(e),c=e.metaReportFeePercent??20,u=e.metaReportMarketingFeeAmount??0,p=metaReportUsesInlineFeeValue(t),m=`data-fee-mode="${esc(l)}" data-fee-enabled="${i?"true":"false"}" data-fee-percent="${esc(c)}" data-marketing-fee="${esc(u)}"`;if(n==="cv"){const g=describeMetaReportFeeModeLabel(d,c,u),y=META_REPORT_FEE_MODE_META.map(({value:b,title:$})=>`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${r?"":" is-hidden"}" id="${t}-fee-nested" ${m}>
          ...
          <div class="meta-cv-fee-footer">
            <div class="meta-cv-fee-value-inline" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,d,c,u,{inline:!0})}</div>
            <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save" disabled>Saved</button>
          </div>
        </div>
      </div>
    </div>
  `}const f=describeMetaReportFeeModeLabel(d,c,u),w=META_REPORT_FEE_MODE_META.map(({value:g,icon:y,title:b,desc:$})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(g)}"${d===g?" checked":""} />
          <span class="meta-report-fee-mode-card-icon" aria-hidden="true">${esc(y)}</span>
          <span class="meta-report-fee-mode-card-text">
            <span class="meta-report-fee-mode-card-title">${esc(b)}</span>
            <small>${esc($)}</small>
          </span>
          <span class="meta-report-fee-mode-card-radio" aria-hidden="true">${ICON_CHECK}</span>
        </label>`).join(""),S=s?metaReportsState.clientReportSettingsExpanded:!0,h=s?`
    ...
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?S?" is-editing":" is-collapsed":""}">
      ${v}
      <div class="meta-report-bottomline-editor${s&&!S?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${r?"":" is-hidden"}" id="${t}-fee-nested" ${m}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
            <span class="meta-report-fee-status-value is-active">${esc(f)}</span>
          </div>
          <div class="meta-report-fee-mode-grid${a?" meta-report-fee-mode-grid--rows":""}">${w}
          </div>
          <div class="meta-report-fee-value" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,d,c,u,{inline:p})}</div>
          ${h}
        </div>
      </div>
    </div>
  `}function syncMetaReportBottomlineFeeDom(e={},t="meta-report"){const a=!!e.metaReportShowBottomline,n=!!e.metaReportFeeEnabled,s=e.metaReportFeeMode||"",o=resolveMetaReportFeeMode(e),r=e.metaReportFeePercent??20,i=e.metaReportMarketingFeeAmount??0,l=document.getElementById(`${t}-setting-bottomline`);l&&(l.checked=a);const d=document.getElementById(`${t}-fee-nested`);d?.classList.toggle("is-hidden",!a),d&&(d.dataset.feeMode=s,d.dataset.feeEnabled=n?"true":"false",d.dataset.feePercent=String(r),d.dataset.marketingFee=String(i)),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(p=>{p.checked=p.value===o});const c=metaReportUsesInlineFeeValue(t),u=document.getElementById(`${t}-fee-value`);u&&(u.innerHTML=metaReportFeeValueBlockHtml(t,o,r,i,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),syncMetaReportFeeSelectionState(t),t==="meta-report"&&updateMetaReportBottomlineFeeSummary(e,t)}function bindMetaReportFeeValueInputEnterKey(e){const t=document.getElementById(`${e}-setting-fee-percent`)||document.getElementById(`${e}-setting-marketing-fee`);t&&(t.onkeydown=a=>{a.key==="Enter"&&(a.preventDefault(),document.getElementById(`${e}-fee-save`)?.click())})}function bindMetaReportBottomlineFeeEvents(e,t,a){const n=a||((l,d)=>saveMetaReportClientSettings(e,l,d)),s=document.getElementById(`${t}-fee-edit`);s&&(s.onclick=()=>setMetaReportClientSettingsExpanded(t,!0));const o=document.getElementById(`${t}-fee-done`);o&&(o.onclick=()=>{const l=metaReportsState.clientPayload?.settings||{};syncMetaReportBottomlineFeeDom(l,t),setMetaReportClientSettingsExpanded(t,!1),updateMetaReportBottomlineFeeSummary(l,t)});const r=document.getElementById(`${t}-setting-bottomline`);r&&(r.onchange=()=>{const l=r.checked;n({metaReportShowBottomline:l},()=>{r.checked=!l})}),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(l=>{l.onchange=()=>{if(!l.checked)return;const d=document.getElementById(`${t}-fee-nested`),c=metaReportUsesInlineFeeValue(t),u=document.getElementById(`${t}-fee-value`);u&&d&&(u.innerHTML=metaReportFeeValueBlockHtml(t,l.value||"",d.dataset.feePercent??20,d.dataset.marketingFee??0,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),bindMetaReportFeeAmountInputs(t),syncMetaReportFeeSelectionState(t)}}),bindMetaReportFeeValueInputEnterKey(t),bindMetaReportFeeDraftTracking(t);const i=document.getElementById(`${t}-fee-save`);i&&(i.onclick=async()=>{if(i.disabled)return;const l=document.getElementById(`${t}-fee-nested`),d=l?.querySelector(`input[name="${t}-fee-mode"]:checked`),c=d&&d.value||null,u={metaReportFeeMode:c};if(c==="performance"){const m=document.getElementById(`${t}-setting-fee-percent`);let f=Number(m?.value);Number.isFinite(f)||(f=20),f=Math.min(100,Math.max(0,f)),m&&(m.value=String(f)),u.metaReportFeePercent=f}else if(c==="marketing"){const m=document.getElementById(`${t}-setting-marketing-fee`);let f=Number(m?.value);Number.isFinite(f)||(f=0),f=Math.max(0,f),m&&(m.value=String(f)),u.metaReportMarketingFeeAmount=f}const p=i.textContent;i.disabled=!0,i.textContent="Saving\u2026",i.classList.remove("is-dirty");try{await n(u,()=>syncMetaReportBottomlineFeeDom({metaReportShowBottomline:r?r.checked:!0,metaReportFeeMode:l?.dataset.feeMode||null,metaReportFeePercent:Number(l?.dataset.feePercent)||20,metaReportMarketingFeeAmount:Number(l?.dataset.marketingFee)||0},t)),syncMetaReportFeeSelectionState(t)}finally{i.disabled=!1,i.textContent=p,syncMetaReportFeeSelectionState(t)}})}function renderMetaReportSaveIndicator(e,t=!1){return`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(w=>w.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const s=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,s);const o=t.editorPayload?.months?.[n]||null,r=a.metaReportShowBottomline,i=s.wonLeads!=null?s.wonLeads:o?.inputs?.wonLeads??o?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:o?.inputs?.avgLeadValue??o?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:o?.inputs?.avgProfitPerWon??"",c=metaReportsState.customValues.draftInputs,u=c?.wonLeads??i,p=c?.avgLeadValue??l,m=c?.avgProfitPerWon??d,f=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(w=>`
            <option value="${esc(w.clientId)}"${w.clientId===a.clientId?" selected":""}>${esc(w.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(w=>`
            <option value="${esc(w)}"${w===n?" selected":""}>${esc(metaMonthLabel(w))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
      ...
      <label>Avg profit per won${r?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(m)}" />
      </label>
    </div>
    <div class="meta-report-settings-group meta-report-settings-group--stacked meta-cv-client-settings${f?" is-saving":""}">
      ${renderMetaReportSaveIndicator("meta-cv-settings-status",f)}
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(a){showToast(a.message,"error")}finally{if(t){t.disabled=!1;const a=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${a}`:a}}}function getMetricsModelLabels(e,t=setupPipelines){const a=new Map((t||[]).map(n=>[n.id,n.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),a=!e.metricsModelSetAt||metricsModelChangeMode,n=a?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",o=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",r=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return a?`
    ...
    <div class="date-picker-panel">
      <div class="date-picker-header">
        ...
        <div class="date-picker-nav">
          ...
          <button type="button" class="date-picker-nav-btn" aria-label="Next month" onclick="shiftDatePickerMonth(1)" ${b?"":"disabled"}>${ICON_CHEVRON_RIGHT}</button>
        </div>
      </div>
      <div class="date-picker-weekdays">
        ${o.map($=>`<div class="date-picker-weekday">${$}</div>`).join("")}
      </div>
      <div class="date-picker-grid">${w}</div>
      ...
      <div class="section-grid">
        ${isVisible("sections","assigneeReport")?`
          <div class="card">
            ...
            ${renderTable("assigneeReport","Assignee",[{key:"won",label:"Won",align:"num"},{key:"totalLeads",label:"Total leads",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"},{key:"totalValue",label:"Total value",align:"num"}],s,(l,d)=>d==="label"?esc(l.assigneeName):d==="wonValue"||d==="totalValue"?`Dkr ${fmt(l[d])}`:fmt(l[d]))}
          </div>
        `:""}
        ${isVisible("sections","pipelineBreakdown")?`
          <div class="card">
            <div class="section-title">Pipeline Breakdown</div>
            ${renderTable("pipelineBreakdown","Pipeline",[{key:"count",label:"Leads",align:"num"},{key:"won",label:"Won",align:"num"},{key:"monetary",label:"Value",align:"num"},{key:"profit",label:"Bundlinje",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"}],o,(l,d)=>d==="label"?esc(l.name):d==="monetary"||d==="profit"||d==="wonValue"?`Dkr ${fmt(l[d])}`:fmt(l[d]))}
          </div>
        `:""}
      </div>
    `:""}
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:a=!1,forceFresh:n=!1}=t,s=!!n,o=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const r=!!document.getElementById("dashboard-content"),i=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!r)o.innerHTML=`
        ...
        `)}`;else if(!a){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),i&&(i.disabled=!0,i.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const p=canReuseBootstrapDashboard(c)&&!s;let m,f;if(p?[f,m]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[m,f]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=f,cachedData=applyMarketingData(m,f),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(l!==fetchGeneration)return;if(c.name==="AbortError"){r?restoreDashboardContentAfterFailedFetch(r):o.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            </div>`)}else r?r&&cachedData?(restoreDashboardContentAfterFailedFetch(r),showToast(c.message||"Failed to load filtered data.","error")):i&&(i.textContent="Apply failed - try again"):o.innerHTML=`
          ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
          ${wrapDashboardShell(`
            <div class="error-state">
              <div>Fejl ved hentning af data</div>
