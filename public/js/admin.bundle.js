...
Resume the interrupted ${v}?`)&&(l=c.id,d=Number(c.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const c=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),u=c.summary||c;l=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||g?.updated||0});do{p=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:S,batchOffset:d,batchLimit:o})});const c=p.summary||p;l=c.runId,d=c.nextBatchOffset??c.batchOffset+(c.batchProcessed||0),b=b.concat(c.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const u=c.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:u,complete:!c.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...c,rows:b}))):updateGlobalApplyProgress({processed:d,total:u,written:c.updated||0,complete:!c.hasMore})}while((p.summary||p).hasMore);y=!0;const $=p.summary||p,m=$.inWindow||d||w,h=$.nextBatchOffset??d;if(n)setFbLeadProgress({processed:h,total:m,complete:!0}),markFbLeadPreviewReady(e,t,$.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=($.updated||0)>0),showToast(($.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:m,written:$.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const c=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete c[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(c))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const f=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=f.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch($){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${$.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast(($.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!y&&n&&hideFbLeadProgress(!0),!n&&!y&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `}function getMetaReportMonthBodyOptions(e={},{editable:t=!1}={}){return{editable:t,showOtherToClient:e.metaReportShowOther!==!1}}function renderMetaReportMonthBody(e,{editable:t=!1,showOtherToClient:n=!0}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return`<div class="meta-report-empty">No ad spend recorded for this month yet.${t?' Click "Refresh from Meta" below, or use "Backfill from Meta" above to pull historical months.':""}</div>`;let a="";t&&(a=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        ...
        <div class="meta-report-line-items-editor">
          ...
          <div id="meta-report-line-items-list">
            ${(e.customLineItems||[]).map((b,y)=>`
              <div class="meta-report-line-item-row" data-index="${y}">
                <input type="text" class="meta-line-label" value="${esc(b.label)}" placeholder="Label" />
                <input type="number" step="any" class="meta-line-amount" value="${esc(b.amount)}" placeholder="Amount" />
    ...
    `);const s=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["CTR",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],i=[["Leads",metaFmtNum(e.topline?.leads),""],["Cost Per Lead (CPL)",metaFmtKr(e.topline?.cpl),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let o="";if(o+=renderMetaReportMetricTable("Meta ads",s,"meta"),o+=renderMetaReportMetricTable("Topline KPI'er",i,"topline",{highlightLastN:2}),e.bottomline){const b=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];e.bottomline.censioFee>0&&(b.push([`Censio performance fee (${metaFmtNum(e.bottomline.feePercent,0)}%)`,metaFmtKr(e.bottomline.censioFee),""]),b.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),b.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])),o+=renderMetaReportMetricTable("Bottomline KPI'er",b,"bottomline",{highlightLastN:2})}if(!!e.customLineItems?.length&&(t||n)){const b=e.customLineItems.map(y=>[y.label,metaFmtKr(y.amount),""]);b.push(["Total",metaFmtKr(e.customLineItemsTotal),""]),o+=renderMetaReportMetricTable("Other",b,"other",{badge:t&&!n?"Hidden from client":"",highlightLastN:1})}const d=`
  ...
  `:"";return`${a}${d}${p}`}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
    ...
    `)}
  `}function renderMetaReportsClientPage(e){const t=e.monthKeys||[],n=metaReportsState.activeMonthKey||t[t.length-1]||"",a=e.months?.[n]||null,s=e.settings||{},i=!!s.metaReportFeeEnabled,o=t.map(r=>`
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      <div class="meta-report-toolbar">
        <div class="meta-report-toolbar-left">
          ...
          ${(()=>{const r=metaReportMonthsNeedingBackfill(e);return r.length?`
              <div class="meta-report-backfill-wrap">
                <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-report-backfill">${ICON_SYNC} Backfill ${r.length} month${r.length===1?"":"s"} from Meta</button>
                <span class="meta-report-backfill-progress" id="meta-report-backfill-progress"></span>
              </div>
            `:""})()}
        </div>
      ...
      <div class="meta-report-settings">
        <div class="meta-report-settings-group">
          ...
          <label class="meta-report-switch">
            <span class="meta-report-switch-track">
              <input type="checkbox" class="meta-report-switch-input" id="meta-report-setting-fee" ${i?"checked":""} />
              <span class="meta-report-switch-thumb"></span>
            </span>
            <span class="meta-report-switch-label">Censio performance fee</span>
          </label>
          <label class="meta-report-fee-field${i?"":" is-disabled"}" id="meta-report-fee-field">Fee %
            <input type="number" step="any" min="0" max="100" id="meta-report-setting-fee-percent" value="${esc(s.metaReportFeePercent??20)}" ${i?"":"disabled"} />
          </label>
        </div>
  ...
  `}function renderMetaReportSwitch(e,t,n,a=!1){return`
    <div class="meta-report-switch">
      ...
      <span class="meta-report-switch-label">${esc(n)}</span>
    </div>
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find(c=>c.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const s=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,s);const i=t.editorPayload?.months?.[a]||null,o=n.metaReportShowBottomline,r=s.wonLeads!=null?s.wonLeads:i?.inputs?.wonLeads??i?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:i?.inputs?.avgLeadValue??i?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:i?.inputs?.avgProfitPerWon??"",p=i?.customLineItems||[],b=n.metaReportShowOther!==!1,y=metaReportsState.customValues.draftInputs,S=y?.wonLeads??r,g=y?.avgLeadValue??l,w=y?.avgProfitPerWon??d,$=y?.lineItems??p,m=!!n.metaReportFeeEnabled,h=t.settingsSaving&&t.settingsSavingScope==="other",f=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-report-settings meta-cv-client-settings${f?" is-saving":""}">
      ...
      <div class="meta-report-settings-group">
        ...
        ${renderMetaReportSwitch("meta-cv-setting-fee",m,"Censio performance fee")}
        <label class="meta-report-fee-field${m?"":" is-disabled"}" id="meta-cv-fee-field">Fee %
          <input type="number" step="any" min="0" max="100" id="meta-cv-setting-fee-percent" value="${esc(n.metaReportFeePercent??20)}" ${m?"":"disabled"} />
        </label>
      </div>
    </div>
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(c=>`
            <option value="${esc(c.clientId)}"${c.clientId===n.clientId?" selected":""}>${esc(c.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(c=>`
            <option value="${esc(c)}"${c===a?" selected":""}>${esc(metaMonthLabel(c))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
    ...
    <div class="meta-report-line-items-editor meta-cv-line-items-editor${h?" is-settings-saving":""}">
      <div class="meta-report-line-items-head meta-cv-line-items-head">
        <div class="meta-cv-line-items-head-row">
          <h4>Other items</h4>
          <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-cv-add-line"${!b||t.settingsSaving?" disabled":""}>Add row</button>
        </div>
        <div class="meta-cv-line-items-toggle-row">
          ${renderMetaReportSwitch("meta-cv-setting-other",b,'Show "Other" section to client')}
          ${renderMetaReportSaveIndicator("meta-cv-other-settings-status",h)}
        </div>
      </div>
      <div id="meta-cv-line-items-list">
        ${$.length?$.map((c,u)=>`
          <div class="meta-report-line-item-row" data-index="${u}">
            <input type="text" class="meta-cv-line-label" placeholder="Label" value="${esc(c.label||"")}" />
            <input type="number" step="any" class="meta-cv-line-amount" placeholder="Amount" value="${esc(c.amount??"")}" />
  ...
  `}function refreshMetaCvListAndEditor(){const e=metaReportsState.customValues.overview;if(!e)return;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e));const n=document.getElementById("meta-cv-editor");n&&(n.innerHTML=renderMetaCvEditor(e)),metaReportsState.customValues.draftInputs=null,bindMetaCvEditorEvents(),bindMetaCvListEvents()}function setMetaCvSavePending(e){metaReportsState.customValues.saving=e;const t=document.getElementById("meta-cv-save"),n=document.getElementById("meta-cv-save-indicator");n&&n.classList.toggle("is-visible",e),t&&(t.disabled=e||metaReportsState.customValues.loadingEditor||metaReportsState.customValues.settingsSaving,t.textContent=e?"Saving\u2026":"Save values")}function setMetaCvSettingsSaving(e,t="report"){metaReportsState.customValues.settingsSaving=e,metaReportsState.customValues.settingsSavingScope=e?t:null;const n=e&&t==="report",a=e&&t==="other";document.getElementById("meta-cv-settings-status")?.classList.toggle("is-visible",n),document.getElementById("meta-cv-other-settings-status")?.classList.toggle("is-visible",a),document.querySelector(".meta-cv-client-settings")?.classList.toggle("is-saving",n),document.querySelector(".meta-cv-line-items-editor")?.classList.toggle("is-settings-saving",a),["meta-cv-setting-bottomline","meta-cv-setting-fee","meta-cv-setting-fee-percent"].forEach(r=>{const l=document.getElementById(r);l&&(l.disabled=n)});const i=document.getElementById("meta-cv-setting-other");i&&(i.disabled=a),syncMetaCvOtherEditorState(i?.checked??!1,{saving:e});const o=document.getElementById("meta-cv-save");o&&(o.disabled=e||metaReportsState.customValues.saving||metaReportsState.customValues.loadingEditor)}function syncMetaCvOtherEditorState(e,{saving:t=!1}={}){const n=document.getElementById("meta-cv-add-line");n&&(n.disabled=!e||t)}function classifyMetaCvMonthStatus(e,t=!1){if(!e)return"empty";const n=e.wonLeads!=null,a=e.avgLeadValue!=null,s=e.avgProfitPerWon!=null,i=Number(e.lineItemCount)||0;return n&&a&&(!t||s)?"complete":n||a||s||i>0?"partial":"empty"}function applyMetaCvSaveResult(e,t,n,a){const i=metaReportsState.customValues.overview?.clients?.find(l=>l.clientId===e),o=i?.months?.[t],r=a?.month||{};o&&(o.wonLeads=r.wonLeads??n.wonLeads,o.avgLeadValue=r.avgLeadValue??n.avgLeadValue,o.avgProfitPerWon=r.avgProfitPerWon??n.avgProfitPerWon,o.lineItemCount=r.lineItems?.length??n.lineItems?.length??0,o.status=classifyMetaCvMonthStatus(o,!!i.metaReportShowBottomline),o.updatedAt=o.status==="empty"?null:r.updatedAt||new Date().toISOString()),a?.monthPayload&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:a.monthPayload}})}async function refreshMetaCvOverviewSilently(){const e=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(e.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=e;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e),bindMetaCvListEvents());const n=document.querySelector(".meta-cv-summary");if(n&&e.summary){const a=e.summary;n.innerHTML=`
    ...
    `}}function collectMetaCvLineItems(){return Array.from(document.querySelectorAll("#meta-cv-line-items-list .meta-report-line-item-row")).map((e,t)=>({label:String(e.querySelector(".meta-cv-line-label")?.value||"").trim(),amount:Number(e.querySelector(".meta-cv-line-amount")?.value)||0,sortOrder:t})).filter(e=>e.label)}function collectMetaCvSaveBody(){return{wonLeads:document.getElementById("meta-cv-won-leads")?.value===""?null:Number(document.getElementById("meta-cv-won-leads")?.value),avgLeadValue:document.getElementById("meta-cv-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-cv-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-cv-avg-profit")?.value===""?null:Number(document.getElementById("meta-cv-avg-profit")?.value),lineItems:collectMetaCvLineItems()}}function syncMetaCvClientSettingsDom(e={}){const t=!!e.metaReportFeeEnabled,n=document.getElementById("meta-cv-setting-bottomline"),a=document.getElementById("meta-cv-setting-fee"),s=document.getElementById("meta-cv-setting-fee-percent"),i=document.getElementById("meta-cv-setting-other");n&&(n.checked=!!e.metaReportShowBottomline),a&&(a.checked=t),i&&(i.checked=e.metaReportShowOther!==!1),s&&e.metaReportFeePercent!=null&&(s.value=e.metaReportFeePercent??20,s.dataset.prevValue=String(s.value)),syncMetaReportFeeFieldState(t,"meta-cv"),syncMetaCvOtherEditorState(e.metaReportShowOther!==!1)}async function saveMetaCvClientSettings(e,t,n){const a=Object.prototype.hasOwnProperty.call(t,"metaReportShowOther")?"other":"report";setMetaCvSettingsSaving(!0,a);try{const s=metaReportsState.customValues.selectedMonthKey,i=await patchMetaReportSettings(e,{...t,monthKey:s||void 0}),r=metaReportsState.customValues.overview?.clients?.find(l=>l.clientId===e);r&&i.settings&&(r.metaReportShowBottomline=!!i.settings.metaReportShowBottomline,r.metaReportFeeEnabled=!!i.settings.metaReportFeeEnabled,r.metaReportFeePercent=i.settings.metaReportFeePercent??20,r.metaReportShowOther=i.settings.metaReportShowOther!==!1),i.monthPayload&&s&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[s]:i.monthPayload}}),setMetaCvSettingsSaving(!1),syncMetaCvClientSettingsDom(i.settings||{}),showToast(describeMetaReportSettingsChange(t,i.settings||{}),"success")}catch(s){showToast(s.message||"Update failed","error"),n&&n(),setMetaCvSettingsSaving(!1)}}function bindMetaCvSettingsEvents(e){const t=document.getElementById("meta-cv-setting-bottomline"),n=document.getElementById("meta-cv-setting-fee"),a=document.getElementById("meta-cv-setting-fee-percent"),s=document.getElementById("meta-cv-setting-other");if(syncMetaReportFeeFieldState(!!n?.checked,"meta-cv"),syncMetaCvOtherEditorState(!!s?.checked),t&&(t.onchange=()=>{const i=t.checked;saveMetaCvClientSettings(e,{metaReportShowBottomline:i},()=>{t.checked=!i})}),n&&(n.onchange=()=>{const i=n.checked;syncMetaReportFeeFieldState(i,"meta-cv");const o={metaReportFeeEnabled:i};i&&t&&!t.checked&&(t.checked=!0,o.metaReportShowBottomline=!0),saveMetaCvClientSettings(e,o,()=>{n.checked=!i,Object.prototype.hasOwnProperty.call(o,"metaReportShowBottomline")&&t&&(t.checked=!1),syncMetaReportFeeFieldState(n.checked,"meta-cv")})}),a){a.dataset.prevValue=a.value;const i=()=>{if(a.disabled)return;const o=a.dataset.prevValue;let r=Number(a.value);Number.isFinite(r)||(r=20),r=Math.min(100,Math.max(0,r)),a.value=String(r),String(r)!==String(o)&&(a.dataset.prevValue=String(r),saveMetaCvClientSettings(e,{metaReportFeePercent:r},()=>{a.value=o,a.dataset.prevValue=o}))};a.onchange=i,a.onkeydown=o=>{o.key==="Enter"&&(o.preventDefault(),a.blur())}}s&&(s.onchange=()=>{const i=s.checked;syncMetaCvOtherEditorState(i),saveMetaCvClientSettings(e,{metaReportShowOther:i},()=>{s.checked=!i,syncMetaCvOtherEditorState(!i)})})}async function selectMetaCvMonth(e,t,{reloadEditor:n=!0}={}){if(metaReportsState.customValues.selectedClientId=e,metaReportsState.customValues.selectedMonthKey=t,refreshMetaCvListAndEditor(),!!n){metaReportsState.customValues.loadingEditor=!0,refreshMetaCvListAndEditor();try{const a=String(t).slice(0,4),s=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}?year=${encodeURIComponent(a)}`);metaReportsState.customValues.selectedClientId===e&&metaReportsState.customValues.selectedMonthKey===t&&(metaReportsState.customValues.editorPayload=s)}catch(a){showToast(a.message||"Failed to load month","error")}finally{metaReportsState.customValues.loadingEditor=!1,refreshMetaCvListAndEditor()}}}function bindMetaCvListEvents(){document.querySelectorAll("[data-meta-cv-month]").forEach(e=>{e.onclick=()=>{selectMetaCvMonth(e.getAttribute("data-meta-cv-client"),e.getAttribute("data-meta-cv-month"))}})}function bindMetaCvEditorEvents(){const e=metaReportsState.customValues.selectedClientId;e&&bindMetaCvSettingsEvents(e);const t=document.getElementById("meta-cv-client-select"),n=document.getElementById("meta-cv-month-select");t&&(t.onchange=()=>{const i=metaReportsState.customValues.selectedMonthKey||metaReportsState.customValues.overview?.monthKeys?.slice(-1)[0];i&&selectMetaCvMonth(t.value,i)}),n&&(n.onchange=()=>{const i=metaReportsState.customValues.selectedClientId;i&&selectMetaCvMonth(i,n.value)});const a=document.getElementById("meta-cv-add-line");a&&(a.onclick=()=>{if(a.disabled)return;const i=document.getElementById("meta-cv-line-items-list");if(!i)return;i.querySelector(".note")&&(i.innerHTML="");const o=document.createElement("div");o.className="meta-report-line-item-row",o.innerHTML=`
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `:'<div class="pipeline-warning">Select at least one pipeline, then click Apply data filters.</div>'}function defaultDisplayPrefs(){const e={kpis:{},sections:{},charts:getDefaultChartPrefs(),statusItems:{},columns:{}};return Object.keys(DISPLAY_OPTIONS.kpis).forEach(t=>{e.kpis[t]=!0}),Object.keys(DISPLAY_OPTIONS.sections).forEach(t=>{e.sections[t]=!0}),Object.keys(DISPLAY_OPTIONS.charts).forEach(t=>{e.charts[t]=!0}),Object.keys(DISPLAY_OPTIONS.statusItems).forEach(t=>{e.statusItems[t]=!0}),Object.entries(DISPLAY_OPTIONS.columns).forEach(([t,n])=>{e.columns[t]={},Object.keys(n).forEach(a=>{e.columns[t][a]=!0})}),e}function loadDisplayPrefs(){if(IS_CLIENT_VIEW&&!IS_PREVIEW)return defaultDisplayPrefs();try{const e=JSON.parse(localStorage.getItem(STORAGE_KEY)||localStorage.getItem(LEGACY_STORAGE_KEY)||"null");if(!e)return defaultDisplayPrefs();const t=defaultDisplayPrefs();return{kpis:{...t.kpis,...e.kpis},sections:{...t.sections,...e.sections},charts:{...t.charts,...e.charts},statusItems:{...t.statusItems,...e.statusItems},columns:{sourceReport:{...t.columns.sourceReport,...e.columns?.sourceReport||{}},assigneeReport:{...t.columns.assigneeReport,...e.columns?.assigneeReport||{}},pipelineBreakdown:{...t.columns.pipelineBreakdown,...e.columns?.pipelineBreakdown||{}}}}}catch{return defaultDisplayPrefs()}}function ensureChartsVisible(){const e=Object.keys(DISPLAY_OPTIONS.charts);e.some(n=>display.charts[n]!==!1)||(e.forEach(n=>{display.charts[n]=!0}),saveDisplayPrefs())}function saveDisplayPrefs(){localStorage.setItem(STORAGE_KEY,JSON.stringify(display))}function isVisible(e,t,n){return e==="columns"?display.columns[n]?.[t]!==!1:display[e]?.[t]!==!1}function toggleDisplay(e,t,n){e==="columns"?display.columns[n][t]=!display.columns[n][t]:display[e][t]=!display[e][t],saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}function setAllDisplay(e,t,n){e==="columns"?Object.keys(DISPLAY_OPTIONS.columns[n]).forEach(a=>{display.columns[n][a]=t}):Object.keys(DISPLAY_OPTIONS[e]).forEach(a=>{display[e][a]=t}),saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}const fmt=e=>new Intl.NumberFormat("da-DK",{minimumFractionDigits:0,maximumFractionDigits:0}).format(Math.round(Number(e)||0)),fmtCompact=e=>{const t=Number(e)||0;return t>=1e6?`${(t/1e6).toFixed(2)}M`:t>=1e3?`${(t/1e3).toFixed(2)}K`:fmt(t)},fmtPct=e=>`${(Number(e)||0).toFixed(2)}%`,fmtRoas=e=>{const t=Number(e)||0;return t>0?`${t.toFixed(2)}x`:"\u2014"};function formatActiveDateFilter(e){if(!e.dateFrom&&!e.dateTo)return"Till date";const t=getDashboardTimeZone();if(window.MarketingMetrics?.formatShortDateLabel){const n=window.MarketingMetrics.formatShortDateLabel(e.dateFrom,t),a=window.MarketingMetrics.formatShortDateLabel(e.dateTo,t);if(n&&a)return`${n} \u2013 ${a}`}return`${e.dateFrom||"start"} to ${e.dateTo||"now"}`}function showDateRangeError(e){document.querySelectorAll("#date-range-error").forEach(t=>{t.textContent=e,t.hidden=!e})}function clearDateRangeError(){showDateRangeError("")}function needsFreshData(){return!lastFetchedAt||Date.now()-lastFetchedAt>DATA_FRESH_MS}function buildQuery(e,t={}){const n=new URLSearchParams,a=getAllPipelineIds(e||[]),s=state.pipelineIds.filter(i=>a.includes(i));if(!s.length)throw new Error("Select at least one pipeline.");return n.set("pipelineIds",s.join(",")),state.dateField&&n.set("dateField",state.dateField),state.dateFrom&&n.set("dateFrom",state.dateFrom),state.dateTo&&n.set("dateTo",state.dateTo),["status","source","assignedTo","adSpend"].forEach(i=>{state[i]&&state[i]!=="all"&&n.set(i,state[i])}),appendTenantParams(n),t.forceFresh&&n.set("fresh","1"),n.toString()}function formatDateInput(e){const t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}function getDashboardTimeZone(){return cachedData?.account?.timezone||"Europe/Copenhagen"}function getCalendarPartsInTimeZone(e,t=new Date){const n=new Intl.DateTimeFormat("en-CA",{timeZone:e,year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(t);return{year:Number(n.find(a=>a.type==="year")?.value),month:Number(n.find(a=>a.type==="month")?.value),day:Number(n.find(a=>a.type==="day")?.value)}}function formatMonthDateRange(e,t){const n=`${e}-${String(t).padStart(2,"0")}-01`,a=new Date(e,t,0).getDate(),s=`${e}-${String(t).padStart(2,"0")}-${String(a).padStart(2,"0")}`;return{start:n,end:s}}const datePickerState={inputId:null,anchorEl:null,viewYear:null,viewMonth:null};let datePickerListenersBound=!1;function isoFromParts(e,t,n){const a=Number(n);return!e||!t||!Number.isFinite(a)?"":`${e}-${String(t).padStart(2,"0")}-${String(a).padStart(2,"0")}`}function formatPickerDisplayLabel(e,t){if(!e||!/^\d{4}-\d{2}-\d{2}$/.test(e))return t;const n=getDashboardTimeZone();return window.MarketingMetrics?.formatShortDateLabel&&window.MarketingMetrics.formatShortDateLabel(e,n)||e}function ensureDatePickerPopover(){if(document.getElementById("date-picker-popover"))return;const e=document.createElement("div");e.id="date-picker-backdrop",e.className="date-picker-backdrop",e.hidden=!0,e.addEventListener("click",closeDatePicker),document.body.appendChild(e);const t=document.createElement("div");t.id="date-picker-popover",t.className="date-picker-popover",t.hidden=!0,t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),document.body.appendChild(t)}function syncDatePickerDisplays(){[{id:"dateFrom",key:"dateFrom",placeholder:"Pick start date"},{id:"dateTo",key:"dateTo",placeholder:"Pick end date"}].forEach(({id:e,key:t,placeholder:n})=>{const a=document.getElementById(e),s=document.getElementById(`${e}-display`),i=document.getElementById(`${e}-trigger`),o=state[t]??"";a&&(a.value=o),s&&(s.textContent=formatPickerDisplayLabel(o,n)),i?.classList.toggle("is-empty",!o),i?.classList.toggle("has-value",!!o)})}function getStateDateValue(e){return state[e]??""}function getDatePickerViewMonth(e){const t=getStateDateValue(e);if(/^\d{4}-\d{2}-\d{2}$/.test(t)){const[n,a]=t.split("-").map(Number);return{year:n,month:a}}return getCalendarPartsInTimeZone(getDashboardTimeZone())}function getTodayIso(){const e=getCalendarPartsInTimeZone(getDashboardTimeZone());return isoFromParts(e.year,e.month,e.day)}function isStartDateDisabled(e){return e>=getTodayIso()}function isFutureDateDisabled(e){return e>getTodayIso()}function isFutureMonthView(e,t){const n=getCalendarPartsInTimeZone(getDashboardTimeZone());return e>n.year||e===n.year&&t>n.month}function isDatePickerDayDisabled(e,t){return t==="dateFrom"&&isStartDateDisabled(e)?!0:isFutureDateDisabled(e)}function renderDatePickerDayCell(e,t,n,a,s,i,o,r){const l=["date-picker-day"];if(n&&l.push("is-outside"),e===a&&l.push("is-today"),e===s&&l.push("is-selected"),i&&o){const d=i<=o?i:o,p=i<=o?o:i;e>=d&&e<=p&&l.push("is-in-range")}return e===i&&l.push("is-range-start"),e===o&&l.push("is-range-end"),isDatePickerDayDisabled(e,r)?(l.push("is-disabled"),`<button type="button" class="${l.join(" ")}" disabled aria-disabled="true">${t}</button>`):`<button type="button" class="${l.join(" ")}" onclick="selectDatePickerDay('${e}')">${t}</button>`}function renderDatePickerPopover(){const e=document.getElementById("date-picker-popover");if(!e||!datePickerState.inputId)return;const{viewYear:t,viewMonth:n,inputId:a}=datePickerState,s=new Intl.DateTimeFormat("en-GB",{month:"long",year:"numeric"}).format(new Date(t,n-1,1)),i=["Mo","Tu","We","Th","Fr","Sa","Su"],r=(new Date(t,n-1,1).getDay()+6)%7,l=new Date(t,n,0).getDate(),d=new Date(t,n-1,0).getDate(),p=getCalendarPartsInTimeZone(getDashboardTimeZone()),b=isoFromParts(p.year,p.month,p.day),y=getStateDateValue(a),S=getStateDateValue("dateFrom"),g=getStateDateValue("dateTo");let w="";for(let v=r-1;v>=0;v-=1){const k=d-v,M=n===1?12:n-1,L=n===1?t-1:t,R=isoFromParts(L,M,k);w+=renderDatePickerDayCell(R,k,!0,b,y,S,g,a)}for(let v=1;v<=l;v+=1){const k=isoFromParts(t,n,v);w+=renderDatePickerDayCell(k,v,!1,b,y,S,g,a)}const $=(7-(r+l)%7)%7;for(let v=1;v<=$;v+=1){const k=n===12?1:n+1,M=n===12?t+1:t,L=isoFromParts(M,k,v);w+=renderDatePickerDayCell(L,v,!0,b,y,S,g,a)}const m=a==="dateFrom"?'<span class="date-picker-hint">Start date must be before today</span>':"",h=a==="dateTo"?'<button type="button" class="date-picker-footer-btn primary" onclick="setDatePickerToday()">Today</button>':"",f=n===12?1:n+1,c=n===12?t+1:t,u=!isFutureMonthView(c,f);e.innerHTML=`
    <div class="date-picker-panel">
      ...
      <div class="date-picker-weekdays">
        ${i.map(v=>`<div class="date-picker-weekday">${v}</div>`).join("")}
      </div>
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),r=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const p=document.getElementById("dashboard-content");p&&(p.innerHTML=renderKpiSkeleton()),r&&(r.disabled=!0,r.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let p=null;if(!availablePipelines.length){if(p=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=p.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,p.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const y=canReuseBootstrapDashboard(p)&&!s;let S,g;if(y?[g,S]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(p)]):[S,g]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=g,cachedData=applyMarketingData(S,g),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(p){if(l!==fetchGeneration)return;if(p.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            `)}`;return}if(IS_ADMIN_CLIENT&&o){const b=document.getElementById("dashboard-content");b&&(b.innerHTML=`
            <div class="note admin-setup-placeholder">
              ${esc(p.message)}
              <div style="margin-top:12px">
                <button type="button" class="refresh-btn primary" onclick="loadDashboard(true)">${RETRY_MSG}</button>
              </div>
            </div>`)}else o?o&&cachedData?(restoreDashboardContentAfterFailedFetch(o),showToast(p.message||"Failed to load filtered data.","error")):r&&(r.textContent="Apply failed - try again"):i.innerHTML=`
          ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
          ${wrapDashboardShell(`
            <div class="error-state">
              <div>Fejl ved hentning af data</div>
