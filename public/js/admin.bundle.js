...
Resume the interrupted ${b}?`)&&(l=m.id,d=Number(m.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const m=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),u=m.summary||m;l=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||v?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:S,batchOffset:d,batchLimit:o})});const m=c.summary||c;l=m.runId,d=m.nextBatchOffset??m.batchOffset+(m.batchProcessed||0),y=y.concat(m.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const u=m.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:u,complete:!m.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...m,rows:y}))):updateGlobalApplyProgress({processed:d,total:u,written:m.updated||0,complete:!m.hasMore})}while((c.summary||c).hasMore);g=!0;const $=c.summary||c,f=$.inWindow||d||w,h=$.nextBatchOffset??d;if(n)setFbLeadProgress({processed:h,total:f,complete:!0}),markFbLeadPreviewReady(e,t,$.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=($.updated||0)>0),showToast(($.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:f,written:$.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const m=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete m[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(m))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch($){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${$.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast(($.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!g&&n&&hideFbLeadProgress(!0),!n&&!g&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
    ...
    `);try{if(e&&fbLeadSyncState.isApplying)return;const[s]=await Promise.all([adminFetch("/api/fb-lead-sync"),loadFbLeadHistory()]);if(fbLeadSyncState.clients=s.clients||[],fbLeadSyncState.cronSummary24h=s.summary?.cronSummary24h||null,hydrateFbLeadPreviewState(fbLeadSyncState.clients),!e){n.innerHTML=renderFbLeadSyncPage(s),bindFbLeadSyncToggles();const i=document.getElementById("fb-lead-sync-refresh");i&&(i.onclick=()=>loadFbLeadSyncPage({fullPreflight:!0})),bindFbLeadHistoryClearButton(),fbLeadSyncRefreshTimer&&(clearInterval(fbLeadSyncRefreshTimer),fbLeadSyncRefreshTimer=null),fbLeadSyncRefreshTimer=window.setInterval(()=>{loadFbLeadSyncPage({silent:!0}).catch(()=>{})},6e4)}(!e||t)&&await loadFbLeadPreflightForClients(fbLeadSyncState.clients,{quick:!t}),e?(updateFbLeadCronSummaryUi(),await refreshFbLeadHistoryUi(),await refreshFbLeadClientRowsUi()):await refreshFbLeadClientRowsUi(),t?(await loadFbLeadPreflightForClients(fbLeadSyncState.clients,{quick:!1}),await refreshFbLeadClientRowsUi()):e||scheduleFbLeadFullPreflight(fbLeadSyncState.clients)}catch(s){e||(n.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("fb-lead-sync"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(s.message)}</div>`)}
      `)}}const metaReportsState={filter:"all",dashboardData:null,hubMounted:!1,clientPayload:null,clientPageMounted:!1,activeMonthKey:null,selectedYear:new Date().getFullYear(),publicPayload:null,publicPageMounted:!1};function filterMetaReportsClients(e,t){const n=e||[];return t==="enabled"?n.filter(a=>a.metaReportEnabled):t==="meta-only"?n.filter(a=>!a.hasGhl):t==="needs-setup"?n.filter(a=>a.needsSetup):n}function getMetaReportsHubView(e,t){const n=e?.clients||[];return{clients:filterMetaReportsClients(n,t),summary:e?.summary||{},meta:e?.meta||{},filter:t}}function renderMetaReportHubRow(e){const t=!!e.clientId,n=e.clientId||e.metaAdAccountId||"",a=e.needsSetup?'<span class="meta-report-badge meta-report-badge--setup">Not in app</span>':e.metaReportEnabled?'<span class="meta-report-badge meta-report-badge--on">Report on</span>':'<span class="meta-report-badge">Report off</span>',s=e.hasGhl?"GHL + Meta":e.partnerSource==="partner"?"Meta partner":"Meta only",i=t?`<a href="/admin/meta-reports/${encodeURIComponent(e.clientId)}" class="meta-report-client-link">${esc(e.accountName)}</a>`:`<span class="meta-report-client-link">${esc(e.accountName)}</span>`,o=e.reportUrl?`<button type="button" class="admin-btn admin-btn--ghost admin-btn--small" data-copy-report-url="${esc(e.reportUrl)}">Copy link</button>`:'<span class="meta-report-muted">\u2014</span>',r=e.needsSetup?`<button type="button" class="admin-btn admin-btn--primary admin-btn--small" data-meta-provision="${esc(e.metaAdAccountId)}" data-meta-provision-name="${esc(e.accountName)}">Add client</button>`:t?`<a class="admin-btn admin-btn--secondary admin-btn--small" href="/admin/meta-reports/${encodeURIComponent(e.clientId)}">Edit</a>`:"\u2014";return`
    <tr class="${e.needsSetup?"meta-report-row--setup":""}" data-meta-hub-row="${esc(n)}">
      <td>${i}${a?`<div class="meta-report-row-meta">${a}</div>`:""}</td>
      <td><code class="meta-report-code">${esc(e.metaAdAccountId||"\u2014")}</code></td>
      <td>${esc(s)}</td>
      <td class="meta-report-toggle-cell">${t?`<input type="checkbox" class="meta-report-toggle" data-meta-report-enabled="${esc(e.clientId)}" ${e.metaReportEnabled?"checked":""} aria-label="Enable report for ${esc(e.accountName)}" />`:"\u2014"}</td>
      <td class="meta-report-toggle-cell">${t?`<input type="checkbox" class="meta-report-toggle" data-meta-bottomline="${esc(e.clientId)}" ${e.metaReportShowBottomline?"checked":""} ${e.metaReportEnabled?"":"disabled"} aria-label="Bottomline for ${esc(e.accountName)}" />`:"\u2014"}</td>
      <td class="meta-report-toggle-cell">${t?`<input type="checkbox" class="meta-report-toggle" data-meta-fee="${esc(e.clientId)}" ${e.metaReportFeeEnabled?"checked":""} ${e.metaReportEnabled?"":"disabled"} aria-label="Censio fee for ${esc(e.accountName)}" />`:"\u2014"}</td>
      <td>${o}</td>
      <td>${r}</td>
    </tr>
  `}function renderMetaReportsSummaryHtml(e){return`
    <div class="meta-report-stat">
      <div class="meta-report-stat-label">Meta ad accounts</div>
      <div class="meta-report-stat-value">${esc(String(e.partnerAccountCount??0))}</div>
    </div>
    <div class="meta-report-stat">
      <div class="meta-report-stat-label">In app</div>
      <div class="meta-report-stat-value">${esc(String(e.inAppCount??0))}</div>
    </div>
    <div class="meta-report-stat">
      <div class="meta-report-stat-label">Reports enabled</div>
      <div class="meta-report-stat-value">${esc(String(e.enabledCount??0))}</div>
    </div>
    <div class="meta-report-stat">
      <div class="meta-report-stat-label">Need setup</div>
      <div class="meta-report-stat-value">${esc(String(e.needsSetupCount??0))}</div>
    </div>
  `}function renderMetaReportsTableRows(e,t="all"){return e.length?e.map(n=>renderMetaReportHubRow(n)).join(""):`<tr><td colspan="8">${t==="all"?"No Meta ad accounts found. Check META_SYSTEM_USER_TOKEN and META_BUSINESS_ID.":"No ad accounts match this filter."}</td></tr>`}function renderMetaReportsBannerHtml(e){return e?.partnerFetchError?`<div class="fb-lead-banner meta-report-banner-warn">${esc(e.partnerFetchError)} Showing only clients already configured in the app.</div>`:""}function recomputeMetaReportsSummaryCounts(){const e=metaReportsState.dashboardData?.clients||[],t=metaReportsState.dashboardData?.summary||{};metaReportsState.dashboardData.summary={...t,inAppCount:e.filter(n=>n.inApp).length,enabledCount:e.filter(n=>n.metaReportEnabled).length,needsSetupCount:e.filter(n=>n.needsSetup).length}}function mergeHubClient(e,t){const n=metaReportsState.dashboardData?.clients;if(!n)return null;const a=n.findIndex(s=>s.clientId===e);return a<0?null:(n[a]={...n[a],...t},recomputeMetaReportsSummaryCounts(),n[a])}function updateMetaReportsHubDom(e){const t=document.getElementById("meta-reports-summary"),n=document.getElementById("meta-reports-banner"),a=document.getElementById("meta-reports-table-body");t&&(t.innerHTML=renderMetaReportsSummaryHtml(e.summary)),n&&(n.innerHTML=renderMetaReportsBannerHtml(e.meta)),a&&(a.innerHTML=renderMetaReportsTableRows(e.clients,e.filter),bindMetaReportsHubRowEvents(a))}function renderMetaReportsClientTabs(e){const t=e.monthKeys||[],n=metaReportsState.activeMonthKey||t[t.length-1]||"";return t.map(a=>`
    <button type="button" class="meta-report-tab${a===n?" is-active":""}" data-meta-month-tab="${esc(a)}">${esc(metaMonthLabel(a))}</button>
  `).join("")}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;if(!t||!metaReportsState.activeMonthKey)return;t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e;const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e,{editable:!0}),bindMetaReportsClientEditEvents(t.clientId,t))}function updateMetaReportsClientContent(e){metaReportsState.clientPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null);const n=document.getElementById("meta-report-year");if(n){const c=[e.year,e.year-1].filter((y,g,S)=>S.indexOf(y)===g);n.innerHTML=c.map(y=>`
      <option value="${y}"${Number(metaReportsState.selectedYear)===Number(y)?" selected":""}>${y}</option>
    `).join("")}const a=document.getElementById("meta-report-copy-link");e.reportUrl?a&&(a.setAttribute("data-copy-report-url",e.reportUrl),a.hidden=!1):a&&(a.hidden=!0);const s=e.settings||{},i=document.getElementById("meta-report-setting-bottomline"),o=document.getElementById("meta-report-setting-fee"),r=document.getElementById("meta-report-setting-fee-percent");i&&(i.checked=!!s.metaReportShowBottomline),o&&(o.checked=!!s.metaReportFeeEnabled),r&&(r.value=s.metaReportFeePercent??20);const l=document.querySelector(".meta-report-tabs");l&&(l.innerHTML=renderMetaReportsClientTabs(e),bindMetaReportsClientTabEvents(e));const d=document.getElementById("meta-report-month-panel");d&&(d.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],{editable:!0}),bindMetaReportsClientEditEvents(e.clientId,e))}function updatePublicMetaReportContent(e){metaReportsState.publicPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null);const n=document.getElementById("meta-report-year");if(n){const i=[e.year,e.year-1].filter((o,r,l)=>l.indexOf(o)===r);n.innerHTML=i.map(o=>`
  ...
  `}function renderMetaReportMonthBody(e,{editable:t=!1}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return'<div class="meta-report-empty">No ad spend this month</div>';const n=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["Conversion Rate",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],a=[["Leads",metaFmtNum(e.topline?.leads),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let s=renderMetaReportMetricTable("Meta ads",n);if(s+=renderMetaReportMetricTable("Topline KPI'er",a),e.bottomline){const i=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];e.bottomline.censioFee>0&&(i.push(["Censio performance fee",metaFmtKr(e.bottomline.censioFee),""]),i.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),i.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])),s+=renderMetaReportMetricTable("Bottomline KPI'er",i)}if(e.customLineItems?.length){const i=e.customLineItems.map(o=>[o.label,metaFmtKr(o.amount),""]);i.push(["Total",metaFmtKr(e.customLineItemsTotal),""]),s+=renderMetaReportMetricTable("Additional items",i)}return t&&(s+=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        ...
        <div class="meta-report-edit-grid">
          ...
          <label>Avg profit per won<input type="number" step="any" id="meta-report-avg-profit" value="${esc(e.bottomline?.avgProfitPerWon??"")}" /></label>
        </div>
        <div class="meta-report-line-items-editor">
          <h4>Custom rows</h4>
          <div id="meta-report-line-items-list">
            ${(e.customLineItems||[]).map((i,o)=>`
              <div class="meta-report-line-item-row" data-index="${o}">
                <input type="text" class="meta-line-label" value="${esc(i.label)}" placeholder="Label" />
                <input type="number" step="any" class="meta-line-amount" value="${esc(i.amount)}" placeholder="Amount" />
    ...
    `),s}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.summary||{},a=e.meta||{};return`
    ...
    <div class="page-hero admin-hub-hero meta-report-hub-hero">
      ...
      <p>All partner ad accounts from Meta Business Manager. Enable reporting and copy share links for each client.</p>
    </div>
    <div class="sync-history-page meta-reports-page">
      <div id="meta-reports-banner">${renderMetaReportsBannerHtml(a)}</div>
      <div class="meta-report-summary" id="meta-reports-summary">
        ${renderMetaReportsSummaryHtml(n)}
      </div>
      <div class="sync-history-toolbar meta-report-toolbar-row">
        <div class="sync-history-toolbar-actions">
          <a class="admin-btn admin-btn--secondary" href="/admin">\u2190 Back to clients</a>
        </div>
        <div class="sync-history-toolbar-actions">
          <select id="meta-reports-filter" class="admin-select">
            <option value="all"${metaReportsState.filter==="all"?" selected":""}>All ad accounts</option>
            <option value="enabled"${metaReportsState.filter==="enabled"?" selected":""}>Reporting enabled</option>
            <option value="meta-only"${metaReportsState.filter==="meta-only"?" selected":""}>Meta-only (no GHL)</option>
            <option value="needs-setup"${metaReportsState.filter==="needs-setup"?" selected":""}>Needs setup</option>
          </select>
          <button class="admin-btn admin-btn--secondary" type="button" id="meta-reports-refresh">${ICON_SYNC} Refresh</button>
        </div>
      </div>
      <div class="dashboard-table-wrap">
        <table class="sync-history-table meta-reports-table">
          <thead>
            <tr>
              <th>Client</th>
              <th>Ad account</th>
          ...
          <tbody id="meta-reports-table-body">${renderMetaReportsTableRows(t,e.filter)}</tbody>
        </table>
      </div>
    </div>
    `)}
  ...
  `).join("");return`
    ...
    <div class="sync-history-page meta-reports-page">
      ...
      <div class="meta-report-month-panel" id="meta-report-month-panel">
        ${renderMetaReportMonthBody(a,{editable:!0})}
      </div>
    </div>
    `)}
  ...
  `}function bindMetaReportCopyButtons(e=document){e.querySelectorAll("[data-copy-report-url]").forEach(t=>{t.onclick=async()=>{const n=t.getAttribute("data-copy-report-url"),a=`${window.location.origin}${n}`;try{await navigator.clipboard.writeText(a),showToast("Share link copied","success")}catch{showToast(a,"info")}}})}async function patchMetaReportSettings(e,t){return adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/settings`,{method:"PATCH",body:JSON.stringify(t)})}async function refreshMetaReportsHubData({silent:e=!0}={}){const t=document.getElementById("meta-reports-refresh");t&&(t.disabled=!0);try{const n=await adminFetch("/api/meta-reports?filter=all");metaReportsState.dashboardData=n,updateMetaReportsHubDom(getMetaReportsHubView(n,metaReportsState.filter)),e||showToast("Meta reports refreshed","success")}catch(n){showToast(n.message||"Refresh failed","error")}finally{t&&(t.disabled=!1)}}function bindMetaReportsHubChromeEvents(){const e=document.getElementById("meta-reports-filter");e&&(e.onchange=()=>{metaReportsState.filter=e.value,metaReportsState.dashboardData&&updateMetaReportsHubDom(getMetaReportsHubView(metaReportsState.dashboardData,metaReportsState.filter))});const t=document.getElementById("meta-reports-refresh");t&&(t.onclick=()=>refreshMetaReportsHubData({silent:!1}))}function bindMetaReportsHubRowEvents(e=document){e.querySelectorAll("[data-meta-report-enabled]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-report-enabled");t.disabled=!0;try{const s=(await patchMetaReportSettings(n,{metaReportEnabled:t.checked})).settings||{};mergeHubClient(n,{metaReportEnabled:s.metaReportEnabled,metaReportShowBottomline:s.metaReportShowBottomline,metaReportFeeEnabled:s.metaReportFeeEnabled,reportUrl:s.reportUrl}),showToast(t.checked?"Report enabled":"Report disabled","success"),updateMetaReportsHubDom(getMetaReportsHubView(metaReportsState.dashboardData,metaReportsState.filter))}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),e.querySelectorAll("[data-meta-bottomline]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-bottomline");t.disabled=!0;try{const a=await patchMetaReportSettings(n,{metaReportShowBottomline:t.checked});mergeHubClient(n,{metaReportShowBottomline:a.settings?.metaReportShowBottomline}),showToast("Bottomline setting saved","success")}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),e.querySelectorAll("[data-meta-fee]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-fee");t.disabled=!0;try{const a=await patchMetaReportSettings(n,{metaReportFeeEnabled:t.checked});mergeHubClient(n,{metaReportFeeEnabled:a.settings?.metaReportFeeEnabled}),showToast("Fee setting saved","success")}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),bindMetaReportCopyButtons(e),e.querySelectorAll("[data-meta-provision]").forEach(t=>{t.onclick=async()=>{t.disabled=!0;try{await adminFetch("/api/meta-reports/provision",{method:"POST",body:JSON.stringify({accountName:t.getAttribute("data-meta-provision-name"),metaAdAccountId:t.getAttribute("data-meta-provision"),metaReportEnabled:!0})}),showToast("Client added","success"),await refreshMetaReportsHubData({silent:!0})}catch(n){showToast(n.message||"Failed to add client","error"),t.disabled=!1}}})}function bindMetaReportsHubEvents(){bindMetaReportsHubChromeEvents(),bindMetaReportsHubRowEvents(document.getElementById("meta-reports-table-body")||document)}function collectMetaReportLineItems(){return Array.from(document.querySelectorAll(".meta-report-line-item-row")).map((e,t)=>({label:e.querySelector(".meta-line-label")?.value||"",amount:Number(e.querySelector(".meta-line-amount")?.value)||0,sortOrder:t})).filter(e=>e.label.trim())}function bindMetaReportsClientTabEvents(e){const t=e.clientId;document.querySelectorAll("[data-meta-month-tab]").forEach(n=>{n.onclick=()=>{metaReportsState.activeMonthKey=n.getAttribute("data-meta-month-tab");const a=document.getElementById("meta-report-month-panel");a&&(a.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],{editable:!0}),bindMetaReportsClientEditEvents(t,e)),document.querySelectorAll("[data-meta-month-tab]").forEach(s=>{s.classList.toggle("is-active",s.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)})}})}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,n=document.getElementById("meta-report-year");n&&(n.onchange=async()=>{metaReportsState.selectedYear=Number(n.value),metaReportsState.activeMonthKey=null,n.disabled=!0;try{await loadMetaReportsClientPage({silent:!0})}finally{n.disabled=!1}}),bindMetaReportCopyButtons();const a=document.getElementById("meta-report-setting-bottomline"),s=document.getElementById("meta-report-setting-fee"),i=document.getElementById("meta-report-setting-fee-percent"),o=async()=>{try{const d=(await patchMetaReportSettings(t,{metaReportShowBottomline:a?.checked,metaReportFeeEnabled:s?.checked,metaReportFeePercent:Number(i?.value)||20})).settings||{},c=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(t)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.clientPayload=c,a&&(a.checked=!!c.settings?.metaReportShowBottomline),s&&(s.checked=!!c.settings?.metaReportFeeEnabled),i&&(i.value=c.settings?.metaReportFeePercent??20),refreshMetaReportMonthPanel(c.months?.[metaReportsState.activeMonthKey]),showToast("Settings saved","success")}catch(l){showToast(l.message||"Update failed","error")}};a&&(a.onchange=o),s&&(s.onchange=o),i&&(i.onchange=o);const r=document.getElementById("meta-report-rotate-token");r&&(r.onclick=async()=>{if(window.confirm("Rotate the client share link? The old link will stop working.")){r.disabled=!0;try{const d=(await patchMetaReportSettings(t,{rotateAccessToken:!0})).settings?.reportUrl,c=metaReportsState.clientPayload;c&&(c.reportUrl=d);const y=document.getElementById("meta-report-copy-link");y&&d&&(y.setAttribute("data-copy-report-url",d),y.hidden=!1),showToast("Share link rotated","success")}catch(l){showToast(l.message||"Rotate failed","error")}finally{r.disabled=!1}}})}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const n=document.getElementById("meta-report-add-line");n&&(n.onclick=()=>{const i=document.getElementById("meta-report-line-items-list");if(!i)return;const o=document.createElement("div");o.className="meta-report-line-item-row",o.innerHTML=`
        ...
        <button type="button" class="admin-btn admin-btn--ghost admin-btn--small meta-line-remove">Remove</button>
      `,i.appendChild(o),o.querySelector(".meta-line-remove")?.addEventListener("click",()=>o.remove())}),document.querySelectorAll(".meta-line-remove").forEach(i=>{i.onclick=()=>i.closest(".meta-report-line-item-row")?.remove()});const a=document.getElementById("meta-report-refresh-meta");a&&(a.onclick=async()=>{a.disabled=!0;try{const i=metaReportsState.activeMonthKey,o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(i)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(o.monthPayload),showToast("Meta data refreshed","success")}catch(i){showToast(i.message||"Meta refresh failed","error")}finally{a.disabled=!1}});const s=document.getElementById("meta-report-save-month");s&&(s.onclick=async()=>{s.disabled=!0;try{const i=metaReportsState.activeMonthKey,o={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:Number(document.getElementById("meta-report-avg-profit")?.value),lineItems:collectMetaReportLineItems()},r=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(i)}`,{method:"PUT",body:JSON.stringify(o)});refreshMetaReportMonthPanel(r.monthPayload),showToast("Month saved","success")}catch(i){showToast(i.message||"Save failed","error")}finally{s.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{metaReportsState.activeMonthKey=t.getAttribute("data-meta-month-tab");const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],{editable:!1})),document.querySelectorAll("[data-meta-month-tab]").forEach(a=>{a.classList.toggle("is-active",a.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{metaReportsState.selectedYear=Number(t.value),metaReportsState.activeMonthKey=null,t.disabled=!0;try{await loadPublicMetaReportPage({silent:!0})}finally{t.disabled=!1}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
      ${renderBrandTopbar("")}
    ...
    `);try{const n=await adminFetch("/api/meta-reports?filter=all");metaReportsState.dashboardData=n;const a=getMetaReportsHubView(n,metaReportsState.filter);metaReportsState.hubMounted?updateMetaReportsHubDom(a):(e.innerHTML=renderMetaReportsHubPage(a),metaReportsState.hubMounted=!0,bindMetaReportsHubEvents())}catch(n){metaReportsState.hubMounted?showToast(n.message||"Load failed","error"):e.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(n.message)}</div>`)}
