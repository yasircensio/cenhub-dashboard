(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){if(t[1]==="sync-history"&&(t[2]==="ghl"||t[2]==="meta")){document.body.dataset.dashboardMode=`sync-history-${t[2]}`,delete document.body.dataset.clientSlug;return}if(t[1]==="fb-lead-sync"){document.body.dataset.dashboardMode="fb-lead-sync",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="custom-values"){document.body.dataset.dashboardMode="meta-reports-custom",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]){document.body.dataset.dashboardMode="meta-reports-client",document.body.dataset.clientSlug=t[2];return}if(t[1]==="meta-reports"){document.body.dataset.dashboardMode="meta-reports",delete document.body.dataset.clientSlug;return}document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}if(t[0]==="report"&&t[1]){document.body.dataset.dashboardMode="report",document.body.dataset.reportToken=t[1],delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_ADMIN_SYNC_HISTORY_GHL=DASHBOARD_MODE==="sync-history-ghl",IS_ADMIN_SYNC_HISTORY_META=DASHBOARD_MODE==="sync-history-meta",IS_ADMIN_FB_LEAD_SYNC=DASHBOARD_MODE==="fb-lead-sync",IS_ADMIN_META_REPORTS=DASHBOARD_MODE==="meta-reports",IS_ADMIN_META_REPORTS_CLIENT=DASHBOARD_MODE==="meta-reports-client",IS_ADMIN_META_REPORTS_CUSTOM=DASHBOARD_MODE==="meta-reports-custom",IS_REPORT_VIEW=DASHBOARD_MODE==="report",REPORT_TOKEN=document.body.dataset.reportToken||"",IS_ADMIN_SYNC_HISTORY=IS_ADMIN_SYNC_HISTORY_GHL||IS_ADMIN_SYNC_HISTORY_META,IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);return e[0]==="admin"&&e[1]==="meta-reports"&&e[2]&&e[2]!=="custom-values"?e[2]:e[0]==="admin"&&e[1]&&e[1]!=="meta-reports"?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"&&e[0]!=="report"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){const t=!!(e&&e.includes("staff-admin-chrome")),n=t||e&&e.includes("admin-topbar-link")?"/admin":"",a=n?`<a href="${n}" class="brand-logo-link" aria-label="Cenhub home"><img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" /></a>`:'<img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" />',s=t?`<button type="button" class="staff-nav-toggle" id="staff-nav-toggle" aria-expanded="false" aria-controls="staff-topbar-panel" aria-label="Open menu">
        ...
        <div class="auth-card">
          <div class="auth-card-header">
            <div class="auth-card-title-row">
              <div class="auth-card-icon" aria-hidden="true">${r}</div>
  ...
  `}function renderSyncHistoryRows(e,t){if(!e.length)return'<div class="sync-history-empty">No sync runs logged yet.</div>';const n=t==="meta"?"Spend / details":"Details",a=e.map(s=>{let o="\u2014";if(t==="meta"){const i=[];if(s.thisMonthSpend!=null&&i.push(`${fmtDkk(s.thisMonthSpend)} DKK this month`),s.spendDateStop){const r=formatSyncHistoryDate(s.spendDateStop);r&&i.push(`through ${r}`)}s.errorMessage&&i.push(s.errorMessage),o=i.join(" \xB7 ")||"\u2014"}else s.opportunityCount!=null?(o=`${s.opportunityCount} opportunities`,s.errorMessage&&(o+=` \xB7 ${s.errorMessage}`)):s.errorMessage&&(o=s.errorMessage);return`
      <tr>
...
${n}`))try{const s=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}`,{method:"DELETE"});showToast(`Cleared ${s.deleted||0} log entr${s.deleted===1?"y":"ies"}.`,"success"),await loadSyncHistoryPage(e)}catch(s){showToast(s.message||`Failed to clear ${t} sync log.`,"error")}}function renderSyncHistoryLoginPrompt(e){const t=e==="meta"?"Meta sync log":"GHL sync log";return`
  ...
  `}async function loadSyncHistoryPage(e,{silent:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){t||(n.innerHTML=renderSyncHistoryLoginPrompt(e));return}currentStaffUser=a,t||(n.innerHTML=`
    ...
    `);try{const s=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}&limit=150`);n.innerHTML=renderSyncHistoryPage(e,s);const o=document.getElementById("sync-history-refresh");o&&(o.onclick=()=>loadSyncHistoryPage(e));const i=document.getElementById("sync-history-clear-log");i&&(i.onclick=()=>clearSyncHistoryLog(e)),syncHistoryRefreshTimer&&(clearInterval(syncHistoryRefreshTimer),syncHistoryRefreshTimer=null),syncHistoryRefreshTimer=window.setInterval(()=>{loadSyncHistoryPage(e,{silent:!0}).catch(()=>{})},6e4)}catch(s){n.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome(e==="meta"?"meta-sync":"ghl-sync"))}
...
${i} contact(s) will get Fb Lead id written in GHL. This writes live data.`)&&(fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:!0,previewRunId:n},await runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!1}))}async function refreshFbLeadClientRowsUi(){const e=document.getElementById("fb-lead-clients-mount"),t=document.getElementById("fb-lead-field-warnings-mount");e&&(e.innerHTML=renderFbLeadClientRows(fbLeadSyncState.clients)),t&&(t.innerHTML=renderFbLeadFieldWarnings(fbLeadSyncState.clients)),bindFbLeadSyncToggles()}function renderFbLeadClientActions(e){const t=esc(e.clientId),n=e.metaPageId?"":`<a class="admin-btn admin-btn--ghost admin-btn--small" href="/admin/${encodeURIComponent(e.clientId)}">Setup</a>`;return`
  ...
  `:'<div class="sync-history-empty">No clients configured yet.</div>'}function renderFbLeadClientRowsWrapper(e){return`<div id="fb-lead-clients-mount">${renderFbLeadClientRows(e)}</div>`}function renderFbLeadHistoryRows(e,t={}){const n=getFbLeadHistoryRunsForDisplay(e);if(!n.length){const s=fbLeadSyncState.showRoutineCronRuns?0:(e||[]).filter(o=>isRoutineSuccessfulCronRun(o)).length;return s>0?`<div class="sync-history-empty">${s} routine auto-sync run(s) hidden \u2014 enable <strong>Show routine auto-sync runs</strong> above to view them.</div>`:'<div class="sync-history-empty">No FB lead sync runs logged yet.</div>'}return e=n,`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table">
        ...
        <tbody>${e.map(s=>{const o=[s.mode,s.dryRun?"dry-run":"apply",s.dryRun?`${s.updated||0} would update`:`${s.updated||0} updated`,`${s.skippedNoMatch||0} no match`,s.errors?`${s.errors} errors`:null].filter(Boolean).join(" \xB7 "),i=dryRunIsReadyForApply(s,e)&&!fbLeadSyncState.isApplying,r=isPartialSyncRun(s)&&!fbLeadSyncState.isApplying,l=fbLeadSyncState.isApplying&&fbLeadSyncState.applyingRunId===s.id,d=renderSyncStatusBadge(l?"applying":s.status);return`
      <tr class="fb-lead-audit-row${l?" is-applying":""}">
        ...
        <td>
          <div class="fb-lead-history-actions">
            ${l?'<span class="fb-lead-history-status-note">In progress above</span>':r?`<button class="admin-btn admin-btn--primary admin-btn--small" type="button" onclick="resumeFbLeadRunFromHistory(${Number(s.id)})">Resume (${partialSyncRemaining(s)})</button>`:i?`<button class="admin-btn admin-btn--primary admin-btn--small" type="button" data-fb-apply-run="${Number(s.id)}" data-apply-count="${Number(s.updated||0)}" onclick="applyFbLeadSyncFromHistory(${Number(s.id)})">Apply (${s.updated})</button>`:""}
  ...
  `}function renderFbLeadAuditTable(e,t){if(!e.length)return'<div class="sync-history-empty" style="padding:12px">No contact-level rows stored for this run.</div>';const n=e.map(s=>{const o=ghlContactUrl(t,s.contactId),i=o?`<a href="${esc(o)}" target="_blank" rel="noopener noreferrer">Open in GHL</a>`:"\u2014";return`
      <tr>
  ...
  `}function renderGlobalApplyProgress(e,t,n=0){const a=fbLeadSyncState.clients.find(i=>i.clientId===e),s=t==="backfill"?"90-day backfill":"recent sync",o=n>0?`${n} contact(s)`:"contacts";return`
  ...
  `}function startGlobalApplyProgress(e,t,n=0){const a=document.getElementById("fb-lead-apply-progress-mount");a&&(fbLeadSyncState.isApplying=!0,fbLeadSyncState.applyStartedAt=Date.now(),fbLeadSyncState.applyProgress=null,a.innerHTML=renderGlobalApplyProgress(e,t,n),a.scrollIntoView({behavior:"smooth",block:"nearest"}),fbLeadSyncState.applyWaitTimer&&clearInterval(fbLeadSyncState.applyWaitTimer),fbLeadSyncState.applyWaitTimer=window.setInterval(()=>{if(!fbLeadSyncState.isApplying)return;const s=Math.floor((Date.now()-fbLeadSyncState.applyStartedAt)/1e3),o=fbLeadSyncState.applyProgress;o?.total>0?updateGlobalApplyProgress({...o,waiting:!0,elapsed:s}):updateGlobalApplyProgress({waiting:!0,elapsed:s})},1e3))}function updateGlobalApplyProgress({processed:e,total:t,written:n,waiting:a=!1,elapsed:s=0,complete:o=!1,error:i=""}={}){const r=document.getElementById("fb-lead-global-progress-card"),l=document.getElementById("fb-lead-global-progress"),d=document.getElementById("fb-lead-global-progress-bar"),c=document.getElementById("fb-lead-global-progress-label"),u=document.getElementById("fb-lead-global-progress-stats"),b=document.getElementById("fb-lead-apply-status-badge");if(!r||!l||!d||!c)return;const w=fbLeadSyncState.applyProgress||{},g=e??w.processed??0,$=t??w.total??0,S=n??w.written??0;!o&&!i&&($>0||g>0)&&(fbLeadSyncState.applyProgress={processed:g,total:$,written:S});const h=$>0||g>0;if(a&&!h&&!o){l.classList.add("is-indeterminate"),d.style.width="35%",u&&(u.textContent="Contacting Meta and GHL\u2026"),c.textContent=`Writing to GHL\u2026 (${s}s \u2014 do not close this page)`;return}l.classList.remove("is-indeterminate");const v=$>0?Math.min(100,Math.round(g/$*100)):o?100:0;d.style.width=`${v}%`,u&&(u.textContent=o?`${S} written to GHL \xB7 ${g} / ${$} contacts processed`:`${S} written \xB7 ${g} / ${$} contacts (${v}%)`),o?(r.classList.add("is-complete"),r.classList.remove("is-error"),c.textContent=`Complete \u2014 ${S} contact(s) updated in GHL`,b&&(b.textContent="Complete",b.classList.add("is-ok"),b.classList.remove("is-missing"))):i?(r.classList.add("is-error"),r.classList.remove("is-complete"),c.textContent=i,b&&(b.textContent="Failed",b.classList.remove("is-ok"),b.classList.add("is-missing"))):a&&h?c.textContent=g>=$?`Finishing up\u2026 (${s}s)`:`Batch in progress \u2014 ${g} / ${$} leads (${v}%) \xB7 ${s}s`:c.textContent=`Writing to GHL \u2014 ${g} / ${$} leads (${v}%)`}function stopGlobalApplyProgress({success:e=!0,keepVisibleMs:t=6e3}={}){fbLeadSyncState.isApplying=!1,fbLeadSyncState.applyProgress=null,fbLeadSyncState.applyWaitTimer&&(clearInterval(fbLeadSyncState.applyWaitTimer),fbLeadSyncState.applyWaitTimer=null),window.setTimeout(()=>{const n=document.getElementById("fb-lead-apply-progress-mount");n&&!fbLeadSyncState.isApplying&&(n.innerHTML="")},t)}function setFbLeadProgress({processed:e=0,total:t=0,complete:n=!1,label:a=""}={}){const s=document.getElementById("fb-lead-progress-wrap"),o=document.getElementById("fb-lead-progress-bar"),i=document.getElementById("fb-lead-progress-label");if(!s||!o)return;s.hidden=!1,s.classList.toggle("is-complete",n);const r=t>0?Math.min(100,Math.round(e/t*100)):n?100:0;o.style.width=`${r}%`,i&&(i.textContent=a||(n?`Complete \u2014 ${e} / ${t} leads (${r}%)`:`Processing \u2014 ${e} / ${t} leads (${r}%)`))}function hideFbLeadProgress(e=!0){const t=document.getElementById("fb-lead-progress-wrap"),n=document.getElementById("fb-lead-progress-bar");t&&e&&(t.hidden=!0,t.classList.remove("is-complete"),n&&(n.style.width="0%"))}function updateFbLeadHistoryClearButton(){const e=document.getElementById("fb-lead-history-clear-all");if(!e)return;const t=(fbLeadSyncState.historyRuns||[]).length>0;e.disabled=!t}function updateFbLeadCronSummaryUi(){const e=document.getElementById("fb-lead-cron-summary-24h");!e||!fbLeadSyncState.cronSummary24h||(e.outerHTML=renderFbLeadCronSummary24h(fbLeadSyncState.cronSummary24h))}async function refreshFbLeadHistoryUi(){await loadFbLeadHistory();const e=document.getElementById("fb-lead-history-mount");if(!e)return;const t=Object.fromEntries(fbLeadSyncState.clients.map(n=>[n.clientId,n.locationId]));e.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,t),updateFbLeadHistoryClearButton()}function bindFbLeadHistoryClearButton(){const e=document.getElementById("fb-lead-history-clear-all");e&&(e.onclick=()=>clearFbLeadSyncHistory());const t=document.getElementById("fb-lead-show-routine-cron");t&&(t.checked=fbLeadSyncState.showRoutineCronRuns,t.onchange=()=>{fbLeadSyncState.showRoutineCronRuns=t.checked;const n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,a)}),updateFbLeadHistoryClearButton()}async function clearFbLeadSyncHistory(){if(!(!isStaffAdmin()||!window.confirm(`Delete all FB lead sync run history?

This only clears the log \u2014 it does not change GHL contacts or Meta data.`)))try{const t=await adminFetch("/api/fb-lead-sync/history",{method:"DELETE"});showToast(`Cleared ${t.deleted||0} run(s).`,"success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(t){showToast(t.message||"Failed to clear FB lead sync log.","error")}}async function deleteFbLeadRun(e){if(!(!isStaffAdmin()||!window.confirm(`Delete run #${e} from history?`)))try{await adminFetch(`/api/fb-lead-sync/history/${e}`,{method:"DELETE"}),showToast("Run deleted.","success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(n){showToast(n.message||"Failed to delete run.","error")}}async function runFbLeadSyncBatch({clientId:e,mode:t,dryRun:n,resumeRunId:a=null}){const s=document.getElementById("fb-lead-preview-btn"),o=document.getElementById("fb-lead-run-results");s&&(s.disabled=!0);const i=n?12:10,r=a!=null?fbLeadSyncState.historyRuns.find(S=>Number(S.id)===Number(a)):null;let l=r?.id||fbLeadSyncState.activeRun?.runId||null,d=r&&Number(r.batchOffset)||0,c=null,u=[],b=!1;const w=a??fbLeadSyncState.activeRun?.previewRunId??null,g=w!=null?fbLeadSyncState.historyRuns.find(S=>Number(S.id)===Number(w)):null,$=r?partialSyncRemaining(r):g?.updated??fbLeadSyncState.previewOkByClient[e]?.updated??0;n||(fbLeadSyncState.applyingRunId=w,startGlobalApplyProgress(e,t,$),await refreshFbLeadHistoryUi());try{if(!a){const f=findPartialSyncRun(e,t,n);if(f){const m=partialSyncRemaining(f),y=n?"preview":"apply";window.confirm(`A ${y} for ${e} was interrupted (${f.updated||0} processed, ${m} remaining).

Resume the interrupted ${y}?`)&&(l=f.id,d=Number(f.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const f=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),m=f.summary||f;l=m.runId,d=0,setFbLeadProgress({processed:0,total:m.inWindow||0,label:`Matching ${m.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||g?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:w,batchOffset:d,batchLimit:i})});const f=c.summary||c;l=f.runId,d=f.nextBatchOffset??f.batchOffset+(f.batchProcessed||0),u=u.concat(f.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const m=f.inWindow||d||$;n?(setFbLeadProgress({processed:d,total:m,complete:!f.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...f,rows:u}))):updateGlobalApplyProgress({processed:d,total:m,written:f.updated||0,complete:!f.hasMore})}while((c.summary||c).hasMore);b=!0;const S=c.summary||c,h=S.inWindow||d||$,v=S.nextBatchOffset??d;if(n)setFbLeadProgress({processed:v,total:h,complete:!0}),markFbLeadPreviewReady(e,t,S.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(S.updated||0)>0),showToast((S.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:h,written:S.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const f=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete f[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(f))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(S){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${S.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((S.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!b&&n&&hideFbLeadProgress(!0),!n&&!b&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(o=>o.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
        ${renderBrandTopbar("")}
        ${wrapDashboardShell(`
  ...
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function metaReportFullUrl(e){return e?`${window.location.origin}${e}`:""}function normalizeMetaReportSlugInput(e){return String(e||"").trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function resolveMetaReportShareSlug(e){const t=e.settings||{};if(t.metaReportSlug)return t.metaReportSlug;const a=String(e.reportUrl||"").replace(/^\/report\//,"").match(/^(.+)-(\d{4})$/);return a?a[1]:""}function resolveMetaReportShareCode(e){const n=String(e.reportUrl||"").replace(/^\/report\//,"").match(/-(\d{4})$/);return n?n[1]:""}function renderMetaReportShareSummary(e,{hidden:t=!1}={}){const a=!!(e.settings||{}).metaReportEnabled,s=resolveMetaReportShareSlug(e),o=resolveMetaReportShareCode(e),i=s?`${s}-${o||"\xB7\xB7\xB7\xB7"}`:"Not set";return`
        ...
        <div class="meta-report-share-panel">
          ...
          <div class="meta-report-share-field">
            <div class="meta-report-share-slug-row">
              ...
              <span class="meta-report-share-slug-suffix" id="meta-report-share-slug-suffix">-${esc(i||"\xB7\xB7\xB7\xB7")}</span>
            </div>
          </div>
  ...
  `}function renderMetaReportClientControlPanel(e){const t=e.settings||{},n=metaReportsState.clientShareExpanded,a=metaReportsState.clientReportSettingsExpanded,s=!!e.reportUrl,o=n||a,i=metaReportMonthsNeedingBackfill(e),r=i.length?`${ICON_SYNC} Backfill ${i.length} month${i.length===1?"":"s"} from Meta`:`${ICON_SYNC} Re-sync year from Meta`;return`
  ...
  `}function refreshMetaReportClientShareUi(e){syncMetaReportToolbarCopy(e),updateMetaReportShareSummary(e),updateMetaReportBottomlineFeeSummary(e.settings||{},"meta-report"),document.getElementById("meta-report-control-panel")?.classList.toggle("is-share-disabled",!e.settings?.metaReportEnabled);const t=document.getElementById("meta-report-share-url");t&&e.reportUrl&&(t.textContent=metaReportFullUrl(e.reportUrl));const n=document.getElementById("meta-report-share-slug-suffix"),a=resolveMetaReportShareCode(e);n&&(n.textContent=`-${a||"\xB7\xB7\xB7\xB7"}`);const s=document.getElementById("meta-report-share-slug");s&&(s.value=resolveMetaReportShareSlug(e));const o=document.getElementById("meta-report-rotate-token");o&&(o.disabled=!e.settings?.metaReportEnabled);const i=document.querySelector("[data-meta-client-preview]");i&&(i.checked=!!e.settings?.metaReportEnabled),setMetaReportShareExpanded(metaReportsState.clientShareExpanded)}function syncMetaReportShareCard(e){const t=document.getElementById("meta-report-share-editor-wrap");if(!t)return;const n=t.parentElement;if(!n)return;const a=renderMetaReportShareEditor(e),s=document.createElement("div");s.innerHTML=a.trim();const o=s.firstElementChild;o&&(n.replaceChild(o,t),setMetaReportShareExpanded(metaReportsState.clientShareExpanded),bindMetaReportShareEvents(e),syncMetaReportToolbarCopy(e))}function bindMetaReportRotateButton(e){const t=document.getElementById("meta-report-rotate-token");t&&(t.onclick=async()=>{if(window.confirm("Rotate the share link? The old link will stop working.")){t.disabled=!0;try{const n=await patchMetaReportSettings(e,{rotateAccessToken:!0}),a=n.settings?.reportUrl||null,s=metaReportsState.clientPayload;s&&(s.reportUrl=a,n.settings&&(s.settings={...s.settings,...n.settings}));const o=s||{clientId:e,reportUrl:a,settings:n.settings||{}};metaReportsState.clientPayload=o,metaReportsState.clientShareExpanded?refreshMetaReportClientShareUi(o):syncMetaReportShareCard(o),showToast("Share link rotated","success")}catch(n){showToast(n.message||"Rotate failed","error")}finally{const n=document.getElementById("meta-report-rotate-token");n&&(n.disabled=!1)}}})}function bindMetaReportShareEvents(e){const t=e.clientId;bindMetaReportCopyButtons(),bindMetaReportRotateButton(t);const n=document.getElementById("meta-report-share-edit");n&&(n.onclick=()=>setMetaReportShareExpanded(!0));const a=document.getElementById("meta-report-share-done");a&&(a.onclick=()=>setMetaReportShareExpanded(!1));const s=document.getElementById("meta-report-share-save");s&&(s.onclick=async()=>{const r=document.getElementById("meta-report-share-slug"),l=normalizeMetaReportSlugInput(r?.value);if(!l||l.length<2){showToast("Enter a valid slug (at least 2 characters).","error");return}s.disabled=!0;try{const c=(await patchMetaReportSettings(t,{metaReportSlug:l})).settings||{},u=metaReportsState.clientPayload||e;u.settings={...u.settings,...c},u.reportUrl=c.reportUrl||null,metaReportsState.clientPayload=u,refreshMetaReportClientShareUi(u),setMetaReportShareExpanded(!1),showToast("Share link slug saved","success")}catch(d){showToast(d.message||"Save failed","error")}finally{const d=document.getElementById("meta-report-share-save");d&&(d.disabled=!1)}});const o=document.getElementById("meta-report-share-slug");o&&(o.oninput=()=>{o.value=normalizeMetaReportSlugInput(o.value)});const i=document.querySelector("[data-meta-client-preview]");if(i){let r=0;i.onchange=async()=>{const l=++r,d=i.checked,c=!d,u=metaReportsState.clientPayload||e;applyMetaReportEnabledUi(u,d);const b=i.closest(".meta-hub-switch");b?.classList.add("is-saving");try{const w=await patchMetaReportSettings(t,{metaReportEnabled:d},{fast:!0});if(l!==r)return;const g=w.settings||{};u.settings={...u.settings,...g},u.reportUrl=g.reportUrl??u.reportUrl,metaReportsState.clientPayload=u,applyMetaReportEnabledUi(u,!!g.metaReportEnabled),showToast(d?"Client preview enabled":"Client preview disabled","success")}catch(w){if(l!==r)return;applyMetaReportEnabledUi(u,c),i.checked=c,showToast(w.message||"Update failed","error")}finally{l===r&&b?.classList.remove("is-saving")}}}}function renderMetaReportHubCard(e){const t=!!e.clientId,n=e.clientId||e.metaAdAccountId||"",a=e.needsSetup?"is-setup":"",s=e.needsSetup?'<span class="meta-report-badge meta-report-badge--setup">Needs setup</span>':e.metaReportEnabled?'<span class="meta-report-badge meta-report-badge--on">Report live</span>':'<span class="meta-report-badge meta-report-badge--off">Report off</span>',o=e.hasGhl?"GHL + Meta":"Meta only",i=t?`<a href="/admin/meta-reports/${encodeURIComponent(e.clientId)}" class="meta-hub-card-name">${esc(e.accountName)}</a>`:`<span class="meta-hub-card-name">${esc(e.accountName)}</span>`,r=[];e.needsSetup?r.push(`<button type="button" class="admin-btn admin-btn--primary" data-meta-provision="${esc(e.metaAdAccountId)}" data-meta-provision-name="${esc(e.accountName)}">Add client</button>`):t&&(r.push(`<a class="admin-btn" href="/admin/meta-reports/${encodeURIComponent(e.clientId)}">Edit report</a>`),r.push(e.reportUrl?`<button type="button" class="admin-btn admin-btn--secondary" data-copy-report-url="${esc(e.reportUrl)}">Copy link</button>`:`<a class="admin-btn admin-btn--secondary" href="/admin/meta-reports/${encodeURIComponent(e.clientId)}">Enable report</a>`));const l=r.length===1?"meta-hub-card-actions meta-hub-card-actions--single":"meta-hub-card-actions";return`
    <article class="meta-hub-card ${a}" data-meta-hub-row="${esc(n)}">
      ...
      ${r.length?`<div class="${l}">${r.join("")}</div>`:""}
    </article>
  ...
  `).join("")}function getMetaReportCalendarYear(e){const t=Number(e?.currentYear);return Number.isFinite(t)&&t>=2e3?t:new Date().getFullYear()}function getMetaReportYearOptions(e){const t=getMetaReportCalendarYear(e),n=t-1,a=new Map;Array.isArray(e?.years)&&e.years.forEach(o=>{const i=Number(o?.year);Number.isFinite(i)&&a.set(i,o.available!==!1)});const s=a.has(n)?a.get(n):e?.previousYearHasData===!0;return[{year:t,available:!0},{year:n,available:!!s}]}function renderMetaReportYearSelectOptions(e,{disableUnavailable:t=!1}={}){const n=Number(metaReportsState.selectedYear)||Number(e?.year);return getMetaReportYearOptions(e).map(({year:a,available:s})=>{const o=t&&!s;return`<option value="${a}"${Number(n)===a?" selected":""}${o?" disabled":""}>${a}${s?"":" (no data)"}</option>`}).join("")}function syncMetaReportSelectedYear(e,{disableUnavailable:t=!1}={}){const n=getMetaReportYearOptions(e),a=Number(metaReportsState.selectedYear),s=n.find(l=>l.year===a);if(s&&(!t||s.available)){metaReportsState.selectedYear=s.year;return}const o=Number(e?.year),i=n.find(l=>l.year===o);if(i&&(!t||i.available)){metaReportsState.selectedYear=i.year;return}const r=n.find(l=>l.available)||n[0];r&&(metaReportsState.selectedYear=r.year)}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;!t||!metaReportsState.activeMonthKey||(t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e,!document.getElementById("meta-report-month-panel"))||(syncMetaReportMonthPanelDom(e,getMetaReportMonthBodyOptions(t.settings,{editable:!0}),t),bindMetaReportsClientEditEvents(t.clientId,t),refreshMetaReportBackfillControl(t))}function refreshMetaReportBackfillControl(e){const t=document.querySelector(".meta-report-client-page .meta-report-toolbar-left"),n=document.querySelector(".meta-report-backfill-wrap"),a=metaReportMonthsNeedingBackfill(e),s=a.length?`${ICON_SYNC} Backfill ${a.length} month${a.length===1?"":"s"} from Meta`:`${ICON_SYNC} Re-sync year from Meta`,o=document.getElementById("meta-report-backfill");if(o)o.innerHTML=s,o.disabled=!1;else if(t){const i=document.createElement("div");i.className="meta-report-backfill-wrap",i.innerHTML=`
      <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-report-backfill">${s}</button>
      <span class="meta-report-backfill-progress" id="meta-report-backfill-progress"></span>
    `,t.appendChild(i),bindMetaReportBackfillButton(e.clientId)}n&&!o&&!t&&n.remove()}function updateMetaReportsClientContent(e){metaReportsState.clientPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null),syncMetaReportSelectedYear(e,{disableUnavailable:!0});const n=document.getElementById("meta-report-year");n&&(n.innerHTML=renderMetaReportYearSelectOptions(e,{disableUnavailable:!0})),syncMetaReportToolbarCopy(e),refreshMetaReportClientShareUi(e);const a=e.settings||{},s=e.months?.[metaReportsState.activeMonthKey]||null;syncMetaReportSettingsControls(a,s);const o=document.querySelector(".meta-report-tabs");o&&(o.innerHTML=renderMetaReportsClientTabs(e),bindMetaReportsClientTabEvents(e)),document.getElementById("meta-report-month-panel")&&(syncMetaReportMonthPanelDom(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(a,{editable:!0}),e),bindMetaReportsClientEditEvents(e.clientId,e)),refreshMetaReportBackfillControl(e)}function updatePublicMetaReportContent(e){metaReportsState.publicPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null),syncMetaReportSelectedYear(e,{disableUnavailable:!0});const n=document.getElementById("meta-report-year");n&&(n.innerHTML=renderMetaReportYearSelectOptions(e,{disableUnavailable:!0}));const a=document.querySelector(".meta-report-tabs");a&&(a.innerHTML=t.map(o=>`
  ...
  `}function renderMetaReportMetricTable(e,t,n="meta",{badge:a="",highlightLastN:s=0}={}){const o=t.map(([i,r,l],d)=>`
      <tr class="${s>0&&d>=t.length-s?`is-highlight accent-${n}`:""}">
        <th scope="row">${esc(i)}</th>
        <td>${esc(String(r??"\u2014"))}${l?`<span class="meta-report-row-unit">${esc(l)}</span>`:""}</td>
      </tr>
    `).join("");return`
  ...
  `}function getMetaReportMonthBodyOptions(e={},{editable:t=!1}={}){return{editable:t}}const META_REPORT_CHART_MIN_POINTS=2;function isMetaReportCensioClient(e){const t=String(e?.clientId||"").toLowerCase(),n=String(e?.accountName||"");return t==="censio"||/censio/i.test(n)}function monthHasChartData(e){if(!e||e.meta?.emptyMonth)return!1;const t=Number(e.meta?.spend)||0,n=Number(e.topline?.leads)||0;return t>0||n>0}function buildMetaReportChartSeries(e){const t=e?.monthKeys||[],n=e?.months||{},a=[];for(const s of t){const o=n[s];if(!o||o.meta?.emptyMonth||!monthHasChartData(o))continue;const i=Number(o.topline?.roasKr)||0,r=Number(o.topline?.roasX)||0,l=!!o.bottomline,d=l?Number(o.bottomline.poasKr)||0:null,c=l?Number(o.bottomline.poasX)||0:null;a.push({monthKey:s,label:metaMonthLabel(s),roasKr:i,roasX:r,poasKr:d,poasX:c,spend:Number(o.meta?.spend)||0,leads:Number(o.topline?.leads)||0,hasData:!0})}return a}function getMetaReportDemoSeries(e,t,n,a){if(!isMetaReportCensioClient({clientId:e,accountName:t}))return null;const o=new Date,i=o.getFullYear(),r=o.getMonth()+1,l=Number(n)||i;let d=(a||[]).filter(c=>{const u=Number(String(c).slice(0,4)),b=Number(String(c).slice(5,7));return!(u!==l||l===i&&b>r)});if(!d.length){const c=l===i?Math.min(8,r):8;d=Array.from({length:c},(u,b)=>`${l}-${String(b+1).padStart(2,"0")}`)}return d.map((c,u)=>{const b=d.length>1?u/(d.length-1):0,w=Math.round(8e3+b*1e4),g=Math.round(2e3+b*15e3),$=Number((.5+b*2.5).toFixed(2)),S=Math.round(1500+b*12e3),h=Number((.4+b*2.2).toFixed(2));return{monthKey:c,label:metaMonthLabel(c),roasKr:g,roasX:$,poasKr:S,poasX:h,spend:w,leads:Math.round(20+b*80),hasData:!0,demo:!0}})}function resolveMetaReportChartSeries(e){const t=buildMetaReportChartSeries(e);if(t.length>=META_REPORT_CHART_MIN_POINTS)return{series:t,demo:!1};if(isMetaReportCensioClient(e)){const n=getMetaReportDemoSeries(e.clientId,e.accountName,e.year||metaReportsState.selectedYear,e.monthKeys);if(n&&n.length>=META_REPORT_CHART_MIN_POINTS)return{series:n,demo:!0}}return{series:t,demo:!1}}function renderMetaReportChartsPanel(){return`
  ...
  `}function destroyMetaReportCharts(){metaReportsState.chartInstances.roas&&(metaReportsState.chartInstances.roas.destroy(),metaReportsState.chartInstances.roas=null),metaReportsState.chartInstances.poas&&(metaReportsState.chartInstances.poas.destroy(),metaReportsState.chartInstances.poas=null)}function metaReportChartValue(e,t,n,a){const s=t==="x"?e[a]:e[n];return Number(s)||0}function metaReportChartTooltipLabel(e,t){return t==="x"?metaFmtX(e):metaFmtKr(e)}function metaReportChartAxisTick(e,t){if(t==="x")return`${Number(e).toFixed(1)}x`;const n=Number(e)||0;return Math.abs(n)>=1e6?`${(n/1e6).toFixed(1)}M`:Math.abs(n)>=1e3?`${Math.round(n/1e3)}K`:n}function buildMetaReportLineChart(e,t,n,a,s){const o=t.map(r=>r.label),i=t.map(r=>metaReportChartValue(r,n,a,s));return new Chart(e,{type:"line",data:{labels:o,datasets:[{label:n==="x"?"Multiplier":"Dkr",data:i,borderColor:"#ff6a00",backgroundColor:"rgba(255, 106, 0, 0.12)",borderWidth:2.5,pointBackgroundColor:"#ff6a00",pointBorderColor:"#ffffff",pointBorderWidth:2,pointRadius:4,pointHoverRadius:5,fill:!0,tension:.3}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!1},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,callbacks:{label(r){return metaReportChartTooltipLabel(r.parsed.y,n)}}}},scales:{x:{grid:{color:"rgba(26, 18, 8, 0.06)"},ticks:{color:"#6b5348",font:{size:11}}},y:{grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:r=>metaReportChartAxisTick(r,n)}}}}})}function updateMetaReportChart(e,t,n){const a=document.getElementById(`meta-report-chart-${e}`),s=document.getElementById(`meta-report-chart-${e}-empty`);if(!a||!s)return;const o=e==="roas"?"roasKr":"poasKr",i=e==="roas"?"roasX":"poasX",r=e==="poas"?t.filter(l=>l.poasKr!=null&&l.poasX!=null):t;if(metaReportsState.chartInstances[e]&&(metaReportsState.chartInstances[e].destroy(),metaReportsState.chartInstances[e]=null),r.length<META_REPORT_CHART_MIN_POINTS||typeof Chart>"u"){a.hidden=!0,s.hidden=!1;return}a.hidden=!1,s.hidden=!0,metaReportsState.chartInstances[e]=buildMetaReportLineChart(a,r,n,o,i)}function bindMetaReportChartToggles(){const e=document.getElementById("meta-chart-roas-mode");e&&e.querySelectorAll("[data-mode]").forEach(n=>{n.onclick=()=>{const a=n.getAttribute("data-mode");metaReportsState.chartRoasMode=a,e.querySelectorAll("[data-mode]").forEach(s=>{s.classList.toggle("is-active",s.getAttribute("data-mode")===a)}),updateMetaReportChart("roas",metaReportsState.chartSeries||[],a)}});const t=document.getElementById("meta-chart-poas-mode");t&&t.querySelectorAll("[data-mode]").forEach(n=>{n.onclick=()=>{const a=n.getAttribute("data-mode");metaReportsState.chartPoasMode=a,t.querySelectorAll("[data-mode]").forEach(s=>{s.classList.toggle("is-active",s.getAttribute("data-mode")===a)}),updateMetaReportChart("poas",metaReportsState.chartSeries||[],a)}})}function syncMetaReportChartToggleUi(){["roas","poas"].forEach(e=>{const t=e==="roas"?metaReportsState.chartRoasMode:metaReportsState.chartPoasMode,n=document.getElementById(`meta-chart-${e}-mode`);n&&n.querySelectorAll("[data-mode]").forEach(a=>{a.classList.toggle("is-active",a.getAttribute("data-mode")===t)})})}function mountMetaReportCharts(e,{editable:t=!1}={}){if(destroyMetaReportCharts(),!e)return;const{series:n,demo:a}=resolveMetaReportChartSeries(e);metaReportsState.chartSeries=n,metaReportsState.chartDemo=a;const s=document.getElementById("meta-report-chart-demo-badge");s&&(s.hidden=!(a&&t)),bindMetaReportChartToggles(),syncMetaReportChartToggleUi(),updateMetaReportChart("roas",n,metaReportsState.chartRoasMode),updateMetaReportChart("poas",n,metaReportsState.chartPoasMode)}function syncMetaReportMonthPanelDom(e,t={},n=null){const a=document.getElementById("meta-report-month-panel");if(!a)return;const s=n||metaReportsState.clientPayload||metaReportsState.publicPayload;a.innerHTML=renderMetaReportMonthBody(e,t),requestAnimationFrame(()=>{mountMetaReportCharts(s,{editable:!!t.editable})})}function renderMetaReportMonthTables(e,{editable:t=!1}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return`<div class="meta-report-empty">No ad spend recorded for this month yet.${t?' Click "Refresh from Meta" below, or use "Backfill from Meta" above to pull historical months.':""}</div>`;const n=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["CTR",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],a=[["Leads",metaFmtNum(e.topline?.leads),""],["Cost Per Lead (CPL)",metaFmtKr(e.topline?.cpl),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let s="";if(s+=renderMetaReportMetricTable("Meta ads",n,"meta"),s+=renderMetaReportMetricTable("Topline KPI'er",a,"topline",{highlightLastN:2}),e.bottomline){const o=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];if(e.bottomline.feeMode){const i=e.bottomline.feeLabel||(e.bottomline.feeMode==="marketing"?"Censio marketing fee":`Censio performance fee (${metaFmtNum(e.bottomline.feePercent,0)}%)`);o.push([i,metaFmtKr(e.bottomline.censioFee),""]),o.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),o.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])}s+=renderMetaReportMetricTable("Bottomline KPI'er",o,"bottomline",{highlightLastN:2})}return s?`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1}={}){let n="";t&&e&&!e.meta?.emptyMonth&&(n=`
    ...
    `);const a=t?"Report preview":"Report",s=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${a}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",o=renderMetaReportMonthTables(e,{editable:t}),i=renderMetaReportChartsPanel();return`${n}${s}
    <div class="meta-report-content-layout">
      <div class="meta-report-content-main">${o}</div>
      ${i}
    </div>`}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
  ...
  `).join("");return`
    ...
    `)}
  `}function renderPublicMetaReportPage(e){const t=e.monthKeys||[],n=metaReportsState.activeMonthKey||t[t.length-1]||"",a=e.months?.[n]||null,s=t.map(o=>`
  ...
  `}function setMetaReportYearLoadingBanner(e,t,n){const a=document.querySelector(e);if(!a)return;let s=a.querySelector(".meta-report-year-loading-banner");n?(s||(s=document.createElement("span"),s.className="meta-report-year-loading-banner",s.setAttribute("aria-live","polite"),a.appendChild(s)),s.innerHTML=`<span class="meta-report-save-indicator-spinner"></span> Loading ${esc(t)}\u2026`,s.hidden=!1):s&&(s.hidden=!0)}function setMetaReportClientYearLoading(e,t){const n=document.querySelector(".meta-report-client-page"),a=document.getElementById("meta-report-year");if(t){a&&(a.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!0);const s=document.querySelector(".meta-report-tabs");s&&(s.innerHTML=renderMetaReportTabsSkeleton(),s.setAttribute("aria-busy","true"));const o=document.getElementById("meta-report-month-panel");o&&(o.innerHTML=renderMetaReportMonthPanelSkeleton(),o.classList.add("is-loading")),destroyMetaReportCharts()}else{a&&(a.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!1);const s=document.querySelector(".meta-report-tabs");s&&s.removeAttribute("aria-busy");const o=document.getElementById("meta-report-month-panel");o&&o.classList.remove("is-loading")}}function setMetaCvYearLoading(e,t){const n=document.querySelector(".sync-history-page.meta-reports-page"),a=document.getElementById("meta-cv-year"),s=document.getElementById("meta-cv-search");if(t){a&&(a.disabled=!0),s&&(s.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!0);const o=document.querySelector(".meta-cv-summary");o&&(o.classList.add("is-skeleton"),o.innerHTML=renderMetaCvSummarySkeleton());const i=document.querySelector(".meta-cv-legend");i&&i.setAttribute("aria-hidden","true");const r=document.getElementById("meta-cv-client-list");r&&(r.innerHTML=renderMetaCvListSkeleton(),r.setAttribute("aria-busy","true"));const l=document.getElementById("meta-cv-editor");l&&(l.innerHTML=renderMetaCvEditorYearSkeleton(),l.setAttribute("aria-busy","true"))}else{a&&(a.disabled=!1),s&&(s.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!1);const o=document.querySelector(".meta-cv-summary");o&&o.classList.remove("is-skeleton");const i=document.querySelector(".meta-cv-legend");i&&i.removeAttribute("aria-hidden");const r=document.getElementById("meta-cv-client-list");r&&r.removeAttribute("aria-busy");const l=document.getElementById("meta-cv-editor");l&&l.removeAttribute("aria-busy")}}function setMetaReportPublicYearLoading(e,t){const n=document.querySelector(".meta-reports-page--public"),a=document.getElementById("meta-report-year");if(t){a&&(a.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!0);const s=document.querySelector(".meta-reports-page--public .meta-report-tabs");s&&(s.innerHTML=renderMetaReportTabsSkeleton(),s.setAttribute("aria-busy","true"));const o=document.getElementById("meta-report-month-panel");o&&(o.innerHTML=renderMetaReportMonthPanelSkeleton(),o.classList.add("is-loading")),destroyMetaReportCharts()}else{a&&(a.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!1);const s=document.querySelector(".meta-reports-page--public .meta-report-tabs");s&&s.removeAttribute("aria-busy");const o=document.getElementById("meta-report-month-panel");o&&o.classList.remove("is-loading")}}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,n=document.getElementById("meta-report-year");n&&(n.onchange=async()=>{if(n.selectedOptions?.[0]?.disabled){n.value=String(metaReportsState.selectedYear);return}const s=metaReportsState.selectedYear,o=Number(n.value);if(o!==s){metaReportsState.selectedYear=o,metaReportsState.activeMonthKey=null,setMetaReportClientYearLoading(o,!0);try{await loadMetaReportsClientPage({silent:!0})}catch(i){metaReportsState.selectedYear=s,n&&(n.value=String(s)),await loadMetaReportsClientPage({silent:!0}),showToast(i.message||"Load failed","error")}finally{setMetaReportClientYearLoading(o,!1)}}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report"),bindMetaReportShareEvents(e)}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const n=document.getElementById("meta-report-refresh-meta");n&&(n.onclick=async()=>{n.disabled=!0;try{const s=metaReportsState.activeMonthKey,o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(o.monthPayload),showToast("Meta data refreshed","success")}catch(s){showToast(s.message||"Meta refresh failed","error")}finally{n.disabled=!1}});const a=document.getElementById("meta-report-save-month");a&&(a.onclick=async()=>{a.disabled=!0;try{const s=metaReportsState.activeMonthKey,o={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:Number(document.getElementById("meta-report-avg-profit")?.value)},i=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}`,{method:"PUT",body:JSON.stringify(o)});refreshMetaReportMonthPanel(i.monthPayload),showToast("Month saved","success")}catch(s){showToast(s.message||"Save failed","error")}finally{a.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{metaReportsState.activeMonthKey=t.getAttribute("data-meta-month-tab"),document.getElementById("meta-report-month-panel")&&syncMetaReportMonthPanelDom(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(e.settings,{editable:!1}),e),document.querySelectorAll("[data-meta-month-tab]").forEach(a=>{a.classList.toggle("is-active",a.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}const a=metaReportsState.selectedYear,s=Number(t.value);if(s!==a){metaReportsState.selectedYear=s,metaReportsState.activeMonthKey=null,setMetaReportPublicYearLoading(s,!0);try{await loadPublicMetaReportPage({silent:!0})}catch(o){metaReportsState.selectedYear=a,t&&(t.value=String(a)),await loadPublicMetaReportPage({silent:!0}),showToast(o.message||"Load failed","error")}finally{setMetaReportPublicYearLoading(s,!1)}}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
    ...
    `).join(""):'<div class="meta-report-empty" style="padding:32px 12px">No matching clients.</div>'}function monthStatusIsEmpty(e,t={}){return e?(e.status??classifyMetaCvMonthStatus({wonLeads:e.inputs?.wonLeads??e.topline?.wonLeads??e.wonLeads,avgLeadValue:e.inputs?.avgLeadValue??e.topline?.avgLeadValue??e.avgLeadValue,avgProfitPerWon:e.inputs?.avgProfitPerWon??e.avgProfitPerWon},!!t.metaReportShowBottomline))==="empty":!0}function metaReportEffectiveFeeEnabled(e,t=null){if(!t)return!!e?.metaReportFeeEnabled;const n=!!e?.metaReportShowBottomline;return(t.status??classifyMetaCvMonthStatus({wonLeads:t.inputs?.wonLeads??t.topline?.wonLeads??t.wonLeads,avgLeadValue:t.inputs?.avgLeadValue??t.topline?.avgLeadValue??t.avgLeadValue,avgProfitPerWon:t.inputs?.avgProfitPerWon??t.avgProfitPerWon},n))==="empty"?!1:!!e?.metaReportFeeEnabled}function resolveMetaReportFeeMode(e={}){return e.metaReportFeeEnabled?resolveMetaReportStoredFeeMode(e):""}const META_REPORT_FEE_MODE_META=[{value:"",icon:"\u2013",title:"None",desc:"No Censio fee"},{value:"performance",icon:"%",title:"Performance fee",desc:"% of profit (POAS)"},{value:"marketing",icon:"Dkr",title:"Marketing fee",desc:"Fixed Dkr amount"}];function metaReportFeeValueBlockHtml(e,t,n,a,{inline:s=!1}={}){return s&&t==="performance"?`
      ...
      </label>`:'<p class="meta-report-fee-value-empty">No amount needed \u2014 just save your selection.</p>'}function metaReportUsesInlineFeeValue(e){return e==="meta-cv"}function describeMetaReportFeeModeLabel(e,t,n,{feeEnabled:a=!0}={}){if(!a||e===""||e==null)return"No Censio fee";if(e==="performance"){const s=Number(t);return`Performance fee \xB7 ${Number.isFinite(s)?s:20}%`}if(e==="marketing"){const s=Number(n);return`Marketing fee \xB7 Dkr ${Number.isFinite(s)?s:0}`}return"No Censio fee"}function readMetaReportFeeDraftState(e){const t=document.getElementById(`${e}-fee-nested`);if(!t)return null;const n=t.querySelector(`input[name="${e}-fee-mode"]:checked`),a=n?n.value||"":t.dataset.feeMode||"";let s=Number(t.dataset.feePercent),o=Number(t.dataset.marketingFee);if(a==="performance"){const u=document.getElementById(`${e}-setting-fee-percent`),b=Number(u?.value);Number.isFinite(b)&&(s=b)}else if(a==="marketing"){const u=document.getElementById(`${e}-setting-marketing-fee`),b=Number(u?.value);Number.isFinite(b)&&(o=b)}const i=t.dataset.feeEnabled==="true"?resolveMetaReportStoredFeeMode({metaReportFeeEnabled:!0,metaReportFeeMode:t.dataset.feeMode||null,metaReportMarketingFeeAmount:t.dataset.marketingFee}):"",r=Number(t.dataset.feePercent),l=Number(t.dataset.marketingFee);return{appliedMode:i,appliedPercent:r,appliedMarketing:l,selectedMode:a,selectedPercent:s,selectedMarketing:o,isDirty:a!==i||(a==="performance"?s!==r:a==="marketing"?o!==l:!1),appliedLabel:describeMetaReportFeeModeLabel(i,r,l),selectedLabel:describeMetaReportFeeModeLabel(a,s,o)}}function metaReportFeeStatusHtml(e){return e.isDirty?`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        <div class="meta-cv-bottomline-head">
          ...
          ${renderMetaReportSwitch(`${t}-setting-bottomline`,i,"Enable bottomline on report",!1,{hideLabel:!0})}
        </div>
        <div class="meta-report-fee-nested meta-cv-bottomline-body${i?"":" is-hidden"}" id="${t}-fee-nested" ${w}>
          ...
          <div class="meta-report-fee-mode-pills">${f}</div>
  ...
  `}const g=describeMetaReportFeeModeLabel(d,c,u),$=META_REPORT_FEE_MODE_META.map(({value:p,icon:f,title:m,desc:y})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(p)}"${d===p?" checked":""} />
          <span class="meta-report-fee-mode-card-icon" aria-hidden="true">${esc(f)}</span>
    ...
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?S?" is-editing":" is-collapsed":""}">
      ...
      <div class="meta-report-bottomline-editor${s&&!S?" is-hidden":""}" id="${t}-fee-editor">
        <div class="meta-report-bottomline-row">
          ...
          ${renderMetaReportSwitch(`${t}-setting-bottomline`,i,"Show bottomline",!1,{hideLabel:!0})}
        </div>
        <div class="meta-report-fee-nested${i?"":" is-hidden"}" id="${t}-fee-nested" ${w}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
            <span class="meta-report-fee-status-value is-active">${esc(g)}</span>
          </div>
          <div class="meta-report-fee-mode-grid${n?" meta-report-fee-mode-grid--rows":""}">${$}
          </div>
  ...
  `}function syncMetaReportBottomlineFeeDom(e={},t="meta-report"){const n=!!e.metaReportShowBottomline,a=!!e.metaReportFeeEnabled,s=e.metaReportFeeMode||"",o=resolveMetaReportFeeMode(e),i=e.metaReportFeePercent??20,r=e.metaReportMarketingFeeAmount??0,l=document.getElementById(`${t}-setting-bottomline`);l&&(l.checked=n);const d=document.getElementById(`${t}-fee-nested`);d?.classList.toggle("is-hidden",!n),d&&(d.dataset.feeMode=s,d.dataset.feeEnabled=a?"true":"false",d.dataset.feePercent=String(i),d.dataset.marketingFee=String(r)),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(b=>{b.checked=b.value===o});const c=metaReportUsesInlineFeeValue(t),u=document.getElementById(`${t}-fee-value`);u&&(u.innerHTML=metaReportFeeValueBlockHtml(t,o,i,r,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),syncMetaReportFeeSelectionState(t),t==="meta-report"&&updateMetaReportBottomlineFeeSummary(e,t)}function bindMetaReportFeeValueInputEnterKey(e){const t=document.getElementById(`${e}-setting-fee-percent`)||document.getElementById(`${e}-setting-marketing-fee`);t&&(t.onkeydown=n=>{n.key==="Enter"&&(n.preventDefault(),document.getElementById(`${e}-fee-save`)?.click())})}function bindMetaReportBottomlineFeeEvents(e,t,n){const a=n||((l,d)=>saveMetaReportClientSettings(e,l,d)),s=document.getElementById(`${t}-fee-edit`);s&&(s.onclick=()=>setMetaReportClientSettingsExpanded(t,!0));const o=document.getElementById(`${t}-fee-done`);o&&(o.onclick=()=>{const l=metaReportsState.clientPayload?.settings||{};syncMetaReportBottomlineFeeDom(l,t),setMetaReportClientSettingsExpanded(t,!1),updateMetaReportBottomlineFeeSummary(l,t)});const i=document.getElementById(`${t}-setting-bottomline`);i&&(i.onchange=()=>{const l=i.checked;a({metaReportShowBottomline:l},()=>{i.checked=!l})}),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(l=>{l.onchange=()=>{if(!l.checked)return;const d=document.getElementById(`${t}-fee-nested`),c=metaReportUsesInlineFeeValue(t),u=document.getElementById(`${t}-fee-value`);u&&d&&(u.innerHTML=metaReportFeeValueBlockHtml(t,l.value||"",d.dataset.feePercent??20,d.dataset.marketingFee??0,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),bindMetaReportFeeAmountInputs(t),syncMetaReportFeeSelectionState(t)}}),bindMetaReportFeeValueInputEnterKey(t),bindMetaReportFeeDraftTracking(t);const r=document.getElementById(`${t}-fee-save`);r&&(r.onclick=async()=>{if(r.disabled)return;const l=document.getElementById(`${t}-fee-nested`),d=l?.querySelector(`input[name="${t}-fee-mode"]:checked`),c=d&&d.value||null,u={metaReportFeeMode:c};if(c==="performance"){const w=document.getElementById(`${t}-setting-fee-percent`);let g=Number(w?.value);Number.isFinite(g)||(g=20),g=Math.min(100,Math.max(0,g)),w&&(w.value=String(g)),u.metaReportFeePercent=g}else if(c==="marketing"){const w=document.getElementById(`${t}-setting-marketing-fee`);let g=Number(w?.value);Number.isFinite(g)||(g=0),g=Math.max(0,g),w&&(w.value=String(g)),u.metaReportMarketingFeeAmount=g}const b=r.textContent;r.disabled=!0,r.textContent="Saving\u2026",r.classList.remove("is-dirty");try{await a(u,()=>syncMetaReportBottomlineFeeDom({metaReportShowBottomline:i?i.checked:!0,metaReportFeeMode:l?.dataset.feeMode||null,metaReportFeePercent:Number(l?.dataset.feePercent)||20,metaReportMarketingFeeAmount:Number(l?.dataset.marketingFee)||0},t)),syncMetaReportFeeSelectionState(t)}finally{r.disabled=!1,r.textContent=b,syncMetaReportFeeSelectionState(t)}})}function renderMetaReportSaveIndicator(e,t=!1){return`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find($=>$.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const s=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,s);const o=t.editorPayload?.months?.[a]||null,i=n.metaReportShowBottomline,r=s.wonLeads!=null?s.wonLeads:o?.inputs?.wonLeads??o?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:o?.inputs?.avgLeadValue??o?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:o?.inputs?.avgProfitPerWon??"",c=metaReportsState.customValues.draftInputs,u=c?.wonLeads??r,b=c?.avgLeadValue??l,w=c?.avgProfitPerWon??d,g=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map($=>`
            <option value="${esc($.clientId)}"${$.clientId===n.clientId?" selected":""}>${esc($.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map($=>`
            <option value="${esc($)}"${$===a?" selected":""}>${esc(metaMonthLabel($))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
      ...
      <label>Avg profit per won${i?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(w)}" />
      </label>
    </div>
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",o=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",i=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `:`
      <div class="metrics-model-panel is-locked">
        <div class="metrics-model-header">
          ...
          <span class="${i}">${e.metricsModelLockedAt?ICON_LOCK:ICON_UNLOCK} ${e.metricsModelLockedAt?"Locked":"Editable"}</span>
        </div>
  ...
  `}function getSetupProgressSteps(e){return[{id:"metrics",label:"Metrics",done:!!e.metricsModelSetAt},{id:"ghl",label:"GHL",done:!!(e.hasGhlToken&&e.locationId)},{id:"pipelines",label:"Pipelines",done:!!(e.newLeadsPipelineId&&e.salesPipelineId)},{id:"meta",label:"Meta",done:e.metaSyncStatus==="ok",partial:!!(e.metaAdAccountId&&e.metaSyncStatus!=="ok")}]}function renderSetupProgressStrip(e){const t=getSetupProgressSteps(e),n='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6L9 17l-5-5"/></svg>';return`
    <nav class="setup-progress" aria-label="Setup progress">
      ${t.map((a,s)=>{const o=a.done?" is-done":a.partial?" is-partial":"",i=a.done?`<span class="setup-progress-mark">${n}</span>`:`<span class="setup-progress-mark">${s+1}</span>`;return`
  ...
  `:`
    ...
    `}function renderTable(e,t,n,a,s){const o=n.filter(i=>isVisible("columns",i.key,e));return o.length?`
    <div class="dashboard-table-wrap">
      <table class="dashboard-table">
        <thead>
          <tr>
            <th>${t}</th>
            ${o.map(i=>`<th class="${i.align||""}">${i.label}</th>`).join("")}
          </tr>
        </thead>
        <tbody>
          ${a.length?a.map(i=>`
            <tr>
              <td>${s(i,"label")}</td>
              ${o.map(r=>`<td class="${r.align||""}">${s(i,r.key)}</td>`).join("")}
            </tr>
  ...
  `}function renderDashboardContent(e){const{kpis:t,statusBreakdown:n,sourceReport:a,assigneeReport:s,pipelines:o,filters:i}=e,r=isVisible("sections","statusBreakdown");return`
    ...
    <div class="note">
      ...
      Active filters: source=${i.source}, assignee=${i.assignedTo}, dates=${formatActiveDateFilter(i)}.
    </div>
    `:""}
    <div class="brand-footer">
      Dashboard by Cenhub
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,o=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const i=!!document.getElementById("dashboard-content"),r=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!i)o.innerHTML=`
        ...
        `)}`;else if(!n){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),r&&(r.disabled=!0,r.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const b=canReuseBootstrapDashboard(c)&&!s;let w,g;if(b?[g,w]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[w,g]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=g,cachedData=applyMarketingData(w,g),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(l!==fetchGeneration)return;if(c.name==="AbortError"){i?restoreDashboardContentAfterFailedFetch(i):o.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            `)}`;return}if(IS_ADMIN_CLIENT&&i){const u=document.getElementById("dashboard-content");u&&(u.innerHTML=`
            <div class="note admin-setup-placeholder">
              ${esc(c.message)}
              <div style="margin-top:12px">
                <button type="button" class="refresh-btn primary" onclick="loadDashboard(true)">${RETRY_MSG}</button>
              </div>
            </div>`)}else i?i&&cachedData?(restoreDashboardContentAfterFailedFetch(i),showToast(c.message||"Failed to load filtered data.","error")):r&&(r.textContent="Apply failed - try again"):o.innerHTML=`
          ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
          ${wrapDashboardShell(`
            <div class="error-state">
              <div>Fejl ved hentning af data</div>
