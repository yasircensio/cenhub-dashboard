...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),y=w.summary||w;l=y.runId,c=0,setFbLeadProgress({processed:0,total:y.inWindow||0,label:`Matching ${y.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:r})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const y=w.inWindow||c||$;a?(setFbLeadProgress({processed:c,total:y,complete:!w.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:y,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const v=d.summary||d,f=v.inWindow||c||$,b=v.nextBatchOffset??c;if(a)setFbLeadProgress({processed:b,total:f,complete:!0}),markFbLeadPreviewReady(e,t,v.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(v.updated||0)>0),showToast((v.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:b,total:f,written:v.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const g=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=g.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(v){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${v.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((v.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function parseComparisonDateKey(e){const t=String(e||"").match(/^([ab])-(start|end)$/);return t?{side:t[1],bound:t[2],stateKey:t[1]==="b"?"comparisonPeriodB":"comparisonPeriodA"}:null}function metaReportComparisonSafePickerBounds(e,t){const a=e||void 0,n=t||void 0;return a&&n&&a>n?{min:n,max:a}:{min:a,max:n}}function applyMetaReportComparisonDateChange(e,t,{editable:a=!1,dateStr:n=null}={}){const s=parseComparisonDateKey(t.getAttribute("data-comparison-date")),o=n||t.value;if(!s||!o)return;const{side:r,bound:i,stateKey:l}=s;metaReportsState[l]||(metaReportsState[l]={startDate:null,endDate:null});const c=metaReportsState[l];if(c[`${i}Date`]=o,c.startDate&&c.endDate&&c.startDate>c.endDate){const d=c.startDate;c.startDate=c.endDate,c.endDate=d;const u=document.querySelector(`[data-comparison-date="${r}-start"]`),p=document.querySelector(`[data-comparison-date="${r}-end"]`);u?._metaComparisonDatePicker&&u._metaComparisonDatePicker.setDate(c.startDate,!1),p?._metaComparisonDatePicker&&p._metaComparisonDatePicker.setDate(c.endDate,!1)}metaReportsState.comparisonMode="custom",syncMetaReportComparisonPeriodPickerBounds(e),document.querySelectorAll("[data-comparison-preset]").forEach(d=>{const u=d.getAttribute("data-comparison-preset")==="custom";d.classList.toggle("active",u),d.setAttribute("aria-pressed",u)}),refreshMetaReportComparison(e,{editable:a})}function syncMetaReportComparisonPeriodPickerBounds(e){const t=metaReportComparisonDateInputBounds(e);["a","b"].forEach(a=>{const s=metaReportsState[a==="b"?"comparisonPeriodB":"comparisonPeriodA"]||{},o=document.querySelector(`[data-comparison-date="${a}-start"]`),r=document.querySelector(`[data-comparison-date="${a}-end"]`);if(o?._metaComparisonDatePicker){const{min:i,max:l}=metaReportComparisonSafePickerBounds(t.min||void 0,s.endDate||t.max||void 0);o._metaComparisonDatePicker.set("minDate",i),o._metaComparisonDatePicker.set("maxDate",l)}if(r?._metaComparisonDatePicker){const{min:i,max:l}=metaReportComparisonSafePickerBounds(s.startDate||t.min||void 0,t.max||void 0);r._metaComparisonDatePicker.set("minDate",i),r._metaComparisonDatePicker.set("maxDate",l)}})}function closeMetaReportComparisonDatePickers(e=null){(metaReportsState.comparisonDatePickers||[]).forEach(t=>{t!==e&&t.isOpen&&t.close()})}function destroyMetaReportComparisonDatePickers(){closeMetaReportComparisonDatePickers(),document.querySelectorAll("[data-comparison-date]").forEach(e=>{e._metaComparisonDatePicker&&(e._metaComparisonDatePicker.destroy(),e._metaComparisonDatePicker=null)}),metaReportsState.comparisonDatePickers=[]}function bindMetaReportComparisonDateInputs(e,{editable:t=!1}={}){if(destroyMetaReportComparisonDatePickers(),typeof flatpickr>"u")return;const a=metaReportComparisonDateInputBounds(e),{periodA:n,periodB:s}=metaReportComparisonPeriodsFromState(e);document.querySelectorAll("[data-comparison-date]").forEach(o=>{const r=parseComparisonDateKey(o.getAttribute("data-comparison-date"));if(!r)return;const{side:i,bound:l}=r,c=i==="b"?s:n,d=c?.[`${l}Date`]||o.value||"";let u=a.min||o.getAttribute("data-min")||void 0,p=a.max||o.getAttribute("data-max")||void 0;l==="start"&&c?.endDate&&(p=c.endDate),l==="end"&&c?.startDate&&(u=c.startDate),{min:u,max:p}=metaReportComparisonSafePickerBounds(u,p);const h=(f,b)=>{b&&(b.preventDefault(),b.stopPropagation()),closeMetaReportComparisonDatePickers(f),f.open()},m=flatpickr(o,{dateFormat:"Y-m-d",altInput:!0,altFormat:"j M Y",defaultDate:d||void 0,minDate:u,maxDate:p,disableMobile:!0,allowInput:!1,clickOpens:!0,animate:!0,static:!1,appendTo:document.body,className:"meta-report-flatpickr",onOpen:(f,b,g)=>{closeMetaReportComparisonDatePickers(g)},onChange:(f,b,g)=>{!b||g._metaComparisonApplying||applyMetaReportComparisonDateChange(e,g.input,{editable:t,dateStr:b})}});o._metaComparisonDatePicker=m,metaReportsState.comparisonDatePickers.push(m);const $=o.closest(".meta-report-comparison-date-input-wrap"),v=o.closest(".meta-report-comparison-date-field");$&&($.onclick=f=>h(m,f)),v&&(v.onclick=f=>{f.target.closest(".meta-report-comparison-date-input-wrap")||h(m,f)}),m.altInput&&m.altInput.addEventListener("click",f=>h(m,f))})}function metaReportComparisonYtdSpanLabel(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n)return"";const s=Number(a.slice(5,7)),o=Number(n.slice(5,7)),r=a.slice(0,4),i=n.slice(0,4);return r!==i?metaReportComparisonFormatDateRangeLabel(a,n):s===1&&o===12?"Full year":s===o?`Through ${META_MONTH_LABELS[s-1]}`:`${META_MONTH_LABELS[s-1]} \u2013 ${META_MONTH_LABELS[o-1]}`}function metaReportComparisonPresetPeriodDisplay(e,t){const a=metaReportComparisonNormalizePeriod(t);if(!a)return{value:"\u2014",hint:""};if(e==="mom"){const n=a.startMonthKey||a.endMonthKey,s=String(n||a.startDate).slice(0,4);return{value:`${metaMonthLabel(n)} ${s}`,hint:""}}if(e==="ytd"){const n=metaReportComparisonYtdSpanLabel(a.startDate,a.endDate);return{value:a.startDate.slice(0,4),hint:n&&n!=="Full year"?n:""}}return{value:metaReportComparisonFormatDateRangeLabel(a.startDate,a.endDate),hint:""}}function renderMetaReportComparisonDuelSide(e,t,a=!1){const n=t.hint?`<span class="meta-report-comparison-duel-hint">${esc(t.hint)}</span>`:"",s=a?" is-partial":"",o=a?' title="Partial data"':"";return`
    <div class="meta-report-comparison-duel-side is-${esc(e)}${s}"${o}>
      <span class="meta-report-comparison-duel-value">${esc(t.value)}</span>
      ${n}
    </div>
  `}function renderMetaReportComparisonSummaryBar(e,t,a=null){const n=a?.periodA?.partialData===!0,s=a?.periodB?.partialData===!0,o=metaReportComparisonPresetPeriodDisplay(metaReportsState.comparisonMode||"mom",e),r=metaReportComparisonPresetPeriodDisplay(metaReportsState.comparisonMode||"mom",t);return`
    <div class="meta-report-comparison-duel" id="meta-report-comparison-range-panel" role="group" aria-label="Compared periods">
      ...
      ${renderMetaReportComparisonDuelSide("b",r,s)}
    </div>
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),$=META_REPORT_FEE_MODE_META.map(({value:g,icon:w,title:y,desc:S})=>`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,b=s&&!o?renderMetaReportBottomlineFeeSummary(e,t):"";return`
