(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);return e[0]==="admin"&&e[1]?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){return`
  ...
  `}async function loadTeamPage(){const e=document.getElementById("dashboard");if(await requireStaffAuth()){if(!isStaffAdmin()){window.location.href="/admin";return}e.innerHTML=`
    ...
    `}}}async function reloadTeamUsersTable(){const e=document.getElementById("team-users-content");if(!e)return;const t=await adminFetch("/api/auth/users");e.innerHTML=renderStaffUsersTable(t.users||[])}function renderStaffUserActions(e){const t=currentStaffUser?.id===e.id,n=e.status==="disabled"?`<button class="admin-btn" type="button" onclick="updateStaffUser('${esc(e.id)}', { status: 'active' })">Enable</button>`:`<button class="admin-btn" type="button" ${t?'disabled title="You cannot disable your own account"':""} onclick="updateStaffUser('${esc(e.id)}', { status: 'disabled' })">Disable</button>`,a=e.status==="pending"||!e.hasPassword?`<button class="admin-btn" type="button" onclick="copyStaffInviteLink('${esc(e.id)}')">Copy invite link</button>`:`<button class="admin-btn" type="button" onclick="resetStaffUserPassword('${esc(e.id)}', true)">Reset password</button>`,i=t?"":`<button class="admin-btn card-menu-item--danger" type="button" onclick="deleteStaffUser('${esc(e.id)}')">Delete</button>`;return`
  ...
  `}async function createStaffUserFromForm(){const e=document.getElementById("new-staff-email")?.value?.trim(),t=document.getElementById("new-staff-name")?.value?.trim(),n=document.getElementById("new-staff-role")?.value==="admin"?"admin":"member";if(!e){showToast("Email is required.","error");return}try{const a=await adminFetch("/api/auth/users",{method:"POST",body:JSON.stringify({email:e,name:t,role:n})});a.setupUrl&&await copyTextToClipboard(a.setupUrl,"Invite link copied to clipboard."),IS_TEAM_PAGE&&await reloadTeamUsersTable(),showToast("Staff member invited.","success")}catch(a){showToast(a.message,"error")}}async function updateStaffUser(e,t){try{await adminFetch(`/api/auth/users/${encodeURIComponent(e)}`,{method:"PATCH",body:JSON.stringify(t)}),IS_TEAM_PAGE&&await reloadTeamUsersTable(),showToast("User updated.","success")}catch(n){showToast(n.message,"error"),IS_TEAM_PAGE&&await reloadTeamUsersTable()}}async function updateStaffUserRole(e,t){await updateStaffUser(e,{role:t==="admin"?"admin":"member"})}async function deleteStaffUser(e){if(window.confirm("Delete this staff member? This permanently removes their account and cannot be undone."))try{await adminFetch(`/api/auth/users/${encodeURIComponent(e)}`,{method:"DELETE"}),IS_TEAM_PAGE&&await reloadTeamUsersTable(),showToast("Staff member deleted.","success")}catch(t){showToast(t.message,"error")}}async function copyStaffInviteLink(e){try{const t=await adminFetch(`/api/auth/users/${encodeURIComponent(e)}/reset-password`,{method:"POST",body:"{}"});t.setupUrl&&await copyTextToClipboard(t.setupUrl,"Invite link copied to clipboard.")}catch(t){showToast(t.message,"error")}}async function resetStaffUserPassword(e,t=!1){try{const n=await adminFetch(`/api/auth/users/${encodeURIComponent(e)}/reset-password`,{method:"POST",body:"{}"});n.setupUrl&&await copyTextToClipboard(n.setupUrl,t?"Reset link copied to clipboard.":"Password reset link generated.")}catch(n){showToast(n.message,"error")}}function renderLoginPage(){const e=new URLSearchParams(window.location.search),t=e.get("token"),n=e.get("next")||"/admin",a=e.get("saved")==="1",i=document.getElementById("dashboard"),s='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>',o='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.94 10.94 0 0112 19c-6.5 0-10-7-10-7a20.3 20.3 0 014.06-5.94M9.9 4.24A10.94 10.94 0 0112 5c6.5 0 10 7 10 7a20.3 20.3 0 01-3.17 4.49"/><path d="M1 1l22 22"/><path d="M14.12 14.12A3 3 0 009.88 9.88"/></svg>',f='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>';function u(m,h,w=""){return`
    ...
    `}if(t){i.innerHTML=`
        ...
        <div class="auth-card">
          <div class="auth-card-header">
            <div class="auth-card-title-row">
              <div class="auth-card-icon" aria-hidden="true">${f}</div>
          ...
          ${u("set-password","New password",`autocomplete="new-password" minlength="8" placeholder="At least 8 characters" oninput="updatePasswordStrength()" onkeydown="if(event.key==='Enter')submitSetPassword()"`)}
          <div id="password-strength" class="password-strength" aria-live="polite"></div>
          ${u("set-password-confirm","Confirm password",`autocomplete="new-password" minlength="8" placeholder="Repeat your password" onkeydown="if(event.key==='Enter')submitSetPassword()"`)}
      ...
      `)}
    `,initAuthPage("set-password");return}i.innerHTML=`
    ${renderBrandTopbar()}
    ${wrapDashboardShell(`
    <div class="auth-page">
      <div class="auth-card">
        ...
        ${u("login-password","Password",`autocomplete="current-password" placeholder="Enter your password" onkeydown="if(event.key==='Enter')submitStaffLogin()"`)}
  ...
  `:"";return`
    ...
    <div class="admin-hub">
      ...
      <div class="client-grid" id="client-grid">
        ${t?e.map(a=>renderClientCard(a)).join(""):`<div class="hub-empty">${isStaffAdmin()?"No clients yet. Create your first client below.":"No clients yet."}</div>`}
  ...
  `}function filterHubCards(){const e=(document.getElementById("hub-search")?.value||"").trim().toLowerCase();let t=0;document.querySelectorAll("#client-grid .client-card").forEach(a=>{const i=!e||(a.dataset.search||"").includes(e);a.style.display=i?"":"none",i&&(t+=1)});const n=document.getElementById("hub-no-results");n&&(n.style.display=e&&!t?"":"none")}function renderAddClientPanel(){return`
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(i){showToast(i.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{t&&(t.disabled=!1,t.textContent="Sync")}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_CHECK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>',ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",i=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",s=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
    <div class="metrics-model-panel${a}">
      ...
      <div class="metrics-option-cards">
        <label class="metrics-option-card">
          <input type="radio" name="metrics-model-type" value="simple" ${i==="simple"?"checked":""} onchange="onMetricsModelTypeChange()" />
        ...
        <label class="metrics-option-card">
          <input type="radio" name="metrics-model-type" value="dedupe" ${i==="dedupe"?"checked":""} onchange="onMetricsModelTypeChange()" />
          <span class="metrics-option-card-top">
      ...
      <div class="metrics-model-win-select" id="metrics-win-pipeline-wrap" style="${i==="dedupe"?"":"display:none"}">
    ...
    <div class="setup-panel">
      ...
      <div class="setup-actions setup-actions--sticky${t}">
        <div class="setup-actions-group">
          <button class="admin-btn" type="button" onclick="fetchSetupPipelines()">Fetch pipelines</button>
          <button class="admin-btn admin-btn--secondary" type="button" onclick="syncClient('${CLIENT_SLUG}')">${ICON_SYNC} Sync data</button>
  ...
  `}function renderAdminClientShell(){const e=document.getElementById("dashboard"),t={pipelines:[],statuses:[],sources:[],assignees:[],dateFields:[]};e.innerHTML=`
    ...
    `)}
  `}async function initAdminClientPage(){if(await requireStaffAuth()){renderAdminClientShell();try{await loadSetupAccount()}catch(t){const n=document.getElementById("setup-panel-mount");n&&(n.innerHTML=`<div class="note" style="padding:24px">${esc(t.message)}</div>`);return}accountCanPreviewDashboard(setupAccount)&&(ensureChartsVisible(),loadDashboard(!0))}}async function loadSetupAccount(){if(IS_ADMIN_CLIENT)try{setupAccount=(await adminFetch(`/api/clients/${encodeURIComponent(CLIENT_SLUG)}`)).account,setupAccount.accessKey&&!CLIENT_ACCESS_KEY&&(CLIENT_ACCESS_KEY=setupAccount.accessKey),setupAccount.newLeadsPipelineId&&!setupPipelines.length&&await fetchSetupPipelines(!0);const t=document.getElementById("setup-panel-mount");t&&(t.innerHTML=renderClientSetupPanel(setupAccount))}catch(e){const t=document.getElementById("setup-panel-mount");throw t&&(t.innerHTML=`<div class="note" style="padding:24px">${esc(e.message)}</div>`),showToast(e.message,"error"),e}}async function fetchSetupPipelines(e=!1){try{setupPipelines=(await adminFetch(`/api/clients/${encodeURIComponent(CLIENT_SLUG)}/sync-pipelines`,{method:"POST",body:"{}"})).pipelines||[],e||showToast(`${setupPipelines.length} pipeline(s) loaded`,"success"),await loadSetupAccount()}catch(t){showToast(t.message,"error")}}async function saveSetupAccount(){try{const e={locationId:document.getElementById("setup-location-id")?.value?.trim()||null,timezone:document.getElementById("setup-timezone")?.value?.trim()||"Europe/Copenhagen",newLeadsPipelineId:document.getElementById("setup-new-leads")?.value||null,salesPipelineId:document.getElementById("setup-sales")?.value||null,afterSalesPipelineId:document.getElementById("setup-after-sales")?.value||null,readyForGhl:!!document.getElementById("setup-ready-ghl")?.checked},t=document.getElementById("setup-ghl-token")?.value?.trim();t&&(e.ghlToken=t),await adminFetch(`/api/clients/${encodeURIComponent(CLIENT_SLUG)}`,{method:"PUT",body:JSON.stringify(e)}),showToast("Account saved","success"),await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0)}catch(e){showToast(e.message,"error")}}(function(t){const n=["#ff6a00","#138b53","#0085f2","#dc640a","#833b08","#a07868","#ff9147","#6b5348"],a={open:"#0085f2",won:"#138b53",lost:"#dc640a",abandoned:"#a07868"};function i(l){const c=String(l).match(/^(\d{4})-W(\d{2})$/);if(!c)return null;const d=Number(c[1]),v=Number(c[2]),r=new Date(Date.UTC(d,0,4)),p=r.getUTCDay()||7,y=new Date(r);y.setUTCDate(r.getUTCDate()-p+1);const b=new Date(y);b.setUTCDate(y.getUTCDate()+(v-1)*7);const k=new Date(b);return k.setUTCDate(b.getUTCDate()+3),{monday:new Date(b.getUTCFullYear(),b.getUTCMonth(),b.getUTCDate()),thursday:new Date(k.getUTCFullYear(),k.getUTCMonth(),k.getUTCDate())}}function s(l){let c=0;for(let d=1;d<=l.getDate();d+=1)new Date(l.getFullYear(),l.getMonth(),d).getDay()===1&&(c+=1);return c}function o(l){const c=i(l);if(!c)return String(l).replace("-W"," W");const d=c.monday,v=d.toLocaleDateString("da-DK",{month:"short"}).replace(".","").replace(/^\w/u,p=>p.toUpperCase()),r=s(d);return`${v} W${r}`}function f(l){const[c,d]=String(l).split("-");return new Date(Number(c),Number(d)-1,1).toLocaleDateString("da-DK",{month:"short",year:"2-digit"})}function u(l,c=18){const d=String(l);return d.length>c?`${d.slice(0,c-1)}\u2026`:d}function m(){return{text:"#1a1208",muted:"#6b5348",grid:"rgba(26, 18, 8, 0.1)",border:"#e8e0d8",tooltipBg:"#ffffff",tooltipBorder:"#e8e0d8",tooltipText:"#1a1208"}}function h(l,c,d,v){return{labels:l.map(r=>v(r[c])),values:l.map(r=>Number(r[d])||0)}}function w(l){const c=Number(l)||0;return c>=1e6?`${(c/1e6).toFixed(1)}M`:c>=1e3?`${Math.round(c/1e3)}K`:c}function A(l){const c=new Map((l.monthlyAdSpend||[]).map(r=>[r.month,Number(r.spend)||0])),d=new Map((l.monthlyRevenue||[]).map(r=>[r.month,Number(r.revenue)||0])),v=[...new Set([...c.keys(),...d.keys()])].sort();return{dualAxis:!0,labels:v.map(r=>f(r)),spendValues:v.map(r=>c.get(r)||0),revenueValues:v.map(r=>d.get(r)||0)}}function S(l,c,d){const v=c.spendValues.some(r=>r>0)||c.revenueValues.some(r=>r>0);return!c.labels.length||!v?null:{type:"bar",data:{labels:c.labels,datasets:[{type:"bar",label:"Ad Spend",data:c.spendValues,backgroundColor:"#ff6a00cc",borderColor:"#ff6a00",borderWidth:2,borderRadius:6,maxBarThickness:42,yAxisID:"y",order:2},{type:"line",label:"Won Revenue",data:c.revenueValues,backgroundColor:"#138b5333",borderColor:"#138b53",borderWidth:2.5,fill:!0,tension:.35,pointRadius:4,pointHoverRadius:6,yAxisID:"y1",order:1}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"bottom",align:"center",labels:{color:d.text,boxWidth:12,boxHeight:12,padding:14}},tooltip:{backgroundColor:d.tooltipBg,borderColor:d.tooltipBorder,borderWidth:1,titleColor:d.tooltipText,bodyColor:d.muted,padding:12,callbacks:{label(r){const p=r.parsed.y??0;return`${r.dataset.label}: Dkr ${Math.round(p).toLocaleString("da-DK")}`}}}},scales:{x:{ticks:{color:d.muted,maxRotation:45,minRotation:0},grid:{color:d.grid},border:{color:d.border}},y:{type:"linear",position:"left",title:{display:!0,text:"Ad Spend (Dkr)",color:"#ff6a00",font:{size:11,weight:"600"}},ticks:{color:"#ff6a00",callback:w},grid:{color:d.grid},border:{color:d.border},beginAtZero:!0},y1:{type:"linear",position:"right",title:{display:!0,text:"Won Revenue (Dkr)",color:"#138b53",font:{size:11,weight:"600"}},ticks:{color:"#138b53",callback:w},grid:{drawOnChartArea:!1},border:{color:d.border},beginAtZero:!0}}}}}const $={weeklyRevenue:{title:"Won Revenue (Weekly)",defaultType:"area",format:"currency",extract(l){return h(l.weeklyRevenue||[],"week","revenue",o)}},monthlyRevenue:{title:"Won Revenue (Monthly)",defaultType:"area",format:"currency",extract(l){return h(l.monthlyRevenue||[],"month","revenue",f)}},weeklyLeads:{title:"New Leads (Weekly)",defaultType:"area",format:"number",extract(l){return h(l.weeklyLeads||[],"week","count",o)}},monthlyLeads:{title:"New Leads (Monthly)",defaultType:"area",format:"number",extract(l){return h(l.monthlyLeads||[],"month","count",f)}},conversionTrend:{title:"Conversion Rate Trend",defaultType:"line",format:"percent",extract(l){return h(l.monthlyConversion||[],"month","rate",f)}},statusBreakdown:{title:"Opportunity Status",defaultType:"doughnut",format:"number",extract(l){const c=l.chartStatusBreakdown||l.statusBreakdown||{},d=["open","won","lost","abandoned"];return{labels:["Open","Won","Lost","Abandoned"],values:d.map(v=>Number(c[v])||0),colors:d.map(v=>a[v])}}},marketingSpendComparison:{title:"Facebook Ad Spend",defaultType:"area",format:"currency",extract(l){return h((l.monthlyAdSpend||[]).slice(-8),"month","spend",f)}},monthlyCostPerLead:{title:"Cost per Lead (Monthly)",defaultType:"area",format:"currency",extract(l){return h(l.monthlyCostPerLead||[],"month","cpl",f)}}};function I(l,c,d,v){const r=Number(l)||0,p=Array.isArray(d)?d[v]:c;return p==="ratio"?`${r.toFixed(2)}x`:p==="currency"||c==="currency"?`Dkr ${Math.round(r).toLocaleString("da-DK")}`:p==="percent"||c==="percent"?`${r.toFixed(1)}%`:Math.round(r).toLocaleString("da-DK")}function T(l,c,d){const v=$[l];if(!v)return null;const r=v.extract(c),p=m();if(r.dualAxis||d==="dualAxis")return S(v,r,p);if(!r.labels?.length||r.values.every(g=>g===0))return null;const y=["pie","doughnut","polarArea"].includes(d),b=r.colors||r.labels.map((g,C)=>n[C%n.length]),k=d==="area"?"line":d==="horizontalBar"?"bar":d,M=`${n[0]}55`,D=d==="doughnut",E={label:v.title,data:r.values,backgroundColor:y?D?b:b.map(g=>`${g}cc`):r.colors?r.colors.map(g=>`${g}cc`):d==="area"?M:`${n[0]}cc`,borderColor:y?b:r.colors||n[0],borderWidth:y?D?0:1:2,fill:d==="area",tension:.35,borderRadius:y?0:6,maxBarThickness:42,...D?{cutout:"62%",borderAlign:"inner"}:{}};return{type:k,data:{labels:r.labels,datasets:[E]},options:{responsive:!0,maintainAspectRatio:D?!1:y,indexAxis:d==="horizontalBar"?"y":"x",plugins:{legend:{display:y,position:"bottom",align:"center",labels:{color:p.text,boxWidth:12,boxHeight:12,padding:14}},tooltip:{backgroundColor:p.tooltipBg,borderColor:p.tooltipBorder,borderWidth:1,titleColor:p.tooltipText,bodyColor:p.muted,padding:12,callbacks:{label(g){const C=g.parsed,L=typeof C=="object"?C.y??C.x??0:C??0;return`${g.label}: ${I(L,v.format,r.valueFormats,g.dataIndex)}`}}}},...D?{layout:{padding:8},devicePixelRatio:typeof window<"u"?Math.min(window.devicePixelRatio||1,2):1,elements:{arc:{borderAlign:"inner"}}}:{},scales:y?{}:{x:{ticks:{color:p.muted,maxRotation:45,minRotation:0},grid:{color:p.grid},border:{color:p.border}},y:{ticks:{color:p.muted,callback(g,C){return v.format==="mixed"&&r.valueFormats?.[C]==="ratio"?`${Number(g).toFixed(2)}x`:v.format==="currency"||r.valueFormats?.[C]==="currency"?g>=1e6?`${(g/1e6).toFixed(1)}M`:g>=1e3?`${Math.round(g/1e3)}K`:g:v.format==="percent"?`${g}%`:g}},grid:{color:p.grid},border:{color:p.border},beginAtZero:!0}}}}}t.DashboardCharts={CHART_DEFINITIONS:$,buildChartConfig:T,formatTooltipValue:I}})(window);const STORAGE_KEY=`cenhub_display_${CLIENT_SLUG}`,LEGACY_STORAGE_KEY="suntech-dashboard-display",PIPELINE_KEY="suntech-dashboard-pipelines",DISPLAY_OPTIONS={kpis:{totalRevenue:"Total Revenue",adSpend:"Ad Spend",roas:"ROAS",roasDk:"ROAS (DK)",poas:"POAS",poasDk:"POAS (DK)",costPerLead:"Cost per Lead",costPerWonClient:"Cost per Won Client",clientsWon:"Clients Won",totalLeads:"Total Leads",totalLeadsValue:"Total Leads Value",averageLeadValue:"Average Lead Value",conversionRate:"Conversion Rate",totalBundlinje:"Total Bundlinje",openLeads:"Open Leads",openPipelineValue:"Open Pipeline Value",averageWonDealSize:"Avg Won Deal Size"},sections:{statusBreakdown:"Opportunity Status (Cards)",sourceReport:"Lead Source Report",assigneeReport:"Leads Closed by Assignee",pipelineBreakdown:"Pipeline Breakdown"},charts:{weeklyRevenue:"Won Revenue (Weekly)",monthlyRevenue:"Won Revenue (Monthly)",marketingSpendComparison:"Facebook Ad Spend",monthlyCostPerLead:"Cost per Lead (Monthly)",weeklyLeads:"New Leads (Weekly)",monthlyLeads:"New Leads (Monthly)",conversionTrend:"Conversion Rate Trend",statusBreakdown:"Opportunity Status"},statusItems:{open:"Open",won:"Won",lost:"Lost",abandoned:"Abandoned"},columns:{sourceReport:{totalLeads:"Total leads",totalValue:"Total values",open:"Open",won:"Won",lost:"Lost",abandoned:"Abandoned",winPct:"Win %"},assigneeReport:{won:"Won",totalLeads:"Total leads",wonValue:"Won revenue",totalValue:"Total value"},pipelineBreakdown:{count:"Leads",won:"Won",monetary:"Value",profit:"Bundlinje",wonValue:"Won revenue"}}},state={pipelineIds:usesClientPipelineDefaults()?[]:loadPipelineSelection(),status:"all",source:"all",assignedTo:"all",dateField:"createdAt",dateFrom:"",dateTo:"",adSpend:"",preset:"all"};let cachedData=null,cachedFacebookMetrics=null,cachedMonthlyAdSpend=null,availablePipelines=[],display=loadDisplayPrefs(),pipelineDefaultsApplied=!1,chartInstances={},chartFieldsCache=null,chartFieldsCacheKey=null,lastFetchedAt=0;const DATA_FRESH_MS=60*1e3,CHART_FIELD_KEYS=["weeklyRevenue","monthlyRevenue","weeklyLeads","monthlyLeads","monthlyLeadsValue","monthlyConversion","chartStatusBreakdown"];function getChartCacheKey(){return[[...state.pipelineIds].sort().join(","),state.status,state.source,state.assignedTo,state.dateField,state.dateFrom||"",state.dateTo||""].join("|")}function cacheChartFields(e){chartFieldsCache={},CHART_FIELD_KEYS.forEach(t=>{e[t]!==void 0&&(chartFieldsCache[t]=e[t])}),chartFieldsCacheKey=getChartCacheKey()}function applyChartFieldsCache(e){if(!!!(state.dateFrom||state.dateTo))return cacheChartFields(e),e;if(chartFieldsCache&&chartFieldsCacheKey===getChartCacheKey()){const n={...e};return CHART_FIELD_KEYS.forEach(a=>{chartFieldsCache[a]!==void 0&&(n[a]=chartFieldsCache[a])}),n}return cacheChartFields(e),e}function getDefaultChartPrefs(){const e={};return(window.DashboardCharts?Object.keys(DashboardCharts.CHART_DEFINITIONS):Object.keys(DISPLAY_OPTIONS.charts)).forEach(n=>{e[n]=!0}),e}function destroyCharts(){Object.values(chartInstances).forEach(e=>e.destroy()),chartInstances={}}function mountCharts(e){typeof Chart>"u"||!window.DashboardCharts||(destroyCharts(),Object.keys(DashboardCharts.CHART_DEFINITIONS).forEach(t=>{if(!isVisible("charts",t))return;const n=DashboardCharts.CHART_DEFINITIONS[t],a=document.getElementById(`chart-${t}`);if(!a)return;const s=a.closest(".chart-card")?.querySelector(".chart-empty"),o=n?.defaultType||"bar",f=DashboardCharts.buildChartConfig(t,e,o);if(!f){a.style.display="none",s&&(s.style.display="block");return}a.style.display="block",s&&(s.style.display="none"),chartInstances[t]=new Chart(a,f)}))}function usesClientPipelineDefaults(){return IS_CLIENT_VIEW||IS_ADMIN}function getPipelineStorageKey(){return`cenhub_pipelines_${facebookClientId||CLIENT_SLUG}`}function loadPipelineSelection(){try{const e=JSON.parse(localStorage.getItem(getPipelineStorageKey())||"[]");return Array.isArray(e)?e:[]}catch{return[]}}function getAllPipelineIds(e){return e.map(t=>t.id)}function getDefaultPipelineIds(e,t=[]){const n=getAllPipelineIds(e),a=(t||[]).filter(i=>n.includes(i));return a.length?a:n.length?n:[]}function ensurePipelineDefaults(e,t=[]){if(!e.length)return;if(usesClientPipelineDefaults()){state.pipelineIds=getDefaultPipelineIds(e,t),pipelineDefaultsApplied=!0;return}const n=getAllPipelineIds(e);let a=loadPipelineSelection();if(!a.length)try{a=JSON.parse(localStorage.getItem(PIPELINE_KEY)||"[]")}catch{a=[]}!a.length||!pipelineDefaultsApplied?(a.length?(state.pipelineIds=a.filter(i=>n.includes(i)),state.pipelineIds.length||(state.pipelineIds=getDefaultPipelineIds(e,t))):state.pipelineIds=getDefaultPipelineIds(e,t),pipelineDefaultsApplied=!0,savePipelineSelection()):state.pipelineIds.length||(state.pipelineIds=getDefaultPipelineIds(e,t),savePipelineSelection())}function ensurePipelineSelectionBeforeFetch(){return state.pipelineIds.length?!0:availablePipelines.length?(state.pipelineIds=getDefaultPipelineIds(availablePipelines,cachedData?.account?.defaultPipelineIds),state.pipelineIds.length&&!usesClientPipelineDefaults()&&savePipelineSelection(),state.pipelineIds.length>0):!1}function savePipelineSelection(){localStorage.setItem(getPipelineStorageKey(),JSON.stringify(state.pipelineIds))}function isPipelineSelected(e){return state.pipelineIds.includes(e)}function isAllPipelinesSelected(e){const t=getAllPipelineIds(e);return t.length>0&&t.every(n=>state.pipelineIds.includes(n))}function selectAllPipelines(e){state.pipelineIds=getAllPipelineIds(e),savePipelineSelection()}function clearPipelineSelection(){state.pipelineIds=[],savePipelineSelection()}function setPipelineSelection(e){state.pipelineIds=[...new Set(e)],savePipelineSelection()}function togglePipelineSelection(e){isPipelineSelected(e)?state.pipelineIds=state.pipelineIds.filter(t=>t!==e):state.pipelineIds=[...state.pipelineIds,e],savePipelineSelection()}function formatSelectedPipelines(e){return state.pipelineIds.length?isAllPipelinesSelected(e)?"All pipelines":state.pipelineIds.map(t=>e.find(n=>n.id===t)?.name||t).join(", "):"None selected"}function renderPipelineChips(e){return state.pipelineIds.length?`
  ...
  `:'<div class="pipeline-warning">Select at least one pipeline, then click Apply data filters.</div>'}function defaultDisplayPrefs(){const e={kpis:{},sections:{},charts:getDefaultChartPrefs(),statusItems:{},columns:{}};return Object.keys(DISPLAY_OPTIONS.kpis).forEach(t=>{e.kpis[t]=!0}),Object.keys(DISPLAY_OPTIONS.sections).forEach(t=>{e.sections[t]=!0}),Object.keys(DISPLAY_OPTIONS.charts).forEach(t=>{e.charts[t]=!0}),Object.keys(DISPLAY_OPTIONS.statusItems).forEach(t=>{e.statusItems[t]=!0}),Object.entries(DISPLAY_OPTIONS.columns).forEach(([t,n])=>{e.columns[t]={},Object.keys(n).forEach(a=>{e.columns[t][a]=!0})}),e}function loadDisplayPrefs(){if(IS_CLIENT_VIEW&&!IS_PREVIEW)return defaultDisplayPrefs();try{const e=JSON.parse(localStorage.getItem(STORAGE_KEY)||localStorage.getItem(LEGACY_STORAGE_KEY)||"null");if(!e)return defaultDisplayPrefs();const t=defaultDisplayPrefs();return{kpis:{...t.kpis,...e.kpis},sections:{...t.sections,...e.sections},charts:{...t.charts,...e.charts},statusItems:{...t.statusItems,...e.statusItems},columns:{sourceReport:{...t.columns.sourceReport,...e.columns?.sourceReport||{}},assigneeReport:{...t.columns.assigneeReport,...e.columns?.assigneeReport||{}},pipelineBreakdown:{...t.columns.pipelineBreakdown,...e.columns?.pipelineBreakdown||{}}}}}catch{return defaultDisplayPrefs()}}function ensureChartsVisible(){const e=Object.keys(DISPLAY_OPTIONS.charts);e.some(n=>display.charts[n]!==!1)||(e.forEach(n=>{display.charts[n]=!0}),saveDisplayPrefs())}function saveDisplayPrefs(){localStorage.setItem(STORAGE_KEY,JSON.stringify(display))}function isVisible(e,t,n){return e==="columns"?display.columns[n]?.[t]!==!1:display[e]?.[t]!==!1}function toggleDisplay(e,t,n){e==="columns"?display.columns[n][t]=!display.columns[n][t]:display[e][t]=!display[e][t],saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}function setAllDisplay(e,t,n){e==="columns"?Object.keys(DISPLAY_OPTIONS.columns[n]).forEach(a=>{display.columns[n][a]=t}):Object.keys(DISPLAY_OPTIONS[e]).forEach(a=>{display[e][a]=t}),saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}const fmt=e=>new Intl.NumberFormat("da-DK",{minimumFractionDigits:0,maximumFractionDigits:0}).format(Math.round(Number(e)||0)),fmtCompact=e=>{const t=Number(e)||0;return t>=1e6?`${(t/1e6).toFixed(2)}M`:t>=1e3?`${(t/1e3).toFixed(2)}K`:fmt(t)},fmtPct=e=>`${(Number(e)||0).toFixed(2)}%`,fmtRoas=e=>{const t=Number(e)||0;return t>0?`${t.toFixed(2)}x`:"\u2014"};function formatActiveDateFilter(e){if(!e.dateFrom&&!e.dateTo)return"Till date";const t=getDashboardTimeZone();if(window.MarketingMetrics?.formatShortDateLabel){const n=window.MarketingMetrics.formatShortDateLabel(e.dateFrom,t),a=window.MarketingMetrics.formatShortDateLabel(e.dateTo,t);if(n&&a)return`${n} \u2013 ${a}`}return`${e.dateFrom||"start"} to ${e.dateTo||"now"}`}function showDateRangeError(e){document.querySelectorAll("#date-range-error").forEach(t=>{t.textContent=e,t.hidden=!e})}function clearDateRangeError(){showDateRangeError("")}function needsFreshData(){return!lastFetchedAt||Date.now()-lastFetchedAt>DATA_FRESH_MS}function buildQuery(e,t={}){const n=new URLSearchParams,a=getAllPipelineIds(e||[]),i=state.pipelineIds.filter(s=>a.includes(s));if(!i.length)throw new Error("Select at least one pipeline.");return n.set("pipelineIds",i.join(",")),state.dateField&&n.set("dateField",state.dateField),state.dateFrom&&n.set("dateFrom",state.dateFrom),state.dateTo&&n.set("dateTo",state.dateTo),["status","source","assignedTo","adSpend"].forEach(s=>{state[s]&&state[s]!=="all"&&n.set(s,state[s])}),appendTenantParams(n),t.forceFresh&&n.set("fresh","1"),n.toString()}function formatDateInput(e){const t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}function getDashboardTimeZone(){return cachedData?.account?.timezone||"Europe/Copenhagen"}function getCalendarPartsInTimeZone(e,t=new Date){const n=new Intl.DateTimeFormat("en-CA",{timeZone:e,year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(t);return{year:Number(n.find(a=>a.type==="year")?.value),month:Number(n.find(a=>a.type==="month")?.value),day:Number(n.find(a=>a.type==="day")?.value)}}function formatMonthDateRange(e,t){const n=`${e}-${String(t).padStart(2,"0")}-01`,a=new Date(e,t,0).getDate(),i=`${e}-${String(t).padStart(2,"0")}-${String(a).padStart(2,"0")}`;return{start:n,end:i}}const datePickerState={inputId:null,anchorEl:null,viewYear:null,viewMonth:null};let datePickerListenersBound=!1;function isoFromParts(e,t,n){const a=Number(n);return!e||!t||!Number.isFinite(a)?"":`${e}-${String(t).padStart(2,"0")}-${String(a).padStart(2,"0")}`}function formatPickerDisplayLabel(e,t){if(!e||!/^\d{4}-\d{2}-\d{2}$/.test(e))return t;const n=getDashboardTimeZone();return window.MarketingMetrics?.formatShortDateLabel&&window.MarketingMetrics.formatShortDateLabel(e,n)||e}function ensureDatePickerPopover(){if(document.getElementById("date-picker-popover"))return;const e=document.createElement("div");e.id="date-picker-backdrop",e.className="date-picker-backdrop",e.hidden=!0,e.addEventListener("click",closeDatePicker),document.body.appendChild(e);const t=document.createElement("div");t.id="date-picker-popover",t.className="date-picker-popover",t.hidden=!0,t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),document.body.appendChild(t)}function syncDatePickerDisplays(){[{id:"dateFrom",key:"dateFrom",placeholder:"Pick start date"},{id:"dateTo",key:"dateTo",placeholder:"Pick end date"}].forEach(({id:e,key:t,placeholder:n})=>{const a=document.getElementById(e),i=document.getElementById(`${e}-display`),s=document.getElementById(`${e}-trigger`),o=state[t]??"";a&&(a.value=o),i&&(i.textContent=formatPickerDisplayLabel(o,n)),s?.classList.toggle("is-empty",!o),s?.classList.toggle("has-value",!!o)})}function getStateDateValue(e){return state[e]??""}function getDatePickerViewMonth(e){const t=getStateDateValue(e);if(/^\d{4}-\d{2}-\d{2}$/.test(t)){const[n,a]=t.split("-").map(Number);return{year:n,month:a}}return getCalendarPartsInTimeZone(getDashboardTimeZone())}function getTodayIso(){const e=getCalendarPartsInTimeZone(getDashboardTimeZone());return isoFromParts(e.year,e.month,e.day)}function isStartDateDisabled(e){return e>=getTodayIso()}function isFutureDateDisabled(e){return e>getTodayIso()}function isFutureMonthView(e,t){const n=getCalendarPartsInTimeZone(getDashboardTimeZone());return e>n.year||e===n.year&&t>n.month}function isDatePickerDayDisabled(e,t){return t==="dateFrom"&&isStartDateDisabled(e)?!0:isFutureDateDisabled(e)}function renderDatePickerDayCell(e,t,n,a,i,s,o,f){const u=["date-picker-day"];if(n&&u.push("is-outside"),e===a&&u.push("is-today"),e===i&&u.push("is-selected"),s&&o){const m=s<=o?s:o,h=s<=o?o:s;e>=m&&e<=h&&u.push("is-in-range")}return e===s&&u.push("is-range-start"),e===o&&u.push("is-range-end"),isDatePickerDayDisabled(e,f)?(u.push("is-disabled"),`<button type="button" class="${u.join(" ")}" disabled aria-disabled="true">${t}</button>`):`<button type="button" class="${u.join(" ")}" onclick="selectDatePickerDay('${e}')">${t}</button>`}function renderDatePickerPopover(){const e=document.getElementById("date-picker-popover");if(!e||!datePickerState.inputId)return;const{viewYear:t,viewMonth:n,inputId:a}=datePickerState,i=new Intl.DateTimeFormat("en-GB",{month:"long",year:"numeric"}).format(new Date(t,n-1,1)),s=["Mo","Tu","We","Th","Fr","Sa","Su"],f=(new Date(t,n-1,1).getDay()+6)%7,u=new Date(t,n,0).getDate(),m=new Date(t,n-1,0).getDate(),h=getCalendarPartsInTimeZone(getDashboardTimeZone()),w=isoFromParts(h.year,h.month,h.day),A=getStateDateValue(a),S=getStateDateValue("dateFrom"),$=getStateDateValue("dateTo");let I="";for(let p=f-1;p>=0;p-=1){const y=m-p,b=n===1?12:n-1,k=n===1?t-1:t,M=isoFromParts(k,b,y);I+=renderDatePickerDayCell(M,y,!0,w,A,S,$,a)}for(let p=1;p<=u;p+=1){const y=isoFromParts(t,n,p);I+=renderDatePickerDayCell(y,p,!1,w,A,S,$,a)}const T=(7-(f+u)%7)%7;for(let p=1;p<=T;p+=1){const y=n===12?1:n+1,b=n===12?t+1:t,k=isoFromParts(b,y,p);I+=renderDatePickerDayCell(k,p,!0,w,A,S,$,a)}const l=a==="dateFrom"?'<span class="date-picker-hint">Start date must be before today</span>':"",c=a==="dateTo"?'<button type="button" class="date-picker-footer-btn primary" onclick="setDatePickerToday()">Today</button>':"",d=n===12?1:n+1,v=n===12?t+1:t,r=!isFutureMonthView(v,d);e.innerHTML=`
    <div class="date-picker-panel">
      ...
      <div class="date-picker-weekdays">
        ${s.map(p=>`<div class="date-picker-weekday">${p}</div>`).join("")}
      </div>
      <div class="date-picker-grid">${I}</div>
  ...
  `:'<div class="pipeline-note">No pipelines found.</div>'}function renderSelect(e,t,n,a){return`
    <div class="filter-group">
      ...
      <select id="${e}" onchange="onFilterChange('${e}', this.value)">
        ${n.map(i=>`
          <option value="${esc(i.id)}" ${i.id===a?"selected":""}>${esc(i.name)}</option>
        `).join("")}
      </select>
    </div>
  `}function renderCheckboxGroup(e,t,n,a){const i=Object.entries(n);return`
  ...
  `:`
    ...
    `}function renderTable(e,t,n,a,i){const s=n.filter(o=>isVisible("columns",o.key,e));return s.length?`
    <table>
      <thead>
        <tr>
          <th>${t}</th>
          ${s.map(o=>`<th class="${o.align||""}">${o.label}</th>`).join("")}
        </tr>
      </thead>
      <tbody>
        ${a.length?a.map(o=>`
          <tr>
            <td>${i(o,"label")}</td>
            ${s.map(f=>`<td class="${f.align||""}">${i(o,f.key)}</td>`).join("")}
          </tr>
        `).join(""):`<tr><td colspan="${s.length+1}">Ingen data for valgte filtre.</td></tr>`}
      </tbody>
    </table>
  `:'<div class="empty-section">No columns selected for this table.</div>'}function renderDatePickerTrigger(e,t,n){const a=state[e]||"",i=formatPickerDisplayLabel(a,n);return`
    <div class="date-picker-field">
      ...
      >
        ...
        <span class="date-picker-value" id="${e}-display">${esc(i)}</span>
      </button>
  ...
  `}function clientKpiCopy(e,t){return IS_CLIENT_VIEW&&!IS_PREVIEW?t:e}function getAdSpendSubtitle(e){if(e.adSpendSource!=="facebook")return"No ad spend data for this period";if(state.preset==="all")return"Ad spend";const t=e.adSpendLabel;if(!t||t==="Custom range"||t==="Till date")return"Ad spend";const n=e.adSpendShowAsAvg?"Avg ad spend":"Ad spend";return/^\d{4}$/.test(t)?`${n} year ${t}`:`${n} ${t.toLowerCase()}`}function getAdSpendDisplayValue(e){return e.adSpendShowAsAvg&&Number(e.adSpendDailyAvg)>0?Number(e.adSpendDailyAvg):Number(e.adSpend)||0}function renderKpiCards(e){const t=[];if(isVisible("kpis","totalRevenue")&&t.push(`
    ...
    `),isVisible("kpis","adSpend")){const n=getAdSpendSubtitle(e),a=getAdSpendDisplayValue(e);t.push(`
      <div class="card accent">
        <div class="card-label">Ad Spend</div>
        <div class="card-value">${a>0?`Dkr ${fmtCompact(a)}`:"\u2014"}</div>
  ...
  `}function renderDashboardContent(e){const{kpis:t,statusBreakdown:n,sourceReport:a,assigneeReport:i,pipelines:s,filters:o}=e,f=isVisible("sections","statusBreakdown");return`
      ...
      <div class="section-grid">
        ${isVisible("sections","assigneeReport")?`
          <div class="card">
            ...
            ${renderTable("assigneeReport","Assignee",[{key:"won",label:"Won",align:"num"},{key:"totalLeads",label:"Total leads",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"},{key:"totalValue",label:"Total value",align:"num"}],i,(u,m)=>m==="label"?esc(u.assigneeName):m==="wonValue"||m==="totalValue"?`Dkr ${fmt(u[m])}`:fmt(u[m]))}
          </div>
        `:""}
        ${isVisible("sections","pipelineBreakdown")?`
          <div class="card">
            <div class="section-title">Pipeline Breakdown</div>
            ${renderTable("pipelineBreakdown","Pipeline",[{key:"count",label:"Leads",align:"num"},{key:"won",label:"Won",align:"num"},{key:"monetary",label:"Value",align:"num"},{key:"profit",label:"Bundlinje",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"}],s,(u,m)=>m==="label"?esc(u.name):m==="monetary"||m==="profit"||m==="wonValue"?`Dkr ${fmt(u[m])}`:fmt(u[m]))}
          </div>
        `:""}
      </div>
    `:""}
  ...
  `}function updateDashboardContent(e){if(IS_ADMIN){const n=document.getElementById("admin-filters-panel");n&&(n.outerHTML=renderAdminFiltersPanel(e.filterOptions||{pipelines:[],statuses:[],sources:[],assignees:[],dateFields:[]}));const a=document.getElementById("admin-display-panel");if(a){const i=!!a.open;a.outerHTML=renderAdminDisplayOptions(i)}}const t=document.getElementById("dashboard-content");t&&(t.innerHTML=renderDashboardContent(e),mountCharts(e))}function renderDashboard(e){const t=document.getElementById("dashboard"),{filterOptions:n,account:a={}}=e,i=document.querySelector("details.panel")?.open,s=a.accountName||"Dashboard";IS_CLIENT_VIEW&&a.accountName&&(document.title=`${a.accountName} \xB7 Cenhub Dashboard`),t.innerHTML=`
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,i=!!a||e&&cachedData&&needsFreshData(),s=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),f=document.getElementById("apply-filters-btn");if(e||!cachedData){const u=++fetchGeneration;activeFetchController=new AbortController;const m=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)s.innerHTML=`
        ...
        `)}`;else if(!n){const h=document.getElementById("dashboard-content");h&&(h.innerHTML=renderKpiSkeleton()),f&&(f.disabled=!0,f.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let h=null;if(!availablePipelines.length){if(h=await bootstrapDashboardData(m),u!==fetchGeneration)return;availablePipelines=h.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,h.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const A=canReuseBootstrapDashboard(h)&&!i;let S,$;if(A?[$,S]=await Promise.all([fetchFacebookMetrics(m),Promise.resolve(h)]):[S,$]=await Promise.all([fetchDashboardData(availablePipelines,m,{forceFresh:i}),fetchFacebookMetrics(m)]),u!==fetchGeneration)return;cachedFacebookMetrics=$,cachedData=applyMarketingData(S,$),cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(h){if(u!==fetchGeneration)return;if(h.name==="AbortError"){o||(s.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            </div>`)}else o?f&&(f.textContent="Apply failed - try again"):s.innerHTML=`
          ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
          ${wrapDashboardShell(`
            <div class="error-state">
              <div>Fejl ved hentning af data</div>
          ...
          `)}`;return}finally{u===fetchGeneration&&(activeFetchController=null,resetFetchUiState()),pendingRefetch&&(pendingRefetch=!1,loadDashboard(!0))}if(u!==fetchGeneration)return}try{o?(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi()):(renderDashboard(cachedData),mountCharts(cachedData))}catch(u){resetFetchUiState(),s.innerHTML=`
      ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
      ${wrapDashboardShell(`
        <div class="error-state">
