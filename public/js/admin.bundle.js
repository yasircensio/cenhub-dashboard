...
Resume the interrupted ${b}?`)&&(l=m.id,d=Number(m.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const m=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),u=m.summary||m;l=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:g||w?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:$,batchOffset:d,batchLimit:o})});const m=c.summary||c;l=m.runId,d=m.nextBatchOffset??m.batchOffset+(m.batchProcessed||0),y=y.concat(m.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const u=m.inWindow||d||g;n?(setFbLeadProgress({processed:d,total:u,complete:!m.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...m,rows:y}))):updateGlobalApplyProgress({processed:d,total:u,written:m.updated||0,complete:!m.hasMore})}while((c.summary||c).hasMore);v=!0;const S=c.summary||c,f=S.inWindow||d||g,h=S.nextBatchOffset??d;if(n)setFbLeadProgress({processed:h,total:f,complete:!0}),markFbLeadPreviewReady(e,t,S.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(S.updated||0)>0),showToast((S.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:f,written:S.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const m=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete m[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(m))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(S){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${S.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((S.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!v&&n&&hideFbLeadProgress(!0),!n&&!v&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `}function bindMetaReportCopyButtons(e=document){e.querySelectorAll("[data-copy-report-url]").forEach(t=>{t.onclick=async()=>{const n=t.getAttribute("data-copy-report-url"),a=`${window.location.origin}${n}`;try{await navigator.clipboard.writeText(a),showToast("Share link copied","success")}catch{showToast(a,"info")}}})}async function patchMetaReportSettings(e,t){metaReportsState._settingsInflight=(metaReportsState._settingsInflight||0)+1;try{return await adminFetchWithRetry(`/api/meta-reports/clients/${encodeURIComponent(e)}/settings`,{method:"PATCH",body:JSON.stringify(t)},{retries:2,timeoutMs:2e4})}finally{metaReportsState._settingsInflight=Math.max(0,(metaReportsState._settingsInflight||1)-1)}}async function refreshMetaReportsHubData({silent:e=!0}={}){const t=document.getElementById("meta-reports-refresh");t&&(t.disabled=!0);try{const n=await adminFetch("/api/meta-reports?filter=all");metaReportsState.dashboardData=n,updateMetaReportsHubDom(getMetaReportsHubView(n,metaReportsState.filter,metaReportsState.searchQuery)),e||showToast("Meta reports refreshed","success")}catch(n){showToast(n.message||"Refresh failed","error")}finally{t&&(t.disabled=!1)}}function bindMetaReportsHubChromeEvents(){document.querySelectorAll("[data-meta-filter]").forEach(n=>{n.onclick=()=>{metaReportsState.filter=n.getAttribute("data-meta-filter")||"all",document.querySelectorAll("[data-meta-filter]").forEach(a=>{a.classList.toggle("is-active",a.getAttribute("data-meta-filter")===metaReportsState.filter)}),metaReportsState.dashboardData&&updateMetaReportsHubDom(getMetaReportsHubView(metaReportsState.dashboardData,metaReportsState.filter,metaReportsState.searchQuery))}});const e=document.getElementById("meta-reports-search");if(e){let n=null;e.oninput=()=>{clearTimeout(n),n=setTimeout(()=>{metaReportsState.searchQuery=e.value||"",metaReportsState.dashboardData&&updateMetaReportsHubDom(getMetaReportsHubView(metaReportsState.dashboardData,metaReportsState.filter,metaReportsState.searchQuery))},180)}}const t=document.getElementById("meta-reports-refresh");t&&(t.onclick=()=>refreshMetaReportsHubData({silent:!1}))}function bindMetaReportsHubRowEvents(e=document){e.querySelectorAll("[data-meta-report-enabled]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-report-enabled");t.disabled=!0;try{const s=(await patchMetaReportSettings(n,{metaReportEnabled:t.checked})).settings||{};mergeHubClient(n,{metaReportEnabled:s.metaReportEnabled,metaReportShowBottomline:s.metaReportShowBottomline,metaReportFeeEnabled:s.metaReportFeeEnabled,reportUrl:s.reportUrl}),showToast(t.checked?"Report enabled":"Report disabled","success"),updateMetaReportsHubDom(getMetaReportsHubView(metaReportsState.dashboardData,metaReportsState.filter,metaReportsState.searchQuery))}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),e.querySelectorAll("[data-meta-bottomline]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-bottomline");t.disabled=!0;try{const a=await patchMetaReportSettings(n,{metaReportShowBottomline:t.checked});mergeHubClient(n,{metaReportShowBottomline:a.settings?.metaReportShowBottomline}),showToast("Bottomline setting saved","success")}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),e.querySelectorAll("[data-meta-fee]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-fee");t.disabled=!0;try{const a=await patchMetaReportSettings(n,{metaReportFeeMode:t.checked?"performance":null});mergeHubClient(n,{metaReportFeeEnabled:a.settings?.metaReportFeeEnabled,metaReportFeeMode:a.settings?.metaReportFeeMode}),showToast("Fee setting saved","success")}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),bindMetaReportCopyButtons(e),e.querySelectorAll("[data-meta-provision]").forEach(t=>{t.onclick=async()=>{t.disabled=!0;try{await adminFetch("/api/meta-reports/provision",{method:"POST",body:JSON.stringify({accountName:t.getAttribute("data-meta-provision-name"),metaAdAccountId:t.getAttribute("data-meta-provision"),metaReportEnabled:!0})}),showToast("Client added","success"),await refreshMetaReportsHubData({silent:!0})}catch(n){showToast(n.message||"Failed to add client","error"),t.disabled=!1}}})}function bindMetaReportsHubEvents(){bindMetaReportsHubChromeEvents(),bindMetaReportsHubRowEvents(document.getElementById("meta-reports-cards")||document)}function bindMetaReportsClientTabEvents(e){const t=e.clientId;document.querySelectorAll("[data-meta-month-tab]").forEach(n=>{n.onclick=()=>{metaReportsState.activeMonthKey=n.getAttribute("data-meta-month-tab");const a=document.getElementById("meta-report-month-panel");a&&(a.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(e.settings,{editable:!0})),bindMetaReportsClientEditEvents(t,e)),document.querySelectorAll("[data-meta-month-tab]").forEach(s=>{s.classList.toggle("is-active",s.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)}),syncMetaReportSettingsControls(e.settings||{},e.months?.[metaReportsState.activeMonthKey]||null)}})}function setMetaReportSavingState(e){const t=document.getElementById("meta-report-month-panel"),n=document.getElementById("meta-report-save-indicator");t&&t.classList.toggle("is-loading",e),n&&n.classList.toggle("is-visible",e)}function syncMetaReportFeeFieldState(e,t="meta-report"){const n=document.getElementById(`${t}-fee-field`),a=document.getElementById(`${t}-setting-fee-percent`);n&&n.classList.toggle("is-disabled",!e),a&&(a.disabled=!e)}function syncMetaReportSettingsControls(e={},t=null){syncMetaReportBottomlineFeeDom(e,"meta-report")}function describeMetaReportSettingsChange(e,t){if(Object.prototype.hasOwnProperty.call(e,"metaReportFeeMode")){const n=t.metaReportFeeMode;return n?n==="marketing"?"Censio marketing fee enabled":"Censio performance fee enabled":"Censio fee disabled"}return Object.prototype.hasOwnProperty.call(e,"metaReportFeeEnabled")?t.metaReportFeeEnabled?"Censio fee enabled":"Censio fee disabled":Object.prototype.hasOwnProperty.call(e,"metaReportFeePercent")?`Performance fee set to ${t.metaReportFeePercent??20}% \u2014 report updated`:Object.prototype.hasOwnProperty.call(e,"metaReportMarketingFeeAmount")?`Marketing fee set to Dkr ${t.metaReportMarketingFeeAmount??0} \u2014 report updated`:Object.prototype.hasOwnProperty.call(e,"metaReportShowBottomline")?t.metaReportShowBottomline?"Bottomline shown on report":"Bottomline hidden from report":"Settings saved"}async function saveMetaReportClientSettings(e,t,n){setMetaReportSavingState(!0);try{const a=await patchMetaReportSettings(e,{...t,monthKey:metaReportsState.activeMonthKey}),s=a.settings||{},i=metaReportsState.clientPayload;i&&(i.settings=s,a.monthPayload&&metaReportsState.activeMonthKey&&(i.months=i.months||{},i.months[metaReportsState.activeMonthKey]=a.monthPayload));const o=a.monthPayload||i?.months?.[metaReportsState.activeMonthKey]||null;if(syncMetaReportSettingsControls(s,o),o){const r=document.getElementById("meta-report-month-panel");r&&(r.innerHTML=renderMetaReportMonthBody(o,getMetaReportMonthBodyOptions(s,{editable:!0})),bindMetaReportsClientEditEvents(e,i))}showToast(describeMetaReportSettingsChange(t,s),"success")}catch(a){showToast(a.message||"Update failed","error"),n&&n()}finally{setMetaReportSavingState(!1)}}function bindMetaReportBackfillButton(e){const t=document.getElementById("meta-report-backfill");t&&(t.onclick=async()=>{const n=metaReportsState.clientPayload,a=metaReportMonthsNeedingBackfill(n);if(!a.length)return;t.disabled=!0;const s=document.getElementById("meta-report-backfill-progress");let i=0,o=0;for(const r of a){i+=1,s&&(s.textContent=`Fetching ${metaMonthLabel(r)} ${r.slice(0,4)}\u2026 (${i}/${a.length})`);try{await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(r)}/refresh`,{method:"POST"})}catch{o+=1}}s&&(s.textContent=""),o?showToast(`Backfilled ${i-o}/${a.length} months. ${o} failed (check Meta token access).`,"error"):showToast(`Backfilled ${a.length} month${a.length===1?"":"s"} from Meta`,"success"),await loadMetaReportsClientPage({silent:!0})})}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,n=document.getElementById("meta-report-year");n&&(n.onchange=async()=>{if(n.selectedOptions?.[0]?.disabled){n.value=String(metaReportsState.selectedYear);return}metaReportsState.selectedYear=Number(n.value),metaReportsState.activeMonthKey=null,n.disabled=!0;try{await loadMetaReportsClientPage({silent:!0})}finally{n.disabled=!1}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report");const a=document.getElementById("meta-report-rotate-token");a&&(a.onclick=async()=>{if(window.confirm("Rotate the client share link? The old link will stop working.")){a.disabled=!0;try{const i=(await patchMetaReportSettings(t,{rotateAccessToken:!0})).settings?.reportUrl,o=metaReportsState.clientPayload;o&&(o.reportUrl=i);const r=document.getElementById("meta-report-copy-link");r&&i&&(r.setAttribute("data-copy-report-url",i),r.hidden=!1),showToast("Share link rotated","success")}catch(s){showToast(s.message||"Rotate failed","error")}finally{a.disabled=!1}}})}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const n=document.getElementById("meta-report-refresh-meta");n&&(n.onclick=async()=>{n.disabled=!0;try{const s=metaReportsState.activeMonthKey,i=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(i.monthPayload),showToast("Meta data refreshed","success")}catch(s){showToast(s.message||"Meta refresh failed","error")}finally{n.disabled=!1}});const a=document.getElementById("meta-report-save-month");a&&(a.onclick=async()=>{a.disabled=!0;try{const s=metaReportsState.activeMonthKey,i={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:Number(document.getElementById("meta-report-avg-profit")?.value)},o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}`,{method:"PUT",body:JSON.stringify(i)});refreshMetaReportMonthPanel(o.monthPayload),showToast("Month saved","success")}catch(s){showToast(s.message||"Save failed","error")}finally{a.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{metaReportsState.activeMonthKey=t.getAttribute("data-meta-month-tab");const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(e.settings,{editable:!1}))),document.querySelectorAll("[data-meta-month-tab]").forEach(a=>{a.classList.toggle("is-active",a.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}metaReportsState.selectedYear=Number(t.value),metaReportsState.activeMonthKey=null,t.disabled=!0;try{await loadPublicMetaReportPage({silent:!0})}finally{t.disabled=!1}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
    ...
    `).join(""):'<div class="meta-report-empty" style="padding:32px 12px">No matching clients.</div>'}function monthStatusIsEmpty(e,t={}){return e?(e.status??classifyMetaCvMonthStatus({wonLeads:e.inputs?.wonLeads??e.topline?.wonLeads??e.wonLeads,avgLeadValue:e.inputs?.avgLeadValue??e.topline?.avgLeadValue??e.avgLeadValue,avgProfitPerWon:e.inputs?.avgProfitPerWon??e.avgProfitPerWon},!!t.metaReportShowBottomline))==="empty":!0}function metaReportEffectiveFeeEnabled(e,t=null){if(!t)return!!e?.metaReportFeeEnabled;const n=!!e?.metaReportShowBottomline;return(t.status??classifyMetaCvMonthStatus({wonLeads:t.inputs?.wonLeads??t.topline?.wonLeads??t.wonLeads,avgLeadValue:t.inputs?.avgLeadValue??t.topline?.avgLeadValue??t.avgLeadValue,avgProfitPerWon:t.inputs?.avgProfitPerWon??t.avgProfitPerWon},n))==="empty"?!1:!!e?.metaReportFeeEnabled}function resolveMetaReportFeeMode(e={}){return e.metaReportFeeMode||(e.metaReportFeeEnabled?"performance":null)}const META_REPORT_FEE_MODE_META=[{value:"",icon:"\u2013",title:"None",desc:"No Censio fee"},{value:"performance",icon:"%",title:"Performance fee",desc:"% of profit (POAS)"},{value:"marketing",icon:"Dkr",title:"Marketing fee",desc:"Fixed Dkr amount"}];function metaReportFeeValueBlockHtml(e,t,n,a,{inline:s=!1}={}){return s&&t==="performance"?`
      ...
      </label>`:'<p class="meta-report-fee-value-empty">No amount needed \u2014 just save your selection.</p>'}function metaReportUsesInlineFeeValue(e){return e==="meta-cv"}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:n=!1,variant:a="default"}={}){const s=!!e.metaReportShowBottomline,i=resolveMetaReportFeeMode(e)||"",o=e.metaReportFeePercent??20,r=e.metaReportMarketingFeeAmount??0,l=metaReportUsesInlineFeeValue(t);if(a==="cv"){const c=META_REPORT_FEE_MODE_META.map(({value:y,title:v})=>`
        <label class="meta-report-fee-mode-pill">
          <input type="radio" name="${t}-fee-mode" value="${esc(y)}"${i===y?" checked":""} />
          <span>${esc(v)}</span>
        </label>`).join("");return`
      ...
      <div class="meta-cv-bottomline-panel">
        <div class="meta-cv-bottomline-head">
          <div class="meta-cv-bottomline-head-text">
            <span class="meta-cv-bottomline-title">Bottomline & Censio fee</span>
            <span class="meta-cv-bottomline-hint">Profit breakdown and fee on client report</span>
          </div>
          ${renderMetaReportSwitch(`${t}-setting-bottomline`,s,"Show bottomline on report",!1,{hideLabel:!0})}
        </div>
        <div class="meta-report-fee-nested meta-cv-bottomline-body${s?"":" is-hidden"}" id="${t}-fee-nested" data-fee-mode="${esc(i)}" data-fee-percent="${esc(o)}" data-marketing-fee="${esc(r)}">
          <div class="meta-report-fee-mode-pills">${c}</div>
  ...
  `}const d=META_REPORT_FEE_MODE_META.map(({value:c,icon:y,title:v,desc:$})=>`
