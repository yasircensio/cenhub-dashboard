...
Resume the interrupted ${S}?`)&&(l=y.id,c=Number(y.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const y=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),g=y.summary||y;l=g.runId,c=0,setFbLeadProgress({processed:0,total:g.inWindow||0,label:`Matching ${g.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:o})});const y=d.summary||d;l=y.runId,c=y.nextBatchOffset??y.batchOffset+(y.batchProcessed||0),u=u.concat(y.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const g=y.inWindow||c||$;a?(setFbLeadProgress({processed:c,total:g,complete:!y.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...y,rows:u}))):updateGlobalApplyProgress({processed:c,total:g,written:y.updated||0,complete:!y.hasMore})}while((d.summary||d).hasMore);p=!0;const w=d.summary||d,f=w.inWindow||c||$,v=w.nextBatchOffset??c;if(a)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,w.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(w.updated||0)>0),showToast((w.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:w.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const y=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete y[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(y))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const b=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=b.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(w){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${w.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((w.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function renderMetaReportComparisonTables(e){return metaReportsState.comparisonLoading?'<p class="meta-report-comparison-loading">Loading comparison data\u2026</p>':!e||e.insufficientData?'<p class="meta-report-comparison-empty">Select two periods with ad data to compare performance.</p>':e.samePeriod?'<p class="meta-report-comparison-warning">Both periods are the same. Choose different ranges to compare.</p>':`<div class="meta-report-groups-stack">${META_REPORT_COMPARISON_TABLE_GROUPS.filter(a=>!a.bottomlineOnly||e.hasBottomline).map(a=>renderMetaReportComparisonGroupTable(a.title,a.rows,a.accent,e,{highlightLastN:a.highlightLastN||0})).join("")}</div>`}function renderMetaReportComparisonCard(){const e=metaReportsState.clientPayload||metaReportsState.publicPayload,t=metaReportsState.comparisonResult||computeMetaReportComparison(e),{periodA:a,periodB:n}=metaReportComparisonPeriodsFromState(e),s=metaReportsState.comparisonTab||"table";return`
    <section class="meta-report-comparison-card" id="meta-report-comparison-card">
      ...
      <div id="meta-report-comparison-range-wrap">
        ${renderMetaReportComparisonRangePanel(a,n,e)}
      </div>
  ...
  `}function destroyMetaReportComparisonChart(){metaReportsState.comparisonChartInstance&&(metaReportsState.comparisonChartInstance.destroy(),metaReportsState.comparisonChartInstance=null)}function metaReportComparisonChartLabel(e,t){return t.find(a=>a.id===e)?.label||e}function buildMetaReportComparisonChart(e,t,a="kr"){const n=document.getElementById("meta-report-comparison-chart-empty");if(!t||t.insufficientData||t.samePeriod)return n&&(n.hidden=!1),null;n&&(n.hidden=!0);const s=metaReportComparisonMetricDefs(t.hasBottomline),r=metaReportComparisonChartIds(t.hasBottomline,a),o=r.map(d=>metaReportComparisonChartLabel(d,s)),i=r.map(d=>metaReportParseAmount(t.periodA.metrics[d])),l=t.periodB.hasData!==!1,c=r.map(d=>l?metaReportParseAmount(t.periodB.metrics[d]):null);return new Chart(e,{type:"bar",data:{labels:o,datasets:[{label:t.periodA.label,data:i,backgroundColor:"rgba(255, 106, 0, 0.82)",borderColor:"#ff6a00",borderWidth:1,borderRadius:6,maxBarThickness:42},{label:t.periodB.label,data:c,backgroundColor:"rgba(148, 163, 184, 0.82)",borderColor:"#94a3b8",borderWidth:1,borderRadius:6,maxBarThickness:42}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"top",labels:{boxWidth:12,font:{size:11}}},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,callbacks:{label(d){const u=d.parsed.y,p=r[d.dataIndex],h=s.find(m=>m.id===p);return`${d.dataset.label}: ${metaReportComparisonFmtValue(u,h?.format||"num")}`}}}},scales:{x:{grid:{display:!1},ticks:{color:"#6b5348",font:{size:11},maxRotation:45,minRotation:0}},y:{grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:d=>{const u=Number(d)||0;return Math.abs(u)>=1e6?`${(u/1e6).toFixed(1)}M`:Math.abs(u)>=1e3?`${Math.round(u/1e3)}K`:u}}}}}})}function mountMetaReportComparisonChart(e){if(destroyMetaReportComparisonChart(),(metaReportsState.comparisonTab||"table")!=="chart")return;const t=document.getElementById("meta-report-comparison-chart");t&&(metaReportsState.comparisonChartInstance=buildMetaReportComparisonChart(t,e,metaReportsState.comparisonChartMode))}function setMetaReportComparisonTab(e){if(metaReportsState.comparisonTab=e==="chart"?"chart":"table",document.querySelectorAll("[data-comparison-tab]").forEach(t=>{const a=t.getAttribute("data-comparison-tab")===metaReportsState.comparisonTab;t.classList.toggle("is-active",a),t.setAttribute("aria-selected",a)}),document.querySelectorAll("[data-comparison-panel]").forEach(t=>{const a=t.getAttribute("data-comparison-panel")===metaReportsState.comparisonTab;t.classList.toggle("is-active",a),t.hidden=!a}),metaReportsState.comparisonTab==="chart"){const t=metaReportsState.clientPayload||metaReportsState.publicPayload;mountMetaReportComparisonChart(metaReportsState.comparisonResult||computeMetaReportComparison(t))}else destroyMetaReportComparisonChart()}function syncMetaReportComparisonUi(e,{editable:t=!1}={}){if(!document.getElementById("meta-report-comparison-card"))return;const n=computeMetaReportComparison(e);metaReportsState.comparisonResult=n;const s=document.getElementById("meta-report-comparison-headline-wrap");s&&s.remove();const r=document.getElementById("meta-report-comparison-table-wrap");r&&(r.innerHTML=renderMetaReportComparisonTables(n));const o=document.getElementById("meta-report-comparison-chart-empty");o&&(o.hidden=!(n.insufficientData||n.samePeriod)),(metaReportsState.comparisonTab||"table")==="chart"&&mountMetaReportComparisonChart(n)}async function refreshMetaReportComparison(e,{editable:t=!1}={}){!e||!document.getElementById("meta-report-comparison-card")||(await ensureMetaReportComparisonYears(e,{editable:t}),syncMetaReportComparisonUi(e,{editable:t}))}function bindMetaReportComparisonControls(e,{editable:t=!1}={}){const a=()=>{document.querySelectorAll("[data-comparison-preset]").forEach(s=>{s.onclick=async()=>{if(s.disabled)return;const r=s.getAttribute("data-comparison-preset");if(!r)return;metaReportsState.comparisonMode=r,metaReportApplyComparisonPresetDefaults(e,r);const o=document.getElementById("meta-report-comparison-card");o&&(o.outerHTML=renderMetaReportComparisonCard(),bindMetaReportComparisonControls(e,{editable:t})),await refreshMetaReportComparison(e,{editable:t})}})},n=()=>{document.querySelectorAll("[data-comparison-date]").forEach(s=>{s.onchange=async()=>{const r=s.getAttribute("data-comparison-date");if(!r||!s.value)return;const[o,i]=r.split("-"),l=`comparisonPeriod${o.toUpperCase()}`;metaReportsState[l]||(metaReportsState[l]={startDate:null,endDate:null});const c=metaReportsState[l];if(c[`${i}Date`]=s.value,c.startDate&&c.endDate&&c.startDate>c.endDate){const u=c.startDate;c.startDate=c.endDate,c.endDate=u}metaReportsState.comparisonMode="custom",await refreshMetaReportComparison(e,{editable:t});const d=document.getElementById("meta-report-comparison-range-wrap");if(d){const{periodA:u,periodB:p}=metaReportComparisonPeriodsFromState(e);d.innerHTML=renderMetaReportComparisonRangePanel(u,p,e),n()}document.querySelectorAll("[data-comparison-preset]").forEach(u=>{u.classList.toggle("active",u.getAttribute("data-comparison-preset")==="custom"),u.setAttribute("aria-pressed",u.getAttribute("data-comparison-preset")==="custom")})}})};document.querySelectorAll("[data-comparison-tab]").forEach(s=>{s.onclick=()=>{const r=s.getAttribute("data-comparison-tab");r&&setMetaReportComparisonTab(r)}}),document.querySelectorAll("#meta-report-comparison-chart-mode .meta-report-chart-toggle-btn").forEach(s=>{s.onclick=()=>{metaReportsState.comparisonChartMode=s.getAttribute("data-mode")||"kr",document.querySelectorAll("#meta-report-comparison-chart-mode .meta-report-chart-toggle-btn").forEach(r=>{r.classList.toggle("is-active",r===s)}),mountMetaReportComparisonChart(metaReportsState.comparisonResult||computeMetaReportComparison(e))}}),a(),n(),setMetaReportComparisonTab(metaReportsState.comparisonTab||"table")}function mountMetaReportComparison(e,{editable:t=!1}={}){document.getElementById("meta-report-comparison-card")&&(bindMetaReportComparisonControls(e,{editable:t}),refreshMetaReportComparison(e,{editable:t}))}function renderMetaReportScenarioCard(){return`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null}={}){const s=n||e?.monthKey||null,r=s?metaReportMonthBounds(s):null;let o="";t&&e&&!e.meta?.emptyMonth&&r&&(o=`
    ...
    `);const i=n||e?.monthKey||null,l=t?i?`Report preview \xB7 ${metaMonthLabel(i)}`:"Report preview":i?`Report \xB7 ${metaMonthLabel(i)}`:"Report",c=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${l}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",d=renderMetaReportMonthTables(e,{editable:t}),u=shouldShowMetaReportYearVisuals(e,a,{editable:t}),p=u?renderMetaReportChartsPanel({editable:t}):"",h=u?renderMetaReportComparisonCard():"",m=t&&u?renderMetaReportScenarioCard():"",$=u?`
    <div class="meta-report-content-layout">
      <div class="meta-report-content-main">${d}</div>
      ${p}
    </div>
    ${h}
    ${m}
  `:`
    <div class="meta-report-content-main">${d}</div>
  `;return`${o}${c}${$}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${o?"":" is-hidden"}" id="${t}-fee-nested" ${h}>
          ...
          <div class="meta-report-fee-mode-pills">${y}</div>
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),$=META_REPORT_FEE_MODE_META.map(({value:b,icon:y,title:g,desc:S})=>`
        ...
        </label>`).join(""),w=s?metaReportsState.clientReportSettingsExpanded:!0,f=s?`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,v=s&&!r?renderMetaReportBottomlineFeeSummary(e,t):"";return`
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?w?" is-editing":" is-collapsed":""}">
      ${v}
      <div class="meta-report-bottomline-editor${s&&!w?" is-hidden":""}" id="${t}-fee-editor">
