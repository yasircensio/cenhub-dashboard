...
Resume the interrupted ${$}?`)&&(l=w.id,d=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&d===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),h=w.summary||w;l=h.runId,d=0,setFbLeadProgress({processed:0,total:h.inWindow||0,label:`Matching ${h.inWindow||0} Meta leads to GHL\u2026`})}else!a&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:S||p?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:b,batchOffset:d,batchLimit:r})});const w=c.summary||c;l=w.runId,d=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const h=w.inWindow||d||S;a?(setFbLeadProgress({processed:d,total:h,complete:!w.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:d,total:h,written:w.updated||0,complete:!w.hasMore})}while((c.summary||c).hasMore);m=!0;const g=c.summary||c,f=g.inWindow||d||S,v=g.nextBatchOffset??d;if(a)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,g.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(g.updated||0)>0),showToast((g.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:g.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const y=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=y.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(g){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${g.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((g.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!m&&a&&hideFbLeadProgress(!0),!a&&!m&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function renderMetaReportBaselineStrip(e=null){if(!e)return"";const t=[],a=metaReportsState.budgetBaseline;if(e.windowAverage>0&&t.push(renderMetaReportBaselinePill("Window avg",e.windowAverage,{active:a==="year",title:"Average ad spend in the history window (completed months)"})),e.lastMonthSpend>0&&e.lastMonthLabel&&t.push(renderMetaReportBaselinePill(`Last month \xB7 ${e.lastMonthLabel}`,e.lastMonthSpend,{active:a==="last",title:"Ad spend for the month before the selected month tab"})),e.selectedMonthSpend>0&&e.selectedMonthLabel){const n=e.activeKey&&e.currentKey&&e.activeKey===e.currentKey;t.push(renderMetaReportBaselinePill(n?`Selected \xB7 ${e.selectedMonthLabel} (current)`:`Selected \xB7 ${e.selectedMonthLabel}`,e.selectedMonthSpend,{active:a==="month",title:"Ad spend for the selected month tab"}))}return e.currentMonthSpend>0&&e.currentMonthLabel&&e.activeKey!==e.currentKey&&t.push(renderMetaReportBaselinePill(`Current \xB7 ${e.currentMonthLabel}`,e.currentMonthSpend,{title:"Ad spend for the current calendar month"})),t.join("")}function syncMetaReportBaselineUi(e=[],t=null,a=null){const n=resolveMetaReportScenarioBaselineHints(e,t,a),s=document.getElementById("meta-report-baseline-strip");if(s){const r=renderMetaReportBaselineStrip(n);s.innerHTML=r,s.hidden=!r}const o=document.getElementById("meta-report-budget-baseline");if(o){const r=o.value||metaReportsState.budgetBaseline;o.innerHTML=renderMetaReportBaselineSelectOptions(n,r),o.value=normalizeMetaReportBudgetBaseline(r)}}function renderMetaReportScenarioContextStrip(e){if(!e||e.insufficientData)return'<p class="meta-report-scenario-context" id="meta-report-scenario-context">Add at least two completed ad months to model a budget scenario.</p>';const t=describeActiveMetaReportScenarioPills(),a=t.length?t.join(" \xB7 "):"Recent months only",n=e.trendDirection==="up"?`trend up ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:e.trendDirection==="down"?`trend down ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:"trend flat",s=e.prepared?.outliersAdjusted?` \xB7 ${e.prepared.outliersAdjusted} uneven month${e.prepared.outliersAdjusted===1?"":"s"} removed`:"",r={low:"Low confidence",medium:"Medium confidence",high:"High confidence"}[e.prepared?.confidence]||"",i=r?` \xB7 ${r} (${e.prepared?.monthsUsed||0} mo data)`:"";return`<p class="meta-report-scenario-context" id="meta-report-scenario-context"><strong>Active:</strong> ${esc(a)} \xB7 last ${e.prepared?.monthsUsed||0} ad months \xB7 ${esc(n)}${esc(s)}${esc(i)}</p>`}function payloadHasMetaReportBottomline(e){return e?.settings?.metaReportShowBottomline===!1?!1:seriesHasBottomline(metaReportsState.chartSeries||buildMetaReportChartSeries(e))}function seriesHasBottomline(e=[]){return e.some(t=>t.poasKr!=null&&t.poasX!=null)}function renderMetaReportChartsPanel({editable:e=!1}={}){const t=metaReportsState.chartTab||"trend";return`
  ...
  `).join("");return`
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      ...
      <div class="meta-report-tabs-row">
        <div class="meta-report-tabs" role="tablist">${o}</div>
        <div class="meta-report-baseline-strip" id="meta-report-baseline-strip" hidden></div>
      </div>
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t,t&&(metaReportsState.clientShareExpanded=!1);const a=document.querySelector(".meta-report-bottomline-fee-settings--client"),n=document.getElementById(`${e}-fee-editor`);a?.classList.toggle("is-editing",t),a?.classList.toggle("is-collapsed",!t),n&&n.classList.toggle("is-hidden",!t);const s=document.getElementById("meta-report-share-summary"),o=document.getElementById("meta-report-share-editor");s&&(s.hidden=metaReportsState.clientShareExpanded),o&&(o.hidden=!metaReportsState.clientShareExpanded),syncMetaReportControlPanelUi()}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const a=document.getElementById(`${t}-fee-summary`);if(!a)return;const n=document.createElement("div");n.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const s=n.firstElementChild;s&&a.replaceWith(s);const o=document.getElementById(`${t}-fee-edit`);o&&(o.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:a=!1,variant:n="default",collapsible:s=!1,externalSummary:o=!1}={}){const r=!!e.metaReportShowBottomline,i=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",d=resolveMetaReportFeeMode(e),c=e.metaReportFeePercent??20,u=e.metaReportMarketingFeeAmount??0,m=metaReportUsesInlineFeeValue(t),b=`data-fee-mode="${esc(l)}" data-fee-enabled="${i?"true":"false"}" data-fee-percent="${esc(c)}" data-marketing-fee="${esc(u)}"`;if(n==="cv"){const y=describeMetaReportFeeModeLabel(d,c,u),w=META_REPORT_FEE_MODE_META.map(({value:h,title:$})=>`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${r?"":" is-hidden"}" id="${t}-fee-nested" ${b}>
  ...
  `}const p=describeMetaReportFeeModeLabel(d,c,u),S=META_REPORT_FEE_MODE_META.map(({value:y,icon:w,title:h,desc:$})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(y)}"${d===y?" checked":""} />
          <span class="meta-report-fee-mode-card-icon" aria-hidden="true">${esc(w)}</span>
          <span class="meta-report-fee-mode-card-text">
            <span class="meta-report-fee-mode-card-title">${esc(h)}</span>
            <small>${esc($)}</small>
          </span>
    ...
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?g?" is-editing":" is-collapsed":""}">
      ...
      <div class="meta-report-bottomline-editor${s&&!g?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${r?"":" is-hidden"}" id="${t}-fee-nested" ${b}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(S=>S.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label>Avg profit per won${r?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(b)}" />
      </label>
    </div>
    ...
    <div class="date-picker-panel">
      <div class="date-picker-header">
        ...
        <div class="date-picker-nav">
          ...
          <button type="button" class="date-picker-nav-btn" aria-label="Next month" onclick="shiftDatePickerMonth(1)" ${h?"":"disabled"}>${ICON_CHEVRON_RIGHT}</button>
        </div>
      </div>
