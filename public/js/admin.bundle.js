...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),v=$.summary||$;l=v.runId,c=0,setFbLeadProgress({processed:0,total:v.inWindow||0,label:`Matching ${v.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:y||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:r})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const v=$.inWindow||c||y;a?(setFbLeadProgress({processed:c,total:v,complete:!$.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:v,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||y,w=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:w,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:w,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const g=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=g.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function applyMetaReportComparisonDateChange(e,t,{editable:a=!1}={}){const n=t.getAttribute("data-comparison-date");if(!n||!t.value)return;const[s,o]=n.split("-"),r=`comparisonPeriod${s.toUpperCase()}`;metaReportsState[r]||(metaReportsState[r]={startDate:null,endDate:null});const i=metaReportsState[r];if(i[`${o}Date`]=t.value,i.startDate&&i.endDate&&i.startDate>i.endDate){const l=i.startDate;i.startDate=i.endDate,i.endDate=l;const c=document.querySelector(`[data-comparison-date="${s}-start"]`),d=document.querySelector(`[data-comparison-date="${s}-end"]`);c?._metaComparisonDatePicker&&c._metaComparisonDatePicker.setDate(i.startDate,!1),d?._metaComparisonDatePicker&&d._metaComparisonDatePicker.setDate(i.endDate,!1)}metaReportsState.comparisonMode="custom",syncMetaReportComparisonPeriodPickerBounds(e),document.querySelectorAll("[data-comparison-preset]").forEach(l=>{const c=l.getAttribute("data-comparison-preset")==="custom";l.classList.toggle("active",c),l.setAttribute("aria-pressed",c)}),refreshMetaReportComparison(e,{editable:a})}function syncMetaReportComparisonPeriodPickerBounds(e){const t=metaReportComparisonDateInputBounds(e);["a","b"].forEach(a=>{const n=`comparisonPeriod${a.toUpperCase()}`,s=metaReportsState[n]||{},o=document.querySelector(`[data-comparison-date="${a}-start"]`),r=document.querySelector(`[data-comparison-date="${a}-end"]`);o?._metaComparisonDatePicker&&(o._metaComparisonDatePicker.set("minDate",t.min||void 0),o._metaComparisonDatePicker.set("maxDate",s.endDate||t.max||void 0)),r?._metaComparisonDatePicker&&(r._metaComparisonDatePicker.set("minDate",s.startDate||t.min||void 0),r._metaComparisonDatePicker.set("maxDate",t.max||void 0))})}function closeMetaReportComparisonDatePickers(e=null){(metaReportsState.comparisonDatePickers||[]).forEach(t=>{t!==e&&t.isOpen&&t.close()})}function destroyMetaReportComparisonDatePickers(){closeMetaReportComparisonDatePickers(),document.querySelectorAll("[data-comparison-date]").forEach(e=>{e._metaComparisonDatePicker&&(e._metaComparisonDatePicker.destroy(),e._metaComparisonDatePicker=null)}),metaReportsState.comparisonDatePickers=[]}function bindMetaReportComparisonDateInputs(e,{editable:t=!1}={}){if(destroyMetaReportComparisonDatePickers(),typeof flatpickr>"u")return;const a=metaReportComparisonDateInputBounds(e),{periodA:n,periodB:s}=metaReportComparisonPeriodsFromState(e);document.querySelectorAll("[data-comparison-date]").forEach(o=>{const r=o.getAttribute("data-comparison-date");if(!r)return;const[i,l]=r.split("-"),c=i==="b"?s:n,d=c?.[`${l}Date`]||o.value||"";let u=a.min||o.getAttribute("data-min")||void 0,p=a.max||o.getAttribute("data-max")||void 0;l==="start"&&c?.endDate&&(p=c.endDate),l==="end"&&c?.startDate&&(u=c.startDate);const h=flatpickr(o,{dateFormat:"Y-m-d",altInput:!0,altFormat:"j M Y",defaultDate:d||void 0,minDate:u,maxDate:p,disableMobile:!0,allowInput:!1,clickOpens:!0,animate:!0,static:!1,appendTo:document.body,className:"meta-report-flatpickr",onOpen:(y,b,f)=>{closeMetaReportComparisonDatePickers(f)},onChange:(y,b,f)=>{!b||f._metaComparisonApplying||applyMetaReportComparisonDateChange(e,f.input,{editable:t})}});o._metaComparisonDatePicker=h,metaReportsState.comparisonDatePickers.push(h);const m=o.closest(".meta-report-comparison-date-input-wrap");m&&(m.onclick=y=>{const b=h.altInput;y.target===o||y.target===b||h.open()})})}function renderMetaReportComparisonPeriodTiles(e,t,a){const n=metaReportsState.comparisonMode||"mom";if(n==="custom"){const r=metaReportComparisonDateInputBounds(a),i=e?.startDate||r.min||"",l=e?.endDate||i,c=t?.startDate||r.min||"",d=t?.endDate||c;return`
  ...
  `:"",p=renderMetaReportMonthTables(e,{editable:t}),h=d?renderMetaReportChartsPanel({editable:t}):"",m=t&&d?renderMetaReportScenarioCard():"",y=d?renderMetaReportViewModeTabs():"";let b;return d?b=`
      ...
      ${m}
    `:b=`<div class="meta-report-content-main">${p}</div>`,`${r}${u}${y}${b}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),y=META_REPORT_FEE_MODE_META.map(({value:g,icon:$,title:v,desc:S})=>`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,w=s&&!o?renderMetaReportBottomlineFeeSummary(e,t):"";return`
