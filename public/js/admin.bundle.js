(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){if(t[1]==="sync-history"&&(t[2]==="ghl"||t[2]==="meta")){document.body.dataset.dashboardMode=`sync-history-${t[2]}`,delete document.body.dataset.clientSlug;return}if(t[1]==="fb-lead-sync"){document.body.dataset.dashboardMode="fb-lead-sync",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="custom-values"){document.body.dataset.dashboardMode="meta-reports-custom",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]){document.body.dataset.dashboardMode="meta-reports-client",document.body.dataset.clientSlug=t[2];return}if(t[1]==="meta-reports"){document.body.dataset.dashboardMode="meta-reports",delete document.body.dataset.clientSlug;return}document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}if(t[0]==="report"&&t[1]){document.body.dataset.dashboardMode="report",document.body.dataset.reportToken=t[1],delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_ADMIN_SYNC_HISTORY_GHL=DASHBOARD_MODE==="sync-history-ghl",IS_ADMIN_SYNC_HISTORY_META=DASHBOARD_MODE==="sync-history-meta",IS_ADMIN_FB_LEAD_SYNC=DASHBOARD_MODE==="fb-lead-sync",IS_ADMIN_META_REPORTS=DASHBOARD_MODE==="meta-reports",IS_ADMIN_META_REPORTS_CLIENT=DASHBOARD_MODE==="meta-reports-client",IS_ADMIN_META_REPORTS_CUSTOM=DASHBOARD_MODE==="meta-reports-custom",IS_REPORT_VIEW=DASHBOARD_MODE==="report",REPORT_TOKEN=document.body.dataset.reportToken||"",IS_ADMIN_SYNC_HISTORY=IS_ADMIN_SYNC_HISTORY_GHL||IS_ADMIN_SYNC_HISTORY_META,IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);return e[0]==="admin"&&e[1]==="meta-reports"&&e[2]&&e[2]!=="custom-values"?e[2]:e[0]==="admin"&&e[1]&&e[1]!=="meta-reports"?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"&&e[0]!=="report"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){const t=!!(e&&e.includes("staff-admin-chrome")),n=t||e&&e.includes("admin-topbar-link")?"/admin":"",a=n?`<a href="${n}" class="brand-logo-link" aria-label="Cenhub home"><img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" /></a>`:'<img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" />',s=t?`<button type="button" class="staff-nav-toggle" id="staff-nav-toggle" aria-expanded="false" aria-controls="staff-topbar-panel" aria-label="Open menu">
  ...
  `}function renderSyncHistoryRows(e,t){if(!e.length)return'<div class="sync-history-empty">No sync runs logged yet.</div>';const n=t==="meta"?"Spend / details":"Details",a=e.map(s=>{let i="\u2014";if(t==="meta"){const o=[];if(s.thisMonthSpend!=null&&o.push(`${fmtDkk(s.thisMonthSpend)} DKK this month`),s.spendDateStop){const r=formatSyncHistoryDate(s.spendDateStop);r&&o.push(`through ${r}`)}s.errorMessage&&o.push(s.errorMessage),i=o.join(" \xB7 ")||"\u2014"}else s.opportunityCount!=null?(i=`${s.opportunityCount} opportunities`,s.errorMessage&&(i+=` \xB7 ${s.errorMessage}`)):s.errorMessage&&(i=s.errorMessage);return`
      <tr>
  ...
  `:'<div class="sync-history-empty">No clients configured yet.</div>'}function renderFbLeadClientRowsWrapper(e){return`<div id="fb-lead-clients-mount">${renderFbLeadClientRows(e)}</div>`}function renderFbLeadHistoryRows(e,t={}){const n=getFbLeadHistoryRunsForDisplay(e);if(!n.length){const s=fbLeadSyncState.showRoutineCronRuns?0:(e||[]).filter(i=>isRoutineSuccessfulCronRun(i)).length;return s>0?`<div class="sync-history-empty">${s} routine auto-sync run(s) hidden \u2014 enable <strong>Show routine auto-sync runs</strong> above to view them.</div>`:'<div class="sync-history-empty">No FB lead sync runs logged yet.</div>'}return e=n,`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table">
        ...
        <tbody>${e.map(s=>{const i=[s.mode,s.dryRun?"dry-run":"apply",s.dryRun?`${s.updated||0} would update`:`${s.updated||0} updated`,`${s.skippedNoMatch||0} no match`,s.errors?`${s.errors} errors`:null].filter(Boolean).join(" \xB7 "),o=dryRunIsReadyForApply(s,e)&&!fbLeadSyncState.isApplying,r=isPartialSyncRun(s)&&!fbLeadSyncState.isApplying,l=fbLeadSyncState.isApplying&&fbLeadSyncState.applyingRunId===s.id,d=renderSyncStatusBadge(l?"applying":s.status);return`
...
Resume the interrupted ${g}?`)&&(l=m.id,d=Number(m.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const m=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),u=m.summary||m;l=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||b?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:$,batchOffset:d,batchLimit:o})});const m=c.summary||c;l=m.runId,d=m.nextBatchOffset??m.batchOffset+(m.batchProcessed||0),h=h.concat(m.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const u=m.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:u,complete:!m.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...m,rows:h}))):updateGlobalApplyProgress({processed:d,total:u,written:m.updated||0,complete:!m.hasMore})}while((c.summary||c).hasMore);y=!0;const S=c.summary||c,v=S.inWindow||d||w,p=S.nextBatchOffset??d;if(n)setFbLeadProgress({processed:p,total:v,complete:!0}),markFbLeadPreviewReady(e,t,S.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(S.updated||0)>0),showToast((S.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:p,total:v,written:S.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const m=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete m[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(m))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const f=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=f.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(S){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${S.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((S.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!y&&n&&hideFbLeadProgress(!0),!n&&!y&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function metaReportFullUrl(e){return e?`${window.location.origin}${e}`:""}function normalizeMetaReportSlugInput(e){return String(e||"").trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function resolveMetaReportShareSlug(e){const t=e.settings||{};if(t.metaReportSlug)return t.metaReportSlug;const a=String(e.reportUrl||"").replace(/^\/report\//,"").match(/^(.+)-(\d{4})$/);return a?a[1]:""}function resolveMetaReportShareCode(e){const n=String(e.reportUrl||"").replace(/^\/report\//,"").match(/-(\d{4})$/);return n?n[1]:""}function renderMetaReportShareSummary(e,{hidden:t=!1}={}){const a=!!(e.settings||{}).metaReportEnabled,s=resolveMetaReportShareSlug(e),i=resolveMetaReportShareCode(e),o=s?`${s}-${i||"\xB7\xB7\xB7\xB7"}`:"Not set";return`
    <div class="meta-report-share-summary" id="meta-report-share-summary"${t?" hidden":""}>
      <div class="meta-report-share-summary-main">
        ...
        <div class="meta-report-share-summary-row">
          <span class="meta-report-share-summary-label">Link slug</span>
  ...
  `}function syncMetaReportToolbarCopy(e){const t=document.getElementById("meta-report-toolbar-copy-link");t&&(e.reportUrl?(t.setAttribute("data-copy-report-url",e.reportUrl),t.disabled=!e.settings?.metaReportEnabled,t.hidden=!1):t.hidden=!0)}function updateMetaReportShareSummary(e){const t=document.getElementById("meta-report-share-summary");if(!t)return;const n=document.createElement("div");n.innerHTML=renderMetaReportShareSummary(e).trim();const a=n.firstElementChild;if(!a)return;a.hidden=metaReportsState.clientShareExpanded,t.replaceWith(a);const s=document.getElementById("meta-report-share-edit");s&&(s.onclick=()=>setMetaReportShareExpanded(!0))}function setMetaReportShareExpanded(e){metaReportsState.clientShareExpanded=!!e;const t=document.getElementById("meta-report-share-card"),n=document.getElementById("meta-report-share-summary"),a=document.getElementById("meta-report-share-editor"),s=document.getElementById("meta-report-share-done");t?.classList.toggle("is-editing",e),n&&(n.hidden=e),a&&(a.hidden=!e),s&&(s.hidden=!e)}function renderMetaReportShareCard(e){const n=!!(e.settings||{}).metaReportEnabled,a=e.reportUrl||null,s=metaReportFullUrl(a),i=resolveMetaReportShareSlug(e),o=resolveMetaReportShareCode(e),r=metaReportsState.clientShareExpanded,l=!!a;return`
    <div class="meta-report-share-card${n?"":" is-disabled"}${r?" is-editing":""}" id="meta-report-share-card">
      <div class="meta-report-share-card-head">
        <span class="meta-report-settings-card-title">Slug setting</span>
        <button type="button" class="admin-btn admin-btn--ghost admin-btn--small" id="meta-report-share-done"${r?"":" hidden"}>Done</button>
      </div>
      ${l?renderMetaReportShareSummary(e,{hidden:r}):`
        <p class="meta-report-share-empty">Share link will appear once this client has a Meta ad account configured.</p>
      `}
      ${l?`
      <div class="meta-report-share-editor" id="meta-report-share-editor"${r?"":" hidden"}>
        <div class="meta-report-share-panel">
          <div class="meta-report-share-preview-row">
            <div class="meta-report-share-preview-copy">
              <span class="meta-report-share-preview-title">Client preview</span>
              <span class="meta-report-share-preview-hint">Turn the public share link on or off</span>
            </div>
  ...
  `}function renderMetaReportMetricTable(e,t,n="meta",{badge:a="",highlightLastN:s=0}={}){const i=t.map(([o,r,l],d)=>`
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t;const n=document.querySelector(".meta-report-bottomline-fee-settings--client"),a=document.querySelector(".meta-report-client-page .meta-report-settings"),s=document.getElementById(`${e}-fee-editor`);n?.classList.toggle("is-editing",t),n?.classList.toggle("is-collapsed",!t),a?.classList.toggle("is-collapsed",!t),s?.classList.toggle("is-hidden",!t)}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const n=document.getElementById(`${t}-fee-summary`);if(!n)return;const a=document.createElement("div");a.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const s=a.firstElementChild;s&&n.replaceWith(s);const i=document.getElementById(`${t}-fee-edit`);i&&(i.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:n=!1,variant:a="default",collapsible:s=!1}={}){const i=!!e.metaReportShowBottomline,o=!!e.metaReportFeeEnabled,r=e.metaReportFeeMode||"",l=resolveMetaReportFeeMode(e),d=e.metaReportFeePercent??20,c=e.metaReportMarketingFeeAmount??0,h=metaReportUsesInlineFeeValue(t),y=`data-fee-mode="${esc(r)}" data-fee-enabled="${o?"true":"false"}" data-fee-percent="${esc(d)}" data-marketing-fee="${esc(c)}"`;if(a==="cv"){const p=describeMetaReportFeeModeLabel(l,d,c),f=META_REPORT_FEE_MODE_META.map(({value:m,title:u})=>`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${i?"":" is-hidden"}" id="${t}-fee-nested" ${y}>
          ...
          <div class="meta-cv-fee-footer">
            <div class="meta-cv-fee-value-inline" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,l,d,c,{inline:!0})}</div>
            <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save" disabled>Saved</button>
          </div>
        </div>
      </div>
    </div>
  `}const $=describeMetaReportFeeModeLabel(l,d,c),b=META_REPORT_FEE_MODE_META.map(({value:p,icon:f,title:m,desc:u})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(p)}"${l===p?" checked":""} />
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,v=s?renderMetaReportBottomlineFeeSummary(e,t):"";return`
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?w?" is-editing":" is-collapsed":""}">
      ...
      <div class="meta-report-bottomline-editor${s&&!w?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${i?"":" is-hidden"}" id="${t}-fee-nested" ${y}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
          ...
          <div class="meta-report-fee-value" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,l,d,c,{inline:h})}</div>
          ${S}
        </div>
      </div>
    </div>
  `}function syncMetaReportBottomlineFeeDom(e={},t="meta-report"){const n=!!e.metaReportShowBottomline,a=!!e.metaReportFeeEnabled,s=e.metaReportFeeMode||"",i=resolveMetaReportFeeMode(e),o=e.metaReportFeePercent??20,r=e.metaReportMarketingFeeAmount??0,l=document.getElementById(`${t}-setting-bottomline`);l&&(l.checked=n);const d=document.getElementById(`${t}-fee-nested`);d?.classList.toggle("is-hidden",!n),d&&(d.dataset.feeMode=s,d.dataset.feeEnabled=a?"true":"false",d.dataset.feePercent=String(o),d.dataset.marketingFee=String(r)),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(y=>{y.checked=y.value===i});const c=metaReportUsesInlineFeeValue(t),h=document.getElementById(`${t}-fee-value`);h&&(h.innerHTML=metaReportFeeValueBlockHtml(t,i,o,r,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),syncMetaReportFeeSelectionState(t),t==="meta-report"&&updateMetaReportBottomlineFeeSummary(e,t)}function bindMetaReportFeeValueInputEnterKey(e){const t=document.getElementById(`${e}-setting-fee-percent`)||document.getElementById(`${e}-setting-marketing-fee`);t&&(t.onkeydown=n=>{n.key==="Enter"&&(n.preventDefault(),document.getElementById(`${e}-fee-save`)?.click())})}function bindMetaReportBottomlineFeeEvents(e,t,n){const a=n||((l,d)=>saveMetaReportClientSettings(e,l,d)),s=document.getElementById(`${t}-fee-edit`);s&&(s.onclick=()=>setMetaReportClientSettingsExpanded(t,!0));const i=document.getElementById(`${t}-fee-done`);i&&(i.onclick=()=>{const l=metaReportsState.clientPayload?.settings||{};syncMetaReportBottomlineFeeDom(l,t),setMetaReportClientSettingsExpanded(t,!1),updateMetaReportBottomlineFeeSummary(l,t)});const o=document.getElementById(`${t}-setting-bottomline`);o&&(o.onchange=()=>{const l=o.checked;a({metaReportShowBottomline:l},()=>{o.checked=!l})}),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(l=>{l.onchange=()=>{if(!l.checked)return;const d=document.getElementById(`${t}-fee-nested`),c=metaReportUsesInlineFeeValue(t),h=document.getElementById(`${t}-fee-value`);h&&d&&(h.innerHTML=metaReportFeeValueBlockHtml(t,l.value||"",d.dataset.feePercent??20,d.dataset.marketingFee??0,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),bindMetaReportFeeAmountInputs(t),syncMetaReportFeeSelectionState(t)}}),bindMetaReportFeeValueInputEnterKey(t),bindMetaReportFeeDraftTracking(t);const r=document.getElementById(`${t}-fee-save`);r&&(r.onclick=async()=>{if(r.disabled)return;const l=document.getElementById(`${t}-fee-nested`),d=l?.querySelector(`input[name="${t}-fee-mode"]:checked`),c=d&&d.value||null,h={metaReportFeeMode:c};if(c==="performance"){const $=document.getElementById(`${t}-setting-fee-percent`);let b=Number($?.value);Number.isFinite(b)||(b=20),b=Math.min(100,Math.max(0,b)),$&&($.value=String(b)),h.metaReportFeePercent=b}else if(c==="marketing"){const $=document.getElementById(`${t}-setting-marketing-fee`);let b=Number($?.value);Number.isFinite(b)||(b=0),b=Math.max(0,b),$&&($.value=String(b)),h.metaReportMarketingFeeAmount=b}const y=r.textContent;r.disabled=!0,r.textContent="Saving\u2026",r.classList.remove("is-dirty");try{await a(h,()=>syncMetaReportBottomlineFeeDom({metaReportShowBottomline:o?o.checked:!0,metaReportFeeMode:l?.dataset.feeMode||null,metaReportFeePercent:Number(l?.dataset.feePercent)||20,metaReportMarketingFeeAmount:Number(l?.dataset.marketingFee)||0},t)),syncMetaReportFeeSelectionState(t)}finally{r.disabled=!1,r.textContent=y,syncMetaReportFeeSelectionState(t)}})}function renderMetaReportSaveIndicator(e,t=!1){return`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find(w=>w.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
    ...
    `;const s=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,s);const i=t.editorPayload?.months?.[a]||null,o=n.metaReportShowBottomline,r=s.wonLeads!=null?s.wonLeads:i?.inputs?.wonLeads??i?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:i?.inputs?.avgLeadValue??i?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:i?.inputs?.avgProfitPerWon??"",c=metaReportsState.customValues.draftInputs,h=c?.wonLeads??r,y=c?.avgLeadValue??l,$=c?.avgProfitPerWon??d,b=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label>Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(h)}" />
      </label>
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `:'<div class="pipeline-warning">Select at least one pipeline, then click Apply data filters.</div>'}function defaultDisplayPrefs(){const e={kpis:{},sections:{},charts:getDefaultChartPrefs(),statusItems:{},columns:{}};return Object.keys(DISPLAY_OPTIONS.kpis).forEach(t=>{e.kpis[t]=!0}),Object.keys(DISPLAY_OPTIONS.sections).forEach(t=>{e.sections[t]=!0}),Object.keys(DISPLAY_OPTIONS.charts).forEach(t=>{e.charts[t]=!0}),Object.keys(DISPLAY_OPTIONS.statusItems).forEach(t=>{e.statusItems[t]=!0}),Object.entries(DISPLAY_OPTIONS.columns).forEach(([t,n])=>{e.columns[t]={},Object.keys(n).forEach(a=>{e.columns[t][a]=!0})}),e}function loadDisplayPrefs(){if(IS_CLIENT_VIEW&&!IS_PREVIEW)return defaultDisplayPrefs();try{const e=JSON.parse(localStorage.getItem(STORAGE_KEY)||localStorage.getItem(LEGACY_STORAGE_KEY)||"null");if(!e)return defaultDisplayPrefs();const t=defaultDisplayPrefs();return{kpis:{...t.kpis,...e.kpis},sections:{...t.sections,...e.sections},charts:{...t.charts,...e.charts},statusItems:{...t.statusItems,...e.statusItems},columns:{sourceReport:{...t.columns.sourceReport,...e.columns?.sourceReport||{}},assigneeReport:{...t.columns.assigneeReport,...e.columns?.assigneeReport||{}},pipelineBreakdown:{...t.columns.pipelineBreakdown,...e.columns?.pipelineBreakdown||{}}}}}catch{return defaultDisplayPrefs()}}function ensureChartsVisible(){const e=Object.keys(DISPLAY_OPTIONS.charts);e.some(n=>display.charts[n]!==!1)||(e.forEach(n=>{display.charts[n]=!0}),saveDisplayPrefs())}function saveDisplayPrefs(){localStorage.setItem(STORAGE_KEY,JSON.stringify(display))}function isVisible(e,t,n){return e==="columns"?display.columns[n]?.[t]!==!1:display[e]?.[t]!==!1}function toggleDisplay(e,t,n){e==="columns"?display.columns[n][t]=!display.columns[n][t]:display[e][t]=!display[e][t],saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}function setAllDisplay(e,t,n){e==="columns"?Object.keys(DISPLAY_OPTIONS.columns[n]).forEach(a=>{display.columns[n][a]=t}):Object.keys(DISPLAY_OPTIONS[e]).forEach(a=>{display[e][a]=t}),saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}const fmt=e=>new Intl.NumberFormat("da-DK",{minimumFractionDigits:0,maximumFractionDigits:0}).format(Math.round(Number(e)||0)),fmtCompact=e=>{const t=Number(e)||0;return t>=1e6?`${(t/1e6).toFixed(2)}M`:t>=1e3?`${(t/1e3).toFixed(2)}K`:fmt(t)},fmtPct=e=>`${(Number(e)||0).toFixed(2)}%`,fmtRoas=e=>{const t=Number(e)||0;return t>0?`${t.toFixed(2)}x`:"\u2014"};function formatActiveDateFilter(e){if(!e.dateFrom&&!e.dateTo)return"Till date";const t=getDashboardTimeZone();if(window.MarketingMetrics?.formatShortDateLabel){const n=window.MarketingMetrics.formatShortDateLabel(e.dateFrom,t),a=window.MarketingMetrics.formatShortDateLabel(e.dateTo,t);if(n&&a)return`${n} \u2013 ${a}`}return`${e.dateFrom||"start"} to ${e.dateTo||"now"}`}function showDateRangeError(e){document.querySelectorAll("#date-range-error").forEach(t=>{t.textContent=e,t.hidden=!e})}function clearDateRangeError(){showDateRangeError("")}function needsFreshData(){return!lastFetchedAt||Date.now()-lastFetchedAt>DATA_FRESH_MS}function buildQuery(e,t={}){const n=new URLSearchParams,a=getAllPipelineIds(e||[]),s=state.pipelineIds.filter(i=>a.includes(i));if(!s.length)throw new Error("Select at least one pipeline.");return n.set("pipelineIds",s.join(",")),state.dateField&&n.set("dateField",state.dateField),state.dateFrom&&n.set("dateFrom",state.dateFrom),state.dateTo&&n.set("dateTo",state.dateTo),["status","source","assignedTo","adSpend"].forEach(i=>{state[i]&&state[i]!=="all"&&n.set(i,state[i])}),appendTenantParams(n),t.forceFresh&&n.set("fresh","1"),n.toString()}function formatDateInput(e){const t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}function getDashboardTimeZone(){return cachedData?.account?.timezone||"Europe/Copenhagen"}function getCalendarPartsInTimeZone(e,t=new Date){const n=new Intl.DateTimeFormat("en-CA",{timeZone:e,year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(t);return{year:Number(n.find(a=>a.type==="year")?.value),month:Number(n.find(a=>a.type==="month")?.value),day:Number(n.find(a=>a.type==="day")?.value)}}function formatMonthDateRange(e,t){const n=`${e}-${String(t).padStart(2,"0")}-01`,a=new Date(e,t,0).getDate(),s=`${e}-${String(t).padStart(2,"0")}-${String(a).padStart(2,"0")}`;return{start:n,end:s}}const datePickerState={inputId:null,anchorEl:null,viewYear:null,viewMonth:null};let datePickerListenersBound=!1;function isoFromParts(e,t,n){const a=Number(n);return!e||!t||!Number.isFinite(a)?"":`${e}-${String(t).padStart(2,"0")}-${String(a).padStart(2,"0")}`}function formatPickerDisplayLabel(e,t){if(!e||!/^\d{4}-\d{2}-\d{2}$/.test(e))return t;const n=getDashboardTimeZone();return window.MarketingMetrics?.formatShortDateLabel&&window.MarketingMetrics.formatShortDateLabel(e,n)||e}function ensureDatePickerPopover(){if(document.getElementById("date-picker-popover"))return;const e=document.createElement("div");e.id="date-picker-backdrop",e.className="date-picker-backdrop",e.hidden=!0,e.addEventListener("click",closeDatePicker),document.body.appendChild(e);const t=document.createElement("div");t.id="date-picker-popover",t.className="date-picker-popover",t.hidden=!0,t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),document.body.appendChild(t)}function syncDatePickerDisplays(){[{id:"dateFrom",key:"dateFrom",placeholder:"Pick start date"},{id:"dateTo",key:"dateTo",placeholder:"Pick end date"}].forEach(({id:e,key:t,placeholder:n})=>{const a=document.getElementById(e),s=document.getElementById(`${e}-display`),i=document.getElementById(`${e}-trigger`),o=state[t]??"";a&&(a.value=o),s&&(s.textContent=formatPickerDisplayLabel(o,n)),i?.classList.toggle("is-empty",!o),i?.classList.toggle("has-value",!!o)})}function getStateDateValue(e){return state[e]??""}function getDatePickerViewMonth(e){const t=getStateDateValue(e);if(/^\d{4}-\d{2}-\d{2}$/.test(t)){const[n,a]=t.split("-").map(Number);return{year:n,month:a}}return getCalendarPartsInTimeZone(getDashboardTimeZone())}function getTodayIso(){const e=getCalendarPartsInTimeZone(getDashboardTimeZone());return isoFromParts(e.year,e.month,e.day)}function isStartDateDisabled(e){return e>=getTodayIso()}function isFutureDateDisabled(e){return e>getTodayIso()}function isFutureMonthView(e,t){const n=getCalendarPartsInTimeZone(getDashboardTimeZone());return e>n.year||e===n.year&&t>n.month}function isDatePickerDayDisabled(e,t){return t==="dateFrom"&&isStartDateDisabled(e)?!0:isFutureDateDisabled(e)}function renderDatePickerDayCell(e,t,n,a,s,i,o,r){const l=["date-picker-day"];if(n&&l.push("is-outside"),e===a&&l.push("is-today"),e===s&&l.push("is-selected"),i&&o){const d=i<=o?i:o,c=i<=o?o:i;e>=d&&e<=c&&l.push("is-in-range")}return e===i&&l.push("is-range-start"),e===o&&l.push("is-range-end"),isDatePickerDayDisabled(e,r)?(l.push("is-disabled"),`<button type="button" class="${l.join(" ")}" disabled aria-disabled="true">${t}</button>`):`<button type="button" class="${l.join(" ")}" onclick="selectDatePickerDay('${e}')">${t}</button>`}function renderDatePickerPopover(){const e=document.getElementById("date-picker-popover");if(!e||!datePickerState.inputId)return;const{viewYear:t,viewMonth:n,inputId:a}=datePickerState,s=new Intl.DateTimeFormat("en-GB",{month:"long",year:"numeric"}).format(new Date(t,n-1,1)),i=["Mo","Tu","We","Th","Fr","Sa","Su"],r=(new Date(t,n-1,1).getDay()+6)%7,l=new Date(t,n,0).getDate(),d=new Date(t,n-1,0).getDate(),c=getCalendarPartsInTimeZone(getDashboardTimeZone()),h=isoFromParts(c.year,c.month,c.day),y=getStateDateValue(a),$=getStateDateValue("dateFrom"),b=getStateDateValue("dateTo");let w="";for(let g=r-1;g>=0;g-=1){const k=d-g,I=n===1?12:n-1,L=n===1?t-1:t,R=isoFromParts(L,I,k);w+=renderDatePickerDayCell(R,k,!0,h,y,$,b,a)}for(let g=1;g<=l;g+=1){const k=isoFromParts(t,n,g);w+=renderDatePickerDayCell(k,g,!1,h,y,$,b,a)}const S=(7-(r+l)%7)%7;for(let g=1;g<=S;g+=1){const k=n===12?1:n+1,I=n===12?t+1:t,L=isoFromParts(I,k,g);w+=renderDatePickerDayCell(L,g,!0,h,y,$,b,a)}const v=a==="dateFrom"?'<span class="date-picker-hint">Start date must be before today</span>':"",p=a==="dateTo"?'<button type="button" class="date-picker-footer-btn primary" onclick="setDatePickerToday()">Today</button>':"",f=n===12?1:n+1,m=n===12?t+1:t,u=!isFutureMonthView(m,f);e.innerHTML=`
    <div class="date-picker-panel">
      ...
      <div class="date-picker-weekdays">
        ${i.map(g=>`<div class="date-picker-weekday">${g}</div>`).join("")}
      </div>
      ...
      <table class="dashboard-table">
        ...
        <tbody>
          ${a.length?a.map(o=>`
            <tr>
              <td>${s(o,"label")}</td>
              ${i.map(r=>`<td class="${r.align||""}">${s(o,r.key)}</td>`).join("")}
            </tr>
  ...
  `}function renderDashboardContent(e){const{kpis:t,statusBreakdown:n,sourceReport:a,assigneeReport:s,pipelines:i,filters:o}=e,r=isVisible("sections","statusBreakdown");return`
      ...
      <div class="section-grid">
        ...
        ${isVisible("sections","pipelineBreakdown")?`
          <div class="card">
            ...
            ${renderTable("pipelineBreakdown","Pipeline",[{key:"count",label:"Leads",align:"num"},{key:"won",label:"Won",align:"num"},{key:"monetary",label:"Value",align:"num"},{key:"profit",label:"Bundlinje",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"}],i,(l,d)=>d==="label"?esc(l.name):d==="monetary"||d==="profit"||d==="wonValue"?`Dkr ${fmt(l[d])}`:fmt(l[d]))}
          </div>
        `:""}
      </div>
    `:""}
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),r=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),r&&(r.disabled=!0,r.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const y=canReuseBootstrapDashboard(c)&&!s;let $,b;if(y?[b,$]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[$,b]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=b,cachedData=applyMarketingData($,b),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(l!==fetchGeneration)return;if(c.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            `)}`;return}if(IS_ADMIN_CLIENT&&o){const h=document.getElementById("dashboard-content");h&&(h.innerHTML=`
            <div class="note admin-setup-placeholder">
              ${esc(c.message)}
              <div style="margin-top:12px">
                <button type="button" class="refresh-btn primary" onclick="loadDashboard(true)">${RETRY_MSG}</button>
              </div>
            </div>`)}else o?o&&cachedData?(restoreDashboardContentAfterFailedFetch(o),showToast(c.message||"Failed to load filtered data.","error")):r&&(r.textContent="Apply failed - try again"):i.innerHTML=`
          ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
          ${wrapDashboardShell(`
            <div class="error-state">
              <div>Fejl ved hentning af data</div>
