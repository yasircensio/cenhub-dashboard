...
Resume the interrupted ${g}?`)&&(l=u.id,d=Number(u.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const u=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),m=u.summary||u;l=m.runId,d=0,setFbLeadProgress({processed:0,total:m.inWindow||0,label:`Matching ${m.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||y?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:S,batchOffset:d,batchLimit:o})});const u=c.summary||c;l=u.runId,d=u.nextBatchOffset??u.batchOffset+(u.batchProcessed||0),v=v.concat(u.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const m=u.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:m,complete:!u.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...u,rows:v}))):updateGlobalApplyProgress({processed:d,total:m,written:u.updated||0,complete:!u.hasMore})}while((c.summary||c).hasMore);b=!0;const $=c.summary||c,h=$.inWindow||d||w,f=$.nextBatchOffset??d;if(n)setFbLeadProgress({processed:f,total:h,complete:!0}),markFbLeadPreviewReady(e,t,$.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=($.updated||0)>0),showToast(($.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:f,total:h,written:$.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const u=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete u[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(u))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch($){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${$.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast(($.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!b&&n&&hideFbLeadProgress(!0),!n&&!b&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `}function getMetaReportMonthBodyOptions(e={},{editable:t=!1}={}){return{editable:t,showOtherToClient:e.metaReportShowOther!==!1}}function renderMetaReportMonthBody(e,{editable:t=!1,showOtherToClient:n=!0}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return`<div class="meta-report-empty">No ad spend recorded for this month yet.${t?' Click "Refresh from Meta" below, or use "Backfill from Meta" above to pull historical months.':""}</div>`;let a="";t&&(a=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        ...
        <div class="meta-report-line-items-editor">
          <div class="meta-report-line-items-head meta-cv-line-items-head">
            <h4>Other items</h4>
            <div class="meta-cv-line-items-toggle-row">
              <label class="meta-report-switch">
                <span class="meta-report-switch-track">
                  <input type="checkbox" class="meta-report-switch-input" id="meta-report-setting-other" ${n?"checked":""} />
                  <span class="meta-report-switch-thumb"></span>
                </span>
                <span class="meta-report-switch-label">Show "Other" section to client</span>
              </label>
              ${renderMetaReportSaveIndicator("meta-report-other-save-indicator")}
            </div>
          </div>
          <div id="meta-report-line-items-list">
            ${(e.customLineItems||[]).map((v,b)=>`
              <div class="meta-report-line-item-row" data-index="${b}">
                <input type="text" class="meta-line-label" value="${esc(v.label)}" placeholder="Label" />
                <input type="number" step="any" class="meta-line-amount" value="${esc(v.amount)}" placeholder="Amount" />
                <button type="button" class="admin-btn admin-btn--ghost admin-btn--small meta-line-remove">Remove</button>
              </div>
            `).join("")}
          </div>
          <button type="button" class="admin-btn admin-btn--secondary admin-btn--small meta-report-add-row" id="meta-report-add-line"${n?"":" disabled"}>+ Add row</button>
        </div>
    ...
    `);const s=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["CTR",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],i=[["Leads",metaFmtNum(e.topline?.leads),""],["Cost Per Lead (CPL)",metaFmtKr(e.topline?.cpl),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let o="";if(o+=renderMetaReportMetricTable("Meta ads",s,"meta"),o+=renderMetaReportMetricTable("Topline KPI'er",i,"topline",{highlightLastN:2}),e.bottomline){const v=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];if(e.bottomline.feeMode){const b=e.bottomline.feeLabel||(e.bottomline.feeMode==="marketing"?"Censio marketing fee":`Censio performance fee (${metaFmtNum(e.bottomline.feePercent,0)}%)`);v.push([b,metaFmtKr(e.bottomline.censioFee),""]),v.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),v.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])}o+=renderMetaReportMetricTable("Bottomline KPI'er",v,"bottomline",{highlightLastN:2})}if(!!e.customLineItems?.length&&(t||n)){const v=e.customLineItems.map(b=>[b.label,metaFmtKr(b.amount),""]);v.push(["Total",metaFmtKr(e.customLineItemsTotal),""]),o+=renderMetaReportMetricTable("Other",v,"other",{badge:t&&!n?"Hidden from client":"",highlightLastN:1})}const d=`
  ...
  `:"";return`${a}${d}${c}`}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      ...
      <div class="meta-report-settings">
        ...
        <div class="meta-report-settings-group meta-report-settings-group--stacked">
          ${renderMetaReportBottomlineFeeSettings(s,"meta-report",{compact:!1})}
        </div>
      </div>
  ...
  `).join("");return`
    ...
    `)}
  `}function bindMetaReportCopyButtons(e=document){e.querySelectorAll("[data-copy-report-url]").forEach(t=>{t.onclick=async()=>{const n=t.getAttribute("data-copy-report-url"),a=`${window.location.origin}${n}`;try{await navigator.clipboard.writeText(a),showToast("Share link copied","success")}catch{showToast(a,"info")}}})}async function patchMetaReportSettings(e,t){metaReportsState._settingsInflight=(metaReportsState._settingsInflight||0)+1;try{return await adminFetchWithRetry(`/api/meta-reports/clients/${encodeURIComponent(e)}/settings`,{method:"PATCH",body:JSON.stringify(t)},{retries:2,timeoutMs:2e4})}finally{metaReportsState._settingsInflight=Math.max(0,(metaReportsState._settingsInflight||1)-1)}}async function refreshMetaReportsHubData({silent:e=!0}={}){const t=document.getElementById("meta-reports-refresh");t&&(t.disabled=!0);try{const n=await adminFetch("/api/meta-reports?filter=all");metaReportsState.dashboardData=n,updateMetaReportsHubDom(getMetaReportsHubView(n,metaReportsState.filter,metaReportsState.searchQuery)),e||showToast("Meta reports refreshed","success")}catch(n){showToast(n.message||"Refresh failed","error")}finally{t&&(t.disabled=!1)}}function bindMetaReportsHubChromeEvents(){document.querySelectorAll("[data-meta-filter]").forEach(n=>{n.onclick=()=>{metaReportsState.filter=n.getAttribute("data-meta-filter")||"all",document.querySelectorAll("[data-meta-filter]").forEach(a=>{a.classList.toggle("is-active",a.getAttribute("data-meta-filter")===metaReportsState.filter)}),metaReportsState.dashboardData&&updateMetaReportsHubDom(getMetaReportsHubView(metaReportsState.dashboardData,metaReportsState.filter,metaReportsState.searchQuery))}});const e=document.getElementById("meta-reports-search");if(e){let n=null;e.oninput=()=>{clearTimeout(n),n=setTimeout(()=>{metaReportsState.searchQuery=e.value||"",metaReportsState.dashboardData&&updateMetaReportsHubDom(getMetaReportsHubView(metaReportsState.dashboardData,metaReportsState.filter,metaReportsState.searchQuery))},180)}}const t=document.getElementById("meta-reports-refresh");t&&(t.onclick=()=>refreshMetaReportsHubData({silent:!1}))}function bindMetaReportsHubRowEvents(e=document){e.querySelectorAll("[data-meta-report-enabled]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-report-enabled");t.disabled=!0;try{const s=(await patchMetaReportSettings(n,{metaReportEnabled:t.checked})).settings||{};mergeHubClient(n,{metaReportEnabled:s.metaReportEnabled,metaReportShowBottomline:s.metaReportShowBottomline,metaReportFeeEnabled:s.metaReportFeeEnabled,reportUrl:s.reportUrl}),showToast(t.checked?"Report enabled":"Report disabled","success"),updateMetaReportsHubDom(getMetaReportsHubView(metaReportsState.dashboardData,metaReportsState.filter,metaReportsState.searchQuery))}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),e.querySelectorAll("[data-meta-bottomline]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-bottomline");t.disabled=!0;try{const a=await patchMetaReportSettings(n,{metaReportShowBottomline:t.checked});mergeHubClient(n,{metaReportShowBottomline:a.settings?.metaReportShowBottomline}),showToast("Bottomline setting saved","success")}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),e.querySelectorAll("[data-meta-fee]").forEach(t=>{t.onchange=async()=>{const n=t.getAttribute("data-meta-fee");t.disabled=!0;try{const a=await patchMetaReportSettings(n,{metaReportFeeMode:t.checked?"performance":null});mergeHubClient(n,{metaReportFeeEnabled:a.settings?.metaReportFeeEnabled,metaReportFeeMode:a.settings?.metaReportFeeMode}),showToast("Fee setting saved","success")}catch(a){showToast(a.message||"Update failed","error"),t.checked=!t.checked}finally{t.disabled=!1}}}),bindMetaReportCopyButtons(e),e.querySelectorAll("[data-meta-provision]").forEach(t=>{t.onclick=async()=>{t.disabled=!0;try{await adminFetch("/api/meta-reports/provision",{method:"POST",body:JSON.stringify({accountName:t.getAttribute("data-meta-provision-name"),metaAdAccountId:t.getAttribute("data-meta-provision"),metaReportEnabled:!0})}),showToast("Client added","success"),await refreshMetaReportsHubData({silent:!0})}catch(n){showToast(n.message||"Failed to add client","error"),t.disabled=!1}}})}function bindMetaReportsHubEvents(){bindMetaReportsHubChromeEvents(),bindMetaReportsHubRowEvents(document.getElementById("meta-reports-cards")||document)}function collectMetaReportLineItems(){return Array.from(document.querySelectorAll(".meta-report-line-item-row")).map((e,t)=>({label:e.querySelector(".meta-line-label")?.value||"",amount:Number(e.querySelector(".meta-line-amount")?.value)||0,sortOrder:t})).filter(e=>e.label.trim())}function bindMetaReportsClientTabEvents(e){const t=e.clientId;document.querySelectorAll("[data-meta-month-tab]").forEach(n=>{n.onclick=()=>{metaReportsState.activeMonthKey=n.getAttribute("data-meta-month-tab");const a=document.getElementById("meta-report-month-panel");a&&(a.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(e.settings,{editable:!0})),bindMetaReportsClientEditEvents(t,e)),document.querySelectorAll("[data-meta-month-tab]").forEach(s=>{s.classList.toggle("is-active",s.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)}),syncMetaReportSettingsControls(e.settings||{},e.months?.[metaReportsState.activeMonthKey]||null)}})}function setMetaReportSavingState(e,t="report"){const n=document.getElementById("meta-report-month-panel"),a=document.getElementById("meta-report-save-indicator"),s=document.getElementById("meta-report-other-save-indicator"),i=e&&t==="report",o=e&&t==="other";n&&n.classList.toggle("is-loading",i),a&&a.classList.toggle("is-visible",i),s&&s.classList.toggle("is-visible",o),document.querySelector("#meta-report-edit-panel .meta-report-line-items-editor")?.classList.toggle("is-settings-saving",o),syncMetaReportOtherEditorState(document.getElementById("meta-report-setting-other")?.checked??!1,{saving:o})}function syncMetaReportOtherEditorState(e,{saving:t=!1}={}){const n=document.getElementById("meta-report-add-line");n&&(n.disabled=!e||t)}function syncMetaReportFeeFieldState(e,t="meta-report"){const n=document.getElementById(`${t}-fee-field`),a=document.getElementById(`${t}-setting-fee-percent`);n&&n.classList.toggle("is-disabled",!e),a&&(a.disabled=!e)}function syncMetaReportSettingsControls(e={},t=null){syncMetaReportBottomlineFeeDom(e,"meta-report")}function describeMetaReportSettingsChange(e,t){if(Object.prototype.hasOwnProperty.call(e,"metaReportFeeMode")){const n=t.metaReportFeeMode;return n?n==="marketing"?"Censio marketing fee enabled":"Censio performance fee enabled":"Censio fee disabled"}return Object.prototype.hasOwnProperty.call(e,"metaReportFeeEnabled")?t.metaReportFeeEnabled?"Censio fee enabled":"Censio fee disabled":Object.prototype.hasOwnProperty.call(e,"metaReportFeePercent")?`Performance fee set to ${t.metaReportFeePercent??20}% \u2014 report updated`:Object.prototype.hasOwnProperty.call(e,"metaReportMarketingFeeAmount")?`Marketing fee set to Dkr ${t.metaReportMarketingFeeAmount??0} \u2014 report updated`:Object.prototype.hasOwnProperty.call(e,"metaReportShowBottomline")?t.metaReportShowBottomline?"Bottomline shown on report":"Bottomline hidden from report":Object.prototype.hasOwnProperty.call(e,"metaReportShowOther")?t.metaReportShowOther!==!1?'"Other" shown to client':'"Other" hidden from client':"Settings saved"}async function saveMetaReportClientSettings(e,t,n){const a=Object.prototype.hasOwnProperty.call(t,"metaReportShowOther")?"other":"report";setMetaReportSavingState(!0,a);try{const s=await patchMetaReportSettings(e,{...t,monthKey:metaReportsState.activeMonthKey}),i=s.settings||{},o=metaReportsState.clientPayload;o&&(o.settings=i,s.monthPayload&&metaReportsState.activeMonthKey&&(o.months=o.months||{},o.months[metaReportsState.activeMonthKey]=s.monthPayload));const r=s.monthPayload||o?.months?.[metaReportsState.activeMonthKey]||null;if(syncMetaReportSettingsControls(i,r),a==="report"&&r){const l=document.getElementById("meta-report-month-panel");l&&(l.innerHTML=renderMetaReportMonthBody(r,getMetaReportMonthBodyOptions(i,{editable:!0})),bindMetaReportsClientEditEvents(e,o))}showToast(describeMetaReportSettingsChange(t,i),"success")}catch(s){showToast(s.message||"Update failed","error"),n&&n()}finally{setMetaReportSavingState(!1)}}function bindMetaReportBackfillButton(e){const t=document.getElementById("meta-report-backfill");t&&(t.onclick=async()=>{const n=metaReportsState.clientPayload,a=metaReportMonthsNeedingBackfill(n);if(!a.length)return;t.disabled=!0;const s=document.getElementById("meta-report-backfill-progress");let i=0,o=0;for(const r of a){i+=1,s&&(s.textContent=`Fetching ${metaMonthLabel(r)} ${r.slice(0,4)}\u2026 (${i}/${a.length})`);try{await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(r)}/refresh`,{method:"POST"})}catch{o+=1}}s&&(s.textContent=""),o?showToast(`Backfilled ${i-o}/${a.length} months. ${o} failed (check Meta token access).`,"error"):showToast(`Backfilled ${a.length} month${a.length===1?"":"s"} from Meta`,"success"),await loadMetaReportsClientPage({silent:!0})})}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,n=document.getElementById("meta-report-year");n&&(n.onchange=async()=>{if(n.selectedOptions?.[0]?.disabled){n.value=String(metaReportsState.selectedYear);return}metaReportsState.selectedYear=Number(n.value),metaReportsState.activeMonthKey=null,n.disabled=!0;try{await loadMetaReportsClientPage({silent:!0})}finally{n.disabled=!1}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report");const a=document.getElementById("meta-report-rotate-token");a&&(a.onclick=async()=>{if(window.confirm("Rotate the client share link? The old link will stop working.")){a.disabled=!0;try{const i=(await patchMetaReportSettings(t,{rotateAccessToken:!0})).settings?.reportUrl,o=metaReportsState.clientPayload;o&&(o.reportUrl=i);const r=document.getElementById("meta-report-copy-link");r&&i&&(r.setAttribute("data-copy-report-url",i),r.hidden=!1),showToast("Share link rotated","success")}catch(s){showToast(s.message||"Rotate failed","error")}finally{a.disabled=!1}}})}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const n=document.getElementById("meta-report-setting-other");syncMetaReportOtherEditorState(!!n?.checked),n&&(n.onchange=()=>{const o=n.checked;syncMetaReportOtherEditorState(o),saveMetaReportClientSettings(e,{metaReportShowOther:o},()=>{n.checked=!o,syncMetaReportOtherEditorState(!o)})});const a=document.getElementById("meta-report-add-line");a&&(a.onclick=()=>{if(a.disabled)return;const o=document.getElementById("meta-report-line-items-list");if(!o)return;const r=document.createElement("div");r.className="meta-report-line-item-row",r.innerHTML=`
        <input type="text" class="meta-line-label" placeholder="Label" />
        <input type="number" step="any" class="meta-line-amount" placeholder="Amount" />
        <button type="button" class="admin-btn admin-btn--ghost admin-btn--small meta-line-remove">Remove</button>
      `,o.appendChild(r),r.querySelector(".meta-line-remove")?.addEventListener("click",()=>r.remove())}),document.querySelectorAll(".meta-line-remove").forEach(o=>{o.onclick=()=>o.closest(".meta-report-line-item-row")?.remove()});const s=document.getElementById("meta-report-refresh-meta");s&&(s.onclick=async()=>{s.disabled=!0;try{const o=metaReportsState.activeMonthKey,r=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(r.monthPayload),showToast("Meta data refreshed","success")}catch(o){showToast(o.message||"Meta refresh failed","error")}finally{s.disabled=!1}});const i=document.getElementById("meta-report-save-month");i&&(i.onclick=async()=>{i.disabled=!0;try{const o=metaReportsState.activeMonthKey,r={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:Number(document.getElementById("meta-report-avg-profit")?.value),lineItems:collectMetaReportLineItems()},l=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}`,{method:"PUT",body:JSON.stringify(r)});refreshMetaReportMonthPanel(l.monthPayload),showToast("Month saved","success")}catch(o){showToast(o.message||"Save failed","error")}finally{i.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{metaReportsState.activeMonthKey=t.getAttribute("data-meta-month-tab");const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(e.settings,{editable:!1}))),document.querySelectorAll("[data-meta-month-tab]").forEach(a=>{a.classList.toggle("is-active",a.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}metaReportsState.selectedYear=Number(t.value),metaReportsState.activeMonthKey=null,t.disabled=!0;try{await loadPublicMetaReportPage({silent:!0})}finally{t.disabled=!1}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
      ${renderBrandTopbar("")}
    ...
    `).join(""):'<div class="meta-report-empty" style="padding:32px 12px">No matching clients.</div>'}function monthStatusIsEmpty(e,t={}){return e?(e.status??classifyMetaCvMonthStatus({wonLeads:e.inputs?.wonLeads??e.topline?.wonLeads??e.wonLeads,avgLeadValue:e.inputs?.avgLeadValue??e.topline?.avgLeadValue??e.avgLeadValue,avgProfitPerWon:e.inputs?.avgProfitPerWon??e.avgProfitPerWon,lineItemCount:e.customLineItems?.length??e.lineItemCount??0},!!t.metaReportShowBottomline))==="empty":!0}function metaReportEffectiveFeeEnabled(e,t=null){if(!t)return!!e?.metaReportFeeEnabled;const n=!!e?.metaReportShowBottomline;return(t.status??classifyMetaCvMonthStatus({wonLeads:t.inputs?.wonLeads??t.topline?.wonLeads??t.wonLeads,avgLeadValue:t.inputs?.avgLeadValue??t.topline?.avgLeadValue??t.avgLeadValue,avgProfitPerWon:t.inputs?.avgProfitPerWon??t.avgProfitPerWon,lineItemCount:t.customLineItems?.length??t.lineItemCount??0},n))==="empty"?!1:!!e?.metaReportFeeEnabled}function resolveMetaReportFeeMode(e={}){return e.metaReportFeeMode||(e.metaReportFeeEnabled?"performance":null)}const META_REPORT_FEE_MODE_META=[{value:"",icon:"\u2013",title:"None",desc:"No Censio fee"},{value:"performance",icon:"%",title:"Performance fee",desc:"% of profit (POAS)"},{value:"marketing",icon:"Dkr",title:"Marketing fee",desc:"Fixed Dkr amount"}];function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:n=!1}={}){const a=!!e.metaReportShowBottomline,s=resolveMetaReportFeeMode(e)||"",i=e.metaReportFeePercent??20,o=e.metaReportMarketingFeeAmount??0,r=META_REPORT_FEE_MODE_META.map(({value:l,icon:d,title:c,desc:v})=>{const b=s===l,S=l==="performance"?`
          <div class="meta-report-fee-mode-card-input${b?" is-visible":""}" id="${t}-fee-percent-block">
            <label>
              <span class="meta-report-fee-mode-card-input-label">Fee percentage</span>
              <span class="meta-report-fee-mode-card-input-wrap">
                <input type="number" step="any" min="0" max="100" id="${t}-setting-fee-percent" value="${esc(i)}"${b?"":" disabled"} />
                <span class="meta-report-fee-mode-card-input-suffix">%</span>
              </span>
            </label>
          </div>`:l==="marketing"?`
          <div class="meta-report-fee-mode-card-input${b?" is-visible":""}" id="${t}-marketing-fee-block">
            <label>
              <span class="meta-report-fee-mode-card-input-label">Fee amount</span>
              <span class="meta-report-fee-mode-card-input-wrap">
                <input type="number" step="any" min="0" id="${t}-setting-marketing-fee" value="${esc(o)}"${b?"":" disabled"} />
                <span class="meta-report-fee-mode-card-input-suffix">Dkr</span>
              </span>
            </label>
          </div>`:"";return`
        <div class="meta-report-fee-mode-card${b?" is-selected":""}" data-fee-mode-card="${esc(l)}">
          <label class="meta-report-fee-mode-card-row">
            <input type="radio" name="${t}-fee-mode" value="${esc(l)}"${b?" checked":""} />
            <span class="meta-report-fee-mode-card-icon" aria-hidden="true">${esc(d)}</span>
            <span class="meta-report-fee-mode-card-text">
              <span class="meta-report-fee-mode-card-title">${esc(c)}</span>
              <small>${esc(v)}</small>
            </span>
          </label>${S}
        </div>`}).join("");return`
    <div class="meta-report-bottomline-fee-settings">
      <div class="meta-report-bottomline-row">
        ...
        ${renderMetaReportSwitch(`${t}-setting-bottomline`,a,"Show bottomline",!1,{hideLabel:!0})}
      </div>
      <div class="meta-report-fee-nested${a?"":" is-hidden"}" id="${t}-fee-nested" data-applied-mode="${esc(s)}" data-applied-fee-percent="${esc(i)}" data-applied-marketing-fee="${esc(o)}">
        <div class="meta-report-fee-mode-grid${n?" meta-report-fee-mode-grid--rows":""}">${r}
        </div>
        <div class="meta-report-fee-actions" id="${t}-fee-actions">
          <span class="meta-report-fee-actions-hint">Unsaved fee changes</span>
          <div class="meta-report-fee-actions-buttons">
            <button type="button" class="admin-btn admin-btn--ghost admin-btn--small" id="${t}-fee-cancel">Cancel</button>
            <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee</button>
          </div>
        </div>
      </div>
    </div>
  `}function updateMetaReportFeeModeUi(e,t){const n=document.getElementById(`${e}-fee-nested`);if(!n)return;n.querySelectorAll("[data-fee-mode-card]").forEach(i=>{i.classList.toggle("is-selected",i.getAttribute("data-fee-mode-card")===(t||""))});const a=document.getElementById(`${e}-setting-fee-percent`),s=document.getElementById(`${e}-setting-marketing-fee`);document.getElementById(`${e}-fee-percent-block`)?.classList.toggle("is-visible",t==="performance"),document.getElementById(`${e}-marketing-fee-block`)?.classList.toggle("is-visible",t==="marketing"),a&&(a.disabled=t!=="performance"),s&&(s.disabled=t!=="marketing")}function updateMetaReportFeeDirtyState(e){const t=document.getElementById(`${e}-fee-nested`),n=document.getElementById(`${e}-fee-actions`);if(!t||!n)return;const a=t.querySelector(`input[name="${e}-fee-mode"]:checked`),s=a?a.value:"";let i=s!==(t.dataset.appliedMode||"");if(!i&&s==="performance"){const o=document.getElementById(`${e}-setting-fee-percent`);i=String(o?.value??"")!==String(t.dataset.appliedFeePercent??"")}if(!i&&s==="marketing"){const o=document.getElementById(`${e}-setting-marketing-fee`);i=String(o?.value??"")!==String(t.dataset.appliedMarketingFee??"")}n.classList.toggle("is-visible",i)}function resetMetaReportFeeUiToApplied(e){const t=document.getElementById(`${e}-fee-nested`);if(!t)return;const n=t.dataset.appliedMode||"";t.querySelectorAll(`input[name="${e}-fee-mode"]`).forEach(i=>{i.checked=i.value===n});const a=document.getElementById(`${e}-setting-fee-percent`),s=document.getElementById(`${e}-setting-marketing-fee`);a&&(a.value=t.dataset.appliedFeePercent??"20"),s&&(s.value=t.dataset.appliedMarketingFee??"0"),updateMetaReportFeeModeUi(e,n),document.getElementById(`${e}-fee-actions`)?.classList.remove("is-visible")}function syncMetaReportBottomlineFeeDom(e={},t="meta-report"){const n=!!e.metaReportShowBottomline,a=resolveMetaReportFeeMode(e)||"",s=e.metaReportFeePercent??20,i=e.metaReportMarketingFeeAmount??0,o=document.getElementById(`${t}-setting-bottomline`);o&&(o.checked=n);const r=document.getElementById(`${t}-fee-nested`);r?.classList.toggle("is-hidden",!n),r&&(r.dataset.appliedMode=a,r.dataset.appliedFeePercent=String(s),r.dataset.appliedMarketingFee=String(i)),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(c=>{c.checked=c.value===a});const l=document.getElementById(`${t}-setting-fee-percent`),d=document.getElementById(`${t}-setting-marketing-fee`);l&&(l.value=s),d&&(d.value=i),updateMetaReportFeeModeUi(t,a),document.getElementById(`${t}-fee-actions`)?.classList.remove("is-visible")}function bindMetaReportBottomlineFeeEvents(e,t,n){const a=n||((d,c)=>saveMetaReportClientSettings(e,d,c)),s=document.getElementById(`${t}-setting-bottomline`);s&&(s.onchange=()=>{const d=s.checked;a({metaReportShowBottomline:d},()=>{s.checked=!d})}),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(d=>{d.onchange=()=>{d.checked&&(updateMetaReportFeeModeUi(t,d.value||""),updateMetaReportFeeDirtyState(t))}});const i=document.getElementById(`${t}-setting-fee-percent`);i&&(i.oninput=()=>updateMetaReportFeeDirtyState(t),i.onkeydown=d=>{d.key==="Enter"&&(d.preventDefault(),document.getElementById(`${t}-fee-save`)?.click())});const o=document.getElementById(`${t}-setting-marketing-fee`);o&&(o.oninput=()=>updateMetaReportFeeDirtyState(t),o.onkeydown=d=>{d.key==="Enter"&&(d.preventDefault(),document.getElementById(`${t}-fee-save`)?.click())});const r=document.getElementById(`${t}-fee-cancel`);r&&(r.onclick=()=>resetMetaReportFeeUiToApplied(t));const l=document.getElementById(`${t}-fee-save`);l&&(l.onclick=async()=>{if(l.disabled)return;const c=document.getElementById(`${t}-fee-nested`)?.querySelector(`input[name="${t}-fee-mode"]:checked`),v=c&&c.value||null,b={metaReportFeeMode:v};if(v==="performance"&&i){let y=Number(i.value);Number.isFinite(y)||(y=20),y=Math.min(100,Math.max(0,y)),i.value=String(y),b.metaReportFeePercent=y}else if(v==="marketing"&&o){let y=Number(o.value);Number.isFinite(y)||(y=0),y=Math.max(0,y),o.value=String(y),b.metaReportMarketingFeeAmount=y}const S=l.textContent;l.disabled=!0,r&&(r.disabled=!0),l.textContent="Saving\u2026";try{await a(b,()=>resetMetaReportFeeUiToApplied(t))}finally{l.disabled=!1,r&&(r.disabled=!1),l.textContent=S}})}function renderMetaReportSaveIndicator(e,t=!1){return`
  ...
  `}function renderMetaCvEditorSkeleton(e,t,n={}){return`
    ...
    <div class="meta-cv-editor-skeleton" aria-busy="true" aria-label="Loading month details">
      <div class="meta-cv-skeleton-section">
        <div class="meta-cv-skeleton-line meta-cv-skeleton-line--sm"></div>
        <div class="meta-cv-skeleton-row">
          <div class="meta-cv-skeleton-pill"></div>
          <div class="meta-cv-skeleton-pill"></div>
          <div class="meta-cv-skeleton-pill meta-cv-skeleton-pill--short"></div>
        </div>
      </div>
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find(u=>u.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const s=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,s);const i=t.editorPayload?.months?.[a]||null,o=n.metaReportShowBottomline,r=s.wonLeads!=null?s.wonLeads:i?.inputs?.wonLeads??i?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:i?.inputs?.avgLeadValue??i?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:i?.inputs?.avgProfitPerWon??"",c=i?.customLineItems||[],v=n.metaReportShowOther!==!1,b=metaReportsState.customValues.draftInputs,S=b?.wonLeads??r,y=b?.avgLeadValue??l,w=b?.avgProfitPerWon??d,$=b?.lineItems??c,h=!!n.metaReportFeeEnabled,f=t.settingsSaving&&t.settingsSavingScope==="other",p=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-report-settings meta-cv-client-settings${p?" is-saving":""}">
      ...
      <div class="meta-report-settings-group meta-report-settings-group--stacked">
        ${renderMetaReportBottomlineFeeSettings(n,"meta-cv",{compact:!0})}
      </div>
    </div>
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(u=>`
            <option value="${esc(u.clientId)}"${u.clientId===n.clientId?" selected":""}>${esc(u.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(u=>`
            <option value="${esc(u)}"${u===a?" selected":""}>${esc(metaMonthLabel(u))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(S)}" />
      </label>
    ...
    <div class="meta-report-line-items-editor meta-cv-line-items-editor${f?" is-settings-saving":""}">
      <div class="meta-report-line-items-head meta-cv-line-items-head">
        <div class="meta-cv-line-items-head-row">
          <h4>Other items</h4>
          <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-cv-add-line"${!v||t.settingsSaving?" disabled":""}>Add row</button>
        </div>
        <div class="meta-cv-line-items-toggle-row">
          ${renderMetaReportSwitch("meta-cv-setting-other",v,'Show "Other" section to client')}
          ${renderMetaReportSaveIndicator("meta-cv-other-settings-status",f)}
        </div>
      </div>
      <div id="meta-cv-line-items-list">
        ${$.length?$.map((u,m)=>`
          <div class="meta-report-line-item-row" data-index="${m}">
            <input type="text" class="meta-cv-line-label" placeholder="Label" value="${esc(u.label||"")}" />
            <input type="number" step="any" class="meta-cv-line-amount" placeholder="Amount" value="${esc(u.amount??"")}" />
            <button type="button" class="admin-btn admin-btn--ghost admin-btn--small meta-cv-line-remove">Remove</button>
          </div>
        `).join(""):'<div class="note">No other items yet.</div>'}
      </div>
    </div>
  ...
  `).join("");return`
    ...
    `)}
  `}function refreshMetaCvListAndEditor(){const e=metaReportsState.customValues.overview;if(!e)return;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e));const n=document.getElementById("meta-cv-editor");n&&(n.innerHTML=renderMetaCvEditor(e)),metaReportsState.customValues.draftInputs=null,bindMetaCvEditorEvents(),bindMetaCvListEvents()}function setMetaCvSavePending(e){metaReportsState.customValues.saving=e;const t=document.getElementById("meta-cv-save"),n=document.getElementById("meta-cv-save-indicator");n&&n.classList.toggle("is-visible",e),t&&(t.disabled=e||metaReportsState.customValues.loadingEditor||metaReportsState.customValues.settingsSaving,t.textContent=e?"Saving\u2026":"Save values")}function setMetaCvSettingsSaving(e,t="report"){metaReportsState.customValues.settingsSaving=e,metaReportsState.customValues.settingsSavingScope=e?t:null;const n=e&&t==="report",a=e&&t==="other";document.getElementById("meta-cv-settings-status")?.classList.toggle("is-visible",n),document.getElementById("meta-cv-other-settings-status")?.classList.toggle("is-visible",a),document.querySelector(".meta-cv-client-settings")?.classList.toggle("is-saving",n),document.querySelector(".meta-cv-line-items-editor")?.classList.toggle("is-settings-saving",a),["meta-cv-setting-bottomline"].forEach(r=>{const l=document.getElementById(r);l&&(l.disabled=n)}),document.querySelectorAll('input[name="meta-cv-fee-mode"]').forEach(r=>{r.disabled=n}),["meta-cv-setting-fee-percent","meta-cv-setting-marketing-fee"].forEach(r=>{const l=document.getElementById(r);l&&(l.disabled=n||l.disabled)});const i=document.getElementById("meta-cv-setting-other");i&&(i.disabled=a),syncMetaCvOtherEditorState(i?.checked??!1,{saving:e});const o=document.getElementById("meta-cv-save");o&&(o.disabled=e||metaReportsState.customValues.saving||metaReportsState.customValues.loadingEditor)}function syncMetaCvOtherEditorState(e,{saving:t=!1}={}){const n=document.getElementById("meta-cv-add-line");n&&(n.disabled=!e||t)}function classifyMetaCvMonthStatus(e,t=!1){if(!e)return"empty";const n=e.wonLeads!=null,a=e.avgLeadValue!=null,s=e.avgProfitPerWon!=null,i=Number(e.lineItemCount)||0;return n&&a&&(!t||s)?"complete":n||a||s||i>0?"partial":"empty"}function applyMetaCvSaveResult(e,t,n,a){const i=metaReportsState.customValues.overview?.clients?.find(l=>l.clientId===e),o=i?.months?.[t],r=a?.month||{};o&&(o.wonLeads=r.wonLeads??n.wonLeads,o.avgLeadValue=r.avgLeadValue??n.avgLeadValue,o.avgProfitPerWon=r.avgProfitPerWon??n.avgProfitPerWon,o.lineItemCount=r.lineItems?.length??n.lineItems?.length??0,o.status=classifyMetaCvMonthStatus(o,!!i.metaReportShowBottomline),o.updatedAt=o.status==="empty"?null:r.updatedAt||new Date().toISOString()),a?.monthPayload&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:a.monthPayload}})}async function refreshMetaCvOverviewSilently(){const e=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(e.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=e;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e),bindMetaCvListEvents());const n=document.querySelector(".meta-cv-summary");if(n&&e.summary){const a=e.summary;n.innerHTML=`
    ...
    `}}function collectMetaCvLineItems(){return Array.from(document.querySelectorAll("#meta-cv-line-items-list .meta-report-line-item-row")).map((e,t)=>({label:String(e.querySelector(".meta-cv-line-label")?.value||"").trim(),amount:Number(e.querySelector(".meta-cv-line-amount")?.value)||0,sortOrder:t})).filter(e=>e.label)}function collectMetaCvSaveBody(){return{wonLeads:document.getElementById("meta-cv-won-leads")?.value===""?null:Number(document.getElementById("meta-cv-won-leads")?.value),avgLeadValue:document.getElementById("meta-cv-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-cv-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-cv-avg-profit")?.value===""?null:Number(document.getElementById("meta-cv-avg-profit")?.value),lineItems:collectMetaCvLineItems()}}function syncMetaCvClientSettingsDom(e={}){syncMetaReportBottomlineFeeDom(e,"meta-cv");const t=document.getElementById("meta-cv-setting-other");t&&(t.checked=e.metaReportShowOther!==!1),syncMetaCvOtherEditorState(e.metaReportShowOther!==!1)}async function saveMetaCvClientSettings(e,t,n){const a=Object.prototype.hasOwnProperty.call(t,"metaReportShowOther")?"other":"report";setMetaCvSettingsSaving(!0,a);try{const s=metaReportsState.customValues.selectedMonthKey,i=await patchMetaReportSettings(e,{...t,monthKey:s||void 0}),r=metaReportsState.customValues.overview?.clients?.find(l=>l.clientId===e);r&&i.settings&&(r.metaReportShowBottomline=!!i.settings.metaReportShowBottomline,r.metaReportFeeEnabled=!!i.settings.metaReportFeeEnabled,r.metaReportFeeMode=i.settings.metaReportFeeMode||null,r.metaReportFeePercent=i.settings.metaReportFeePercent??20,r.metaReportMarketingFeeAmount=i.settings.metaReportMarketingFeeAmount??0,r.metaReportShowOther=i.settings.metaReportShowOther!==!1),i.monthPayload&&s&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[s]:i.monthPayload}}),setMetaCvSettingsSaving(!1),syncMetaCvClientSettingsDom(i.settings||{}),showToast(describeMetaReportSettingsChange(t,i.settings||{}),"success")}catch(s){showToast(s.message||"Update failed","error"),n&&n(),setMetaCvSettingsSaving(!1)}}function bindMetaCvSettingsEvents(e){const t=document.getElementById("meta-cv-setting-other");syncMetaCvOtherEditorState(!!t?.checked),bindMetaReportBottomlineFeeEvents(e,"meta-cv",(n,a)=>saveMetaCvClientSettings(e,n,a)),t&&(t.onchange=()=>{const n=t.checked;syncMetaCvOtherEditorState(n),saveMetaCvClientSettings(e,{metaReportShowOther:n},()=>{t.checked=!n,syncMetaCvOtherEditorState(!n)})})}async function selectMetaCvMonth(e,t,{reloadEditor:n=!0}={}){if(metaReportsState.customValues.selectedClientId=e,metaReportsState.customValues.selectedMonthKey=t,refreshMetaCvListAndEditor(),!!n){metaReportsState.customValues.loadingEditor=!0,refreshMetaCvListAndEditor();try{const a=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(t)}`);metaReportsState.customValues.selectedClientId===e&&metaReportsState.customValues.selectedMonthKey===t&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:a.monthPayload}})}catch(a){showToast(a.message||"Failed to load month","error")}finally{metaReportsState.customValues.loadingEditor=!1,refreshMetaCvListAndEditor()}}}function bindMetaCvListEvents(){document.querySelectorAll("[data-meta-cv-month]").forEach(e=>{e.onclick=()=>{selectMetaCvMonth(e.getAttribute("data-meta-cv-client"),e.getAttribute("data-meta-cv-month"))}})}function bindMetaCvEditorEvents(){const e=metaReportsState.customValues.selectedClientId;e&&bindMetaCvSettingsEvents(e);const t=document.getElementById("meta-cv-client-select"),n=document.getElementById("meta-cv-month-select");t&&(t.onchange=()=>{const i=metaReportsState.customValues.selectedMonthKey||metaReportsState.customValues.overview?.monthKeys?.slice(-1)[0];i&&selectMetaCvMonth(t.value,i)}),n&&(n.onchange=()=>{const i=metaReportsState.customValues.selectedClientId;i&&selectMetaCvMonth(i,n.value)});const a=document.getElementById("meta-cv-add-line");a&&(a.onclick=()=>{if(a.disabled)return;const i=document.getElementById("meta-cv-line-items-list");if(!i)return;i.querySelector(".note")&&(i.innerHTML="");const o=document.createElement("div");o.className="meta-report-line-item-row",o.innerHTML=`
        <input type="text" class="meta-cv-line-label" placeholder="Label" value="" />
        <input type="number" step="any" class="meta-cv-line-amount" placeholder="Amount" value="" />
        <button type="button" class="admin-btn admin-btn--ghost admin-btn--small meta-cv-line-remove">Remove</button>
      `,i.appendChild(o),o.querySelector(".meta-cv-line-remove")?.addEventListener("click",()=>o.remove())}),document.querySelectorAll(".meta-cv-line-remove").forEach(i=>{i.onclick=()=>i.closest(".meta-report-line-item-row")?.remove()});const s=document.getElementById("meta-cv-save");s&&(s.onclick=async()=>{const i=metaReportsState.customValues.selectedClientId,o=metaReportsState.customValues.selectedMonthKey;if(!i||!o)return;const r=collectMetaCvSaveBody();setMetaCvSavePending(!0);try{const l=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(i)}/months/${encodeURIComponent(o)}`,{method:"PUT",body:JSON.stringify(r)});applyMetaCvSaveResult(i,o,r,l),metaReportsState.customValues.draftInputs=r,setMetaCvSavePending(!1),refreshMetaCvListAndEditor(),showToast("Custom values saved","success"),await refreshMetaCvOverviewSilently()}catch(l){showToast(l.message||"Save failed","error"),setMetaCvSavePending(!1)}})}function bindMetaReportsCustomValuesEvents(e){const t=document.getElementById("meta-cv-year");t&&(t.onchange=async()=>{metaReportsState.selectedYear=Number(t.value),metaReportsState.customValues.selectedClientId=null,metaReportsState.customValues.selectedMonthKey=null,metaReportsState.customValues.editorPayload=null,await loadMetaReportsCustomValuesPage({silent:!0})});const n=document.getElementById("meta-cv-search");n&&(n.oninput=()=>{metaReportsState.customValues.searchQuery=n.value;const a=document.getElementById("meta-cv-client-list");a&&(a.innerHTML=renderMetaCvClientRows(e)),bindMetaCvListEvents()}),bindMetaCvListEvents(),bindMetaCvEditorEvents()}async function loadMetaReportsCustomValuesPage({silent:e=!1,keepSelection:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){window.location.href=`/login?next=${encodeURIComponent(window.location.pathname)}`;return}currentStaffUser=a,!metaReportsState.customValues.mounted&&!e&&(n.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("meta-reports-custom"))}
      ${wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading custom values...</p></div>')}
    `);try{const s=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(s.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=s,t||(s.clients||[]).some(o=>o.clientId===metaReportsState.customValues.selectedClientId)||(metaReportsState.customValues.selectedClientId=null,metaReportsState.customValues.selectedMonthKey=null,metaReportsState.customValues.editorPayload=null),n.innerHTML=renderMetaReportsCustomValuesPage(s),metaReportsState.customValues.mounted=!0,metaReportsState.customValues.saving=!1,bindMetaReportsCustomValuesEvents(s)}catch(s){metaReportsState.customValues.mounted?showToast(s.message||"Load failed","error"):n.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("meta-reports-custom"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(s.message)}</div>`)}
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),r=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),r&&(r.disabled=!0,r.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const b=canReuseBootstrapDashboard(c)&&!s;let S,y;if(b?[y,S]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[S,y]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=y,cachedData=applyMarketingData(S,y),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(l!==fetchGeneration)return;if(c.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            `)}`;return}if(IS_ADMIN_CLIENT&&o){const v=document.getElementById("dashboard-content");v&&(v.innerHTML=`
            <div class="note admin-setup-placeholder">
              ${esc(c.message)}
