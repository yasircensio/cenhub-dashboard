(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){if(t[1]==="sync-history"&&(t[2]==="ghl"||t[2]==="meta"||t[2]==="meta-reports")){document.body.dataset.dashboardMode=t[2]==="meta-reports"?"sync-history-meta-reports":`sync-history-${t[2]}`,delete document.body.dataset.clientSlug;return}if(t[1]==="fb-lead-sync"){document.body.dataset.dashboardMode="fb-lead-sync",delete document.body.dataset.clientSlug;return}if(t[1]==="google-ads"&&t[2]){document.body.dataset.dashboardMode="google-ads-client",document.body.dataset.clientSlug=t[2];return}if(t[1]==="google-ads"){document.body.dataset.dashboardMode="google-ads",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="custom-values"){document.body.dataset.dashboardMode="meta-reports-custom",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="ghl-clients"){document.body.dataset.dashboardMode="meta-reports-ghl-clients",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]){document.body.dataset.dashboardMode="meta-reports-client",document.body.dataset.clientSlug=t[2];return}if(t[1]==="meta-reports"){document.body.dataset.dashboardMode="meta-reports",delete document.body.dataset.clientSlug;return}document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}if(t[0]==="report"&&t[1]){document.body.dataset.dashboardMode="report",document.body.dataset.reportToken=t[1],delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_ADMIN_SYNC_HISTORY_GHL=DASHBOARD_MODE==="sync-history-ghl",IS_ADMIN_SYNC_HISTORY_META=DASHBOARD_MODE==="sync-history-meta",IS_ADMIN_SYNC_HISTORY_META_REPORTS=DASHBOARD_MODE==="sync-history-meta-reports",IS_ADMIN_FB_LEAD_SYNC=DASHBOARD_MODE==="fb-lead-sync",IS_ADMIN_META_REPORTS=DASHBOARD_MODE==="meta-reports",IS_ADMIN_META_REPORTS_CLIENT=DASHBOARD_MODE==="meta-reports-client",IS_ADMIN_META_REPORTS_CUSTOM=DASHBOARD_MODE==="meta-reports-custom",IS_ADMIN_META_REPORTS_GHL_CLIENTS=DASHBOARD_MODE==="meta-reports-ghl-clients",IS_ADMIN_GOOGLE_ADS=DASHBOARD_MODE==="google-ads",IS_ADMIN_GOOGLE_ADS_CLIENT=DASHBOARD_MODE==="google-ads-client",IS_REPORT_VIEW=DASHBOARD_MODE==="report",REPORT_TOKEN=document.body.dataset.reportToken||"",IS_ADMIN_SYNC_HISTORY=IS_ADMIN_SYNC_HISTORY_GHL||IS_ADMIN_SYNC_HISTORY_META||IS_ADMIN_SYNC_HISTORY_META_REPORTS,IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN_META_REPORTS_GHL_CLIENTS||IS_ADMIN_GOOGLE_ADS||IS_ADMIN_GOOGLE_ADS_CLIENT||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN_META_REPORTS_GHL_CLIENTS||IS_ADMIN_GOOGLE_ADS||IS_ADMIN_GOOGLE_ADS_CLIENT||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean),t=new Set(["meta-reports","google-ads","sync-history","fb-lead-sync"]);return e[0]==="admin"&&e[1]==="google-ads"&&e[2]||e[0]==="admin"&&e[1]==="meta-reports"&&e[2]&&e[2]!=="custom-values"&&e[2]!=="ghl-clients"?e[2]:e[0]==="admin"&&e[1]&&!t.has(e[1])?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"&&e[0]!=="report"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){const t=!!(e&&e.includes("staff-admin-chrome")),a=t||e&&e.includes("admin-topbar-link")?"/admin":"",n=a?`<a href="${a}" class="brand-logo-link" aria-label="Cenhub home"><img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" /></a>`:'<img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" />',o=t?`<button type="button" class="staff-nav-toggle" id="staff-nav-toggle" aria-expanded="false" aria-controls="staff-topbar-panel" aria-label="Open menu">
        ...
        <span class="staff-nav-toggle-bar" aria-hidden="true"></span>
        <span class="staff-nav-toggle-bar" aria-hidden="true"></span>
      </button>`:"",s=e?t?`<div class="brand-topbar-right brand-topbar-panel" id="staff-topbar-panel">${e}</div>`:`<div class="brand-topbar-right">${e}</div>`:"";return`
...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),y=$.summary||$;l=y.runId,c=0,setFbLeadProgress({processed:0,total:y.inWindow||0,label:`Matching ${y.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:b||g?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:r})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const y=$.inWindow||c||b;a?(setFbLeadProgress({processed:c,total:y,complete:!$.hasMore}),s&&(s.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:y,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const w=d.summary||d,m=w.inWindow||c||b,f=w.nextBatchOffset??c;if(a)setFbLeadProgress({processed:f,total:m,complete:!0}),markFbLeadPreviewReady(e,t,w.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(w.updated||0)>0),showToast((w.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:f,total:m,written:w.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(w){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${w.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((w.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(s=>s.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function renderMetaReportMonthTables(e,{editable:t=!1}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return renderMetaReportEmptyMonthCard({editable:t,synced:!!e.metaFetchedAt});const a=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["CTR",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],n=[["Leads",metaFmtNum(e.topline?.leads),""],["Cost Per Lead (CPL)",metaFmtKr(e.topline?.cpl),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let o="";if(o+=renderMetaReportMetricTable("Meta ads",a,"meta"),o+=renderMetaReportMetricTable("Topline KPI'er",n,"topline",{highlightLastN:2}),e.bottomline){const s=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];if(e.bottomline.feeMode){const r=e.bottomline.feeLabel||(e.bottomline.feeMode==="marketing"?"Censio marketing fee":`Censio performance fee (${metaFmtNum(e.bottomline.feePercent,0)}%)`);s.push([r,metaFmtKr(e.bottomline.censioFee),""]),s.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),s.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])}o+=renderMetaReportMetricTable("Bottomline KPI'er",s,"bottomline",{highlightLastN:2})}return o?`
    <div class="meta-report-tables-card">
      <div class="meta-report-tables-card-label">Details</div>
      <div class="meta-report-groups-stack">${o}</div>
    </div>
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null,settings:o=null}={}){const s=n||e?.monthKey||null,r=s?metaReportMonthBounds(s):null,i=o||a?.settings||{},l=isMetaReportGhlListClient(i),c=resolveClientToplineMode(i),d=resolveMetaToplineSource(e,i),u=l&&c==="cenhub"&&d==="ghl";let p="";if(t&&e&&!e.meta?.emptyMonth&&r){const k=u?" readonly":"",L=renderMetaReportMonthManualRow(e,i);p=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        ...
        <div class="meta-report-edit-grid">
          ...
          <label class="meta-cv-value-field${u?" is-locked":""}">Number of leads<input type="number" step="any" id="meta-report-leads" value="${esc(u?e.ghlTotals?.leads??e.topline?.leads??"":e.topline?.leads??"")}"${k}${u?' disabled tabindex="-1" aria-readonly="true"':""} /></label>
          <label class="meta-cv-value-field${u?" is-locked":""}">Won leads<input type="number" step="any" id="meta-report-won-leads" value="${esc(e.topline?.wonLeads??"")}"${k}${u?' disabled tabindex="-1" aria-readonly="true"':""} /></label>
          <label>Avg lead value<input type="number" step="any" id="meta-report-avg-lead-value" value="${esc(e.topline?.avgLeadValue??"")}"${k} /></label>
          <label>Avg profit per won<input type="number" step="any" id="meta-report-avg-profit" value="${esc(e.inputs?.avgProfitPerWon??"")}"${k} /></label>
        </div>
        ${L}
    ...
    `}const h=n||e?.monthKey||null,g=t?h?`Report preview \xB7 ${metaMonthLabel(h)}`:"Report preview":h?`Report \xB7 ${metaMonthLabel(h)}`:"Report",b=metaReportsState.reportViewMode||"monthly",w=shouldShowMetaReportYearVisuals(e,a,{editable:t}),m=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${g}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",f=renderMetaReportMonthTables(e,{editable:t}),v=w?renderMetaReportChartsPanel({editable:t}):"",$=t&&w?renderMetaReportScenarioCard():"",y=w?renderMetaReportViewModeTabs():"";let S;return w?S=`
      <div class="meta-report-view-panel${b==="monthly"?" is-active":""}" data-report-view-panel="monthly" role="tabpanel"${b==="monthly"?"":" hidden"}>
      ...
      ${$}
    `:S=`<div class="meta-report-content-main">${f}</div>`,`${p}${m}${y}${S}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
    ...
    `)}
  `}function renderMetaReportsClientPage(e){const t=e.monthKeys||[],a=metaReportsState.activeMonthKey||t[t.length-1]||"",n=e.months?.[a]||null,o=e.settings||{},s=t.map(r=>`
  ...
  `).join("");return`
    ...
    <div class="meta-reports-page meta-reports-page--public">
      ...
      <div class="meta-report-tabs" role="tablist">${s}</div>
      <div class="meta-report-month-panel" id="meta-report-month-panel">
        ${renderMetaReportMonthBody(o,getMetaReportMonthBodyOptions(e.settings,{editable:!1,yearPayload:e}))}
      </div>
    </div>
  ...
  `}function setMetaReportYearLoadingBanner(e,t,a){const n=document.querySelector(e);if(!n)return;let o=n.querySelector(".meta-report-year-loading-banner");a?(o||(o=document.createElement("span"),o.className="meta-report-year-loading-banner",o.setAttribute("aria-live","polite"),n.appendChild(o)),o.innerHTML=`<span class="meta-report-save-indicator-spinner"></span> Loading ${esc(t)}\u2026`,o.hidden=!1):o&&(o.hidden=!0)}function setMetaReportClientYearLoading(e,t){const a=document.querySelector(".meta-report-client-page"),n=document.getElementById("meta-report-year");if(t){n&&(n.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!0);const o=document.querySelector(".meta-report-tabs");o&&(o.innerHTML=renderMetaReportTabsSkeleton(),o.setAttribute("aria-busy","true"));const s=document.getElementById("meta-report-month-panel");s&&(s.innerHTML=renderMetaReportMonthPanelSkeleton(),s.classList.add("is-loading")),destroyMetaReportCharts()}else{n&&(n.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!1);const o=document.querySelector(".meta-report-tabs");o&&o.removeAttribute("aria-busy");const s=document.getElementById("meta-report-month-panel");s&&s.classList.remove("is-loading")}}function setMetaCvYearLoading(e,t){const a=document.querySelector(".sync-history-page.meta-reports-page"),n=document.getElementById("meta-cv-year"),o=document.getElementById("meta-cv-search");if(t){n&&(n.disabled=!0),o&&(o.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!0);const s=document.querySelector(".meta-cv-summary");s&&(s.classList.add("is-skeleton"),s.innerHTML=renderMetaCvSummarySkeleton());const r=document.querySelector(".meta-cv-legend");r&&r.setAttribute("aria-hidden","true");const i=document.getElementById("meta-cv-client-list");i&&(i.innerHTML=renderMetaCvListSkeleton(),i.setAttribute("aria-busy","true"));const l=document.getElementById("meta-cv-editor");l&&(l.innerHTML=renderMetaCvEditorYearSkeleton(),l.setAttribute("aria-busy","true"))}else{n&&(n.disabled=!1),o&&(o.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!1);const s=document.querySelector(".meta-cv-summary");s&&s.classList.remove("is-skeleton");const r=document.querySelector(".meta-cv-legend");r&&r.removeAttribute("aria-hidden");const i=document.getElementById("meta-cv-client-list");i&&i.removeAttribute("aria-busy");const l=document.getElementById("meta-cv-editor");l&&l.removeAttribute("aria-busy")}}function setMetaReportPublicYearLoading(e,t){const a=document.querySelector(".meta-reports-page--public"),n=document.getElementById("meta-report-year");if(t){n&&(n.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!0);const o=document.querySelector(".meta-reports-page--public .meta-report-tabs");o&&(o.innerHTML=renderMetaReportTabsSkeleton(),o.setAttribute("aria-busy","true"));const s=document.getElementById("meta-report-month-panel");s&&(s.innerHTML=renderMetaReportMonthPanelSkeleton(),s.classList.add("is-loading")),destroyMetaReportCharts()}else{n&&(n.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!1);const o=document.querySelector(".meta-reports-page--public .meta-report-tabs");o&&o.removeAttribute("aria-busy");const s=document.getElementById("meta-report-month-panel");s&&s.classList.remove("is-loading")}}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,a=document.getElementById("meta-report-year");a&&(a.onchange=async()=>{if(a.selectedOptions?.[0]?.disabled){a.value=String(metaReportsState.selectedYear);return}const o=metaReportsState.selectedYear,s=Number(a.value);if(s!==o){metaReportsState.selectedYear=s,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportClientYearLoading(s,!0);try{await loadMetaReportsClientPage({silent:!0})}catch(r){metaReportsState.selectedYear=o,a&&(a.value=String(o)),await loadMetaReportsClientPage({silent:!0}),showToast(r.message||"Load failed","error")}finally{setMetaReportClientYearLoading(s,!1)}}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineSummaryEvents(e),bindMetaReportExcelSheetSummaryEvents(e),bindMetaReportShareEvents(e)}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),rebindMetaReportMonthPanelEvents(e.clientId,e),isMetaReportGhlListClient(e.settings||{})&&bindMetaReportToplineSourceControl(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const a=document.getElementById("meta-report-refresh-meta");a&&(a.onclick=async()=>{a.disabled=!0;try{const o=metaReportsState.activeMonthKey,s=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(s.monthPayload),showToast("Meta data refreshed","success")}catch(o){showToast(o.message||"Meta refresh failed","error")}finally{a.disabled=!1}});const n=document.getElementById("meta-report-save-month");n&&(n.onclick=async()=>{n.disabled=!0;try{const o=metaReportsState.activeMonthKey,s=t?.months?.[o],r=t?.settings||{},i=resolveClientToplineMode(r),l=resolveMetaToplineSource(s,r),c=isMetaReportGhlListClient(r)&&i==="cenhub"&&l==="manual",d={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:document.getElementById("meta-report-won-leads")?.value===""?null:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:document.getElementById("meta-report-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-report-avg-profit")?.value===""?null:Number(document.getElementById("meta-report-avg-profit")?.value)};c&&(d.toplineSource="manual",d.manualOverride=!0,d.manualLeads=document.getElementById("meta-report-leads")?.value===""?null:Number(document.getElementById("meta-report-leads")?.value));const u=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}`,{method:"PUT",body:JSON.stringify(d)});refreshMetaReportMonthPanel(u.monthPayload),showToast("Month saved","success")}catch(o){showToast(o.message||"Save failed","error")}finally{n.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{switchMetaReportMonthTab(t.getAttribute("data-meta-month-tab"),{editable:!1})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}const n=metaReportsState.selectedYear,o=Number(t.value);if(o!==n){metaReportsState.selectedYear=o,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportPublicYearLoading(o,!0);try{await loadPublicMetaReportPage({silent:!0})}catch(s){metaReportsState.selectedYear=n,t&&(t.value=String(n)),await loadPublicMetaReportPage({silent:!0}),showToast(s.message||"Load failed","error")}finally{setMetaReportPublicYearLoading(o,!1)}}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
    ...
    `;else if(!e){const o=document.getElementById("meta-report-month-panel");o&&o.classList.add("is-loading")}try{const o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(n)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`);if(syncMetaReportSelectedYear(o,{disableUnavailable:!0}),Number(metaReportsState.selectedYear)!==Number(o.year))return loadMetaReportsClientPage({silent:!0});metaReportsState.activeMonthKey||(metaReportsState.activeMonthKey=o.monthKeys?.[o.monthKeys.length-1]||null),await ensureMetaReportScenarioSource(o,{editable:!0}),metaReportsState.clientPageMounted?updateMetaReportsClientContent(o):(metaReportsState.clientPayload=o,metaReportsState.clientReportSettingsExpanded=!1,metaReportsState.clientShareExpanded=!1,hydrateMetaReportSpendChartType(o),hydrateMetaReportScenarioSettings(o),t.innerHTML=renderMetaReportsClientPage(o),metaReportsState.clientPageMounted=!0,bindMetaReportsClientEvents(o),mountMetaReportCharts(o,{editable:!0}))}catch(o){if(!metaReportsState.clientPageMounted)t.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(o.message)}</div>`)}
      `;else{if(e)throw o;showToast(o.message||"Load failed","error")}}finally{const o=document.getElementById("meta-report-month-panel");o&&o.classList.remove("is-loading")}}const googleAdsReportsState={filter:"all",searchQuery:"",dashboardData:null,hubMounted:!1,clientPageMounted:!1,clientPayload:null,activeMonthKey:null,selectedYear:new Date().getFullYear(),chart:null},GOOGLE_ADS_HUB_FILTERS=[{value:"all",label:"All"},{value:"enabled",label:"Live"},{value:"needs-setup",label:"Needs setup"}];function filterGoogleAdsClients(e,t,a=""){let n=e||[];t==="enabled"&&(n=n.filter(s=>s.googleAdsReportEnabled)),t==="needs-setup"&&(n=n.filter(s=>s.needsSetup));const o=String(a||"").trim().toLowerCase();return o&&(n=n.filter(s=>[s.accountName,s.googleCustomerId,s.googleCustomerLabel,s.clientId].filter(Boolean).join(" ").toLowerCase().includes(o))),n}function getGoogleAdsHubView(e,t,a=googleAdsReportsState.searchQuery){const n=e?.clients||[];return{clients:filterGoogleAdsClients(n,t,a),allClients:n,summary:e?.summary||{},google:e?.google||{},filter:t,searchQuery:a}}function renderGoogleAdsHubFiltersHtml(e){return GOOGLE_ADS_HUB_FILTERS.map(t=>`
    <button
      type="button"
      class="meta-hub-filter${e===t.value?" is-active":""}"
      data-google-ads-filter="${esc(t.value)}"
    >${esc(t.label)}</button>
  `).join("")}function renderGoogleAdsHubCard(e){const t=!!e.clientId,a=e.clientId||e.googleCustomerId||"",n=e.needsSetup?'<span class="meta-report-badge meta-report-badge--setup">Needs setup</span>':e.googleAdsReportEnabled?'<span class="meta-report-badge meta-report-badge--on">Report live</span>':'<span class="meta-report-badge meta-report-badge--off">Report off</span>',o=t?`<a href="/admin/google-ads/${encodeURIComponent(e.clientId)}" class="meta-hub-card-name">${esc(e.accountName)}</a>`:`<span class="meta-hub-card-name">${esc(e.accountName)}</span>`,s=[];return e.needsSetup?s.push(`<button type="button" class="admin-btn admin-btn--primary" data-google-ads-enable="${esc(e.googleCustomerId)}" data-google-ads-name="${esc(e.accountName)}">Enable</button>`):t&&s.push(`<a class="admin-btn" href="/admin/google-ads/${encodeURIComponent(e.clientId)}">Edit report</a>`),`
    <article class="meta-hub-card ${e.needsSetup?"is-setup":""}" data-google-ads-row="${esc(a)}">
      <div class="meta-hub-card-head">
        <span class="meta-hub-avatar" aria-hidden="true">${metaHubClientInitial(e.accountName)}</span>
        <div class="meta-hub-card-title">
          ${o}
      ...
      <div class="meta-hub-card-actions${s.length===1?" meta-hub-card-actions--single":""}">
        ${s.join("")}
      </div>
    </article>
  `}function renderGoogleAdsHubPage(e){const t=e.clients||[],a=e.google||{},n=a.partnerFetchError?`<div class="admin-banner admin-banner--warn">${esc(a.partnerFetchError)}</div>`:"";return`
    ...
    `)}
  `}function updateGoogleAdsHubDom(e){const t=document.getElementById("google-ads-banner"),a=document.getElementById("google-ads-cards"),n=document.getElementById("google-ads-count"),o=e.google||{};if(t&&(t.innerHTML=o.partnerFetchError?`<div class="admin-banner admin-banner--warn">${esc(o.partnerFetchError)}</div>`:""),n){const s=(e.clients||[]).length;n.textContent=`${s} account${s===1?"":"s"}`}a&&(a.innerHTML=(e.clients||[]).length?e.clients.map(renderGoogleAdsHubCard).join(""):'<div class="sync-history-empty" style="padding:24px">No Google Ads accounts found.</div>'),bindGoogleAdsHubRowEvents()}function bindGoogleAdsHubRowEvents(){document.querySelectorAll("[data-google-ads-enable]").forEach(e=>{e.onclick=async()=>{const t=e.getAttribute("data-google-ads-enable"),a=e.getAttribute("data-google-ads-name")||t;e.disabled=!0;try{const n=await adminFetch("/api/google-ads-reports/provision",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({googleCustomerId:t,accountName:a})});if(showToast(`Enabled ${a}`,"success"),n?.clientId){window.location.href=`/admin/google-ads/${encodeURIComponent(n.clientId)}`;return}await loadGoogleAdsHubPage()}catch(n){showToast(n.message||"Enable failed","error"),e.disabled=!1}}})}function bindGoogleAdsHubEvents(){const e=document.getElementById("google-ads-search");e&&(e.oninput=()=>{googleAdsReportsState.searchQuery=e.value,updateGoogleAdsHubDom(getGoogleAdsHubView(googleAdsReportsState.dashboardData,googleAdsReportsState.filter,googleAdsReportsState.searchQuery))}),document.querySelectorAll("[data-google-ads-filter]").forEach(a=>{a.onclick=()=>{googleAdsReportsState.filter=a.getAttribute("data-google-ads-filter")||"all",document.querySelectorAll("[data-google-ads-filter]").forEach(n=>{n.classList.toggle("is-active",n===a)}),updateGoogleAdsHubDom(getGoogleAdsHubView(googleAdsReportsState.dashboardData,googleAdsReportsState.filter,googleAdsReportsState.searchQuery))}});const t=document.getElementById("google-ads-refresh");t&&(t.onclick=()=>loadGoogleAdsHubPage()),bindGoogleAdsHubRowEvents()}async function loadGoogleAdsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
      ${renderBrandTopbar("")}
      ${wrapDashboardShell('<div class="sync-history-empty" style="padding:24px;text-align:center"><a class="admin-btn admin-btn--primary" href="/login?next='+encodeURIComponent(window.location.pathname)+'">Sign in</a></div>')}
    ...
    `);try{const a=await adminFetch("/api/google-ads-reports?filter=all");googleAdsReportsState.dashboardData=a;const n=getGoogleAdsHubView(a,googleAdsReportsState.filter,googleAdsReportsState.searchQuery);googleAdsReportsState.hubMounted?updateGoogleAdsHubDom(n):(e.innerHTML=renderGoogleAdsHubPage(n),googleAdsReportsState.hubMounted=!0,bindGoogleAdsHubEvents())}catch(a){e.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("google-ads"))}
      ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(a.message)}</div>`)}
    `}}function googleAdsMonthValue(e,t){if(!e)return 0;const a=t.split(".");let n=e;for(const o of a)n=n?.[o];return Number(n)||0}function renderGoogleAdsYearTable(e){const t=e.monthKeys||[],a=googleAdsReportsState.activeMonthKey;return`
    <div class="google-ads-table-wrap">
      <table class="google-ads-table">
        ...
        <tbody>${t.map(o=>{const s=e.months?.[o],r=s?.google||{},i=s?.topline||{},l=s?.bottomline||{};return`
      <tr class="${o===a?"is-active":""}" data-google-ads-month-row="${esc(o)}">
        <td>${esc(metaMonthLabel(o))}</td>
        <td>${metaFmtKr(r.budget)}</td>
        <td>${metaFmtNum(r.impressions)}</td>
        <td>${metaFmtNum(r.clicks)}</td>
        <td>${metaFmtKr(r.cpc)}</td>
        <td>${metaFmtNum(r.ctr,2)}%</td>
        <td>${metaFmtKr(r.spend)}</td>
        <td>${metaFmtNum(r.conversions,1)}</td>
        <td>${metaFmtNum(r.sales,1)}</td>
        <td>${metaFmtKr(r.conversionsValue)}</td>
        <td>${metaFmtKr(r.costPerLead||i.cpl)}</td>
        <td>${metaFmtKr(i.roasKr)}</td>
        <td>${metaFmtX(i.roasX)}</td>
        <td>${metaFmtKr(l.totalProfit)}</td>
        <td>${metaFmtKr(l.censioFee)}</td>
        <td>${metaFmtKr(l.poiKr)}</td>
      </tr>
    `}).join("")||'<tr><td colspan="16">No months yet.</td></tr>'}</tbody>
      </table>
    </div>
  `}function renderGoogleAdsControlPanel(e){const t=e.settings||{},a=googleAdsReportsState.activeMonthKey,n=e.months?.[a]||{},o=t.googleAdsReportFeeMode||(t.googleAdsReportFeeEnabled?"performance":"");return`
    <div class="meta-report-control-panel" id="google-ads-control-panel">
      <div class="google-ads-control-grid">
        <label class="meta-report-year-field">Year
          <select id="google-ads-year" class="admin-select">
            ${(e.years||[]).map(s=>`<option value="${s.year}"${Number(e.year)===Number(s.year)?" selected":""}>${s.year}</option>`).join("")}
          </select>
        </label>
        <label>Won leads
          <input type="number" class="admin-input" id="google-ads-won-leads" value="${n.inputs?.wonLeads??""}" min="0" step="1" />
        </label>
        <label>Avg lead value
          <input type="number" class="admin-input" id="google-ads-avg-lead-value" value="${n.inputs?.avgLeadValue??""}" min="0" step="0.01" />
        </label>
        <label>Avg profit per won
          <input type="number" class="admin-input" id="google-ads-avg-profit" value="${n.inputs?.avgProfitPerWon??""}" min="0" step="0.01" />
        </label>
      </div>
      <div class="hub-toolbar-actions" style="margin-top:12px;gap:12px;flex-wrap:wrap">
        <label class="meta-hub-switch">
          <span class="meta-hub-switch-label">Report</span>
          <span class="meta-hub-switch-track">
            <input type="checkbox" class="meta-hub-switch-input" id="google-ads-enabled" ${t.googleAdsReportEnabled?"checked":""} />
            <span class="meta-hub-switch-thumb"></span>
          </span>
        </label>
        <label class="meta-hub-switch">
          <span class="meta-hub-switch-label">Bottomline</span>
          <span class="meta-hub-switch-track">
            <input type="checkbox" class="meta-hub-switch-input" id="google-ads-bottomline" ${t.googleAdsReportShowBottomline?"checked":""} />
            <span class="meta-hub-switch-thumb"></span>
          </span>
        </label>
        <label>Fee
          <select id="google-ads-fee-mode" class="admin-select">
            <option value=""${o?"":" selected"}>Off</option>
            <option value="performance"${o==="performance"?" selected":""}>Performance %</option>
            <option value="marketing"${o==="marketing"?" selected":""}>Marketing amount</option>
          </select>
        </label>
        <label>Fee %
          <input type="number" class="admin-input" id="google-ads-fee-percent" value="${t.googleAdsReportFeePercent??20}" min="0" step="0.1" />
        </label>
        <label>Marketing fee
          <input type="number" class="admin-input" id="google-ads-marketing-fee" value="${t.googleAdsReportMarketingFeeAmount??0}" min="0" step="0.01" />
        </label>
        <button type="button" class="admin-btn admin-btn--secondary" id="google-ads-save-values">Save values</button>
        <button type="button" class="admin-btn admin-btn--primary" id="google-ads-refresh-month">${ICON_SYNC} Refresh month</button>
      </div>
    </div>
  `}function renderGoogleAdsClientPage(e){const t=e.monthKeys||[],a=googleAdsReportsState.activeMonthKey||t[t.length-1]||"",n=t.map(o=>`
    <button type="button" class="meta-report-tab${o===a?" is-active":""}" data-google-ads-month-tab="${esc(o)}">${esc(metaMonthLabel(o))}</button>
  `).join("");return`
    ${renderBrandTopbar(renderStaffAdminChrome("google-ads"))}
    ${wrapDashboardShell(`
    <div class="page-hero admin-hub-hero meta-premium-page-hero">
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      ${renderGoogleAdsControlPanel(e)}
      <div class="meta-report-tabs" role="tablist">${n}</div>
      <div class="google-ads-chart-wrap">
        <canvas id="google-ads-year-chart" height="110"></canvas>
      </div>
      <div id="google-ads-table-host" style="margin-top:18px">
        ${renderGoogleAdsYearTable(e)}
      </div>
    </div>
    `)}
  `}function mountGoogleAdsChart(e){const t=document.getElementById("google-ads-year-chart");if(!t||typeof Chart>"u")return;const a=e.monthKeys||[],n=a.map(s=>googleAdsMonthValue(e.months?.[s],"google.spend")),o=a.map(s=>googleAdsMonthValue(e.months?.[s],"google.conversions"));googleAdsReportsState.chart&&(googleAdsReportsState.chart.destroy(),googleAdsReportsState.chart=null),googleAdsReportsState.chart=new Chart(t,{type:"line",data:{labels:a.map(metaMonthLabel),datasets:[{label:"Spend",data:n,borderColor:"#ff6a00",backgroundColor:"rgba(255,106,0,0.12)",yAxisID:"y",tension:.3,fill:!0},{label:"Conversions",data:o,borderColor:"#0085f2",backgroundColor:"rgba(0,133,242,0.08)",yAxisID:"y1",tension:.3}]},options:{responsive:!0,interaction:{mode:"index",intersect:!1},scales:{y:{position:"left",ticks:{callback:s=>`Dkr ${s}`}},y1:{position:"right",grid:{drawOnChartArea:!1}}}}})}function bindGoogleAdsClientEvents(e){document.querySelectorAll("[data-google-ads-month-tab]").forEach(o=>{o.onclick=()=>{googleAdsReportsState.activeMonthKey=o.getAttribute("data-google-ads-month-tab"),dashboardReplaceGoogleAdsClient(googleAdsReportsState.clientPayload||e)}}),document.querySelectorAll("[data-google-ads-month-row]").forEach(o=>{o.onclick=()=>{googleAdsReportsState.activeMonthKey=o.getAttribute("data-google-ads-month-row"),dashboardReplaceGoogleAdsClient(googleAdsReportsState.clientPayload||e)}});const t=document.getElementById("google-ads-year");t&&(t.onchange=()=>{googleAdsReportsState.selectedYear=Number(t.value),googleAdsReportsState.activeMonthKey=null,loadGoogleAdsClientPage()});const a=document.getElementById("google-ads-save-values");a&&(a.onclick=()=>saveGoogleAdsMonthValues(e.clientId));const n=document.getElementById("google-ads-refresh-month");n&&(n.onclick=()=>refreshGoogleAdsMonth(e.clientId)),["google-ads-enabled","google-ads-bottomline","google-ads-fee-mode","google-ads-fee-percent","google-ads-marketing-fee"].forEach(o=>{const s=document.getElementById(o);s&&(s.onchange=()=>saveGoogleAdsClientSettings(e.clientId))})}function dashboardReplaceGoogleAdsClient(e){const t=document.getElementById("dashboard");!t||!e||(googleAdsReportsState.clientPayload=e,t.innerHTML=renderGoogleAdsClientPage(e),bindGoogleAdsClientEvents(e),mountGoogleAdsChart(e))}async function saveGoogleAdsMonthValues(e){const t=googleAdsReportsState.activeMonthKey;if(!(!e||!t))try{const a=await adminFetch(`/api/google-ads-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(t)}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({wonLeads:document.getElementById("google-ads-won-leads")?.value||null,avgLeadValue:document.getElementById("google-ads-avg-lead-value")?.value||null,avgProfitPerWon:document.getElementById("google-ads-avg-profit")?.value||null})});googleAdsReportsState.clientPayload=a,dashboardReplaceGoogleAdsClient(a),showToast("Values saved","success")}catch(a){showToast(a.message||"Save failed","error")}}async function refreshGoogleAdsMonth(e){const t=googleAdsReportsState.activeMonthKey;if(!(!e||!t))try{const a=await fetch(`/api/google-ads-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(t)}/refresh`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:"{}"}),n=await a.json().catch(()=>({}));if(fetch("http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c",{method:"POST",headers:{"Content-Type":"application/json","X-Debug-Session-Id":"f182bb"},body:JSON.stringify({sessionId:"f182bb",runId:"pre-fix",hypothesisId:"A",location:"index.html:refreshGoogleAdsMonth",message:"refresh month response",data:{ok:a.ok,status:a.status,clientId:String(e||""),monthKey:String(t||""),error:n.error||null,debug:n.debug||null,errorMeta:n.errorMeta||null},timestamp:Date.now()})}).catch(()=>{}),!a.ok)throw new Error(n.error||"Refresh failed");googleAdsReportsState.clientPayload=n,dashboardReplaceGoogleAdsClient(n),showToast("Google Ads month refreshed","success")}catch(a){showToast(a.message||"Refresh failed","error")}}async function saveGoogleAdsClientSettings(e){const t=document.getElementById("google-ads-fee-mode")?.value||null;try{const a=await adminFetch(`/api/google-ads-reports/clients/${encodeURIComponent(e)}/settings`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({googleAdsReportEnabled:!!document.getElementById("google-ads-enabled")?.checked,googleAdsReportShowBottomline:!!document.getElementById("google-ads-bottomline")?.checked,googleAdsReportFeeMode:t,googleAdsReportFeePercent:Number(document.getElementById("google-ads-fee-percent")?.value||20),googleAdsReportMarketingFeeAmount:Number(document.getElementById("google-ads-marketing-fee")?.value||0)})});googleAdsReportsState.clientPayload&&(googleAdsReportsState.clientPayload.settings={...googleAdsReportsState.clientPayload.settings,...a.settings||{}}),showToast("Settings saved","success"),await loadGoogleAdsClientPage()}catch(a){showToast(a.message||"Settings failed","error")}}async function loadGoogleAdsClientPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){window.location.href=`/login?next=${encodeURIComponent(window.location.pathname)}`;return}currentStaffUser=t;const a=CLIENT_SLUG;googleAdsReportsState.clientPageMounted||(e.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("google-ads"))}
      ${wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading Google Ads report...</p></div>')}
    `);try{const n=await adminFetch(`/api/google-ads-reports/clients/${encodeURIComponent(a)}?year=${encodeURIComponent(googleAdsReportsState.selectedYear)}`);googleAdsReportsState.selectedYear=n.year||googleAdsReportsState.selectedYear,(!googleAdsReportsState.activeMonthKey||!(n.monthKeys||[]).includes(googleAdsReportsState.activeMonthKey))&&(googleAdsReportsState.activeMonthKey=n.monthKeys?.[n.monthKeys.length-1]||null),googleAdsReportsState.clientPayload=n,googleAdsReportsState.clientPageMounted=!0,dashboardReplaceGoogleAdsClient(n)}catch(n){e.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("google-ads"))}
      ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(n.message)}</div>`)}
    `}}async function loadPublicMetaReportPage({silent:e=!1}={}){const t=document.getElementById("dashboard");if(!(!t||!REPORT_TOKEN)){if(!metaReportsState.publicPageMounted)t.innerHTML=wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading report...</p></div>');else if(!e){const a=document.getElementById("meta-report-month-panel");a&&a.classList.add("is-loading")}try{const a=await fetch(`/api/meta-reports/public/${encodeURIComponent(REPORT_TOKEN)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`).then(async n=>{const o=await n.json();if(!n.ok)throw new Error(o.error||"Report not found");return o});if(syncMetaReportSelectedYear(a,{disableUnavailable:!0}),Number(metaReportsState.selectedYear)!==Number(a.year))return loadPublicMetaReportPage({silent:!0});metaReportsState.activeMonthKey||(metaReportsState.activeMonthKey=a.monthKeys?.[a.monthKeys.length-1]||null),await ensureMetaReportScenarioSource(a,{editable:!1}),metaReportsState.publicPageMounted?updatePublicMetaReportContent(a):(metaReportsState.publicPayload=a,hydrateMetaReportSpendChartType(a),hydrateMetaReportScenarioSettings(a),t.innerHTML=renderPublicMetaReportPage(a),metaReportsState.publicPageMounted=!0,syncMetaReportPublicBranding(a),bindPublicMetaReportEvents(a),mountMetaReportCharts(a,{editable:!1}))}catch(a){if(!metaReportsState.publicPageMounted)t.innerHTML=wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(a.message||"Report not found")}</div>`);else{if(e)throw a;showToast(a.message||"Load failed","error")}}finally{const a=document.getElementById("meta-report-month-panel");a&&a.classList.remove("is-loading")}}}function metaCvShortMonth(e){const t=Number(String(e||"").slice(5,7));return["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][t-1]||e}function metaCvFormatUpdatedAt(e){if(!e)return"Not updated yet";const t=new Date(e);return Number.isNaN(t.getTime())?"Not updated yet":t.toLocaleString("en-GB",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}function getMetaCvFilteredClients(e=metaReportsState.customValues.overview){const t=e?.clients||[],a=String(metaReportsState.customValues.searchQuery||"").trim().toLowerCase();return a?t.filter(n=>[n.accountName,n.clientId,n.metaAdAccountId].filter(Boolean).join(" ").toLowerCase().includes(a)):t}function renderMetaCvMonthPills(e,t=[]){const a=metaReportsState.customValues.selectedClientId,n=metaReportsState.customValues.selectedMonthKey;return t.map(o=>{const r=(e.months?.[o]||{}).status||"empty",i=a===e.clientId&&n===o;return`
      <button
        type="button"
...
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!g)return;g.trim().toLowerCase()==="overwrite"&&(u=!1,p=!0)}const h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(c)}`,{method:"POST",body:JSON.stringify({skipManual:u,overwriteManual:p})});s?await s():r&&await r(h,null,{yearSync:!0}),showToast(`Synced ${h.synced?.length||0} month(s)${h.skipped?.length?`, skipped ${h.skipped.length}`:""}`,"success")}catch(c){showToast(c.message||"Year sync failed","error")}finally{o&&o(!1)}});const l=document.getElementById(`${t}-refresh-snapshot`);l&&(l.onclick=async()=>{const c=a?.();if(c){o&&o(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/refresh-snapshot`,{method:"POST",body:"{}"});r&&await r(d,c),showToast("Cenhub data refreshed and synced","success")}catch(d){showToast(d.message||"Refresh failed","error")}finally{o&&o(!1)}}})}function monthHasCenhubSnapshot(e,t={}){return e?resolveMetaToplineSource(e,t)==="ghl"?!0:!!e.ghlSyncedAt:!1}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),a=e.monthKeys||[];return t.length?t.map(n=>`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(E=>E.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const o=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,o);const s=t.editorPayload?.months?.[n]||null,r=a.metaReportShowBottomline,i=!!a.metaReportGhlDataEnabled,l={metaReportGhlDataEnabled:a.metaReportGhlDataEnabled,metaReportToplineMode:a.metaReportToplineMode},c=resolveClientToplineMode(l),d=resolveMetaCvMonthToplineSource(s,o,l),u=i&&c==="cenhub"&&d==="ghl",p=o.wonLeads!=null?o.wonLeads:s?.inputs?.wonLeads??s?.topline?.wonLeads??"",h=o.avgLeadValue!=null?o.avgLeadValue:s?.inputs?.avgLeadValue??s?.topline?.avgLeadValue??"",g=o.avgProfitPerWon!=null?o.avgProfitPerWon:s?.inputs?.avgProfitPerWon??"",b=o.ghlLeads??s?.ghlTotals?.leads??s?.topline?.leads??"",w=o.ghlTotalRevenue??s?.ghlTotals?.totalRevenue??"",m=o.ghlTotalProfit??s?.ghlTotals?.totalProfit??"",f=metaReportsState.customValues.draftInputs,v=i&&c==="cenhub"&&d==="manual",$=u?b:f?.manualLeads??(v?o.manualLeads??s?.topline?.leads??"":s?.topline?.leads??o.manualLeads??""),y=f?.wonLeads??p,S=f?.avgLeadValue??h,k=f?.avgProfitPerWon??g,L=u?' readonly disabled tabindex="-1" aria-readonly="true"':"",I=u?" disabled":"",C=u?" is-locked":"",B=t.settingsSaving&&t.settingsSavingScope==="report",A=i?renderMetaCvToplineSourceControl(a,e):"",M=renderMetaCvMonthManualRow(s||{dataSource:d},a),T=i?renderMetaMonthSourcePill(s||{dataSource:d},l):"";return`
    <div class="meta-report-edit-panel-head">
      <div>
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(E=>`
            <option value="${esc(E.clientId)}"${E.clientId===a.clientId?" selected":""}>${esc(E.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(E=>`
            <option value="${esc(E)}"${E===n?" selected":""}>${esc(metaMonthLabel(E))}</option>
          `).join("")}
        </select>
      </label>
      <label class="meta-cv-value-field${C}">Number of leads
        <input type="number" step="any" id="meta-cv-leads" value="${esc($)}"${L} />
      </label>
      <label class="meta-cv-value-field${C}">Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(y)}"${L} />
      </label>
      <label class="meta-cv-value-field${C}">Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(S)}"${L} />
      </label>
      <label class="meta-cv-value-field${C}">Avg profit per won${r?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(k)}"${L} />
      </label>
    </div>
    ${M}
    <div class="meta-report-settings-group meta-report-settings-group--stacked meta-cv-client-settings${B?" is-saving":""}">
      ${renderMetaReportSaveIndicator("meta-cv-settings-status",B)}
    ...
    <div class="date-picker-panel">
      <div class="date-picker-header">
        ...
        <div class="date-picker-nav">
          ...
          <button type="button" class="date-picker-nav-btn" aria-label="Next month" onclick="shiftDatePickerMonth(1)" ${y?"":"disabled"}>${ICON_CHEVRON_RIGHT}</button>
        </div>
      </div>
      <div class="date-picker-weekdays">
