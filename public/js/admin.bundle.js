...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),g=w.summary||w;l=g.runId,c=0,setFbLeadProgress({processed:0,total:g.inWindow||0,label:`Matching ${g.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:o})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const g=w.inWindow||c||$;a?(setFbLeadProgress({processed:c,total:g,complete:!w.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:g,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const v=d.summary||d,f=v.inWindow||c||$,y=v.nextBatchOffset??c;if(a)setFbLeadProgress({processed:y,total:f,complete:!0}),markFbLeadPreviewReady(e,t,v.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(v.updated||0)>0),showToast((v.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:y,total:f,written:v.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const b=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=b.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(v){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${v.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((v.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function renderMetaReportComparisonView(){const e=metaReportsState.clientPayload||metaReportsState.publicPayload,t=metaReportsState.comparisonResult||computeMetaReportComparison(e),{periodA:a,periodB:n}=metaReportComparisonPeriodsFromState(e),s=metaReportsState.comparisonTab||"table";return`
  ...
  `}function destroyMetaReportComparisonChart(){metaReportsState.comparisonChartInstance&&(metaReportsState.comparisonChartInstance.destroy(),metaReportsState.comparisonChartInstance=null)}function metaReportComparisonChartLabel(e,t){return t.find(a=>a.id===e)?.label||e}function buildMetaReportComparisonGroupedBarChart(e,t,a="kr"){const n=metaReportComparisonMetricDefs(t.hasBottomline),s=metaReportComparisonChartIds(t.hasBottomline,a),r=s.map(c=>metaReportComparisonChartLabel(c,n)),o=s.map(c=>metaReportParseAmount(t.periodA.metrics[c])),i=t.periodB.hasData!==!1,l=s.map(c=>i?metaReportParseAmount(t.periodB.metrics[c]):null);return new Chart(e,{type:"bar",data:{labels:r,datasets:[{label:t.periodA.label,data:o,backgroundColor:"rgba(255, 106, 0, 0.82)",borderColor:"#ff6a00",borderWidth:1,borderRadius:6,maxBarThickness:42},{label:t.periodB.label,data:l,backgroundColor:"rgba(148, 163, 184, 0.82)",borderColor:"#94a3b8",borderWidth:1,borderRadius:6,maxBarThickness:42}]},options:{responsive:!0,maintainAspectRatio:!1,layout:{padding:{left:8,right:12,top:0,bottom:0}},interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"top",labels:{boxWidth:12,font:{size:11}}},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,callbacks:{label(c){const d=c.parsed.y,u=s[c.dataIndex],p=n.find(h=>h.id===u);return`${c.dataset.label}: ${metaReportComparisonFmtValue(d,p?.format||"num")}`}}}},scales:{x:{grid:{display:!1},ticks:{color:"#6b5348",font:{size:11},maxRotation:45,minRotation:0}},y:{grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:c=>{const d=Number(c)||0;return Math.abs(d)>=1e6?`${(d/1e6).toFixed(1)}M`:Math.abs(d)>=1e3?`${Math.round(d/1e3)}K`:d}}}}}})}function metaReportComparisonYtdLineMetricId(e,t){return t==="x"?e?"poasX":"roasX":e?"poasKr":"totalLeadValue"}function buildMetaReportComparisonYtdLineChart(e,t,a="kr"){const n=t.ytdByMonth||[];if(!n.length)return null;const s=metaReportComparisonYtdLineMetricId(t.hasBottomline,a),o=metaReportComparisonMetricDefs(t.hasBottomline).find(u=>u.id===s),i=n.map(u=>u.label),l=n.map(u=>u.hasDataA&&u.periodA?metaReportParseAmount(u.periodA[s]):null),c=n.map(u=>u.hasDataB&&u.periodB?metaReportParseAmount(u.periodB[s]):null);return l.some(u=>u!=null)||c.some(u=>u!=null)?new Chart(e,{type:"line",data:{labels:i,datasets:[{label:t.periodA.label,data:l,borderColor:"#ff6a00",backgroundColor:"rgba(255, 106, 0, 0.08)",borderWidth:2,pointBackgroundColor:"#ff6a00",pointRadius:3,tension:.3,spanGaps:!1},{label:t.periodB.label,data:c,borderColor:"#94a3b8",backgroundColor:"rgba(148, 163, 184, 0.08)",borderWidth:2,pointBackgroundColor:"#94a3b8",pointRadius:3,tension:.3,spanGaps:!1}]},options:{responsive:!0,maintainAspectRatio:!1,layout:{padding:{left:8,right:12,top:0,bottom:0}},interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"top",labels:{boxWidth:12,font:{size:11}}},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,callbacks:{label(u){const p=u.parsed.y;return p==null?`${u.dataset.label}: \u2014`:`${u.dataset.label}: ${metaReportComparisonFmtValue(p,o?.format||"num")}`}}},title:{display:!0,text:o?.label||s,color:"#6b5348",font:{size:12,weight:"600"},padding:{bottom:8}}},scales:{x:{grid:{display:!1},ticks:{color:"#6b5348",font:{size:11}}},y:{grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:u=>{const p=Number(u)||0;return a==="x"?`${p.toFixed(2)}x`:Math.abs(p)>=1e6?`${(p/1e6).toFixed(1)}M`:Math.abs(p)>=1e3?`${Math.round(p/1e3)}K`:p}}}}}}):null}function buildMetaReportComparisonChart(e,t,a="kr"){const n=document.getElementById("meta-report-comparison-chart-empty");if(!t||t.insufficientData||t.samePeriod)return n&&(n.hidden=!1),null;let s=null;return t.mode==="ytd"?s=buildMetaReportComparisonYtdLineChart(e,t,a):s=buildMetaReportComparisonGroupedBarChart(e,t,a),n&&(n.hidden=!!s),s}function mountMetaReportComparisonChart(e){if(destroyMetaReportComparisonChart(),(metaReportsState.comparisonTab||"table")!=="chart")return;const t=document.getElementById("meta-report-comparison-chart");t&&(metaReportsState.comparisonChartInstance=buildMetaReportComparisonChart(t,e,metaReportsState.comparisonChartMode))}function setMetaReportComparisonTab(e){if(metaReportsState.comparisonTab=e==="chart"?"chart":"table",document.querySelectorAll("[data-comparison-tab]").forEach(t=>{const a=t.getAttribute("data-comparison-tab")===metaReportsState.comparisonTab;t.classList.toggle("is-active",a),t.setAttribute("aria-selected",a)}),document.querySelectorAll("[data-comparison-panel]").forEach(t=>{const a=t.getAttribute("data-comparison-panel")===metaReportsState.comparisonTab;t.classList.toggle("is-active",a),t.hidden=!a}),metaReportsState.comparisonTab==="chart"){const t=metaReportsState.clientPayload||metaReportsState.publicPayload;mountMetaReportComparisonChart(metaReportsState.comparisonResult||computeMetaReportComparison(t))}else destroyMetaReportComparisonChart()}function setMetaReportViewMode(e,t,{editable:a=!1}={}){metaReportsState.reportViewMode=e==="comparison"?"comparison":"monthly",document.querySelectorAll("[data-report-view-mode]").forEach(s=>{const r=s.getAttribute("data-report-view-mode")===metaReportsState.reportViewMode;s.classList.toggle("is-active",r),s.setAttribute("aria-selected",r)}),document.querySelectorAll("[data-report-view-panel]").forEach(s=>{const r=s.getAttribute("data-report-view-panel")===metaReportsState.reportViewMode;s.classList.toggle("is-active",r),s.hidden=!r});const n=document.querySelector("#meta-report-month-panel .meta-report-preview-card");if(n&&(n.hidden=metaReportsState.reportViewMode==="comparison"),metaReportsState.reportViewMode==="monthly")destroyMetaReportComparisonChart(),mountMetaReportTrendCharts(t,{editable:a,clientId:t?.clientId||null}),a&&syncMetaReportScenario(t);else if(destroyMetaReportTrendCharts(),t){const{series:s}=resolveMetaReportChartSeries(t);s.length&&(metaReportsState.chartSeries=s),bindMetaReportComparisonControls(t,{editable:a}),refreshMetaReportComparison(t,{editable:a}),a&&(bindMetaReportScenarioControls(t,{editable:!0}),syncMetaReportScenario(t))}}function bindMetaReportViewModeTabs(e,{editable:t=!1}={}){document.querySelectorAll("[data-report-view-mode]").forEach(a=>{a.onclick=()=>{const n=a.getAttribute("data-report-view-mode");!n||n===metaReportsState.reportViewMode||setMetaReportViewMode(n,e,{editable:t})}})}function syncMetaReportComparisonUi(e,{editable:t=!1}={}){if(!document.getElementById("meta-report-comparison-view"))return;const n=computeMetaReportComparison(e);metaReportsState.comparisonResult=n;const s=document.getElementById("meta-report-comparison-headline-wrap");s&&s.remove();const r=document.getElementById("meta-report-comparison-table-wrap");r&&(r.innerHTML=renderMetaReportComparisonTables(n));const o=document.getElementById("meta-report-comparison-chart-empty");o&&(o.hidden=!(n.insufficientData||n.samePeriod)),(metaReportsState.comparisonTab||"table")==="chart"&&mountMetaReportComparisonChart(n)}async function refreshMetaReportComparison(e,{editable:t=!1}={}){!e||!document.getElementById("meta-report-comparison-view")||(await ensureMetaReportComparisonYears(e,{editable:t}),syncMetaReportComparisonUi(e,{editable:t}))}function bindMetaReportComparisonControls(e,{editable:t=!1}={}){const a=()=>{document.querySelectorAll("[data-comparison-preset]").forEach(s=>{s.onclick=async()=>{if(s.disabled)return;const r=s.getAttribute("data-comparison-preset");if(!r)return;metaReportsState.comparisonMode=r,metaReportApplyComparisonPresetDefaults(e,r);const o=document.getElementById("meta-report-comparison-view");o&&(o.outerHTML=renderMetaReportComparisonView(),bindMetaReportComparisonControls(e,{editable:t})),await refreshMetaReportComparison(e,{editable:t})}})},n=()=>{document.querySelectorAll("[data-comparison-date]").forEach(s=>{s.onchange=async()=>{const r=s.getAttribute("data-comparison-date");if(!r||!s.value)return;const[o,i]=r.split("-"),l=`comparisonPeriod${o.toUpperCase()}`;metaReportsState[l]||(metaReportsState[l]={startDate:null,endDate:null});const c=metaReportsState[l];if(c[`${i}Date`]=s.value,c.startDate&&c.endDate&&c.startDate>c.endDate){const u=c.startDate;c.startDate=c.endDate,c.endDate=u}metaReportsState.comparisonMode="custom",await refreshMetaReportComparison(e,{editable:t});const d=document.getElementById("meta-report-comparison-range-wrap");if(d){const{periodA:u,periodB:p}=metaReportComparisonPeriodsFromState(e);d.innerHTML=renderMetaReportComparisonPeriodTiles(u,p,e),n()}document.querySelectorAll("[data-comparison-preset]").forEach(u=>{u.classList.toggle("active",u.getAttribute("data-comparison-preset")==="custom"),u.setAttribute("aria-pressed",u.getAttribute("data-comparison-preset")==="custom")})}})};document.querySelectorAll("[data-comparison-tab]").forEach(s=>{s.onclick=()=>{const r=s.getAttribute("data-comparison-tab");r&&setMetaReportComparisonTab(r)}}),document.querySelectorAll("#meta-report-comparison-chart-mode .meta-report-chart-toggle-btn").forEach(s=>{s.onclick=()=>{metaReportsState.comparisonChartMode=s.getAttribute("data-mode")||"kr",document.querySelectorAll("#meta-report-comparison-chart-mode .meta-report-chart-toggle-btn").forEach(r=>{r.classList.toggle("is-active",r===s)}),mountMetaReportComparisonChart(metaReportsState.comparisonResult||computeMetaReportComparison(e))}}),a(),n(),setMetaReportComparisonTab(metaReportsState.comparisonTab||"table")}function mountMetaReportComparison(e,{editable:t=!1}={}){document.getElementById("meta-report-comparison-view")&&(bindMetaReportComparisonControls(e,{editable:t}),refreshMetaReportComparison(e,{editable:t}))}function renderMetaReportScenarioCard(){return`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null}={}){const s=n||e?.monthKey||null,r=s?metaReportMonthBounds(s):null;let o="";t&&e&&!e.meta?.emptyMonth&&r&&(o=`
    ...
    `);const i=n||e?.monthKey||null,l=t?i?`Report preview \xB7 ${metaMonthLabel(i)}`:"Report preview":i?`Report \xB7 ${metaMonthLabel(i)}`:"Report",c=metaReportsState.reportViewMode||"monthly",d=shouldShowMetaReportYearVisuals(e,a,{editable:t}),u=c==="monthly"&&e&&!e.meta?.emptyMonth?`
  ...
  `:"",p=renderMetaReportMonthTables(e,{editable:t}),h=d?renderMetaReportChartsPanel({editable:t}):"",m=t&&d?renderMetaReportScenarioCard():"",$=d?renderMetaReportViewModeTabs():"";let v;return d?v=`
      ...
      ${m}
    `:v=`<div class="meta-report-content-main">${p}</div>`,`${o}${u}${$}${v}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),$=META_REPORT_FEE_MODE_META.map(({value:b,icon:w,title:g,desc:S})=>`
        ...
        </label>`).join(""),v=s?metaReportsState.clientReportSettingsExpanded:!0,f=s?`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,y=s&&!r?renderMetaReportBottomlineFeeSummary(e,t):"";return`
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?v?" is-editing":" is-collapsed":""}">
      ${y}
      <div class="meta-report-bottomline-editor${s&&!v?" is-hidden":""}" id="${t}-fee-editor">
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find($=>$.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map($=>`
            <option value="${esc($.clientId)}"${$.clientId===a.clientId?" selected":""}>${esc($.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map($=>`
            <option value="${esc($)}"${$===n?" selected":""}>${esc(metaMonthLabel($))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
