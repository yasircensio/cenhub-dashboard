...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),y=w.summary||w;l=y.runId,c=0,setFbLeadProgress({processed:0,total:y.inWindow||0,label:`Matching ${y.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:b||p?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:r})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const y=w.inWindow||c||b;a?(setFbLeadProgress({processed:c,total:y,complete:!w.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:y,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);m=!0;const g=d.summary||d,f=g.inWindow||c||b,v=g.nextBatchOffset??c;if(a)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,g.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(g.updated||0)>0),showToast((g.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:g.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const $=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=$.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(g){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${g.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((g.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!m&&a&&hideFbLeadProgress(!0),!a&&!m&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function renderMetaReportComparisonCard(){const e=metaReportsState.clientPayload||metaReportsState.publicPayload,t=metaReportsState.comparisonResult||computeMetaReportComparison(e),a=metaReportComparisonAvailableMonthKeys(e),{periodA:n,periodB:s}=metaReportComparisonPeriodsFromState(e),o=metaReportsState.comparisonMode==="ytd";return`
    <section class="meta-report-comparison-card" id="meta-report-comparison-card">
      ...
      <div id="meta-report-comparison-details-wrap">${renderMetaReportComparisonDetailTable(t)}</div>
    </section>
  `}function destroyMetaReportComparisonChart(){metaReportsState.comparisonChartInstance&&(metaReportsState.comparisonChartInstance.destroy(),metaReportsState.comparisonChartInstance=null)}function metaReportComparisonChartLabel(e,t){return t.find(a=>a.id===e)?.label||e}function buildMetaReportComparisonChart(e,t,a="kr",n="summary"){const s=document.getElementById("meta-report-comparison-chart-empty");if(!t||t.insufficientData||t.samePeriod)return s&&(s.hidden=!1),null;s&&(s.hidden=!0);const o=metaReportComparisonMetricDefs(t.hasBottomline);let r=[],i=[],l=[];if(n==="by_month"&&t.mode==="ytd"&&t.ytdByMonth.length){const c=metaReportsState.comparisonYtdMetric||"spend";r=t.ytdByMonth.map(d=>d.label),i=t.ytdByMonth.map(d=>metaReportParseAmount(d.periodA?.[c])),l=t.ytdByMonth.map(d=>metaReportParseAmount(d.periodB?.[c]))}else{const c=metaReportComparisonChartIds(t.hasBottomline,a);r=c.map(d=>metaReportComparisonChartLabel(d,o)),i=c.map(d=>metaReportParseAmount(t.periodA.metrics[d])),l=c.map(d=>metaReportParseAmount(t.periodB.metrics[d]))}return new Chart(e,{type:"bar",data:{labels:r,datasets:[{label:t.periodA.label,data:i,backgroundColor:"rgba(255, 106, 0, 0.82)",borderColor:"#ff6a00",borderWidth:1,borderRadius:6,maxBarThickness:42},{label:t.periodB.label,data:l,backgroundColor:"rgba(148, 163, 184, 0.82)",borderColor:"#94a3b8",borderWidth:1,borderRadius:6,maxBarThickness:42}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"top",labels:{boxWidth:12,font:{size:11}}},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,callbacks:{label(c){const d=c.parsed.y;if(n==="by_month"){const h=metaReportsState.comparisonYtdMetric||"spend",p=o.find(b=>b.id===h);return`${c.dataset.label}: ${metaReportComparisonFmtValue(d,p?.format||"num")}`}const u=metaReportComparisonChartIds(t.hasBottomline,a)[c.dataIndex],m=o.find(h=>h.id===u);return`${c.dataset.label}: ${metaReportComparisonFmtValue(d,m?.format||"num")}`}}}},scales:{x:{grid:{display:!1},ticks:{color:"#6b5348",font:{size:11},maxRotation:45,minRotation:0}},y:{grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:c=>{const d=Number(c)||0;return Math.abs(d)>=1e6?`${(d/1e6).toFixed(1)}M`:Math.abs(d)>=1e3?`${Math.round(d/1e3)}K`:d}}}}}})}function mountMetaReportComparisonChart(e){destroyMetaReportComparisonChart();const t=document.getElementById("meta-report-comparison-chart");t&&(metaReportsState.comparisonChartInstance=buildMetaReportComparisonChart(t,e,metaReportsState.comparisonChartMode,metaReportsState.comparisonView))}function syncMetaReportComparisonUi(e,{editable:t=!1}={}){if(!document.getElementById("meta-report-comparison-card"))return;const n=computeMetaReportComparison(e);metaReportsState.comparisonResult=n;const s=document.getElementById("meta-report-comparison-context-wrap");s&&(s.innerHTML=renderMetaReportComparisonContextStrip(n));const o=document.getElementById("meta-report-comparison-hero-wrap");o&&(o.innerHTML=renderMetaReportComparisonHeroCards(n));const r=document.getElementById("meta-report-comparison-details-wrap");r&&(r.innerHTML=renderMetaReportComparisonDetailTable(n));const i=document.getElementById("meta-report-comparison-chart-empty");i&&(i.hidden=!(n.insufficientData||n.samePeriod)),mountMetaReportComparisonChart(n)}async function refreshMetaReportComparison(e,{editable:t=!1}={}){!e||!document.getElementById("meta-report-comparison-card")||(await ensureMetaReportComparisonYears(e,{editable:t}),syncMetaReportComparisonUi(e,{editable:t}))}function bindMetaReportComparisonControls(e,{editable:t=!1}={}){const a=()=>{document.querySelectorAll("[data-comparison-preset]").forEach(s=>{s.onclick=async()=>{if(s.disabled)return;const o=s.getAttribute("data-comparison-preset");if(!o)return;metaReportsState.comparisonMode=o,o!=="ytd"&&(metaReportsState.comparisonView="summary");const r=document.getElementById("meta-report-comparison-presets"),i=document.getElementById("meta-report-comparison-range-row"),l=document.getElementById("meta-report-comparison-card");l&&(l.outerHTML=renderMetaReportComparisonCard(),bindMetaReportComparisonControls(e,{editable:t})),await refreshMetaReportComparison(e,{editable:t})}})},n=()=>{document.querySelectorAll("[data-comparison-range]").forEach(s=>{s.onchange=async()=>{const o=s.getAttribute("data-comparison-range");if(!o)return;const[r,i]=o.split("-");metaReportsState[`comparisonPeriod${r.toUpperCase()}`]||(metaReportsState[`comparisonPeriod${r.toUpperCase()}`]={startMonthKey:null,endMonthKey:null});const l=metaReportsState[`comparisonPeriod${r.toUpperCase()}`];if(l[`${i}MonthKey`]=s.value,l.startMonthKey&&l.endMonthKey&&l.startMonthKey>l.endMonthKey){const d=l.startMonthKey;l.startMonthKey=l.endMonthKey,l.endMonthKey=d}metaReportsState.comparisonMode="custom",await refreshMetaReportComparison(e,{editable:t});const c=document.getElementById("meta-report-comparison-range-row");if(c){const d=metaReportComparisonAvailableMonthKeys(e);c.innerHTML=`
            ${renderMetaReportComparisonRangeGroup("Current / primary range","is-a",metaReportsState.comparisonPeriodA,d,"a")}
            ${renderMetaReportComparisonRangeGroup("Comparison range","is-b",metaReportsState.comparisonPeriodB,d,"b")}
          `,n()}document.querySelectorAll("[data-comparison-preset]").forEach(d=>{d.classList.toggle("is-active",d.getAttribute("data-comparison-preset")==="custom"),d.setAttribute("aria-pressed",d.getAttribute("data-comparison-preset")==="custom")})}})};document.querySelectorAll("#meta-report-comparison-chart-mode .meta-report-chart-toggle-btn").forEach(s=>{s.onclick=()=>{metaReportsState.comparisonChartMode=s.getAttribute("data-mode")||"kr",document.querySelectorAll("#meta-report-comparison-chart-mode .meta-report-chart-toggle-btn").forEach(o=>{o.classList.toggle("is-active",o===s)}),mountMetaReportComparisonChart(metaReportsState.comparisonResult||computeMetaReportComparison(e))}}),document.querySelectorAll("[data-comparison-view]").forEach(s=>{s.onclick=()=>{metaReportsState.comparisonView=s.getAttribute("data-comparison-view")||"summary",document.querySelectorAll("[data-comparison-view]").forEach(o=>{o.classList.toggle("is-active",o===s)}),mountMetaReportComparisonChart(metaReportsState.comparisonResult||computeMetaReportComparison(e))}}),a(),n()}function mountMetaReportComparison(e,{editable:t=!1}={}){document.getElementById("meta-report-comparison-card")&&(bindMetaReportComparisonControls(e,{editable:t}),refreshMetaReportComparison(e,{editable:t}))}function renderMetaReportScenarioCard(){return`
