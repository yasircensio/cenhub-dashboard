...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),v=$.summary||$;l=v.runId,c=0,setFbLeadProgress({processed:0,total:v.inWindow||0,label:`Matching ${v.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:y||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:s})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const v=$.inWindow||c||y;a?(setFbLeadProgress({processed:c,total:v,complete:!$.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:v,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||y,w=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:w,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:w,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const g=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=g.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function metaReportComparisonMonthKeyFromDate(e){const t=metaReportComparisonParseDate(e);return t?t.slice(0,7):""}function metaReportComparisonMonthKeyBounds(e){const{min:t,max:a}=metaReportComparisonDateInputBounds(e);return{min:t?t.slice(0,7):"",max:a?a.slice(0,7):""}}function metaReportComparisonDateFromMonthKey(e,t){if(!e||!/^\d{4}-\d{2}$/.test(e))return null;const a=metaReportMonthBounds(e);return t==="start"?a.start:a.end}function renderMetaReportComparisonDateInput(e,t,a,n,o=null){const r=metaReportComparisonMonthKeyFromDate(a)||n.min||"";let s=n.min||"",i=n.max||"";if(t==="start"&&o?.endDate&&(i=metaReportComparisonMonthKeyFromDate(o.endDate)||i),t==="end"&&o?.startDate&&(s=metaReportComparisonMonthKeyFromDate(o.startDate)||s),s&&i&&s>i){const d=metaReportComparisonSafePickerBounds(s,i);s=d.min||s,i=d.max||i}const l=s?` min="${esc(s)}"`:"",c=i?` max="${esc(i)}"`:"";return`<input type="month" class="meta-report-comparison-month-input" data-comparison-date="${esc(e)}-${esc(t)}" value="${esc(r)}"${l}${c} aria-label="${t==="start"?"From month":"To month"}" />`}function renderMetaReportComparisonCustomRangeSide(e,t,a,n,o=null){const r=metaReportComparisonMonthKeyBounds(n);return`
    <div class="meta-report-comparison-duel-side ${e==="b"?"is-b":"is-a"}">
      <div class="meta-report-comparison-custom-range">
        <div class="meta-report-comparison-custom-range-field" data-comparison-range="${esc(e)}-start">
          ${renderMetaReportComparisonDateInput(e,"start",t,r,o)}
        </div>
        ...
        <div class="meta-report-comparison-custom-range-field" data-comparison-range="${esc(e)}-end">
          ${renderMetaReportComparisonDateInput(e,"end",a,r,o)}
        </div>
  ...
  `}function parseComparisonDateKey(e){const t=String(e||"").match(/^([ab])-(start|end)$/);return t?{side:t[1],bound:t[2],stateKey:t[1]==="b"?"comparisonPeriodB":"comparisonPeriodA"}:null}function metaReportComparisonSafePickerBounds(e,t){const a=e||void 0,n=t||void 0;return a&&n&&a>n?{min:n,max:a}:{min:a,max:n}}function applyMetaReportComparisonDateChange(e,t,{editable:a=!1,monthKey:n=null}={}){const o=parseComparisonDateKey(t.getAttribute("data-comparison-date")),r=n||t.value;if(!o||!r)return;const{side:s,bound:i,stateKey:l}=o,c=metaReportComparisonDateFromMonthKey(r,i);if(!c)return;metaReportsState[l]||(metaReportsState[l]={startDate:null,endDate:null});const d=metaReportsState[l];d[`${i}Date`]=c,fetch("http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c",{method:"POST",headers:{"Content-Type":"application/json","X-Debug-Session-Id":"b952dc"},body:JSON.stringify({sessionId:"b952dc",runId:"post-fix",hypothesisId:"A",location:"index.html:applyMetaReportComparisonDateChange",message:"month change applied",data:{side:s,bound:i,stateKey:l,monthKey:r,startDate:d.startDate,endDate:d.endDate},timestamp:Date.now()})}).catch(()=>{}),d.startDate&&d.endDate&&d.startDate>d.endDate&&(i==="end"?d.startDate=metaReportMonthBounds(r).start:d.endDate=metaReportMonthBounds(r).end);const u=document.querySelector(`[data-comparison-date="${s}-start"]`),p=document.querySelector(`[data-comparison-date="${s}-end"]`);u&&(u.value=metaReportComparisonMonthKeyFromDate(d.startDate)||""),p&&(p.value=metaReportComparisonMonthKeyFromDate(d.endDate)||""),metaReportsState.comparisonMode="custom",syncMetaReportComparisonPeriodPickerBounds(e),document.querySelectorAll("[data-comparison-preset]").forEach(h=>{const m=h.getAttribute("data-comparison-preset")==="custom";h.classList.toggle("active",m),h.setAttribute("aria-pressed",m)}),refreshMetaReportComparison(e,{editable:a})}function syncMetaReportComparisonPeriodPickerBounds(e){const t=metaReportComparisonMonthKeyBounds(e);["a","b"].forEach(a=>{const o=metaReportsState[a==="b"?"comparisonPeriodB":"comparisonPeriodA"]||{},r=document.querySelector(`[data-comparison-date="${a}-start"]`),s=document.querySelector(`[data-comparison-date="${a}-end"]`);if(r){t.min&&(r.min=t.min);const i=metaReportComparisonMonthKeyFromDate(o.endDate)||t.max||"";i&&(r.max=i),fetch("http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c",{method:"POST",headers:{"Content-Type":"application/json","X-Debug-Session-Id":"b952dc"},body:JSON.stringify({sessionId:"b952dc",runId:"post-fix",hypothesisId:"B",location:"index.html:syncMetaReportComparisonPeriodPickerBounds",message:"start month bounds synced",data:{side:a,boundsMin:t.min,boundsMax:t.max,periodStart:o.startDate,periodEnd:o.endDate,appliedMin:r.min,appliedMax:r.max},timestamp:Date.now()})}).catch(()=>{})}if(s){const i=metaReportComparisonMonthKeyFromDate(o.startDate)||t.min||"";i&&(s.min=i),t.max&&(s.max=t.max),fetch("http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c",{method:"POST",headers:{"Content-Type":"application/json","X-Debug-Session-Id":"b952dc"},body:JSON.stringify({sessionId:"b952dc",runId:"post-fix",hypothesisId:"B",location:"index.html:syncMetaReportComparisonPeriodPickerBounds",message:"end month bounds synced",data:{side:a,boundsMin:t.min,boundsMax:t.max,periodStart:o.startDate,periodEnd:o.endDate,appliedMin:s.min,appliedMax:s.max},timestamp:Date.now()})}).catch(()=>{})}})}function closeMetaReportComparisonDatePickers(e=null){(metaReportsState.comparisonDatePickers||[]).forEach(t=>{t!==e&&t.isOpen&&t.close()})}function destroyMetaReportComparisonDatePickers(){closeMetaReportComparisonDatePickers(),document.querySelectorAll("[data-comparison-date]").forEach(e=>{e._metaComparisonDatePicker&&(e._metaComparisonDatePicker.destroy(),e._metaComparisonDatePicker=null)}),metaReportsState.comparisonDatePickers=[]}function bindMetaReportComparisonDateInputs(e,{editable:t=!1}={}){destroyMetaReportComparisonDatePickers(),syncMetaReportComparisonPeriodPickerBounds(e),document.querySelectorAll("[data-comparison-date]").forEach(a=>{a.onchange=()=>{fetch("http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c",{method:"POST",headers:{"Content-Type":"application/json","X-Debug-Session-Id":"b952dc"},body:JSON.stringify({sessionId:"b952dc",runId:"post-fix",hypothesisId:"C",location:"index.html:monthInput.onchange",message:"month input changed",data:{inputKey:a.getAttribute("data-comparison-date"),value:a.value,min:a.min,max:a.max},timestamp:Date.now()})}).catch(()=>{}),applyMetaReportComparisonDateChange(e,a,{editable:t})},a.onfocus=()=>{syncMetaReportComparisonPeriodPickerBounds(e)}})}function metaReportComparisonYtdSpanLabel(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n)return"";const o=Number(a.slice(5,7)),r=Number(n.slice(5,7)),s=a.slice(0,4),i=n.slice(0,4);return s!==i?metaReportComparisonFormatDateRangeLabel(a,n):o===1&&r===12?"Full year":o===r?`Through ${META_MONTH_LABELS[o-1]}`:`${META_MONTH_LABELS[o-1]} \u2013 ${META_MONTH_LABELS[r-1]}`}function metaReportComparisonPresetPeriodDisplay(e,t){const a=metaReportComparisonNormalizePeriod(t);if(!a)return{value:"\u2014",hint:""};if(e==="mom"||e==="months"){const n=a.startMonthKey||a.endMonthKey,o=String(n||a.startDate).slice(0,4);return{value:`${metaMonthLabel(n)} ${o}`,hint:""}}if(e==="ytd"){const n=metaReportComparisonYtdSpanLabel(a.startDate,a.endDate);return{value:a.startDate.slice(0,4),hint:n&&n!=="Full year"?n:""}}return{value:metaReportComparisonFormatDateRangeLabel(a.startDate,a.endDate),hint:""}}function renderMetaReportComparisonDuelSide(e,t,a=!1){const n=t.hint?`<span class="meta-report-comparison-duel-hint">${esc(t.hint)}</span>`:"",o=a?" is-partial":"",r=a?' title="Partial data"':"";return`
