(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){if(t[1]==="sync-history"&&(t[2]==="ghl"||t[2]==="meta")){document.body.dataset.dashboardMode=`sync-history-${t[2]}`,delete document.body.dataset.clientSlug;return}if(t[1]==="fb-lead-sync"){document.body.dataset.dashboardMode="fb-lead-sync",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="custom-values"){document.body.dataset.dashboardMode="meta-reports-custom",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]){document.body.dataset.dashboardMode="meta-reports-client",document.body.dataset.clientSlug=t[2];return}if(t[1]==="meta-reports"){document.body.dataset.dashboardMode="meta-reports",delete document.body.dataset.clientSlug;return}document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}if(t[0]==="report"&&t[1]){document.body.dataset.dashboardMode="report",document.body.dataset.reportToken=t[1],delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_ADMIN_SYNC_HISTORY_GHL=DASHBOARD_MODE==="sync-history-ghl",IS_ADMIN_SYNC_HISTORY_META=DASHBOARD_MODE==="sync-history-meta",IS_ADMIN_FB_LEAD_SYNC=DASHBOARD_MODE==="fb-lead-sync",IS_ADMIN_META_REPORTS=DASHBOARD_MODE==="meta-reports",IS_ADMIN_META_REPORTS_CLIENT=DASHBOARD_MODE==="meta-reports-client",IS_ADMIN_META_REPORTS_CUSTOM=DASHBOARD_MODE==="meta-reports-custom",IS_REPORT_VIEW=DASHBOARD_MODE==="report",REPORT_TOKEN=document.body.dataset.reportToken||"",IS_ADMIN_SYNC_HISTORY=IS_ADMIN_SYNC_HISTORY_GHL||IS_ADMIN_SYNC_HISTORY_META,IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);return e[0]==="admin"&&e[1]==="meta-reports"&&e[2]&&e[2]!=="custom-values"?e[2]:e[0]==="admin"&&e[1]&&e[1]!=="meta-reports"?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"&&e[0]!=="report"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){const t=!!(e&&e.includes("staff-admin-chrome")),n=t||e&&e.includes("admin-topbar-link")?"/admin":"",a=n?`<a href="${n}" class="brand-logo-link" aria-label="Cenhub home"><img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" /></a>`:'<img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" />',s=t?`<button type="button" class="staff-nav-toggle" id="staff-nav-toggle" aria-expanded="false" aria-controls="staff-topbar-panel" aria-label="Open menu">
  ...
  `}async function createStaffUserFromForm(){const e=document.getElementById("new-staff-email")?.value?.trim(),t=document.getElementById("new-staff-name")?.value?.trim(),n=document.getElementById("new-staff-role")?.value==="admin"?"admin":"member";if(!e){showToast("Email is required.","error");return}try{const a=await adminFetch("/api/auth/users",{method:"POST",body:JSON.stringify({email:e,name:t,role:n})});a.setupUrl&&await copyTextToClipboard(a.setupUrl,"Invite link copied to clipboard."),IS_TEAM_PAGE&&await reloadTeamUsersTable(),showToast("Staff member invited.","success")}catch(a){showToast(a.message,"error")}}async function updateStaffUser(e,t){try{await adminFetch(`/api/auth/users/${encodeURIComponent(e)}`,{method:"PATCH",body:JSON.stringify(t)}),IS_TEAM_PAGE&&await reloadTeamUsersTable(),showToast("User updated.","success")}catch(n){showToast(n.message,"error"),IS_TEAM_PAGE&&await reloadTeamUsersTable()}}async function updateStaffUserRole(e,t){await updateStaffUser(e,{role:t==="admin"?"admin":"member"})}async function deleteStaffUser(e){if(window.confirm("Delete this staff member? This permanently removes their account and cannot be undone."))try{await adminFetch(`/api/auth/users/${encodeURIComponent(e)}`,{method:"DELETE"}),IS_TEAM_PAGE&&await reloadTeamUsersTable(),showToast("Staff member deleted.","success")}catch(t){showToast(t.message,"error")}}async function copyStaffInviteLink(e){try{const t=await adminFetch(`/api/auth/users/${encodeURIComponent(e)}/reset-password`,{method:"POST",body:"{}"});t.setupUrl&&await copyTextToClipboard(t.setupUrl,"Invite link copied to clipboard.")}catch(t){showToast(t.message,"error")}}async function resetStaffUserPassword(e,t=!1){try{const n=await adminFetch(`/api/auth/users/${encodeURIComponent(e)}/reset-password`,{method:"POST",body:"{}"});n.setupUrl&&await copyTextToClipboard(n.setupUrl,t?"Reset link copied to clipboard.":"Password reset link generated.")}catch(n){showToast(n.message,"error")}}function renderLoginPage(){const e=new URLSearchParams(window.location.search),t=e.get("token"),n=e.get("next")||"/admin",a=e.get("saved")==="1",s=document.getElementById("dashboard"),i='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/></svg>',o='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.94 10.94 0 0112 19c-6.5 0-10-7-10-7a20.3 20.3 0 014.06-5.94M9.9 4.24A10.94 10.94 0 0112 5c6.5 0 10 7 10 7a20.3 20.3 0 01-3.17 4.49"/><path d="M1 1l22 22"/><path d="M14.12 14.12A3 3 0 009.88 9.88"/></svg>',l='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>';function r(d,c,m=""){return`
        ...
        <div class="auth-card">
          <div class="auth-card-header">
            <div class="auth-card-title-row">
              <div class="auth-card-icon" aria-hidden="true">${l}</div>
          ...
          ${r("set-password","New password",`autocomplete="new-password" minlength="8" placeholder="At least 8 characters" oninput="updatePasswordStrength()" onkeydown="if(event.key==='Enter')submitSetPassword()"`)}
          <div id="password-strength" class="password-strength" aria-live="polite"></div>
          ${r("set-password-confirm","Confirm password",`autocomplete="new-password" minlength="8" placeholder="Repeat your password" onkeydown="if(event.key==='Enter')submitSetPassword()"`)}
    ...
    <div class="auth-page">
      <div class="auth-card">
        ...
        ${r("login-password","Password",`autocomplete="current-password" placeholder="Enter your password" onkeydown="if(event.key==='Enter')submitStaffLogin()"`)}
  ...
  `}function renderSyncHistoryRows(e,t){if(!e.length)return'<div class="sync-history-empty">No sync runs logged yet.</div>';const n=t==="meta"?"Spend / details":"Details",a=e.map(s=>{let i="\u2014";if(t==="meta"){const o=[];if(s.thisMonthSpend!=null&&o.push(`${fmtDkk(s.thisMonthSpend)} DKK this month`),s.spendDateStop){const l=formatSyncHistoryDate(s.spendDateStop);l&&o.push(`through ${l}`)}s.errorMessage&&o.push(s.errorMessage),i=o.join(" \xB7 ")||"\u2014"}else s.opportunityCount!=null?(i=`${s.opportunityCount} opportunities`,s.errorMessage&&(i+=` \xB7 ${s.errorMessage}`)):s.errorMessage&&(i=s.errorMessage);return`
      <tr>
...
${o} contact(s) will get Fb Lead id written in GHL. This writes live data.`)&&(fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:!0,previewRunId:n},await runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!1}))}async function refreshFbLeadClientRowsUi(){const e=document.getElementById("fb-lead-clients-mount"),t=document.getElementById("fb-lead-field-warnings-mount");e&&(e.innerHTML=renderFbLeadClientRows(fbLeadSyncState.clients)),t&&(t.innerHTML=renderFbLeadFieldWarnings(fbLeadSyncState.clients)),bindFbLeadSyncToggles()}function renderFbLeadClientActions(e){const t=esc(e.clientId),n=e.metaPageId?"":`<a class="admin-btn admin-btn--ghost admin-btn--small" href="/admin/${encodeURIComponent(e.clientId)}">Setup</a>`;return`
  ...
  `}function lastRunIsSuccessfulApply(e){if(!e||e.dryRun)return!1;const t=String(e.status||"").toLowerCase();return t==="success"||t==="ok"}function lastRunIsSuccessfulPreview(e){if(!e||!e.dryRun)return!1;const t=String(e.status||"").toLowerCase();return t==="success"||t==="ok"}function resolveFbLeadClientStats(e,t={}){const n=e?.displayStats||{},a=t.metaLeadCount90d??n.metaLeadCount90d??null,s=t.estimatedMissing??n.outstanding??null,i=t.estimatedMissing!=null?"Estimated contacts still missing Fb Lead id (from a Meta sample)":n.outstandingHint||"Estimated contacts still missing Fb Lead id (from a Meta sample)";return{metaLeads:a!=null?String(a):"\u2014",missing:s!=null?String(s):"\u2014",missingTitle:i}}function formatFbLeadLastRunLabel(e){if(!e)return"\u2014";const t=[e.dryRun?"preview":e.status,`${e.updated||0} ${e.dryRun?"would update":"updated"}`];return(e.skippedNoMatch??0)>0&&t.push(`${e.skippedNoMatch} no match`),t.join(" \xB7 ")}function renderFbLeadClientRows(e){return e.length?`
    <div class="sync-history-table-wrap">
  ...
  `:'<div class="sync-history-empty">No clients configured yet.</div>'}function renderFbLeadClientRowsWrapper(e){return`<div id="fb-lead-clients-mount">${renderFbLeadClientRows(e)}</div>`}function renderFbLeadHistoryRows(e,t={}){const n=getFbLeadHistoryRunsForDisplay(e);if(!n.length){const s=fbLeadSyncState.showRoutineCronRuns?0:(e||[]).filter(i=>isRoutineSuccessfulCronRun(i)).length;return s>0?`<div class="sync-history-empty">${s} routine auto-sync run(s) hidden \u2014 enable <strong>Show routine auto-sync runs</strong> above to view them.</div>`:'<div class="sync-history-empty">No FB lead sync runs logged yet.</div>'}return e=n,`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table">
        ...
        <tbody>${e.map(s=>{const i=[s.mode,s.dryRun?"dry-run":"apply",s.dryRun?`${s.updated||0} would update`:`${s.updated||0} updated`,`${s.skippedNoMatch||0} no match`,s.errors?`${s.errors} errors`:null].filter(Boolean).join(" \xB7 "),o=dryRunIsReadyForApply(s,e)&&!fbLeadSyncState.isApplying,l=isPartialSyncRun(s)&&!fbLeadSyncState.isApplying,r=fbLeadSyncState.isApplying&&fbLeadSyncState.applyingRunId===s.id,d=renderSyncStatusBadge(r?"applying":s.status);return`
      <tr class="fb-lead-audit-row${r?" is-applying":""}">
        <td>${esc(formatSyncHistoryTimestamp(s.startedAt))}</td>
        <td>${esc(s.accountName||s.clientId)}</td>
        <td>${d}</td>
        ...
        <td>
          <div class="fb-lead-history-actions">
            ${r?'<span class="fb-lead-history-status-note">In progress above</span>':l?`<button class="admin-btn admin-btn--primary admin-btn--small" type="button" onclick="resumeFbLeadRunFromHistory(${Number(s.id)})">Resume (${partialSyncRemaining(s)})</button>`:o?`<button class="admin-btn admin-btn--primary admin-btn--small" type="button" data-fb-apply-run="${Number(s.id)}" data-apply-count="${Number(s.updated||0)}" onclick="applyFbLeadSyncFromHistory(${Number(s.id)})">Apply (${s.updated})</button>`:""}
  ...
  `}function renderFbLeadAuditTable(e,t){if(!e.length)return'<div class="sync-history-empty" style="padding:12px">No contact-level rows stored for this run.</div>';const n=e.map(s=>{const i=ghlContactUrl(t,s.contactId),o=i?`<a href="${esc(i)}" target="_blank" rel="noopener noreferrer">Open in GHL</a>`:"\u2014";return`
      <tr>
  ...
  `}function closeFbLeadRunPanel(){const e=document.getElementById("fb-lead-run-panel-mount");e&&(e.innerHTML=""),fbLeadSyncState.activeRun=null}function openFbLeadRunPanel(e,t){const n=fbLeadSyncState.clients.find(i=>i.clientId===e);if(!clientFbLeadFieldReady(n)){showToast(getClientFbLeadFieldHint(n),"error");return}const a=document.getElementById("fb-lead-run-panel-mount");if(!a)return;fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:clientHasReadyPreview(fbLeadSyncState.clients.find(i=>i.clientId===e))},a.innerHTML=renderFbLeadRunPanel(e,t);const s=document.getElementById("fb-lead-preview-btn");s&&(s.onclick=()=>runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!0})),a.scrollIntoView({behavior:"smooth",block:"nearest"})}function renderFbLeadRunResults(e){const t=e.rows||[];if(!t.length)return'<div class="sync-history-empty" style="padding:12px">No rows in this batch.</div>';const n=t.map(i=>`
    <tr>
      <td>${esc(i.email||i.phone||"\u2014")}</td>
      <td><code>${esc(i.metaLeadId||"\u2014")}</code></td>
      <td>${esc(i.contactId||"\u2014")}</td>
      <td>${renderSyncStatusBadge(i.status)}</td>
    </tr>
  ...
  `}function renderGlobalApplyProgress(e,t,n=0){const a=fbLeadSyncState.clients.find(o=>o.clientId===e),s=t==="backfill"?"90-day backfill":"recent sync",i=n>0?`${n} contact(s)`:"contacts";return`
    <div class="fb-lead-apply-progress-card" id="fb-lead-global-progress-card">
      <div class="fb-lead-apply-progress-header">
        <div>
          ...
          <div class="fb-lead-apply-progress-sub">${esc(a?.accountName||e)} \xB7 ${esc(i)}</div>
        </div>
  ...
  `}function startGlobalApplyProgress(e,t,n=0){const a=document.getElementById("fb-lead-apply-progress-mount");a&&(fbLeadSyncState.isApplying=!0,fbLeadSyncState.applyStartedAt=Date.now(),fbLeadSyncState.applyProgress=null,a.innerHTML=renderGlobalApplyProgress(e,t,n),a.scrollIntoView({behavior:"smooth",block:"nearest"}),fbLeadSyncState.applyWaitTimer&&clearInterval(fbLeadSyncState.applyWaitTimer),fbLeadSyncState.applyWaitTimer=window.setInterval(()=>{if(!fbLeadSyncState.isApplying)return;const s=Math.floor((Date.now()-fbLeadSyncState.applyStartedAt)/1e3),i=fbLeadSyncState.applyProgress;i?.total>0?updateGlobalApplyProgress({...i,waiting:!0,elapsed:s}):updateGlobalApplyProgress({waiting:!0,elapsed:s})},1e3))}function updateGlobalApplyProgress({processed:e,total:t,written:n,waiting:a=!1,elapsed:s=0,complete:i=!1,error:o=""}={}){const l=document.getElementById("fb-lead-global-progress-card"),r=document.getElementById("fb-lead-global-progress"),d=document.getElementById("fb-lead-global-progress-bar"),c=document.getElementById("fb-lead-global-progress-label"),m=document.getElementById("fb-lead-global-progress-stats"),g=document.getElementById("fb-lead-apply-status-badge");if(!l||!r||!d||!c)return;const $=fbLeadSyncState.applyProgress||{},b=e??$.processed??0,w=t??$.total??0,S=n??$.written??0;!i&&!o&&(w>0||b>0)&&(fbLeadSyncState.applyProgress={processed:b,total:w,written:S});const h=w>0||b>0;if(a&&!h&&!i){r.classList.add("is-indeterminate"),d.style.width="35%",m&&(m.textContent="Contacting Meta and GHL\u2026"),c.textContent=`Writing to GHL\u2026 (${s}s \u2014 do not close this page)`;return}r.classList.remove("is-indeterminate");const f=w>0?Math.min(100,Math.round(b/w*100)):i?100:0;d.style.width=`${f}%`,m&&(m.textContent=i?`${S} written to GHL \xB7 ${b} / ${w} contacts processed`:`${S} written \xB7 ${b} / ${w} contacts (${f}%)`),i?(l.classList.add("is-complete"),l.classList.remove("is-error"),c.textContent=`Complete \u2014 ${S} contact(s) updated in GHL`,g&&(g.textContent="Complete",g.classList.add("is-ok"),g.classList.remove("is-missing"))):o?(l.classList.add("is-error"),l.classList.remove("is-complete"),c.textContent=o,g&&(g.textContent="Failed",g.classList.remove("is-ok"),g.classList.add("is-missing"))):a&&h?c.textContent=b>=w?`Finishing up\u2026 (${s}s)`:`Batch in progress \u2014 ${b} / ${w} leads (${f}%) \xB7 ${s}s`:c.textContent=`Writing to GHL \u2014 ${b} / ${w} leads (${f}%)`}function stopGlobalApplyProgress({success:e=!0,keepVisibleMs:t=6e3}={}){fbLeadSyncState.isApplying=!1,fbLeadSyncState.applyProgress=null,fbLeadSyncState.applyWaitTimer&&(clearInterval(fbLeadSyncState.applyWaitTimer),fbLeadSyncState.applyWaitTimer=null),window.setTimeout(()=>{const n=document.getElementById("fb-lead-apply-progress-mount");n&&!fbLeadSyncState.isApplying&&(n.innerHTML="")},t)}function setFbLeadProgress({processed:e=0,total:t=0,complete:n=!1,label:a=""}={}){const s=document.getElementById("fb-lead-progress-wrap"),i=document.getElementById("fb-lead-progress-bar"),o=document.getElementById("fb-lead-progress-label");if(!s||!i)return;s.hidden=!1,s.classList.toggle("is-complete",n);const l=t>0?Math.min(100,Math.round(e/t*100)):n?100:0;i.style.width=`${l}%`,o&&(o.textContent=a||(n?`Complete \u2014 ${e} / ${t} leads (${l}%)`:`Processing \u2014 ${e} / ${t} leads (${l}%)`))}function hideFbLeadProgress(e=!0){const t=document.getElementById("fb-lead-progress-wrap"),n=document.getElementById("fb-lead-progress-bar");t&&e&&(t.hidden=!0,t.classList.remove("is-complete"),n&&(n.style.width="0%"))}function updateFbLeadHistoryClearButton(){const e=document.getElementById("fb-lead-history-clear-all");if(!e)return;const t=(fbLeadSyncState.historyRuns||[]).length>0;e.disabled=!t}function updateFbLeadCronSummaryUi(){const e=document.getElementById("fb-lead-cron-summary-24h");!e||!fbLeadSyncState.cronSummary24h||(e.outerHTML=renderFbLeadCronSummary24h(fbLeadSyncState.cronSummary24h))}async function refreshFbLeadHistoryUi(){await loadFbLeadHistory();const e=document.getElementById("fb-lead-history-mount");if(!e)return;const t=Object.fromEntries(fbLeadSyncState.clients.map(n=>[n.clientId,n.locationId]));e.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,t),updateFbLeadHistoryClearButton()}function bindFbLeadHistoryClearButton(){const e=document.getElementById("fb-lead-history-clear-all");e&&(e.onclick=()=>clearFbLeadSyncHistory());const t=document.getElementById("fb-lead-show-routine-cron");t&&(t.checked=fbLeadSyncState.showRoutineCronRuns,t.onchange=()=>{fbLeadSyncState.showRoutineCronRuns=t.checked;const n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,a)}),updateFbLeadHistoryClearButton()}async function clearFbLeadSyncHistory(){if(!(!isStaffAdmin()||!window.confirm(`Delete all FB lead sync run history?

This only clears the log \u2014 it does not change GHL contacts or Meta data.`)))try{const t=await adminFetch("/api/fb-lead-sync/history",{method:"DELETE"});showToast(`Cleared ${t.deleted||0} run(s).`,"success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(t){showToast(t.message||"Failed to clear FB lead sync log.","error")}}async function deleteFbLeadRun(e){if(!(!isStaffAdmin()||!window.confirm(`Delete run #${e} from history?`)))try{await adminFetch(`/api/fb-lead-sync/history/${e}`,{method:"DELETE"}),showToast("Run deleted.","success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(n){showToast(n.message||"Failed to delete run.","error")}}async function runFbLeadSyncBatch({clientId:e,mode:t,dryRun:n,resumeRunId:a=null}){const s=document.getElementById("fb-lead-preview-btn"),i=document.getElementById("fb-lead-run-results");s&&(s.disabled=!0);const o=n?12:10,l=a!=null?fbLeadSyncState.historyRuns.find(S=>Number(S.id)===Number(a)):null;let r=l?.id||fbLeadSyncState.activeRun?.runId||null,d=l&&Number(l.batchOffset)||0,c=null,m=[],g=!1;const $=a??fbLeadSyncState.activeRun?.previewRunId??null,b=$!=null?fbLeadSyncState.historyRuns.find(S=>Number(S.id)===Number($)):null,w=l?partialSyncRemaining(l):b?.updated??fbLeadSyncState.previewOkByClient[e]?.updated??0;n||(fbLeadSyncState.applyingRunId=$,startGlobalApplyProgress(e,t,w),await refreshFbLeadHistoryUi());try{if(!a){const p=findPartialSyncRun(e,t,n);if(p){const u=partialSyncRemaining(p),y=n?"preview":"apply";window.confirm(`A ${y} for ${e} was interrupted (${p.updated||0} processed, ${u} remaining).

Resume the interrupted ${y}?`)&&(r=p.id,d=Number(p.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const p=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:r})}),u=p.summary||p;r=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||b?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:r,previewRunId:n?null:$,batchOffset:d,batchLimit:o})});const p=c.summary||c;r=p.runId,d=p.nextBatchOffset??p.batchOffset+(p.batchProcessed||0),m=m.concat(p.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=r);const u=p.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:u,complete:!p.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...p,rows:m}))):updateGlobalApplyProgress({processed:d,total:u,written:p.updated||0,complete:!p.hasMore})}while((c.summary||c).hasMore);g=!0;const S=c.summary||c,h=S.inWindow||d||w,f=S.nextBatchOffset??d;if(n)setFbLeadProgress({processed:f,total:h,complete:!0}),markFbLeadPreviewReady(e,t,S.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(S.updated||0)>0),showToast((S.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:f,total:h,written:S.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const p=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete p[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(p))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(S){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${S.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((S.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!g&&n&&hideFbLeadProgress(!0),!n&&!g&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
        ${renderBrandTopbar("")}
        ${wrapDashboardShell(`
  ...
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function metaReportFullUrl(e){return e?`${window.location.origin}${e}`:""}function normalizeMetaReportSlugInput(e){return String(e||"").trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function resolveMetaReportShareSlug(e){const t=e.settings||{};if(t.metaReportSlug)return t.metaReportSlug;const a=String(e.reportUrl||"").replace(/^\/report\//,"").match(/^(.+)-(\d{4})$/);return a?a[1]:""}function resolveMetaReportShareCode(e){const n=String(e.reportUrl||"").replace(/^\/report\//,"").match(/-(\d{4})$/);return n?n[1]:""}function renderMetaReportShareSummary(e,{hidden:t=!1}={}){const a=!!(e.settings||{}).metaReportEnabled,s=resolveMetaReportShareSlug(e),i=resolveMetaReportShareCode(e),o=s?`${s}-${i||"\xB7\xB7\xB7\xB7"}`:"Not set";return`
  ...
  `}function syncMetaReportToolbarCopy(e){const t=document.getElementById("meta-report-toolbar-copy-link");t&&(e.reportUrl?(t.setAttribute("data-copy-report-url",e.reportUrl),t.disabled=!e.settings?.metaReportEnabled,t.hidden=!1):t.hidden=!0)}function applyMetaReportEnabledUi(e,t){e.settings||(e.settings={}),e.settings.metaReportEnabled=t,document.getElementById("meta-report-share-card")?.classList.toggle("is-disabled",!t);const n=document.getElementById("meta-report-share-summary");n&&n.querySelectorAll(".meta-report-share-summary-row").forEach(r=>{if(r.querySelector(".meta-report-share-summary-label")?.textContent!=="Preview")return;const d=r.querySelector(".meta-report-share-summary-value");d&&(d.textContent=t?"Enabled":"Disabled",d.classList.toggle("is-on",t),d.classList.toggle("is-off",!t))});const a=document.querySelector("[data-meta-client-preview]");a&&(a.checked=t);const s=a?.closest(".meta-hub-switch")?.querySelector(".meta-hub-switch-label");s&&(s.textContent=t?"Enabled":"Disabled");const i=document.getElementById("meta-report-rotate-token");i&&(i.disabled=!t);const o=document.querySelector(".meta-report-share-panel"),l=o?.querySelector(".meta-report-share-note");if(t)l&&l.remove();else if(!l&&o){const r=document.createElement("p");r.className="meta-report-share-note",r.textContent="Preview disabled \u2014 link won\u2019t load.",o.appendChild(r)}syncMetaReportToolbarCopy(e)}function updateMetaReportHubCardReportState(e,t){const n=document.querySelector(`[data-meta-hub-row="${e}"]`);if(!n)return;const a=n.querySelector(".meta-report-badge");a&&!a.classList.contains("meta-report-badge--setup")&&(a.className=t?"meta-report-badge meta-report-badge--on":"meta-report-badge meta-report-badge--off",a.textContent=t?"Report live":"Report off"),n.querySelectorAll("[data-meta-bottomline], [data-meta-fee]").forEach(s=>{s.disabled=!t,s.closest(".meta-hub-switch")?.classList.toggle("is-disabled",!t)})}function updateMetaReportShareSummary(e){const t=document.getElementById("meta-report-share-summary");if(!t)return;const n=document.createElement("div");n.innerHTML=renderMetaReportShareSummary(e).trim();const a=n.firstElementChild;if(!a)return;a.hidden=metaReportsState.clientShareExpanded,t.replaceWith(a);const s=document.getElementById("meta-report-share-edit");s&&(s.onclick=()=>setMetaReportShareExpanded(!0))}function setMetaReportShareExpanded(e){metaReportsState.clientShareExpanded=!!e;const t=document.getElementById("meta-report-share-card"),n=document.getElementById("meta-report-share-summary"),a=document.getElementById("meta-report-share-editor");t?.classList.toggle("is-editing",e),n&&(n.hidden=e),a&&(a.hidden=!e)}function renderMetaReportShareCard(e){const n=!!(e.settings||{}).metaReportEnabled,a=e.reportUrl||null,s=metaReportFullUrl(a),i=resolveMetaReportShareSlug(e),o=resolveMetaReportShareCode(e),l=metaReportsState.clientShareExpanded,r=!!a;return`
    <div class="meta-report-share-card${n?"":" is-disabled"}${l?" is-editing":""}" id="meta-report-share-card">
      <div class="meta-report-share-card-head">
        <span class="meta-report-settings-card-title">Slug setting</span>
      </div>
      ${r?renderMetaReportShareSummary(e,{hidden:l}):`
        <p class="meta-report-share-empty">Share link will appear once this client has a Meta ad account configured.</p>
      `}
      ${r?`
      <div class="meta-report-share-editor" id="meta-report-share-editor"${l?"":" hidden"}>
        <div class="meta-report-share-panel">
          ...
          <div class="meta-report-share-field">
            <div class="meta-report-share-slug-row">
              ...
              <input type="text" id="meta-report-share-slug" class="admin-input meta-report-share-slug-input" value="${esc(i)}" autocomplete="off" spellcheck="false" aria-label="Report slug" />
  ...
  `}function refreshMetaReportClientShareUi(e){syncMetaReportToolbarCopy(e),updateMetaReportShareSummary(e);const t=document.getElementById("meta-report-share-card");t&&t.classList.toggle("is-disabled",!e.settings?.metaReportEnabled);const n=document.getElementById("meta-report-share-url");n&&e.reportUrl&&(n.textContent=metaReportFullUrl(e.reportUrl));const a=document.getElementById("meta-report-share-slug-suffix"),s=resolveMetaReportShareCode(e);a&&(a.textContent=`-${s||"\xB7\xB7\xB7\xB7"}`);const i=document.getElementById("meta-report-share-slug");i&&(i.value=resolveMetaReportShareSlug(e));const o=document.getElementById("meta-report-rotate-token");o&&(o.disabled=!e.settings?.metaReportEnabled);const l=document.querySelector("[data-meta-client-preview]");l&&(l.checked=!!e.settings?.metaReportEnabled),setMetaReportShareExpanded(metaReportsState.clientShareExpanded)}function syncMetaReportShareCard(e){const t=document.getElementById("meta-report-share-card");if(!t)return;const n=t.parentElement;if(!n)return;const a=renderMetaReportShareCard(e),s=document.createElement("div");s.innerHTML=a.trim();const i=s.firstElementChild;i&&(n.replaceChild(i,t),setMetaReportShareExpanded(metaReportsState.clientShareExpanded),bindMetaReportShareEvents(e),syncMetaReportToolbarCopy(e))}function bindMetaReportRotateButton(e){const t=document.getElementById("meta-report-rotate-token");t&&(t.onclick=async()=>{if(window.confirm("Rotate the share link? The old link will stop working.")){t.disabled=!0;try{const n=await patchMetaReportSettings(e,{rotateAccessToken:!0}),a=n.settings?.reportUrl||null,s=metaReportsState.clientPayload;s&&(s.reportUrl=a,n.settings&&(s.settings={...s.settings,...n.settings}));const i=s||{clientId:e,reportUrl:a,settings:n.settings||{}};metaReportsState.clientPayload=i,metaReportsState.clientShareExpanded?refreshMetaReportClientShareUi(i):syncMetaReportShareCard(i),showToast("Share link rotated","success")}catch(n){showToast(n.message||"Rotate failed","error")}finally{const n=document.getElementById("meta-report-rotate-token");n&&(n.disabled=!1)}}})}function bindMetaReportShareEvents(e){const t=e.clientId;bindMetaReportCopyButtons(),bindMetaReportRotateButton(t);const n=document.getElementById("meta-report-share-edit");n&&(n.onclick=()=>setMetaReportShareExpanded(!0));const a=document.getElementById("meta-report-share-done");a&&(a.onclick=()=>setMetaReportShareExpanded(!1));const s=document.getElementById("meta-report-share-save");s&&(s.onclick=async()=>{const l=document.getElementById("meta-report-share-slug"),r=normalizeMetaReportSlugInput(l?.value);if(!r||r.length<2){showToast("Enter a valid slug (at least 2 characters).","error");return}s.disabled=!0;try{const c=(await patchMetaReportSettings(t,{metaReportSlug:r})).settings||{},m=metaReportsState.clientPayload||e;m.settings={...m.settings,...c},m.reportUrl=c.reportUrl||null,metaReportsState.clientPayload=m,refreshMetaReportClientShareUi(m),setMetaReportShareExpanded(!1),showToast("Share link slug saved","success")}catch(d){showToast(d.message||"Save failed","error")}finally{const d=document.getElementById("meta-report-share-save");d&&(d.disabled=!1)}});const i=document.getElementById("meta-report-share-slug");i&&(i.oninput=()=>{i.value=normalizeMetaReportSlugInput(i.value)});const o=document.querySelector("[data-meta-client-preview]");if(o){let l=0;o.onchange=async()=>{const r=++l,d=o.checked,c=!d,m=metaReportsState.clientPayload||e;applyMetaReportEnabledUi(m,d);const g=o.closest(".meta-hub-switch");g?.classList.add("is-saving");try{const $=await patchMetaReportSettings(t,{metaReportEnabled:d},{fast:!0});if(r!==l)return;const b=$.settings||{};m.settings={...m.settings,...b},m.reportUrl=b.reportUrl??m.reportUrl,metaReportsState.clientPayload=m,applyMetaReportEnabledUi(m,!!b.metaReportEnabled),showToast(d?"Client preview enabled":"Client preview disabled","success")}catch($){if(r!==l)return;applyMetaReportEnabledUi(m,c),o.checked=c,showToast($.message||"Update failed","error")}finally{r===l&&g?.classList.remove("is-saving")}}}}function renderMetaReportHubCard(e){const t=!!e.clientId,n=e.clientId||e.metaAdAccountId||"",a=e.needsSetup?"is-setup":"",s=e.needsSetup?'<span class="meta-report-badge meta-report-badge--setup">Needs setup</span>':e.metaReportEnabled?'<span class="meta-report-badge meta-report-badge--on">Report live</span>':'<span class="meta-report-badge meta-report-badge--off">Report off</span>',i=e.hasGhl?"GHL + Meta":"Meta only",o=t?`<a href="/admin/meta-reports/${encodeURIComponent(e.clientId)}" class="meta-hub-card-name">${esc(e.accountName)}</a>`:`<span class="meta-hub-card-name">${esc(e.accountName)}</span>`,l=[];e.needsSetup?l.push(`<button type="button" class="admin-btn admin-btn--primary" data-meta-provision="${esc(e.metaAdAccountId)}" data-meta-provision-name="${esc(e.accountName)}">Add client</button>`):t&&(l.push(`<a class="admin-btn" href="/admin/meta-reports/${encodeURIComponent(e.clientId)}">Edit report</a>`),l.push(e.reportUrl?`<button type="button" class="admin-btn admin-btn--secondary" data-copy-report-url="${esc(e.reportUrl)}">Copy link</button>`:`<a class="admin-btn admin-btn--secondary" href="/admin/meta-reports/${encodeURIComponent(e.clientId)}">Enable report</a>`));const r=l.length===1?"meta-hub-card-actions meta-hub-card-actions--single":"meta-hub-card-actions";return`
    <article class="meta-hub-card ${a}" data-meta-hub-row="${esc(n)}">
      ...
      ${l.length?`<div class="${r}">${l.join("")}</div>`:""}
    </article>
  ...
  `).join("")}function getMetaReportCalendarYear(e){const t=Number(e?.currentYear);return Number.isFinite(t)&&t>=2e3?t:new Date().getFullYear()}function getMetaReportYearOptions(e){const t=getMetaReportCalendarYear(e),n=t-1,a=new Map;Array.isArray(e?.years)&&e.years.forEach(i=>{const o=Number(i?.year);Number.isFinite(o)&&a.set(o,i.available!==!1)});const s=a.has(n)?a.get(n):e?.previousYearHasData===!0;return[{year:t,available:!0},{year:n,available:!!s}]}function renderMetaReportYearSelectOptions(e,{disableUnavailable:t=!1}={}){const n=Number(metaReportsState.selectedYear)||Number(e?.year);return getMetaReportYearOptions(e).map(({year:a,available:s})=>{const i=t&&!s;return`<option value="${a}"${Number(n)===a?" selected":""}${i?" disabled":""}>${a}${s?"":" (no data)"}</option>`}).join("")}function syncMetaReportSelectedYear(e,{disableUnavailable:t=!1}={}){const n=getMetaReportYearOptions(e),a=Number(metaReportsState.selectedYear),s=n.find(r=>r.year===a);if(s&&(!t||s.available)){metaReportsState.selectedYear=s.year;return}const i=Number(e?.year),o=n.find(r=>r.year===i);if(o&&(!t||o.available)){metaReportsState.selectedYear=o.year;return}const l=n.find(r=>r.available)||n[0];l&&(metaReportsState.selectedYear=l.year)}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;if(!t||!metaReportsState.activeMonthKey)return;t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e;const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e,getMetaReportMonthBodyOptions(t.settings,{editable:!0})),bindMetaReportsClientEditEvents(t.clientId,t),refreshMetaReportBackfillControl(t))}function refreshMetaReportBackfillControl(e){const t=document.querySelector(".meta-report-client-page .meta-report-toolbar-left"),n=document.querySelector(".meta-report-backfill-wrap"),a=metaReportMonthsNeedingBackfill(e);if(!a.length){n&&n.remove();return}const s=`${ICON_SYNC} Backfill ${a.length} month${a.length===1?"":"s"} from Meta`,i=document.getElementById("meta-report-backfill");if(i)i.innerHTML=s,i.disabled=!1;else if(t){const o=document.createElement("div");o.className="meta-report-backfill-wrap",o.innerHTML=`
    ...
    <div class="meta-report-highlight-strip">
      ${n.map(({label:a,value:s,tone:i})=>`
        <div class="meta-report-highlight-item${i&&i!=="neutral"?` is-${i}`:""}">
          <div class="meta-report-highlight-label">${esc(a)}</div>
          <div class="meta-report-highlight-value">${esc(s)}</div>
        </div>
      `).join("")}
    </div>
  `}function renderMetaReportMetricTable(e,t,n="meta",{badge:a="",highlightLastN:s=0}={}){const i=t.map(([o,l,r],d)=>`
      <tr class="${s>0&&d>=t.length-s?`is-highlight accent-${n}`:""}">
        <th scope="row">${esc(o)}</th>
        <td>${esc(String(l??"\u2014"))}${r?`<span class="meta-report-row-unit">${esc(r)}</span>`:""}</td>
      </tr>
    `).join("");return`
  ...
  `,r=i?`
    <div class="meta-report-tables-card">
      ...
      <div class="meta-report-groups-stack">${i}</div>
    </div>
  `:"";return`${n}${l}${r}`}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
    ...
    `)}
  `}function renderMetaReportsClientPage(e){const t=e.monthKeys||[],n=metaReportsState.activeMonthKey||t[t.length-1]||"",a=e.months?.[n]||null,s=e.settings||{},i=t.map(o=>`
  ...
  `).join("");return`
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      <div class="meta-report-toolbar">
        ...
        <div class="meta-report-toolbar-actions">
          ${e.reportUrl?`<button type="button" class="admin-btn admin-btn--secondary" id="meta-report-toolbar-copy-link" data-copy-report-url="${esc(e.reportUrl)}"${s.metaReportEnabled?"":" disabled"}>Copy link</button>`:""}
        </div>
      </div>
      ${renderMetaReportShareCard(e)}
      <div class="meta-report-settings${metaReportsState.clientReportSettingsExpanded?"":" is-collapsed"}">
        ...
        <div class="meta-report-settings-group meta-report-settings-group--stacked">
          ${renderMetaReportBottomlineFeeSettings(s,"meta-report",{compact:!1,collapsible:!0})}
        </div>
      </div>
      <div class="meta-report-tabs" role="tablist">${i}</div>
    ...
    `)}
  `}function renderPublicMetaReportPage(e){const t=e.monthKeys||[],n=metaReportsState.activeMonthKey||t[t.length-1]||"",a=e.months?.[n]||null,s=t.map(i=>`
  ...
  `}function setMetaReportYearLoadingBanner(e,t,n){const a=document.querySelector(e);if(!a)return;let s=a.querySelector(".meta-report-year-loading-banner");n?(s||(s=document.createElement("span"),s.className="meta-report-year-loading-banner",s.setAttribute("aria-live","polite"),a.appendChild(s)),s.innerHTML=`<span class="meta-report-save-indicator-spinner"></span> Loading ${esc(t)}\u2026`,s.hidden=!1):s&&(s.hidden=!0)}function setMetaReportClientYearLoading(e,t){const n=document.querySelector(".meta-report-client-page"),a=document.getElementById("meta-report-year");if(t){a&&(a.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!0);const s=document.querySelector(".meta-report-tabs");s&&(s.innerHTML=renderMetaReportTabsSkeleton(),s.setAttribute("aria-busy","true"));const i=document.getElementById("meta-report-month-panel");i&&(i.innerHTML=renderMetaReportMonthPanelSkeleton(),i.classList.add("is-loading"))}else{a&&(a.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!1);const s=document.querySelector(".meta-report-tabs");s&&s.removeAttribute("aria-busy");const i=document.getElementById("meta-report-month-panel");i&&i.classList.remove("is-loading")}}function setMetaCvYearLoading(e,t){const n=document.querySelector(".sync-history-page.meta-reports-page"),a=document.getElementById("meta-cv-year"),s=document.getElementById("meta-cv-search");if(t){a&&(a.disabled=!0),s&&(s.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!0);const i=document.querySelector(".meta-cv-summary");i&&(i.classList.add("is-skeleton"),i.innerHTML=renderMetaCvSummarySkeleton());const o=document.querySelector(".meta-cv-legend");o&&o.setAttribute("aria-hidden","true");const l=document.getElementById("meta-cv-client-list");l&&(l.innerHTML=renderMetaCvListSkeleton(),l.setAttribute("aria-busy","true"));const r=document.getElementById("meta-cv-editor");r&&(r.innerHTML=renderMetaCvEditorYearSkeleton(),r.setAttribute("aria-busy","true"))}else{a&&(a.disabled=!1),s&&(s.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!1);const i=document.querySelector(".meta-cv-summary");i&&i.classList.remove("is-skeleton");const o=document.querySelector(".meta-cv-legend");o&&o.removeAttribute("aria-hidden");const l=document.getElementById("meta-cv-client-list");l&&l.removeAttribute("aria-busy");const r=document.getElementById("meta-cv-editor");r&&r.removeAttribute("aria-busy")}}function setMetaReportPublicYearLoading(e,t){const n=document.querySelector(".meta-reports-page--public"),a=document.getElementById("meta-report-year");if(t){a&&(a.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!0);const s=document.querySelector(".meta-reports-page--public .meta-report-tabs");s&&(s.innerHTML=renderMetaReportTabsSkeleton(),s.setAttribute("aria-busy","true"));const i=document.getElementById("meta-report-month-panel");i&&(i.innerHTML=renderMetaReportMonthPanelSkeleton(),i.classList.add("is-loading"))}else{a&&(a.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!1);const s=document.querySelector(".meta-reports-page--public .meta-report-tabs");s&&s.removeAttribute("aria-busy");const i=document.getElementById("meta-report-month-panel");i&&i.classList.remove("is-loading")}}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,n=document.getElementById("meta-report-year");n&&(n.onchange=async()=>{if(n.selectedOptions?.[0]?.disabled){n.value=String(metaReportsState.selectedYear);return}const s=metaReportsState.selectedYear,i=Number(n.value);if(i!==s){metaReportsState.selectedYear=i,metaReportsState.activeMonthKey=null,setMetaReportClientYearLoading(i,!0);try{await loadMetaReportsClientPage({silent:!0})}catch(o){metaReportsState.selectedYear=s,n&&(n.value=String(s)),await loadMetaReportsClientPage({silent:!0}),showToast(o.message||"Load failed","error")}finally{setMetaReportClientYearLoading(i,!1)}}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report"),bindMetaReportShareEvents(e)}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const n=document.getElementById("meta-report-refresh-meta");n&&(n.onclick=async()=>{n.disabled=!0;try{const s=metaReportsState.activeMonthKey,i=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(i.monthPayload),showToast("Meta data refreshed","success")}catch(s){showToast(s.message||"Meta refresh failed","error")}finally{n.disabled=!1}});const a=document.getElementById("meta-report-save-month");a&&(a.onclick=async()=>{a.disabled=!0;try{const s=metaReportsState.activeMonthKey,i={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:Number(document.getElementById("meta-report-avg-profit")?.value)},o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}`,{method:"PUT",body:JSON.stringify(i)});refreshMetaReportMonthPanel(o.monthPayload),showToast("Month saved","success")}catch(s){showToast(s.message||"Save failed","error")}finally{a.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{metaReportsState.activeMonthKey=t.getAttribute("data-meta-month-tab");const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(e.settings,{editable:!1}))),document.querySelectorAll("[data-meta-month-tab]").forEach(a=>{a.classList.toggle("is-active",a.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}const a=metaReportsState.selectedYear,s=Number(t.value);if(s!==a){metaReportsState.selectedYear=s,metaReportsState.activeMonthKey=null,setMetaReportPublicYearLoading(s,!0);try{await loadPublicMetaReportPage({silent:!0})}catch(i){metaReportsState.selectedYear=a,t&&(t.value=String(a)),await loadPublicMetaReportPage({silent:!0}),showToast(i.message||"Load failed","error")}finally{setMetaReportPublicYearLoading(s,!1)}}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
      ${renderBrandTopbar("")}
    ...
    `).join(""):'<div class="meta-report-empty" style="padding:32px 12px">No matching clients.</div>'}function monthStatusIsEmpty(e,t={}){return e?(e.status??classifyMetaCvMonthStatus({wonLeads:e.inputs?.wonLeads??e.topline?.wonLeads??e.wonLeads,avgLeadValue:e.inputs?.avgLeadValue??e.topline?.avgLeadValue??e.avgLeadValue,avgProfitPerWon:e.inputs?.avgProfitPerWon??e.avgProfitPerWon},!!t.metaReportShowBottomline))==="empty":!0}function metaReportEffectiveFeeEnabled(e,t=null){if(!t)return!!e?.metaReportFeeEnabled;const n=!!e?.metaReportShowBottomline;return(t.status??classifyMetaCvMonthStatus({wonLeads:t.inputs?.wonLeads??t.topline?.wonLeads??t.wonLeads,avgLeadValue:t.inputs?.avgLeadValue??t.topline?.avgLeadValue??t.avgLeadValue,avgProfitPerWon:t.inputs?.avgProfitPerWon??t.avgProfitPerWon},n))==="empty"?!1:!!e?.metaReportFeeEnabled}function resolveMetaReportFeeMode(e={}){return e.metaReportFeeEnabled?resolveMetaReportStoredFeeMode(e):""}const META_REPORT_FEE_MODE_META=[{value:"",icon:"\u2013",title:"None",desc:"No Censio fee"},{value:"performance",icon:"%",title:"Performance fee",desc:"% of profit (POAS)"},{value:"marketing",icon:"Dkr",title:"Marketing fee",desc:"Fixed Dkr amount"}];function metaReportFeeValueBlockHtml(e,t,n,a,{inline:s=!1}={}){return s&&t==="performance"?`
      ...
      </label>`:'<p class="meta-report-fee-value-empty">No amount needed \u2014 just save your selection.</p>'}function metaReportUsesInlineFeeValue(e){return e==="meta-cv"}function describeMetaReportFeeModeLabel(e,t,n,{feeEnabled:a=!0}={}){if(!a||e===""||e==null)return"No Censio fee";if(e==="performance"){const s=Number(t);return`Performance fee \xB7 ${Number.isFinite(s)?s:20}%`}if(e==="marketing"){const s=Number(n);return`Marketing fee \xB7 Dkr ${Number.isFinite(s)?s:0}`}return"No Censio fee"}function readMetaReportFeeDraftState(e){const t=document.getElementById(`${e}-fee-nested`);if(!t)return null;const n=t.querySelector(`input[name="${e}-fee-mode"]:checked`),a=n?n.value||"":t.dataset.feeMode||"";let s=Number(t.dataset.feePercent),i=Number(t.dataset.marketingFee);if(a==="performance"){const m=document.getElementById(`${e}-setting-fee-percent`),g=Number(m?.value);Number.isFinite(g)&&(s=g)}else if(a==="marketing"){const m=document.getElementById(`${e}-setting-marketing-fee`),g=Number(m?.value);Number.isFinite(g)&&(i=g)}const o=t.dataset.feeEnabled==="true"?resolveMetaReportStoredFeeMode({metaReportFeeEnabled:!0,metaReportFeeMode:t.dataset.feeMode||null,metaReportMarketingFeeAmount:t.dataset.marketingFee}):"",l=Number(t.dataset.feePercent),r=Number(t.dataset.marketingFee);return{appliedMode:o,appliedPercent:l,appliedMarketing:r,selectedMode:a,selectedPercent:s,selectedMarketing:i,isDirty:a!==o||(a==="performance"?s!==l:a==="marketing"?i!==r:!1),appliedLabel:describeMetaReportFeeModeLabel(o,l,r),selectedLabel:describeMetaReportFeeModeLabel(a,s,i)}}function metaReportFeeStatusHtml(e){return e.isDirty?`
    ...
    <div class="meta-report-bottomline-summary" id="${t}-fee-summary">
      <div class="meta-report-bottomline-summary-content">
        <div class="meta-report-bottomline-summary-row">
          ...
          <span class="meta-report-bottomline-summary-value${n?" is-on":""}">${n?"Enabled":"Off"}</span>
        </div>
        ${n?`
        <div class="meta-report-bottomline-summary-row">
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t;const n=document.querySelector(".meta-report-bottomline-fee-settings--client"),a=document.querySelector(".meta-report-client-page .meta-report-settings"),s=document.getElementById(`${e}-fee-editor`);n?.classList.toggle("is-editing",t),n?.classList.toggle("is-collapsed",!t),a?.classList.toggle("is-collapsed",!t),s?.classList.toggle("is-hidden",!t)}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const n=document.getElementById(`${t}-fee-summary`);if(!n)return;const a=document.createElement("div");a.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const s=a.firstElementChild;s&&n.replaceWith(s);const i=document.getElementById(`${t}-fee-edit`);i&&(i.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:n=!1,variant:a="default",collapsible:s=!1}={}){const i=!!e.metaReportShowBottomline,o=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",r=resolveMetaReportFeeMode(e),d=e.metaReportFeePercent??20,c=e.metaReportMarketingFeeAmount??0,m=metaReportUsesInlineFeeValue(t),g=`data-fee-mode="${esc(l)}" data-fee-enabled="${o?"true":"false"}" data-fee-percent="${esc(d)}" data-marketing-fee="${esc(c)}"`;if(a==="cv"){const f=describeMetaReportFeeModeLabel(r,d,c),v=META_REPORT_FEE_MODE_META.map(({value:p,title:u})=>`
        <label class="meta-report-fee-mode-pill">
          <input type="radio" name="${t}-fee-mode" value="${esc(p)}"${r===p?" checked":""} />
          <span>${esc(u)}</span>
        </label>`).join("");return`
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${i?"":" is-hidden"}" id="${t}-fee-nested" ${g}>
          ...
          <div class="meta-cv-fee-footer">
            <div class="meta-cv-fee-value-inline" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,r,d,c,{inline:!0})}</div>
            <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save" disabled>Saved</button>
          </div>
        </div>
      </div>
    </div>
  `}const $=describeMetaReportFeeModeLabel(r,d,c),b=META_REPORT_FEE_MODE_META.map(({value:f,icon:v,title:p,desc:u})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(f)}"${r===f?" checked":""} />
        ...
        </label>`).join(""),w=s?metaReportsState.clientReportSettingsExpanded:!0,S=s?`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,h=s?renderMetaReportBottomlineFeeSummary(e,t):"";return`
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?w?" is-editing":" is-collapsed":""}">
      ...
      <div class="meta-report-bottomline-editor${s&&!w?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${i?"":" is-hidden"}" id="${t}-fee-nested" ${g}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
          ...
          <div class="meta-report-fee-value" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,r,d,c,{inline:m})}</div>
          ${S}
        </div>
      </div>
    </div>
  `}function syncMetaReportBottomlineFeeDom(e={},t="meta-report"){const n=!!e.metaReportShowBottomline,a=!!e.metaReportFeeEnabled,s=e.metaReportFeeMode||"",i=resolveMetaReportFeeMode(e),o=e.metaReportFeePercent??20,l=e.metaReportMarketingFeeAmount??0,r=document.getElementById(`${t}-setting-bottomline`);r&&(r.checked=n);const d=document.getElementById(`${t}-fee-nested`);d?.classList.toggle("is-hidden",!n),d&&(d.dataset.feeMode=s,d.dataset.feeEnabled=a?"true":"false",d.dataset.feePercent=String(o),d.dataset.marketingFee=String(l)),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(g=>{g.checked=g.value===i});const c=metaReportUsesInlineFeeValue(t),m=document.getElementById(`${t}-fee-value`);m&&(m.innerHTML=metaReportFeeValueBlockHtml(t,i,o,l,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),syncMetaReportFeeSelectionState(t),t==="meta-report"&&updateMetaReportBottomlineFeeSummary(e,t)}function bindMetaReportFeeValueInputEnterKey(e){const t=document.getElementById(`${e}-setting-fee-percent`)||document.getElementById(`${e}-setting-marketing-fee`);t&&(t.onkeydown=n=>{n.key==="Enter"&&(n.preventDefault(),document.getElementById(`${e}-fee-save`)?.click())})}function bindMetaReportBottomlineFeeEvents(e,t,n){const a=n||((r,d)=>saveMetaReportClientSettings(e,r,d)),s=document.getElementById(`${t}-fee-edit`);s&&(s.onclick=()=>setMetaReportClientSettingsExpanded(t,!0));const i=document.getElementById(`${t}-fee-done`);i&&(i.onclick=()=>{const r=metaReportsState.clientPayload?.settings||{};syncMetaReportBottomlineFeeDom(r,t),setMetaReportClientSettingsExpanded(t,!1),updateMetaReportBottomlineFeeSummary(r,t)});const o=document.getElementById(`${t}-setting-bottomline`);o&&(o.onchange=()=>{const r=o.checked;a({metaReportShowBottomline:r},()=>{o.checked=!r})}),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(r=>{r.onchange=()=>{if(!r.checked)return;const d=document.getElementById(`${t}-fee-nested`),c=metaReportUsesInlineFeeValue(t),m=document.getElementById(`${t}-fee-value`);m&&d&&(m.innerHTML=metaReportFeeValueBlockHtml(t,r.value||"",d.dataset.feePercent??20,d.dataset.marketingFee??0,{inline:c}),bindMetaReportFeeValueInputEnterKey(t)),bindMetaReportFeeAmountInputs(t),syncMetaReportFeeSelectionState(t)}}),bindMetaReportFeeValueInputEnterKey(t),bindMetaReportFeeDraftTracking(t);const l=document.getElementById(`${t}-fee-save`);l&&(l.onclick=async()=>{if(l.disabled)return;const r=document.getElementById(`${t}-fee-nested`),d=r?.querySelector(`input[name="${t}-fee-mode"]:checked`),c=d&&d.value||null,m={metaReportFeeMode:c};if(c==="performance"){const $=document.getElementById(`${t}-setting-fee-percent`);let b=Number($?.value);Number.isFinite(b)||(b=20),b=Math.min(100,Math.max(0,b)),$&&($.value=String(b)),m.metaReportFeePercent=b}else if(c==="marketing"){const $=document.getElementById(`${t}-setting-marketing-fee`);let b=Number($?.value);Number.isFinite(b)||(b=0),b=Math.max(0,b),$&&($.value=String(b)),m.metaReportMarketingFeeAmount=b}const g=l.textContent;l.disabled=!0,l.textContent="Saving\u2026",l.classList.remove("is-dirty");try{await a(m,()=>syncMetaReportBottomlineFeeDom({metaReportShowBottomline:o?o.checked:!0,metaReportFeeMode:r?.dataset.feeMode||null,metaReportFeePercent:Number(r?.dataset.feePercent)||20,metaReportMarketingFeeAmount:Number(r?.dataset.marketingFee)||0},t)),syncMetaReportFeeSelectionState(t)}finally{l.disabled=!1,l.textContent=g,syncMetaReportFeeSelectionState(t)}})}function renderMetaReportSaveIndicator(e,t=!1){return`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find(w=>w.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const s=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,s);const i=t.editorPayload?.months?.[a]||null,o=n.metaReportShowBottomline,l=s.wonLeads!=null?s.wonLeads:i?.inputs?.wonLeads??i?.topline?.wonLeads??"",r=s.avgLeadValue!=null?s.avgLeadValue:i?.inputs?.avgLeadValue??i?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:i?.inputs?.avgProfitPerWon??"",c=metaReportsState.customValues.draftInputs,m=c?.wonLeads??l,g=c?.avgLeadValue??r,$=c?.avgProfitPerWon??d,b=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(w=>`
            <option value="${esc(w.clientId)}"${w.clientId===n.clientId?" selected":""}>${esc(w.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(w=>`
            <option value="${esc(w)}"${w===a?" selected":""}>${esc(metaMonthLabel(w))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
      ...
      <label>Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(g)}" />
      </label>
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `:'<div class="pipeline-warning">Select at least one pipeline, then click Apply data filters.</div>'}function defaultDisplayPrefs(){const e={kpis:{},sections:{},charts:getDefaultChartPrefs(),statusItems:{},columns:{}};return Object.keys(DISPLAY_OPTIONS.kpis).forEach(t=>{e.kpis[t]=!0}),Object.keys(DISPLAY_OPTIONS.sections).forEach(t=>{e.sections[t]=!0}),Object.keys(DISPLAY_OPTIONS.charts).forEach(t=>{e.charts[t]=!0}),Object.keys(DISPLAY_OPTIONS.statusItems).forEach(t=>{e.statusItems[t]=!0}),Object.entries(DISPLAY_OPTIONS.columns).forEach(([t,n])=>{e.columns[t]={},Object.keys(n).forEach(a=>{e.columns[t][a]=!0})}),e}function loadDisplayPrefs(){if(IS_CLIENT_VIEW&&!IS_PREVIEW)return defaultDisplayPrefs();try{const e=JSON.parse(localStorage.getItem(STORAGE_KEY)||localStorage.getItem(LEGACY_STORAGE_KEY)||"null");if(!e)return defaultDisplayPrefs();const t=defaultDisplayPrefs();return{kpis:{...t.kpis,...e.kpis},sections:{...t.sections,...e.sections},charts:{...t.charts,...e.charts},statusItems:{...t.statusItems,...e.statusItems},columns:{sourceReport:{...t.columns.sourceReport,...e.columns?.sourceReport||{}},assigneeReport:{...t.columns.assigneeReport,...e.columns?.assigneeReport||{}},pipelineBreakdown:{...t.columns.pipelineBreakdown,...e.columns?.pipelineBreakdown||{}}}}}catch{return defaultDisplayPrefs()}}function ensureChartsVisible(){const e=Object.keys(DISPLAY_OPTIONS.charts);e.some(n=>display.charts[n]!==!1)||(e.forEach(n=>{display.charts[n]=!0}),saveDisplayPrefs())}function saveDisplayPrefs(){localStorage.setItem(STORAGE_KEY,JSON.stringify(display))}function isVisible(e,t,n){return e==="columns"?display.columns[n]?.[t]!==!1:display[e]?.[t]!==!1}function toggleDisplay(e,t,n){e==="columns"?display.columns[n][t]=!display.columns[n][t]:display[e][t]=!display[e][t],saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}function setAllDisplay(e,t,n){e==="columns"?Object.keys(DISPLAY_OPTIONS.columns[n]).forEach(a=>{display.columns[n][a]=t}):Object.keys(DISPLAY_OPTIONS[e]).forEach(a=>{display[e][a]=t}),saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}const fmt=e=>new Intl.NumberFormat("da-DK",{minimumFractionDigits:0,maximumFractionDigits:0}).format(Math.round(Number(e)||0)),fmtCompact=e=>{const t=Number(e)||0;return t>=1e6?`${(t/1e6).toFixed(2)}M`:t>=1e3?`${(t/1e3).toFixed(2)}K`:fmt(t)},fmtPct=e=>`${(Number(e)||0).toFixed(2)}%`,fmtRoas=e=>{const t=Number(e)||0;return t>0?`${t.toFixed(2)}x`:"\u2014"};function formatActiveDateFilter(e){if(!e.dateFrom&&!e.dateTo)return"Till date";const t=getDashboardTimeZone();if(window.MarketingMetrics?.formatShortDateLabel){const n=window.MarketingMetrics.formatShortDateLabel(e.dateFrom,t),a=window.MarketingMetrics.formatShortDateLabel(e.dateTo,t);if(n&&a)return`${n} \u2013 ${a}`}return`${e.dateFrom||"start"} to ${e.dateTo||"now"}`}function showDateRangeError(e){document.querySelectorAll("#date-range-error").forEach(t=>{t.textContent=e,t.hidden=!e})}function clearDateRangeError(){showDateRangeError("")}function needsFreshData(){return!lastFetchedAt||Date.now()-lastFetchedAt>DATA_FRESH_MS}function buildQuery(e,t={}){const n=new URLSearchParams,a=getAllPipelineIds(e||[]),s=state.pipelineIds.filter(i=>a.includes(i));if(!s.length)throw new Error("Select at least one pipeline.");return n.set("pipelineIds",s.join(",")),state.dateField&&n.set("dateField",state.dateField),state.dateFrom&&n.set("dateFrom",state.dateFrom),state.dateTo&&n.set("dateTo",state.dateTo),["status","source","assignedTo","adSpend"].forEach(i=>{state[i]&&state[i]!=="all"&&n.set(i,state[i])}),appendTenantParams(n),t.forceFresh&&n.set("fresh","1"),n.toString()}function formatDateInput(e){const t=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),a=String(e.getDate()).padStart(2,"0");return`${t}-${n}-${a}`}function getDashboardTimeZone(){return cachedData?.account?.timezone||"Europe/Copenhagen"}function getCalendarPartsInTimeZone(e,t=new Date){const n=new Intl.DateTimeFormat("en-CA",{timeZone:e,year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(t);return{year:Number(n.find(a=>a.type==="year")?.value),month:Number(n.find(a=>a.type==="month")?.value),day:Number(n.find(a=>a.type==="day")?.value)}}function formatMonthDateRange(e,t){const n=`${e}-${String(t).padStart(2,"0")}-01`,a=new Date(e,t,0).getDate(),s=`${e}-${String(t).padStart(2,"0")}-${String(a).padStart(2,"0")}`;return{start:n,end:s}}const datePickerState={inputId:null,anchorEl:null,viewYear:null,viewMonth:null};let datePickerListenersBound=!1;function isoFromParts(e,t,n){const a=Number(n);return!e||!t||!Number.isFinite(a)?"":`${e}-${String(t).padStart(2,"0")}-${String(a).padStart(2,"0")}`}function formatPickerDisplayLabel(e,t){if(!e||!/^\d{4}-\d{2}-\d{2}$/.test(e))return t;const n=getDashboardTimeZone();return window.MarketingMetrics?.formatShortDateLabel&&window.MarketingMetrics.formatShortDateLabel(e,n)||e}function ensureDatePickerPopover(){if(document.getElementById("date-picker-popover"))return;const e=document.createElement("div");e.id="date-picker-backdrop",e.className="date-picker-backdrop",e.hidden=!0,e.addEventListener("click",closeDatePicker),document.body.appendChild(e);const t=document.createElement("div");t.id="date-picker-popover",t.className="date-picker-popover",t.hidden=!0,t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),document.body.appendChild(t)}function syncDatePickerDisplays(){[{id:"dateFrom",key:"dateFrom",placeholder:"Pick start date"},{id:"dateTo",key:"dateTo",placeholder:"Pick end date"}].forEach(({id:e,key:t,placeholder:n})=>{const a=document.getElementById(e),s=document.getElementById(`${e}-display`),i=document.getElementById(`${e}-trigger`),o=state[t]??"";a&&(a.value=o),s&&(s.textContent=formatPickerDisplayLabel(o,n)),i?.classList.toggle("is-empty",!o),i?.classList.toggle("has-value",!!o)})}function getStateDateValue(e){return state[e]??""}function getDatePickerViewMonth(e){const t=getStateDateValue(e);if(/^\d{4}-\d{2}-\d{2}$/.test(t)){const[n,a]=t.split("-").map(Number);return{year:n,month:a}}return getCalendarPartsInTimeZone(getDashboardTimeZone())}function getTodayIso(){const e=getCalendarPartsInTimeZone(getDashboardTimeZone());return isoFromParts(e.year,e.month,e.day)}function isStartDateDisabled(e){return e>=getTodayIso()}function isFutureDateDisabled(e){return e>getTodayIso()}function isFutureMonthView(e,t){const n=getCalendarPartsInTimeZone(getDashboardTimeZone());return e>n.year||e===n.year&&t>n.month}function isDatePickerDayDisabled(e,t){return t==="dateFrom"&&isStartDateDisabled(e)?!0:isFutureDateDisabled(e)}function renderDatePickerDayCell(e,t,n,a,s,i,o,l){const r=["date-picker-day"];if(n&&r.push("is-outside"),e===a&&r.push("is-today"),e===s&&r.push("is-selected"),i&&o){const d=i<=o?i:o,c=i<=o?o:i;e>=d&&e<=c&&r.push("is-in-range")}return e===i&&r.push("is-range-start"),e===o&&r.push("is-range-end"),isDatePickerDayDisabled(e,l)?(r.push("is-disabled"),`<button type="button" class="${r.join(" ")}" disabled aria-disabled="true">${t}</button>`):`<button type="button" class="${r.join(" ")}" onclick="selectDatePickerDay('${e}')">${t}</button>`}function renderDatePickerPopover(){const e=document.getElementById("date-picker-popover");if(!e||!datePickerState.inputId)return;const{viewYear:t,viewMonth:n,inputId:a}=datePickerState,s=new Intl.DateTimeFormat("en-GB",{month:"long",year:"numeric"}).format(new Date(t,n-1,1)),i=["Mo","Tu","We","Th","Fr","Sa","Su"],l=(new Date(t,n-1,1).getDay()+6)%7,r=new Date(t,n,0).getDate(),d=new Date(t,n-1,0).getDate(),c=getCalendarPartsInTimeZone(getDashboardTimeZone()),m=isoFromParts(c.year,c.month,c.day),g=getStateDateValue(a),$=getStateDateValue("dateFrom"),b=getStateDateValue("dateTo");let w="";for(let y=l-1;y>=0;y-=1){const k=d-y,I=n===1?12:n-1,L=n===1?t-1:t,R=isoFromParts(L,I,k);w+=renderDatePickerDayCell(R,k,!0,m,g,$,b,a)}for(let y=1;y<=r;y+=1){const k=isoFromParts(t,n,y);w+=renderDatePickerDayCell(k,y,!1,m,g,$,b,a)}const S=(7-(l+r)%7)%7;for(let y=1;y<=S;y+=1){const k=n===12?1:n+1,I=n===12?t+1:t,L=isoFromParts(I,k,y);w+=renderDatePickerDayCell(L,y,!0,m,g,$,b,a)}const h=a==="dateFrom"?'<span class="date-picker-hint">Start date must be before today</span>':"",f=a==="dateTo"?'<button type="button" class="date-picker-footer-btn primary" onclick="setDatePickerToday()">Today</button>':"",v=n===12?1:n+1,p=n===12?t+1:t,u=!isFutureMonthView(p,v);e.innerHTML=`
    <div class="date-picker-panel">
      ...
      <div class="date-picker-weekdays">
        ${i.map(y=>`<div class="date-picker-weekday">${y}</div>`).join("")}
      </div>
      ...
      <table class="dashboard-table">
        ...
        <tbody>
          ${a.length?a.map(o=>`
            <tr>
              <td>${s(o,"label")}</td>
              ${i.map(l=>`<td class="${l.align||""}">${s(o,l.key)}</td>`).join("")}
            </tr>
  ...
  `}function renderDashboardContent(e){const{kpis:t,statusBreakdown:n,sourceReport:a,assigneeReport:s,pipelines:i,filters:o}=e,l=isVisible("sections","statusBreakdown");return`
      ...
      <div class="section-grid">
        ${isVisible("sections","assigneeReport")?`
          <div class="card">
            ...
            ${renderTable("assigneeReport","Assignee",[{key:"won",label:"Won",align:"num"},{key:"totalLeads",label:"Total leads",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"},{key:"totalValue",label:"Total value",align:"num"}],s,(r,d)=>d==="label"?esc(r.assigneeName):d==="wonValue"||d==="totalValue"?`Dkr ${fmt(r[d])}`:fmt(r[d]))}
          </div>
        `:""}
        ${isVisible("sections","pipelineBreakdown")?`
          <div class="card">
            <div class="section-title">Pipeline Breakdown</div>
            ${renderTable("pipelineBreakdown","Pipeline",[{key:"count",label:"Leads",align:"num"},{key:"won",label:"Won",align:"num"},{key:"monetary",label:"Value",align:"num"},{key:"profit",label:"Bundlinje",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"}],i,(r,d)=>d==="label"?esc(r.name):d==="monetary"||d==="profit"||d==="wonValue"?`Dkr ${fmt(r[d])}`:fmt(r[d]))}
          </div>
        `:""}
      </div>
    `:""}
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),l=document.getElementById("apply-filters-btn");if(e||!cachedData){const r=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),l&&(l.disabled=!0,l.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),r!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const g=canReuseBootstrapDashboard(c)&&!s;let $,b;if(g?[b,$]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[$,b]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),r!==fetchGeneration)return;cachedFacebookMetrics=b,cachedData=applyMarketingData($,b),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(r!==fetchGeneration)return;if(c.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            `)}`;return}if(IS_ADMIN_CLIENT&&o){const m=document.getElementById("dashboard-content");m&&(m.innerHTML=`
            <div class="note admin-setup-placeholder">
              ${esc(c.message)}
              <div style="margin-top:12px">
                <button type="button" class="refresh-btn primary" onclick="loadDashboard(true)">${RETRY_MSG}</button>
              </div>
            </div>`)}else o?o&&cachedData?(restoreDashboardContentAfterFailedFetch(o),showToast(c.message||"Failed to load filtered data.","error")):l&&(l.textContent="Apply failed - try again"):i.innerHTML=`
          ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
          ${wrapDashboardShell(`
            <div class="error-state">
              <div>Fejl ved hentning af data</div>
          ...
          `)}`;return}finally{r===fetchGeneration&&(activeFetchController=null,resetFetchUiState()),pendingRefetch&&(pendingRefetch=!1,loadDashboard(!0))}if(r!==fetchGeneration)return}try{o?(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi()):(renderDashboard(cachedData),mountCharts(cachedData))}catch(r){resetFetchUiState(),i.innerHTML=`
      ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
      ${wrapDashboardShell(`
        <div class="error-state">
          <div>Fejl ved visning af dashboard</div>
          <div style="margin-top:8px;font-size:12px;color:#666">${esc(r.message)}</div>
          <button class="refresh-btn primary" onclick="loadDashboard(true)">${RETRY_MSG}</button>
        </div>
      `)}
