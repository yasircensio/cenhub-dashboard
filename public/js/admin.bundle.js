...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(n?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),y=w.summary||w;l=y.runId,c=0,setFbLeadProgress({processed:0,total:y.inWindow||0,label:`Matching ${y.inWindow||0} Meta leads to GHL\u2026`})}else!n&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:g||f?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:b,batchOffset:c,batchLimit:s})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const y=w.inWindow||c||g;n?(setFbLeadProgress({processed:c,total:y,complete:!w.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:y,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const $=d.summary||d,m=$.inWindow||c||g,h=$.nextBatchOffset??c;if(n)setFbLeadProgress({processed:h,total:m,complete:!0}),markFbLeadPreviewReady(e,t,$.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=($.updated||0)>0),showToast(($.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:m,written:$.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch($){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${$.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast(($.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&n&&hideFbLeadProgress(!0),!n&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(r=>r.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function metaReportFullUrl(e){return e?`${window.location.origin}${e}`:""}function normalizeMetaReportSlugInput(e){return String(e||"").trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function resolveMetaReportShareSlug(e){const t=e.settings||{};if(t.metaReportSlug)return t.metaReportSlug;const a=String(e.reportUrl||"").replace(/^\/report\//,"").match(/^(.+)-(\d{4})$/);return a?a[1]:""}function resolveMetaReportShareCode(e){const n=String(e.reportUrl||"").replace(/^\/report\//,"").match(/-(\d{4})$/);return n?n[1]:""}function closeMetaReportModal(){document.getElementById("meta-report-modal-root")?.remove(),document.body.classList.remove("meta-report-modal-open"),metaReportsState.clientShareExpanded=!1,metaReportsState.clientExcelSheetExpanded=!1,syncMetaReportControlPanelUi()}function openMetaReportModal(e,{wizard:t=!1}={}){closeMetaReportModal();const n=document.createElement("div");n.id="meta-report-modal-root",n.className="meta-report-modal-root",n.innerHTML=`
  ...
  `}function resolveMetaReportExcelSheetUrl(e={}){return String(e?.metaReportExcelSheetUrl||"").trim()||null}function renderMetaReportExcelSheetButton(e={},{prominent:t=!1,hero:n=!1}={}){const a=resolveMetaReportExcelSheetUrl(e);return a?`<a class="${["admin-btn","admin-btn--primary",t?"meta-report-excel-sheet-btn":"",n?"meta-report-public-excel-cta":""].filter(Boolean).join(" ")}" href="${esc(a)}" target="_blank" rel="noopener noreferrer">See Excel sheet</a>`:""}function updateMetaReportPublicExcelSheetButton(e={}){const t=document.getElementById("meta-report-public-excel-sheet-btn");t&&(t.innerHTML=renderMetaReportExcelSheetButton(e,{prominent:!0,hero:!0}))}function describeMetaReportExcelSheetLabel(e){if(!e)return"Not set";try{const t=new URL(e),n=t.pathname.length>24?`${t.pathname.slice(0,24)}\u2026`:t.pathname;return`${t.hostname}${n}`}catch{return e.length>40?`${e.slice(0,40)}\u2026`:e}}function renderMetaReportExcelSheetSummary(e={}){const t=resolveMetaReportExcelSheetUrl(e),n=describeMetaReportExcelSheetLabel(t);return`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:n=null,activeMonthKey:a=null,settings:o=null}={}){const r=a||e?.monthKey||null,s=r?metaReportMonthBounds(r):null,i=o||n?.settings||{},l=isMetaReportGhlListClient(i),c=resolveClientToplineMode(i),d=resolveMetaToplineSource(e,i),u=l&&c==="cenhub"&&d==="ghl";let p="";if(t&&e&&!e.meta?.emptyMonth&&s){const k=u?" readonly":"",E=renderMetaReportMonthManualRow(e,i);p=`
    ...
    `}const b=a||e?.monthKey||null,f=t?b?`Report preview \xB7 ${metaMonthLabel(b)}`:"Report preview":b?`Report \xB7 ${metaMonthLabel(b)}`:"Report",g=metaReportsState.reportViewMode||"monthly",$=shouldShowMetaReportYearVisuals(e,n,{editable:t}),m=e&&!e.meta?.emptyMonth?`
  ...
  `:"",h=renderMetaReportMonthTables(e,{editable:t}),v=$?renderMetaReportChartsPanel({editable:t}):"",w=t&&$?renderMetaReportScenarioCard():"",y=$?renderMetaReportViewModeTabs():"";let S;return $?S=`
      ...
      ${w}
    `:S=`<div class="meta-report-content-main">${h}</div>`,`${p}${m}${y}${S}`}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
      ...
      <div class="meta-report-topline-summary">
        <div class="meta-report-topline-summary-content">
          <div class="meta-report-topline-summary-row">
            ...
            <span class="meta-report-topline-summary-value ${n==="cenhub"?"is-cenhub":"is-meta"}">${esc(n==="cenhub"?"Cenhub":"Meta Analytics")}</span>
          </div>
          <div class="meta-report-topline-summary-row">
  ...
  `}function renderMetaReportToplineChangeModal(e,t){const n=resolveClientToplineMode(e?.settings||{});return`
    <div class="meta-report-modal-head">
...
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!r)return!1;r.trim().toLowerCase()==="overwrite"&&(a={...a,skipManual:!1,overwriteManual:!0})}}return await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/switch-topline-source`,{method:"POST",body:JSON.stringify(a)}),showToast(n==="cenhub"?"Switched all months to Cenhub":"Switched all months to Meta Analytics","success"),typeof t.reload=="function"?await t.reload():await loadMetaReportsClientPage({silent:!0}),!0}function bindMetaReportToplineChangeModal(e){const t=e.clientId;document.querySelectorAll('input[name="meta-topline-draft-mode"]').forEach(a=>{a.addEventListener("change",updateMetaReportToplineApplyState)}),document.getElementById("meta-report-topline-ack")?.addEventListener("change",updateMetaReportToplineApplyState),document.getElementById("meta-report-topline-confirm")?.addEventListener("input",updateMetaReportToplineApplyState);const n=document.getElementById("meta-report-topline-apply");n&&(n.onclick=async()=>{const a=getMetaReportToplineDraftMode(),o=resolveClientToplineMode(e.settings||{});if(a===o)return;n.disabled=!0,document.getElementById("meta-report-topline-modal-loading")?.removeAttribute("hidden");const r=typeof e.reload=="function";r?setMetaCvSavePending(!0):(metaReportsState.clientToplineSaving=!0,syncMetaReportToplineSourceBar(metaReportsState.clientPayload||e));try{await applyMetaReportToplineSwitch(t,e,a)&&closeMetaReportModal()}catch(s){showToast(s.message||"Could not switch data source","error")}finally{r?setMetaCvSavePending(!1):(metaReportsState.clientToplineSaving=!1,syncMetaReportToplineSourceBar(metaReportsState.clientPayload)),document.getElementById("meta-report-topline-modal-loading")?.setAttribute("hidden","")}})}function renderMetaReportMonthManualRow(e,t={},{prefix:n="meta-report"}={}){return!isMetaReportGhlListClient(t)||resolveClientToplineMode(t)!=="cenhub"?"":resolveMetaToplineSource(e,t)==="manual"?`
    <div class="meta-report-month-manual-row">
      ...
      <div class="meta-report-month-manual-row-actions">
        ...
        <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${n}-restore-month-source">Switch back to Cenhub</button>
      </div>
    </div>
  `:`
      <div class="meta-report-month-manual-row">
...
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!f)return;f.trim().toLowerCase()==="overwrite"&&(u=!1,p=!0)}const b=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(c)}`,{method:"POST",body:JSON.stringify({skipManual:u,overwriteManual:p})});r?await r():s&&await s(b,null,{yearSync:!0}),showToast(`Synced ${b.synced?.length||0} month(s)${b.skipped?.length?`, skipped ${b.skipped.length}`:""}`,"success")}catch(c){showToast(c.message||"Year sync failed","error")}finally{o&&o(!1)}});const l=document.getElementById(`${t}-refresh-snapshot`);l&&(l.onclick=async()=>{const c=n?.();if(c){o&&o(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/refresh-snapshot`,{method:"POST",body:"{}"});s&&await s(d,c),showToast("Cenhub data refreshed and synced","success")}catch(d){showToast(d.message||"Refresh failed","error")}finally{o&&o(!1)}}})}function monthHasCenhubSnapshot(e){if(resolveMetaToplineSource(e)==="ghl")return!0;const n=e?.ghlTotals||{};return!!(e?.ghlSyncedAt||n.leads!=null||n.wonLeads!=null)}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),n=e.monthKeys||[];return t.length?t.map(a=>`
    ...
    <div class="meta-report-bottomline-fee-settings${o?" meta-report-bottomline-fee-settings--client":""}${o?$?" is-editing":" is-collapsed":""}">
      ...
      <div class="meta-report-bottomline-editor${o&&!$?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${s?"":" is-hidden"}" id="${t}-fee-nested" ${b}>
          ...
          <div class="meta-report-fee-mode-grid${n?" meta-report-fee-mode-grid--rows":""}">${g}
          </div>
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find(L=>L.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
    ...
    `;const o=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,o);const r=t.editorPayload?.months?.[a]||null,s=n.metaReportShowBottomline,i=!!n.metaReportGhlDataEnabled,l={metaReportGhlDataEnabled:n.metaReportGhlDataEnabled,metaReportToplineMode:n.metaReportToplineMode},c=resolveClientToplineMode(l),d=resolveMetaToplineSource(r||{dataSource:o.toplineSource},l),u=i&&c==="cenhub"&&d==="ghl",p=o.wonLeads!=null?o.wonLeads:r?.inputs?.wonLeads??r?.topline?.wonLeads??"",b=o.avgLeadValue!=null?o.avgLeadValue:r?.inputs?.avgLeadValue??r?.topline?.avgLeadValue??"",f=o.avgProfitPerWon!=null?o.avgProfitPerWon:r?.inputs?.avgProfitPerWon??"",g=o.ghlLeads??r?.ghlTotals?.leads??"",$=o.ghlTotalRevenue??r?.ghlTotals?.totalRevenue??"",m=o.ghlTotalProfit??r?.ghlTotals?.totalProfit??"",h=metaReportsState.customValues.draftInputs,v=h?.wonLeads??p,w=h?.avgLeadValue??b,y=h?.avgProfitPerWon??f,S=u?" readonly":"",k=u?" disabled":"",E=t.settingsSaving&&t.settingsSavingScope==="report",I=i?renderMetaCvToplineSourceControl(n,e):"",C=renderMetaCvMonthManualRow(r||{dataSource:d},n),B=i?renderMetaMonthSourcePill(r||{dataSource:d},l):"";return`
    <div class="meta-report-edit-panel-head">
      <div>
