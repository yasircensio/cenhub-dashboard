(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){if(t[1]==="sync-history"&&(t[2]==="ghl"||t[2]==="meta")){document.body.dataset.dashboardMode=`sync-history-${t[2]}`,delete document.body.dataset.clientSlug;return}if(t[1]==="fb-lead-sync"){document.body.dataset.dashboardMode="fb-lead-sync",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="custom-values"){document.body.dataset.dashboardMode="meta-reports-custom",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]){document.body.dataset.dashboardMode="meta-reports-client",document.body.dataset.clientSlug=t[2];return}if(t[1]==="meta-reports"){document.body.dataset.dashboardMode="meta-reports",delete document.body.dataset.clientSlug;return}document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}if(t[0]==="report"&&t[1]){document.body.dataset.dashboardMode="report",document.body.dataset.reportToken=t[1],delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_ADMIN_SYNC_HISTORY_GHL=DASHBOARD_MODE==="sync-history-ghl",IS_ADMIN_SYNC_HISTORY_META=DASHBOARD_MODE==="sync-history-meta",IS_ADMIN_FB_LEAD_SYNC=DASHBOARD_MODE==="fb-lead-sync",IS_ADMIN_META_REPORTS=DASHBOARD_MODE==="meta-reports",IS_ADMIN_META_REPORTS_CLIENT=DASHBOARD_MODE==="meta-reports-client",IS_ADMIN_META_REPORTS_CUSTOM=DASHBOARD_MODE==="meta-reports-custom",IS_REPORT_VIEW=DASHBOARD_MODE==="report",REPORT_TOKEN=document.body.dataset.reportToken||"",IS_ADMIN_SYNC_HISTORY=IS_ADMIN_SYNC_HISTORY_GHL||IS_ADMIN_SYNC_HISTORY_META,IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);return e[0]==="admin"&&e[1]==="meta-reports"&&e[2]&&e[2]!=="custom-values"?e[2]:e[0]==="admin"&&e[1]&&e[1]!=="meta-reports"?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"&&e[0]!=="report"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){const t=!!(e&&e.includes("staff-admin-chrome")),a=t||e&&e.includes("admin-topbar-link")?"/admin":"",n=a?`<a href="${a}" class="brand-logo-link" aria-label="Cenhub home"><img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" /></a>`:'<img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" />',s=t?`<button type="button" class="staff-nav-toggle" id="staff-nav-toggle" aria-expanded="false" aria-controls="staff-topbar-panel" aria-label="Open menu">
  ...
  `}async function loadTeamPage(){const e=document.getElementById("dashboard");if(await requireStaffAuth()){if(!isStaffAdmin()){window.location.href="/admin";return}e.innerHTML=`
    ...
    `}}}async function reloadTeamUsersTable(){const e=document.getElementById("team-users-content");if(!e)return;const t=await adminFetch("/api/auth/users");e.innerHTML=renderStaffUsersTable(t.users||[])}function renderStaffUserActions(e){const t=currentStaffUser?.id===e.id,a=e.status==="disabled"?`<button class="admin-btn" type="button" onclick="updateStaffUser('${esc(e.id)}', { status: 'active' })">Enable</button>`:`<button class="admin-btn" type="button" ${t?'disabled title="You cannot disable your own account"':""} onclick="updateStaffUser('${esc(e.id)}', { status: 'disabled' })">Disable</button>`,n=e.status==="pending"||!e.hasPassword?`<button class="admin-btn" type="button" onclick="copyStaffInviteLink('${esc(e.id)}')">Copy invite link</button>`:`<button class="admin-btn" type="button" onclick="resetStaffUserPassword('${esc(e.id)}', true)">Reset password</button>`,s=t?"":`<button class="admin-btn card-menu-item--danger" type="button" onclick="deleteStaffUser('${esc(e.id)}')">Delete</button>`;return`
  ...
  `}function renderSyncHistoryRows(e,t){if(!e.length)return'<div class="sync-history-empty">No sync runs logged yet.</div>';const a=t==="meta"?"Spend / details":"Details",n=e.map(s=>{let r="\u2014";if(t==="meta"){const o=[];if(s.thisMonthSpend!=null&&o.push(`${fmtDkk(s.thisMonthSpend)} DKK this month`),s.spendDateStop){const i=formatSyncHistoryDate(s.spendDateStop);i&&o.push(`through ${i}`)}s.errorMessage&&o.push(s.errorMessage),r=o.join(" \xB7 ")||"\u2014"}else s.opportunityCount!=null?(r=`${s.opportunityCount} opportunities`,s.errorMessage&&(r+=` \xB7 ${s.errorMessage}`)):s.errorMessage&&(r=s.errorMessage);return`
      <tr>
...
${a}`))try{const s=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}`,{method:"DELETE"});showToast(`Cleared ${s.deleted||0} log entr${s.deleted===1?"y":"ies"}.`,"success"),await loadSyncHistoryPage(e)}catch(s){showToast(s.message||`Failed to clear ${t} sync log.`,"error")}}function renderSyncHistoryLoginPrompt(e){const t=e==="meta"?"Meta sync log":"GHL sync log";return`
  ...
  `}async function loadSyncHistoryPage(e,{silent:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){t||(a.innerHTML=renderSyncHistoryLoginPrompt(e));return}currentStaffUser=n,t||(a.innerHTML=`
    ...
    `);try{const s=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}&limit=150`);a.innerHTML=renderSyncHistoryPage(e,s);const r=document.getElementById("sync-history-refresh");r&&(r.onclick=()=>loadSyncHistoryPage(e));const o=document.getElementById("sync-history-clear-log");o&&(o.onclick=()=>clearSyncHistoryLog(e)),syncHistoryRefreshTimer&&(clearInterval(syncHistoryRefreshTimer),syncHistoryRefreshTimer=null),syncHistoryRefreshTimer=window.setInterval(()=>{loadSyncHistoryPage(e,{silent:!0}).catch(()=>{})},6e4)}catch(s){a.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome(e==="meta"?"meta-sync":"ghl-sync"))}
...
${o} contact(s) will get Fb Lead id written in GHL. This writes live data.`)&&(fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:!0,previewRunId:a},await runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!1}))}async function refreshFbLeadClientRowsUi(){const e=document.getElementById("fb-lead-clients-mount"),t=document.getElementById("fb-lead-field-warnings-mount");e&&(e.innerHTML=renderFbLeadClientRows(fbLeadSyncState.clients)),t&&(t.innerHTML=renderFbLeadFieldWarnings(fbLeadSyncState.clients)),bindFbLeadSyncToggles()}function renderFbLeadClientActions(e){const t=esc(e.clientId),a=e.metaPageId?"":`<a class="admin-btn admin-btn--ghost admin-btn--small" href="/admin/${encodeURIComponent(e.clientId)}">Setup</a>`;return`
  ...
  `}function lastRunIsSuccessfulApply(e){if(!e||e.dryRun)return!1;const t=String(e.status||"").toLowerCase();return t==="success"||t==="ok"}function lastRunIsSuccessfulPreview(e){if(!e||!e.dryRun)return!1;const t=String(e.status||"").toLowerCase();return t==="success"||t==="ok"}function resolveFbLeadClientStats(e,t={}){const a=e?.displayStats||{},n=t.metaLeadCount90d??a.metaLeadCount90d??null,s=t.estimatedMissing??a.outstanding??null,r=t.estimatedMissing!=null?"Estimated contacts still missing Fb Lead id (from a Meta sample)":a.outstandingHint||"Estimated contacts still missing Fb Lead id (from a Meta sample)";return{metaLeads:n!=null?String(n):"\u2014",missing:s!=null?String(s):"\u2014",missingTitle:r}}function formatFbLeadLastRunLabel(e){if(!e)return"\u2014";const t=[e.dryRun?"preview":e.status,`${e.updated||0} ${e.dryRun?"would update":"updated"}`];return(e.skippedNoMatch??0)>0&&t.push(`${e.skippedNoMatch} no match`),t.join(" \xB7 ")}function renderFbLeadClientRows(e){return e.length?`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table fb-lead-clients-table">
        ...
        <tbody>${e.map(a=>{const n=fbLeadSyncState.preflightByClient[a.clientId]||{},s=resolveFbLeadClientStats(a,n),r=formatFbLeadLastRunLabel(a.lastRun);return`
      <tr>
        <td>
  ...
  `:'<div class="sync-history-empty">No clients configured yet.</div>'}function renderFbLeadClientRowsWrapper(e){return`<div id="fb-lead-clients-mount">${renderFbLeadClientRows(e)}</div>`}function renderFbLeadHistoryRows(e,t={}){const a=getFbLeadHistoryRunsForDisplay(e);if(!a.length){const s=fbLeadSyncState.showRoutineCronRuns?0:(e||[]).filter(r=>isRoutineSuccessfulCronRun(r)).length;return s>0?`<div class="sync-history-empty">${s} routine auto-sync run(s) hidden \u2014 enable <strong>Show routine auto-sync runs</strong> above to view them.</div>`:'<div class="sync-history-empty">No FB lead sync runs logged yet.</div>'}return e=a,`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table">
        <thead>
          <tr>
        ...
        <tbody>${e.map(s=>{const r=[s.mode,s.dryRun?"dry-run":"apply",s.dryRun?`${s.updated||0} would update`:`${s.updated||0} updated`,`${s.skippedNoMatch||0} no match`,s.errors?`${s.errors} errors`:null].filter(Boolean).join(" \xB7 "),o=dryRunIsReadyForApply(s,e)&&!fbLeadSyncState.isApplying,i=isPartialSyncRun(s)&&!fbLeadSyncState.isApplying,l=fbLeadSyncState.isApplying&&fbLeadSyncState.applyingRunId===s.id,c=renderSyncStatusBadge(l?"applying":s.status);return`
      <tr class="fb-lead-audit-row${l?" is-applying":""}">
        <td>${esc(formatSyncHistoryTimestamp(s.startedAt))}</td>
        <td>${esc(s.accountName||s.clientId)}</td>
        <td>${c}</td>
        <td>${esc(s.mode||"recent")}</td>
        <td>${esc(formatSyncSource(s.source))}</td>
        <td class="sync-history-detail">${esc(r)}</td>
        <td>
          <div class="fb-lead-history-actions">
            ${l?'<span class="fb-lead-history-status-note">In progress above</span>':i?`<button class="admin-btn admin-btn--primary admin-btn--small" type="button" onclick="resumeFbLeadRunFromHistory(${Number(s.id)})">Resume (${partialSyncRemaining(s)})</button>`:o?`<button class="admin-btn admin-btn--primary admin-btn--small" type="button" data-fb-apply-run="${Number(s.id)}" data-apply-count="${Number(s.updated||0)}" onclick="applyFbLeadSyncFromHistory(${Number(s.id)})">Apply (${s.updated})</button>`:""}
            <button class="admin-btn admin-btn--ghost admin-btn--small" type="button" onclick="expandFbLeadRun(${Number(s.id)})">Audit</button>
            ${isStaffAdmin()?`<button class="admin-btn admin-btn--ghost admin-btn--small" type="button" onclick="deleteFbLeadRun(${Number(s.id)})">Delete</button>`:""}
          </div>
        </td>
      </tr>
      <tr id="fb-lead-run-audit-${Number(s.id)}" hidden>
        <td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${Number(s.id)}">${renderFbLeadAuditTable(s.rows||[],t[s.clientId])}</div></td>
      </tr>
    `}).join("")}</tbody>
      </table>
    </div>
  `}function renderFbLeadAuditTable(e,t){if(!e.length)return'<div class="sync-history-empty" style="padding:12px">No contact-level rows stored for this run.</div>';const a=e.map(s=>{const r=ghlContactUrl(t,s.contactId),o=r?`<a href="${esc(r)}" target="_blank" rel="noopener noreferrer">Open in GHL</a>`:"\u2014";return`
      <tr>
  ...
  `}function renderFbLeadSyncPage(e){const t=e.summary||{};return`
    ...
    `)}
  `}async function toggleFbLeadSyncEnabled(e,t){try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"PUT",body:JSON.stringify({fbLeadSyncEnabled:t})}),showToast(t?"Automatic FB lead sync enabled":"Automatic FB lead sync disabled","success");const a=fbLeadSyncState.clients.find(n=>n.clientId===e);a&&(a.fbLeadSyncEnabled=t)}catch(a){showToast(a.message||"Failed to update auto-sync setting.","error"),await loadFbLeadSyncPage({silent:!0})}}function bindFbLeadSyncToggles(){document.querySelectorAll("[data-fb-sync-toggle]").forEach(e=>{e.onchange=()=>toggleFbLeadSyncEnabled(e.dataset.fbSyncToggle,e.checked)})}async function loadFbLeadPreflightForClients(e,{quick:t=!1}={}){const a=t?"&quick=1":"";await Promise.all((e||[]).map(async n=>{const s=fbLeadSyncState.preflightByClient[n.clientId]||{};try{const r=await adminFetch(`/api/fb-lead-sync/preflight?client=${encodeURIComponent(n.clientId)}${a}`);fbLeadSyncState.preflightByClient[n.clientId]=t?{...r,metaLeadCount90d:r.metaLeadCount90d??s.metaLeadCount90d??null,estimatedMissing:r.estimatedMissing??s.estimatedMissing??null,sampleSize:r.sampleSize||s.sampleSize||0,sampleWouldUpdate:r.sampleWouldUpdate??s.sampleWouldUpdate??0}:r}catch(r){fbLeadSyncState.preflightByClient[n.clientId]={...s,preflightError:r.message}}}))}function scheduleFbLeadFullPreflight(e){e?.length&&loadFbLeadPreflightForClients(e,{quick:!1}).then(()=>refreshFbLeadClientRowsUi()).catch(()=>{})}async function loadFbLeadHistory(e=50){const t=await adminFetch(`/api/fb-lead-sync/history?limit=${e}`);return fbLeadSyncState.historyRuns=t.runs||[],t}function renderFbLeadRunPanel(e,t="recent"){const a=fbLeadSyncState.clients.find(r=>r.clientId===e),n=clientHasReadyPreview(a),s=t==="backfill"?"Backfill (90 days)":"Recent (2 days)";return`
    <div class="fb-lead-run-panel" id="fb-lead-run-panel">
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start">
        <div>
          <strong>${esc(a?.accountName||e)}</strong>
          <div style="color:var(--text-soft);font-size:13px;margin-top:4px">${esc(s)}</div>
        </div>
  ...
  `).join(""),n=e.nextBatchOffset??(e.batchOffset||0)+(e.batchProcessed||0),s=e.inWindow||n;return`
    ...
    <p style="color:var(--text-soft);font-size:12px;margin-top:8px">
      ${n} / ${s} leads scanned \xB7 ${e.updated||0} would update/updated \xB7 ${e.errors||0} errors
    </p>
  `}function renderGlobalApplyProgress(e,t,a=0){const n=fbLeadSyncState.clients.find(o=>o.clientId===e),s=t==="backfill"?"90-day backfill":"recent sync",r=a>0?`${a} contact(s)`:"contacts";return`
    <div class="fb-lead-apply-progress-card" id="fb-lead-global-progress-card">
      <div class="fb-lead-apply-progress-header">
        <div>
          <strong>Writing to GHL \u2014 ${esc(s)}</strong>
  ...
  `}function startGlobalApplyProgress(e,t,a=0){const n=document.getElementById("fb-lead-apply-progress-mount");n&&(fbLeadSyncState.isApplying=!0,fbLeadSyncState.applyStartedAt=Date.now(),fbLeadSyncState.applyProgress=null,n.innerHTML=renderGlobalApplyProgress(e,t,a),n.scrollIntoView({behavior:"smooth",block:"nearest"}),fbLeadSyncState.applyWaitTimer&&clearInterval(fbLeadSyncState.applyWaitTimer),fbLeadSyncState.applyWaitTimer=window.setInterval(()=>{if(!fbLeadSyncState.isApplying)return;const s=Math.floor((Date.now()-fbLeadSyncState.applyStartedAt)/1e3),r=fbLeadSyncState.applyProgress;r?.total>0?updateGlobalApplyProgress({...r,waiting:!0,elapsed:s}):updateGlobalApplyProgress({waiting:!0,elapsed:s})},1e3))}function updateGlobalApplyProgress({processed:e,total:t,written:a,waiting:n=!1,elapsed:s=0,complete:r=!1,error:o=""}={}){const i=document.getElementById("fb-lead-global-progress-card"),l=document.getElementById("fb-lead-global-progress"),c=document.getElementById("fb-lead-global-progress-bar"),d=document.getElementById("fb-lead-global-progress-label"),u=document.getElementById("fb-lead-global-progress-stats"),p=document.getElementById("fb-lead-apply-status-badge");if(!i||!l||!c||!d)return;const h=fbLeadSyncState.applyProgress||{},m=e??h.processed??0,y=t??h.total??0,b=a??h.written??0;!r&&!o&&(y>0||m>0)&&(fbLeadSyncState.applyProgress={processed:m,total:y,written:b});const f=y>0||m>0;if(n&&!f&&!r){l.classList.add("is-indeterminate"),c.style.width="35%",u&&(u.textContent="Contacting Meta and GHL\u2026"),d.textContent=`Writing to GHL\u2026 (${s}s \u2014 do not close this page)`;return}l.classList.remove("is-indeterminate");const w=y>0?Math.min(100,Math.round(m/y*100)):r?100:0;c.style.width=`${w}%`,u&&(u.textContent=r?`${b} written to GHL \xB7 ${m} / ${y} contacts processed`:`${b} written \xB7 ${m} / ${y} contacts (${w}%)`),r?(i.classList.add("is-complete"),i.classList.remove("is-error"),d.textContent=`Complete \u2014 ${b} contact(s) updated in GHL`,p&&(p.textContent="Complete",p.classList.add("is-ok"),p.classList.remove("is-missing"))):o?(i.classList.add("is-error"),i.classList.remove("is-complete"),d.textContent=o,p&&(p.textContent="Failed",p.classList.remove("is-ok"),p.classList.add("is-missing"))):n&&f?d.textContent=m>=y?`Finishing up\u2026 (${s}s)`:`Batch in progress \u2014 ${m} / ${y} leads (${w}%) \xB7 ${s}s`:d.textContent=`Writing to GHL \u2014 ${m} / ${y} leads (${w}%)`}function stopGlobalApplyProgress({success:e=!0,keepVisibleMs:t=6e3}={}){fbLeadSyncState.isApplying=!1,fbLeadSyncState.applyProgress=null,fbLeadSyncState.applyWaitTimer&&(clearInterval(fbLeadSyncState.applyWaitTimer),fbLeadSyncState.applyWaitTimer=null),window.setTimeout(()=>{const a=document.getElementById("fb-lead-apply-progress-mount");a&&!fbLeadSyncState.isApplying&&(a.innerHTML="")},t)}function setFbLeadProgress({processed:e=0,total:t=0,complete:a=!1,label:n=""}={}){const s=document.getElementById("fb-lead-progress-wrap"),r=document.getElementById("fb-lead-progress-bar"),o=document.getElementById("fb-lead-progress-label");if(!s||!r)return;s.hidden=!1,s.classList.toggle("is-complete",a);const i=t>0?Math.min(100,Math.round(e/t*100)):a?100:0;r.style.width=`${i}%`,o&&(o.textContent=n||(a?`Complete \u2014 ${e} / ${t} leads (${i}%)`:`Processing \u2014 ${e} / ${t} leads (${i}%)`))}function hideFbLeadProgress(e=!0){const t=document.getElementById("fb-lead-progress-wrap"),a=document.getElementById("fb-lead-progress-bar");t&&e&&(t.hidden=!0,t.classList.remove("is-complete"),a&&(a.style.width="0%"))}function updateFbLeadHistoryClearButton(){const e=document.getElementById("fb-lead-history-clear-all");if(!e)return;const t=(fbLeadSyncState.historyRuns||[]).length>0;e.disabled=!t}function updateFbLeadCronSummaryUi(){const e=document.getElementById("fb-lead-cron-summary-24h");!e||!fbLeadSyncState.cronSummary24h||(e.outerHTML=renderFbLeadCronSummary24h(fbLeadSyncState.cronSummary24h))}async function refreshFbLeadHistoryUi(){await loadFbLeadHistory();const e=document.getElementById("fb-lead-history-mount");if(!e)return;const t=Object.fromEntries(fbLeadSyncState.clients.map(a=>[a.clientId,a.locationId]));e.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,t),updateFbLeadHistoryClearButton()}function bindFbLeadHistoryClearButton(){const e=document.getElementById("fb-lead-history-clear-all");e&&(e.onclick=()=>clearFbLeadSyncHistory());const t=document.getElementById("fb-lead-show-routine-cron");t&&(t.checked=fbLeadSyncState.showRoutineCronRuns,t.onchange=()=>{fbLeadSyncState.showRoutineCronRuns=t.checked;const a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,n)}),updateFbLeadHistoryClearButton()}async function clearFbLeadSyncHistory(){if(!(!isStaffAdmin()||!window.confirm(`Delete all FB lead sync run history?

This only clears the log \u2014 it does not change GHL contacts or Meta data.`)))try{const t=await adminFetch("/api/fb-lead-sync/history",{method:"DELETE"});showToast(`Cleared ${t.deleted||0} run(s).`,"success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(t){showToast(t.message||"Failed to clear FB lead sync log.","error")}}async function deleteFbLeadRun(e){if(!(!isStaffAdmin()||!window.confirm(`Delete run #${e} from history?`)))try{await adminFetch(`/api/fb-lead-sync/history/${e}`,{method:"DELETE"}),showToast("Run deleted.","success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(a){showToast(a.message||"Failed to delete run.","error")}}async function runFbLeadSyncBatch({clientId:e,mode:t,dryRun:a,resumeRunId:n=null}){const s=document.getElementById("fb-lead-preview-btn"),r=document.getElementById("fb-lead-run-results");s&&(s.disabled=!0);const o=a?12:10,i=n!=null?fbLeadSyncState.historyRuns.find(b=>Number(b.id)===Number(n)):null;let l=i?.id||fbLeadSyncState.activeRun?.runId||null,c=i&&Number(i.batchOffset)||0,d=null,u=[],p=!1;const h=n??fbLeadSyncState.activeRun?.previewRunId??null,m=h!=null?fbLeadSyncState.historyRuns.find(b=>Number(b.id)===Number(h)):null,y=i?partialSyncRemaining(i):m?.updated??fbLeadSyncState.previewOkByClient[e]?.updated??0;a||(fbLeadSyncState.applyingRunId=h,startGlobalApplyProgress(e,t,y),await refreshFbLeadHistoryUi());try{if(!n){const $=findPartialSyncRun(e,t,a);if($){const v=partialSyncRemaining($),S=a?"preview":"apply";window.confirm(`A ${S} for ${e} was interrupted (${$.updated||0} processed, ${v} remaining).

Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),v=$.summary||$;l=v.runId,c=0,setFbLeadProgress({processed:0,total:v.inWindow||0,label:`Matching ${v.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:y||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:o})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const v=$.inWindow||c||y;a?(setFbLeadProgress({processed:c,total:v,complete:!$.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:v,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||y,w=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:w,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:w,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const g=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=g.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
        ${renderBrandTopbar("")}
        ${wrapDashboardShell(`
    ...
    `);try{if(e&&fbLeadSyncState.isApplying)return;const[s]=await Promise.all([adminFetch("/api/fb-lead-sync"),loadFbLeadHistory()]);if(fbLeadSyncState.clients=s.clients||[],fbLeadSyncState.cronSummary24h=s.summary?.cronSummary24h||null,hydrateFbLeadPreviewState(fbLeadSyncState.clients),!e){a.innerHTML=renderFbLeadSyncPage(s),bindFbLeadSyncToggles();const r=document.getElementById("fb-lead-sync-refresh");r&&(r.onclick=()=>loadFbLeadSyncPage({fullPreflight:!0})),bindFbLeadHistoryClearButton(),fbLeadSyncRefreshTimer&&(clearInterval(fbLeadSyncRefreshTimer),fbLeadSyncRefreshTimer=null),fbLeadSyncRefreshTimer=window.setInterval(()=>{loadFbLeadSyncPage({silent:!0}).catch(()=>{})},6e4)}(!e||t)&&await loadFbLeadPreflightForClients(fbLeadSyncState.clients,{quick:!t}),e?(updateFbLeadCronSummaryUi(),await refreshFbLeadHistoryUi(),await refreshFbLeadClientRowsUi()):await refreshFbLeadClientRowsUi(),t?(await loadFbLeadPreflightForClients(fbLeadSyncState.clients,{quick:!1}),await refreshFbLeadClientRowsUi()):e||scheduleFbLeadFullPreflight(fbLeadSyncState.clients)}catch(s){e||(a.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("fb-lead-sync"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(s.message)}</div>`)}
      `)}}const metaReportsState={filter:"all",searchQuery:"",dashboardData:null,hubMounted:!1,clientPayload:null,clientPageMounted:!1,clientReportSettingsExpanded:!1,clientShareExpanded:!1,activeMonthKey:null,selectedYear:new Date().getFullYear(),publicPayload:null,publicPageMounted:!1,chartInstances:{roas:null,poas:null},chartScatterInstances:{roas:null,poas:null},chartTab:"trend",chartRoasMode:"kr",chartPoasMode:"kr",chartScatterRoasMode:"kr",chartScatterPoasMode:"kr",chartScenarioRoasMode:"x",chartScenarioPoasMode:"x",chartSeries:null,chartDemo:!1,chartProjection:null,spendChartType:"area",scenarioChartInstances:{roas:null,poas:null},budgetMultiplier:2,budgetBaseline:"year",scenarioMonthWindow:"6",scenarioSmoothUneven:!0,scenarioBlendHistory:!1,scenarioIncludeTrend:!1,comparisonMode:"mom",comparisonPeriodA:{startDate:null,endDate:null},comparisonPeriodB:{startDate:null,endDate:null},comparisonChartMode:"kr",comparisonTab:"table",reportViewMode:"monthly",comparisonYearCache:{},comparisonResult:null,comparisonLoading:!1,comparisonDatePickers:[],comparisonChartInstance:null,customValues:{mounted:!1,overview:null,searchQuery:"",selectedClientId:null,selectedMonthKey:null,editorPayload:null,draftInputs:null,loadingEditor:!1,saving:!1,settingsSaving:!1,settingsSavingScope:null}},META_HUB_FILTER_OPTIONS=[{value:"all",label:"All"},{value:"enabled",label:"Live reports"},{value:"meta-only",label:"Meta only"},{value:"needs-setup",label:"Needs setup"}];function metaHubClientInitial(e){const t=String(e||"?").trim();return esc(t.charAt(0).toUpperCase())}function filterMetaReportsClients(e,t,a=""){let n=e||[];t==="enabled"&&(n=n.filter(r=>r.metaReportEnabled)),t==="meta-only"&&(n=n.filter(r=>!r.hasGhl)),t==="needs-setup"&&(n=n.filter(r=>r.needsSetup));const s=String(a||"").trim().toLowerCase();return s&&(n=n.filter(r=>[r.accountName,r.metaAdAccountId,r.clientId,r.metaName].filter(Boolean).join(" ").toLowerCase().includes(s))),n}function getMetaReportsHubView(e,t,a=metaReportsState.searchQuery){const n=e?.clients||[],s=filterMetaReportsClients(n,t,a),r=e?.summary||{};return{clients:s,allClients:n,summary:{...r,totalListed:n.length},meta:e?.meta||{},filter:t,searchQuery:a}}function renderMetaHubSwitch({label:e,attr:t,value:a,checked:n,disabled:s=!1}){return a?`
    <label class="meta-hub-switch${s?" is-disabled":""}">
      <span class="meta-hub-switch-label">${esc(e)}</span>
      <span class="meta-hub-switch-track">
        <input type="checkbox" class="meta-hub-switch-input" ${t}="${esc(a)}" ${n?"checked":""} ${s?"disabled":""} aria-label="${esc(e)}" />
        <span class="meta-hub-switch-thumb"></span>
      </span>
    </label>
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function metaReportFullUrl(e){return e?`${window.location.origin}${e}`:""}function normalizeMetaReportSlugInput(e){return String(e||"").trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function resolveMetaReportShareSlug(e){const t=e.settings||{};if(t.metaReportSlug)return t.metaReportSlug;const n=String(e.reportUrl||"").replace(/^\/report\//,"").match(/^(.+)-(\d{4})$/);return n?n[1]:""}function resolveMetaReportShareCode(e){const a=String(e.reportUrl||"").replace(/^\/report\//,"").match(/-(\d{4})$/);return a?a[1]:""}function renderMetaReportShareSummary(e,{hidden:t=!1}={}){const n=!!(e.settings||{}).metaReportEnabled,s=resolveMetaReportShareSlug(e),r=resolveMetaReportShareCode(e),o=s?`${s}-${r||"\xB7\xB7\xB7\xB7"}`:"Not set";return`
  ...
  `}function refreshMetaReportClientShareUi(e){syncMetaReportToolbarCopy(e),updateMetaReportShareSummary(e),updateMetaReportBottomlineFeeSummary(e.settings||{},"meta-report"),document.getElementById("meta-report-control-panel")?.classList.toggle("is-share-disabled",!e.settings?.metaReportEnabled);const t=document.getElementById("meta-report-share-url");t&&e.reportUrl&&(t.textContent=metaReportFullUrl(e.reportUrl));const a=document.getElementById("meta-report-share-slug-suffix"),n=resolveMetaReportShareCode(e);a&&(a.textContent=`-${n||"\xB7\xB7\xB7\xB7"}`);const s=document.getElementById("meta-report-share-slug");s&&(s.value=resolveMetaReportShareSlug(e));const r=document.getElementById("meta-report-rotate-token");r&&(r.disabled=!e.settings?.metaReportEnabled);const o=document.querySelector("[data-meta-client-preview]");o&&(o.checked=!!e.settings?.metaReportEnabled),setMetaReportShareExpanded(metaReportsState.clientShareExpanded)}function syncMetaReportShareCard(e){const t=document.getElementById("meta-report-share-editor-wrap");if(!t)return;const a=t.parentElement;if(!a)return;const n=renderMetaReportShareEditor(e),s=document.createElement("div");s.innerHTML=n.trim();const r=s.firstElementChild;r&&(a.replaceChild(r,t),setMetaReportShareExpanded(metaReportsState.clientShareExpanded),bindMetaReportShareEvents(e),syncMetaReportToolbarCopy(e))}function bindMetaReportRotateButton(e){const t=document.getElementById("meta-report-rotate-token");t&&(t.onclick=async()=>{if(window.confirm("Rotate the share link? The old link will stop working.")){t.disabled=!0;try{const a=await patchMetaReportSettings(e,{rotateAccessToken:!0}),n=a.settings?.reportUrl||null,s=metaReportsState.clientPayload;s&&(s.reportUrl=n,a.settings&&(s.settings={...s.settings,...a.settings}));const r=s||{clientId:e,reportUrl:n,settings:a.settings||{}};metaReportsState.clientPayload=r,metaReportsState.clientShareExpanded?refreshMetaReportClientShareUi(r):syncMetaReportShareCard(r),showToast("Share link rotated","success")}catch(a){showToast(a.message||"Rotate failed","error")}finally{const a=document.getElementById("meta-report-rotate-token");a&&(a.disabled=!1)}}})}function bindMetaReportShareEvents(e){const t=e.clientId;bindMetaReportCopyButtons(),bindMetaReportRotateButton(t);const a=document.getElementById("meta-report-share-edit");a&&(a.onclick=()=>setMetaReportShareExpanded(!0));const n=document.getElementById("meta-report-share-done");n&&(n.onclick=()=>setMetaReportShareExpanded(!1));const s=document.getElementById("meta-report-share-save");s&&(s.onclick=async()=>{const i=document.getElementById("meta-report-share-slug"),l=normalizeMetaReportSlugInput(i?.value);if(!l||l.length<2){showToast("Enter a valid slug (at least 2 characters).","error");return}s.disabled=!0;try{const d=(await patchMetaReportSettings(t,{metaReportSlug:l})).settings||{},u=metaReportsState.clientPayload||e;u.settings={...u.settings,...d},u.reportUrl=d.reportUrl||null,metaReportsState.clientPayload=u,refreshMetaReportClientShareUi(u),setMetaReportShareExpanded(!1),showToast("Share link slug saved","success")}catch(c){showToast(c.message||"Save failed","error")}finally{const c=document.getElementById("meta-report-share-save");c&&(c.disabled=!1)}});const r=document.getElementById("meta-report-share-slug");r&&(r.oninput=()=>{r.value=normalizeMetaReportSlugInput(r.value)});const o=document.querySelector("[data-meta-client-preview]");if(o){let i=0;o.onchange=async()=>{const l=++i,c=o.checked,d=!c,u=metaReportsState.clientPayload||e;applyMetaReportEnabledUi(u,c);const p=o.closest(".meta-hub-switch");p?.classList.add("is-saving");try{const h=await patchMetaReportSettings(t,{metaReportEnabled:c},{fast:!0});if(l!==i)return;const m=h.settings||{};u.settings={...u.settings,...m},u.reportUrl=m.reportUrl??u.reportUrl,metaReportsState.clientPayload=u,applyMetaReportEnabledUi(u,!!m.metaReportEnabled),showToast(c?"Client preview enabled":"Client preview disabled","success")}catch(h){if(l!==i)return;applyMetaReportEnabledUi(u,d),o.checked=d,showToast(h.message||"Update failed","error")}finally{l===i&&p?.classList.remove("is-saving")}}}}function renderMetaReportHubCard(e){const t=!!e.clientId,a=e.clientId||e.metaAdAccountId||"",n=e.needsSetup?"is-setup":"",s=e.needsSetup?'<span class="meta-report-badge meta-report-badge--setup">Needs setup</span>':e.metaReportEnabled?'<span class="meta-report-badge meta-report-badge--on">Report live</span>':'<span class="meta-report-badge meta-report-badge--off">Report off</span>',r=e.hasGhl?"GHL + Meta":"Meta only",o=t?`<a href="/admin/meta-reports/${encodeURIComponent(e.clientId)}" class="meta-hub-card-name">${esc(e.accountName)}</a>`:`<span class="meta-hub-card-name">${esc(e.accountName)}</span>`,i=[];e.needsSetup?i.push(`<button type="button" class="admin-btn admin-btn--primary" data-meta-provision="${esc(e.metaAdAccountId)}" data-meta-provision-name="${esc(e.accountName)}">Add client</button>`):t&&(i.push(`<a class="admin-btn" href="/admin/meta-reports/${encodeURIComponent(e.clientId)}">Edit report</a>`),i.push(e.reportUrl?`<button type="button" class="admin-btn admin-btn--secondary" data-copy-report-url="${esc(e.reportUrl)}">Copy link</button>`:`<a class="admin-btn admin-btn--secondary" href="/admin/meta-reports/${encodeURIComponent(e.clientId)}">Enable report</a>`));const l=i.length===1?"meta-hub-card-actions meta-hub-card-actions--single":"meta-hub-card-actions";return`
  ...
  `).join("")}function getMetaReportCalendarYear(e){const t=Number(e?.currentYear);return Number.isFinite(t)&&t>=2e3?t:new Date().getFullYear()}function getMetaReportYearOptions(e){const t=getMetaReportCalendarYear(e),a=t-1,n=new Map;Array.isArray(e?.years)&&e.years.forEach(r=>{const o=Number(r?.year);Number.isFinite(o)&&n.set(o,r.available!==!1)});const s=n.has(a)?n.get(a):e?.previousYearHasData===!0;return[{year:t,available:!0},{year:a,available:!!s}]}function renderMetaReportYearSelectOptions(e,{disableUnavailable:t=!1}={}){const a=Number(metaReportsState.selectedYear)||Number(e?.year);return getMetaReportYearOptions(e).map(({year:n,available:s})=>{const r=t&&!s;return`<option value="${n}"${Number(a)===n?" selected":""}${r?" disabled":""}>${n}${s?"":" (no data)"}</option>`}).join("")}function syncMetaReportSelectedYear(e,{disableUnavailable:t=!1}={}){const a=getMetaReportYearOptions(e),n=Number(metaReportsState.selectedYear),s=a.find(l=>l.year===n);if(s&&(!t||s.available)){metaReportsState.selectedYear=s.year;return}const r=Number(e?.year),o=a.find(l=>l.year===r);if(o&&(!t||o.available)){metaReportsState.selectedYear=o.year;return}const i=a.find(l=>l.available)||a[0];i&&(metaReportsState.selectedYear=i.year)}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;!t||!metaReportsState.activeMonthKey||(t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e,!document.getElementById("meta-report-month-panel"))||(syncMetaReportMonthPanelDom(e,getMetaReportMonthBodyOptions(t.settings,{editable:!0}),t),bindMetaReportsClientEditEvents(t.clientId,t),refreshMetaReportBackfillControl(t))}function refreshMetaReportBackfillControl(e){const t=document.querySelector(".meta-report-client-page .meta-report-toolbar-left"),a=document.querySelector(".meta-report-backfill-wrap"),n=metaReportMonthsNeedingBackfill(e),s=n.length?`${ICON_SYNC} Backfill ${n.length} month${n.length===1?"":"s"} from Meta`:`${ICON_SYNC} Re-sync year from Meta`,r=document.getElementById("meta-report-backfill");if(r)r.innerHTML=s,r.disabled=!1;else if(t){const o=document.createElement("div");o.className="meta-report-backfill-wrap",o.innerHTML=`
    ...
    `).join(""),bindPublicMetaReportTabEvents(e)),document.getElementById("meta-report-month-panel")&&syncMetaReportMonthPanelDom(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(e.settings,{editable:!1}),e)}const META_MONTH_LABELS=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],META_MONTH_LABELS_FULL=["January","February","March","April","May","June","July","August","September","October","November","December"];function metaFmtKr(e){return`Dkr ${(Number(e)||0).toLocaleString("da-DK",{minimumFractionDigits:0,maximumFractionDigits:2})}`}function metaFmtNum(e,t=0){return(Number(e)||0).toLocaleString("da-DK",{minimumFractionDigits:t,maximumFractionDigits:t})}function metaReportFmtBaselineAmount(e){const t=Number(e)||0;return t<=0?"\u2014":metaFmtKr(t)}function metaFmtX(e){const t=Number(e);return Number.isFinite(t)?t===0?"0.00x":`${t.toFixed(2)}x`:"\u2014"}function metaMonthLabel(e){const t=Number(String(e||"").slice(5,7));return META_MONTH_LABELS[t-1]||e}function metaMonthLabelFull(e){const t=Number(String(e||"").slice(5,7));return META_MONTH_LABELS_FULL[t-1]||metaMonthLabel(e)}function metaReportMonthBounds(e){const t=String(e||"").trim();if(!/^\d{4}-\d{2}$/.test(t))return{start:"",end:""};const[a,n]=t.split("-").map(Number),s=new Date(a,n,0).getDate(),r=String(n).padStart(2,"0");return{start:`${a}-${r}-01`,end:`${a}-${r}-${String(s).padStart(2,"0")}`}}function metaReportMonthPeriodAligned(e){if(!e?.monthKey)return!0;const t=metaReportMonthBounds(e.monthKey),a=String(e.periodStart||"").slice(0,10),n=String(e.periodEnd||"").slice(0,10);return!!(a===t.start&&n===t.end||e.metaFetchedAt&&Number(e.meta?.spend)>0)}function resolveMetaReportActiveMonthPayload(e=null){const t=e||metaReportsState.clientPayload||metaReportsState.publicPayload,a=metaReportsState.activeMonthKey;return!t||!a?null:t.months?.[a]||null}function switchMetaReportMonthTab(e,{editable:t=!1}={}){if(!e)return;metaReportsState.reportViewMode==="comparison"&&(metaReportsState.reportViewMode="monthly"),metaReportsState.activeMonthKey=e;const a=metaReportsState.clientPayload||metaReportsState.publicPayload;if(!a)return;document.querySelectorAll("[data-meta-month-tab]").forEach(s=>{s.classList.toggle("is-active",s.getAttribute("data-meta-month-tab")===e)});const{series:n}=resolveMetaReportChartSeries(a);n.length&&(metaReportsState.chartSeries=n),(async()=>{const s=a.months?.[e];if(t&&a.clientId&&s&&!metaReportMonthPeriodAligned(s))try{const r=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(a.clientId)}/months/${encodeURIComponent(e)}`);r?.monthPayload&&(a.months[e]=r.monthPayload)}catch{}if(syncMetaReportMonthPanelDom(null,getMetaReportMonthBodyOptions(a.settings||{},{editable:t,yearPayload:a}),a),t&&a.clientId&&bindMetaReportsClientEditEvents(a.clientId,a),syncMetaReportSettingsControls(a.settings||{},resolveMetaReportActiveMonthPayload(a)),metaReportsState.chartSeries?.length){const r=buildMetaReportScenarioProjection(metaReportsState.chartSeries,a);metaReportsState.chartProjection=r,syncMetaReportBaselineUi(metaReportsState.chartSeries,r,a)}})()}function metaReportMonthsNeedingBackfill(e){return(e?.monthKeys||[]).filter(a=>!e.months?.[a]?.metaFetchedAt)}function metaReportTone(e){const t=Number(e);return!Number.isFinite(t)||t===0?"neutral":t>0?"positive":"negative"}function renderMetaReportHighlightStrip(e){if(!e||e.meta?.emptyMonth)return"";const t=Number(e.topline?.roasX)||0,a=[{label:"Ad spend",value:metaFmtKr(e.meta?.spend),tone:"neutral"},{label:"Leads",value:metaFmtNum(e.topline?.leads),tone:"neutral"},{label:"Return on ad spend",value:metaFmtX(t),tone:metaReportTone(t)}];if(e.bottomline){const n=Number(e.bottomline.feePercent)>0,s=n?e.bottomline.poiKr:e.bottomline.poasKr;a.push({label:n?"Profit on investment":"Profit on ad spend",value:metaFmtKr(s),tone:metaReportTone(s)})}else a.push({label:"Won leads",value:metaFmtNum(e.topline?.wonLeads),tone:"neutral"});return`
    <div class="meta-report-highlight-strip">
      ${a.map(({label:n,value:s,tone:r})=>`
        <div class="meta-report-highlight-item${r&&r!=="neutral"?` is-${r}`:""}">
          <div class="meta-report-highlight-label">${esc(n)}</div>
          <div class="meta-report-highlight-value">${esc(s)}</div>
        </div>
      `).join("")}
    </div>
  `}function renderMetaReportMetricTable(e,t,a="meta",{badge:n="",highlightLastN:s=0}={}){const r=t.map(([o,i,l],c)=>`
      <tr class="${s>0&&c>=t.length-s?`is-highlight accent-${a}`:""}">
  ...
  `}function isMetaReportCensioClient(e){const t=String(e?.clientId||"").toLowerCase(),a=String(e?.accountName||"");return t==="censio"||/censio/i.test(a)}function monthHasChartData(e){if(!e||e.meta?.emptyMonth)return!1;if(!metaReportMonthPeriodAligned(e))return(Number(e.topline?.leads)||0)>0;const t=Number(e.meta?.spend)||0,a=Number(e.topline?.leads)||0;return t>0||a>0}function buildMetaReportChartSeries(e){const t=e?.monthKeys||[],a=e?.months||{},n=[];for(const s of t){const r=a[s];if(!r||r.meta?.emptyMonth||!monthHasChartData(r))continue;const o=Number(r.topline?.roasKr)||0,i=Number(r.topline?.roasX)||0,l=!!r.bottomline,c=l?Number(r.bottomline.poasKr)||0:null,d=l?Number(r.bottomline.poasX)||0:null;n.push({monthKey:s,label:metaMonthLabel(s),periodEnd:r.periodEnd||null,roasKr:o,roasX:i,poasKr:c,poasX:d,spend:Number(r.meta?.spend)||0,leads:Number(r.topline?.leads)||0,wonLeads:Number(r.topline?.wonLeads)||0,avgLeadValue:Number(r.topline?.avgLeadValue)||0,avgProfitPerWon:Number(r.inputs?.avgProfitPerWon)||0,hasData:!0})}return n}function getMetaReportDemoSeries(e,t,a,n){if(!isMetaReportCensioClient({clientId:e,accountName:t}))return null;const r=new Date,o=r.getFullYear(),i=r.getMonth()+1,l=Number(a)||o;let c=(n||[]).filter(d=>{const u=Number(String(d).slice(0,4)),p=Number(String(d).slice(5,7));return!(u!==l||l===o&&p>i)});if(!c.length){const d=l===o?Math.min(8,i):8;c=Array.from({length:d},(u,p)=>`${l}-${String(p+1).padStart(2,"0")}`)}return c.map((d,u)=>{const p=c.length>1?u/(c.length-1):0,h=Math.round(8e3+p*1e4),m=Math.round(2e3+p*15e3),y=Number((.5+p*2.5).toFixed(2)),b=Math.round(1500+p*12e3),f=Number((.4+p*2.2).toFixed(2)),w=Math.round(20+p*80),g=Math.max(1,Math.round(w*(.08+p*.04)));return{monthKey:d,label:metaMonthLabel(d),roasKr:m,roasX:y,poasKr:b,poasX:f,spend:h,leads:w,wonLeads:g,avgLeadValue:85e3,avgProfitPerWon:42e3,hasData:!0,demo:!0}})}function resolveMetaReportChartSeries(e){const t=buildMetaReportChartSeries(e);if(t.length>=META_REPORT_CHART_MIN_POINTS)return{series:t,demo:!1};if(isMetaReportCensioClient(e)){const a=getMetaReportDemoSeries(e.clientId,e.accountName,e.year||metaReportsState.selectedYear,e.monthKeys);if(a&&a.length>=META_REPORT_CHART_MIN_POINTS)return{series:a,demo:!0}}return{series:t,demo:!1}}function metaReportParseAmount(e){const t=Number.parseFloat(e);return Number.isFinite(t)?t:0}function metaReportRoundMoney(e){return Math.round(metaReportParseAmount(e)*100)/100}function metaReportRoundRatio(e){return Math.round(metaReportParseAmount(e)*1e8)/1e8}function metaReportAggregateSeriesEfficiency(e=[]){let t=0,a=0,n=0,s=0,r=0;for(const i of e){const l=metaReportParseAmount(i.spend),c=metaReportParseAmount(i.leads),d=metaReportParseAmount(i.wonLeads);l<=0&&c<=0||(t+=l,a+=c,n+=d,s+=d*metaReportParseAmount(i.avgLeadValue),r+=d*metaReportParseAmount(i.avgProfitPerWon))}const o=e.filter(i=>metaReportParseAmount(i.spend)>0).length;return{avgCpl:a>0?metaReportRoundMoney(t/a):0,winRate:a>0?metaReportRoundRatio(n/a):0,avgLeadValue:n>0?metaReportRoundMoney(s/n):0,avgProfitPerWon:n>0?metaReportRoundMoney(r/n):0,avgMonthlySpend:o>0?metaReportRoundMoney(t/o):0}}const META_SCENARIO_MIN_EFFICIENCY_MONTHS=2,META_SCENARIO_MIN_TREND_MONTHS=4,META_SCENARIO_MAD_MULTIPLIER=2.5,META_SCENARIO_MAX_MONTHLY_TREND=.03;function metaReportScenarioParseDate(e){if(!e)return null;const t=String(e).slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(t))return null;const a=new Date(`${t}T12:00:00.000Z`);return Number.isNaN(a.getTime())?null:a}function metaReportScenarioIsIncompleteMonth(e,t=new Date){if(!metaReportScenarioParseDate(e.periodEnd))return!0;const n=t.toISOString().slice(0,10);return String(e.periodEnd).slice(0,10)>=n}function metaReportScenarioIsAdActive(e){return metaReportParseAmount(e.spend)>0&&metaReportParseAmount(e.leads)>0}function metaReportScenarioMedian(e=[]){const t=e.filter(n=>Number.isFinite(n)).slice().sort((n,s)=>n-s);if(!t.length)return 0;const a=Math.floor(t.length/2);return t.length%2===0?(t[a-1]+t[a])/2:t[a]}function metaReportScenarioMad(e=[]){const t=metaReportScenarioMedian(e);return metaReportScenarioMedian(e.map(a=>Math.abs(a-t)))}function metaReportScenarioLinearRegression(e=[]){const t=e.length;if(t<2)return{slope:0,intercept:0,r:0};let a=0,n=0,s=0,r=0,o=0;for(const p of e){const h=metaReportParseAmount(p.x),m=metaReportParseAmount(p.y);a+=h,n+=m,s+=h*m,r+=h*h,o+=m*m}const i=t*r-a*a;if(i===0)return{slope:0,intercept:n/t,r:0};const l=(t*s-a*n)/i,c=(n-l*a)/t,d=Math.sqrt((t*r-a*a)*(t*o-n*n)),u=d>0?(t*s-a*n)/d:0;return{slope:l,intercept:c,r:u}}function metaReportScenarioEnrichMonth(e){const t=metaReportParseAmount(e.spend),a=metaReportParseAmount(e.leads),n=metaReportParseAmount(e.wonLeads),s=metaReportParseAmount(e.avgLeadValue),r=metaReportParseAmount(e.avgProfitPerWon),o=metaReportRoundMoney(n*s);return{...e,spend:t,leads:a,wonLeads:n,avgLeadValue:s,avgProfitPerWon:r,cpl:a>0?metaReportRoundMoney(t/a):0,totalLeadValue:o,efficiencyIndex:t>0?o/t:0}}function metaReportPrepareScenarioSeries(e=[],{windowMonths:t="6",asOfDate:a=new Date,smoothOutliers:n=!0}={}){const s=normalizeMetaReportScenarioMonthWindow(t)==="all"?1/0:Number.parseInt(normalizeMetaReportScenarioMonthWindow(t),10)||6,r=e.filter(metaReportScenarioIsAdActive).filter(d=>!metaReportScenarioIsIncompleteMonth(d,a)).map(metaReportScenarioEnrichMonth),o=r.length,i=s===1/0?r:r.slice(-s);let l=0,c=i;if(n&&i.length>=3){const d=i.map(b=>metaReportParseAmount(b.cpl)),u=i.map(b=>metaReportParseAmount(b.leads)),p=metaReportScenarioMedian(d),h=metaReportScenarioMedian(u),m=metaReportScenarioMad(d),y=metaReportScenarioMad(u);c=i.map(b=>{const f={...b};let w=!1;const g=metaReportParseAmount(b.leads);if(m>0){const $=p-META_SCENARIO_MAD_MULTIPLIER*m,v=p+META_SCENARIO_MAD_MULTIPLIER*m,S=Math.min(v,Math.max($,metaReportParseAmount(b.cpl)));S!==metaReportParseAmount(b.cpl)&&(f.cpl=metaReportRoundMoney(S),f.leads=Math.max(1,Math.round(metaReportParseAmount(b.spend)/S)),w=!0)}if(y>0){const $=h-META_SCENARIO_MAD_MULTIPLIER*y,v=h+META_SCENARIO_MAD_MULTIPLIER*y,S=Math.min(v,Math.max($,metaReportParseAmount(b.leads)));Math.round(S)!==Math.round(g)&&(f.leads=Math.max(1,Math.round(S)),w=!0)}if(w){l+=1;const $=g>0?metaReportParseAmount(b.wonLeads)/g:0,v=metaReportParseAmount(f.leads);f.wonLeads=Math.max(0,Math.round(v*$)),f.totalLeadValue=metaReportRoundMoney(f.wonLeads*metaReportParseAmount(b.avgLeadValue));const S=metaReportParseAmount(f.spend);f.efficiencyIndex=S>0?f.totalLeadValue/S:0}return f})}return{months:c,monthsAvailable:o,monthsUsed:c.length,outliersAdjusted:l,confidence:metaReportResolveScenarioConfidence(c.length)}}function metaReportScenarioRecencyEfficiency(e=[]){if(e.length<META_SCENARIO_MIN_EFFICIENCY_MONTHS)return null;let t=0,a=0,n=0,s=0,r=0,o=0;return e.forEach((i,l)=>{const c=l+1;t+=metaReportParseAmount(i.spend)*c,a+=metaReportParseAmount(i.leads)*c,n+=metaReportParseAmount(i.wonLeads)*c,s+=metaReportParseAmount(i.wonLeads)*metaReportParseAmount(i.avgLeadValue)*c,r+=metaReportParseAmount(i.wonLeads)*metaReportParseAmount(i.avgProfitPerWon)*c,o+=metaReportParseAmount(i.spend)}),{avgCpl:a>0?metaReportRoundMoney(t/a):0,winRate:a>0?metaReportRoundRatio(n/a):0,avgLeadValue:n>0?metaReportRoundMoney(s/n):0,avgProfitPerWon:n>0?metaReportRoundMoney(r/n):0,avgMonthlySpend:e.length>0?metaReportRoundMoney(o/e.length):0}}function metaReportScenarioPooledEfficiency(e=[]){return metaReportAggregateSeriesEfficiency(e)}function metaReportScenarioBlendEfficiency(e,t,a=.5){if(!e||!t)return e||t;const n=1-a;return{avgCpl:metaReportRoundMoney(e.avgCpl*a+t.avgCpl*n),winRate:metaReportRoundRatio(e.winRate*a+t.winRate*n),avgLeadValue:metaReportRoundMoney(e.avgLeadValue*a+t.avgLeadValue*n),avgProfitPerWon:metaReportRoundMoney(e.avgProfitPerWon*a+t.avgProfitPerWon*n),avgMonthlySpend:metaReportRoundMoney(e.avgMonthlySpend*a+t.avgMonthlySpend*n)}}function metaReportScenarioMonthlyTrend(e=[],t=.75,{dampenHotStreak:a=!1}={}){if(e.length<META_SCENARIO_MIN_TREND_MONTHS)return{trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};const n=e.map(d=>metaReportParseAmount(d.spend)).filter(d=>d>0),s=metaReportScenarioMedian(n)||1,r=e.map((d,u)=>{const p=metaReportParseAmount(d.spend),h=metaReportParseAmount(d.efficiencyIndex),m=h>0?h:1e-6,y=p>0?p:s;return{x:u,y:Math.log(m)-t*Math.log(y/s)}}),{slope:o,r:i}=metaReportScenarioLinearRegression(r);let l=Math.max(-META_SCENARIO_MAX_MONTHLY_TREND,Math.min(META_SCENARIO_MAX_MONTHLY_TREND,o));if(a){const d=e.map(h=>metaReportParseAmount(h.efficiencyIndex)).filter(h=>h>0),u=metaReportScenarioMedian(d),p=e.slice(-3).reduce((h,m)=>h+metaReportParseAmount(m.efficiencyIndex),0)/Math.min(3,e.length);u>0&&p>u*1.15&&l>0&&(l*=.5)}let c="flat";return Math.abs(l)>=.005&&(c=l>0?"up":"down"),{trendRate:l,trendDirection:c,trendRatePct:metaReportRoundRatio(l*100),trendR:metaReportRoundRatio(i)}}function metaReportScenarioCalibrateElasticity(){return META_REPORT_SCENARIO_FIXED_ELASTICITY}function metaReportResolveScenarioConfidence(e=0){const t=Number.parseInt(e,10)||0;return t<4?"low":t<7?"medium":"high"}function metaReportComputeScenarioEfficiency(e=[],{blendHistory:t=!1,includeTrend:a=!1,cautionStrongMonths:n=!1}={}){const s=META_REPORT_SCENARIO_FIXED_ELASTICITY,r=metaReportScenarioCalibrateElasticity(),o=metaReportScenarioRecencyEfficiency(e),i=metaReportScenarioPooledEfficiency(e);if(!o||o.avgCpl<=0)return{efficiency:null,elasticity:r,presetElasticity:s,trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};const l=t?metaReportScenarioBlendEfficiency(i,o,.5):o;let c={trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};if(a){const d=metaReportScenarioMonthlyTrend(e,r,{dampenHotStreak:n});Math.abs(d.trendR)>=META_SCENARIO_TREND_R_THRESHOLD&&(c=d)}return{efficiency:l,elasticity:r,presetElasticity:s,...c}}function metaReportProjectBudgetAtSpend({baselineSpend:e,spend:t,efficiency:a,elasticity:n=.75,trendAdj:s=1,hasBottomline:r=!1}={}){const o=metaReportParseAmount(e),i=metaReportParseAmount(t);if(i<=0||o<=0||!a||a.avgCpl<=0)return{spend:0,leads:0,wonLeads:0,totalLeadValue:0,roasKr:0,roasX:0,poasKr:null,poasX:null};const l=o/a.avgCpl,c=i/o,d=metaReportParseAmount(s)>0?metaReportParseAmount(s):1,u=Math.max(0,Math.round(l*c**n*d)),p=Math.round(u*a.winRate),h=metaReportRoundMoney(p*a.avgLeadValue),m=metaReportRoundMoney(h-i),y=i>0?metaReportRoundRatio(m/i):0;let b=null,f=null;if(r&&a.avgProfitPerWon>0){const w=metaReportRoundMoney(p*a.avgProfitPerWon);b=metaReportRoundMoney(w-i),f=i>0?metaReportRoundRatio(b/i):0}return{spend:i,leads:u,wonLeads:p,totalLeadValue:h,roasKr:m,roasX:y,poasKr:b,poasX:f}}function metaReportSpendFromMonthKey(e=[],t=null,a=null){if(!a)return 0;const n=t?.months?.[a];if(n&&!n.meta?.emptyMonth&&metaReportMonthPeriodAligned(n)){const r=metaReportParseAmount(n.meta?.spend);if(r>0)return r}const s=e.find(r=>r.monthKey===a);return s?metaReportParseAmount(s.spend):0}function buildMetaReportScenarioProjection(e,t){const a=getMetaReportScenarioPillsFromState();return metaReportProjectBudgetScenario({series:e,multiplier:metaReportsState.budgetMultiplier,hasBottomline:payloadHasMetaReportBottomline(t),blendHistory:a.blendHistory,includeTrend:a.includeTrend,monthWindow:metaReportsState.scenarioMonthWindow,baselineMode:metaReportsState.budgetBaseline,activeMonthKey:metaReportsState.activeMonthKey,smoothOutliers:a.smoothUneven})}function resolveMetaReportLastMonthBaseline(e=[],{activeMonthKey:t=null,asOfDate:a=new Date,payload:n=null}={}){const s=e.filter(c=>metaReportParseAmount(c.spend)>0).sort((c,d)=>String(c.monthKey).localeCompare(String(d.monthKey)));if(!s.length)return null;const r=t||metaReportMonthKeyFromDate(a);if(!r)return null;const o=r?metaReportAddMonthsToMonthKey(r,-1):null;if(!o)return null;const i=s.find(c=>c.monthKey===o),l=metaReportSpendFromMonthKey(e,n,o);return!i&&l<=0?null:{monthKey:o,label:metaMonthLabel(o),spend:l>0?l:metaReportParseAmount(i.spend)}}function resolveMetaReportScenarioBaselineSpend(e,t,{baselineMode:a="year",activeMonthKey:n=null,asOfDate:s=new Date,payload:r=null}={}){const o=t?.months||[];if(a==="month"&&n){const i=metaReportSpendFromMonthKey(e,r,n);if(i>0)return i;const l=o.find(c=>c.monthKey===n);return l&&metaReportParseAmount(l.spend)>0?metaReportParseAmount(l.spend):0}if(a==="last"){const i=resolveMetaReportLastMonthBaseline(e,{activeMonthKey:n,asOfDate:s,payload:r});return i?.spend>0?i.spend:0}if(a==="year"&&o.length>0){const i=o.reduce((l,c)=>l+metaReportParseAmount(c.spend),0);return metaReportRoundMoney(i/o.length)}return 0}function metaReportProjectBudgetScenario({series:e=[],baselineSpend:t,multiplier:a=1,hasBottomline:n=!1,blendHistory:s=!1,includeTrend:r=!1,cautionStrongMonths:o=!1,monthWindow:i="6",baselineMode:l="year",activeMonthKey:c=null,asOfDate:d=new Date,smoothOutliers:u=!0}={}){const p=metaReportPrepareScenarioSeries(e,{windowMonths:i,asOfDate:d,smoothOutliers:u}),h=metaReportComputeScenarioEfficiency(p.months,{blendHistory:s,includeTrend:r,cautionStrongMonths:o});let m=metaReportParseAmount(t);m||(m=resolveMetaReportScenarioBaselineSpend(e,p,{baselineMode:l,activeMonthKey:c,asOfDate:d}));const y=metaReportParseAmount(a)||1,{efficiency:b,elasticity:f,trendRate:w}=h;if(!b||b.avgCpl<=0||m<=0||p.monthsUsed<META_SCENARIO_MIN_EFFICIENCY_MONTHS)return{...h,prepared:p,baseline:metaReportProjectBudgetAtSpend({baselineSpend:0,spend:0,efficiency:b}),projected:metaReportProjectBudgetAtSpend({baselineSpend:0,spend:0,efficiency:b}),projectedConservative:null,projectedOptimistic:null,multiplier:y,baselineSpend:m,hasBottomline:n,insufficientData:!0};const g=Math.exp(w*1),$=metaReportProjectBudgetAtSpend({baselineSpend:m,spend:m,efficiency:b,elasticity:f,trendAdj:1,hasBottomline:n}),v=metaReportProjectBudgetAtSpend({baselineSpend:m,spend:metaReportRoundMoney(m*y),efficiency:b,elasticity:f,trendAdj:g,hasBottomline:n}),S=metaReportProjectBudgetAtSpend({baselineSpend:m,spend:metaReportRoundMoney(m*y),efficiency:b,elasticity:f,trendAdj:Math.exp(Math.min(w,0)*1),hasBottomline:n}),L=metaReportProjectBudgetAtSpend({baselineSpend:m,spend:metaReportRoundMoney(m*y),efficiency:b,elasticity:f,trendAdj:Math.exp(Math.max(w,0)*1),hasBottomline:n});return{...h,prepared:p,baseline:$,projected:v,projectedConservative:S,projectedOptimistic:L,multiplier:y,baselineSpend:m,hasBottomline:n,insufficientData:!1}}function metaReportSpendAxisTick(e){const t=Number(e)||0;return Math.abs(t)>=1e6?`${(t/1e6).toFixed(1)}M`:Math.abs(t)>=1e3?`${Math.round(t/1e3)}K`:t}function metaReportSpendMultiplierValue(e,t){const a=metaReportParseAmount(t),n=metaReportParseAmount(e);return a<=0?0:metaReportRoundRatio(n/a)}function resolveMetaReportSpendChartBaseline(e,t=null){const a=metaReportParseAmount(t?.baselineSpend);return a>0?a:metaReportAggregateSeriesEfficiency(e).avgMonthlySpend}function trimMetaReportScenarioSeries(e,t=META_REPORT_SCENARIO_MAX_HISTORICAL){const a=e.filter(n=>metaReportParseAmount(n.spend)>0);return a.length>t?a.slice(-t):a}function metaReportMonthKeyFromDate(e=new Date){const t=e instanceof Date?e:new Date(e);return Number.isNaN(t.getTime())?null:`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}`}function metaReportAddMonthsToMonthKey(e,t=0){const a=String(e||"").trim();if(!/^\d{4}-\d{2}$/.test(a))return null;const[n,s]=a.split("-").map(Number),r=new Date(n,s-1+t,1);return`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,"0")}`}function metaReportBuildScenarioProjectionMonthKeys(e=META_REPORT_SCENARIO_PROJECTED_MONTHS,t=new Date){const a=metaReportMonthKeyFromDate(t);if(!a||e<=0)return[];const n=[];for(let s=1;s<=e;s+=1)n.push(metaReportAddMonthsToMonthKey(a,s));return n}function buildMetaReportScenarioProjectionSteps(e,{months:t=META_REPORT_SCENARIO_PROJECTED_MONTHS,hasBottomline:a=!1,asOfDate:n=new Date,targetMultiplier:s=null}={}){if(!e||e.insufficientData)return[];const r=metaReportParseAmount(e.baselineSpend),o=metaReportParseAmount(s??e.multiplier)||1,i=e.efficiency,l=e.elasticity||.75,c=e.trendRate||0,d=metaReportBuildScenarioProjectionMonthKeys(t,n),u=[];for(let p=1;p<=t;p+=1){const h=o,m=metaReportRoundMoney(r*h),y=Math.exp(c*p),b=metaReportProjectBudgetAtSpend({baselineSpend:r,spend:m,efficiency:i,elasticity:l,trendAdj:y,hasBottomline:a}),f=d[p-1]||null;u.push({label:f?metaMonthLabel(f):`+${p} mo`,monthKey:f,spendMultiplier:metaReportRoundRatio(h),spend:b.spend,leads:b.leads,wonLeads:b.wonLeads,totalLeadValue:b.totalLeadValue,roasKr:b.roasKr,roasX:b.roasX,poasKr:b.poasKr,poasX:b.poasX})}return u}function metaReportDeltaPct(e,t){const a=metaReportParseAmount(t),n=metaReportParseAmount(e);return a?metaReportRoundRatio((n-a)/Math.abs(a)*100):null}function metaReportResolveBaselineSpend(e,t,a=null){if(a?.baselineSpend>0)return metaReportParseAmount(a.baselineSpend);const n=getMetaReportScenarioPillsFromState(),s=a?.prepared||metaReportPrepareScenarioSeries(t,{windowMonths:metaReportsState.scenarioMonthWindow,smoothOutliers:n.smoothUneven});return resolveMetaReportScenarioBaselineSpend(t,s,{baselineMode:metaReportsState.budgetBaseline,activeMonthKey:metaReportsState.activeMonthKey})}function resolveMetaReportScenarioChartSeries(e){const t=metaReportMonthKeyFromDate(new Date),a=e.filter(n=>!(metaReportParseAmount(n.spend)<=0||t&&n.monthKey&&n.monthKey>t));return trimMetaReportScenarioSeries(a)}function resolveMetaReportScenarioBaselineHints(e=[],t=null,a=null){const n=a||metaReportsState.clientPayload||null,s=getMetaReportScenarioPillsFromState(),r=t?.prepared||metaReportPrepareScenarioSeries(e,{windowMonths:metaReportsState.scenarioMonthWindow,smoothOutliers:s.smoothUneven}),o=metaReportsState.activeMonthKey,i=metaReportMonthKeyFromDate(new Date),l=n?.year||metaReportsState.selectedYear||(o?Number(String(o).slice(0,4)):null),c={activeMonthKey:o,asOfDate:new Date,payload:n},d=resolveMetaReportLastMonthBaseline(e,c),u=resolveMetaReportScenarioBaselineSpend(e,r,{baselineMode:"year",...c}),p=resolveMetaReportScenarioBaselineSpend(e,r,{baselineMode:"last",...c}),h=resolveMetaReportScenarioBaselineSpend(e,r,{baselineMode:"month",...c}),m=!!(i&&l&&String(i).startsWith(`${l}-`));return{windowAverage:u,windowMonthCount:r.monthsUsed||r.months.length||0,selectedMonthSpend:h,selectedMonthLabel:o?metaMonthLabel(o):"",activeKey:o,currentMonthSpend:m?metaReportSpendFromMonthKey(e,n,i):0,currentMonthLabel:m?metaMonthLabel(i):"",currentKey:m?i:null,lastMonthSpend:p,lastMonthLabel:d?.label||"",lastMonthKey:d?.monthKey||null}}function renderMetaReportBaselineSelectOptions(e=null,t="year"){const a=e?.windowAverage>0?metaReportFmtBaselineAmount(e.windowAverage):"\u2014",n=e?.lastMonthSpend>0?metaReportFmtBaselineAmount(e.lastMonthSpend):"\u2014",s=e?.selectedMonthSpend>0?metaReportFmtBaselineAmount(e.selectedMonthSpend):"\u2014",r=normalizeMetaReportBudgetBaseline(t),o=e?.lastMonthLabel?`Last month \xB7 ${e.lastMonthLabel}`:"Last month";return`
    <option value="year"${r==="year"?" selected":""}>Window average \xB7 ${esc(a)}</option>
    <option value="last"${r==="last"?" selected":""}>${esc(o)} \xB7 ${esc(n)}</option>
    <option value="month"${r==="month"?" selected":""}>Selected month \xB7 ${esc(s)}</option>
  `}function syncMetaReportBaselineUi(e=[],t=null,a=null){const n=resolveMetaReportScenarioBaselineHints(e,t,a),s=document.getElementById("meta-report-budget-baseline");if(s){const r=s.value||metaReportsState.budgetBaseline;s.innerHTML=renderMetaReportBaselineSelectOptions(n,r),s.value=normalizeMetaReportBudgetBaseline(r)}}function renderMetaReportScenarioContextStrip(e){if(!e||e.insufficientData)return'<p class="meta-report-scenario-context" id="meta-report-scenario-context">Add at least two completed ad months to model a budget scenario.</p>';const t=describeActiveMetaReportScenarioPills(),a=t.length?t.join(" \xB7 "):"Recent months only",n=e.trendDirection==="up"?`trend up ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:e.trendDirection==="down"?`trend down ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:"trend flat",s=e.prepared?.outliersAdjusted?` \xB7 ${e.prepared.outliersAdjusted} uneven month${e.prepared.outliersAdjusted===1?"":"s"} removed`:"",o={low:"Low confidence",medium:"Medium confidence",high:"High confidence"}[e.prepared?.confidence]||"",i=o?` \xB7 ${o} (${e.prepared?.monthsUsed||0} mo data)`:"";return`<p class="meta-report-scenario-context" id="meta-report-scenario-context"><strong>Active:</strong> ${esc(a)} \xB7 last ${e.prepared?.monthsUsed||0} ad months \xB7 ${esc(n)}${esc(s)}${esc(i)}</p>`}function payloadHasMetaReportBottomline(e){return e?.settings?.metaReportShowBottomline===!1?!1:seriesHasBottomline(metaReportsState.chartSeries||buildMetaReportChartSeries(e))}function seriesHasBottomline(e=[]){return e.some(t=>t.poasKr!=null&&t.poasX!=null)}function renderMetaReportChartsPanel({editable:e=!1}={}){const t=metaReportsState.chartTab||"trend";return`
  ...
  `}const META_REPORT_COMPARISON_PRESETS=[{id:"mom",label:"This month vs last month"},{id:"custom",label:"Custom dates"},{id:"ytd",label:"This year vs last year"}],META_REPORT_COMPARISON_TABLE_GROUPS=[{title:"Meta ads",accent:"meta",rows:[{label:"Total spend",metricId:"spend",format:"kr",higherIsBetter:null},{label:"Cost Pr Mile (CPM)",metricId:"cpm",format:"kr",higherIsBetter:!1},{label:"Impressions",metricId:"impressions",format:"num",higherIsBetter:!0},{label:"Reach",metricId:"reach",format:"num",higherIsBetter:!0},{label:"Click",metricId:"clicks",format:"num",higherIsBetter:!0},{label:"CTR",metricId:"conversionRatePercent",format:"pct",higherIsBetter:!0}]},{title:"Topline KPI'er",accent:"topline",highlightLastN:2,rows:[{label:"Leads",metricId:"leads",format:"num",higherIsBetter:!0},{label:"Cost Per Lead (CPL)",metricId:"cpl",format:"kr",higherIsBetter:!1},{label:"Won leads",metricId:"wonLeads",format:"num",higherIsBetter:!0},{label:"Total Lead Value",metricId:"totalLeadValue",format:"kr",higherIsBetter:!0},{label:"Average Lead Value",metricId:"avgLeadValue",format:"kr",higherIsBetter:!0},{label:"Client acquisition cost (CAC)",metricId:"cac",format:"kr",higherIsBetter:!1},{label:"Return on Ads Spend (ROAS)",metricId:"roasKr",format:"kr",higherIsBetter:!0},{label:"Return on Ads Spend % (ROAS)",metricId:"roasX",format:"x",higherIsBetter:!0}]},{title:"Bottomline KPI'er",accent:"bottomline",highlightLastN:2,bottomlineOnly:!0,rows:[{label:"Leads",metricId:"leads",format:"num",higherIsBetter:!0},{label:"Won leads",metricId:"wonLeads",format:"num",higherIsBetter:!0},{label:"Total Lead Value",metricId:"totalLeadValue",format:"kr",higherIsBetter:!0},{label:"Average Lead Value",metricId:"avgLeadValue",format:"kr",higherIsBetter:!0},{label:"Client acquisition cost (CAC)",metricId:"cac",format:"kr",higherIsBetter:!1},{label:"Avg Total Profit",metricId:"totalProfit",format:"kr",higherIsBetter:!0},{label:"Avg Single Profit Order",metricId:"avgProfitPerWon",format:"kr",higherIsBetter:!0},{label:"Profit on Ads Spend (POAS)",metricId:"poasKr",format:"kr",higherIsBetter:!0},{label:"Profit on Ads Spend % (POAS)",metricId:"poasX",format:"x",higherIsBetter:!0},{label:"Censio fee",metricId:"censioFee",format:"kr",higherIsBetter:!1},{label:"Profit on Investment (POI)",metricId:"poiKr",format:"kr",higherIsBetter:!0},{label:"Profit on Investment % (POI)",metricId:"poiX",format:"x",higherIsBetter:!0}]}];function metaReportComparisonAddMonths(e,t=0){const a=String(e||"").trim();if(!/^\d{4}-\d{2}$/.test(a))return null;const[n,s]=a.split("-").map(Number),r=new Date(n,s-1+t,1);return`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,"0")}`}function metaReportComparisonMonthKeysInRange(e,t){if(!e||!t)return[];const[a,n]=e<=t?[e,t]:[t,e],s=[];let r=a;for(;r&&r<=n&&(s.push(r),r!==n);)r=metaReportComparisonAddMonths(r,1);return s}function metaReportComparisonParseDate(e){const t=String(e||"").trim().slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(t))return null;const[a,n,s]=t.split("-").map(Number),r=new Date(a,n-1,s);return r.getFullYear()!==a||r.getMonth()!==n-1||r.getDate()!==s?null:t}function metaReportComparisonDaysInclusive(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n||a>n)return 0;const[s,r,o]=a.split("-").map(Number),[i,l,c]=n.split("-").map(Number),d=Date.UTC(s,r-1,o),u=Date.UTC(i,l-1,c);return Math.floor((u-d)/864e5)+1}function metaReportComparisonOverlapDays(e,t,a,n){const s=e>a?e:a,r=t<n?t:n;return!s||!r||s>r?0:metaReportComparisonDaysInclusive(s,r)}function metaReportComparisonMonthKeyFromDate(e){const t=metaReportComparisonParseDate(e);return t?t.slice(0,7):null}function metaReportComparisonMonthKeysOverlappingDates(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n)return[];const[s,r]=a<=n?[a,n]:[n,a],o=[];let i=metaReportComparisonMonthKeyFromDate(s);const l=metaReportComparisonMonthKeyFromDate(r);for(;i&&i<=l&&(o.push(i),i!==l);)i=metaReportComparisonAddMonths(i,1);return o}function metaReportComparisonMonthEffectivePeriod(e,t){const a=metaReportMonthBounds(t||e?.monthKey),n=metaReportComparisonParseDate(e?.periodStart)||a.start,s=metaReportComparisonParseDate(e?.periodEnd)||a.end;return n&&s&&n<=s?{start:n,end:s}:a}function metaReportComparisonFormatDateLabel(e){const t=metaReportComparisonParseDate(e);if(!t)return"\u2014";const[a,n,s]=t.split("-").map(Number);return`${s} ${META_MONTH_LABELS[n-1]||n} ${a}`}function metaReportComparisonFormatDateRangeLabel(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n)return"\u2014";if(a===n)return metaReportComparisonFormatDateLabel(a);const s=a.slice(0,4),r=n.slice(0,4),o=a.slice(5,7),i=n.slice(5,7);if(s===r&&o===i){const[,l,c]=a.split("-").map(Number),[,,d]=n.split("-").map(Number);return`${c}\u2013${d} ${META_MONTH_LABELS[l-1]||l} ${s}`}return s===r?`${metaReportComparisonFormatDateLabel(a).replace(` ${s}`,"")}\u2013${metaReportComparisonFormatDateLabel(n)}`:`${metaReportComparisonFormatDateLabel(a)} \u2013 ${metaReportComparisonFormatDateLabel(n)}`}function metaReportComparisonMonthPayloadDates(e,t){const a=metaReportComparisonMonthEffectivePeriod(e,t);return{startDate:a.start,endDate:a.end,startMonthKey:t,endMonthKey:t}}function metaReportComparisonFullMonthDates(e){const t=metaReportMonthBounds(e);return{startDate:t.start,endDate:t.end,startMonthKey:e,endMonthKey:e}}function metaReportComparisonFormatDisplayLabel(e,t){const a=metaReportComparisonNormalizePeriod(t);return a?e==="mom"?metaReportComparisonPeriodLabel(a.startMonthKey,a.endMonthKey):e==="ytd"?a.startDate.slice(0,4):metaReportComparisonFormatDateRangeLabel(a.startDate,a.endDate):"\u2014"}function metaReportComparisonAggregateForMode(e,t,a){const n=metaReportComparisonNormalizePeriod(a);if(!n)return{label:"\u2014",monthKeys:[],monthCount:0,expectedMonthCount:0,partialData:!1,hasData:!1,hasBottomline:!1,metrics:metaReportComparisonAggregateDateRange({},null,null).metrics};let s;return e==="mom"?s=metaReportAggregateComparisonRange(t,n.startMonthKey,n.endMonthKey):s=metaReportComparisonAggregateDateRange(t,n.startDate,n.endDate),s.label=metaReportComparisonFormatDisplayLabel(e,n),s}function metaReportComparisonNormalizePeriod(e=null){if(!e)return null;const t=metaReportComparisonParseDate(e.startDate)||(e.startMonthKey?metaReportMonthBounds(e.startMonthKey).start:null),a=metaReportComparisonParseDate(e.endDate)||(e.endMonthKey?metaReportMonthBounds(e.endMonthKey).end:null);if(!t||!a)return null;const[n,s]=t<=a?[t,a]:[a,t];return{startDate:n,endDate:s,startMonthKey:metaReportComparisonMonthKeyFromDate(n),endMonthKey:metaReportComparisonMonthKeyFromDate(s)}}function metaReportComparisonPeriodLabel(e,t){if(!e||!t)return"\u2014";if(e===t)return`${metaMonthLabel(e)} ${String(e).slice(0,4)}`;const a=String(e).slice(0,4),n=String(t).slice(0,4);return a===n?`${metaMonthLabel(e)}\u2013${metaMonthLabel(t)} ${a}`:`${metaMonthLabel(e)} ${a}\u2013${metaMonthLabel(t)} ${n}`}function metaReportMonthHasComparisonData(e){return!e||e.meta?.emptyMonth?!1:metaReportParseAmount(e.meta?.spend)>0||metaReportParseAmount(e.topline?.leads)>0}function metaReportAggregateComparisonRange(e={},t,a){const n=metaReportComparisonMonthKeysInRange(t,a),s=[];let r=!1,o=0,i=0,l=0,c=0,d=0,u=0,p=0,h=0,m=0,y=0,b=0,f=0,w=0;for(const R of n){const A=e[R];if(!metaReportMonthHasComparisonData(A))continue;s.push(R),A.bottomline&&(r=!0);const k=metaReportParseAmount(A.meta?.spend),F=metaReportParseAmount(A.meta?.impressions),O=metaReportParseAmount(A.topline?.leads),x=metaReportParseAmount(A.topline?.wonLeads);o+=k,i+=F,l+=metaReportParseAmount(A.meta?.reach),c+=metaReportParseAmount(A.meta?.clicks),d+=O,u+=x,p+=metaReportParseAmount(A.topline?.totalLeadValue),x>0&&(y+=metaReportParseAmount(A.topline?.avgLeadValue)*x,A.bottomline&&(b+=metaReportParseAmount(A.bottomline.avgProfitPerWon)*x)),F>0&&(f+=metaReportParseAmount(A.meta?.cpm)*F,w+=F),A.bottomline&&(h+=metaReportParseAmount(A.bottomline.totalProfit),m+=metaReportParseAmount(A.bottomline.censioFee))}const g=d>0?Math.round(o/d*100)/100:0,$=u>0?Math.round(o/u*100)/100:0,v=u>0?Math.round(y/u*100)/100:0,S=u>0?Math.round(b/u*100)/100:0,L=w>0?Math.round(f/w*100)/100:0,B=i>0?Math.round(c/i*100*1e8)/1e8:0,P=Math.round((p-o)*100)/100,C=o>0?Math.round(P/o*1e8)/1e8:0,M=Math.round((h-o)*100)/100,E=o>0?Math.round(M/o*1e8)/1e8:0,I=r?Math.round((M-m)*100)/100:0,T=o>0&&r?Math.round(I/o*1e8)/1e8:0;return{startKey:t,endKey:a,label:metaReportComparisonPeriodLabel(t,a),monthKeys:s,monthCount:s.length,expectedMonthCount:n.length,partialData:s.length>0&&s.length<n.length,hasData:s.length>0,hasBottomline:r,metrics:{spend:o,cpm:L,impressions:i,reach:l,clicks:c,conversionRatePercent:B,leads:d,cpl:g,wonLeads:u,totalLeadValue:p,avgLeadValue:v,cac:$,roasKr:P,roasX:C,totalProfit:h,avgProfitPerWon:S,poasKr:M,poasX:E,censioFee:m,poiKr:I,poiX:T}}}function metaReportComparisonAggregateDateRange(e={},t,a){const n=metaReportComparisonParseDate(t),s=metaReportComparisonParseDate(a);if(!n||!s)return{startDate:t||null,endDate:a||null,label:"\u2014",monthKeys:[],monthCount:0,expectedMonthCount:0,partialData:!1,hasData:!1,hasBottomline:!1,metrics:metaReportAggregateComparisonRange({},"0000-00","0000-00").metrics};const[r,o]=n<=s?[n,s]:[s,n],i=metaReportComparisonMonthKeysOverlappingDates(r,o),l=[];let c=!1,d=0,u=0,p=0,h=0,m=0,y=0,b=0,f=0,w=0,g=0,$=0,v=0,S=0,L=0,B=0;for(const K of i){const D=e[K],H=metaReportComparisonMonthEffectivePeriod(D,K),_=metaReportComparisonOverlapDays(r,o,H.start,H.end);if(_<=0||(d+=_,!metaReportMonthHasComparisonData(D)))continue;l.push(K),u+=_,D.bottomline&&(c=!0);const W=metaReportComparisonDaysInclusive(H.start,H.end),N=W>0?_/W:0,j=metaReportParseAmount(D.meta?.spend)*N,V=metaReportParseAmount(D.meta?.impressions)*N,q=metaReportParseAmount(D.topline?.leads)*N,U=metaReportParseAmount(D.topline?.wonLeads)*N;p+=j,h+=V,m+=metaReportParseAmount(D.meta?.reach)*N,y+=metaReportParseAmount(D.meta?.clicks)*N,b+=q,f+=U,w+=metaReportParseAmount(D.topline?.totalLeadValue)*N,U>0&&(v+=metaReportParseAmount(D.topline?.avgLeadValue)*U,D.bottomline&&(S+=metaReportParseAmount(D.bottomline.avgProfitPerWon)*U)),V>0&&(L+=metaReportParseAmount(D.meta?.cpm)*V,B+=V),D.bottomline&&(g+=metaReportParseAmount(D.bottomline.totalProfit)*N,$+=metaReportParseAmount(D.bottomline.censioFee)*N)}const P=b>0?Math.round(p/b*100)/100:0,C=f>0?Math.round(p/f*100)/100:0,M=f>0?Math.round(v/f*100)/100:0,E=f>0?Math.round(S/f*100)/100:0,I=B>0?Math.round(L/B*100)/100:0,T=h>0?Math.round(y/h*100*1e8)/1e8:0,R=Math.round((w-p)*100)/100,A=p>0?Math.round(R/p*1e8)/1e8:0,k=Math.round((g-p)*100)/100,F=p>0?Math.round(k/p*1e8)/1e8:0,O=c?Math.round((k-$)*100)/100:0,x=p>0&&c?Math.round(O/p*1e8)/1e8:0;return{startDate:r,endDate:o,label:metaReportComparisonFormatDateRangeLabel(r,o),monthKeys:l,monthCount:l.length,expectedMonthCount:i.length,partialData:d>0&&u<d,hasData:l.length>0,hasBottomline:c,metrics:{spend:p,cpm:I,impressions:h,reach:m,clicks:y,conversionRatePercent:T,leads:b,cpl:P,wonLeads:f,totalLeadValue:w,avgLeadValue:M,cac:C,roasKr:R,roasX:A,totalProfit:g,avgProfitPerWon:E,poasKr:k,poasX:F,censioFee:$,poiKr:O,poiX:x}}}function metaReportComparisonMergedMonthsMap(e,t,a){const n=metaReportComparisonYearsNeeded(t,a),s={...e?.months||{}};return n.forEach(r=>{Number(e?.year)!==Number(r)&&Object.assign(s,metaReportComparisonMonthsForYear(e,r))}),s}function metaReportResolveComparisonPresetDates(e,t="mom"){const a=metaReportsState.activeMonthKey||null,n=metaReportComparisonMergedMonthsMap(e,null,null);if(t==="custom")return{mode:t,periodA:metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodA),periodB:metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodB)};if(!a)return{mode:t,periodA:null,periodB:null};if(t==="mom"){const s=metaReportComparisonAddMonths(a,-1);return{mode:t,periodA:metaReportComparisonFullMonthDates(a),periodB:s?metaReportComparisonFullMonthDates(s):null}}if(t==="ytd"){const s=String(a).slice(0,4),r=String(a).slice(5,7),o=String(Number(s)-1),i=metaReportMonthBounds(a),l=`${o}-${r}`,c=metaReportMonthBounds(l);return{mode:t,periodA:{startDate:`${s}-01-01`,endDate:i.end,startMonthKey:`${s}-01`,endMonthKey:a},periodB:{startDate:`${o}-01-01`,endDate:c.end,startMonthKey:`${o}-01`,endMonthKey:l}}}return{mode:t,periodA:null,periodB:null}}function metaReportApplyComparisonPresetDefaults(e,t=metaReportsState.comparisonMode){const a=metaReportResolveComparisonPresetDates(e,t);return a.periodA&&(metaReportsState.comparisonPeriodA={...a.periodA}),a.periodB&&(metaReportsState.comparisonPeriodB={...a.periodB}),a}function metaReportResolveComparisonPresets({activeMonthKey:e=null,mode:t="mom"}={}){const a=metaReportsState.clientPayload||metaReportsState.publicPayload;return metaReportResolveComparisonPresetDates(a,t)}function metaReportComparisonMetricDefs(e=!1){const t=[{id:"spend",label:"Ad spend",group:"meta",format:"kr",higherIsBetter:null},{id:"cpm",label:"CPM",group:"meta",format:"kr",higherIsBetter:!1},{id:"impressions",label:"Impressions",group:"meta",format:"num",higherIsBetter:!0},{id:"reach",label:"Reach",group:"meta",format:"num",higherIsBetter:!0},{id:"clicks",label:"Clicks",group:"meta",format:"num",higherIsBetter:!0},{id:"conversionRatePercent",label:"CTR",group:"meta",format:"pct",higherIsBetter:!0},{id:"leads",label:"Leads",group:"topline",format:"num",higherIsBetter:!0},{id:"cpl",label:"CPL",group:"topline",format:"kr",higherIsBetter:!1},{id:"wonLeads",label:"Won leads",group:"topline",format:"num",higherIsBetter:!0},{id:"totalLeadValue",label:"Total lead value",group:"topline",format:"kr",higherIsBetter:!0},{id:"avgLeadValue",label:"Average lead value",group:"topline",format:"kr",higherIsBetter:!0},{id:"cac",label:"CAC",group:"topline",format:"kr",higherIsBetter:!1},{id:"roasKr",label:"ROAS",group:"topline",format:"kr",higherIsBetter:!0},{id:"roasX",label:"ROAS %",group:"topline",format:"x",higherIsBetter:!0}];return e&&t.push({id:"totalProfit",label:"Total profit",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"avgProfitPerWon",label:"Avg profit per won",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poasKr",label:"POAS",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poasX",label:"POAS %",group:"bottomline",format:"x",higherIsBetter:!0},{id:"poiKr",label:"POI",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poiX",label:"POI %",group:"bottomline",format:"x",higherIsBetter:!0}),t}function metaReportComparisonHeroIds(e=!1){return e?["spend","leads","wonLeads","roasKr","totalLeadValue","poasKr"]:["spend","leads","wonLeads","roasKr","totalLeadValue"]}function metaReportComparisonChartIds(e=!1,t="kr"){const a=["spend","leads","totalLeadValue"];return t==="x"?[...a,"roasX",...e?["poasX"]:[]]:[...a,"roasKr",...e?["poasKr"]:[]]}function metaReportBuildYtdByMonthComparison(e,t,a,n){if(!a||!n)return[];const s=metaReportComparisonMonthKeysInRange(a.startMonthKey,a.endMonthKey),r=String(n.startMonthKey||"").slice(0,4);return s.map(o=>{const i=String(o).slice(5,7),l=`${r}-${i}`,c=metaReportAggregateComparisonRange(e,o,o),d=metaReportAggregateComparisonRange(t,l,l);return{monthKey:o,label:metaMonthLabel(o),periodA:c.hasData?c.metrics:null,periodB:d.hasData?d.metrics:null,hasDataA:c.hasData,hasDataB:d.hasData}})}function metaReportBuildComparison({monthsMap:e={},monthsMapA:t=null,monthsMapB:a=null,periodA:n=null,periodB:s=null,mode:r="mom"}={}){const o=t||a||e,i=metaReportComparisonNormalizePeriod(n),l=metaReportComparisonNormalizePeriod(s);if(!i?.startDate||!i?.endDate||!l?.startDate||!l?.endDate)return{mode:r,insufficientData:!0,samePeriod:!1,periodA:null,periodB:null,deltas:{},heroMetrics:[],detailRows:[],ytdByMonth:[],hasBottomline:!1};const c=i.startDate===l.startDate&&i.endDate===l.endDate,d=metaReportComparisonAggregateForMode(r,o,n),u=metaReportComparisonAggregateForMode(r,o,s),p=d.hasBottomline||u.hasBottomline,h=!d.hasData&&!u.hasData,m=metaReportComparisonMetricDefs(p),y={},b=m.map(g=>{const $=d.metrics[g.id],v=u.metrics[g.id],S=metaReportDeltaPct($,v);return y[g.id]={pct:S,abs:Math.round((metaReportParseAmount($)-metaReportParseAmount(v))*100)/100},{...g,valueA:$,valueB:v,deltaPct:S}}),f=metaReportComparisonHeroIds(p).map(g=>{const $=m.find(v=>v.id===g);return{id:g,label:$?.label||g,valueA:d.metrics[g],valueB:u.metrics[g],deltaPct:y[g]?.pct??null,format:$?.format||"num",higherIsBetter:$?.higherIsBetter??null}}),w=r==="ytd"?metaReportBuildYtdByMonthComparison(o,o,i,l):[];return{mode:r,insufficientData:h,samePeriod:c,hasBottomline:p,periodA:d,periodB:u,deltas:y,heroMetrics:f,detailRows:b,ytdByMonth:w}}function metaReportComparisonYearsNeeded(e,t){const a=new Set;for(const n of[e,t]){const s=metaReportComparisonNormalizePeriod(n);s&&(s.startDate&&a.add(s.startDate.slice(0,4)),s.endDate&&a.add(s.endDate.slice(0,4)))}return[...a].filter(Boolean)}function metaReportComparisonMonthsForYear(e,t){return!e||Number(e.year)===Number(t)?e?.months||{}:metaReportsState.comparisonYearCache?.[t]?.months||{}}function metaReportComparisonAvailableMonthKeys(e){const t=new Set,a=n=>{n&&Object.entries(n.months||{}).forEach(([s,r])=>{metaReportMonthHasComparisonData(r)&&t.add(s)})};return a(e),Object.values(metaReportsState.comparisonYearCache||{}).forEach(a),[...t].sort()}function metaReportComparisonPresetDisabled(e,t){const a=metaReportsState.activeMonthKey;return a?e==="mom"?!metaReportComparisonAddMonths(a,-1):!1:!0}function metaReportComparisonFmtValue(e,t="num",a=!0){return a?t==="kr"?metaFmtKr(e):t==="x"?metaFmtX(e):t==="pct"?`${metaFmtNum(e,2)}%`:metaFmtNum(e):"\u2014"}function metaReportComparisonDeltaTone(e,t=null){return e==null||!Number.isFinite(e)||e===0?"neutral":t===!1?e<0?"positive":"negative":e>0?"positive":"negative"}function metaReportComparisonPeriodsFromState(e){let t=metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodA),a=metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodB);if(!t||!a){const n=metaReportApplyComparisonPresetDefaults(e,metaReportsState.comparisonMode);t=n.periodA,a=n.periodB}return{periodA:t,periodB:a,mode:metaReportsState.comparisonMode}}function metaReportComparisonMaps(e,t,a){const n=metaReportComparisonMergedMonthsMap(e,t,a);return{monthsMapA:n,monthsMapB:n}}async function ensureMetaReportComparisonYears(e,{editable:t=!1}={}){const{periodA:a,periodB:n}=metaReportComparisonPeriodsFromState(e),s=metaReportComparisonYearsNeeded(a,n),r=String(e?.year||metaReportsState.selectedYear),o=s.filter(i=>i!==r&&!metaReportsState.comparisonYearCache[i]);if(o.length)for(const i of o)try{let l;t&&e?.clientId?l=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e.clientId)}?year=${encodeURIComponent(i)}`):REPORT_TOKEN&&(l=await fetch(`/api/meta-reports/public/${encodeURIComponent(REPORT_TOKEN)}?year=${encodeURIComponent(i)}`).then(async c=>{const d=await c.json();if(!c.ok)throw new Error(d.error||"Report not found");return d})),l&&(metaReportsState.comparisonYearCache[i]=l)}catch{}}function computeMetaReportComparison(e){const{periodA:t,periodB:a,mode:n}=metaReportComparisonPeriodsFromState(e),s=metaReportComparisonMergedMonthsMap(e,t,a);return metaReportBuildComparison({monthsMap:s,periodA:t,periodB:a,mode:n})}function metaReportComparisonDateInputBounds(e){const t=metaReportComparisonAvailableMonthKeys(e);if(!t.length)return{min:"",max:""};const a=metaReportComparisonMergedMonthsMap(e,null,null);let n="",s="";return t.forEach(r=>{const o=metaReportComparisonMonthEffectivePeriod(a[r],r);(!n||o.start<n)&&(n=o.start),(!s||o.end>s)&&(s=o.end)}),{min:n,max:s}}function renderMetaReportComparisonDateField(e,t,a,n,s){const r=n.min?` data-min="${esc(n.min)}"`:"",o=n.max?` data-max="${esc(n.max)}"`:"";return`
    <label class="meta-report-comparison-date-field">
      <span class="meta-report-comparison-date-field-label">${esc(s)}</span>
      <span class="meta-report-comparison-date-input-wrap">
        <input type="text" class="meta-report-comparison-date-input" data-comparison-date="${esc(e)}-${esc(t)}" value="${esc(a)}" placeholder="Select date" autocomplete="off" inputmode="none" readonly${r}${o} />
  ...
  `}function applyMetaReportComparisonDateChange(e,t,{editable:a=!1}={}){const n=t.getAttribute("data-comparison-date");if(!n||!t.value)return;const[s,r]=n.split("-"),o=`comparisonPeriod${s.toUpperCase()}`;metaReportsState[o]||(metaReportsState[o]={startDate:null,endDate:null});const i=metaReportsState[o];if(i[`${r}Date`]=t.value,i.startDate&&i.endDate&&i.startDate>i.endDate){const l=i.startDate;i.startDate=i.endDate,i.endDate=l;const c=document.querySelector(`[data-comparison-date="${s}-start"]`),d=document.querySelector(`[data-comparison-date="${s}-end"]`);c?._metaComparisonDatePicker&&c._metaComparisonDatePicker.setDate(i.startDate,!1),d?._metaComparisonDatePicker&&d._metaComparisonDatePicker.setDate(i.endDate,!1)}metaReportsState.comparisonMode="custom",syncMetaReportComparisonPeriodPickerBounds(e),document.querySelectorAll("[data-comparison-preset]").forEach(l=>{const c=l.getAttribute("data-comparison-preset")==="custom";l.classList.toggle("active",c),l.setAttribute("aria-pressed",c)}),refreshMetaReportComparison(e,{editable:a})}function syncMetaReportComparisonPeriodPickerBounds(e){const t=metaReportComparisonDateInputBounds(e);["a","b"].forEach(a=>{const n=`comparisonPeriod${a.toUpperCase()}`,s=metaReportsState[n]||{},r=document.querySelector(`[data-comparison-date="${a}-start"]`),o=document.querySelector(`[data-comparison-date="${a}-end"]`);r?._metaComparisonDatePicker&&(r._metaComparisonDatePicker.set("minDate",t.min||void 0),r._metaComparisonDatePicker.set("maxDate",s.endDate||t.max||void 0)),o?._metaComparisonDatePicker&&(o._metaComparisonDatePicker.set("minDate",s.startDate||t.min||void 0),o._metaComparisonDatePicker.set("maxDate",t.max||void 0))})}function closeMetaReportComparisonDatePickers(e=null){(metaReportsState.comparisonDatePickers||[]).forEach(t=>{t!==e&&t.isOpen&&t.close()})}function destroyMetaReportComparisonDatePickers(){closeMetaReportComparisonDatePickers(),document.querySelectorAll("[data-comparison-date]").forEach(e=>{e._metaComparisonDatePicker&&(e._metaComparisonDatePicker.destroy(),e._metaComparisonDatePicker=null)}),metaReportsState.comparisonDatePickers=[]}function bindMetaReportComparisonDateInputs(e,{editable:t=!1}={}){if(destroyMetaReportComparisonDatePickers(),typeof flatpickr>"u")return;const a=metaReportComparisonDateInputBounds(e),{periodA:n,periodB:s}=metaReportComparisonPeriodsFromState(e);document.querySelectorAll("[data-comparison-date]").forEach(r=>{const o=r.getAttribute("data-comparison-date");if(!o)return;const[i,l]=o.split("-"),c=i==="b"?s:n,d=c?.[`${l}Date`]||r.value||"";let u=a.min||r.getAttribute("data-min")||void 0,p=a.max||r.getAttribute("data-max")||void 0;l==="start"&&c?.endDate&&(p=c.endDate),l==="end"&&c?.startDate&&(u=c.startDate);const h=flatpickr(r,{dateFormat:"Y-m-d",altInput:!0,altFormat:"j M Y",defaultDate:d||void 0,minDate:u,maxDate:p,disableMobile:!0,allowInput:!1,clickOpens:!0,animate:!0,static:!1,appendTo:document.body,className:"meta-report-flatpickr",onOpen:(y,b,f)=>{closeMetaReportComparisonDatePickers(f)},onChange:(y,b,f)=>{!b||f._metaComparisonApplying||applyMetaReportComparisonDateChange(e,f.input,{editable:t})}});r._metaComparisonDatePicker=h,metaReportsState.comparisonDatePickers.push(h);const m=r.closest(".meta-report-comparison-date-input-wrap");m&&(m.onclick=y=>{const b=h.altInput;y.target===r||y.target===b||h.open()})})}function metaReportComparisonYtdSpanLabel(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n)return"";const s=Number(a.slice(5,7)),r=Number(n.slice(5,7)),o=a.slice(0,4),i=n.slice(0,4);return o!==i?metaReportComparisonFormatDateRangeLabel(a,n):s===1&&r===12?"Full year":s===r?`Through ${META_MONTH_LABELS[s-1]}`:`${META_MONTH_LABELS[s-1]} \u2013 ${META_MONTH_LABELS[r-1]}`}function metaReportComparisonPresetPeriodDisplay(e,t){const a=metaReportComparisonNormalizePeriod(t);if(!a)return{primary:"\u2014",secondary:""};if(e==="mom"){const n=a.startMonthKey||a.endMonthKey,s=String(n||a.startDate).slice(0,4);return{primary:`${metaMonthLabelFull(n)} ${s}`,secondary:""}}return e==="ytd"?{primary:a.startDate.slice(0,4),secondary:metaReportComparisonYtdSpanLabel(a.startDate,a.endDate)}:{primary:metaReportComparisonFormatDateRangeLabel(a.startDate,a.endDate),secondary:""}}function renderMetaReportComparisonSummaryContext(e,t,a){return e==="mom"?`<p class="meta-report-comparison-summary-context">Comparing <strong>${esc(t.primary)}</strong> with <strong>${esc(a.primary)}</strong></p>`:e==="ytd"?`<p class="meta-report-comparison-summary-context">Comparing <strong>${esc(t.primary)}</strong> with <strong>${esc(a.primary)}</strong> \xB7 year to date</p>`:""}function renderMetaReportComparisonSummaryItem(e,t,a,n=!1){const s=n?'<span class="meta-report-comparison-partial-badge">Partial data</span>':"",r=a.secondary?`<span class="meta-report-comparison-summary-secondary">${esc(a.secondary)}</span>`:"";return`
    <div class="meta-report-comparison-summary-item is-${esc(e)}">
      <div class="meta-report-comparison-summary-item-head">
        <span class="meta-report-comparison-summary-dot" aria-hidden="true"></span>
        <span class="meta-report-comparison-summary-caption">${esc(t)}</span>
        ${s}
      </div>
      <span class="meta-report-comparison-summary-primary">${esc(a.primary)}</span>
      ${r}
    </div>
  `}function renderMetaReportComparisonSummaryBar(e,t,a=null){const n=metaReportsState.comparisonMode||"mom",s=a?.periodA?.partialData===!0,r=a?.periodB?.partialData===!0,o=metaReportComparisonPresetPeriodDisplay(n,e),i=metaReportComparisonPresetPeriodDisplay(n,t),l=n==="mom"?"This month":n==="ytd"?"This year":"Period A",c=n==="mom"?"Previous month":n==="ytd"?"Last year":"Period B";return`
    <div class="meta-report-comparison-summary-bar-wrap" id="meta-report-comparison-range-panel">
      ${renderMetaReportComparisonSummaryContext(n,o,i)}
      <div class="meta-report-comparison-summary-bar">
        ${renderMetaReportComparisonSummaryItem("a",l,o,s)}
        <div class="meta-report-comparison-summary-vs" aria-hidden="true">vs</div>
        ${renderMetaReportComparisonSummaryItem("b",c,i,r)}
      </div>
    </div>
  `}function renderMetaReportComparisonCustomDatesPanel(e,t,a){const n=metaReportComparisonDateInputBounds(a),s=e?.startDate||n.min||"",r=e?.endDate||s,o=t?.startDate||n.min||"",i=t?.endDate||o;return`
    <div class="meta-report-comparison-custom-dates" id="meta-report-comparison-range-panel">
      <div class="meta-report-comparison-custom-period is-a">
        <span class="meta-report-comparison-custom-period-caption">Period A</span>
        <div class="meta-report-comparison-custom-period-dates">
          ${renderMetaReportComparisonDateField("a","start",s,n,"From")}
          ${renderMetaReportComparisonDateField("a","end",r,n,"To")}
        </div>
      </div>
      ...
      <div class="meta-report-comparison-custom-period is-b">
        <span class="meta-report-comparison-custom-period-caption">Period B</span>
        <div class="meta-report-comparison-custom-period-dates">
          ${renderMetaReportComparisonDateField("b","start",o,n,"From")}
          ${renderMetaReportComparisonDateField("b","end",i,n,"To")}
        </div>
      </div>
    </div>
  `}function renderMetaReportComparisonPeriodControls(e,t,a,n=null){return(metaReportsState.comparisonMode||"mom")==="custom"?renderMetaReportComparisonCustomDatesPanel(e,t,a):renderMetaReportComparisonSummaryBar(e,t,n)}function renderMetaReportComparisonPeriodTiles(e,t,a,n=null){return renderMetaReportComparisonPeriodControls(e,t,a,n)}function renderMetaReportComparisonChangeCell(e,t,a=!0,n=!0){const s=metaReportComparisonDeltaTone(e,t);if(!a&&!n||!a||!n||e==null||!Number.isFinite(e))return'<span class="meta-report-comparison-change is-neutral">\u2014</span>';const r=e>0?"\u2191":e<0?"\u2193":"\u2014",o=metaReportFormatDeltaPct(e);return`<span class="meta-report-comparison-change is-${s}">${esc(r)} ${esc(o)}</span>`}function renderMetaReportComparisonGroupTable(e,t,a,n,{highlightLastN:s=0}={}){if(!n||n.insufficientData||n.samePeriod)return"";const r=n.periodA.label,o=n.periodB.label,i=n.periodA.hasData!==!1,l=n.periodB.hasData!==!1,c=t.map((d,u)=>{const p=n.periodA.metrics[d.metricId],h=n.periodB.metrics[d.metricId],m=i&&l?metaReportDeltaPct(p,h):null;return`
      <tr class="${s>0&&u>=t.length-s?`is-highlight accent-${a}`:""}">
        ...
        <table class="meta-report-group-table meta-report-comparison-table">
          <thead>
            <tr>
              ...
              <th scope="col" class="meta-report-comparison-col-b">${esc(o)}</th>
  ...
  `}function renderMetaReportComparisonView(){const e=metaReportsState.clientPayload||metaReportsState.publicPayload,t=metaReportsState.comparisonResult||computeMetaReportComparison(e),{periodA:a,periodB:n}=metaReportComparisonPeriodsFromState(e),s=metaReportsState.comparisonTab||"table",r=metaReportsState.comparisonLoading;return`
    <section class="meta-report-comparison-view" id="meta-report-comparison-view">
      <div class="meta-report-comparison-controls">
        <div class="meta-report-comparison-presets" id="meta-report-comparison-presets">
          ${META_REPORT_COMPARISON_PRESETS.map(({id:o,label:i})=>{const l=metaReportComparisonPresetDisabled(o,e),c=l&&o==="mom"?"No previous month available":"";return`
              <button type="button" class="preset-btn${metaReportsState.comparisonMode===o?" active":""}${l?" is-disabled":""}" data-comparison-preset="${esc(o)}" aria-pressed="${metaReportsState.comparisonMode===o}"${l?" disabled":""}${c?` title="${esc(c)}"`:""}>${esc(i)}</button>
            `}).join("")}
        </div>
        <div class="meta-report-comparison-period-card${metaReportsState.comparisonMode==="custom"?" is-custom":""}">
          <div id="meta-report-comparison-range-wrap">
            ${renderMetaReportComparisonPeriodControls(a,n,e,t)}
          </div>
        </div>
      </div>
      <div class="meta-report-comparison-view-card${r?" is-loading":""}">
  ...
  `}function destroyMetaReportComparisonChart(){metaReportsState.comparisonChartInstance&&(metaReportsState.comparisonChartInstance.destroy(),metaReportsState.comparisonChartInstance=null)}function metaReportComparisonChartLabel(e,t){return t.find(a=>a.id===e)?.label||e}function buildMetaReportComparisonGroupedBarChart(e,t,a="kr"){const n=metaReportComparisonMetricDefs(t.hasBottomline),s=metaReportComparisonChartIds(t.hasBottomline,a),r=s.map(c=>metaReportComparisonChartLabel(c,n)),o=s.map(c=>metaReportParseAmount(t.periodA.metrics[c])),i=t.periodB.hasData!==!1,l=s.map(c=>i?metaReportParseAmount(t.periodB.metrics[c]):null);return new Chart(e,{type:"bar",data:{labels:r,datasets:[{label:t.periodA.label,data:o,backgroundColor:"rgba(255, 106, 0, 0.82)",borderColor:"#ff6a00",borderWidth:1,borderRadius:6,maxBarThickness:42},{label:t.periodB.label,data:l,backgroundColor:"rgba(148, 163, 184, 0.82)",borderColor:"#94a3b8",borderWidth:1,borderRadius:6,maxBarThickness:42}]},options:{responsive:!0,maintainAspectRatio:!1,layout:{padding:{left:8,right:12,top:0,bottom:0}},interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"top",labels:{boxWidth:12,font:{size:11}}},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,callbacks:{label(c){const d=c.parsed.y,u=s[c.dataIndex],p=n.find(h=>h.id===u);return`${c.dataset.label}: ${metaReportComparisonFmtValue(d,p?.format||"num")}`}}}},scales:{x:{grid:{display:!1},ticks:{color:"#6b5348",font:{size:11},maxRotation:45,minRotation:0}},y:{grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:c=>{const d=Number(c)||0;return Math.abs(d)>=1e6?`${(d/1e6).toFixed(1)}M`:Math.abs(d)>=1e3?`${Math.round(d/1e3)}K`:d}}}}}})}function metaReportComparisonYtdLineMetricId(e,t){return t==="x"?e?"poasX":"roasX":e?"poasKr":"totalLeadValue"}function buildMetaReportComparisonYtdLineChart(e,t,a="kr"){const n=t.ytdByMonth||[];if(!n.length)return null;const s=metaReportComparisonYtdLineMetricId(t.hasBottomline,a),o=metaReportComparisonMetricDefs(t.hasBottomline).find(u=>u.id===s),i=n.map(u=>u.label),l=n.map(u=>u.hasDataA&&u.periodA?metaReportParseAmount(u.periodA[s]):null),c=n.map(u=>u.hasDataB&&u.periodB?metaReportParseAmount(u.periodB[s]):null);return l.some(u=>u!=null)||c.some(u=>u!=null)?new Chart(e,{type:"line",data:{labels:i,datasets:[{label:t.periodA.label,data:l,borderColor:"#ff6a00",backgroundColor:"rgba(255, 106, 0, 0.08)",borderWidth:2,pointBackgroundColor:"#ff6a00",pointRadius:3,tension:.3,spanGaps:!1},{label:t.periodB.label,data:c,borderColor:"#94a3b8",backgroundColor:"rgba(148, 163, 184, 0.08)",borderWidth:2,pointBackgroundColor:"#94a3b8",pointRadius:3,tension:.3,spanGaps:!1}]},options:{responsive:!0,maintainAspectRatio:!1,layout:{padding:{left:8,right:12,top:0,bottom:0}},interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"top",labels:{boxWidth:12,font:{size:11}}},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,callbacks:{label(u){const p=u.parsed.y;return p==null?`${u.dataset.label}: \u2014`:`${u.dataset.label}: ${metaReportComparisonFmtValue(p,o?.format||"num")}`}}},title:{display:!0,text:o?.label||s,color:"#6b5348",font:{size:12,weight:"600"},padding:{bottom:8}}},scales:{x:{grid:{display:!1},ticks:{color:"#6b5348",font:{size:11}}},y:{grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:u=>{const p=Number(u)||0;return a==="x"?`${p.toFixed(2)}x`:Math.abs(p)>=1e6?`${(p/1e6).toFixed(1)}M`:Math.abs(p)>=1e3?`${Math.round(p/1e3)}K`:p}}}}}}):null}function buildMetaReportComparisonChart(e,t,a="kr"){const n=document.getElementById("meta-report-comparison-chart-empty");if(!t||t.insufficientData||t.samePeriod)return n&&(n.hidden=!1),null;let s=null;return t.mode==="ytd"?s=buildMetaReportComparisonYtdLineChart(e,t,a):s=buildMetaReportComparisonGroupedBarChart(e,t,a),n&&(n.hidden=!!s),s}function mountMetaReportComparisonChart(e){if(destroyMetaReportComparisonChart(),(metaReportsState.comparisonTab||"table")!=="chart")return;const t=document.getElementById("meta-report-comparison-chart");t&&(metaReportsState.comparisonChartInstance=buildMetaReportComparisonChart(t,e,metaReportsState.comparisonChartMode))}function setMetaReportComparisonTab(e){if(metaReportsState.comparisonTab=e==="chart"?"chart":"table",document.querySelectorAll("[data-comparison-tab]").forEach(t=>{const a=t.getAttribute("data-comparison-tab")===metaReportsState.comparisonTab;t.classList.toggle("is-active",a),t.setAttribute("aria-selected",a)}),document.querySelectorAll("[data-comparison-panel]").forEach(t=>{const a=t.getAttribute("data-comparison-panel")===metaReportsState.comparisonTab;t.classList.toggle("is-active",a),t.hidden=!a}),metaReportsState.comparisonTab==="chart"){const t=metaReportsState.clientPayload||metaReportsState.publicPayload;mountMetaReportComparisonChart(metaReportsState.comparisonResult||computeMetaReportComparison(t))}else destroyMetaReportComparisonChart()}function setMetaReportViewMode(e,t,{editable:a=!1}={}){if(metaReportsState.reportViewMode=e==="comparison"?"comparison":"monthly",document.querySelectorAll("[data-report-view-mode]").forEach(n=>{const s=n.getAttribute("data-report-view-mode")===metaReportsState.reportViewMode;n.classList.toggle("is-active",s),n.setAttribute("aria-selected",s)}),document.querySelectorAll("[data-report-view-panel]").forEach(n=>{const s=n.getAttribute("data-report-view-panel")===metaReportsState.reportViewMode;n.classList.toggle("is-active",s),n.hidden=!s}),metaReportsState.reportViewMode==="monthly")destroyMetaReportComparisonChart(),mountMetaReportTrendCharts(t,{editable:a,clientId:t?.clientId||null}),a&&syncMetaReportScenario(t);else if(destroyMetaReportTrendCharts(),t){const{series:n}=resolveMetaReportChartSeries(t);n.length&&(metaReportsState.chartSeries=n),bindMetaReportComparisonControls(t,{editable:a}),refreshMetaReportComparison(t,{editable:a}),a&&(bindMetaReportScenarioControls(t,{editable:!0}),syncMetaReportScenario(t))}}function bindMetaReportViewModeTabs(e,{editable:t=!1}={}){document.querySelectorAll("[data-report-view-mode]").forEach(a=>{a.onclick=()=>{const n=a.getAttribute("data-report-view-mode");!n||n===metaReportsState.reportViewMode||setMetaReportViewMode(n,e,{editable:t})}})}function syncMetaReportComparisonUi(e,{editable:t=!1}={}){const a=document.getElementById("meta-report-comparison-view");if(!a)return;const n=computeMetaReportComparison(e);metaReportsState.comparisonResult=n;const s=document.getElementById("meta-report-comparison-headline-wrap");s&&s.remove();const r=a.querySelector(".meta-report-comparison-view-card");r&&r.classList.toggle("is-loading",!!metaReportsState.comparisonLoading);const o=document.getElementById("meta-report-comparison-table-wrap");o&&(o.innerHTML=renderMetaReportComparisonTables(n));const i=document.getElementById("meta-report-comparison-chart-empty");i&&(i.hidden=!(n.insufficientData||n.samePeriod)),(metaReportsState.comparisonTab||"table")==="chart"&&mountMetaReportComparisonChart(n)}async function refreshMetaReportComparison(e,{editable:t=!1}={}){if(!e||!document.getElementById("meta-report-comparison-view"))return;const a=Date.now();metaReportsState.comparisonLoading=!0,syncMetaReportComparisonUi(e,{editable:t});try{await ensureMetaReportComparisonYears(e,{editable:t})}finally{const n=Date.now()-a;n<180&&await new Promise(s=>setTimeout(s,180-n)),metaReportsState.comparisonLoading=!1,syncMetaReportComparisonUi(e,{editable:t})}}function bindMetaReportComparisonControls(e,{editable:t=!1}={}){const a=()=>{document.querySelectorAll("[data-comparison-preset]").forEach(n=>{n.onclick=async()=>{if(n.disabled)return;const s=n.getAttribute("data-comparison-preset");if(!s)return;metaReportsState.comparisonMode=s,metaReportApplyComparisonPresetDefaults(e,s),metaReportsState.comparisonLoading=!0;const r=document.getElementById("meta-report-comparison-view");r&&(r.outerHTML=renderMetaReportComparisonView(),bindMetaReportComparisonControls(e,{editable:t})),await refreshMetaReportComparison(e,{editable:t})}})};document.querySelectorAll("[data-comparison-tab]").forEach(n=>{n.onclick=()=>{const s=n.getAttribute("data-comparison-tab");s&&setMetaReportComparisonTab(s)}}),document.querySelectorAll("#meta-report-comparison-chart-mode .meta-report-chart-toggle-btn").forEach(n=>{n.onclick=()=>{metaReportsState.comparisonChartMode=n.getAttribute("data-mode")||"kr",document.querySelectorAll("#meta-report-comparison-chart-mode .meta-report-chart-toggle-btn").forEach(s=>{s.classList.toggle("is-active",s===n)}),mountMetaReportComparisonChart(metaReportsState.comparisonResult||computeMetaReportComparison(e))}}),a(),bindMetaReportComparisonDateInputs(e,{editable:t}),setMetaReportComparisonTab(metaReportsState.comparisonTab||"table")}function mountMetaReportComparison(e,{editable:t=!1}={}){document.getElementById("meta-report-comparison-view")&&(bindMetaReportComparisonControls(e,{editable:t}),refreshMetaReportComparison(e,{editable:t}))}function renderMetaReportScenarioCard(){return`
  ...
  `}function metaReportFormatDeltaPct(e){return e==null||!Number.isFinite(e)?"":`${e>0?"+":""}${e.toFixed(0)}%`}function metaReportScenarioDeltaTone(e=""){return String(e).startsWith("+")?"positive":String(e).startsWith("-")?"negative":"neutral"}function renderMetaReportScenarioOutcomeCard({label:e,projectedValue:t,projectedSub:a="",baselineValue:n="",delta:s=""}={}){const r=metaReportScenarioDeltaTone(s);return`
    <div class="meta-report-scenario-outcome-card ${r==="negative"?"is-negative":"is-positive"}">
      ...
      ${s?`<div class="meta-report-scenario-outcome-delta is-${r}">${esc(s)}</div>`:""}
      ${n?`<div class="meta-report-scenario-outcome-baseline">Baseline ${esc(n)}</div>`:""}
    </div>
  `}function renderMetaReportScenarioDetailRow(e,t,a,n){const s=metaReportScenarioDeltaTone(n);return`
  ...
  `}function destroyMetaReportTrendCharts(){["roas","poas"].forEach(e=>{metaReportsState.chartInstances[e]&&(metaReportsState.chartInstances[e].destroy(),metaReportsState.chartInstances[e]=null),metaReportsState.chartScatterInstances[e]&&(metaReportsState.chartScatterInstances[e].destroy(),metaReportsState.chartScatterInstances[e]=null)})}function destroyMetaReportCharts(){destroyMetaReportTrendCharts(),destroyMetaReportScenarioCharts(),destroyMetaReportComparisonChart()}function destroyMetaReportScenarioCharts(){["roas","poas"].forEach(e=>{metaReportsState.scenarioChartInstances[e]&&(metaReportsState.scenarioChartInstances[e].destroy(),metaReportsState.scenarioChartInstances[e]=null)})}function metaReportChartValue(e,t,a,n){const s=t==="x"?e[n]:e[a];return Number(s)||0}function metaReportChartTooltipLabel(e,t){return t==="x"?metaFmtX(e):metaFmtKr(e)}function metaReportChartAxisTick(e,t){if(t==="x")return`${Number(e).toFixed(1)}x`;const a=Number(e)||0;return Math.abs(a)>=1e6?`${(a/1e6).toFixed(1)}M`:Math.abs(a)>=1e3?`${Math.round(a/1e3)}K`:a}function buildMetaReportLineChart(e,t,a,n,s){const r=t.map(i=>i.label),o=t.map(i=>metaReportChartValue(i,a,n,s));return new Chart(e,{type:"line",data:{labels:r,datasets:[{label:a==="x"?"Multiplier":"Dkr",data:o,borderColor:"#ff6a00",backgroundColor:"rgba(255, 106, 0, 0.12)",borderWidth:2.5,pointBackgroundColor:"#ff6a00",pointBorderColor:"#ffffff",pointBorderWidth:2,pointRadius:4,pointHoverRadius:5,fill:!0,tension:.3}]},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!1},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,callbacks:{label(i){return metaReportChartTooltipLabel(i.parsed.y,a)}}}},scales:{x:{grid:{color:"rgba(26, 18, 8, 0.06)"},ticks:{color:"#6b5348",font:{size:11}}},y:{grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:i=>metaReportChartAxisTick(i,a)}}}}})}function buildMetaReportDualMetricChart(e,{series:t,mode:a,krKey:n,xKey:s,returnLabel:r,chartType:o="area",projection:i=null,projectionLabel:l="Projected",showProjection:c=!1,baselineSpend:d=null}={}){const u=t.filter(C=>metaReportParseAmount(C.spend)>0),p=u.map(C=>C.label),h=metaReportParseAmount(d??i?.baselineSpend??0)||resolveMetaReportSpendChartBaseline(u,i),m=a==="x"&&h>0,y=u.map(C=>m?metaReportSpendMultiplierValue(C.spend,h):metaReportParseAmount(C.spend)),b=u.map(C=>metaReportChartValue(C,a,n,s)),f=normalizeMetaReportSpendChartType(o),w=a==="x",g=u.length-1;c&&i&&!i.insufficientData&&(p.push(l||`Projected (${Number(i.multiplier).toFixed(1)}\xD7)`),y.push(metaReportParseAmount(m?i.multiplier:i.projected.spend)),b.push(metaReportParseAmount(i.projected[a==="x"?s:n])));const $=p.length-1,v=c&&i&&!i.insufficientData&&$>g;function S(C,M){const E=v&&C===$;return{pointBackgroundColor:M,pointBorderColor:"#ffffff",pointBorderWidth:2,pointRadius:E?7:f==="scatter"?5:4,pointHoverRadius:E?8:f==="scatter"?6:5,pointStyle:E?"rectRot":"circle"}}function L(C,M,E,I,T){const R=M.map((k,F)=>S(F,E)),A={label:C,data:M,borderColor:E,yAxisID:T};return f==="bar"?{...A,type:"bar",backgroundColor:E==="#138b53"?"rgba(19, 139, 83, 0.72)":"rgba(255, 106, 0, 0.72)",borderRadius:4,borderWidth:0}:f==="scatter"?{...A,type:"line",backgroundColor:"transparent",borderWidth:0,showLine:!1,fill:!1,pointBackgroundColor:R.map(k=>k.pointBackgroundColor),pointBorderColor:R.map(k=>k.pointBorderColor),pointBorderWidth:R.map(k=>k.pointBorderWidth),pointRadius:R.map(k=>k.pointRadius),pointHoverRadius:R.map(k=>k.pointHoverRadius),pointStyle:R.map(k=>k.pointStyle)}:{...A,type:"line",backgroundColor:I,borderWidth:2.5,fill:f==="area",tension:.3,pointBackgroundColor:R.map(k=>k.pointBackgroundColor),pointBorderColor:R.map(k=>k.pointBorderColor),pointBorderWidth:R.map(k=>k.pointBorderWidth),pointRadius:R.map(k=>k.pointRadius),pointHoverRadius:R.map(k=>k.pointHoverRadius),pointStyle:R.map(k=>k.pointStyle)}}const B=[L("Ad spend",y,"#138b53","rgba(19, 139, 83, 0.16)","ySpend"),L(r,b,"#ff6a00","rgba(255, 106, 0, 0.12)",w?"yReturn":"ySpend")];if(v&&f!=="bar"){const C={type:"line",borderWidth:2,fill:!1,tension:0,pointRadius:0,pointHoverRadius:0,spanGaps:!0};B.push({...C,label:"",data:p.map((M,E)=>E===g||E===$?y[E]:null),borderColor:"rgba(19, 139, 83, 0.55)",borderDash:[5,4],yAxisID:"ySpend"}),B.push({...C,label:"",data:p.map((M,E)=>E===g||E===$?b[E]:null),borderColor:"rgba(255, 106, 0, 0.55)",borderDash:[5,4],yAxisID:w?"yReturn":"ySpend"})}const P={x:{grid:{color:"rgba(26, 18, 8, 0.06)"},ticks:{color:"#6b5348",font:{size:11},maxRotation:45,minRotation:0}},ySpend:{type:"linear",position:"left",title:{display:!0,text:m?"Ad spend (\xD7)":w?"Ad spend (Dkr)":"Dkr",color:w?"#138b53":"#6b5348",font:{size:11,weight:"600"}},grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:w?"#138b53":"#6b5348",font:{size:11},callback:C=>m?metaReportChartAxisTick(C,"x"):metaReportSpendAxisTick(C)}}};return w&&(P.yReturn={type:"linear",position:"right",title:{display:!0,text:"Multiplier",color:"#ff6a00",font:{size:11,weight:"600"}},grid:{drawOnChartArea:!1},ticks:{color:"#ff6a00",font:{size:11},callback:C=>metaReportChartAxisTick(C,"x")}}),new Chart(e,{type:f==="bar"?"bar":"line",data:{labels:p,datasets:B},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"top",align:"end",labels:{boxWidth:10,boxHeight:10,font:{size:11,weight:"600"},color:"#6b5348",filter(C){return!!C.text}}},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,filter(C){return!!C.dataset.label},callbacks:{label(C){const M=C.dataset.label||"";return C.dataset.yAxisID==="yReturn"?`${M}: ${metaReportChartTooltipLabel(C.parsed.y,"x")}`:m&&C.dataset.yAxisID==="ySpend"?`${M}: ${metaReportChartTooltipLabel(C.parsed.y,"x")}`:C.datasetIndex===1&&!w?`${M}: ${metaReportChartTooltipLabel(C.parsed.y,a)}`:`${M}: ${metaFmtKr(C.parsed.y)}`}}}},scales:P}})}function buildMetaReportScenarioExtrapolationChart(e,{series:t,mode:a,kind:n,projection:s,chartType:r="area",hasBottomline:o=!1}={}){const i=n==="roas"?"roasKr":"poasKr",l=n==="roas"?"roasX":"poasX",c=n==="roas"?"ROAS":"POAS",d=resolveMetaReportScenarioChartSeries(n==="poas"?t.filter(M=>M.poasKr!=null&&M.poasX!=null):t),u=resolveMetaReportSpendChartBaseline(d,s),p=a==="x"&&u>0,h=buildMetaReportScenarioProjectionSteps(s,{hasBottomline:o,targetMultiplier:metaReportsState.budgetMultiplier}),m=normalizeMetaReportSpendChartType(r),y=d.map(M=>M.label),b=d.map(M=>p?metaReportSpendMultiplierValue(M.spend,u):metaReportParseAmount(M.spend)),f=d.map(M=>metaReportChartValue(M,a,i,l)),w=d.length-1,g=y.length;for(const M of h)y.push(M.label),b.push(p?metaReportSpendMultiplierValue(M.spend,u):M.spend),f.push(metaReportParseAmount(M[a==="x"?l:i]));const $=new Set(h.map((M,E)=>g+E)),v=h.length>0;function S(M,E){const I=$.has(M);return{pointBackgroundColor:E,pointBorderColor:"#ffffff",pointBorderWidth:2,pointRadius:I?7:m==="scatter"?5:4,pointHoverRadius:I?8:m==="scatter"?6:5,pointStyle:I?"rectRot":"circle"}}function L(M,E,I,T){const R=E.map((k,F)=>S(F,I)),A={label:M,data:E,borderColor:I,yAxisID:"y"};return m==="bar"?{...A,type:"bar",backgroundColor:I==="#138b53"?"rgba(19, 139, 83, 0.72)":"rgba(255, 106, 0, 0.72)",borderRadius:4,borderWidth:0}:m==="scatter"?{...A,type:"line",backgroundColor:"transparent",borderWidth:0,showLine:!1,fill:!1,pointBackgroundColor:R.map(k=>k.pointBackgroundColor),pointBorderColor:R.map(k=>k.pointBorderColor),pointBorderWidth:R.map(k=>k.pointBorderWidth),pointRadius:R.map(k=>k.pointRadius),pointHoverRadius:R.map(k=>k.pointHoverRadius),pointStyle:R.map(k=>k.pointStyle)}:{...A,type:"line",backgroundColor:T,borderWidth:2.5,fill:m==="area",tension:.3,pointBackgroundColor:R.map(k=>k.pointBackgroundColor),pointBorderColor:R.map(k=>k.pointBorderColor),pointBorderWidth:R.map(k=>k.pointBorderWidth),pointRadius:R.map(k=>k.pointRadius),pointHoverRadius:R.map(k=>k.pointHoverRadius),pointStyle:R.map(k=>k.pointStyle)}}const B=[L("Ad spend",b,"#138b53","rgba(19, 139, 83, 0.16)"),L(c,f,"#ff6a00","rgba(255, 106, 0, 0.12)")];if(v&&m!=="bar"){const M={type:"line",borderWidth:2,fill:!1,tension:0,pointRadius:0,pointHoverRadius:0,spanGaps:!0,yAxisID:"y"},E=I=>y.map((T,R)=>R===w||$.has(R)?I[R]:null);B.push({...M,label:"",data:E(b),borderColor:"rgba(19, 139, 83, 0.55)",borderDash:[5,4]}),B.push({...M,label:"",data:E(f),borderColor:"rgba(255, 106, 0, 0.55)",borderDash:[5,4]})}const P=p?"Multiplier (\xD7)":"Dkr",C=M=>p?metaReportChartAxisTick(M,"x"):metaReportChartAxisTick(M,"kr");return new Chart(e,{type:m==="bar"?"bar":"line",data:{labels:y,datasets:B},options:{responsive:!0,maintainAspectRatio:!1,interaction:{mode:"index",intersect:!1},plugins:{legend:{display:!0,position:"top",align:"end",labels:{boxWidth:10,boxHeight:10,font:{size:11,weight:"600"},color:"#6b5348",filter(M){return!!M.text}}},tooltip:{backgroundColor:"#ffffff",borderColor:"#e8e0d8",borderWidth:1,titleColor:"#1a1208",bodyColor:"#1a1208",padding:10,filter(M){return!!M.dataset.label},callbacks:{label(M){return`${M.dataset.label||""}: ${metaReportChartTooltipLabel(M.parsed.y,a)}`}}}},scales:{x:{grid:{color:"rgba(26, 18, 8, 0.06)"},ticks:{color:"#6b5348",font:{size:11},maxRotation:45,minRotation:0}},y:{type:"linear",position:"left",title:{display:!0,text:P,color:"#6b5348",font:{size:11,weight:"600"}},grid:{color:"rgba(26, 18, 8, 0.08)"},ticks:{color:"#6b5348",font:{size:11},callback:C}}}}})}function updateMetaReportChart(e,t,a){const n=document.getElementById(`meta-report-chart-${e}`),s=document.getElementById(`meta-report-chart-${e}-empty`);if(!n||!s)return;const r=e==="roas"?"roasKr":"poasKr",o=e==="roas"?"roasX":"poasX",i=e==="poas"?t.filter(l=>l.poasKr!=null&&l.poasX!=null):t;if(metaReportsState.chartInstances[e]&&(metaReportsState.chartInstances[e].destroy(),metaReportsState.chartInstances[e]=null),i.length<META_REPORT_CHART_MIN_POINTS||typeof Chart>"u"){n.hidden=!0,s.hidden=!1;return}n.hidden=!1,s.hidden=!0,metaReportsState.chartInstances[e]=buildMetaReportLineChart(n,i,a,r,o)}function updateMetaReportSpendReturnChart(e,t,a,{projection:n=null,showProjection:s=!1,projectionLabel:r="Projected",chartType:o=metaReportsState.spendChartType,target:i="spend"}={}){const l=i==="scenario"?`meta-report-scenario-chart-${e}`:`meta-report-chart-scatter-${e}`,c=i==="scenario"?`meta-report-scenario-chart-${e}-empty`:`meta-report-chart-scatter-${e}-empty`,d=document.getElementById(l),u=document.getElementById(c);if(!d||!u)return;const p=e==="roas"?"roasKr":"poasKr",h=e==="roas"?"roasX":"poasX",m=e==="poas"?t.filter(f=>f.poasKr!=null&&f.poasX!=null):t.filter(f=>metaReportParseAmount(f.spend)>0),y=i==="scenario"?"scenarioChartInstances":"chartScatterInstances";if(metaReportsState[y][e]&&(metaReportsState[y][e].destroy(),metaReportsState[y][e]=null),m.length<META_REPORT_CHART_MIN_POINTS||typeof Chart>"u"){d.hidden=!0,u.hidden=!1;return}if(s&&n?.insufficientData){d.hidden=!0,u.hidden=!1;return}d.hidden=!1,u.hidden=!0;const b=a==="x"?resolveMetaReportSpendChartBaseline(m,n):null;metaReportsState[y][e]=buildMetaReportDualMetricChart(d,{series:m,mode:a,krKey:p,xKey:h,returnLabel:e==="roas"?"ROAS":"POAS",chartType:o,projection:n,projectionLabel:r,showProjection:s,baselineSpend:b})}function updateMetaReportScatterChart(e,t,a){updateMetaReportSpendReturnChart(e,t,a,{showProjection:!1,chartType:metaReportsState.spendChartType,target:"spend"})}function updateMetaReportScenarioChart(e,t,a,n){const s=document.getElementById(`meta-report-scenario-chart-${e}`),r=document.getElementById(`meta-report-scenario-chart-${e}-empty`);if(!s||!r)return;const o=e==="poas"?t.filter(l=>l.poasKr!=null&&l.poasX!=null):t.filter(l=>metaReportParseAmount(l.spend)>0),i=resolveMetaReportScenarioChartSeries(o);if(metaReportsState.scenarioChartInstances[e]&&(metaReportsState.scenarioChartInstances[e].destroy(),metaReportsState.scenarioChartInstances[e]=null),i.length<META_REPORT_CHART_MIN_POINTS||typeof Chart>"u"||a?.insufficientData){s.hidden=!0,r.hidden=!1;return}s.hidden=!1,r.hidden=!0,metaReportsState.scenarioChartInstances[e]=buildMetaReportScenarioExtrapolationChart(s,{series:o,mode:n,kind:e,projection:a,chartType:metaReportsState.spendChartType,hasBottomline:e==="poas"})}function mountMetaReportScenarioCharts(e){const t=metaReportsState.chartSeries||[],a=payloadHasMetaReportBottomline(e),n=document.getElementById("meta-report-scenario-poas-wrap");n&&(n.hidden=!a);const s=metaReportsState.chartProjection;updateMetaReportScenarioChart("roas",t,s,metaReportsState.chartScenarioRoasMode),a?updateMetaReportScenarioChart("poas",t,s,metaReportsState.chartScenarioPoasMode):metaReportsState.scenarioChartInstances.poas&&(metaReportsState.scenarioChartInstances.poas.destroy(),metaReportsState.scenarioChartInstances.poas=null)}function syncMetaReportSpendChartTypeToolbar(e=metaReportsState.chartTab||"trend"){const t=document.getElementById("meta-report-spend-chart-type-wrap");t&&(t.hidden=e!=="spend")}function setMetaReportChartTab(e){metaReportsState.chartTab=e,document.querySelectorAll("[data-meta-chart-tab]").forEach(t=>{const a=t.getAttribute("data-meta-chart-tab")===e;t.classList.toggle("is-active",a),t.setAttribute("aria-selected",a?"true":"false")}),document.querySelectorAll("[data-meta-chart-panel]").forEach(t=>{const a=t.getAttribute("data-meta-chart-panel")===e;t.classList.toggle("is-active",a),t.hidden=!a}),syncMetaReportSpendChartTypeToolbar(e),mountActiveMetaReportCharts()}function mountActiveMetaReportCharts(){const e=metaReportsState.chartSeries||[];metaReportsState.chartTab==="spend"?(updateMetaReportScatterChart("roas",e,metaReportsState.chartScatterRoasMode),updateMetaReportScatterChart("poas",e,metaReportsState.chartScatterPoasMode)):(updateMetaReportChart("roas",e,metaReportsState.chartRoasMode),updateMetaReportChart("poas",e,metaReportsState.chartPoasMode))}function refreshMetaReportSpendAndScenarioCharts(e){mountActiveMetaReportCharts(),document.getElementById("meta-report-scenario-card")&&e?syncMetaReportScenario(e):mountMetaReportScenarioCharts(e)}function setMetaReportSpendChartType(e,{persist:t=!1,clientId:a=null,payload:n=null}={}){const s=normalizeMetaReportSpendChartType(e);metaReportsState.spendChartType=s,document.querySelectorAll("[data-spend-chart-type]").forEach(o=>{o.classList.toggle("is-active",o.getAttribute("data-spend-chart-type")===s)});const r=n||metaReportsState.clientPayload||metaReportsState.publicPayload;return refreshMetaReportSpendAndScenarioCharts(r),t&&a?patchMetaReportSettings(a,{metaReportSpendChartType:s}):Promise.resolve(null)}function bindMetaReportChartTabs(){document.querySelectorAll("[data-meta-chart-tab]").forEach(e=>{e.onclick=()=>{setMetaReportChartTab(e.getAttribute("data-meta-chart-tab"))}})}function bindMetaReportChartToggles(){const e=document.getElementById("meta-chart-roas-mode");e&&e.querySelectorAll("[data-mode]").forEach(o=>{o.onclick=()=>{const i=o.getAttribute("data-mode");metaReportsState.chartRoasMode=i,e.querySelectorAll("[data-mode]").forEach(l=>{l.classList.toggle("is-active",l.getAttribute("data-mode")===i)}),updateMetaReportChart("roas",metaReportsState.chartSeries||[],i)}});const t=document.getElementById("meta-chart-poas-mode");t&&t.querySelectorAll("[data-mode]").forEach(o=>{o.onclick=()=>{const i=o.getAttribute("data-mode");metaReportsState.chartPoasMode=i,t.querySelectorAll("[data-mode]").forEach(l=>{l.classList.toggle("is-active",l.getAttribute("data-mode")===i)}),updateMetaReportChart("poas",metaReportsState.chartSeries||[],i)}});const a=document.getElementById("meta-chart-scatter-roas-mode");a&&a.querySelectorAll("[data-mode]").forEach(o=>{o.onclick=()=>{const i=o.getAttribute("data-mode");metaReportsState.chartScatterRoasMode=i,a.querySelectorAll("[data-mode]").forEach(l=>{l.classList.toggle("is-active",l.getAttribute("data-mode")===i)}),updateMetaReportScatterChart("roas",metaReportsState.chartSeries||[],i)}});const n=document.getElementById("meta-chart-scatter-poas-mode");n&&n.querySelectorAll("[data-mode]").forEach(o=>{o.onclick=()=>{const i=o.getAttribute("data-mode");metaReportsState.chartScatterPoasMode=i,n.querySelectorAll("[data-mode]").forEach(l=>{l.classList.toggle("is-active",l.getAttribute("data-mode")===i)}),updateMetaReportScatterChart("poas",metaReportsState.chartSeries||[],i)}});const s=document.getElementById("meta-scenario-roas-mode");s&&s.querySelectorAll("[data-mode]").forEach(o=>{o.onclick=()=>{const i=o.getAttribute("data-mode");metaReportsState.chartScenarioRoasMode=i,s.querySelectorAll("[data-mode]").forEach(l=>{l.classList.toggle("is-active",l.getAttribute("data-mode")===i)}),updateMetaReportScenarioChart("roas",metaReportsState.chartSeries||[],metaReportsState.chartProjection,i)}});const r=document.getElementById("meta-scenario-poas-mode");r&&r.querySelectorAll("[data-mode]").forEach(o=>{o.onclick=()=>{const i=o.getAttribute("data-mode");metaReportsState.chartScenarioPoasMode=i,r.querySelectorAll("[data-mode]").forEach(l=>{l.classList.toggle("is-active",l.getAttribute("data-mode")===i)}),updateMetaReportScenarioChart("poas",metaReportsState.chartSeries||[],metaReportsState.chartProjection,i)}})}function bindMetaReportSpendChartTypePicker(e,t){document.querySelectorAll("#meta-report-spend-chart-type-wrap [data-spend-chart-type]").forEach(a=>{a.onclick=async()=>{const n=a.getAttribute("data-spend-chart-type");if(!(!n||n===metaReportsState.spendChartType)&&(setMetaReportSpendChartType(n,{payload:t}),!!e))try{const s=await patchMetaReportSettings(e,{metaReportSpendChartType:normalizeMetaReportSpendChartType(n)});s?.settings&&t&&(t.settings={...t.settings,...s.settings},metaReportsState.clientPayload?.clientId===t.clientId&&(metaReportsState.clientPayload.settings=t.settings))}catch(s){showToast(s.message||"Could not save chart type default","error")}}})}function syncMetaReportChartToggleUi(){[["roas",metaReportsState.chartRoasMode,"meta-chart-roas-mode"],["poas",metaReportsState.chartPoasMode,"meta-chart-poas-mode"],["scatter-roas",metaReportsState.chartScatterRoasMode,"meta-chart-scatter-roas-mode"],["scatter-poas",metaReportsState.chartScatterPoasMode,"meta-chart-scatter-poas-mode"],["scenario-roas",metaReportsState.chartScenarioRoasMode,"meta-scenario-roas-mode"],["scenario-poas",metaReportsState.chartScenarioPoasMode,"meta-scenario-poas-mode"]].forEach(([,e,t])=>{const a=document.getElementById(t);a&&a.querySelectorAll("[data-mode]").forEach(n=>{n.classList.toggle("is-active",n.getAttribute("data-mode")===e)})})}function scheduleMetaReportScenarioSettingsPersist(e){e&&(clearTimeout(metaReportsState._scenarioSettingsTimer),metaReportsState._scenarioSettingsTimer=setTimeout(()=>{patchMetaReportSettings(e,{metaReportBudgetMultiplier:metaReportsState.budgetMultiplier,metaReportBudgetBaseline:metaReportsState.budgetBaseline,metaReportScenarioMonthWindow:metaReportsState.scenarioMonthWindow,metaReportScenarioSmoothUneven:metaReportsState.scenarioSmoothUneven,metaReportScenarioBlendHistory:metaReportsState.scenarioBlendHistory,metaReportScenarioIncludeTrend:metaReportsState.scenarioIncludeTrend},{fast:!0}).catch(()=>{})},350))}function bindMetaReportScenarioControls(e,{editable:t=!1}={}){if(!t)return;const a=e?.clientId||null,n=document.getElementById("meta-report-budget-multiplier"),s=document.getElementById("meta-report-budget-multiplier-value"),r=document.getElementById("meta-report-budget-baseline"),o=document.getElementById("meta-report-scenario-month-window"),i=()=>scheduleMetaReportScenarioSettingsPersist(a),l=()=>syncMetaReportScenario(e);n&&(n.oninput=()=>{metaReportsState.budgetMultiplier=normalizeMetaReportBudgetMultiplier(n.value),s&&(s.textContent=`${metaReportsState.budgetMultiplier.toFixed(1)}\xD7`),l(),i()}),r&&(r.onchange=()=>{metaReportsState.budgetBaseline=normalizeMetaReportBudgetBaseline(r.value),l(),i()}),o&&(o.onchange=()=>{metaReportsState.scenarioMonthWindow=normalizeMetaReportScenarioMonthWindow(o.value),l(),i()});const c=()=>{document.querySelectorAll("[data-scenario-pill]").forEach(d=>{d.onclick=()=>{const u=d.getAttribute("data-scenario-pill");if(!u||d.disabled)return;metaReportsState[u]=!metaReportsState[u],applyMetaReportScenarioPillsToState(getMetaReportScenarioPillsFromState());const p=document.getElementById("meta-report-scenario-pills-wrap");p&&(p.outerHTML=renderMetaReportScenarioModelPills(),c()),l(),i()}})};c()}function syncMetaReportScenario(e){const t=metaReportsState.chartSeries||[],a=buildMetaReportScenarioProjection(t,e);if(metaReportsState.chartProjection=a,syncMetaReportBaselineUi(t,a,e),!document.getElementById("meta-report-scenario-card"))return;const s=document.getElementById("meta-report-scenario-context-wrap");s&&(s.innerHTML=renderMetaReportScenarioContextStrip(a));const r=document.getElementById("meta-report-scenario-grid");r&&(r.innerHTML=renderMetaReportScenarioMetrics(a)),mountMetaReportScenarioCharts(e)}function mountMetaReportTrendCharts(e,{editable:t=!1,clientId:a=null}={}){const n=document.getElementById("meta-report-charts-panel");if(!e||!n)return;hydrateMetaReportSpendChartType(e),hydrateMetaReportScenarioSettings(e);const{series:s,demo:r}=resolveMetaReportChartSeries(e);metaReportsState.chartSeries=s,metaReportsState.chartDemo=r;const o=document.getElementById("meta-report-chart-demo-badge");o&&(o.hidden=!(r&&t)),bindMetaReportChartTabs(),bindMetaReportChartToggles(),t&&(bindMetaReportScenarioControls(e,{editable:!0}),bindMetaReportSpendChartTypePicker(a,e)),syncMetaReportChartToggleUi(),syncMetaReportSpendChartTypeToolbar(metaReportsState.chartTab||"trend"),document.querySelectorAll("[data-spend-chart-type]").forEach(i=>{i.classList.toggle("is-active",i.getAttribute("data-spend-chart-type")===metaReportsState.spendChartType)}),setMetaReportChartTab(metaReportsState.chartTab||"trend"),t&&syncMetaReportScenario(e)}function mountMetaReportCharts(e,{editable:t=!1,clientId:a=null}={}){if(destroyMetaReportCharts(),destroyMetaReportComparisonChart(),!e)return;const n=metaReportsState.reportViewMode||"monthly";if(bindMetaReportViewModeTabs(e,{editable:t}),n==="monthly"){mountMetaReportTrendCharts(e,{editable:t,clientId:a});return}if(mountMetaReportComparison(e,{editable:t}),t){const{series:s}=resolveMetaReportChartSeries(e);s.length&&(metaReportsState.chartSeries=s),bindMetaReportScenarioControls(e,{editable:!0}),syncMetaReportScenario(e)}}function syncMetaReportMonthPanelDom(e,t={},a=null){const n=a||metaReportsState.clientPayload||metaReportsState.publicPayload,s=metaReportsState.activeMonthKey,r=resolveMetaReportActiveMonthPayload(n)||e,o=document.getElementById("meta-report-month-panel");o&&(o.innerHTML=renderMetaReportMonthBody(r,{...t,yearPayload:t.yearPayload||n,activeMonthKey:s||r?.monthKey||null}),requestAnimationFrame(()=>{mountMetaReportCharts(n,{editable:!!t.editable,clientId:n?.clientId||null})}))}function renderMetaReportMonthTables(e,{editable:t=!1}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return`<div class="meta-report-empty">No ad spend recorded for this month yet.${t?' Click "Refresh from Meta" below, or use "Backfill from Meta" above to pull historical months.':""}</div>`;const a=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["CTR",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],n=[["Leads",metaFmtNum(e.topline?.leads),""],["Cost Per Lead (CPL)",metaFmtKr(e.topline?.cpl),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let s="";if(s+=renderMetaReportMetricTable("Meta ads",a,"meta"),s+=renderMetaReportMetricTable("Topline KPI'er",n,"topline",{highlightLastN:2}),e.bottomline){const r=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];if(e.bottomline.feeMode){const o=e.bottomline.feeLabel||(e.bottomline.feeMode==="marketing"?"Censio marketing fee":`Censio performance fee (${metaFmtNum(e.bottomline.feePercent,0)}%)`);r.push([o,metaFmtKr(e.bottomline.censioFee),""]),r.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),r.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])}s+=renderMetaReportMetricTable("Bottomline KPI'er",r,"bottomline",{highlightLastN:2})}return s?`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null}={}){const s=n||e?.monthKey||null,r=s?metaReportMonthBounds(s):null;let o="";t&&e&&!e.meta?.emptyMonth&&r&&(o=`
  ...
  `:"",p=renderMetaReportMonthTables(e,{editable:t}),h=d?renderMetaReportChartsPanel({editable:t}):"",m=t&&d?renderMetaReportScenarioCard():"",y=d?renderMetaReportViewModeTabs():"";let b;return d?b=`
      ...
      ${m}
    `:b=`<div class="meta-report-content-main">${p}</div>`,`${o}${u}${y}${b}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
    ...
    `)}
  `}function renderMetaReportsClientPage(e){const t=e.monthKeys||[],a=metaReportsState.activeMonthKey||t[t.length-1]||"",n=e.months?.[a]||null,s=e.settings||{},r=t.map(o=>`
    <button type="button" class="meta-report-tab${o===a?" is-active":""}" data-meta-month-tab="${esc(o)}">${esc(metaMonthLabel(o))}</button>
  `).join("");return`
    ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
    ${wrapDashboardShell(`
    <div class="page-hero admin-hub-hero meta-premium-page-hero">
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      ...
      <div class="meta-report-month-panel" id="meta-report-month-panel">
        ${renderMetaReportMonthBody(n,getMetaReportMonthBodyOptions(s,{editable:!0,yearPayload:e}))}
      </div>
    </div>
    `)}
  `}function metaReportClientDisplayName(e){return String(e?.accountName||e?.clientId||"").trim()||"Meta report"}function syncMetaReportPublicBranding(e){const t=metaReportClientDisplayName(e);document.title=`${t} \xB7 Censio Analytics`;const a=document.querySelector(".meta-report-public-hero h1");a&&(a.textContent=t)}function renderPublicMetaReportPage(e){const t=metaReportClientDisplayName(e),a=e.monthKeys||[],n=metaReportsState.activeMonthKey||a[a.length-1]||"",s=e.months?.[n]||null,r=a.map(o=>`
    <button type="button" class="meta-report-tab${o===n?" is-active":""}" data-meta-month-tab="${esc(o)}">${esc(metaMonthLabel(o))}</button>
  `).join("");return`
    ...
    <div class="meta-reports-page meta-reports-page--public">
      ...
      <div class="meta-report-tabs" role="tablist">${r}</div>
      <div class="meta-report-month-panel" id="meta-report-month-panel">
        ${renderMetaReportMonthBody(s,getMetaReportMonthBodyOptions(e.settings,{editable:!1,yearPayload:e}))}
      </div>
    </div>
  ...
  `}function setMetaReportYearLoadingBanner(e,t,a){const n=document.querySelector(e);if(!n)return;let s=n.querySelector(".meta-report-year-loading-banner");a?(s||(s=document.createElement("span"),s.className="meta-report-year-loading-banner",s.setAttribute("aria-live","polite"),n.appendChild(s)),s.innerHTML=`<span class="meta-report-save-indicator-spinner"></span> Loading ${esc(t)}\u2026`,s.hidden=!1):s&&(s.hidden=!0)}function setMetaReportClientYearLoading(e,t){const a=document.querySelector(".meta-report-client-page"),n=document.getElementById("meta-report-year");if(t){n&&(n.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!0);const s=document.querySelector(".meta-report-tabs");s&&(s.innerHTML=renderMetaReportTabsSkeleton(),s.setAttribute("aria-busy","true"));const r=document.getElementById("meta-report-month-panel");r&&(r.innerHTML=renderMetaReportMonthPanelSkeleton(),r.classList.add("is-loading")),destroyMetaReportCharts()}else{n&&(n.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!1);const s=document.querySelector(".meta-report-tabs");s&&s.removeAttribute("aria-busy");const r=document.getElementById("meta-report-month-panel");r&&r.classList.remove("is-loading")}}function setMetaCvYearLoading(e,t){const a=document.querySelector(".sync-history-page.meta-reports-page"),n=document.getElementById("meta-cv-year"),s=document.getElementById("meta-cv-search");if(t){n&&(n.disabled=!0),s&&(s.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!0);const r=document.querySelector(".meta-cv-summary");r&&(r.classList.add("is-skeleton"),r.innerHTML=renderMetaCvSummarySkeleton());const o=document.querySelector(".meta-cv-legend");o&&o.setAttribute("aria-hidden","true");const i=document.getElementById("meta-cv-client-list");i&&(i.innerHTML=renderMetaCvListSkeleton(),i.setAttribute("aria-busy","true"));const l=document.getElementById("meta-cv-editor");l&&(l.innerHTML=renderMetaCvEditorYearSkeleton(),l.setAttribute("aria-busy","true"))}else{n&&(n.disabled=!1),s&&(s.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!1);const r=document.querySelector(".meta-cv-summary");r&&r.classList.remove("is-skeleton");const o=document.querySelector(".meta-cv-legend");o&&o.removeAttribute("aria-hidden");const i=document.getElementById("meta-cv-client-list");i&&i.removeAttribute("aria-busy");const l=document.getElementById("meta-cv-editor");l&&l.removeAttribute("aria-busy")}}function setMetaReportPublicYearLoading(e,t){const a=document.querySelector(".meta-reports-page--public"),n=document.getElementById("meta-report-year");if(t){n&&(n.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!0);const s=document.querySelector(".meta-reports-page--public .meta-report-tabs");s&&(s.innerHTML=renderMetaReportTabsSkeleton(),s.setAttribute("aria-busy","true"));const r=document.getElementById("meta-report-month-panel");r&&(r.innerHTML=renderMetaReportMonthPanelSkeleton(),r.classList.add("is-loading")),destroyMetaReportCharts()}else{n&&(n.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!1);const s=document.querySelector(".meta-reports-page--public .meta-report-tabs");s&&s.removeAttribute("aria-busy");const r=document.getElementById("meta-report-month-panel");r&&r.classList.remove("is-loading")}}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,a=document.getElementById("meta-report-year");a&&(a.onchange=async()=>{if(a.selectedOptions?.[0]?.disabled){a.value=String(metaReportsState.selectedYear);return}const s=metaReportsState.selectedYear,r=Number(a.value);if(r!==s){metaReportsState.selectedYear=r,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportClientYearLoading(r,!0);try{await loadMetaReportsClientPage({silent:!0})}catch(o){metaReportsState.selectedYear=s,a&&(a.value=String(s)),await loadMetaReportsClientPage({silent:!0}),showToast(o.message||"Load failed","error")}finally{setMetaReportClientYearLoading(r,!1)}}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report"),bindMetaReportShareEvents(e)}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const a=document.getElementById("meta-report-refresh-meta");a&&(a.onclick=async()=>{a.disabled=!0;try{const s=metaReportsState.activeMonthKey,r=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(r.monthPayload),showToast("Meta data refreshed","success")}catch(s){showToast(s.message||"Meta refresh failed","error")}finally{a.disabled=!1}});const n=document.getElementById("meta-report-save-month");n&&(n.onclick=async()=>{n.disabled=!0;try{const s=metaReportsState.activeMonthKey,r={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:Number(document.getElementById("meta-report-avg-profit")?.value)},o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}`,{method:"PUT",body:JSON.stringify(r)});refreshMetaReportMonthPanel(o.monthPayload),showToast("Month saved","success")}catch(s){showToast(s.message||"Save failed","error")}finally{n.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{switchMetaReportMonthTab(t.getAttribute("data-meta-month-tab"),{editable:!1})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}const n=metaReportsState.selectedYear,s=Number(t.value);if(s!==n){metaReportsState.selectedYear=s,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportPublicYearLoading(s,!0);try{await loadPublicMetaReportPage({silent:!0})}catch(r){metaReportsState.selectedYear=n,t&&(t.value=String(n)),await loadPublicMetaReportPage({silent:!0}),showToast(r.message||"Load failed","error")}finally{setMetaReportPublicYearLoading(s,!1)}}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
      ${renderBrandTopbar("")}
    ...
    `;else if(!e){const s=document.getElementById("meta-report-month-panel");s&&s.classList.add("is-loading")}try{const s=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(n)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`);if(syncMetaReportSelectedYear(s,{disableUnavailable:!0}),Number(metaReportsState.selectedYear)!==Number(s.year))return loadMetaReportsClientPage({silent:!0});metaReportsState.activeMonthKey||(metaReportsState.activeMonthKey=s.monthKeys?.[s.monthKeys.length-1]||null),metaReportsState.clientPageMounted?updateMetaReportsClientContent(s):(metaReportsState.clientPayload=s,metaReportsState.clientReportSettingsExpanded=!1,metaReportsState.clientShareExpanded=!1,t.innerHTML=renderMetaReportsClientPage(s),metaReportsState.clientPageMounted=!0,bindMetaReportsClientEvents(s),mountMetaReportCharts(s,{editable:!0}))}catch(s){if(!metaReportsState.clientPageMounted)t.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(s.message)}</div>`)}
      `;else{if(e)throw s;showToast(s.message||"Load failed","error")}}finally{const s=document.getElementById("meta-report-month-panel");s&&s.classList.remove("is-loading")}}async function loadPublicMetaReportPage({silent:e=!1}={}){const t=document.getElementById("dashboard");if(!(!t||!REPORT_TOKEN)){if(!metaReportsState.publicPageMounted)t.innerHTML=wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading report...</p></div>');else if(!e){const a=document.getElementById("meta-report-month-panel");a&&a.classList.add("is-loading")}try{const a=await fetch(`/api/meta-reports/public/${encodeURIComponent(REPORT_TOKEN)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`).then(async n=>{const s=await n.json();if(!n.ok)throw new Error(s.error||"Report not found");return s});if(syncMetaReportSelectedYear(a,{disableUnavailable:!0}),Number(metaReportsState.selectedYear)!==Number(a.year))return loadPublicMetaReportPage({silent:!0});metaReportsState.activeMonthKey||(metaReportsState.activeMonthKey=a.monthKeys?.[a.monthKeys.length-1]||null),metaReportsState.publicPageMounted?updatePublicMetaReportContent(a):(metaReportsState.publicPayload=a,t.innerHTML=renderPublicMetaReportPage(a),metaReportsState.publicPageMounted=!0,syncMetaReportPublicBranding(a),bindPublicMetaReportEvents(a),mountMetaReportCharts(a,{editable:!1}))}catch(a){if(!metaReportsState.publicPageMounted)t.innerHTML=wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(a.message||"Report not found")}</div>`);else{if(e)throw a;showToast(a.message||"Load failed","error")}}finally{const a=document.getElementById("meta-report-month-panel");a&&a.classList.remove("is-loading")}}}function metaCvShortMonth(e){const t=Number(String(e||"").slice(5,7));return["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][t-1]||e}function metaCvFormatUpdatedAt(e){if(!e)return"Not updated yet";const t=new Date(e);return Number.isNaN(t.getTime())?"Not updated yet":t.toLocaleString("en-GB",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}function getMetaCvFilteredClients(e=metaReportsState.customValues.overview){const t=e?.clients||[],a=String(metaReportsState.customValues.searchQuery||"").trim().toLowerCase();return a?t.filter(n=>[n.accountName,n.clientId,n.metaAdAccountId].filter(Boolean).join(" ").toLowerCase().includes(a)):t}function renderMetaCvMonthPills(e,t=[]){const a=metaReportsState.customValues.selectedClientId,n=metaReportsState.customValues.selectedMonthKey;return t.map(s=>{const r=e.months?.[s]?.status||"empty",o=a===e.clientId&&n===s;return`
      <button
        type="button"
        class="meta-cv-month-pill is-${esc(r)}${o?" is-active":""}"
        data-meta-cv-client="${esc(e.clientId)}"
        data-meta-cv-month="${esc(s)}"
        title="${esc(metaCvShortMonth(s))} \xB7 ${esc(r)}"
      >${esc(metaCvShortMonth(s))}</button>
    ...
    `).join(""):'<div class="meta-report-empty" style="padding:32px 12px">No matching clients.</div>'}function monthStatusIsEmpty(e,t={}){return e?(e.status??classifyMetaCvMonthStatus({wonLeads:e.inputs?.wonLeads??e.topline?.wonLeads??e.wonLeads,avgLeadValue:e.inputs?.avgLeadValue??e.topline?.avgLeadValue??e.avgLeadValue,avgProfitPerWon:e.inputs?.avgProfitPerWon??e.avgProfitPerWon},!!t.metaReportShowBottomline))==="empty":!0}function metaReportEffectiveFeeEnabled(e,t=null){if(!t)return!!e?.metaReportFeeEnabled;const a=!!e?.metaReportShowBottomline;return(t.status??classifyMetaCvMonthStatus({wonLeads:t.inputs?.wonLeads??t.topline?.wonLeads??t.wonLeads,avgLeadValue:t.inputs?.avgLeadValue??t.topline?.avgLeadValue??t.avgLeadValue,avgProfitPerWon:t.inputs?.avgProfitPerWon??t.avgProfitPerWon},a))==="empty"?!1:!!e?.metaReportFeeEnabled}function resolveMetaReportFeeMode(e={}){return e.metaReportFeeEnabled?resolveMetaReportStoredFeeMode(e):""}const META_REPORT_FEE_MODE_META=[{value:"",icon:"\u2013",title:"None",desc:"No Censio fee"},{value:"performance",icon:"%",title:"Performance fee",desc:"% of profit (POAS)"},{value:"marketing",icon:"Dkr",title:"Marketing fee",desc:"Fixed Dkr amount"}];function metaReportFeeValueBlockHtml(e,t,a,n,{inline:s=!1}={}){return s&&t==="performance"?`
      ...
      </label>`:s?'<span class="meta-cv-fee-inline-hint">No fee amount \u2014 save to apply.</span>':t==="performance"?`
      ...
      </label>`:'<p class="meta-report-fee-value-empty">No amount needed \u2014 just save your selection.</p>'}function metaReportUsesInlineFeeValue(e){return e==="meta-cv"}function describeMetaReportFeeModeLabel(e,t,a,{feeEnabled:n=!0}={}){if(!n||e===""||e==null)return"No Censio fee";if(e==="performance"){const s=Number(t);return`Performance fee \xB7 ${Number.isFinite(s)?s:20}%`}if(e==="marketing"){const s=Number(a);return`Marketing fee \xB7 Dkr ${Number.isFinite(s)?s:0}`}return"No Censio fee"}function readMetaReportFeeDraftState(e){const t=document.getElementById(`${e}-fee-nested`);if(!t)return null;const a=t.querySelector(`input[name="${e}-fee-mode"]:checked`),n=a?a.value||"":t.dataset.feeMode||"";let s=Number(t.dataset.feePercent),r=Number(t.dataset.marketingFee);if(n==="performance"){const u=document.getElementById(`${e}-setting-fee-percent`),p=Number(u?.value);Number.isFinite(p)&&(s=p)}else if(n==="marketing"){const u=document.getElementById(`${e}-setting-marketing-fee`),p=Number(u?.value);Number.isFinite(p)&&(r=p)}const o=t.dataset.feeEnabled==="true"?resolveMetaReportStoredFeeMode({metaReportFeeEnabled:!0,metaReportFeeMode:t.dataset.feeMode||null,metaReportMarketingFeeAmount:t.dataset.marketingFee}):"",i=Number(t.dataset.feePercent),l=Number(t.dataset.marketingFee);return{appliedMode:o,appliedPercent:i,appliedMarketing:l,selectedMode:n,selectedPercent:s,selectedMarketing:r,isDirty:n!==o||(n==="performance"?s!==i:n==="marketing"?r!==l:!1),appliedLabel:describeMetaReportFeeModeLabel(o,i,l),selectedLabel:describeMetaReportFeeModeLabel(n,s,r)}}function metaReportFeeStatusHtml(e){return e.isDirty?`
    ...
    <div class="meta-report-bottomline-summary" id="${t}-fee-summary">
      <div class="meta-report-bottomline-summary-content">
        ...
        <div class="meta-report-bottomline-summary-row">
          ...
          <span class="meta-report-bottomline-summary-value">${esc(o)}</span>
        </div>`:""}
      </div>
      <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="${t}-fee-edit">Edit</button>
    </div>
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t,t&&(metaReportsState.clientShareExpanded=!1);const a=document.querySelector(".meta-report-bottomline-fee-settings--client"),n=document.getElementById(`${e}-fee-editor`);a?.classList.toggle("is-editing",t),a?.classList.toggle("is-collapsed",!t),n&&n.classList.toggle("is-hidden",!t);const s=document.getElementById("meta-report-share-summary"),r=document.getElementById("meta-report-share-editor");s&&(s.hidden=metaReportsState.clientShareExpanded),r&&(r.hidden=!metaReportsState.clientShareExpanded),syncMetaReportControlPanelUi()}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const a=document.getElementById(`${t}-fee-summary`);if(!a)return;const n=document.createElement("div");n.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const s=n.firstElementChild;s&&a.replaceWith(s);const r=document.getElementById(`${t}-fee-edit`);r&&(r.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:a=!1,variant:n="default",collapsible:s=!1,externalSummary:r=!1}={}){const o=!!e.metaReportShowBottomline,i=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",c=resolveMetaReportFeeMode(e),d=e.metaReportFeePercent??20,u=e.metaReportMarketingFeeAmount??0,p=metaReportUsesInlineFeeValue(t),h=`data-fee-mode="${esc(l)}" data-fee-enabled="${i?"true":"false"}" data-fee-percent="${esc(d)}" data-marketing-fee="${esc(u)}"`;if(n==="cv"){const g=describeMetaReportFeeModeLabel(c,d,u),$=META_REPORT_FEE_MODE_META.map(({value:v,title:S})=>`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        <div class="meta-cv-bottomline-head">
          ...
          ${renderMetaReportSwitch(`${t}-setting-bottomline`,o,"Enable bottomline on report",!1,{hideLabel:!0})}
        </div>
        <div class="meta-report-fee-nested meta-cv-bottomline-body${o?"":" is-hidden"}" id="${t}-fee-nested" ${h}>
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),y=META_REPORT_FEE_MODE_META.map(({value:g,icon:$,title:v,desc:S})=>`
        ...
        </label>`).join(""),b=s?metaReportsState.clientReportSettingsExpanded:!0,f=s?`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,w=s&&!r?renderMetaReportBottomlineFeeSummary(e,t):"";return`
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?b?" is-editing":" is-collapsed":""}">
      ${w}
      <div class="meta-report-bottomline-editor${s&&!b?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${o?"":" is-hidden"}" id="${t}-fee-nested" ${h}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
            <span class="meta-report-fee-status-value is-active">${esc(m)}</span>
          </div>
          <div class="meta-report-fee-mode-grid${a?" meta-report-fee-mode-grid--rows":""}">${y}
          </div>
  ...
  `}function renderMetaReportSwitch(e,t,a,n=!1,{hideLabel:s=!1}={}){return`
    <div class="meta-report-switch">
      ...
      ${s?"":`<span class="meta-report-switch-label">${esc(a)}</span>`}
    </div>
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(y=>y.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const s=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,s);const r=t.editorPayload?.months?.[n]||null,o=a.metaReportShowBottomline,i=s.wonLeads!=null?s.wonLeads:r?.inputs?.wonLeads??r?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:r?.inputs?.avgLeadValue??r?.topline?.avgLeadValue??"",c=s.avgProfitPerWon!=null?s.avgProfitPerWon:r?.inputs?.avgProfitPerWon??"",d=metaReportsState.customValues.draftInputs,u=d?.wonLeads??i,p=d?.avgLeadValue??l,h=d?.avgProfitPerWon??c,m=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(y=>`
            <option value="${esc(y.clientId)}"${y.clientId===a.clientId?" selected":""}>${esc(y.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(y=>`
            <option value="${esc(y)}"${y===n?" selected":""}>${esc(metaMonthLabel(y))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
      ...
      <label>Avg profit per won${o?"":" (optional)"}
  ...
  `}function refreshMetaCvListAndEditor(){const e=metaReportsState.customValues.overview;if(!e)return;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e));const a=document.getElementById("meta-cv-editor");a&&(a.innerHTML=renderMetaCvEditor(e)),metaReportsState.customValues.draftInputs=null,bindMetaCvEditorEvents(),bindMetaCvListEvents()}function setMetaCvSavePending(e){metaReportsState.customValues.saving=e;const t=document.getElementById("meta-cv-save"),a=document.getElementById("meta-cv-save-indicator");a&&a.classList.toggle("is-visible",e),t&&(t.disabled=e||metaReportsState.customValues.loadingEditor||metaReportsState.customValues.settingsSaving,t.textContent=e?"Saving\u2026":"Save values")}function setMetaCvSettingsSaving(e,t="report"){metaReportsState.customValues.settingsSaving=e,metaReportsState.customValues.settingsSavingScope=e?t:null;const a=e&&t==="report";document.getElementById("meta-cv-settings-status")?.classList.toggle("is-visible",a),document.querySelector(".meta-cv-client-settings")?.classList.toggle("is-saving",a),["meta-cv-setting-bottomline"].forEach(r=>{const o=document.getElementById(r);o&&(o.disabled=a)}),document.querySelectorAll('input[name="meta-cv-fee-mode"]').forEach(r=>{r.disabled=a}),["meta-cv-setting-fee-percent","meta-cv-setting-marketing-fee"].forEach(r=>{const o=document.getElementById(r);o&&(o.disabled=a||o.disabled)});const s=document.getElementById("meta-cv-save");s&&(s.disabled=e||metaReportsState.customValues.saving||metaReportsState.customValues.loadingEditor)}function classifyMetaCvMonthStatus(e,t=!1){if(!e)return"empty";const a=e.wonLeads!=null,n=e.avgLeadValue!=null,s=e.avgProfitPerWon!=null;return a&&n&&(!t||s)?"complete":a||n||s?"partial":"empty"}function applyMetaCvSaveResult(e,t,a,n){const r=metaReportsState.customValues.overview?.clients?.find(l=>l.clientId===e),o=r?.months?.[t],i=n?.month||{};o&&(o.wonLeads=i.wonLeads??a.wonLeads,o.avgLeadValue=i.avgLeadValue??a.avgLeadValue,o.avgProfitPerWon=i.avgProfitPerWon??a.avgProfitPerWon,o.status=classifyMetaCvMonthStatus(o,!!r.metaReportShowBottomline),o.updatedAt=o.status==="empty"?null:i.updatedAt||new Date().toISOString()),n?.monthPayload&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:n.monthPayload}})}async function refreshMetaCvOverviewSilently(){const e=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(e.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=e;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e),bindMetaCvListEvents());const a=document.querySelector(".meta-cv-summary");if(a&&e.summary){const n=e.summary;a.innerHTML=`
    ...
    `}}function collectMetaCvSaveBody(){return{wonLeads:document.getElementById("meta-cv-won-leads")?.value===""?null:Number(document.getElementById("meta-cv-won-leads")?.value),avgLeadValue:document.getElementById("meta-cv-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-cv-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-cv-avg-profit")?.value===""?null:Number(document.getElementById("meta-cv-avg-profit")?.value)}}function syncMetaCvClientSettingsDom(e={}){syncMetaReportBottomlineFeeDom(e,"meta-cv")}async function saveMetaCvClientSettings(e,t,a){setMetaCvSettingsSaving(!0,"report");try{const n=metaReportsState.customValues.selectedMonthKey,s=await patchMetaReportSettings(e,{...t,monthKey:n||void 0}),o=metaReportsState.customValues.overview?.clients?.find(i=>i.clientId===e);o&&s.settings&&(o.metaReportShowBottomline=!!s.settings.metaReportShowBottomline,o.metaReportFeeEnabled=!!s.settings.metaReportFeeEnabled,o.metaReportFeeMode=s.settings.metaReportFeeMode||null,o.metaReportFeePercent=s.settings.metaReportFeePercent??20,o.metaReportMarketingFeeAmount=s.settings.metaReportMarketingFeeAmount??0),s.monthPayload&&n&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[n]:s.monthPayload}}),setMetaCvSettingsSaving(!1),syncMetaCvClientSettingsDom(s.settings||{}),showToast(describeMetaReportSettingsChange(t,s.settings||{}),"success")}catch(n){showToast(n.message||"Update failed","error"),a&&a(),setMetaCvSettingsSaving(!1)}}function bindMetaCvSettingsEvents(e){bindMetaReportBottomlineFeeEvents(e,"meta-cv",(t,a)=>saveMetaCvClientSettings(e,t,a))}async function selectMetaCvMonth(e,t,{reloadEditor:a=!0}={}){if(metaReportsState.customValues.selectedClientId=e,metaReportsState.customValues.selectedMonthKey=t,refreshMetaCvListAndEditor(),!!a){metaReportsState.customValues.loadingEditor=!0,refreshMetaCvListAndEditor();try{const n=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(t)}`);metaReportsState.customValues.selectedClientId===e&&metaReportsState.customValues.selectedMonthKey===t&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:n.monthPayload}})}catch(n){showToast(n.message||"Failed to load month","error")}finally{metaReportsState.customValues.loadingEditor=!1,refreshMetaCvListAndEditor()}}}function bindMetaCvListEvents(){document.querySelectorAll("[data-meta-cv-month]").forEach(e=>{e.onclick=()=>{selectMetaCvMonth(e.getAttribute("data-meta-cv-client"),e.getAttribute("data-meta-cv-month"))}})}function bindMetaCvEditorEvents(){const e=metaReportsState.customValues.selectedClientId;e&&bindMetaCvSettingsEvents(e);const t=document.getElementById("meta-cv-client-select"),a=document.getElementById("meta-cv-month-select");t&&(t.onchange=()=>{const s=metaReportsState.customValues.selectedMonthKey||metaReportsState.customValues.overview?.monthKeys?.slice(-1)[0];s&&selectMetaCvMonth(t.value,s)}),a&&(a.onchange=()=>{const s=metaReportsState.customValues.selectedClientId;s&&selectMetaCvMonth(s,a.value)});const n=document.getElementById("meta-cv-save");n&&(n.onclick=async()=>{const s=metaReportsState.customValues.selectedClientId,r=metaReportsState.customValues.selectedMonthKey;if(!s||!r)return;const o=collectMetaCvSaveBody();setMetaCvSavePending(!0);try{const i=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(s)}/months/${encodeURIComponent(r)}`,{method:"PUT",body:JSON.stringify(o)});applyMetaCvSaveResult(s,r,o,i),metaReportsState.customValues.draftInputs=o,setMetaCvSavePending(!1),refreshMetaCvListAndEditor(),showToast("Custom values saved","success"),await refreshMetaCvOverviewSilently()}catch(i){showToast(i.message||"Save failed","error"),setMetaCvSavePending(!1)}})}function bindMetaReportsCustomValuesEvents(e){const t=document.getElementById("meta-cv-year");t&&(t.onchange=async()=>{const n=metaReportsState.selectedYear,s=Number(t.value);if(s!==n){metaReportsState.selectedYear=s,metaReportsState.customValues.selectedClientId=null,metaReportsState.customValues.selectedMonthKey=null,metaReportsState.customValues.editorPayload=null,setMetaCvYearLoading(s,!0);try{await loadMetaReportsCustomValuesPage({silent:!0})}catch(r){metaReportsState.selectedYear=n,t&&(t.value=String(n)),await loadMetaReportsCustomValuesPage({silent:!0,keepSelection:!0}),showToast(r.message||"Load failed","error")}finally{setMetaCvYearLoading(s,!1)}}});const a=document.getElementById("meta-cv-search");a&&(a.oninput=()=>{metaReportsState.customValues.searchQuery=a.value;const n=document.getElementById("meta-cv-client-list");n&&(n.innerHTML=renderMetaCvClientRows(e)),bindMetaCvListEvents()}),bindMetaCvListEvents(),bindMetaCvEditorEvents()}async function loadMetaReportsCustomValuesPage({silent:e=!1,keepSelection:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){window.location.href=`/login?next=${encodeURIComponent(window.location.pathname)}`;return}currentStaffUser=n,!metaReportsState.customValues.mounted&&!e&&(a.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("meta-reports-custom"))}
      ${wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading custom values...</p></div>')}
    `);try{const s=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(s.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=s,t||(s.clients||[]).some(o=>o.clientId===metaReportsState.customValues.selectedClientId)||(metaReportsState.customValues.selectedClientId=null,metaReportsState.customValues.selectedMonthKey=null,metaReportsState.customValues.editorPayload=null),a.innerHTML=renderMetaReportsCustomValuesPage(s),metaReportsState.customValues.mounted=!0,metaReportsState.customValues.saving=!1,bindMetaReportsCustomValuesEvents(s)}catch(s){if(!metaReportsState.customValues.mounted)a.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("meta-reports-custom"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(s.message)}</div>`)}
      `;else{if(e)throw s;showToast(s.message||"Load failed","error")}}}function renderAdminHubPage(e){const t=e.length,a=currentStaffUser?`
  ...
  `}function filterHubCards(){const e=(document.getElementById("hub-search")?.value||"").trim().toLowerCase();let t=0;document.querySelectorAll("#client-grid .client-card").forEach(n=>{const s=!e||(n.dataset.search||"").includes(e);n.style.display=s?"":"none",s&&(t+=1)});const a=document.getElementById("hub-no-results");a&&(a.style.display=e&&!t?"":"none")}function renderAddClientPanel(){return`
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(a){showToast(a.message,"error")}finally{if(t){t.disabled=!1;const a=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${a}`:a}}}function getMetricsModelLabels(e,t=setupPipelines){const a=new Map((t||[]).map(n=>[n.id,n.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),a=!e.metricsModelSetAt||metricsModelChangeMode,n=a?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",r=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return a?`
  ...
  `}function getSetupProgressSteps(e){return[{id:"metrics",label:"Metrics",done:!!e.metricsModelSetAt},{id:"ghl",label:"GHL",done:!!(e.hasGhlToken&&e.locationId)},{id:"pipelines",label:"Pipelines",done:!!(e.newLeadsPipelineId&&e.salesPipelineId)},{id:"meta",label:"Meta",done:e.metaSyncStatus==="ok",partial:!!(e.metaAdAccountId&&e.metaSyncStatus!=="ok")}]}function renderSetupProgressStrip(e){const t=getSetupProgressSteps(e),a='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6L9 17l-5-5"/></svg>';return`
    <nav class="setup-progress" aria-label="Setup progress">
      ${t.map((n,s)=>{const r=n.done?" is-done":n.partial?" is-partial":"",o=n.done?`<span class="setup-progress-mark">${a}</span>`:`<span class="setup-progress-mark">${s+1}</span>`;return`
  ...
  `:'<div class="pipeline-warning">Select at least one pipeline, then click Apply data filters.</div>'}function defaultDisplayPrefs(){const e={kpis:{},sections:{},charts:getDefaultChartPrefs(),statusItems:{},columns:{}};return Object.keys(DISPLAY_OPTIONS.kpis).forEach(t=>{e.kpis[t]=!0}),Object.keys(DISPLAY_OPTIONS.sections).forEach(t=>{e.sections[t]=!0}),Object.keys(DISPLAY_OPTIONS.charts).forEach(t=>{e.charts[t]=!0}),Object.keys(DISPLAY_OPTIONS.statusItems).forEach(t=>{e.statusItems[t]=!0}),Object.entries(DISPLAY_OPTIONS.columns).forEach(([t,a])=>{e.columns[t]={},Object.keys(a).forEach(n=>{e.columns[t][n]=!0})}),e}function loadDisplayPrefs(){if(IS_CLIENT_VIEW&&!IS_PREVIEW)return defaultDisplayPrefs();try{const e=JSON.parse(localStorage.getItem(STORAGE_KEY)||localStorage.getItem(LEGACY_STORAGE_KEY)||"null");if(!e)return defaultDisplayPrefs();const t=defaultDisplayPrefs();return{kpis:{...t.kpis,...e.kpis},sections:{...t.sections,...e.sections},charts:{...t.charts,...e.charts},statusItems:{...t.statusItems,...e.statusItems},columns:{sourceReport:{...t.columns.sourceReport,...e.columns?.sourceReport||{}},assigneeReport:{...t.columns.assigneeReport,...e.columns?.assigneeReport||{}},pipelineBreakdown:{...t.columns.pipelineBreakdown,...e.columns?.pipelineBreakdown||{}}}}}catch{return defaultDisplayPrefs()}}function ensureChartsVisible(){const e=Object.keys(DISPLAY_OPTIONS.charts);e.some(a=>display.charts[a]!==!1)||(e.forEach(a=>{display.charts[a]=!0}),saveDisplayPrefs())}function saveDisplayPrefs(){localStorage.setItem(STORAGE_KEY,JSON.stringify(display))}function isVisible(e,t,a){return e==="columns"?display.columns[a]?.[t]!==!1:display[e]?.[t]!==!1}function toggleDisplay(e,t,a){e==="columns"?display.columns[a][t]=!display.columns[a][t]:display[e][t]=!display[e][t],saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}function setAllDisplay(e,t,a){e==="columns"?Object.keys(DISPLAY_OPTIONS.columns[a]).forEach(n=>{display.columns[a][n]=t}):Object.keys(DISPLAY_OPTIONS[e]).forEach(n=>{display[e][n]=t}),saveDisplayPrefs(),cachedData&&updateDashboardContent(cachedData)}const fmt=e=>new Intl.NumberFormat("da-DK",{minimumFractionDigits:0,maximumFractionDigits:0}).format(Math.round(Number(e)||0)),fmtCompact=e=>{const t=Number(e)||0;return t>=1e6?`${(t/1e6).toFixed(2)}M`:t>=1e3?`${(t/1e3).toFixed(2)}K`:fmt(t)},fmtPct=e=>`${(Number(e)||0).toFixed(2)}%`,fmtRoas=e=>{const t=Number(e)||0;return t>0?`${t.toFixed(2)}x`:"\u2014"};function formatActiveDateFilter(e){if(!e.dateFrom&&!e.dateTo)return"Till date";const t=getDashboardTimeZone();if(window.MarketingMetrics?.formatShortDateLabel){const a=window.MarketingMetrics.formatShortDateLabel(e.dateFrom,t),n=window.MarketingMetrics.formatShortDateLabel(e.dateTo,t);if(a&&n)return`${a} \u2013 ${n}`}return`${e.dateFrom||"start"} to ${e.dateTo||"now"}`}function showDateRangeError(e){document.querySelectorAll("#date-range-error").forEach(t=>{t.textContent=e,t.hidden=!e})}function clearDateRangeError(){showDateRangeError("")}function needsFreshData(){return!lastFetchedAt||Date.now()-lastFetchedAt>DATA_FRESH_MS}function buildQuery(e,t={}){const a=new URLSearchParams,n=getAllPipelineIds(e||[]),s=state.pipelineIds.filter(r=>n.includes(r));if(!s.length)throw new Error("Select at least one pipeline.");return a.set("pipelineIds",s.join(",")),state.dateField&&a.set("dateField",state.dateField),state.dateFrom&&a.set("dateFrom",state.dateFrom),state.dateTo&&a.set("dateTo",state.dateTo),["status","source","assignedTo","adSpend"].forEach(r=>{state[r]&&state[r]!=="all"&&a.set(r,state[r])}),appendTenantParams(a),t.forceFresh&&a.set("fresh","1"),a.toString()}function formatDateInput(e){const t=e.getFullYear(),a=String(e.getMonth()+1).padStart(2,"0"),n=String(e.getDate()).padStart(2,"0");return`${t}-${a}-${n}`}function getDashboardTimeZone(){return cachedData?.account?.timezone||"Europe/Copenhagen"}function getCalendarPartsInTimeZone(e,t=new Date){const a=new Intl.DateTimeFormat("en-CA",{timeZone:e,year:"numeric",month:"2-digit",day:"2-digit"}).formatToParts(t);return{year:Number(a.find(n=>n.type==="year")?.value),month:Number(a.find(n=>n.type==="month")?.value),day:Number(a.find(n=>n.type==="day")?.value)}}function formatMonthDateRange(e,t){const a=`${e}-${String(t).padStart(2,"0")}-01`,n=new Date(e,t,0).getDate(),s=`${e}-${String(t).padStart(2,"0")}-${String(n).padStart(2,"0")}`;return{start:a,end:s}}const datePickerState={inputId:null,anchorEl:null,viewYear:null,viewMonth:null};let datePickerListenersBound=!1;function isoFromParts(e,t,a){const n=Number(a);return!e||!t||!Number.isFinite(n)?"":`${e}-${String(t).padStart(2,"0")}-${String(n).padStart(2,"0")}`}function formatPickerDisplayLabel(e,t){if(!e||!/^\d{4}-\d{2}-\d{2}$/.test(e))return t;const a=getDashboardTimeZone();return window.MarketingMetrics?.formatShortDateLabel&&window.MarketingMetrics.formatShortDateLabel(e,a)||e}function ensureDatePickerPopover(){if(document.getElementById("date-picker-popover"))return;const e=document.createElement("div");e.id="date-picker-backdrop",e.className="date-picker-backdrop",e.hidden=!0,e.addEventListener("click",closeDatePicker),document.body.appendChild(e);const t=document.createElement("div");t.id="date-picker-popover",t.className="date-picker-popover",t.hidden=!0,t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),document.body.appendChild(t)}function syncDatePickerDisplays(){[{id:"dateFrom",key:"dateFrom",placeholder:"Pick start date"},{id:"dateTo",key:"dateTo",placeholder:"Pick end date"}].forEach(({id:e,key:t,placeholder:a})=>{const n=document.getElementById(e),s=document.getElementById(`${e}-display`),r=document.getElementById(`${e}-trigger`),o=state[t]??"";n&&(n.value=o),s&&(s.textContent=formatPickerDisplayLabel(o,a)),r?.classList.toggle("is-empty",!o),r?.classList.toggle("has-value",!!o)})}function getStateDateValue(e){return state[e]??""}function getDatePickerViewMonth(e){const t=getStateDateValue(e);if(/^\d{4}-\d{2}-\d{2}$/.test(t)){const[a,n]=t.split("-").map(Number);return{year:a,month:n}}return getCalendarPartsInTimeZone(getDashboardTimeZone())}function getTodayIso(){const e=getCalendarPartsInTimeZone(getDashboardTimeZone());return isoFromParts(e.year,e.month,e.day)}function isStartDateDisabled(e){return e>=getTodayIso()}function isFutureDateDisabled(e){return e>getTodayIso()}function isFutureMonthView(e,t){const a=getCalendarPartsInTimeZone(getDashboardTimeZone());return e>a.year||e===a.year&&t>a.month}function isDatePickerDayDisabled(e,t){return t==="dateFrom"&&isStartDateDisabled(e)?!0:isFutureDateDisabled(e)}function renderDatePickerDayCell(e,t,a,n,s,r,o,i){const l=["date-picker-day"];if(a&&l.push("is-outside"),e===n&&l.push("is-today"),e===s&&l.push("is-selected"),r&&o){const c=r<=o?r:o,d=r<=o?o:r;e>=c&&e<=d&&l.push("is-in-range")}return e===r&&l.push("is-range-start"),e===o&&l.push("is-range-end"),isDatePickerDayDisabled(e,i)?(l.push("is-disabled"),`<button type="button" class="${l.join(" ")}" disabled aria-disabled="true">${t}</button>`):`<button type="button" class="${l.join(" ")}" onclick="selectDatePickerDay('${e}')">${t}</button>`}function renderDatePickerPopover(){const e=document.getElementById("date-picker-popover");if(!e||!datePickerState.inputId)return;const{viewYear:t,viewMonth:a,inputId:n}=datePickerState,s=new Intl.DateTimeFormat("en-GB",{month:"long",year:"numeric"}).format(new Date(t,a-1,1)),r=["Mo","Tu","We","Th","Fr","Sa","Su"],i=(new Date(t,a-1,1).getDay()+6)%7,l=new Date(t,a,0).getDate(),c=new Date(t,a-1,0).getDate(),d=getCalendarPartsInTimeZone(getDashboardTimeZone()),u=isoFromParts(d.year,d.month,d.day),p=getStateDateValue(n),h=getStateDateValue("dateFrom"),m=getStateDateValue("dateTo");let y="";for(let S=i-1;S>=0;S-=1){const L=c-S,B=a===1?12:a-1,P=a===1?t-1:t,C=isoFromParts(P,B,L);y+=renderDatePickerDayCell(C,L,!0,u,p,h,m,n)}for(let S=1;S<=l;S+=1){const L=isoFromParts(t,a,S);y+=renderDatePickerDayCell(L,S,!1,u,p,h,m,n)}const b=(7-(i+l)%7)%7;for(let S=1;S<=b;S+=1){const L=a===12?1:a+1,B=a===12?t+1:t,P=isoFromParts(B,L,S);y+=renderDatePickerDayCell(P,S,!0,u,p,h,m,n)}const f=n==="dateFrom"?'<span class="date-picker-hint">Start date must be before today</span>':"",w=n==="dateTo"?'<button type="button" class="date-picker-footer-btn primary" onclick="setDatePickerToday()">Today</button>':"",g=a===12?1:a+1,$=a===12?t+1:t,v=!isFutureMonthView($,g);e.innerHTML=`
    <div class="date-picker-panel">
      ...
      <div class="date-picker-grid">${y}</div>
  ...
  `}function renderCheckboxGroup(e,t,a,n){const s=Object.entries(a);return`
  ...
  `:`
    ...
    `}function renderTable(e,t,a,n,s){const r=a.filter(o=>isVisible("columns",o.key,e));return r.length?`
    <div class="dashboard-table-wrap">
      <table class="dashboard-table">
        <thead>
          <tr>
            <th>${t}</th>
            ${r.map(o=>`<th class="${o.align||""}">${o.label}</th>`).join("")}
          </tr>
        </thead>
        <tbody>
          ${n.length?n.map(o=>`
            <tr>
              <td>${s(o,"label")}</td>
              ${r.map(i=>`<td class="${i.align||""}">${s(o,i.key)}</td>`).join("")}
            </tr>
          `).join(""):`<tr><td colspan="${r.length+1}">Ingen data for valgte filtre.</td></tr>`}
        </tbody>
      </table>
    </div>
  `:'<div class="empty-section">No columns selected for this table.</div>'}function renderDatePickerTrigger(e,t,a){const n=state[e]||"",s=formatPickerDisplayLabel(n,a);return`
  ...
  `}function renderDashboardContent(e){const{kpis:t,statusBreakdown:a,sourceReport:n,assigneeReport:s,pipelines:r,filters:o}=e,i=isVisible("sections","statusBreakdown");return`
      ...
      <div class="section-grid">
        ${isVisible("sections","assigneeReport")?`
          <div class="card">
            ...
            ${renderTable("assigneeReport","Assignee",[{key:"won",label:"Won",align:"num"},{key:"totalLeads",label:"Total leads",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"},{key:"totalValue",label:"Total value",align:"num"}],s,(l,c)=>c==="label"?esc(l.assigneeName):c==="wonValue"||c==="totalValue"?`Dkr ${fmt(l[c])}`:fmt(l[c]))}
          </div>
        `:""}
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:a=!1,forceFresh:n=!1}=t,s=!!n,r=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),i=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const c=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)r.innerHTML=`
        ...
        `)}`;else if(!a){const d=document.getElementById("dashboard-content");d&&(d.innerHTML=renderKpiSkeleton()),i&&(i.disabled=!0,i.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let d=null;if(!availablePipelines.length){if(d=await bootstrapDashboardData(c),l!==fetchGeneration)return;availablePipelines=d.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,d.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const p=canReuseBootstrapDashboard(d)&&!s;let h,m;if(p?[m,h]=await Promise.all([fetchFacebookMetrics(c),Promise.resolve(d)]):[h,m]=await Promise.all([fetchDashboardData(availablePipelines,c,{forceFresh:s}),fetchFacebookMetrics(c)]),l!==fetchGeneration)return;cachedFacebookMetrics=m,cachedData=applyMarketingData(h,m),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(d){if(l!==fetchGeneration)return;if(d.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):r.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
            ...
            `)}`;return}if(IS_ADMIN_CLIENT&&o){const u=document.getElementById("dashboard-content");u&&(u.innerHTML=`
            <div class="note admin-setup-placeholder">
              ${esc(d.message)}
              <div style="margin-top:12px">
                <button type="button" class="refresh-btn primary" onclick="loadDashboard(true)">${RETRY_MSG}</button>
              </div>
            </div>`)}else o?o&&cachedData?(restoreDashboardContentAfterFailedFetch(o),showToast(d.message||"Failed to load filtered data.","error")):i&&(i.textContent="Apply failed - try again"):r.innerHTML=`
          ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
          ${wrapDashboardShell(`
            <div class="error-state">
              <div>Fejl ved hentning af data</div>
