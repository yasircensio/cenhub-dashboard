...
Resume the interrupted ${g}?`)&&(l=m.id,d=Number(m.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const m=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),u=m.summary||m;l=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||b?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:$,batchOffset:d,batchLimit:o})});const m=c.summary||c;l=m.runId,d=m.nextBatchOffset??m.batchOffset+(m.batchProcessed||0),h=h.concat(m.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const u=m.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:u,complete:!m.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...m,rows:h}))):updateGlobalApplyProgress({processed:d,total:u,written:m.updated||0,complete:!m.hasMore})}while((c.summary||c).hasMore);y=!0;const S=c.summary||c,v=S.inWindow||d||w,p=S.nextBatchOffset??d;if(n)setFbLeadProgress({processed:p,total:v,complete:!0}),markFbLeadPreviewReady(e,t,S.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(S.updated||0)>0),showToast((S.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:p,total:v,written:S.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const m=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete m[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(m))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const f=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=f.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(S){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${S.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((S.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!y&&n&&hideFbLeadProgress(!0),!n&&!y&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
    ...
    <div class="meta-report-share-summary" id="meta-report-share-summary">
      <div class="meta-report-share-summary-main">
        ...
        <div class="meta-report-share-summary-row">
          ...
          <span class="meta-report-share-summary-value">${esc(i)}</span>
        </div>
      </div>
      <div class="meta-report-share-summary-actions">
        ${e.reportUrl?`<button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-report-copy-link" data-copy-report-url="${esc(e.reportUrl)}"${n?"":" disabled"}>Copy link</button>`:""}
        <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-report-share-edit">Edit</button>
      </div>
    </div>
  `}function setMetaReportShareExpanded(e){metaReportsState.clientShareExpanded=!!e;const t=document.getElementById("meta-report-share-card");t?.classList.toggle("is-collapsed",!e),t?.classList.toggle("is-editing",e)}function renderMetaReportShareCard(e){const n=!!(e.settings||{}).metaReportEnabled,a=e.reportUrl||null,s=metaReportFullUrl(a),i=resolveMetaReportShareSlug(e),o=resolveMetaReportShareCode(e),r=metaReportsState.clientShareExpanded,l=!!a;return`
    <div class="meta-report-share-card${n?"":" is-disabled"}${r?" is-editing":" is-collapsed"}" id="meta-report-share-card">
      ...
      `}
      ${l?`
      <div class="meta-report-share-editor" id="meta-report-share-editor">
        ...
        <div class="meta-report-share-field">
          ...
          <div class="meta-report-share-slug-row">
            ...
            <span class="meta-report-share-slug-suffix">-${esc(o||"\xB7\xB7\xB7\xB7")}</span>
          </div>
          <p class="meta-report-share-field-hint">Use a short name like \u201Cml-tag\u201D. Saving updates the slug; rotate code only changes the 4 digits.</p>
        </div>
        ...
        <div class="meta-report-share-editor-actions">
          <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-report-copy-link-editor" data-copy-report-url="${esc(a)}"${n?"":" disabled"}>Copy link</button>
  ...
  `).join("")}function getMetaReportCalendarYear(e){const t=Number(e?.currentYear);return Number.isFinite(t)&&t>=2e3?t:new Date().getFullYear()}function getMetaReportYearOptions(e){const t=getMetaReportCalendarYear(e),n=t-1,a=new Map;Array.isArray(e?.years)&&e.years.forEach(i=>{const o=Number(i?.year);Number.isFinite(o)&&a.set(o,i.available!==!1)});const s=a.has(n)?a.get(n):e?.previousYearHasData===!0;return[{year:t,available:!0},{year:n,available:!!s}]}function renderMetaReportYearSelectOptions(e,{disableUnavailable:t=!1}={}){const n=Number(metaReportsState.selectedYear)||Number(e?.year);return getMetaReportYearOptions(e).map(({year:a,available:s})=>{const i=t&&!s;return`<option value="${a}"${Number(n)===a?" selected":""}${i?" disabled":""}>${a}${s?"":" (no data)"}</option>`}).join("")}function syncMetaReportSelectedYear(e,{disableUnavailable:t=!1}={}){const n=getMetaReportYearOptions(e),a=Number(metaReportsState.selectedYear),s=n.find(l=>l.year===a);if(s&&(!t||s.available)){metaReportsState.selectedYear=s.year;return}const i=Number(e?.year),o=n.find(l=>l.year===i);if(o&&(!t||o.available)){metaReportsState.selectedYear=o.year;return}const r=n.find(l=>l.available)||n[0];r&&(metaReportsState.selectedYear=r.year)}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;if(!t||!metaReportsState.activeMonthKey)return;t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e;const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e,getMetaReportMonthBodyOptions(t.settings,{editable:!0})),bindMetaReportsClientEditEvents(t.clientId,t),refreshMetaReportBackfillControl(t))}function refreshMetaReportBackfillControl(e){const t=document.querySelector(".meta-report-client-page .meta-report-toolbar-left"),n=document.querySelector(".meta-report-backfill-wrap"),a=metaReportMonthsNeedingBackfill(e);if(!a.length){n&&n.remove();return}const s=`${ICON_SYNC} Backfill ${a.length} month${a.length===1?"":"s"} from Meta`,i=document.getElementById("meta-report-backfill");if(i)i.innerHTML=s,i.disabled=!1;else if(t){const o=document.createElement("div");o.className="meta-report-backfill-wrap",o.innerHTML=`
    ...
    `,t.appendChild(o),bindMetaReportBackfillButton(e.clientId)}}function updateMetaReportsClientContent(e){metaReportsState.clientPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null),syncMetaReportSelectedYear(e,{disableUnavailable:!0});const n=document.getElementById("meta-report-year");n&&(n.innerHTML=renderMetaReportYearSelectOptions(e,{disableUnavailable:!0}));const a=document.getElementById("meta-report-copy-link");e.reportUrl?a&&(a.setAttribute("data-copy-report-url",e.reportUrl),a.disabled=!e.settings?.metaReportEnabled,a.hidden=!1):a&&(a.hidden=!0),syncMetaReportShareCard(e);const s=e.settings||{},i=e.months?.[metaReportsState.activeMonthKey]||null;syncMetaReportSettingsControls(s,i);const o=document.querySelector(".meta-report-tabs");o&&(o.innerHTML=renderMetaReportsClientTabs(e),bindMetaReportsClientTabEvents(e));const r=document.getElementById("meta-report-month-panel");r&&(r.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(s,{editable:!0})),bindMetaReportsClientEditEvents(e.clientId,e)),refreshMetaReportBackfillControl(e)}function updatePublicMetaReportContent(e){metaReportsState.publicPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null),syncMetaReportSelectedYear(e,{disableUnavailable:!0});const n=document.getElementById("meta-report-year");n&&(n.innerHTML=renderMetaReportYearSelectOptions(e,{disableUnavailable:!0}));const a=document.querySelector(".meta-report-tabs");a&&(a.innerHTML=t.map(i=>`
