...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),w=$.summary||$;l=w.runId,c=0,setFbLeadProgress({processed:0,total:w.inWindow||0,label:`Matching ${w.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:g||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:s})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const w=$.inWindow||c||g;a?(setFbLeadProgress({processed:c,total:w,complete:!$.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:w,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||g,y=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:y,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:y,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function metaReportFullUrl(e){return e?`${window.location.origin}${e}`:""}function normalizeMetaReportSlugInput(e){return String(e||"").trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function resolveMetaReportShareSlug(e){const t=e.settings||{};if(t.metaReportSlug)return t.metaReportSlug;const n=String(e.reportUrl||"").replace(/^\/report\//,"").match(/^(.+)-(\d{4})$/);return n?n[1]:""}function resolveMetaReportShareCode(e){const a=String(e.reportUrl||"").replace(/^\/report\//,"").match(/-(\d{4})$/);return a?a[1]:""}function renderMetaReportShareSummary(e,{hidden:t=!1}={}){const n=!!(e.settings||{}).metaReportEnabled,o=resolveMetaReportShareSlug(e),r=resolveMetaReportShareCode(e),s=o?`${o}-${r||"\xB7\xB7\xB7\xB7"}`:"Not set";return`
  ...
  `}function renderMetaReportClientControlPanel(e){const t=e.settings||{},a=metaReportsState.clientShareExpanded,n=metaReportsState.clientReportSettingsExpanded,o=!!e.reportUrl,r=a||n,s=metaReportMonthsNeedingBackfill(e),i=s.length?`${ICON_SYNC} Backfill ${s.length} month${s.length===1?"":"s"} from Meta`:`${ICON_SYNC} Re-sync year from Meta`,l=metaReportsState.activeMonthKey||e.monthKeys?.[e.monthKeys.length-1]||null,c=l?e.months?.[l]:null,d=isMetaReportGhlListClient(t)?`<div id="meta-report-topline-card">${renderMetaToplineSourcePanel({prefix:"meta-report",monthKey:l,monthPayload:c,settings:t,saving:metaReportsState.clientToplineSaving,overrideManual:metaReportsState.clientOverrideManual})}</div>`:"";return`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null,settings:o=null}={}){const r=n||e?.monthKey||null,s=r?metaReportMonthBounds(r):null,i=o||a?.settings||{},l=isMetaReportGhlListClient(i),c=!!metaReportsState.clientOverrideManual,d=resolveMetaToplineSource(e),u=l&&d==="ghl"&&!c,p=l&&(c||d==="manual");let h="";if(t&&e&&!e.meta?.emptyMonth&&s){const B=u?" readonly":"",L=u?" disabled":"",k=e.manualLeads??e.inputs?.manualLeads??"",M=l&&u?"Values come from Cenhub. Use the topline panel above to sync or enable manual override.":"These fields drive Topline KPI'er. Ad spend comes from Meta.";h=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        <h3>Edit month inputs \xB7 ${esc(metaMonthLabel(r))}</h3>
        <p class="meta-report-edit-note">${esc(M)}</p>
        <div class="meta-report-edit-grid">
          ...
          ${p?`<label>Manual leads (optional)<input type="number" step="any" id="meta-report-manual-leads" value="${esc(k)}" /></label>`:""}
          <label>Won leads<input type="number" step="any" id="meta-report-won-leads" value="${esc(e.topline?.wonLeads??"")}"${B} /></label>
          <label>Avg lead value<input type="number" step="any" id="meta-report-avg-lead-value" value="${esc(e.topline?.avgLeadValue??"")}"${B} /></label>
          <label>Avg profit per won<input type="number" step="any" id="meta-report-avg-profit" value="${esc(e.inputs?.avgProfitPerWon??"")}"${B} /></label>
        </div>
    ...
    `}const m=n||e?.monthKey||null,g=t?m?`Report preview \xB7 ${metaMonthLabel(m)}`:"Report preview":m?`Report \xB7 ${metaMonthLabel(m)}`:"Report",b=metaReportsState.reportViewMode||"monthly",f=shouldShowMetaReportYearVisuals(e,a,{editable:t}),y=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${g}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",v=renderMetaReportMonthTables(e,{editable:t}),$=f?renderMetaReportChartsPanel({editable:t}):"",w=t&&f?renderMetaReportScenarioCard():"",S=f?renderMetaReportViewModeTabs():"";let I;return f?I=`
      ...
      ${w}
    `:I=`<div class="meta-report-content-main">${v}</div>`,`${h}${y}${S}${I}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `}function setMetaReportYearLoadingBanner(e,t,a){const n=document.querySelector(e);if(!n)return;let o=n.querySelector(".meta-report-year-loading-banner");a?(o||(o=document.createElement("span"),o.className="meta-report-year-loading-banner",o.setAttribute("aria-live","polite"),n.appendChild(o)),o.innerHTML=`<span class="meta-report-save-indicator-spinner"></span> Loading ${esc(t)}\u2026`,o.hidden=!1):o&&(o.hidden=!0)}function setMetaReportClientYearLoading(e,t){const a=document.querySelector(".meta-report-client-page"),n=document.getElementById("meta-report-year");if(t){n&&(n.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!0);const o=document.querySelector(".meta-report-tabs");o&&(o.innerHTML=renderMetaReportTabsSkeleton(),o.setAttribute("aria-busy","true"));const r=document.getElementById("meta-report-month-panel");r&&(r.innerHTML=renderMetaReportMonthPanelSkeleton(),r.classList.add("is-loading")),destroyMetaReportCharts()}else{n&&(n.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!1);const o=document.querySelector(".meta-report-tabs");o&&o.removeAttribute("aria-busy");const r=document.getElementById("meta-report-month-panel");r&&r.classList.remove("is-loading")}}function setMetaCvYearLoading(e,t){const a=document.querySelector(".sync-history-page.meta-reports-page"),n=document.getElementById("meta-cv-year"),o=document.getElementById("meta-cv-search");if(t){n&&(n.disabled=!0),o&&(o.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!0);const r=document.querySelector(".meta-cv-summary");r&&(r.classList.add("is-skeleton"),r.innerHTML=renderMetaCvSummarySkeleton());const s=document.querySelector(".meta-cv-legend");s&&s.setAttribute("aria-hidden","true");const i=document.getElementById("meta-cv-client-list");i&&(i.innerHTML=renderMetaCvListSkeleton(),i.setAttribute("aria-busy","true"));const l=document.getElementById("meta-cv-editor");l&&(l.innerHTML=renderMetaCvEditorYearSkeleton(),l.setAttribute("aria-busy","true"))}else{n&&(n.disabled=!1),o&&(o.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!1);const r=document.querySelector(".meta-cv-summary");r&&r.classList.remove("is-skeleton");const s=document.querySelector(".meta-cv-legend");s&&s.removeAttribute("aria-hidden");const i=document.getElementById("meta-cv-client-list");i&&i.removeAttribute("aria-busy");const l=document.getElementById("meta-cv-editor");l&&l.removeAttribute("aria-busy")}}function setMetaReportPublicYearLoading(e,t){const a=document.querySelector(".meta-reports-page--public"),n=document.getElementById("meta-report-year");if(t){n&&(n.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!0);const o=document.querySelector(".meta-reports-page--public .meta-report-tabs");o&&(o.innerHTML=renderMetaReportTabsSkeleton(),o.setAttribute("aria-busy","true"));const r=document.getElementById("meta-report-month-panel");r&&(r.innerHTML=renderMetaReportMonthPanelSkeleton(),r.classList.add("is-loading")),destroyMetaReportCharts()}else{n&&(n.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!1);const o=document.querySelector(".meta-reports-page--public .meta-report-tabs");o&&o.removeAttribute("aria-busy");const r=document.getElementById("meta-report-month-panel");r&&r.classList.remove("is-loading")}}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,a=document.getElementById("meta-report-year");a&&(a.onchange=async()=>{if(a.selectedOptions?.[0]?.disabled){a.value=String(metaReportsState.selectedYear);return}const o=metaReportsState.selectedYear,r=Number(a.value);if(r!==o){metaReportsState.selectedYear=r,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportClientYearLoading(r,!0);try{await loadMetaReportsClientPage({silent:!0})}catch(s){metaReportsState.selectedYear=o,a&&(a.value=String(o)),await loadMetaReportsClientPage({silent:!0}),showToast(s.message||"Load failed","error")}finally{setMetaReportClientYearLoading(r,!1)}}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report"),bindMetaReportShareEvents(e)}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e),isMetaReportGhlListClient(e.settings||{})&&bindMetaReportToplineEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const a=document.getElementById("meta-report-refresh-meta");a&&(a.onclick=async()=>{a.disabled=!0;try{const o=metaReportsState.activeMonthKey,r=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(r.monthPayload),showToast("Meta data refreshed","success")}catch(o){showToast(o.message||"Meta refresh failed","error")}finally{a.disabled=!1}});const n=document.getElementById("meta-report-save-month");n&&(n.onclick=async()=>{n.disabled=!0;try{const o=metaReportsState.activeMonthKey,r=t?.months?.[o],s=t?.settings||{},i=isMetaReportGhlListClient(s)&&(metaReportsState.clientOverrideManual||resolveMetaToplineSource(r)==="manual"),l={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:document.getElementById("meta-report-won-leads")?.value===""?null:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:document.getElementById("meta-report-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-report-avg-profit")?.value===""?null:Number(document.getElementById("meta-report-avg-profit")?.value)};if(i){l.toplineSource="manual",l.manualOverride=!0;const d=document.getElementById("meta-report-manual-leads");d&&(l.manualLeads=d.value===""?null:Number(d.value))}const c=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}`,{method:"PUT",body:JSON.stringify(l)});refreshMetaReportMonthPanel(c.monthPayload),showToast("Month saved","success")}catch(o){showToast(o.message||"Save failed","error")}finally{n.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{switchMetaReportMonthTab(t.getAttribute("data-meta-month-tab"),{editable:!1})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}const n=metaReportsState.selectedYear,o=Number(t.value);if(o!==n){metaReportsState.selectedYear=o,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportPublicYearLoading(o,!0);try{await loadPublicMetaReportPage({silent:!0})}catch(r){metaReportsState.selectedYear=n,t&&(t.value=String(n)),await loadPublicMetaReportPage({silent:!0}),showToast(r.message||"Load failed","error")}finally{setMetaReportPublicYearLoading(o,!1)}}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
    ...
    `;else if(!e){const o=document.getElementById("meta-report-month-panel");o&&o.classList.add("is-loading")}try{const o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(n)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`);if(syncMetaReportSelectedYear(o,{disableUnavailable:!0}),Number(metaReportsState.selectedYear)!==Number(o.year))return loadMetaReportsClientPage({silent:!0});metaReportsState.activeMonthKey||(metaReportsState.activeMonthKey=o.monthKeys?.[o.monthKeys.length-1]||null),await ensureMetaReportScenarioSource(o,{editable:!0}),metaReportsState.clientPageMounted?updateMetaReportsClientContent(o):(metaReportsState.clientPayload=o,metaReportsState.clientReportSettingsExpanded=!1,metaReportsState.clientShareExpanded=!1,t.innerHTML=renderMetaReportsClientPage(o),metaReportsState.clientPageMounted=!0,bindMetaReportsClientEvents(o),mountMetaReportCharts(o,{editable:!0}))}catch(o){if(!metaReportsState.clientPageMounted)t.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(o.message)}</div>`)}
      `;else{if(e)throw o;showToast(o.message||"Load failed","error")}}finally{const o=document.getElementById("meta-report-month-panel");o&&o.classList.remove("is-loading")}}async function loadPublicMetaReportPage({silent:e=!1}={}){const t=document.getElementById("dashboard");if(!(!t||!REPORT_TOKEN)){if(!metaReportsState.publicPageMounted)t.innerHTML=wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading report...</p></div>');else if(!e){const a=document.getElementById("meta-report-month-panel");a&&a.classList.add("is-loading")}try{const a=await fetch(`/api/meta-reports/public/${encodeURIComponent(REPORT_TOKEN)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`).then(async n=>{const o=await n.json();if(!n.ok)throw new Error(o.error||"Report not found");return o});if(syncMetaReportSelectedYear(a,{disableUnavailable:!0}),Number(metaReportsState.selectedYear)!==Number(a.year))return loadPublicMetaReportPage({silent:!0});metaReportsState.activeMonthKey||(metaReportsState.activeMonthKey=a.monthKeys?.[a.monthKeys.length-1]||null),await ensureMetaReportScenarioSource(a,{editable:!1}),metaReportsState.publicPageMounted?updatePublicMetaReportContent(a):(metaReportsState.publicPayload=a,t.innerHTML=renderPublicMetaReportPage(a),metaReportsState.publicPageMounted=!0,syncMetaReportPublicBranding(a),bindPublicMetaReportEvents(a),mountMetaReportCharts(a,{editable:!1}))}catch(a){if(!metaReportsState.publicPageMounted)t.innerHTML=wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(a.message||"Report not found")}</div>`);else{if(e)throw a;showToast(a.message||"Load failed","error")}}finally{const a=document.getElementById("meta-report-month-panel");a&&a.classList.remove("is-loading")}}}function metaCvShortMonth(e){const t=Number(String(e||"").slice(5,7));return["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"][t-1]||e}function metaCvFormatUpdatedAt(e){if(!e)return"Not updated yet";const t=new Date(e);return Number.isNaN(t.getTime())?"Not updated yet":t.toLocaleString("en-GB",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"})}function getMetaCvFilteredClients(e=metaReportsState.customValues.overview){const t=e?.clients||[],a=String(metaReportsState.customValues.searchQuery||"").trim().toLowerCase();return a?t.filter(n=>[n.accountName,n.clientId,n.metaAdAccountId].filter(Boolean).join(" ").toLowerCase().includes(a)):t}function metaCvSourceBadgeHtml(e){return!e||e==="meta"?'<span class="meta-cv-source-badge is-meta">Meta</span>':e==="ghl"?'<span class="meta-cv-source-badge is-ghl">GHL</span>':e==="manual"?'<span class="meta-cv-source-badge is-manual">Manual</span>':""}function isMetaReportGhlListClient(e={}){return!!e.metaReportGhlDataEnabled}function resolveMetaToplineSource(e){return e?.dataSource||e?.toplineSource||"meta"}function classifyMetaReportMonthInputStatus(e,t=!1){if(!e)return"empty";const a=e.topline?.wonLeads!=null||e.inputs?.wonLeads!=null,n=e.topline?.avgLeadValue!=null||e.inputs?.avgLeadValue!=null,o=e.inputs?.avgProfitPerWon!=null;return a&&n&&(!t||o)?"complete":a||n||o?"partial":"empty"}function renderMetaToplineSourceDetail(e,{overrideManual:t=!1}={}){const a=resolveMetaToplineSource(e),n=e?.ghlTotals||{},o=n.leads??e?.ghlLeads,r=n.wonLeads??e?.ghlWonLeads,s=n.totalRevenue??e?.ghlTotalRevenue,i=n.totalProfit??e?.ghlTotalProfit;return a==="ghl"&&!t?!e?.ghlSyncedAt&&o==null&&r==null?"No Cenhub data for this month yet. Sync to pull from dashboard snapshots.":o!=null||r!=null||s!=null||i!=null?`Leads ${o??"\u2014"} \xB7 Won ${r??"\u2014"} \xB7 Revenue ${s??"\u2014"} \xB7 Profit ${i??"\u2014"}`:"Cenhub snapshot synced \u2014 topline values drive this month's report.":a==="manual"||t?"Manual override active \u2014 edit values below and save.":"Topline uses Meta metrics until you sync from Cenhub."}function renderMetaToplineSourcePanel({prefix:e,monthKey:t,monthPayload:a,settings:n={},saving:o=!1,overrideManual:r=!1,showOverrideToggle:s=!0,compact:i=!1}={}){if(!isMetaReportGhlListClient(n))return"";const l=resolveMetaToplineSource(a),c=l==="ghl"?"Sourced from Cenhub snapshot":l==="manual"?"Manual override":"Meta metrics",d=t?metaMonthLabel(t):"This month",u=a?.ghlSyncedAt?` \xB7 Synced ${metaCvFormatUpdatedAt(a.ghlSyncedAt)}`:"",p=renderMetaToplineSourceDetail(a,{overrideManual:r}),h=o?" disabled":"",m=n.hasGhl!==!1;return`
    <div class="meta-topline-source-panel${i?" is-compact":""}" data-meta-topline-prefix="${esc(e)}">
      <div class="meta-topline-source-head">
        <div>
          <div class="meta-topline-source-title">Topline data \xB7 ${esc(d)}</div>
          <p class="meta-topline-source-sub">${esc(c)}${u?esc(u):""}</p>
        </div>
        ${metaCvSourceBadgeHtml(l)}
      </div>
      <p class="meta-topline-source-detail">${esc(p)}</p>
      <div class="meta-cv-ghl-actions">
        <button type="button" class="admin-btn admin-btn--secondary" id="${esc(e)}-sync-ghl"${h||!m?" disabled":""}${m?"":' title="GHL not connected"'}>Sync with Cenhub</button>
        <button type="button" class="admin-btn admin-btn--ghost" id="${esc(e)}-sync-ghl-year"${h||!m?" disabled":""}>Sync year\u2026</button>
        <button type="button" class="admin-btn admin-btn--ghost" id="${esc(e)}-refresh-snapshot"${h||!m?" disabled":""}>Refresh Cenhub data</button>
        ${l!=="meta"?`<button type="button" class="admin-btn admin-btn--ghost" id="${esc(e)}-use-meta"${h}>Use Meta metrics</button>`:""}
      </div>
      ${s?`
        <label style="display:flex;align-items:center;gap:8px;margin:0;font-size:13px">
          <input type="checkbox" id="${esc(e)}-override-ghl"${r?" checked":""}${h} />
          Override GHL \u2014 enter manually
        </label>
      `:""}
    </div>
  `}function bindMetaToplineSourceEvents({clientId:e,prefix:t,getMonthKey:a,getYear:n,getMonthRecord:o,setSaving:r,getOverrideManual:s,setOverrideManual:i,onMonthUpdated:l}){if(!e||!t)return;const c=document.getElementById(`${t}-override-ghl`);c&&(c.onchange=async()=>{const m=c.checked;i&&i(m);const g=a?.();if(m&&g)try{await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(g)}/use-manual`,{method:"POST",body:"{}"}),l&&await l({},g)}catch(b){showToast(b.message||"Could not switch to manual mode","error")}l&&await l({},g,{refreshUiOnly:!0})});const d=document.getElementById(`${t}-sync-ghl`);d&&(d.onclick=async()=>{const m=a?.();if(!m)return;const g=o?.(m);let b=!1;if(g?.manualOverride&&(g?.toplineSource==="manual"||g?.dataSource==="manual")){if(!window.confirm("This month has manual values. Overwrite with Cenhub data?"))return;b=!0}r&&r(!0);try{const f=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(m)}/sync-ghl`,{method:"POST",body:JSON.stringify({overwriteManual:b})});i&&i(!1),l&&await l(f,m),showToast("Synced from Cenhub snapshot","success")}catch(f){showToast(f.message||"Sync failed","error")}finally{r&&r(!1)}});const u=document.getElementById(`${t}-sync-ghl-year`);u&&(u.onclick=async()=>{r&&r(!0);try{const m=n?.()||metaReportsState.selectedYear,g=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-preview?year=${encodeURIComponent(m)}`);let b=!0,f=!1;if((g.skipManual||[]).length){const v=window.prompt(`${g.skipManual.length} month(s) have manual values (${g.skipManual.join(", ")}).
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!v)return;v.trim().toLowerCase()==="overwrite"&&(b=!1,f=!0)}const y=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(m)}`,{method:"POST",body:JSON.stringify({skipManual:b,overwriteManual:f})});l&&await l(y,null,{yearSync:!0}),showToast(`Synced ${y.synced?.length||0} month(s)${y.skipped?.length?`, skipped ${y.skipped.length}`:""}`,"success")}catch(m){showToast(m.message||"Year sync failed","error")}finally{r&&r(!1)}});const p=document.getElementById(`${t}-refresh-snapshot`);p&&(p.onclick=async()=>{const m=a?.();if(m){r&&r(!0);try{const g=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(m)}/refresh-snapshot`,{method:"POST",body:"{}"});l&&await l(g,m),showToast("Cenhub data refreshed and synced","success")}catch(g){showToast(g.message||"Refresh failed","error")}finally{r&&r(!1)}}});const h=document.getElementById(`${t}-use-meta`);h&&(h.onclick=async()=>{const m=a?.();if(m){r&&r(!0);try{const g=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(m)}/use-meta`,{method:"POST",body:"{}"});i&&i(!1),l&&await l(g,m),showToast("Using Meta metrics for this month","success")}catch(g){showToast(g.message||"Update failed","error")}finally{r&&r(!1)}}})}function renderMetaCvMonthPills(e,t=[]){const a=metaReportsState.customValues.selectedClientId,n=metaReportsState.customValues.selectedMonthKey;return t.map(o=>{const r=e.months?.[o]||{},s=r.status||"empty",i=r.toplineSource||null,l=a===e.clientId&&n===o,c=e.metaReportGhlDataEnabled?metaCvSourceBadgeHtml(i):"";return`
      <button
        type="button"
        class="meta-cv-month-pill is-${esc(s)}${l?" is-active":""}"
        data-meta-cv-client="${esc(e.clientId)}"
        data-meta-cv-month="${esc(o)}"
        title="${esc(metaCvShortMonth(o))} \xB7 ${esc(s)}${i?` \xB7 ${i}`:""}"
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),g=META_REPORT_FEE_MODE_META.map(({value:v,icon:$,title:w,desc:S})=>`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,y=o&&!r?renderMetaReportBottomlineFeeSummary(e,t):"";return`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(R=>R.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    `;const o=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,o);const r=t.editorPayload?.months?.[n]||null,s=a.metaReportShowBottomline,i=!!a.metaReportGhlDataEnabled,l=o.toplineSource||r?.dataSource||"meta",c=!!t.overrideManual,d=i&&l==="ghl"&&!c,u=o.wonLeads!=null?o.wonLeads:r?.inputs?.wonLeads??r?.topline?.wonLeads??"",p=o.avgLeadValue!=null?o.avgLeadValue:r?.inputs?.avgLeadValue??r?.topline?.avgLeadValue??"",h=o.avgProfitPerWon!=null?o.avgProfitPerWon:r?.inputs?.avgProfitPerWon??"",m=o.ghlLeads??r?.ghlTotals?.leads??"",g=o.ghlTotalRevenue??r?.ghlTotals?.totalRevenue??"",b=o.ghlTotalProfit??r?.ghlTotals?.totalProfit??"",f=metaReportsState.customValues.draftInputs,y=f?.wonLeads??u,v=f?.avgLeadValue??p,$=f?.avgProfitPerWon??h,w=f?.manualLeads??o.manualLeads??"",S=d?" readonly":"",I=d?" disabled":"",B=t.settingsSaving&&t.settingsSavingScope==="report",L=l==="ghl"?"Sourced from Cenhub snapshot":l==="manual"?"Manual override":"Meta metrics",k=i?renderMetaToplineSourcePanel({prefix:"meta-cv",monthKey:n,monthPayload:r||{dataSource:l,ghlTotals:{leads:m!==""?m:null,totalRevenue:g!==""?g:null,totalProfit:b!==""?b:null},ghlSyncedAt:o.ghlSyncedAt||null},settings:{metaReportGhlDataEnabled:!0,hasGhl:a.hasGhl!==!1},saving:t.saving,overrideManual:c}):"",M=i&&(c||l==="manual")?`
      <label>Manual leads (optional)
        <input type="number" step="any" id="meta-cv-manual-leads" value="${esc(w)}" />
      </label>
  `:"";return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label>Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(v)}"${S} />
      </label>
      <label>Avg profit per won${s?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc($)}"${S} />
      </label>
    </div>
    ...
    <div class="meta-cv-editor-actions">
      ...
      <button type="button" class="admin-btn admin-btn--primary" id="meta-cv-save"${t.saving||t.loadingEditor||t.settingsSaving||d?" disabled":""}${I}>
