...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),y=w.summary||w;l=y.runId,c=0,setFbLeadProgress({processed:0,total:y.inWindow||0,label:`Matching ${y.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:r})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const y=w.inWindow||c||$;a?(setFbLeadProgress({processed:c,total:y,complete:!w.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:y,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||$,g=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:g,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:g,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function renderMetaReportComparisonCustomDatesPanel(e,t,a){const n=metaReportComparisonDateInputBounds(a),s=e?.startDate||n.min||"",o=e?.endDate||s,r=t?.startDate||n.min||"",i=t?.endDate||r;return`
    <div class="meta-report-comparison-duel is-custom-dates" id="meta-report-comparison-range-panel">
      ${renderMetaReportComparisonCustomRangeSide("a",s,o,n)}
      <div class="meta-report-comparison-duel-divider" aria-hidden="true">vs</div>
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),$=META_REPORT_FEE_MODE_META.map(({value:v,icon:w,title:y,desc:S})=>`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,g=s&&!o?renderMetaReportBottomlineFeeSummary(e,t):"";return`
