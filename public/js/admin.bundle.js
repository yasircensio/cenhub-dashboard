...
${n}`))try{const s=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}`,{method:"DELETE"});showToast(`Cleared ${s.deleted||0} log entr${s.deleted===1?"y":"ies"}.`,"success"),await loadSyncHistoryPage(e)}catch(s){showToast(s.message||`Failed to clear ${t} sync log.`,"error")}}function renderSyncHistoryLoginPrompt(e){const t=e==="meta"?"Meta sync log":"GHL sync log";return`
  ...
  `}async function loadSyncHistoryPage(e,{silent:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){t||(n.innerHTML=renderSyncHistoryLoginPrompt(e));return}currentStaffUser=a,t||(n.innerHTML=`
    ...
    `}}let fbLeadSyncState={clients:[],preflightByClient:{},historyRuns:[],activeRun:null,previewOkByClient:{}},fbLeadSyncRefreshTimer=null;function renderFbLeadReadinessBadges(e){return`<div class="fb-lead-readiness">${[{key:"metaPageId",label:"Page",ok:!!e.metaPageId},{key:"ghl",label:"GHL",ok:e.hasGhlToken&&e.locationId},{key:"metaToken",label:"Meta token",ok:e.hasMetaToken},{key:"field",label:"Field id",ok:!0}].map(n=>`<span class="fb-lead-badge${n.ok?" is-ok":" is-missing"}">${esc(n.label)}</span>`).join("")}</div>`}function ghlContactUrl(e,t){return!e||!t?null:`https://app.gohighlevel.com/v2/location/${encodeURIComponent(e)}/contacts/detail/${encodeURIComponent(t)}`}function clientHasReadyPreview(e){if(!e)return!1;if(fbLeadSyncState.previewOkByClient[e.clientId])return!0;const t=e.lastRun;return!!(t?.dryRun&&String(t.status||"").toLowerCase()==="success"&&(t.updated||0)>0)}function markFbLeadPreviewReady(e,t,n){fbLeadSyncState.previewOkByClient[e]={mode:t,updated:n,at:Date.now()};try{const a=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");a[e]={mode:t,updated:n,at:Date.now()},sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(a))}catch{}}function hydrateFbLeadPreviewState(e){try{const t=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");for(const[n,a]of Object.entries(t))fbLeadSyncState.previewOkByClient[n]=a}catch{}for(const t of e||[])if(clientHasReadyPreview(t)){const n=t.lastRun;fbLeadSyncState.previewOkByClient[t.clientId]={mode:n?.mode||"backfill",updated:n?.updated||0,at:Date.parse(n?.startedAt||"")||Date.now()}}}function renderFbLeadApplyCta(e,t,n){const a=t==="backfill"?"Apply backfill now":"Apply sync now";return`
    <div class="fb-lead-apply-cta" style="margin-top:16px;padding:14px 16px;background:#e8f7ef;border:1px solid #b8e6cf;border-radius:12px">
      <strong>Preview complete</strong>
      <p style="margin:8px 0 12px;color:var(--text-muted);font-size:13px">
        ${esc(String(n||0))} contact(s) are ready to receive <code>Fb Lead id</code> in GHL.
      </p>
      <button class="admin-btn admin-btn--primary" type="button"
        onclick="applyFbLeadSync('${esc(e)}', '${esc(t)}')">${a}</button>
    </div>
  `}async function applyFbLeadSync(e,t="backfill"){const n=fbLeadSyncState.clients.find(c=>c.clientId===e),s=fbLeadSyncState.previewOkByClient[e]?.updated||n?.lastRun?.updated||0;if(t==="backfill"&&!clientHasReadyPreview(n)){showToast("Run Preview first before applying backfill.","error"),openFbLeadRunPanel(e,t);return}if(!window.confirm(`Apply ${t==="backfill"?"90-day backfill":"recent sync"} for ${e}?

${s} contact(s) will get Fb Lead id written in GHL. This writes live data.`))return;openFbLeadRunPanel(e,t),fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:!0};const o=document.getElementById("fb-lead-apply-btn");o&&(o.disabled=!1),await runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!1})}async function refreshFbLeadClientRowsUi(){const e=document.getElementById("fb-lead-clients-mount"),t=document.getElementById("fb-lead-ready-banner-mount");e&&(e.innerHTML=renderFbLeadClientRows(fbLeadSyncState.clients)),t&&(t.innerHTML=renderFbLeadReadyBanners(fbLeadSyncState.clients)),bindFbLeadSyncToggles()}function renderFbLeadReadyBanners(e){const t=(e||[]).filter(n=>clientHasReadyPreview(n));return t.length?t.map(n=>{const a=n.lastRun?.updated||fbLeadSyncState.previewOkByClient[n.clientId]?.updated||0,s=fbLeadSyncState.previewOkByClient[n.clientId]?.mode||n.lastRun?.mode||"backfill";return`
      <div class="fb-lead-ready-banner">
        <div class="fb-lead-ready-banner-copy">
          <strong>${esc(n.accountName)} \u2014 preview complete</strong>
          <p>${esc(String(a))} contact(s) ready for <code>Fb Lead id</code>. Apply to write to GHL now.</p>
        </div>
        <button class="admin-btn admin-btn--primary" type="button"
          onclick="applyFbLeadSync('${esc(n.clientId)}', '${esc(s)}')">
      ...
      <tr class="fb-lead-audit-row">
        ...
        <td>
          <div class="fb-lead-history-actions">
            ${i?`<button class="admin-btn admin-btn--primary admin-btn--small" type="button" onclick="applyFbLeadSync('${esc(a.clientId)}', '${esc(a.mode||"backfill")}')">Apply (${a.updated})</button>`:""}
  ...
  `}function renderFbLeadSyncPage(e){const t=e.summary||{};return`
    ...
    <div class="sync-history-page">
      ...
      <div id="fb-lead-ready-banner-mount">${renderFbLeadReadyBanners(e.clients||[])}</div>
  ...
  `}async function toggleFbLeadSyncEnabled(e,t){try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"PUT",body:JSON.stringify({fbLeadSyncEnabled:t})}),showToast(t?"Hourly FB lead sync enabled":"Hourly FB lead sync disabled","success");const n=fbLeadSyncState.clients.find(a=>a.clientId===e);n&&(n.fbLeadSyncEnabled=t)}catch(n){showToast(n.message||"Failed to update auto-sync setting.","error"),await loadFbLeadSyncPage({silent:!0})}}function bindFbLeadSyncToggles(){document.querySelectorAll("[data-fb-sync-toggle]").forEach(e=>{e.onchange=()=>toggleFbLeadSyncEnabled(e.dataset.fbSyncToggle,e.checked)})}async function loadFbLeadPreflightForClients(e){const t={};await Promise.all(e.map(async n=>{try{t[n.clientId]=await adminFetch(`/api/fb-lead-sync/preflight?client=${encodeURIComponent(n.clientId)}`)}catch(a){t[n.clientId]={preflightError:a.message}}})),fbLeadSyncState.preflightByClient=t}async function loadFbLeadHistory(e=50){const t=await adminFetch(`/api/fb-lead-sync/history?limit=${e}`);return fbLeadSyncState.historyRuns=t.runs||[],t}function renderFbLeadRunPanel(e,t="recent"){const n=fbLeadSyncState.clients.find(i=>i.clientId===e),a=clientHasReadyPreview(n),s=t==="backfill"?"Backfill (90 days)":"Recent (2 days)";return`
    <div class="fb-lead-run-panel" id="fb-lead-run-panel">
      ...
      ${t==="backfill"&&a?'<div class="fb-lead-banner" style="margin-top:12px;margin-bottom:0;background:#e8f7ef;border-color:#b8e6cf;color:#138b53">Preview already completed \u2014 click <strong>Apply sync \u2014 write to GHL</strong> below.</div>':t==="backfill"?'<div class="fb-lead-banner" style="margin-top:12px;margin-bottom:0">Run Preview first. Backfill only matches leads Meta still has (~90 days).</div>':""}
      <div class="fb-lead-run-actions">
        ...
        <button class="admin-btn admin-btn--primary" type="button" id="fb-lead-apply-btn"${t==="backfill"&&!a?" disabled":""}>Apply sync \u2014 write to GHL</button>
      </div>
  ...
  `}function closeFbLeadRunPanel(){const e=document.getElementById("fb-lead-run-panel-mount");e&&(e.innerHTML=""),fbLeadSyncState.activeRun=null}function openFbLeadRunPanel(e,t){const n=document.getElementById("fb-lead-run-panel-mount");if(!n)return;fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:clientHasReadyPreview(fbLeadSyncState.clients.find(i=>i.clientId===e))},n.innerHTML=renderFbLeadRunPanel(e,t);const a=document.getElementById("fb-lead-preview-btn"),s=document.getElementById("fb-lead-apply-btn");a&&(a.onclick=()=>runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!0})),s&&(s.disabled=t==="backfill"&&!clientHasReadyPreview(client),s.onclick=()=>applyFbLeadSync(e,t)),n.scrollIntoView({behavior:"smooth",block:"nearest"})}function renderFbLeadRunResults(e){const t=e.rows||[];if(!t.length)return'<div class="sync-history-empty" style="padding:12px">No rows in this batch.</div>';const n=t.map(i=>`
    <tr>
  ...
  `}function setFbLeadProgress({processed:e=0,total:t=0,complete:n=!1,label:a=""}={}){const s=document.getElementById("fb-lead-progress-wrap"),i=document.getElementById("fb-lead-progress-bar"),o=document.getElementById("fb-lead-progress-label");if(!s||!i)return;s.hidden=!1,s.classList.toggle("is-complete",n);const c=t>0?Math.min(100,Math.round(e/t*100)):n?100:0;i.style.width=`${c}%`,o&&(o.textContent=a||(n?`Complete \u2014 ${e} / ${t} leads (${c}%)`:`Processing \u2014 ${e} / ${t} leads (${c}%)`))}function hideFbLeadProgress(e=!0){const t=document.getElementById("fb-lead-progress-wrap"),n=document.getElementById("fb-lead-progress-bar");t&&e&&(t.hidden=!0,t.classList.remove("is-complete"),n&&(n.style.width="0%"))}async function refreshFbLeadHistoryUi(){await loadFbLeadHistory();const e=document.getElementById("fb-lead-history-mount");if(!e)return;const t=Object.fromEntries(fbLeadSyncState.clients.map(n=>[n.clientId,n.locationId]));e.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,t)}async function clearFbLeadSyncHistory(){if(!(!isStaffAdmin()||!window.confirm(`Delete all FB lead sync run history?

This only clears the log \u2014 it does not change GHL contacts or Meta data.`)))try{const t=await adminFetch("/api/fb-lead-sync/history",{method:"DELETE"});showToast(`Cleared ${t.deleted||0} run(s).`,"success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(t){showToast(t.message||"Failed to clear FB lead sync log.","error")}}async function deleteFbLeadRun(e){if(!(!isStaffAdmin()||!window.confirm(`Delete run #${e} from history?`)))try{await adminFetch(`/api/fb-lead-sync/history/${e}`,{method:"DELETE"}),showToast("Run deleted.","success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(n){showToast(n.message||"Failed to delete run.","error")}}async function runFbLeadSyncBatch({clientId:e,mode:t,dryRun:n}){const a=document.getElementById("fb-lead-preview-btn"),s=document.getElementById("fb-lead-apply-btn"),i=document.getElementById("fb-lead-run-results");a&&(a.disabled=!0),s&&(s.disabled=!0);let o=fbLeadSyncState.activeRun?.runId||null,c=0,l=null,f=[],h=!1;try{setFbLeadProgress({processed:0,total:0,label:"Starting sync\u2026"});do{l=await adminFetch("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:o,batchOffset:c,batchLimit:25})});const b=l.summary||l;o=b.runId,c=b.nextBatchOffset??b.batchOffset+(b.batchProcessed||0),f=f.concat(b.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=o),setFbLeadProgress({processed:c,total:b.inWindow||c,complete:!b.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...b,rows:f}))}while((l.summary||l).hasMore);h=!0;const v=l.summary||l;if(setFbLeadProgress({processed:v.nextBatchOffset??c,total:v.inWindow||c,complete:!0}),n)markFbLeadPreviewReady(e,t,v.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=!0),s&&(s.disabled=!1),i&&(i.innerHTML+=renderFbLeadApplyCta(e,t,v.updated)),showToast("Preview complete \u2014 click Apply to write to GHL","success");else{delete fbLeadSyncState.previewOkByClient[e];try{const b=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete b[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(b))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success")}await refreshFbLeadHistoryUi();const S=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=S.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(v){hideFbLeadProgress(!0),showToast((v.message||"FB lead sync failed.")+" Partial progress may be saved \u2014 refresh the page and run Preview again.","error")}finally{if(a&&(a.disabled=!1),s){const v=fbLeadSyncState.clients.find(S=>S.clientId===e);s.disabled=t==="backfill"&&!clientHasReadyPreview(v)}h||hideFbLeadProgress(!0)}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=fbLeadSyncState.historyRuns.filter(s=>s.clientId===e),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1}={}){const t=document.getElementById("dashboard");if(!t)return;const n=await fetchStaffMe();if(!n){e||(t.innerHTML=`
        ${renderBrandTopbar("")}
        ${wrapDashboardShell(`
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_CHECK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>',ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),c=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const f=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const h=document.getElementById("dashboard-content");h&&(h.innerHTML=renderKpiSkeleton()),c&&(c.disabled=!0,c.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let h=null;if(!availablePipelines.length){if(h=await bootstrapDashboardData(f),l!==fetchGeneration)return;availablePipelines=h.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,h.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const S=canReuseBootstrapDashboard(h)&&!s;let b,k;if(S?[k,b]=await Promise.all([fetchFacebookMetrics(f),Promise.resolve(h)]):[b,k]=await Promise.all([fetchDashboardData(availablePipelines,f,{forceFresh:s}),fetchFacebookMetrics(f)]),l!==fetchGeneration)return;cachedFacebookMetrics=k,cachedData=applyMarketingData(b,k),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(h){if(l!==fetchGeneration)return;if(h.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
