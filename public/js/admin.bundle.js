...
Resume the interrupted ${b}?`)&&(l=m.id,d=Number(m.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const m=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),c=m.summary||m;l=c.runId,d=0,setFbLeadProgress({processed:0,total:c.inWindow||0,label:`Matching ${c.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||g?.updated||0});do{u=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:I,batchOffset:d,batchLimit:o})});const m=u.summary||u;l=m.runId,d=m.nextBatchOffset??m.batchOffset+(m.batchProcessed||0),y=y.concat(m.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const c=m.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:c,complete:!m.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...m,rows:y}))):updateGlobalApplyProgress({processed:d,total:c,written:m.updated||0,complete:!m.hasMore})}while((u.summary||u).hasMore);v=!0;const $=u.summary||u,f=$.inWindow||d||w,h=$.nextBatchOffset??d;if(n)setFbLeadProgress({processed:h,total:f,complete:!0}),markFbLeadPreviewReady(e,t,$.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=($.updated||0)>0),showToast(($.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:f,written:$.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const m=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete m[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(m))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch($){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${$.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast(($.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!v&&n&&hideFbLeadProgress(!0),!n&&!v&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `}function renderMetaReportMonthBody(e,{editable:t=!1}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return'<div class="meta-report-empty">No ad spend this month</div>';const n=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["Conversion Rate",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],a=[["Leads",metaFmtNum(e.topline?.leads),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let s=renderMetaReportMetricTable("Meta ads",n);if(s+=renderMetaReportMetricTable("Topline KPI'er",a),e.bottomline){const i=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];e.bottomline.censioFee>0&&(i.push(["Censio performance fee",metaFmtKr(e.bottomline.censioFee),""]),i.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),i.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])),s+=renderMetaReportMetricTable("Bottomline KPI'er",i)}if(e.customLineItems?.length){const i=e.customLineItems.map(o=>[o.label,metaFmtKr(o.amount),""]);i.push(["Total",metaFmtKr(e.customLineItemsTotal),""]),s+=renderMetaReportMetricTable("Additional items",i)}return t&&(s+=`
    ...
    `),s}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.summary||{},a=e.meta||{},s=t.map(o=>{const r=!!o.clientId,l=o.needsSetup?'<span class="meta-report-badge meta-report-badge--setup">Not in app</span>':o.metaReportEnabled?'<span class="meta-report-badge meta-report-badge--on">Report on</span>':'<span class="meta-report-badge">Report off</span>',d=o.hasGhl?"GHL + Meta":o.partnerSource==="partner"?"Meta partner":"Meta only",u=r?`<a href="/admin/meta-reports/${encodeURIComponent(o.clientId)}" class="meta-report-client-link">${esc(o.accountName)}</a>`:`<span class="meta-report-client-link">${esc(o.accountName)}</span>`,y=o.reportUrl?`<button type="button" class="admin-btn admin-btn--ghost admin-btn--small" data-copy-report-url="${esc(o.reportUrl)}">Copy link</button>`:'<span class="meta-report-muted">\u2014</span>',v=o.needsSetup?`<button type="button" class="admin-btn admin-btn--primary admin-btn--small" data-meta-provision="${esc(o.metaAdAccountId)}" data-meta-provision-name="${esc(o.accountName)}">Add client</button>`:r?`<a class="admin-btn admin-btn--secondary admin-btn--small" href="/admin/meta-reports/${encodeURIComponent(o.clientId)}">Edit</a>`:"\u2014";return`
      <tr class="${o.needsSetup?"meta-report-row--setup":""}">
        <td>${u}${l?`<div class="meta-report-row-meta">${l}</div>`:""}</td>
        <td><code class="meta-report-code">${esc(o.metaAdAccountId||"\u2014")}</code></td>
        <td>${esc(d)}</td>
        <td class="meta-report-toggle-cell">${r?`<input type="checkbox" class="meta-report-toggle" data-meta-report-enabled="${esc(o.clientId)}" ${o.metaReportEnabled?"checked":""} aria-label="Enable report for ${esc(o.accountName)}" />`:"\u2014"}</td>
        <td class="meta-report-toggle-cell">${r?`<input type="checkbox" class="meta-report-toggle" data-meta-bottomline="${esc(o.clientId)}" ${o.metaReportShowBottomline?"checked":""} ${o.metaReportEnabled?"":"disabled"} aria-label="Bottomline for ${esc(o.accountName)}" />`:"\u2014"}</td>
        <td class="meta-report-toggle-cell">${r?`<input type="checkbox" class="meta-report-toggle" data-meta-fee="${esc(o.clientId)}" ${o.metaReportFeeEnabled?"checked":""} ${o.metaReportEnabled?"":"disabled"} aria-label="Censio fee for ${esc(o.accountName)}" />`:"\u2014"}</td>
        <td>${y}</td>
        <td>${v}</td>
      </tr>
    `}).join(""),i=a.partnerFetchError?`<div class="fb-lead-banner meta-report-banner-warn">${esc(a.partnerFetchError)} Showing only clients already configured in the app.</div>`:"";return`
    ...
    <div class="sync-history-page meta-reports-page">
      ${i}
      <div class="meta-report-summary">
        <div class="meta-report-stat">
          <div class="meta-report-stat-label">Meta ad accounts</div>
          <div class="meta-report-stat-value">${esc(String(n.partnerAccountCount??t.length))}</div>
        </div>
        <div class="meta-report-stat">
          <div class="meta-report-stat-label">In app</div>
          <div class="meta-report-stat-value">${esc(String(n.inAppCount??0))}</div>
        </div>
        <div class="meta-report-stat">
          <div class="meta-report-stat-label">Reports enabled</div>
          <div class="meta-report-stat-value">${esc(String(n.enabledCount??0))}</div>
        </div>
        <div class="meta-report-stat">
          <div class="meta-report-stat-label">Need setup</div>
          <div class="meta-report-stat-value">${esc(String(n.needsSetupCount??0))}</div>
        </div>
      </div>
      ...
      <div class="dashboard-table-wrap">
        <table class="sync-history-table meta-reports-table">
          ...
          <tbody>${s||'<tr><td colspan="8">No Meta ad accounts found. Check META_SYSTEM_USER_TOKEN and META_BUSINESS_ID.</td></tr>'}</tbody>
        </table>
      </div>
    </div>
    `)}
  ...
  `).join("");return`
    ...
    `)}
  `}function bindMetaReportCopyButtons(e=document){e.querySelectorAll("[data-copy-report-url]").forEach(t=>{t.onclick=async()=>{const n=t.getAttribute("data-copy-report-url"),a=`${window.location.origin}${n}`;try{await navigator.clipboard.writeText(a),showToast("Share link copied","success")}catch{showToast(a,"info")}}})}async function patchMetaReportSettings(e,t){await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/settings`,{method:"PATCH",body:JSON.stringify(t)})}function bindMetaReportsHubEvents(){const e=document.getElementById("meta-reports-filter");e&&(e.onchange=()=>{metaReportsState.filter=e.value,loadMetaReportsHubPage()});const t=document.getElementById("meta-reports-refresh");t&&(t.onclick=()=>loadMetaReportsHubPage()),document.querySelectorAll("[data-meta-report-enabled]").forEach(n=>{n.onchange=async()=>{const a=n.getAttribute("data-meta-report-enabled");try{await patchMetaReportSettings(a,{metaReportEnabled:n.checked}),showToast(n.checked?"Report enabled":"Report disabled","success"),await loadMetaReportsHubPage()}catch(s){showToast(s.message||"Update failed","error"),n.checked=!n.checked}}}),document.querySelectorAll("[data-meta-bottomline]").forEach(n=>{n.onchange=async()=>{const a=n.getAttribute("data-meta-bottomline");try{await patchMetaReportSettings(a,{metaReportShowBottomline:n.checked}),showToast("Bottomline setting saved","success")}catch(s){showToast(s.message||"Update failed","error"),n.checked=!n.checked}}}),document.querySelectorAll("[data-meta-fee]").forEach(n=>{n.onchange=async()=>{const a=n.getAttribute("data-meta-fee");try{await patchMetaReportSettings(a,{metaReportFeeEnabled:n.checked}),showToast("Fee setting saved","success")}catch(s){showToast(s.message||"Update failed","error"),n.checked=!n.checked}}}),bindMetaReportCopyButtons(),document.querySelectorAll("[data-meta-provision]").forEach(n=>{n.onclick=async()=>{n.disabled=!0;try{await adminFetch("/api/meta-reports/provision",{method:"POST",body:JSON.stringify({accountName:n.getAttribute("data-meta-provision-name"),metaAdAccountId:n.getAttribute("data-meta-provision"),metaReportEnabled:!0})}),showToast("Client added \u2014 enable report and copy link","success"),await loadMetaReportsHubPage()}catch(a){showToast(a.message||"Failed to add client","error"),n.disabled=!1}}})}function collectMetaReportLineItems(){return Array.from(document.querySelectorAll(".meta-report-line-item-row")).map((e,t)=>({label:e.querySelector(".meta-line-label")?.value||"",amount:Number(e.querySelector(".meta-line-amount")?.value)||0,sortOrder:t})).filter(e=>e.label.trim())}function bindMetaReportsClientEvents(e){const t=e.clientId;document.querySelectorAll("[data-meta-month-tab]").forEach(l=>{l.onclick=()=>{metaReportsState.activeMonthKey=l.getAttribute("data-meta-month-tab");const d=document.getElementById("meta-report-month-panel");d&&(d.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],{editable:!0}),bindMetaReportsClientEditEvents(t,e)),document.querySelectorAll("[data-meta-month-tab]").forEach(u=>{u.classList.toggle("is-active",u.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)})}});const n=document.getElementById("meta-report-year");n&&(n.onchange=()=>{metaReportsState.selectedYear=Number(n.value),metaReportsState.activeMonthKey=null,loadMetaReportsClientPage()}),bindMetaReportCopyButtons(),bindMetaReportsClientEditEvents(t,e);const a=document.getElementById("meta-report-setting-bottomline"),s=document.getElementById("meta-report-setting-fee"),i=document.getElementById("meta-report-setting-fee-percent"),o=async()=>{await patchMetaReportSettings(t,{metaReportShowBottomline:a?.checked,metaReportFeeEnabled:s?.checked,metaReportFeePercent:Number(i?.value)||20}),await loadMetaReportsClientPage()};a&&(a.onchange=o),s&&(s.onchange=o),i&&(i.onchange=o);const r=document.getElementById("meta-report-rotate-token");r&&(r.onclick=async()=>{window.confirm("Rotate the client share link? The old link will stop working.")&&(await patchMetaReportSettings(t,{rotateAccessToken:!0}),showToast("Share link rotated","success"),await loadMetaReportsClientPage())})}function bindMetaReportsClientEditEvents(e,t){const n=document.getElementById("meta-report-add-line");n&&(n.onclick=()=>{const i=document.getElementById("meta-report-line-items-list");if(!i)return;const o=document.createElement("div");o.className="meta-report-line-item-row",o.innerHTML=`
        ...
        <button type="button" class="admin-btn admin-btn--ghost admin-btn--small meta-line-remove">Remove</button>
      `,i.appendChild(o),o.querySelector(".meta-line-remove")?.addEventListener("click",()=>o.remove())}),document.querySelectorAll(".meta-line-remove").forEach(i=>{i.onclick=()=>i.closest(".meta-report-line-item-row")?.remove()});const a=document.getElementById("meta-report-refresh-meta");a&&(a.onclick=async()=>{a.disabled=!0;try{const i=metaReportsState.activeMonthKey;await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(i)}/refresh`,{method:"POST"}),showToast("Meta data refreshed","success"),await loadMetaReportsClientPage()}catch(i){showToast(i.message||"Meta refresh failed","error")}finally{a.disabled=!1}});const s=document.getElementById("meta-report-save-month");s&&(s.onclick=async()=>{s.disabled=!0;try{const i=metaReportsState.activeMonthKey,o={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:Number(document.getElementById("meta-report-avg-profit")?.value),lineItems:collectMetaReportLineItems()};await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(i)}`,{method:"PUT",body:JSON.stringify(o)}),showToast("Month saved","success"),await loadMetaReportsClientPage()}catch(i){showToast(i.message||"Save failed","error")}finally{s.disabled=!1}})}function bindPublicMetaReportEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(n=>{n.onclick=()=>{metaReportsState.activeMonthKey=n.getAttribute("data-meta-month-tab");const a=document.getElementById("meta-report-month-panel");a&&(a.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],{editable:!1})),document.querySelectorAll("[data-meta-month-tab]").forEach(s=>{s.classList.toggle("is-active",s.getAttribute("data-meta-month-tab")===metaReportsState.activeMonthKey)})}});const t=document.getElementById("meta-report-year");t&&(t.onchange=()=>{metaReportsState.selectedYear=Number(t.value),metaReportsState.activeMonthKey=null,loadPublicMetaReportPage()})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
      ${renderBrandTopbar("")}
    ...
    `;return}currentStaffUser=t,e.innerHTML=`
    ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
    ${wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading Meta reports...</p></div>')}
  `;try{const n=await adminFetch(`/api/meta-reports?filter=${encodeURIComponent(metaReportsState.filter)}`);metaReportsState.clients=n.clients||[],metaReportsState.summary=n.summary||{},metaReportsState.metaInfo=n.meta||{},e.innerHTML=renderMetaReportsHubPage(n),bindMetaReportsHubEvents()}catch(n){e.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
      ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(n.message)}</div>`)}
    `}}async function loadMetaReportsClientPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){window.location.href=`/login?next=${encodeURIComponent(window.location.pathname)}`;return}currentStaffUser=t;const n=CLIENT_SLUG;e.innerHTML=`
    ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
    ${wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading report editor...</p></div>')}
  `;try{const a=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(n)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.clientPayload=a,metaReportsState.activeMonthKey||(metaReportsState.activeMonthKey=a.monthKeys?.[a.monthKeys.length-1]||null),e.innerHTML=renderMetaReportsClientPage(a),bindMetaReportsClientEvents(a)}catch(a){e.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("meta-reports"))}
      ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(a.message)}</div>`)}
    `}}async function loadPublicMetaReportPage(){const e=document.getElementById("dashboard");if(!(!e||!REPORT_TOKEN)){e.innerHTML=wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading report...</p></div>');try{const t=await fetch(`/api/meta-reports/public/${encodeURIComponent(REPORT_TOKEN)}?year=${encodeURIComponent(metaReportsState.selectedYear)}`).then(async n=>{const a=await n.json();if(!n.ok)throw new Error(a.error||"Report not found");return a});metaReportsState.publicPayload=t,metaReportsState.activeMonthKey||(metaReportsState.activeMonthKey=t.monthKeys?.[t.monthKeys.length-1]||null),e.innerHTML=renderPublicMetaReportPage(t),bindPublicMetaReportEvents(t)}catch(t){e.innerHTML=wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(t.message||"Report not found")}</div>`)}}}function renderAdminHubPage(e){const t=e.length,n=currentStaffUser?`
