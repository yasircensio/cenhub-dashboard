...
Resume the interrupted ${b}?`)&&(r=m.id,d=Number(m.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const m=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:r})}),u=m.summary||m;r=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||g?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:r,previewRunId:n?null:S,batchOffset:d,batchLimit:o})});const m=c.summary||c;r=m.runId,d=m.nextBatchOffset??m.batchOffset+(m.batchProcessed||0),y=y.concat(m.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=r);const u=m.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:u,complete:!m.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...m,rows:y}))):updateGlobalApplyProgress({processed:d,total:u,written:m.updated||0,complete:!m.hasMore})}while((c.summary||c).hasMore);v=!0;const $=c.summary||c,f=$.inWindow||d||w,h=$.nextBatchOffset??d;if(n)setFbLeadProgress({processed:h,total:f,complete:!0}),markFbLeadPreviewReady(e,t,$.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=($.updated||0)>0),showToast(($.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:f,written:$.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const m=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete m[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(m))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch($){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${$.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast(($.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!v&&n&&hideFbLeadProgress(!0),!n&&!v&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function renderMetaReportHubCard(e){const t=!!e.clientId,n=e.clientId||e.metaAdAccountId||"",a=e.needsSetup?"is-setup":e.metaReportEnabled?"is-live":"",s=e.needsSetup?'<span class="meta-report-badge meta-report-badge--setup">Needs setup</span>':e.metaReportEnabled?'<span class="meta-report-badge meta-report-badge--on">Report live</span>':'<span class="meta-report-badge meta-report-badge--off">Report off</span>',i=e.hasGhl?'<span class="meta-report-badge meta-report-badge--type">GHL + Meta</span>':'<span class="meta-report-badge meta-report-badge--type">Meta only</span>',o=t?`<a href="/admin/meta-reports/${encodeURIComponent(e.clientId)}" class="meta-hub-card-name">${esc(e.accountName)}</a>`:`<span class="meta-hub-card-name">${esc(e.accountName)}</span>`,l=e.reportUrl?`<button type="button" class="admin-btn admin-btn--ghost admin-btn--small" data-copy-report-url="${esc(e.reportUrl)}">Copy link</button>`:"",r=e.needsSetup?`<button type="button" class="admin-btn admin-btn--primary admin-btn--small" data-meta-provision="${esc(e.metaAdAccountId)}" data-meta-provision-name="${esc(e.accountName)}">Add client</button>`:t?`<a class="admin-btn admin-btn--secondary admin-btn--small" href="/admin/meta-reports/${encodeURIComponent(e.clientId)}">Edit report</a>`:"";return`
    <article class="meta-hub-card ${a}" data-meta-hub-row="${esc(n)}">
      <div class="meta-hub-card-head">
        <div class="meta-hub-avatar" aria-hidden="true">${metaHubClientInitial(e.accountName)}</div>
        <div class="meta-hub-card-title">
          ${o}
          <div class="meta-hub-card-badges">${s}${i}</div>
        </div>
        <div class="meta-hub-card-actions">${l}${r}</div>
      </div>
      <div class="meta-hub-card-meta">
        <code>act_${esc(e.metaAdAccountId||"\u2014")}</code>
        ${e.partnerSource==="partner"?'<span class="meta-hub-card-meta-dot">\xB7</span><span>Partner account</span>':""}
        ${e.partnerSource==="app-only"?'<span class="meta-hub-card-meta-dot">\xB7</span><span>In app only</span>':""}
      </div>
  ...
  `}function renderMetaReportsSummaryHtml(e){const t=e.totalListed??e.inAppCount??0,n=e.partnerAccountCount??0;return`
    <div class="meta-hub-stat">
      <div class="meta-hub-stat-icon meta-hub-stat-icon--total" aria-hidden="true">${ICON_CHART}</div>
      <div class="meta-hub-stat-body">
        <div class="meta-hub-stat-value">${esc(String(t))}</div>
        <div class="meta-hub-stat-label">Clients listed</div>
        ${n>0?`<div class="meta-hub-stat-hint">${esc(String(n))} from Meta BM</div>`:""}
      </div>
    </div>
    <div class="meta-hub-stat">
      <div class="meta-hub-stat-icon meta-hub-stat-icon--live" aria-hidden="true">${ICON_CHECK}</div>
      <div class="meta-hub-stat-body">
        <div class="meta-hub-stat-value">${esc(String(e.enabledCount??0))}</div>
        <div class="meta-hub-stat-label">Reports live</div>
      </div>
    </div>
    <div class="meta-hub-stat">
      <div class="meta-hub-stat-icon meta-hub-stat-icon--app" aria-hidden="true">${ICON_PLUS}</div>
      <div class="meta-hub-stat-body">
        <div class="meta-hub-stat-value">${esc(String(e.inAppCount??0))}</div>
        <div class="meta-hub-stat-label">Configured in app</div>
      </div>
    </div>
    <div class="meta-hub-stat">
      <div class="meta-hub-stat-icon meta-hub-stat-icon--setup" aria-hidden="true">${ICON_SYNC}</div>
      <div class="meta-hub-stat-body">
        <div class="meta-hub-stat-value">${esc(String(e.needsSetupCount??0))}</div>
        <div class="meta-hub-stat-label">Need setup</div>
      </div>
    </div>
  ...
  `}function renderMetaReportMonthBody(e,{editable:t=!1}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return'<div class="meta-report-empty">No ad spend this month</div>';const n=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["Conversion Rate",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],a=[["Leads",metaFmtNum(e.topline?.leads),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let s=renderMetaReportMetricTable("Meta ads",n);if(s+=renderMetaReportMetricTable("Topline KPI'er",a),e.bottomline){const i=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];e.bottomline.censioFee>0&&(i.push(["Censio performance fee",metaFmtKr(e.bottomline.censioFee),""]),i.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),i.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])),s+=renderMetaReportMetricTable("Bottomline KPI'er",i)}if(e.customLineItems?.length){const i=e.customLineItems.map(o=>[o.label,metaFmtKr(o.amount),""]);i.push(["Total",metaFmtKr(e.customLineItemsTotal),""]),s+=renderMetaReportMetricTable("Additional items",i)}return t&&(s+=`
    ...
    `),s}function renderMetaReportsHubPage(e){const t=e.clients||[],n={...e.summary,totalListed:e.allClients?.length??e.summary?.totalListed??t.length},a=e.meta||{};return`
    ...
    <div class="sync-history-page meta-reports-page">
      <div class="meta-hub-layout">
        ...
        <div class="meta-hub-stats" id="meta-reports-summary">
          ${renderMetaReportsSummaryHtml(n)}
        </div>
        <div class="meta-hub-toolbar">
          <div class="meta-hub-toolbar-left">
            <a class="admin-btn admin-btn--secondary" href="/admin">\u2190 Back</a>
            <div class="hub-search meta-hub-search">
              ${ICON_SEARCH}
          ...
          <div class="meta-hub-toolbar-right">
            ...
            <span class="meta-hub-count" id="meta-reports-count">${t.length} shown</span>
      ...
      <div class="meta-report-toolbar">
        <label>Year
          <select id="meta-report-year" class="admin-select">
            ${[e.year,e.year-1].filter((o,l,r)=>r.indexOf(o)===l).map(o=>`
              <option value="${o}"${Number(metaReportsState.selectedYear)===Number(o)?" selected":""}>${o}</option>
            `).join("")}
          </select>
        </label>
  ...
  `).join("");return`
    ...
    <div class="meta-reports-page meta-reports-page--public">
      <div class="meta-report-toolbar">
        <label>Year
          <select id="meta-report-year" class="admin-select">
            ${[e.year,e.year-1].filter((i,o,l)=>l.indexOf(i)===o).map(i=>`
