...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),y=w.summary||w;l=y.runId,c=0,setFbLeadProgress({processed:0,total:y.inWindow||0,label:`Matching ${y.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:s})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const y=w.inWindow||c||$;a?(setFbLeadProgress({processed:c,total:y,complete:!w.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:y,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||$,v=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const g=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=g.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function parseComparisonDateKey(e){const t=String(e||"").match(/^([ab])-(start|end)$/);return t?{side:t[1],bound:t[2],stateKey:t[1]==="b"?"comparisonPeriodB":"comparisonPeriodA"}:null}function metaReportComparisonSafePickerBounds(e,t){const a=e||void 0,n=t||void 0;return a&&n&&a>n?{min:n,max:a}:{min:a,max:n}}function syncMetaReportComparisonDateDisplays(){document.querySelectorAll("[data-comparison-date]").forEach(e=>{const t=e.getAttribute("data-comparison-date"),a=document.querySelector(`[data-comparison-date-trigger="${t}"]`);if(!t||!a)return;const n=e.value||"",o=a.querySelector(".meta-report-comparison-date-trigger-value");o&&(o.textContent=n?metaReportComparisonFormatDateLabel(n):"Select date"),a.classList.toggle("has-value",!!n),a.classList.toggle("is-empty",!n)})}function applyMetaReportComparisonDateChange(e,t,a,{editable:n=!1}={}){const o=parseComparisonDateKey(t);if(!o||!a)return;const{side:r,bound:s,stateKey:i}=o;metaReportsState[i]||(metaReportsState[i]={startDate:null,endDate:null});const l=metaReportsState[i];l[`${s}Date`]=a,l.startDate&&l.endDate&&l.startDate>l.endDate&&(s==="end"?l.startDate=l.endDate:l.endDate=l.startDate);const c=document.querySelector(`[data-comparison-date="${r}-start"]`),d=document.querySelector(`[data-comparison-date="${r}-end"]`);c&&(c.value=l.startDate||""),d&&(d.value=l.endDate||""),syncMetaReportComparisonDateDisplays(),metaReportsState.comparisonMode="custom",document.querySelectorAll("[data-comparison-preset]").forEach(u=>{const p=u.getAttribute("data-comparison-preset")==="custom";u.classList.toggle("active",p),u.setAttribute("aria-pressed",p)}),refreshMetaReportComparison(e,{editable:n})}function ensureMetaReportComparisonDatePickerPopover(){if(document.getElementById("meta-report-comparison-date-popover"))return;const e=document.createElement("div");e.id="meta-report-comparison-date-backdrop",e.className="meta-report-comparison-date-backdrop",e.hidden=!0,e.addEventListener("click",closeMetaReportComparisonDatePicker),document.body.appendChild(e);const t=document.createElement("div");t.id="meta-report-comparison-date-popover",t.className="meta-report-comparison-date-popover",t.hidden=!0,t.setAttribute("role","dialog"),t.setAttribute("aria-modal","true"),document.body.appendChild(t)}function isMetaReportComparisonDatePickerDayDisabled(e,t,a,n){const{min:o,max:r}=getMetaReportComparisonDatePickerFieldBounds(t,a,n);return!!(o&&e<o||r&&e>r)}function metaReportComparisonDatePickerCanShiftMonth(e){const{viewYear:t,viewMonth:a,payload:n,side:o,bound:r}=metaReportComparisonDatePickerState;if(!t||!a||!n||!o||!r)return!1;let s=a+e,i=t;s<1&&(s=12,i-=1),s>12&&(s=1,i+=1);const l=`${i}-${String(s).padStart(2,"0")}`,{min:c,max:d}=getMetaReportComparisonDatePickerFieldBounds(n,o,r),u=c?c.slice(0,7):null,p=d?d.slice(0,7):null;return!(u&&l<u||p&&l>p)}function renderMetaReportComparisonDatePickerDayCell(e,t,a,n,o,r,s,i,l,c){const d=["date-picker-day"];if(a&&d.push("is-outside"),e===n&&d.push("is-today"),e===o&&d.push("is-selected"),r&&s){const u=r<=s?r:s,p=r<=s?s:r;e>=u&&e<=p&&d.push("is-in-range"),e===r&&d.push("is-range-start"),e===s&&d.push("is-range-end")}return isMetaReportComparisonDatePickerDayDisabled(e,i,l,c)?(d.push("is-disabled"),`<button type="button" class="${d.join(" ")}" disabled aria-disabled="true">${t}</button>`):`<button type="button" class="${d.join(" ")}" onclick="selectMetaReportComparisonDatePickerDay('${e}')">${t}</button>`}function renderMetaReportComparisonDatePickerPopover(){const e=document.getElementById("meta-report-comparison-date-popover"),{viewYear:t,viewMonth:a,inputKey:n,payload:o,side:r,bound:s}=metaReportComparisonDatePickerState;if(!e||!n||!t||!a||!o||!r||!s)return;const i=new Intl.DateTimeFormat("en-GB",{month:"long",year:"numeric"}).format(new Date(t,a-1,1)),l=["Mo","Tu","We","Th","Fr","Sa","Su"],d=(new Date(t,a-1,1).getDay()+6)%7,u=new Date(t,a,0).getDate(),p=new Date(t,a-1,0).getDate(),h=isoFromParts(new Date().getFullYear(),new Date().getMonth()+1,new Date().getDate()),$=document.querySelector(`[data-comparison-date="${n}"]`)?.value||"",f=metaReportsState[r==="b"?"comparisonPeriodB":"comparisonPeriodA"]||{},v=f.startDate||"",g=f.endDate||"",{min:w,max:y}=getMetaReportComparisonDatePickerFieldBounds(o,r,s);let S="";for(let R=d-1;R>=0;R-=1){const C=p-R,B=a===1?12:a-1,A=a===1?t-1:t,M=isoFromParts(A,B,C);S+=renderMetaReportComparisonDatePickerDayCell(M,C,!0,h,$,v,g,o,r,s)}for(let R=1;R<=u;R+=1){const C=isoFromParts(t,a,R);S+=renderMetaReportComparisonDatePickerDayCell(C,R,!1,h,$,v,g,o,r,s)}const I=(7-(d+u)%7)%7;for(let R=1;R<=I;R+=1){const C=a===12?1:a+1,B=a===12?t+1:t,A=isoFromParts(B,C,R);S+=renderMetaReportComparisonDatePickerDayCell(A,R,!0,h,$,v,g,o,r,s)}const E=s==="start"&&y?`<span class="date-picker-hint">Through ${esc(metaReportComparisonFormatDateLabel(y))}</span>`:s==="end"&&w?`<span class="date-picker-hint">From ${esc(metaReportComparisonFormatDateLabel(w))}</span>`:"";e.innerHTML=`
    <div class="date-picker-panel">
      ...
      <div class="date-picker-weekdays">
        ${l.map(R=>`<div class="date-picker-weekday">${R}</div>`).join("")}
      </div>
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null,settings:o=null}={}){const r=n||e?.monthKey||null,s=r?metaReportMonthBounds(r):null,i=o||a?.settings||{},l=isMetaReportGhlListClient(i),c=resolveClientToplineMode(i),d=resolveMetaToplineSource(e),u=l&&c==="cenhub"&&d==="ghl",p=l&&(d==="manual"||c==="cenhub"&&d!=="ghl");let h="";if(t&&e&&!e.meta?.emptyMonth&&s){const E=u?" readonly":"",R=e.manualLeads??e.inputs?.manualLeads??"",C=renderMetaReportMonthManualRow(e,i);h=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        ...
        <div class="meta-report-edit-grid">
          ...
          ${p?`<label>Manual leads (optional)<input type="number" step="any" id="meta-report-manual-leads" value="${esc(R)}" /></label>`:""}
    ...
    `}const m=n||e?.monthKey||null,$=t?m?`Report preview \xB7 ${metaMonthLabel(m)}`:"Report preview":m?`Report \xB7 ${metaMonthLabel(m)}`:"Report",b=metaReportsState.reportViewMode||"monthly",f=shouldShowMetaReportYearVisuals(e,a,{editable:t}),v=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${$}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",g=renderMetaReportMonthTables(e,{editable:t}),w=f?renderMetaReportChartsPanel({editable:t}):"",y=t&&f?renderMetaReportScenarioCard():"",S=f?renderMetaReportViewModeTabs():"";let I;return f?I=`
      ...
      ${y}
    `:I=`<div class="meta-report-content-main">${g}</div>`,`${h}${v}${S}${I}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
...
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!m)return;m.trim().toLowerCase()==="overwrite"&&(u=!1,p=!0)}const h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(c)}`,{method:"POST",body:JSON.stringify({skipManual:u,overwriteManual:p})});r?await r():s&&await s(h,null,{yearSync:!0}),showToast(`Synced ${h.synced?.length||0} month(s)${h.skipped?.length?`, skipped ${h.skipped.length}`:""}`,"success")}catch(c){showToast(c.message||"Year sync failed","error")}finally{o&&o(!1)}});const l=document.getElementById(`${t}-refresh-snapshot`);l&&(l.onclick=async()=>{const c=a?.();if(c){o&&o(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/refresh-snapshot`,{method:"POST",body:"{}"});s&&await s(d,c),showToast("Cenhub data refreshed and synced","success")}catch(d){showToast(d.message||"Refresh failed","error")}finally{o&&o(!1)}}})}function monthHasCenhubSnapshot(e){if(resolveMetaToplineSource(e)==="ghl")return!0;const a=e?.ghlTotals||{};return!!(e?.ghlSyncedAt||a.leads!=null||a.wonLeads!=null)}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),a=e.monthKeys||[];return t.length?t.map(n=>`
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),$=META_REPORT_FEE_MODE_META.map(({value:g,icon:w,title:y,desc:S})=>`
        ...
        </label>`).join(""),b=o?metaReportsState.clientReportSettingsExpanded:!0,f=o?`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,v=o&&!r?renderMetaReportBottomlineFeeSummary(e,t):"";return`
    <div class="meta-report-bottomline-fee-settings${o?" meta-report-bottomline-fee-settings--client":""}${o?b?" is-editing":" is-collapsed":""}">
      ${v}
      <div class="meta-report-bottomline-editor${o&&!b?" is-hidden":""}" id="${t}-fee-editor">
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(L=>L.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    `;const o=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,o);const r=t.editorPayload?.months?.[n]||null,s=a.metaReportShowBottomline,i=!!a.metaReportGhlDataEnabled,l=a.metaReportToplineMode==="cenhub"?"cenhub":"meta",c=o.toplineSource||r?.dataSource||"meta",d=i&&l==="cenhub"&&c==="ghl",u=i&&c==="manual",p=o.wonLeads!=null?o.wonLeads:r?.inputs?.wonLeads??r?.topline?.wonLeads??"",h=o.avgLeadValue!=null?o.avgLeadValue:r?.inputs?.avgLeadValue??r?.topline?.avgLeadValue??"",m=o.avgProfitPerWon!=null?o.avgProfitPerWon:r?.inputs?.avgProfitPerWon??"",$=o.ghlLeads??r?.ghlTotals?.leads??"",b=o.ghlTotalRevenue??r?.ghlTotals?.totalRevenue??"",f=o.ghlTotalProfit??r?.ghlTotals?.totalProfit??"",v=metaReportsState.customValues.draftInputs,g=v?.wonLeads??p,w=v?.avgLeadValue??h,y=v?.avgProfitPerWon??m,S=v?.manualLeads??o.manualLeads??"",I=d?" readonly":"",E=d?" disabled":"",R=t.settingsSaving&&t.settingsSavingScope==="report",C=i?renderMetaCvToplineSourceControl(a,e):"",B=renderMetaCvMonthManualRow(r||{dataSource:c},a),A=u?`
  ...
  `:"",M={metaReportGhlDataEnabled:a.metaReportGhlDataEnabled,metaReportToplineMode:a.metaReportToplineMode},T=i?renderMetaMonthSourcePill(r||{dataSource:c},M):"";return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      ${A}
      <label>Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(g)}"${I} />
      </label>
      ...
      <label>Avg profit per won${s?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(y)}"${I} />
      </label>
    </div>
    ${B}
    <div class="meta-report-settings-group meta-report-settings-group--stacked meta-cv-client-settings${R?" is-saving":""}">
      ${renderMetaReportSaveIndicator("meta-cv-settings-status",R)}
