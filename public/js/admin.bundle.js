...
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!h)return;h.trim().toLowerCase()==="overwrite"&&(u=!1,p=!0)}const f=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(c)}`,{method:"POST",body:JSON.stringify({skipManual:u,overwriteManual:p})});r?await r():s&&await s(f,null,{yearSync:!0}),showToast(`Synced ${f.synced?.length||0} month(s)${f.skipped?.length?`, skipped ${f.skipped.length}`:""}`,"success")}catch(c){showToast(c.message||"Year sync failed","error")}finally{o&&o(!1)}});const l=document.getElementById(`${t}-refresh-snapshot`);l&&(l.onclick=async()=>{const c=a?.();if(c){o&&o(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/refresh-snapshot`,{method:"POST",body:"{}"});s&&await s(d,c),showToast("Cenhub data refreshed and synced","success")}catch(d){showToast(d.message||"Refresh failed","error")}finally{o&&o(!1)}}})}function monthHasCenhubSnapshot(e,t={}){return e?resolveMetaToplineSource(e,t)==="ghl"?!0:!!e.ghlSyncedAt:!1}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),a=e.monthKeys||[];return t.length?t.map(n=>`
  ...
  `}function bindMetaReportsGhlClientsEvents(){document.querySelectorAll("[data-ghl-list-toggle]").forEach(e=>{e.onchange=async()=>{const t=e.getAttribute("data-ghl-list-toggle"),a=e.checked,n=!a;metaReportsState.ghlClients.savingClientId=t;try{await adminFetch(`/api/meta-reports/ghl-clients/${encodeURIComponent(t)}`,{method:"PATCH",body:JSON.stringify({enabled:a})});const o=metaReportsState.ghlClients.data?.clients?.find(r=>r.clientId===t);o&&(o.onGhlList=a),showToast(a?"Added to Cenhub list":"Removed from Cenhub list","success")}catch(o){e.checked=n,showToast(o.message||"Update failed","error")}finally{metaReportsState.ghlClients.savingClientId=null}}})}async function loadMetaReportsGhlClientsPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){window.location.href=`/login?next=${encodeURIComponent(window.location.pathname)}`;return}currentStaffUser=t,metaReportsState.ghlClients.mounted||(e.innerHTML=`
    ...
    `);try{const o=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(o.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=o,t||(o.clients||[]).some(s=>s.clientId===metaReportsState.customValues.selectedClientId)||(metaReportsState.customValues.selectedClientId=null,metaReportsState.customValues.selectedMonthKey=null,metaReportsState.customValues.editorPayload=null),a.innerHTML=renderMetaReportsCustomValuesPage(o),metaReportsState.customValues.mounted=!0,metaReportsState.customValues.saving=!1,bindMetaReportsCustomValuesEvents(o)}catch(o){if(!metaReportsState.customValues.mounted)a.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("meta-reports-custom"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(o.message)}</div>`)}
      `;else{if(e)throw o;showToast(o.message||"Load failed","error")}}}function renderAdminHubPage(e){const t=e.length,a=currentStaffUser?`
  ...
  `}function filterHubCards(){const e=(document.getElementById("hub-search")?.value||"").trim().toLowerCase();let t=0;document.querySelectorAll("#client-grid .client-card").forEach(n=>{const o=!e||(n.dataset.search||"").includes(e);n.style.display=o?"":"none",o&&(t+=1)});const a=document.getElementById("hub-no-results");a&&(a.style.display=e&&!t?"":"none")}function renderAddClientPanel(){return`
