...
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!b)return;b.trim().toLowerCase()==="overwrite"&&(u=!1,p=!0)}const h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(c)}`,{method:"POST",body:JSON.stringify({skipManual:u,overwriteManual:p})});r?await r():s&&await s(h,null,{yearSync:!0}),showToast(`Synced ${h.synced?.length||0} month(s)${h.skipped?.length?`, skipped ${h.skipped.length}`:""}`,"success")}catch(c){showToast(c.message||"Year sync failed","error")}finally{o&&o(!1)}});const l=document.getElementById(`${t}-refresh-snapshot`);l&&(l.onclick=async()=>{const c=a?.();if(c){o&&o(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/refresh-snapshot`,{method:"POST",body:"{}"});s&&await s(d,c),showToast("Cenhub data refreshed and synced","success")}catch(d){showToast(d.message||"Refresh failed","error")}finally{o&&o(!1)}}})}function monthHasCenhubSnapshot(e,t={}){return e?resolveMetaToplineSource(e,t)==="ghl"?!0:!!e.ghlSyncedAt:!1}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),a=e.monthKeys||[];return t.length?t.map(n=>`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(L=>L.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    `;const o=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,o);const r=t.editorPayload?.months?.[n]||null,s=a.metaReportShowBottomline,i=!!a.metaReportGhlDataEnabled,l={metaReportGhlDataEnabled:a.metaReportGhlDataEnabled,metaReportToplineMode:a.metaReportToplineMode},c=resolveClientToplineMode(l),d=resolveMetaToplineSource(r||{dataSource:o.toplineSource},l),u=i&&c==="cenhub"&&d==="ghl",p=o.wonLeads!=null?o.wonLeads:r?.inputs?.wonLeads??r?.topline?.wonLeads??"",h=o.avgLeadValue!=null?o.avgLeadValue:r?.inputs?.avgLeadValue??r?.topline?.avgLeadValue??"",b=o.avgProfitPerWon!=null?o.avgProfitPerWon:r?.inputs?.avgProfitPerWon??"",v=o.ghlLeads??r?.ghlTotals?.leads??r?.topline?.leads??"",w=o.ghlTotalRevenue??r?.ghlTotals?.totalRevenue??"",m=o.ghlTotalProfit??r?.ghlTotals?.totalProfit??"",f=metaReportsState.customValues.draftInputs,g=i&&c==="cenhub"&&d==="manual",$=u?v:f?.manualLeads??(g?o.manualLeads??r?.topline?.leads??"":r?.topline?.leads??o.manualLeads??""),y=f?.wonLeads??p,S=f?.avgLeadValue??h,k=f?.avgProfitPerWon??b,E=u?' readonly disabled tabindex="-1" aria-readonly="true"':"",I=u?" disabled":"",C=u?" is-locked":"",B=t.settingsSaving&&t.settingsSavingScope==="report",A=i?renderMetaCvToplineSourceControl(a,e):"",M=renderMetaCvMonthManualRow(r||{dataSource:d},a),T=i?renderMetaMonthSourcePill(r||{dataSource:d},l):"";return`
    <div class="meta-report-edit-panel-head">
      <div>
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label class="meta-cv-value-field${C}">Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(y)}"${E} />
      </label>
