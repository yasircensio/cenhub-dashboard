...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),v=w.summary||w;l=v.runId,c=0,setFbLeadProgress({processed:0,total:v.inWindow||0,label:`Matching ${v.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:b,batchOffset:c,batchLimit:s})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const v=w.inWindow||c||$;a?(setFbLeadProgress({processed:c,total:v,complete:!w.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:v,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const y=d.summary||d,f=y.inWindow||c||$,h=y.nextBatchOffset??c;if(a)setFbLeadProgress({processed:h,total:f,complete:!0}),markFbLeadPreviewReady(e,t,y.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(y.updated||0)>0),showToast((y.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:f,written:y.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const g=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=g.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(y){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${y.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((y.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null,settings:o=null}={}){const r=n||e?.monthKey||null,s=r?metaReportMonthBounds(r):null,i=o||a?.settings||{},l=isMetaReportGhlListClient(i),c=resolveClientToplineMode(i),d=resolveMetaToplineSource(e),u=l&&c==="cenhub"&&d==="ghl",p=l&&(d==="manual"||c==="cenhub"&&d!=="ghl");let b="";if(t&&e&&!e.meta?.emptyMonth&&s){const E=u?" readonly":"",I=e.manualLeads??e.inputs?.manualLeads??"",C=renderMetaReportMonthManualRow(e,i);b=`
      <div class="meta-report-edit-panel" id="meta-report-edit-panel">
        ...
        <div class="meta-report-edit-grid">
          ...
          ${p?`<label>Manual leads (optional)<input type="number" step="any" id="meta-report-manual-leads" value="${esc(I)}" /></label>`:""}
          <label>Won leads<input type="number" step="any" id="meta-report-won-leads" value="${esc(e.topline?.wonLeads??"")}"${E} /></label>
    ...
    `}const m=n||e?.monthKey||null,$=t?m?`Report preview \xB7 ${metaMonthLabel(m)}`:"Report preview":m?`Report \xB7 ${metaMonthLabel(m)}`:"Report",y=metaReportsState.reportViewMode||"monthly",f=shouldShowMetaReportYearVisuals(e,a,{editable:t}),h=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${$}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",g=renderMetaReportMonthTables(e,{editable:t}),w=f?renderMetaReportChartsPanel({editable:t}):"",v=t&&f?renderMetaReportScenarioCard():"",S=f?renderMetaReportViewModeTabs():"";let k;return f?k=`
      ...
      ${v}
    `:k=`<div class="meta-report-content-main">${g}</div>`,`${b}${h}${S}${k}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
...
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!r)return!1;r.trim().toLowerCase()==="overwrite"&&(n={...n,skipManual:!1,overwriteManual:!0})}}return await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/switch-topline-source`,{method:"POST",body:JSON.stringify(n)}),showToast(a==="cenhub"?"Switched all months to Cenhub":"Switched all months to Meta Analytics","success"),typeof t.reload=="function"?await t.reload():await loadMetaReportsClientPage({silent:!0}),!0}function bindMetaReportToplineChangeModal(e){const t=e.clientId;document.querySelectorAll('input[name="meta-topline-draft-mode"]').forEach(n=>{n.addEventListener("change",updateMetaReportToplineApplyState)}),document.getElementById("meta-report-topline-ack")?.addEventListener("change",updateMetaReportToplineApplyState),document.getElementById("meta-report-topline-confirm")?.addEventListener("input",updateMetaReportToplineApplyState);const a=document.getElementById("meta-report-topline-apply");a&&(a.onclick=async()=>{const n=getMetaReportToplineDraftMode(),o=resolveClientToplineMode(e.settings||{});if(n===o)return;a.disabled=!0,document.getElementById("meta-report-topline-modal-loading")?.removeAttribute("hidden");const r=typeof e.reload=="function";r?setMetaCvSavePending(!0):(metaReportsState.clientToplineSaving=!0,syncMetaReportToplineSourceBar(metaReportsState.clientPayload||e));try{await applyMetaReportToplineSwitch(t,e,n)&&closeMetaReportModal()}catch(s){showToast(s.message||"Could not switch data source","error")}finally{r?setMetaCvSavePending(!1):(metaReportsState.clientToplineSaving=!1,syncMetaReportToplineSourceBar(metaReportsState.clientPayload)),document.getElementById("meta-report-topline-modal-loading")?.setAttribute("hidden","")}})}function renderMetaReportMonthManualRow(e,t={}){if(!isMetaReportGhlListClient(t)||resolveClientToplineMode(t)!=="cenhub")return"";const o=resolveMetaToplineSource(e)==="manual",r=!o;return!o&&!r?"":`
  ...
  `}function renderMetaCvMonthManualRow(e,t){if(!t?.metaReportGhlDataEnabled||(t.metaReportToplineMode==="cenhub"?"cenhub":"meta")!=="cenhub")return"";const o=(e?.dataSource||e?.toplineSource||"meta")==="manual",r=!o;return!o&&!r?"":`
    <div class="meta-report-month-manual-row">
      ${o?renderMetaToplineSourcePillHtml("Manual values this month","manual"):""}
      ${r?'<button type="button" class="meta-topline-manual-pill-btn" id="meta-cv-use-manual-month">Use manual values for this month</button>':""}
    </div>
  `}function bindMetaToplineCenhubActionEvents({clientId:e,prefix:t,getMonthKey:a,getYear:n,setSaving:o,onReload:r,onMonthUpdated:s}){if(!e||!t)return;const i=document.getElementById(`${t}-sync-ghl-year`);i&&(i.onclick=async()=>{o&&o(!0);try{const c=n?.()||metaReportsState.selectedYear,d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-preview?year=${encodeURIComponent(c)}`);let u=!0,p=!1;if((d.skipManual||[]).length){const m=window.prompt(`${d.skipManual.length} month(s) have manual values (${d.skipManual.join(", ")}).
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!m)return;m.trim().toLowerCase()==="overwrite"&&(u=!1,p=!0)}const b=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(c)}`,{method:"POST",body:JSON.stringify({skipManual:u,overwriteManual:p})});r?await r():s&&await s(b,null,{yearSync:!0}),showToast(`Synced ${b.synced?.length||0} month(s)${b.skipped?.length?`, skipped ${b.skipped.length}`:""}`,"success")}catch(c){showToast(c.message||"Year sync failed","error")}finally{o&&o(!1)}});const l=document.getElementById(`${t}-refresh-snapshot`);l&&(l.onclick=async()=>{const c=a?.();if(c){o&&o(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/refresh-snapshot`,{method:"POST",body:"{}"});s&&await s(d,c),showToast("Cenhub data refreshed and synced","success")}catch(d){showToast(d.message||"Refresh failed","error")}finally{o&&o(!1)}}})}function monthHasCenhubSnapshot(e){if(resolveMetaToplineSource(e)==="ghl")return!0;const a=e?.ghlTotals||{};return!!(e?.ghlSyncedAt||a.leads!=null||a.wonLeads!=null)}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),a=e.monthKeys||[];return t.length?t.map(n=>`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(L=>L.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    `;const o=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,o);const r=t.editorPayload?.months?.[n]||null,s=a.metaReportShowBottomline,i=!!a.metaReportGhlDataEnabled,l=a.metaReportToplineMode==="cenhub"?"cenhub":"meta",c=o.toplineSource||r?.dataSource||"meta",d=i&&l==="cenhub"&&c==="ghl",u=i&&c==="manual",p=o.wonLeads!=null?o.wonLeads:r?.inputs?.wonLeads??r?.topline?.wonLeads??"",b=o.avgLeadValue!=null?o.avgLeadValue:r?.inputs?.avgLeadValue??r?.topline?.avgLeadValue??"",m=o.avgProfitPerWon!=null?o.avgProfitPerWon:r?.inputs?.avgProfitPerWon??"",$=o.ghlLeads??r?.ghlTotals?.leads??"",y=o.ghlTotalRevenue??r?.ghlTotals?.totalRevenue??"",f=o.ghlTotalProfit??r?.ghlTotals?.totalProfit??"",h=metaReportsState.customValues.draftInputs,g=h?.wonLeads??p,w=h?.avgLeadValue??b,v=h?.avgProfitPerWon??m,S=h?.manualLeads??o.manualLeads??"",k=d?" readonly":"",E=d?" disabled":"",I=t.settingsSaving&&t.settingsSavingScope==="report",C=i?renderMetaCvToplineSourceControl(a,e):"",B=renderMetaCvMonthManualRow(r||{dataSource:c},a),A=u?`
      <label>Manual leads (optional)
        <input type="number" step="any" id="meta-cv-manual-leads" value="${esc(S)}" />
      </label>
  `:"",M={metaReportGhlDataEnabled:a.metaReportGhlDataEnabled,metaReportToplineMode:a.metaReportToplineMode},T=i?renderMetaMonthSourcePill(r||{dataSource:c},M):"";return`
    <div class="meta-report-edit-panel-head">
      <div>
    ...
    <div class="meta-cv-editor-grid">
      ...
      ${A}
      <label>Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(g)}"${k} />
      </label>
      <label>Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(w)}"${k} />
      </label>
      <label>Avg profit per won${s?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(v)}"${k} />
      </label>
    </div>
    ${B}
    ...
    <div class="meta-cv-editor-actions">
      ...
      <button type="button" class="admin-btn admin-btn--primary" id="meta-cv-save"${t.saving||t.loadingEditor||t.settingsSaving||d?" disabled":""}${E}>
