...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),g=w.summary||w;l=g.runId,c=0,setFbLeadProgress({processed:0,total:g.inWindow||0,label:`Matching ${g.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:r})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const g=w.inWindow||c||$;a?(setFbLeadProgress({processed:c,total:g,complete:!w.hasMore}),s&&(s.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:g,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const v=d.summary||d,f=v.inWindow||c||$,y=v.nextBatchOffset??c;if(a)setFbLeadProgress({processed:y,total:f,complete:!0}),markFbLeadPreviewReady(e,t,v.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(v.updated||0)>0),showToast((v.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:y,total:f,written:v.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const b=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=b.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(v){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${v.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((v.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(s=>s.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null}={}){const o=n||e?.monthKey||null,s=o?metaReportMonthBounds(o):null;let r="";t&&e&&!e.meta?.emptyMonth&&s&&(r=`
  ...
  `:"",p=renderMetaReportMonthTables(e,{editable:t}),h=d?renderMetaReportChartsPanel({editable:t}):"",m=t&&d?renderMetaReportScenarioCard():"",$=d?renderMetaReportViewModeTabs():"";let v;return d?v=`
      ...
      ${m}
    `:v=`<div class="meta-report-content-main">${p}</div>`,`${r}${u}${$}${v}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),$=META_REPORT_FEE_MODE_META.map(({value:b,icon:w,title:g,desc:S})=>`
        <label class="meta-report-fee-mode-card">
          ...
          <span class="meta-report-fee-mode-card-text">
            <span class="meta-report-fee-mode-card-title">${esc(g)}</span>
            <small>${esc(S)}</small>
          </span>
          <span class="meta-report-fee-mode-card-radio" aria-hidden="true">${ICON_CHECK}</span>
        </label>`).join(""),v=o?metaReportsState.clientReportSettingsExpanded:!0,f=o?`
        ...
        <div class="meta-report-fee-save-row">
          <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save">Save fee settings</button>
        </div>`,y=o&&!s?renderMetaReportBottomlineFeeSummary(e,t):"";return`
    <div class="meta-report-bottomline-fee-settings${o?" meta-report-bottomline-fee-settings--client":""}${o?v?" is-editing":" is-collapsed":""}">
      ${y}
      <div class="meta-report-bottomline-editor${o&&!v?" is-hidden":""}" id="${t}-fee-editor">
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(R=>R.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    `;const o=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,o);const s=t.editorPayload?.months?.[n]||null,r=a.metaReportShowBottomline,i=!!a.metaReportGhlDataEnabled,l=o.toplineSource||s?.dataSource||"meta",c=!!t.overrideManual,d=i&&l==="ghl"&&!c,u=o.wonLeads!=null?o.wonLeads:s?.inputs?.wonLeads??s?.topline?.wonLeads??"",p=o.avgLeadValue!=null?o.avgLeadValue:s?.inputs?.avgLeadValue??s?.topline?.avgLeadValue??"",h=o.avgProfitPerWon!=null?o.avgProfitPerWon:s?.inputs?.avgProfitPerWon??"",m=o.ghlLeads??s?.ghlTotals?.leads??"",$=o.ghlTotalRevenue??s?.ghlTotals?.totalRevenue??"",v=o.ghlTotalProfit??s?.ghlTotals?.totalProfit??"",f=metaReportsState.customValues.draftInputs,y=f?.wonLeads??u,b=f?.avgLeadValue??p,w=f?.avgProfitPerWon??h,g=f?.manualLeads??o.manualLeads??"",S=d?" readonly":"",B=d?" disabled":"",A=t.settingsSaving&&t.settingsSavingScope==="report",I=l==="ghl"?"Sourced from Cenhub snapshot":l==="manual"?"Manual override":"Meta metrics",k=i?`
    <div class="meta-cv-ghl-banner">
      ${esc(I)}
      ${o.ghlSyncedAt?` \xB7 Synced ${esc(metaCvFormatUpdatedAt(o.ghlSyncedAt))}`:""}
      ${l==="ghl"&&m!==""?`<br>Leads ${esc(m)} \xB7 Revenue ${esc($)} \xB7 Profit ${esc(v)}`:""}
    </div>
    <div class="meta-cv-ghl-actions">
      <button type="button" class="admin-btn admin-btn--secondary" id="meta-cv-sync-ghl"${t.saving?" disabled":""}>Sync with Cenhub</button>
      <button type="button" class="admin-btn admin-btn--ghost" id="meta-cv-sync-ghl-year"${t.saving?" disabled":""}>Sync year\u2026</button>
      <button type="button" class="admin-btn admin-btn--ghost" id="meta-cv-refresh-snapshot"${t.saving?" disabled":""}>Refresh Cenhub data</button>
      ${l!=="meta"?`<button type="button" class="admin-btn admin-btn--ghost" id="meta-cv-use-meta"${t.saving?" disabled":""}>Use Meta metrics</button>`:""}
    </div>
    <label style="display:flex;align-items:center;gap:8px;margin:0 0 12px;font-size:13px">
      <input type="checkbox" id="meta-cv-override-ghl"${c?" checked":""} />
      Override GHL \u2014 enter manually
    </label>
  `:"",M=i&&(c||l==="manual")?`
      <label>Manual leads (optional)
        <input type="number" step="any" id="meta-cv-manual-leads" value="${esc(g)}" />
      </label>
  `:"";return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      ${M}
      <label>Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(y)}"${S} />
      </label>
      <label>Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(b)}"${S} />
      </label>
      <label>Avg profit per won${r?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(w)}"${S} />
      </label>
    </div>
    ...
    <div class="meta-cv-editor-actions">
      ...
      <button type="button" class="admin-btn admin-btn--primary" id="meta-cv-save"${t.saving||t.loadingEditor||t.settingsSaving||d?" disabled":""}${B}>
  ...
  `}function refreshMetaCvListAndEditor(){const e=metaReportsState.customValues.overview;if(!e)return;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e));const a=document.getElementById("meta-cv-editor");a&&(a.innerHTML=renderMetaCvEditor(e)),metaReportsState.customValues.draftInputs=null,bindMetaCvEditorEvents(),bindMetaCvListEvents()}function setMetaCvSavePending(e){metaReportsState.customValues.saving=e;const t=document.getElementById("meta-cv-save"),a=document.getElementById("meta-cv-save-indicator");a&&a.classList.toggle("is-visible",e),t&&(t.disabled=e||metaReportsState.customValues.loadingEditor||metaReportsState.customValues.settingsSaving,t.textContent=e?"Saving\u2026":"Save values")}function setMetaCvSettingsSaving(e,t="report"){metaReportsState.customValues.settingsSaving=e,metaReportsState.customValues.settingsSavingScope=e?t:null;const a=e&&t==="report";document.getElementById("meta-cv-settings-status")?.classList.toggle("is-visible",a),document.querySelector(".meta-cv-client-settings")?.classList.toggle("is-saving",a),["meta-cv-setting-bottomline"].forEach(s=>{const r=document.getElementById(s);r&&(r.disabled=a)}),document.querySelectorAll('input[name="meta-cv-fee-mode"]').forEach(s=>{s.disabled=a}),["meta-cv-setting-fee-percent","meta-cv-setting-marketing-fee"].forEach(s=>{const r=document.getElementById(s);r&&(r.disabled=a||r.disabled)});const o=document.getElementById("meta-cv-save");o&&(o.disabled=e||metaReportsState.customValues.saving||metaReportsState.customValues.loadingEditor)}function classifyMetaCvMonthStatus(e,t=!1){if(!e)return"empty";const a=e.wonLeads!=null,n=e.avgLeadValue!=null,o=e.avgProfitPerWon!=null;return a&&n&&(!t||o)?"complete":a||n||o?"partial":"empty"}function applyMetaCvSaveResult(e,t,a,n){const s=metaReportsState.customValues.overview?.clients?.find(l=>l.clientId===e),r=s?.months?.[t],i=n?.month||{};r&&(r.wonLeads=i.wonLeads??a.wonLeads,r.avgLeadValue=i.avgLeadValue??a.avgLeadValue,r.avgProfitPerWon=i.avgProfitPerWon??a.avgProfitPerWon,r.toplineSource=i.toplineSource??a.toplineSource??r.toplineSource,r.manualOverride=i.manualOverride??a.manualOverride??r.manualOverride,r.ghlLeads=i.ghlLeads??r.ghlLeads,r.ghlWonLeads=i.ghlWonLeads??r.ghlWonLeads,r.ghlTotalRevenue=i.ghlTotalRevenue??r.ghlTotalRevenue,r.ghlTotalProfit=i.ghlTotalProfit??r.ghlTotalProfit,r.ghlSyncedAt=i.ghlSyncedAt??r.ghlSyncedAt,r.manualLeads=i.manualLeads??a.manualLeads??r.manualLeads,r.status=classifyMetaCvMonthStatus(r,!!s.metaReportShowBottomline),r.updatedAt=r.status==="empty"?null:i.updatedAt||new Date().toISOString()),n?.monthPayload&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:n.monthPayload}})}async function refreshMetaCvOverviewSilently(){const e=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(e.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=e;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e),bindMetaCvListEvents());const a=document.querySelector(".meta-cv-summary");if(a&&e.summary){const n=e.summary;a.innerHTML=`
    ...
    `}}function collectMetaCvSaveBody(){const e=metaReportsState.customValues.selectedClientId,t=metaReportsState.customValues.selectedMonthKey,a=metaReportsState.customValues.overview?.clients?.find(r=>r.clientId===e),n=a?.months?.[t],o=!!a?.metaReportGhlDataEnabled&&(metaReportsState.customValues.overrideManual||n?.toplineSource==="manual"),s={wonLeads:document.getElementById("meta-cv-won-leads")?.value===""?null:Number(document.getElementById("meta-cv-won-leads")?.value),avgLeadValue:document.getElementById("meta-cv-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-cv-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-cv-avg-profit")?.value===""?null:Number(document.getElementById("meta-cv-avg-profit")?.value)};if(o){s.toplineSource="manual",s.manualOverride=!0;const r=document.getElementById("meta-cv-manual-leads");r&&(s.manualLeads=r.value===""?null:Number(r.value))}return s}function syncMetaCvClientSettingsDom(e={}){syncMetaReportBottomlineFeeDom(e,"meta-cv")}async function saveMetaCvClientSettings(e,t,a){setMetaCvSettingsSaving(!0,"report");try{const n=metaReportsState.customValues.selectedMonthKey,o=await patchMetaReportSettings(e,{...t,monthKey:n||void 0}),r=metaReportsState.customValues.overview?.clients?.find(i=>i.clientId===e);r&&o.settings&&(r.metaReportShowBottomline=!!o.settings.metaReportShowBottomline,r.metaReportFeeEnabled=!!o.settings.metaReportFeeEnabled,r.metaReportFeeMode=o.settings.metaReportFeeMode||null,r.metaReportFeePercent=o.settings.metaReportFeePercent??20,r.metaReportMarketingFeeAmount=o.settings.metaReportMarketingFeeAmount??0),o.monthPayload&&n&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[n]:o.monthPayload}}),setMetaCvSettingsSaving(!1),syncMetaCvClientSettingsDom(o.settings||{}),showToast(describeMetaReportSettingsChange(t,o.settings||{}),"success")}catch(n){showToast(n.message||"Update failed","error"),a&&a(),setMetaCvSettingsSaving(!1)}}function bindMetaCvSettingsEvents(e){bindMetaReportBottomlineFeeEvents(e,"meta-cv",(t,a)=>saveMetaCvClientSettings(e,t,a))}async function selectMetaCvMonth(e,t,{reloadEditor:a=!0}={}){if(metaReportsState.customValues.selectedClientId=e,metaReportsState.customValues.selectedMonthKey=t,metaReportsState.customValues.overrideManual=!1,refreshMetaCvListAndEditor(),!!a){metaReportsState.customValues.loadingEditor=!0,refreshMetaCvListAndEditor();try{const n=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(t)}`);metaReportsState.customValues.selectedClientId===e&&metaReportsState.customValues.selectedMonthKey===t&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:n.monthPayload}})}catch(n){showToast(n.message||"Failed to load month","error")}finally{metaReportsState.customValues.loadingEditor=!1,refreshMetaCvListAndEditor()}}}function bindMetaCvListEvents(){document.querySelectorAll("[data-meta-cv-month]").forEach(e=>{e.onclick=()=>{selectMetaCvMonth(e.getAttribute("data-meta-cv-client"),e.getAttribute("data-meta-cv-month"))}})}function bindMetaCvEditorEvents(){const e=metaReportsState.customValues.selectedClientId;e&&bindMetaCvSettingsEvents(e);const t=document.getElementById("meta-cv-client-select"),a=document.getElementById("meta-cv-month-select");t&&(t.onchange=()=>{const c=metaReportsState.customValues.selectedMonthKey||metaReportsState.customValues.overview?.monthKeys?.slice(-1)[0];c&&selectMetaCvMonth(t.value,c)}),a&&(a.onchange=()=>{const c=metaReportsState.customValues.selectedClientId;c&&selectMetaCvMonth(c,a.value)});const n=document.getElementById("meta-cv-save");n&&(n.onclick=async()=>{const c=metaReportsState.customValues.selectedClientId,d=metaReportsState.customValues.selectedMonthKey;if(!c||!d)return;const u=collectMetaCvSaveBody();setMetaCvSavePending(!0);try{const p=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(c)}/months/${encodeURIComponent(d)}`,{method:"PUT",body:JSON.stringify(u)});applyMetaCvSaveResult(c,d,u,p),metaReportsState.customValues.draftInputs=u,setMetaCvSavePending(!1),refreshMetaCvListAndEditor(),showToast("Custom values saved","success"),await refreshMetaCvOverviewSilently()}catch(p){showToast(p.message||"Save failed","error"),setMetaCvSavePending(!1)}});const o=document.getElementById("meta-cv-override-ghl");o&&(o.onchange=async()=>{metaReportsState.customValues.overrideManual=o.checked;const c=metaReportsState.customValues.selectedMonthKey;if(o.checked&&e&&c)try{await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/use-manual`,{method:"POST",body:"{}"}),await refreshMetaCvOverviewSilently()}catch(d){showToast(d.message||"Could not switch to manual mode","error")}refreshMetaCvListAndEditor()});const s=document.getElementById("meta-cv-sync-ghl");s&&e&&(s.onclick=async()=>{const c=metaReportsState.customValues.selectedMonthKey;if(!c)return;const d=metaReportsState.customValues.overview?.clients?.find(p=>p.clientId===e)?.months?.[c];let u=!1;if(d?.manualOverride&&d?.toplineSource==="manual"){if(!window.confirm("This month has manual values. Overwrite with Cenhub data?"))return;u=!0}setMetaCvSavePending(!0);try{const p=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/sync-ghl`,{method:"POST",body:JSON.stringify({overwriteManual:u})});applyMetaCvSaveResult(e,c,{},p),metaReportsState.customValues.overrideManual=!1,showToast("Synced from Cenhub snapshot","success"),await refreshMetaCvOverviewSilently(),refreshMetaCvListAndEditor()}catch(p){showToast(p.message||"Sync failed","error")}finally{setMetaCvSavePending(!1)}});const r=document.getElementById("meta-cv-sync-ghl-year");r&&e&&(r.onclick=async()=>{setMetaCvSavePending(!0);try{const c=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-preview?year=${encodeURIComponent(metaReportsState.selectedYear)}`);let d=!0,u=!1;if((c.skipManual||[]).length){const h=window.prompt(`${c.skipManual.length} month(s) have manual values (${c.skipManual.join(", ")}).
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!h){setMetaCvSavePending(!1);return}h.trim().toLowerCase()==="overwrite"&&(d=!1,u=!0)}const p=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(metaReportsState.selectedYear)}`,{method:"POST",body:JSON.stringify({skipManual:d,overwriteManual:u})});showToast(`Synced ${p.synced?.length||0} month(s)${p.skipped?.length?`, skipped ${p.skipped.length}`:""}`,"success"),await refreshMetaCvOverviewSilently(),refreshMetaCvListAndEditor()}catch(c){showToast(c.message||"Year sync failed","error")}finally{setMetaCvSavePending(!1)}});const i=document.getElementById("meta-cv-refresh-snapshot");i&&e&&(i.onclick=async()=>{const c=metaReportsState.customValues.selectedMonthKey;if(c){setMetaCvSavePending(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/refresh-snapshot`,{method:"POST",body:"{}"});applyMetaCvSaveResult(e,c,{},d),showToast("Cenhub data refreshed and synced","success"),await refreshMetaCvOverviewSilently(),refreshMetaCvListAndEditor()}catch(d){showToast(d.message||"Refresh failed","error")}finally{setMetaCvSavePending(!1)}}});const l=document.getElementById("meta-cv-use-meta");l&&e&&(l.onclick=async()=>{const c=metaReportsState.customValues.selectedMonthKey;if(c){setMetaCvSavePending(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/use-meta`,{method:"POST",body:"{}"});applyMetaCvSaveResult(e,c,{},d),metaReportsState.customValues.overrideManual=!1,await refreshMetaCvOverviewSilently(),refreshMetaCvListAndEditor(),showToast("Using Meta metrics for this month","success")}catch(d){showToast(d.message||"Update failed","error")}finally{setMetaCvSavePending(!1)}}})}function bindMetaReportsCustomValuesEvents(e){const t=document.getElementById("meta-cv-year");t&&(t.onchange=async()=>{const n=metaReportsState.selectedYear,o=Number(t.value);if(o!==n){metaReportsState.selectedYear=o,metaReportsState.customValues.selectedClientId=null,metaReportsState.customValues.selectedMonthKey=null,metaReportsState.customValues.editorPayload=null,setMetaCvYearLoading(o,!0);try{await loadMetaReportsCustomValuesPage({silent:!0})}catch(s){metaReportsState.selectedYear=n,t&&(t.value=String(n)),await loadMetaReportsCustomValuesPage({silent:!0,keepSelection:!0}),showToast(s.message||"Load failed","error")}finally{setMetaCvYearLoading(o,!1)}}});const a=document.getElementById("meta-cv-search");a&&(a.oninput=()=>{metaReportsState.customValues.searchQuery=a.value;const n=document.getElementById("meta-cv-client-list");n&&(n.innerHTML=renderMetaCvClientRows(e)),bindMetaCvListEvents()}),bindMetaCvListEvents(),bindMetaCvEditorEvents()}function renderMetaReportsGhlClientsPage(e){const t=e.summary||{},a=(e.clients||[]).map(n=>`
    <tr>
    ...
    <div class="date-picker-panel">
      <div class="date-picker-header">
        ...
        <div class="date-picker-nav">
          ...
          <button type="button" class="date-picker-nav-btn" aria-label="Next month" onclick="shiftDatePickerMonth(1)" ${g?"":"disabled"}>${ICON_CHEVRON_RIGHT}</button>
        </div>
      </div>
      <div class="date-picker-weekdays">
