...
${o} contact(s) will get Fb Lead id written in GHL. This writes live data.`)&&(fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:!0,previewRunId:n},await runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!1}))}async function refreshFbLeadClientRowsUi(){const e=document.getElementById("fb-lead-clients-mount");e&&(e.innerHTML=renderFbLeadClientRows(fbLeadSyncState.clients)),bindFbLeadSyncToggles()}function fbLeadRunHasLaterApply(e,t){const n=Date.parse(t.startedAt||"");return Number.isFinite(n)?(e||[]).some(a=>{if(a.id===t.id||a.clientId!==t.clientId||(a.mode||"recent")!==(t.mode||"recent")||a.dryRun)return!1;const s=String(a.status||"").toLowerCase();return s!=="success"&&s!=="ok"?!1:Date.parse(a.startedAt||"")>n}):!1}function renderFbLeadClientActions(e){const t=esc(e.clientId),n=e.metaPageId?"":`<a class="admin-btn admin-btn--ghost admin-btn--small" href="/admin/${encodeURIComponent(e.clientId)}">Setup</a>`;return`
  ...
  `:'<div class="sync-history-empty">No clients configured yet.</div>'}function renderFbLeadClientRowsWrapper(e){return`<div id="fb-lead-clients-mount">${renderFbLeadClientRows(e)}</div>`}function renderFbLeadHistoryRows(e,t={}){return e.length?`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table">
        ...
        <tbody>${e.map(a=>{const s=[a.mode,a.dryRun?"dry-run":"apply",`${a.updated||0} updated`,`${a.skippedNoMatch||0} no match`,a.errors?`${a.errors} errors`:null].filter(Boolean).join(" \xB7 "),i=dryRunIsReadyForApply(a,e)&&!fbLeadSyncState.isApplying,o=fbLeadSyncState.isApplying&&fbLeadSyncState.applyingRunId===a.id,d=renderSyncStatusBadge(o?"applying":a.status);return`
  ...
  `}function closeFbLeadRunPanel(){const e=document.getElementById("fb-lead-run-panel-mount");e&&(e.innerHTML=""),fbLeadSyncState.activeRun=null}function openFbLeadRunPanel(e,t){const n=document.getElementById("fb-lead-run-panel-mount");if(!n)return;fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:clientHasReadyPreview(fbLeadSyncState.clients.find(s=>s.clientId===e))},n.innerHTML=renderFbLeadRunPanel(e,t);const a=document.getElementById("fb-lead-preview-btn");a&&(a.onclick=()=>runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!0})),n.scrollIntoView({behavior:"smooth",block:"nearest"})}function renderFbLeadRunResults(e){const t=e.rows||[];if(!t.length)return'<div class="sync-history-empty" style="padding:12px">No rows in this batch.</div>';const n=t.map(i=>`
    <tr>
...
This only clears the log \u2014 it does not change GHL contacts or Meta data.`)))try{const t=await adminFetch("/api/fb-lead-sync/history",{method:"DELETE"});showToast(`Cleared ${t.deleted||0} run(s).`,"success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(t){showToast(t.message||"Failed to clear FB lead sync log.","error")}}async function deleteFbLeadRun(e){if(!(!isStaffAdmin()||!window.confirm(`Delete run #${e} from history?`)))try{await adminFetch(`/api/fb-lead-sync/history/${e}`,{method:"DELETE"}),showToast("Run deleted.","success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(n){showToast(n.message||"Failed to delete run.","error")}}async function runFbLeadSyncBatch({clientId:e,mode:t,dryRun:n}){const a=document.getElementById("fb-lead-preview-btn"),s=document.getElementById("fb-lead-run-results");a&&(a.disabled=!0);let i=fbLeadSyncState.activeRun?.runId||null,o=0,d=null,l=[],m=!1;const f=fbLeadSyncState.activeRun?.previewRunId??null,b=(f!=null?fbLeadSyncState.historyRuns.find(v=>Number(v.id)===Number(f)):null)?.updated??fbLeadSyncState.previewOkByClient[e]?.updated??0;n||(fbLeadSyncState.applyingRunId=f,startGlobalApplyProgress(e,t,b),await refreshFbLeadHistoryUi());try{n&&setFbLeadProgress({processed:0,total:0,label:"Starting preview\u2026"});do{d=await adminFetch("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:i,batchOffset:o,batchLimit:25})});const r=d.summary||d;i=r.runId,o=r.nextBatchOffset??r.batchOffset+(r.batchProcessed||0),l=l.concat(r.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=i);const p=r.inWindow||o||b;n?(setFbLeadProgress({processed:o,total:p,complete:!r.hasMore}),s&&(s.innerHTML=renderFbLeadRunResults({...r,rows:l}))):updateGlobalApplyProgress({processed:o,total:p,written:r.updated||0,complete:!r.hasMore})}while((d.summary||d).hasMore);m=!0;const v=d.summary||d,k=v.inWindow||o||b,I=v.nextBatchOffset??o;if(n)setFbLeadProgress({processed:I,total:k,complete:!0}),markFbLeadPreviewReady(e,t,v.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=!0),showToast("Preview complete \u2014 use Apply in Run history below.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:I,total:k,written:v.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const r=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete r[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(r))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const M=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=M.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(v){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${v.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((v.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress may be saved \u2014 refresh and try Preview again.":""),"error")}finally{a&&(a.disabled=!1),fbLeadSyncState.applyingRunId=null,!m&&n&&hideFbLeadProgress(!0),!n&&!m&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=fbLeadSyncState.historyRuns.filter(s=>s.clientId===e),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1}={}){const t=document.getElementById("dashboard");if(!t)return;const n=await fetchStaffMe();if(!n){e||(t.innerHTML=`
    ...
    `);try{if(e&&fbLeadSyncState.isApplying)return;const a=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=a.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await Promise.all([loadFbLeadPreflightForClients(fbLeadSyncState.clients),loadFbLeadHistory()]),t.innerHTML=renderFbLeadSyncPage(a),bindFbLeadSyncToggles();const s=document.getElementById("fb-lead-sync-refresh");s&&(s.onclick=()=>loadFbLeadSyncPage());const i=document.getElementById("fb-lead-sync-clear-log");i&&(i.onclick=()=>clearFbLeadSyncHistory()),fbLeadSyncRefreshTimer&&(clearInterval(fbLeadSyncRefreshTimer),fbLeadSyncRefreshTimer=null),fbLeadSyncRefreshTimer=window.setInterval(()=>{loadFbLeadSyncPage({silent:!0}).catch(()=>{})},6e4)}catch(a){t.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("fb-lead-sync"))}
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_CHECK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>',ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
      ...
      <div class="section-grid">
        ${isVisible("sections","assigneeReport")?`
          <div class="card">
            ...
            ${renderTable("assigneeReport","Assignee",[{key:"won",label:"Won",align:"num"},{key:"totalLeads",label:"Total leads",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"},{key:"totalValue",label:"Total value",align:"num"}],s,(l,m)=>m==="label"?esc(l.assigneeName):m==="wonValue"||m==="totalValue"?`Dkr ${fmt(l[m])}`:fmt(l[m]))}
          </div>
        `:""}
        ${isVisible("sections","pipelineBreakdown")?`
          <div class="card">
            <div class="section-title">Pipeline Breakdown</div>
            ${renderTable("pipelineBreakdown","Pipeline",[{key:"count",label:"Leads",align:"num"},{key:"won",label:"Won",align:"num"},{key:"monetary",label:"Value",align:"num"},{key:"profit",label:"Bundlinje",align:"num"},{key:"wonValue",label:"Won revenue",align:"num"}],i,(l,m)=>m==="label"?esc(l.name):m==="monetary"||m==="profit"||m==="wonValue"?`Dkr ${fmt(l[m])}`:fmt(l[m]))}
          </div>
        `:""}
      </div>
    `:""}
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),d=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const m=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const f=document.getElementById("dashboard-content");f&&(f.innerHTML=renderKpiSkeleton()),d&&(d.disabled=!0,d.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let f=null;if(!availablePipelines.length){if(f=await bootstrapDashboardData(m),l!==fetchGeneration)return;availablePipelines=f.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,f.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const b=canReuseBootstrapDashboard(f)&&!s;let v,k;if(b?[k,v]=await Promise.all([fetchFacebookMetrics(m),Promise.resolve(f)]):[v,k]=await Promise.all([fetchDashboardData(availablePipelines,m,{forceFresh:s}),fetchFacebookMetrics(m)]),l!==fetchGeneration)return;cachedFacebookMetrics=k,cachedData=applyMarketingData(v,k),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(f){if(l!==fetchGeneration)return;if(f.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
