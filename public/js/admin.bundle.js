...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),w=$.summary||$;l=w.runId,c=0,setFbLeadProgress({processed:0,total:w.inWindow||0,label:`Matching ${w.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:g||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:s})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const w=$.inWindow||c||g;a?(setFbLeadProgress({processed:c,total:w,complete:!$.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:w,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||g,v=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const y=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=y.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}const META_REPORT_COMPARISON_PRESETS=[{id:"mom",label:"This month vs last month"},{id:"months",label:"Compare months"},{id:"custom",label:"Custom dates"},{id:"ytd",label:"This year vs last year"}],META_REPORT_COMPARISON_TABLE_GROUPS=[{title:"Meta ads",accent:"meta",rows:[{label:"Total spend",metricId:"spend",format:"kr",higherIsBetter:null},{label:"Cost Pr Mile (CPM)",metricId:"cpm",format:"kr",higherIsBetter:!1},{label:"Impressions",metricId:"impressions",format:"num",higherIsBetter:!0},{label:"Reach",metricId:"reach",format:"num",higherIsBetter:!0},{label:"Click",metricId:"clicks",format:"num",higherIsBetter:!0},{label:"CTR",metricId:"conversionRatePercent",format:"pct",higherIsBetter:!0}]},{title:"Topline KPI'er",accent:"topline",highlightLastN:2,rows:[{label:"Leads",metricId:"leads",format:"num",higherIsBetter:!0},{label:"Cost Per Lead (CPL)",metricId:"cpl",format:"kr",higherIsBetter:!1},{label:"Won leads",metricId:"wonLeads",format:"num",higherIsBetter:!0},{label:"Total Lead Value",metricId:"totalLeadValue",format:"kr",higherIsBetter:!0},{label:"Average Lead Value",metricId:"avgLeadValue",format:"kr",higherIsBetter:!0},{label:"Client acquisition cost (CAC)",metricId:"cac",format:"kr",higherIsBetter:!1},{label:"Return on Ads Spend (ROAS)",metricId:"roasKr",format:"kr",higherIsBetter:!0},{label:"Return on Ads Spend % (ROAS)",metricId:"roasX",format:"x",higherIsBetter:!0}]},{title:"Bottomline KPI'er",accent:"bottomline",highlightLastN:2,bottomlineOnly:!0,rows:[{label:"Leads",metricId:"leads",format:"num",higherIsBetter:!0},{label:"Won leads",metricId:"wonLeads",format:"num",higherIsBetter:!0},{label:"Total Lead Value",metricId:"totalLeadValue",format:"kr",higherIsBetter:!0},{label:"Average Lead Value",metricId:"avgLeadValue",format:"kr",higherIsBetter:!0},{label:"Client acquisition cost (CAC)",metricId:"cac",format:"kr",higherIsBetter:!1},{label:"Avg Total Profit",metricId:"totalProfit",format:"kr",higherIsBetter:!0},{label:"Avg Single Profit Order",metricId:"avgProfitPerWon",format:"kr",higherIsBetter:!0},{label:"Profit on Ads Spend (POAS)",metricId:"poasKr",format:"kr",higherIsBetter:!0},{label:"Profit on Ads Spend % (POAS)",metricId:"poasX",format:"x",higherIsBetter:!0},{label:"Censio fee",metricId:"censioFee",format:"kr",higherIsBetter:!1},{label:"Profit on Investment (POI)",metricId:"poiKr",format:"kr",higherIsBetter:!0},{label:"Profit on Investment % (POI)",metricId:"poiX",format:"x",higherIsBetter:!0}]}];function metaReportComparisonAddMonths(e,t=0){const a=String(e||"").trim();if(!/^\d{4}-\d{2}$/.test(a))return null;const[n,o]=a.split("-").map(Number),r=new Date(n,o-1+t,1);return`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,"0")}`}function metaReportComparisonMonthKeysInRange(e,t){if(!e||!t)return[];const[a,n]=e<=t?[e,t]:[t,e],o=[];let r=a;for(;r&&r<=n&&(o.push(r),r!==n);)r=metaReportComparisonAddMonths(r,1);return o}function metaReportComparisonParseDate(e){const t=String(e||"").trim().slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(t))return null;const[a,n,o]=t.split("-").map(Number),r=new Date(a,n-1,o);return r.getFullYear()!==a||r.getMonth()!==n-1||r.getDate()!==o?null:t}function metaReportComparisonDaysInclusive(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n||a>n)return 0;const[o,r,s]=a.split("-").map(Number),[i,l,c]=n.split("-").map(Number),d=Date.UTC(o,r-1,s),u=Date.UTC(i,l-1,c);return Math.floor((u-d)/864e5)+1}function metaReportComparisonOverlapDays(e,t,a,n){const o=e>a?e:a,r=t<n?t:n;return!o||!r||o>r?0:metaReportComparisonDaysInclusive(o,r)}function metaReportComparisonMonthKeyFromDate(e){const t=metaReportComparisonParseDate(e);return t?t.slice(0,7):null}function metaReportComparisonMonthKeysOverlappingDates(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n)return[];const[o,r]=a<=n?[a,n]:[n,a],s=[];let i=metaReportComparisonMonthKeyFromDate(o);const l=metaReportComparisonMonthKeyFromDate(r);for(;i&&i<=l&&(s.push(i),i!==l);)i=metaReportComparisonAddMonths(i,1);return s}function metaReportComparisonMonthEffectivePeriod(e,t){const a=metaReportMonthBounds(t||e?.monthKey),n=metaReportComparisonParseDate(e?.periodStart)||a.start,o=metaReportComparisonParseDate(e?.periodEnd)||a.end;return n&&o&&n<=o?{start:n,end:o}:a}function metaReportComparisonFormatDateLabel(e){const t=metaReportComparisonParseDate(e);if(!t)return"\u2014";const[a,n,o]=t.split("-").map(Number);return`${o} ${META_MONTH_LABELS[n-1]||n} ${a}`}function metaReportComparisonFormatDateRangeLabel(e,t){const a=metaReportComparisonParseDate(e),n=metaReportComparisonParseDate(t);if(!a||!n)return"\u2014";if(a===n)return metaReportComparisonFormatDateLabel(a);const o=a.slice(0,4),r=n.slice(0,4),s=a.slice(5,7),i=n.slice(5,7);if(o===r&&s===i){const[,l,c]=a.split("-").map(Number),[,,d]=n.split("-").map(Number);return`${c}\u2013${d} ${META_MONTH_LABELS[l-1]||l} ${o}`}return o===r?`${metaReportComparisonFormatDateLabel(a).replace(` ${o}`,"")}\u2013${metaReportComparisonFormatDateLabel(n)}`:`${metaReportComparisonFormatDateLabel(a)} \u2013 ${metaReportComparisonFormatDateLabel(n)}`}function metaReportComparisonMonthPayloadDates(e,t){const a=metaReportComparisonMonthEffectivePeriod(e,t);return{startDate:a.start,endDate:a.end,startMonthKey:t,endMonthKey:t}}function metaReportComparisonFullMonthDates(e){const t=metaReportMonthBounds(e);return{startDate:t.start,endDate:t.end,startMonthKey:e,endMonthKey:e}}function metaReportComparisonFormatDisplayLabel(e,t){const a=metaReportComparisonNormalizePeriod(t);return a?e==="mom"||e==="months"?metaReportComparisonPeriodLabel(a.startMonthKey,a.endMonthKey):e==="ytd"?a.startDate.slice(0,4):metaReportComparisonFormatDateRangeLabel(a.startDate,a.endDate):"\u2014"}function metaReportComparisonAggregateForMode(e,t,a){const n=metaReportComparisonNormalizePeriod(a);if(!n)return{label:"\u2014",monthKeys:[],monthCount:0,expectedMonthCount:0,partialData:!1,hasData:!1,hasBottomline:!1,metrics:metaReportComparisonAggregateDateRange({},null,null).metrics};let o;return e==="mom"||e==="months"?o=metaReportAggregateComparisonRange(t,n.startMonthKey,n.endMonthKey):o=metaReportComparisonAggregateDateRange(t,n.startDate,n.endDate),o.label=metaReportComparisonFormatDisplayLabel(e,n),o}function metaReportComparisonNormalizePeriod(e=null){if(!e)return null;const t=metaReportComparisonParseDate(e.startDate)||(e.startMonthKey?metaReportMonthBounds(e.startMonthKey).start:null),a=metaReportComparisonParseDate(e.endDate)||(e.endMonthKey?metaReportMonthBounds(e.endMonthKey).end:null);if(!t||!a)return null;const[n,o]=t<=a?[t,a]:[a,t];return{startDate:n,endDate:o,startMonthKey:metaReportComparisonMonthKeyFromDate(n),endMonthKey:metaReportComparisonMonthKeyFromDate(o)}}function metaReportComparisonPeriodLabel(e,t){if(!e||!t)return"\u2014";if(e===t)return`${metaMonthLabel(e)} ${String(e).slice(0,4)}`;const a=String(e).slice(0,4),n=String(t).slice(0,4);return a===n?`${metaMonthLabel(e)}\u2013${metaMonthLabel(t)} ${a}`:`${metaMonthLabel(e)} ${a}\u2013${metaMonthLabel(t)} ${n}`}function metaReportMonthHasComparisonData(e){return!e||e.meta?.emptyMonth?!1:metaReportParseAmount(e.meta?.spend)>0||metaReportParseAmount(e.topline?.leads)>0}function metaReportAggregateComparisonRange(e={},t,a){const n=metaReportComparisonMonthKeysInRange(t,a),o=[];let r=!1,s=0,i=0,l=0,c=0,d=0,u=0,p=0,h=0,m=0,g=0,b=0,f=0,v=0;for(const R of n){const A=e[R];if(!metaReportMonthHasComparisonData(A))continue;o.push(R),A.bottomline&&(r=!0);const k=metaReportParseAmount(A.meta?.spend),F=metaReportParseAmount(A.meta?.impressions),O=metaReportParseAmount(A.topline?.leads),x=metaReportParseAmount(A.topline?.wonLeads);s+=k,i+=F,l+=metaReportParseAmount(A.meta?.reach),c+=metaReportParseAmount(A.meta?.clicks),d+=O,u+=x,p+=metaReportParseAmount(A.topline?.totalLeadValue),x>0&&(g+=metaReportParseAmount(A.topline?.avgLeadValue)*x,A.bottomline&&(b+=metaReportParseAmount(A.bottomline.avgProfitPerWon)*x)),F>0&&(f+=metaReportParseAmount(A.meta?.cpm)*F,v+=F),A.bottomline&&(h+=metaReportParseAmount(A.bottomline.totalProfit),m+=metaReportParseAmount(A.bottomline.censioFee))}const y=d>0?Math.round(s/d*100)/100:0,$=u>0?Math.round(s/u*100)/100:0,w=u>0?Math.round(g/u*100)/100:0,S=u>0?Math.round(b/u*100)/100:0,L=v>0?Math.round(f/v*100)/100:0,B=i>0?Math.round(c/i*100*1e8)/1e8:0,P=Math.round((p-s)*100)/100,C=s>0?Math.round(P/s*1e8)/1e8:0,M=Math.round((h-s)*100)/100,E=s>0?Math.round(M/s*1e8)/1e8:0,I=r?Math.round((M-m)*100)/100:0,D=s>0&&r?Math.round(I/s*1e8)/1e8:0;return{startKey:t,endKey:a,label:metaReportComparisonPeriodLabel(t,a),monthKeys:o,monthCount:o.length,expectedMonthCount:n.length,partialData:o.length>0&&o.length<n.length,hasData:o.length>0,hasBottomline:r,metrics:{spend:s,cpm:L,impressions:i,reach:l,clicks:c,conversionRatePercent:B,leads:d,cpl:y,wonLeads:u,totalLeadValue:p,avgLeadValue:w,cac:$,roasKr:P,roasX:C,totalProfit:h,avgProfitPerWon:S,poasKr:M,poasX:E,censioFee:m,poiKr:I,poiX:D}}}function metaReportComparisonAggregateDateRange(e={},t,a){const n=metaReportComparisonParseDate(t),o=metaReportComparisonParseDate(a);if(!n||!o)return{startDate:t||null,endDate:a||null,label:"\u2014",monthKeys:[],monthCount:0,expectedMonthCount:0,partialData:!1,hasData:!1,hasBottomline:!1,metrics:metaReportAggregateComparisonRange({},"0000-00","0000-00").metrics};const[r,s]=n<=o?[n,o]:[o,n],i=metaReportComparisonMonthKeysOverlappingDates(r,s),l=[];let c=!1,d=0,u=0,p=0,h=0,m=0,g=0,b=0,f=0,v=0,y=0,$=0,w=0,S=0,L=0,B=0;for(const U of i){const T=e[U],H=metaReportComparisonMonthEffectivePeriod(T,U),_=metaReportComparisonOverlapDays(r,s,H.start,H.end);if(_<=0||(d+=_,!metaReportMonthHasComparisonData(T)))continue;l.push(U),u+=_,T.bottomline&&(c=!0);const W=metaReportComparisonDaysInclusive(H.start,H.end),N=W>0?_/W:0,j=metaReportParseAmount(T.meta?.spend)*N,V=metaReportParseAmount(T.meta?.impressions)*N,q=metaReportParseAmount(T.topline?.leads)*N,K=metaReportParseAmount(T.topline?.wonLeads)*N;p+=j,h+=V,m+=metaReportParseAmount(T.meta?.reach)*N,g+=metaReportParseAmount(T.meta?.clicks)*N,b+=q,f+=K,v+=metaReportParseAmount(T.topline?.totalLeadValue)*N,K>0&&(w+=metaReportParseAmount(T.topline?.avgLeadValue)*K,T.bottomline&&(S+=metaReportParseAmount(T.bottomline.avgProfitPerWon)*K)),V>0&&(L+=metaReportParseAmount(T.meta?.cpm)*V,B+=V),T.bottomline&&(y+=metaReportParseAmount(T.bottomline.totalProfit)*N,$+=metaReportParseAmount(T.bottomline.censioFee)*N)}const P=b>0?Math.round(p/b*100)/100:0,C=f>0?Math.round(p/f*100)/100:0,M=f>0?Math.round(w/f*100)/100:0,E=f>0?Math.round(S/f*100)/100:0,I=B>0?Math.round(L/B*100)/100:0,D=h>0?Math.round(g/h*100*1e8)/1e8:0,R=Math.round((v-p)*100)/100,A=p>0?Math.round(R/p*1e8)/1e8:0,k=Math.round((y-p)*100)/100,F=p>0?Math.round(k/p*1e8)/1e8:0,O=c?Math.round((k-$)*100)/100:0,x=p>0&&c?Math.round(O/p*1e8)/1e8:0;return{startDate:r,endDate:s,label:metaReportComparisonFormatDateRangeLabel(r,s),monthKeys:l,monthCount:l.length,expectedMonthCount:i.length,partialData:d>0&&u<d,hasData:l.length>0,hasBottomline:c,metrics:{spend:p,cpm:I,impressions:h,reach:m,clicks:g,conversionRatePercent:D,leads:b,cpl:P,wonLeads:f,totalLeadValue:v,avgLeadValue:M,cac:C,roasKr:R,roasX:A,totalProfit:y,avgProfitPerWon:E,poasKr:k,poasX:F,censioFee:$,poiKr:O,poiX:x}}}function metaReportComparisonMergedMonthsMap(e,t,a){const n=metaReportComparisonYearsNeeded(t,a),o={...e?.months||{}};return n.forEach(r=>{Number(e?.year)!==Number(r)&&Object.assign(o,metaReportComparisonMonthsForYear(e,r))}),o}function metaReportResolveComparisonPresetDates(e,t="mom"){const a=metaReportsState.activeMonthKey||null,n=metaReportComparisonMergedMonthsMap(e,null,null);if(t==="custom")return{mode:t,periodA:metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodA),periodB:metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodB)};if(t==="months"){const o=metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodA),r=metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodB),s=l=>l&&l.startMonthKey===l.endMonthKey;if(s(o)&&s(r))return{mode:t,periodA:o,periodB:r};if(!a)return{mode:t,periodA:null,periodB:null};const i=metaReportComparisonAddMonths(a,-1);return{mode:t,periodA:metaReportComparisonFullMonthDates(a),periodB:i?metaReportComparisonFullMonthDates(i):null}}if(!a)return{mode:t,periodA:null,periodB:null};if(t==="mom"){const o=metaReportComparisonAddMonths(a,-1);return{mode:t,periodA:metaReportComparisonFullMonthDates(a),periodB:o?metaReportComparisonFullMonthDates(o):null}}if(t==="ytd"){const o=String(a).slice(0,4),r=String(a).slice(5,7),s=String(Number(o)-1),i=metaReportMonthBounds(a),l=`${s}-${r}`,c=metaReportMonthBounds(l);return{mode:t,periodA:{startDate:`${o}-01-01`,endDate:i.end,startMonthKey:`${o}-01`,endMonthKey:a},periodB:{startDate:`${s}-01-01`,endDate:c.end,startMonthKey:`${s}-01`,endMonthKey:l}}}return{mode:t,periodA:null,periodB:null}}function metaReportApplyComparisonPresetDefaults(e,t=metaReportsState.comparisonMode){const a=metaReportResolveComparisonPresetDates(e,t);return a.periodA&&(metaReportsState.comparisonPeriodA={...a.periodA}),a.periodB&&(metaReportsState.comparisonPeriodB={...a.periodB}),a}function metaReportResolveComparisonPresets({activeMonthKey:e=null,mode:t="mom"}={}){const a=metaReportsState.clientPayload||metaReportsState.publicPayload;return metaReportResolveComparisonPresetDates(a,t)}function metaReportComparisonMetricDefs(e=!1){const t=[{id:"spend",label:"Ad spend",group:"meta",format:"kr",higherIsBetter:null},{id:"cpm",label:"CPM",group:"meta",format:"kr",higherIsBetter:!1},{id:"impressions",label:"Impressions",group:"meta",format:"num",higherIsBetter:!0},{id:"reach",label:"Reach",group:"meta",format:"num",higherIsBetter:!0},{id:"clicks",label:"Clicks",group:"meta",format:"num",higherIsBetter:!0},{id:"conversionRatePercent",label:"CTR",group:"meta",format:"pct",higherIsBetter:!0},{id:"leads",label:"Leads",group:"topline",format:"num",higherIsBetter:!0},{id:"cpl",label:"CPL",group:"topline",format:"kr",higherIsBetter:!1},{id:"wonLeads",label:"Won leads",group:"topline",format:"num",higherIsBetter:!0},{id:"totalLeadValue",label:"Total lead value",group:"topline",format:"kr",higherIsBetter:!0},{id:"avgLeadValue",label:"Average lead value",group:"topline",format:"kr",higherIsBetter:!0},{id:"cac",label:"CAC",group:"topline",format:"kr",higherIsBetter:!1},{id:"roasKr",label:"ROAS",group:"topline",format:"kr",higherIsBetter:!0},{id:"roasX",label:"ROAS %",group:"topline",format:"x",higherIsBetter:!0}];return e&&t.push({id:"totalProfit",label:"Total profit",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"avgProfitPerWon",label:"Avg profit per won",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poasKr",label:"POAS",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poasX",label:"POAS %",group:"bottomline",format:"x",higherIsBetter:!0},{id:"poiKr",label:"POI",group:"bottomline",format:"kr",higherIsBetter:!0},{id:"poiX",label:"POI %",group:"bottomline",format:"x",higherIsBetter:!0}),t}function metaReportComparisonHeroIds(e=!1){return e?["spend","leads","wonLeads","roasKr","totalLeadValue","poasKr"]:["spend","leads","wonLeads","roasKr","totalLeadValue"]}function metaReportComparisonChartIds(e=!1,t="kr"){const a=["spend","leads","totalLeadValue"];return t==="x"?[...a,"roasX",...e?["poasX"]:[]]:[...a,"roasKr",...e?["poasKr"]:[]]}function metaReportBuildYtdByMonthComparison(e,t,a,n){if(!a||!n)return[];const o=metaReportComparisonMonthKeysInRange(a.startMonthKey,a.endMonthKey),r=String(n.startMonthKey||"").slice(0,4);return o.map(s=>{const i=String(s).slice(5,7),l=`${r}-${i}`,c=metaReportAggregateComparisonRange(e,s,s),d=metaReportAggregateComparisonRange(t,l,l);return{monthKey:s,label:metaMonthLabel(s),periodA:c.hasData?c.metrics:null,periodB:d.hasData?d.metrics:null,hasDataA:c.hasData,hasDataB:d.hasData}})}function metaReportBuildComparison({monthsMap:e={},monthsMapA:t=null,monthsMapB:a=null,periodA:n=null,periodB:o=null,mode:r="mom"}={}){const s=t||a||e,i=metaReportComparisonNormalizePeriod(n),l=metaReportComparisonNormalizePeriod(o);if(!i?.startDate||!i?.endDate||!l?.startDate||!l?.endDate)return{mode:r,insufficientData:!0,samePeriod:!1,periodA:null,periodB:null,deltas:{},heroMetrics:[],detailRows:[],ytdByMonth:[],hasBottomline:!1};const c=i.startDate===l.startDate&&i.endDate===l.endDate,d=metaReportComparisonAggregateForMode(r,s,n),u=metaReportComparisonAggregateForMode(r,s,o),p=d.hasBottomline||u.hasBottomline,h=!d.hasData&&!u.hasData,m=metaReportComparisonMetricDefs(p),g={},b=m.map(y=>{const $=d.metrics[y.id],w=u.metrics[y.id],S=metaReportDeltaPct($,w);return g[y.id]={pct:S,abs:Math.round((metaReportParseAmount($)-metaReportParseAmount(w))*100)/100},{...y,valueA:$,valueB:w,deltaPct:S}}),f=metaReportComparisonHeroIds(p).map(y=>{const $=m.find(w=>w.id===y);return{id:y,label:$?.label||y,valueA:d.metrics[y],valueB:u.metrics[y],deltaPct:g[y]?.pct??null,format:$?.format||"num",higherIsBetter:$?.higherIsBetter??null}}),v=r==="ytd"?metaReportBuildYtdByMonthComparison(s,s,i,l):[];return{mode:r,insufficientData:h,samePeriod:c,hasBottomline:p,periodA:d,periodB:u,deltas:g,heroMetrics:f,detailRows:b,ytdByMonth:v}}function metaReportComparisonYearsNeeded(e,t){const a=new Set;for(const n of[e,t]){const o=metaReportComparisonNormalizePeriod(n);o&&(o.startDate&&a.add(o.startDate.slice(0,4)),o.endDate&&a.add(o.endDate.slice(0,4)))}return[...a].filter(Boolean)}function metaReportComparisonMonthsForYear(e,t){return!e||Number(e.year)===Number(t)?e?.months||{}:metaReportsState.comparisonYearCache?.[t]?.months||{}}function metaReportComparisonAvailableMonthKeys(e){const t=new Set,a=n=>{n&&Object.entries(n.months||{}).forEach(([o,r])=>{metaReportMonthHasComparisonData(r)&&t.add(o)})};return a(e),Object.values(metaReportsState.comparisonYearCache||{}).forEach(a),[...t].sort()}function metaReportComparisonPresetDisabled(e,t){const a=metaReportsState.activeMonthKey;return a?e==="mom"?!metaReportComparisonAddMonths(a,-1):!1:!0}function metaReportComparisonFmtValue(e,t="num",a=!0){return a?t==="kr"?metaFmtKr(e):t==="x"?metaFmtX(e):t==="pct"?`${metaFmtNum(e,2)}%`:metaFmtNum(e):"\u2014"}function metaReportComparisonDeltaTone(e,t=null){return e==null||!Number.isFinite(e)||e===0?"neutral":t===!1?e<0?"positive":"negative":e>0?"positive":"negative"}function metaReportComparisonPeriodsFromState(e){let t=metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodA),a=metaReportComparisonNormalizePeriod(metaReportsState.comparisonPeriodB);if(!t||!a){const n=metaReportApplyComparisonPresetDefaults(e,metaReportsState.comparisonMode);t=n.periodA,a=n.periodB}return{periodA:t,periodB:a,mode:metaReportsState.comparisonMode}}function metaReportComparisonMaps(e,t,a){const n=metaReportComparisonMergedMonthsMap(e,t,a);return{monthsMapA:n,monthsMapB:n}}async function ensureMetaReportComparisonYears(e,{editable:t=!1}={}){const{periodA:a,periodB:n}=metaReportComparisonPeriodsFromState(e),o=metaReportComparisonYearsNeeded(a,n),r=String(e?.year||metaReportsState.selectedYear),s=o.filter(i=>i!==r&&!metaReportsState.comparisonYearCache[i]);if(s.length)for(const i of s)try{let l;t&&e?.clientId?l=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e.clientId)}?year=${encodeURIComponent(i)}`):REPORT_TOKEN&&(l=await fetch(`/api/meta-reports/public/${encodeURIComponent(REPORT_TOKEN)}?year=${encodeURIComponent(i)}`).then(async c=>{const d=await c.json();if(!c.ok)throw new Error(d.error||"Report not found");return d})),l&&(metaReportsState.comparisonYearCache[i]=l)}catch{}}function computeMetaReportComparison(e){const{periodA:t,periodB:a,mode:n}=metaReportComparisonPeriodsFromState(e),o=metaReportComparisonMergedMonthsMap(e,t,a);return metaReportBuildComparison({monthsMap:o,periodA:t,periodB:a,mode:n})}function metaReportComparisonDateInputBounds(e){const t=metaReportComparisonAvailableMonthKeys(e);if(!t.length)return{min:"",max:""};const a=metaReportComparisonMergedMonthsMap(e,null,null);let n="",o="";return t.forEach(r=>{const s=metaReportComparisonMonthEffectivePeriod(a[r],r);(!n||s.start<n)&&(n=s.start),(!o||s.end>o)&&(o=s.end)}),{min:n,max:o}}function metaReportComparisonMonthInputBounds(e){const t=new Set;if(e?.year&&t.add(String(e.year)),Object.keys(metaReportsState.comparisonYearCache||{}).forEach(n=>t.add(String(n))),metaReportComparisonAvailableMonthKeys(e).forEach(n=>t.add(String(n).slice(0,4))),!t.size){const n=metaReportsState.activeMonthKey;n&&t.add(String(n).slice(0,4))}if(!t.size){const n=String(new Date().getFullYear());return{min:`${n}-01`,max:`${n}-12`}}const a=[...t].sort((n,o)=>Number(n)-Number(o));return{min:`${a[0]}-01`,max:`${a[a.length-1]}-12`}}function metaReportComparisonReportYears(e){const t=new Set,a=i=>{i&&(i.year&&t.add(String(i.year)),Object.keys(i.months||{}).forEach(l=>{/^\d{4}-\d{2}$/.test(l)&&t.add(String(l).slice(0,4))}))};a(e),Object.values(metaReportsState.comparisonYearCache||{}).forEach(a),!t.size&&metaReportsState.activeMonthKey&&t.add(String(metaReportsState.activeMonthKey).slice(0,4)),t.size||t.add(String(new Date().getFullYear()));const n=[...t].sort((i,l)=>Number(i)-Number(l)),o=Number(n[0]),r=Number(n[n.length-1]),s=[];for(let i=o;i<=r;i+=1)s.push(String(i));return s}function metaReportComparisonMonthPickerYears(e){return metaReportComparisonReportYears(e)}function renderMetaReportComparisonMonthDropdown(e,t,a,n){const o=a.find(l=>l.value===t)||a[0],r=o?.value||t||"",s=o?.label||t||"\u2014",i=a.map(l=>`<button type="button" class="meta-report-comparison-month-option${l.value===r?" is-selected":""}" data-value="${esc(l.value)}" role="option" aria-selected="${l.value===r}">${esc(l.label)}</button>`).join("");return`
    <div class="meta-report-comparison-month-dropdown" data-comparison-month-part="${esc(e)}" data-selected-value="${esc(r)}">
      ...
      <div class="meta-report-comparison-month-menu" role="listbox" hidden>${i}</div>
    </div>
