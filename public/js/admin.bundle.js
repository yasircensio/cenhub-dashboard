...
Resume the interrupted ${$}?`)&&(l=y.id,d=Number(y.batchOffset)||0)}}if(a?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&d===0){const y=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),b=y.summary||y;l=b.runId,d=0,setFbLeadProgress({processed:0,total:b.inWindow||0,label:`Matching ${b.inWindow||0} Meta leads to GHL\u2026`})}else!a&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:S||p?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:d,batchLimit:r})});const y=c.summary||c;l=y.runId,d=y.nextBatchOffset??y.batchOffset+(y.batchProcessed||0),u=u.concat(y.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const b=y.inWindow||d||S;a?(setFbLeadProgress({processed:d,total:b,complete:!y.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...y,rows:u}))):updateGlobalApplyProgress({processed:d,total:b,written:y.updated||0,complete:!y.hasMore})}while((c.summary||c).hasMore);m=!0;const g=c.summary||c,f=g.inWindow||d||S,v=g.nextBatchOffset??d;if(a)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,g.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(g.updated||0)>0),showToast((g.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:g.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const y=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete y[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(y))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const w=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=w.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(g){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${g.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((g.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!m&&a&&hideFbLeadProgress(!0),!a&&!m&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function getMetaReportMonthBodyOptions(e={},{editable:t=!1,yearPayload:a=null}={}){return{editable:t,yearPayload:a}}function shouldShowMetaReportYearVisuals(e,t,{editable:a=!1}={}){if(!e||e.meta?.emptyMonth||!t)return!1;const{series:n,demo:s}=resolveMetaReportChartSeries(t);return!(n.length<META_REPORT_CHART_MIN_POINTS||s&&!a)}const META_REPORT_CHART_MIN_POINTS=2,META_REPORT_SCENARIO_MAX_HISTORICAL=9,META_REPORT_SCENARIO_PROJECTED_MONTHS=4,META_REPORT_SPEND_CHART_TYPE_OPTIONS=[{value:"area",label:"Area"},{value:"bar",label:"Bar"}],META_REPORT_SCENARIO_MONTH_WINDOW_OPTIONS=[{value:"3",label:"Last 3 ad months"},{value:"6",label:"Last 6 ad months"},{value:"9",label:"Last 9 ad months"},{value:"12",label:"Last 12 ad months"},{value:"all",label:"All ad-active months"}],META_REPORT_SCENARIO_FIXED_ELASTICITY=.8,META_SCENARIO_TREND_R_THRESHOLD=.35,META_REPORT_SCENARIO_MODEL_PILLS=[{id:"scenarioSmoothUneven",label:"Remove uneven months",defaultOn:!0},{id:"scenarioBlendHistory",label:"Balance recent months",defaultOn:!1},{id:"scenarioIncludeTrend",label:"Follow trend",defaultOn:!1},{id:"scenarioCautionStrongMonths",label:"Cap hot streak",defaultOn:!1,requires:"scenarioIncludeTrend"}];function normalizeMetaReportSpendChartType(e){const t=String(e||"area").trim().toLowerCase();return META_REPORT_SPEND_CHART_TYPE_OPTIONS.some(a=>a.value===t)?t:"area"}function hydrateMetaReportSpendChartType(e){metaReportsState.spendChartType=normalizeMetaReportSpendChartType(e?.settings?.metaReportSpendChartType)}function normalizeMetaReportScenarioPill(e,t=!1){if(typeof e=="boolean")return e;if(e==null)return t;const a=String(e).trim().toLowerCase();return a==="true"||a==="1"?!0:a==="false"||a==="0"?!1:t}function normalizeMetaReportScenarioMonthWindow(e){const t=String(e||"6").trim().toLowerCase();return META_REPORT_SCENARIO_MONTH_WINDOW_OPTIONS.some(a=>a.value===t)?t:"6"}function normalizeMetaReportBudgetBaseline(e){const t=String(e||"year").trim().toLowerCase();return t==="month"||t==="last"?t:"year"}function normalizeMetaReportBudgetMultiplier(e){const t=Number.parseFloat(e);return Number.isFinite(t)?Math.min(3,Math.max(.5,Math.round(t*10)/10)):2}function getMetaReportScenarioPillsFromState(){return{smoothUneven:!!metaReportsState.scenarioSmoothUneven,blendHistory:!!metaReportsState.scenarioBlendHistory,includeTrend:!!metaReportsState.scenarioIncludeTrend,cautionStrongMonths:!!metaReportsState.scenarioCautionStrongMonths}}function applyMetaReportScenarioPillsToState(e={}){metaReportsState.scenarioSmoothUneven=!!e.smoothUneven,metaReportsState.scenarioBlendHistory=!!e.blendHistory,metaReportsState.scenarioIncludeTrend=!!e.includeTrend,metaReportsState.scenarioCautionStrongMonths=!!e.cautionStrongMonths}function describeActiveMetaReportScenarioPills(){return META_REPORT_SCENARIO_MODEL_PILLS.filter(e=>!!metaReportsState[e.id]&&(!e.requires||!!metaReportsState[e.requires])).map(e=>e.label)}function hydrateMetaReportScenarioSettings(e){const t=e?.settings||{};metaReportsState.budgetMultiplier=normalizeMetaReportBudgetMultiplier(t.metaReportBudgetMultiplier),metaReportsState.budgetBaseline=normalizeMetaReportBudgetBaseline(t.metaReportBudgetBaseline),metaReportsState.scenarioMonthWindow=normalizeMetaReportScenarioMonthWindow(t.metaReportScenarioMonthWindow),applyMetaReportScenarioPillsToState({smoothUneven:normalizeMetaReportScenarioPill(t.metaReportScenarioSmoothUneven,!0),blendHistory:normalizeMetaReportScenarioPill(t.metaReportScenarioBlendHistory,!1),includeTrend:normalizeMetaReportScenarioPill(t.metaReportScenarioIncludeTrend,!1),cautionStrongMonths:normalizeMetaReportScenarioPill(t.metaReportScenarioCautionStrongMonths,!1)})}function renderMetaReportScenarioModelPills(){return`
    <div class="meta-report-scenario-pill-section" id="meta-report-scenario-pills-wrap">
      <div class="meta-report-scenario-pill-row" role="group" aria-label="Forecast options">
        ${META_REPORT_SCENARIO_MODEL_PILLS.map(({id:e,label:t,requires:a})=>{const n=!!metaReportsState[e],s=a?!metaReportsState[a]:!1,o=s?'Turn on "Follow trend" first':"";return`
            <button type="button" class="meta-report-scenario-pill${n&&!s?" is-active":""}${s?" is-disabled":""}" data-scenario-pill="${esc(e)}" aria-pressed="${n&&!s}"${s?" disabled":""}${o?` title="${esc(o)}"`:""}>${esc(t)}</button>
          `}).join("")}
      </div>
    </div>
  ...
  `}function renderMetaReportScenarioMetrics(e){if(!e||e.insufficientData)return'<p class="meta-report-scenario-empty">Add months with ad spend and leads to model a budget scenario.</p>';const t=metaReportDeltaPct(e.projected.totalLeadValue,e.baseline.totalLeadValue),a=metaReportFormatDeltaPct(t),n=metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.roasKr,e.baseline.roasKr));fetch("http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c",{method:"POST",headers:{"Content-Type":"application/json","X-Debug-Session-Id":"b952dc"},body:JSON.stringify({sessionId:"b952dc",hypothesisId:"A,B,D",location:"index.html:renderMetaReportScenarioMetrics",message:"grid metrics render",data:{displayedProjectedRevenue:metaFmtKr(e.projected.totalLeadValue),displayedBaselineRevenue:metaFmtKr(e.baseline.totalLeadValue),rawProjectedRevenue:e.projected.totalLeadValue,rawBaselineRevenue:e.baseline.totalLeadValue,revenueDeltaRaw:t,revenueDeltaFormatted:a,projectedSpend:e.projected.spend,baselineSpend:e.baseline.spend,spendDeltaRaw:metaReportDeltaPct(e.projected.spend,e.baseline.spend)},timestamp:Date.now()})}).catch(()=>{});const s=e.projectedConservative&&e.projectedOptimistic&&e.projectedConservative.totalLeadValue!==e.projectedOptimistic.totalLeadValue?`${metaFmtKr(e.projectedConservative.totalLeadValue)} \u2013 ${metaFmtKr(e.projectedOptimistic.totalLeadValue)}`:"",o=[renderMetaReportScenarioOutcomeCard({label:"Projected revenue",projectedValue:metaFmtKr(e.projected.totalLeadValue),projectedSub:s,baselineValue:metaFmtKr(e.baseline.totalLeadValue),delta:a}),renderMetaReportScenarioOutcomeCard({label:"Projected ROAS",projectedValue:metaFmtKr(e.projected.roasKr),projectedSub:metaFmtX(e.projected.roasX),baselineValue:`${metaFmtKr(e.baseline.roasKr)} \xB7 ${metaFmtX(e.baseline.roasX)}`,delta:n})];if(e.hasBottomline){const i=metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.poasKr,e.baseline.poasKr));o.push(renderMetaReportScenarioOutcomeCard({label:"Projected profit",projectedValue:metaFmtKr(e.projected.poasKr),projectedSub:metaFmtX(e.projected.poasX),baselineValue:`${metaFmtKr(e.baseline.poasKr)} \xB7 ${metaFmtX(e.baseline.poasX)}`,delta:i}))}else{const i=metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.wonLeads,e.baseline.wonLeads));o.push(renderMetaReportScenarioOutcomeCard({label:"Projected won leads",projectedValue:metaFmtNum(e.projected.wonLeads),baselineValue:metaFmtNum(e.baseline.wonLeads),delta:i}))}const r=[renderMetaReportScenarioDetailRow("Ad spend",metaFmtKr(e.baseline.spend),metaFmtKr(e.projected.spend),metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.spend,e.baseline.spend))),renderMetaReportScenarioDetailRow("Leads",metaFmtNum(e.baseline.leads),metaFmtNum(e.projected.leads),metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.leads,e.baseline.leads)))];return e.hasBottomline&&r.push(renderMetaReportScenarioDetailRow("Won leads",metaFmtNum(e.baseline.wonLeads),metaFmtNum(e.projected.wonLeads),metaReportFormatDeltaPct(metaReportDeltaPct(e.projected.wonLeads,e.baseline.wonLeads)))),`
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null}={}){const s=n||e?.monthKey||null,o=s?metaReportMonthBounds(s):null;let r="";t&&e&&!e.meta?.emptyMonth&&o&&(r=`
    ...
    `);const i=n||e?.monthKey||null,l=t?i?`Report preview \xB7 ${metaMonthLabel(i)}`:"Report preview":i?`Report \xB7 ${metaMonthLabel(i)}`:"Report",d=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${l}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",c=renderMetaReportMonthTables(e,{editable:t}),u=shouldShowMetaReportYearVisuals(e,a,{editable:t}),m=u?renderMetaReportChartsPanel({editable:t}):"",h=t&&u?renderMetaReportScenarioCard():"",p=u?`
    <div class="meta-report-content-layout">
      <div class="meta-report-content-main">${c}</div>
      ${m}
    </div>
    ${h}
  `:`
    <div class="meta-report-content-main">${c}</div>
  `;return`${r}${d}${p}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
      ...
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${r?"":" is-hidden"}" id="${t}-fee-nested" ${h}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
            <span class="meta-report-fee-status-value is-active">${esc(w)}</span>
          </div>
  ...
  `}const p=describeMetaReportFeeModeLabel(d,c,u),S=META_REPORT_FEE_MODE_META.map(({value:w,icon:y,title:b,desc:$})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(w)}"${d===w?" checked":""} />
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(S=>`
            <option value="${esc(S.clientId)}"${S.clientId===a.clientId?" selected":""}>${esc(S.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(S=>`
            <option value="${esc(S)}"${S===n?" selected":""}>${esc(metaMonthLabel(S))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
