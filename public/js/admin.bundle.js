...
Resume the interrupted ${b}?`)&&(l=m.id,d=Number(m.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const m=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),c=m.summary||m;l=c.runId,d=0,setFbLeadProgress({processed:0,total:c.inWindow||0,label:`Matching ${c.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:w||v?.updated||0});do{u=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:k,batchOffset:d,batchLimit:o})});const m=u.summary||u;l=m.runId,d=m.nextBatchOffset??m.batchOffset+(m.batchProcessed||0),y=y.concat(m.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const c=m.inWindow||d||w;n?(setFbLeadProgress({processed:d,total:c,complete:!m.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...m,rows:y}))):updateGlobalApplyProgress({processed:d,total:c,written:m.updated||0,complete:!m.hasMore})}while((u.summary||u).hasMore);g=!0;const $=u.summary||u,f=$.inWindow||d||w,h=$.nextBatchOffset??d;if(n)setFbLeadProgress({processed:h,total:f,complete:!0}),markFbLeadPreviewReady(e,t,$.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=($.updated||0)>0),showToast(($.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:h,total:f,written:$.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const m=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete m[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(m))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const p=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=p.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch($){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${$.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast(($.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!g&&n&&hideFbLeadProgress(!0),!n&&!g&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
  ...
  `).join("")}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;if(!t||!metaReportsState.activeMonthKey)return;t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e;const n=document.getElementById("meta-report-month-panel");n&&(n.innerHTML=renderMetaReportMonthBody(e,{editable:!0,showOtherToClient:t.settings?.metaReportShowOther!==!1}),bindMetaReportsClientEditEvents(t.clientId,t),refreshMetaReportBackfillControl(t))}function refreshMetaReportBackfillControl(e){const t=document.querySelector(".meta-report-client-page .meta-report-toolbar"),n=document.querySelector(".meta-report-backfill-wrap"),a=metaReportMonthsNeedingBackfill(e);if(!a.length){n&&n.remove();return}const s=`${ICON_SYNC} Backfill ${a.length} month${a.length===1?"":"s"} from Meta`,i=document.getElementById("meta-report-backfill");if(i)i.innerHTML=s,i.disabled=!1;else if(t){const o=document.createElement("div");o.className="meta-report-backfill-wrap",o.innerHTML=`
    ...
    `,t.appendChild(o),bindMetaReportBackfillButton(e.clientId)}}function updateMetaReportsClientContent(e){metaReportsState.clientPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null);const n=document.getElementById("meta-report-year");if(n){const u=[e.year,e.year-1].filter((y,g,k)=>k.indexOf(y)===g);n.innerHTML=u.map(y=>`
    ...
    `).join(""),bindPublicMetaReportTabEvents(e));const s=document.getElementById("meta-report-month-panel");s&&(s.innerHTML=renderMetaReportMonthBody(e.months?.[metaReportsState.activeMonthKey],{editable:!1,showOtherToClient:e.settings?.metaReportShowOther!==!1}))}const META_MONTH_LABELS=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function metaFmtKr(e){return`Dkr ${(Number(e)||0).toLocaleString("da-DK",{minimumFractionDigits:0,maximumFractionDigits:2})}`}function metaFmtNum(e,t=0){return(Number(e)||0).toLocaleString("da-DK",{minimumFractionDigits:t,maximumFractionDigits:t})}function metaFmtX(e){const t=Number(e)||0;return t>0?`${t.toFixed(2)}x`:"\u2014"}function metaMonthLabel(e){const t=Number(String(e||"").slice(5,7));return META_MONTH_LABELS[t-1]||e}function metaReportMonthsNeedingBackfill(e){return(e?.monthKeys||[]).filter(n=>!e.months?.[n]?.metaFetchedAt)}function renderMetaReportMetricTable(e,t,n="meta",a=""){const s=t.map(([i,o,r],l)=>`
    <div class="meta-kpi-card meta-kpi-card--${n}${l===0?" meta-kpi-card--headline":""}">
      <div class="meta-kpi-value">${esc(String(o??"\u2014"))}${r?`<span class="meta-kpi-unit">${esc(r)}</span>`:""}</div>
      <div class="meta-kpi-label">${esc(i)}</div>
    </div>
  `).join("");return`
    <section class="meta-kpi-section">
      <div class="meta-kpi-section-head">
        <span class="meta-kpi-dot meta-kpi-dot--${n}" aria-hidden="true"></span>
        <h3 class="meta-kpi-section-title">${esc(e)}</h3>
        ${a?`<span class="meta-kpi-section-badge">${esc(a)}</span>`:""}
      </div>
      <div class="meta-kpi-grid">${s}</div>
    </section>
  `}function renderMetaReportMonthBody(e,{editable:t=!1,showOtherToClient:n=!0}={}){if(!e)return'<div class="meta-report-empty">No data for this month yet.</div>';if(e.meta?.emptyMonth)return`<div class="meta-report-empty">No ad spend recorded for this month yet.${t?' Click "Refresh from Meta" below, or use "Backfill from Meta" above to pull historical months.':""}</div>`;const a=[["Total spend",metaFmtKr(e.meta?.spend),""],["Cost Pr Mile (CPM)",metaFmtKr(e.meta?.cpm),""],["Impressions",metaFmtNum(e.meta?.impressions),""],["Reach",metaFmtNum(e.meta?.reach),""],["Click",metaFmtNum(e.meta?.clicks),""],["Conversion Rate",metaFmtNum(e.meta?.conversionRatePercent,2),"%"]],s=[["Leads",metaFmtNum(e.topline?.leads),""],["Won leads",metaFmtNum(e.topline?.wonLeads),""],["Total Lead Value",metaFmtKr(e.topline?.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.topline?.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.topline?.cac),""],["Return on Ads Spend (ROAS)",metaFmtKr(e.topline?.roasKr),""],["Return on Ads Spend % (ROAS)",metaFmtX(e.topline?.roasX),""]];let i=renderMetaReportMetricTable("Meta ads",a,"meta");if(i+=renderMetaReportMetricTable("Topline KPI'er",s,"topline"),e.bottomline){const r=[["Leads",metaFmtNum(e.bottomline.leads),""],["Won leads",metaFmtNum(e.bottomline.wonLeads),""],["Total Lead Value",metaFmtKr(e.bottomline.totalLeadValue),""],["Average Lead Value",metaFmtKr(e.bottomline.avgLeadValue),""],["Client acquisition cost (CAC)",metaFmtKr(e.bottomline.cac),""],["Avg Total Profit",metaFmtKr(e.bottomline.totalProfit),""],["Avg Single Profit Order",metaFmtKr(e.bottomline.avgProfitPerWon),""],["Profit on Ads Spend (POAS)",metaFmtKr(e.bottomline.poasKr),""],["Profit on Ads Spend % (POAS)",metaFmtX(e.bottomline.poasX),""]];e.bottomline.censioFee>0&&(r.push([`Censio performance fee (${metaFmtNum(e.bottomline.feePercent,0)}%)`,metaFmtKr(e.bottomline.censioFee),""]),r.push(["Profit on Investment (POI)",metaFmtKr(e.bottomline.poiKr),""]),r.push(["Profit on Investment % (POI)",metaFmtX(e.bottomline.poiX),""])),i+=renderMetaReportMetricTable("Bottomline KPI'er",r,"bottomline")}if(!!e.customLineItems?.length&&(t||n)){const r=e.customLineItems.map(l=>[l.label,metaFmtKr(l.amount),""]);r.push(["Total",metaFmtKr(e.customLineItemsTotal),""]),i+=renderMetaReportMetricTable("Other",r,"other",t&&!n?"Hidden from client":"")}return t&&(i+=`
    ...
    `),i}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),r=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const u=document.getElementById("dashboard-content");u&&(u.innerHTML=renderKpiSkeleton()),r&&(r.disabled=!0,r.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let u=null;if(!availablePipelines.length){if(u=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=u.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,u.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const g=canReuseBootstrapDashboard(u)&&!s;let k,v;if(g?[v,k]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(u)]):[k,v]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=v,cachedData=applyMarketingData(k,v),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(u){if(l!==fetchGeneration)return;if(u.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
