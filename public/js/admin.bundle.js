  ...
  `}function renderMetaReportToplineChangeModal(e,t){const a=e?.settings||{},n=resolveClientToplineMode(a),o=a.hasGhl!==!1;return`
    ...
    ${n==="cenhub"?`
      <div class="meta-report-topline-sync-strip">
        ...
        <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-report-topline-modal-sync-ghl-year"${o?"":' disabled title="GHL not connected"'}>Sync year from Cenhub</button>
      </div>
    `:""}
...
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!b)return;b.trim().toLowerCase()==="overwrite"&&(u=!1,p=!0)}const h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(c)}`,{method:"POST",body:JSON.stringify({skipManual:u,overwriteManual:p})});r?await r():s&&await s(h,null,{yearSync:!0}),showToast(`Synced ${h.synced?.length||0} month(s)${h.skipped?.length?`, skipped ${h.skipped.length}`:""}`,"success")}catch(c){showToast(c.message||"Year sync failed","error")}finally{o&&o(!1)}});const l=document.getElementById(`${t}-refresh-snapshot`);l&&(l.onclick=async()=>{const c=a?.();if(c){o&&o(!0);try{const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(c)}/refresh-snapshot`,{method:"POST",body:"{}"});s&&await s(d,c),showToast("Cenhub data refreshed and synced","success")}catch(d){showToast(d.message||"Refresh failed","error")}finally{o&&o(!1)}}})}function monthHasCenhubSnapshot(e,t={}){return e?resolveMetaToplineSource(e,t)==="ghl"?!0:!!e.ghlSyncedAt:!1}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),a=e.monthKeys||[];return t.length?t.map(n=>`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(M=>M.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const o=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,o);const r=t.editorPayload?.months?.[n]||null,s=a.metaReportShowBottomline,i=!!a.metaReportGhlDataEnabled,l={metaReportGhlDataEnabled:a.metaReportGhlDataEnabled,metaReportToplineMode:a.metaReportToplineMode},c=resolveClientToplineMode(l),d=resolveMetaToplineSource(r||{dataSource:o.toplineSource},l),u=i&&c==="cenhub"&&d==="ghl",p=o.wonLeads!=null?o.wonLeads:r?.inputs?.wonLeads??r?.topline?.wonLeads??"",h=o.avgLeadValue!=null?o.avgLeadValue:r?.inputs?.avgLeadValue??r?.topline?.avgLeadValue??"",b=o.avgProfitPerWon!=null?o.avgProfitPerWon:r?.inputs?.avgProfitPerWon??"",v=o.ghlLeads??r?.ghlTotals?.leads??"",w=o.ghlTotalRevenue??r?.ghlTotals?.totalRevenue??"",m=o.ghlTotalProfit??r?.ghlTotals?.totalProfit??"",f=metaReportsState.customValues.draftInputs,g=f?.wonLeads??p,$=f?.avgLeadValue??h,y=f?.avgProfitPerWon??b,S=u?' readonly disabled tabindex="-1" aria-readonly="true"':"",k=u?" disabled":"",L=u?" is-locked":"",I=t.settingsSaving&&t.settingsSavingScope==="report",C=i?renderMetaCvToplineSourceControl(a,e):"",E=renderMetaCvMonthManualRow(r||{dataSource:d},a),A=i?renderMetaMonthSourcePill(r||{dataSource:d},l):"";return`
    <div class="meta-report-edit-panel-head">
      <div>
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label class="meta-cv-value-field${L}">Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(g)}"${S} />
      </label>
      <label class="meta-cv-value-field${L}">Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc($)}"${S} />
      </label>
      <label class="meta-cv-value-field${L}">Avg profit per won${s?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc(y)}"${S} />
      </label>
    </div>
    ${E}
    <div class="meta-report-settings-group meta-report-settings-group--stacked meta-cv-client-settings${I?" is-saving":""}">
      ${renderMetaReportSaveIndicator("meta-cv-settings-status",I)}
