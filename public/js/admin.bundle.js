...
Resume the interrupted ${$}?`)&&(l=g.id,d=Number(g.batchOffset)||0)}}if(a?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&d===0){const g=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),p=g.summary||g;l=p.runId,d=0,setFbLeadProgress({processed:0,total:p.inWindow||0,label:`Matching ${p.inWindow||0} Meta leads to GHL\u2026`})}else!a&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:y||m?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:S,batchOffset:d,batchLimit:r})});const g=c.summary||c;l=g.runId,d=g.nextBatchOffset??g.batchOffset+(g.batchProcessed||0),u=u.concat(g.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const p=g.inWindow||d||y;a?(setFbLeadProgress({processed:d,total:p,complete:!g.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...g,rows:u}))):updateGlobalApplyProgress({processed:d,total:p,written:g.updated||0,complete:!g.hasMore})}while((c.summary||c).hasMore);v=!0;const k=c.summary||c,h=k.inWindow||d||y,b=k.nextBatchOffset??d;if(a)setFbLeadProgress({processed:b,total:h,complete:!0}),markFbLeadPreviewReady(e,t,k.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(k.updated||0)>0),showToast((k.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:b,total:h,written:k.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const g=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete g[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(g))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const f=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=f.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(k){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${k.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((k.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!v&&a&&hideFbLeadProgress(!0),!a&&!v&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function isMetaReportCensioClient(e){const t=String(e?.clientId||"").toLowerCase(),a=String(e?.accountName||"");return t==="censio"||/censio/i.test(a)}function monthHasChartData(e){if(!e||e.meta?.emptyMonth)return!1;const t=Number(e.meta?.spend)||0,a=Number(e.topline?.leads)||0;return t>0||a>0}function buildMetaReportChartSeries(e){const t=e?.monthKeys||[],a=e?.months||{},n=[];for(const s of t){const o=a[s];if(!o||o.meta?.emptyMonth||!monthHasChartData(o))continue;const r=Number(o.topline?.roasKr)||0,i=Number(o.topline?.roasX)||0,l=!!o.bottomline,d=l?Number(o.bottomline.poasKr)||0:null,c=l?Number(o.bottomline.poasX)||0:null;n.push({monthKey:s,label:metaMonthLabel(s),roasKr:r,roasX:i,poasKr:d,poasX:c,spend:Number(o.meta?.spend)||0,leads:Number(o.topline?.leads)||0,wonLeads:Number(o.topline?.wonLeads)||0,avgLeadValue:Number(o.topline?.avgLeadValue)||0,avgProfitPerWon:Number(o.inputs?.avgProfitPerWon)||0,hasData:!0})}return n}function getMetaReportDemoSeries(e,t,a,n){if(!isMetaReportCensioClient({clientId:e,accountName:t}))return null;const o=new Date,r=o.getFullYear(),i=o.getMonth()+1,l=Number(a)||r;let d=(n||[]).filter(c=>{const u=Number(String(c).slice(0,4)),v=Number(String(c).slice(5,7));return!(u!==l||l===r&&v>i)});if(!d.length){const c=l===r?Math.min(8,i):8;d=Array.from({length:c},(u,v)=>`${l}-${String(v+1).padStart(2,"0")}`)}return d.map((c,u)=>{const v=d.length>1?u/(d.length-1):0,S=Math.round(8e3+v*1e4),m=Math.round(2e3+v*15e3),y=Number((.5+v*2.5).toFixed(2)),k=Math.round(1500+v*12e3),h=Number((.4+v*2.2).toFixed(2)),b=Math.round(20+v*80),f=Math.max(1,Math.round(b*(.08+v*.04)));return{monthKey:c,label:metaMonthLabel(c),roasKr:m,roasX:y,poasKr:k,poasX:h,spend:S,leads:b,wonLeads:f,avgLeadValue:85e3,avgProfitPerWon:42e3,hasData:!0,demo:!0}})}function resolveMetaReportChartSeries(e){const t=buildMetaReportChartSeries(e);if(t.length>=META_REPORT_CHART_MIN_POINTS)return{series:t,demo:!1};if(isMetaReportCensioClient(e)){const a=getMetaReportDemoSeries(e.clientId,e.accountName,e.year||metaReportsState.selectedYear,e.monthKeys);if(a&&a.length>=META_REPORT_CHART_MIN_POINTS)return{series:a,demo:!0}}return{series:t,demo:!1}}function metaReportParseAmount(e){const t=Number.parseFloat(e);return Number.isFinite(t)?t:0}function metaReportRoundMoney(e){return Math.round(metaReportParseAmount(e)*100)/100}function metaReportRoundRatio(e){return Math.round(metaReportParseAmount(e)*1e8)/1e8}function metaReportAggregateSeriesEfficiency(e=[]){let t=0,a=0,n=0,s=0,o=0;for(const i of e){const l=metaReportParseAmount(i.spend),d=metaReportParseAmount(i.leads),c=metaReportParseAmount(i.wonLeads);l<=0&&d<=0||(t+=l,a+=d,n+=c,s+=c*metaReportParseAmount(i.avgLeadValue),o+=c*metaReportParseAmount(i.avgProfitPerWon))}const r=e.filter(i=>metaReportParseAmount(i.spend)>0).length;return{avgCpl:a>0?metaReportRoundMoney(t/a):0,winRate:a>0?metaReportRoundRatio(n/a):0,avgLeadValue:n>0?metaReportRoundMoney(s/n):0,avgProfitPerWon:n>0?metaReportRoundMoney(o/n):0,avgMonthlySpend:r>0?metaReportRoundMoney(t/r):0}}function metaReportProjectBudgetAtSpend(e,t,a=!1){const n=metaReportParseAmount(e);if(n<=0||t.avgCpl<=0)return{spend:0,leads:0,wonLeads:0,totalLeadValue:0,roasKr:0,roasX:0,poasKr:null,poasX:null};const s=metaReportRoundMoney(n/t.avgCpl),o=metaReportRoundMoney(s*t.winRate),r=metaReportRoundMoney(o*t.avgLeadValue),i=metaReportRoundMoney(r-n),l=n>0?metaReportRoundRatio(i/n):0;let d=null,c=null;if(a&&t.avgProfitPerWon>0){const u=metaReportRoundMoney(o*t.avgProfitPerWon);d=metaReportRoundMoney(u-n),c=n>0?metaReportRoundRatio(d/n):0}return{spend:n,leads:s,wonLeads:o,totalLeadValue:r,roasKr:i,roasX:l,poasKr:d,poasX:c}}function metaReportProjectBudgetScenario({series:e=[],baselineSpend:t,multiplier:a=1,hasBottomline:n=!1}={}){const s=metaReportAggregateSeriesEfficiency(e),o=metaReportParseAmount(t),r=metaReportParseAmount(a)||1,i=metaReportProjectBudgetAtSpend(o,s,n),l=metaReportProjectBudgetAtSpend(metaReportRoundMoney(o*r),s,n);return{efficiency:s,baseline:i,projected:l,multiplier:r,baselineSpend:o,hasBottomline:n,insufficientData:s.avgCpl<=0||o<=0}}function metaReportSpendAxisTick(e){const t=Number(e)||0;return Math.abs(t)>=1e6?`${(t/1e6).toFixed(1)}M`:Math.abs(t)>=1e3?`${Math.round(t/1e3)}K`:t}function metaReportDeltaPct(e,t){const a=metaReportParseAmount(t),n=metaReportParseAmount(e);return a?metaReportRoundRatio((n-a)/Math.abs(a)*100):null}function metaReportFormatDeltaPct(e){return e==null||!Number.isFinite(e)?"":`${e>0?"+":""}${e.toFixed(0)}%`}function metaReportResolveBaselineSpend(e,t){if(metaReportsState.budgetBaseline==="month"&&metaReportsState.activeMonthKey){const n=t.find(s=>s.monthKey===metaReportsState.activeMonthKey);if(n&&metaReportParseAmount(n.spend)>0)return metaReportParseAmount(n.spend)}return metaReportAggregateSeriesEfficiency(t).avgMonthlySpend}function payloadHasMetaReportBottomline(e){return e?.settings?.metaReportShowBottomline===!1?!1:seriesHasBottomline(metaReportsState.chartSeries||buildMetaReportChartSeries(e))}function seriesHasBottomline(e=[]){return e.some(t=>t.poasKr!=null&&t.poasX!=null)}function renderMetaReportChartsPanel({editable:e=!1}={}){const t=metaReportsState.chartTab||"trend";return`
    <aside class="meta-report-charts-panel" id="meta-report-charts-panel">
      ...
      <div class="meta-report-chart-tab-panel${t==="spend"?" is-active":""}" data-meta-chart-panel="spend" role="tabpanel"${t==="spend"?"":" hidden"}>
        ${e?`
        <div class="meta-report-spend-chart-toolbar">
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1}={}){let a="";t&&e&&!e.meta?.emptyMonth&&(a=`
    ...
    `);const n=t?"Report preview":"Report",s=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${n}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",o=renderMetaReportMonthTables(e,{editable:t}),r=renderMetaReportChartsPanel({editable:t});return`${a}${s}
    <div class="meta-report-content-layout">
      <div class="meta-report-content-main">${o}</div>
      ${r}
    </div>
    ${renderMetaReportScenarioCard()}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `).join("");return`
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      ...
      <div class="meta-report-month-panel" id="meta-report-month-panel">
        ${renderMetaReportMonthBody(n,getMetaReportMonthBodyOptions(s,{editable:!0}))}
      </div>
    </div>
    `)}
  ...
  `).join("");return`
    ...
    <div class="meta-reports-page meta-reports-page--public">
      ...
      <div class="meta-report-month-panel" id="meta-report-month-panel">
        ${renderMetaReportMonthBody(n,getMetaReportMonthBodyOptions(e.settings,{editable:!1}))}
      </div>
    </div>
    `)}
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t,t&&(metaReportsState.clientShareExpanded=!1);const a=document.querySelector(".meta-report-bottomline-fee-settings--client"),n=document.getElementById(`${e}-fee-editor`);a?.classList.toggle("is-editing",t),a?.classList.toggle("is-collapsed",!t),n&&n.classList.toggle("is-hidden",!t);const s=document.getElementById("meta-report-share-summary"),o=document.getElementById("meta-report-share-editor");s&&(s.hidden=metaReportsState.clientShareExpanded),o&&(o.hidden=!metaReportsState.clientShareExpanded),syncMetaReportControlPanelUi()}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const a=document.getElementById(`${t}-fee-summary`);if(!a)return;const n=document.createElement("div");n.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const s=n.firstElementChild;s&&a.replaceWith(s);const o=document.getElementById(`${t}-fee-edit`);o&&(o.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:a=!1,variant:n="default",collapsible:s=!1,externalSummary:o=!1}={}){const r=!!e.metaReportShowBottomline,i=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",d=resolveMetaReportFeeMode(e),c=e.metaReportFeePercent??20,u=e.metaReportMarketingFeeAmount??0,v=metaReportUsesInlineFeeValue(t),S=`data-fee-mode="${esc(l)}" data-fee-enabled="${i?"true":"false"}" data-fee-percent="${esc(c)}" data-marketing-fee="${esc(u)}"`;if(n==="cv"){const f=describeMetaReportFeeModeLabel(d,c,u),g=META_REPORT_FEE_MODE_META.map(({value:p,title:$})=>`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${r?"":" is-hidden"}" id="${t}-fee-nested" ${S}>
  ...
  `}const m=describeMetaReportFeeModeLabel(d,c,u),y=META_REPORT_FEE_MODE_META.map(({value:f,icon:g,title:p,desc:$})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(f)}"${d===f?" checked":""} />
        ...
        </label>`).join(""),k=s?metaReportsState.clientReportSettingsExpanded:!0,h=s?`
    ...
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?k?" is-editing":" is-collapsed":""}">
      ${b}
      <div class="meta-report-bottomline-editor${s&&!k?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${r?"":" is-hidden"}" id="${t}-fee-nested" ${S}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(y=>y.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label>Avg lead value
        <input type="number" step="any" id="meta-cv-avg-lead-value" value="${esc(v)}" />
      </label>
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(a){showToast(a.message,"error")}finally{if(t){t.disabled=!1;const a=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${a}`:a}}}function getMetricsModelLabels(e,t=setupPipelines){const a=new Map((t||[]).map(n=>[n.id,n.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),a=!e.metricsModelSetAt||metricsModelChangeMode,n=a?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",o=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",r=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return a?`
    ...
    <div class="date-picker-panel">
      <div class="date-picker-header">
        ...
        <div class="date-picker-nav">
          ...
          <button type="button" class="date-picker-nav-btn" aria-label="Next month" onclick="shiftDatePickerMonth(1)" ${p?"":"disabled"}>${ICON_CHEVRON_RIGHT}</button>
        </div>
      </div>
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:a=!1,forceFresh:n=!1}=t,s=!!n,o=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const r=!!document.getElementById("dashboard-content"),i=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!r)o.innerHTML=`
        ...
        `)}`;else if(!a){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),i&&(i.disabled=!0,i.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const v=canReuseBootstrapDashboard(c)&&!s;let S,m;if(v?[m,S]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[S,m]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=m,cachedData=applyMarketingData(S,m),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(l!==fetchGeneration)return;if(c.name==="AbortError"){r?restoreDashboardContentAfterFailedFetch(r):o.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
