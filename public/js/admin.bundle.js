...
Resume the interrupted ${S}?`)&&(l=g.id,c=Number(g.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const g=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),v=g.summary||g;l=v.runId,c=0,setFbLeadProgress({processed:0,total:v.inWindow||0,label:`Matching ${v.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:$||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:r})});const g=d.summary||d;l=g.runId,c=g.nextBatchOffset??g.batchOffset+(g.batchProcessed||0),u=u.concat(g.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const v=g.inWindow||c||$;a?(setFbLeadProgress({processed:c,total:v,complete:!g.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...g,rows:u}))):updateGlobalApplyProgress({processed:c,total:v,written:g.updated||0,complete:!g.hasMore})}while((d.summary||d).hasMore);p=!0;const w=d.summary||d,f=w.inWindow||c||$,y=w.nextBatchOffset??c;if(a)setFbLeadProgress({processed:y,total:f,complete:!0}),markFbLeadPreviewReady(e,t,w.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(w.updated||0)>0),showToast((w.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:y,total:f,written:w.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const g=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete g[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(g))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const b=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=b.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(w){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${w.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((w.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(o=>o.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function closeMetaReportComparisonDatePickers(e=null){(metaReportsState.comparisonDatePickers||[]).forEach(t=>{t!==e&&t.isOpen&&t.close()})}function destroyMetaReportComparisonDatePickers(){closeMetaReportComparisonDatePickers(),document.querySelectorAll("[data-comparison-range]").forEach(e=>{e._metaComparisonDatePicker&&(e._metaComparisonDatePicker.destroy(),e._metaComparisonDatePicker=null)}),metaReportsState.comparisonDatePickers=[]}function bindMetaReportComparisonDateInputs(e,{editable:t=!1}={}){if(destroyMetaReportComparisonDatePickers(),typeof flatpickr>"u")return;const a=metaReportComparisonDateInputBounds(e),{periodA:n,periodB:s}=metaReportComparisonPeriodsFromState(e);document.querySelectorAll("[data-comparison-range]").forEach(o=>{const r=o.getAttribute("data-comparison-range");if(!r)return;const i=r==="b"?s:n,l=i?.startDate||a.min||"",c=i?.endDate||l||"",d=l&&c?[l,c]:l?[l]:[],u=flatpickr(o,{mode:"range",dateFormat:"Y-m-d",altInput:!0,altFormat:"j M Y",defaultDate:d,minDate:a.min||o.getAttribute("data-min")||void 0,maxDate:a.max||o.getAttribute("data-max")||void 0,disableMobile:!0,allowInput:!1,clickOpens:!0,animate:!0,showMonths:1,static:!1,appendTo:document.body,className:"meta-report-flatpickr",locale:{rangeSeparator:" \u2013 "},onOpen:(h,m,$)=>{closeMetaReportComparisonDatePickers($)},onChange:(h,m,$)=>{if(h.length<2||$._metaComparisonApplying)return;const w=$.input.getAttribute("data-comparison-range");if(!w)return;const f=`comparisonPeriod${w.toUpperCase()}`;metaReportsState[f]||(metaReportsState[f]={startDate:null,endDate:null});let y=$.formatDate(h[0],"Y-m-d"),b=$.formatDate(h[1],"Y-m-d");if(y>b){const g=y;y=b,b=g}metaReportsState[f].startDate=y,metaReportsState[f].endDate=b,metaReportsState.comparisonMode="custom",document.querySelectorAll("[data-comparison-preset]").forEach(g=>{const v=g.getAttribute("data-comparison-preset")==="custom";g.classList.toggle("active",v),g.setAttribute("aria-pressed",v)}),refreshMetaReportComparison(e,{editable:t})}});o._metaComparisonDatePicker=u,metaReportsState.comparisonDatePickers.push(u);const p=o.closest(".meta-report-comparison-range-wrap");p&&(p.onclick=h=>{const m=u.altInput;h.target===o||h.target===m||u.open()})})}function renderMetaReportComparisonPeriodTiles(e,t,a){const n=metaReportsState.comparisonMode||"mom";if(n==="custom"){const r=metaReportComparisonDateInputBounds(a),i=e?.startDate||r.min||"",l=e?.endDate||i,c=t?.startDate||r.min||"",d=t?.endDate||c;return`
      <div class="meta-report-comparison-tiles" id="meta-report-comparison-range-panel">
        <div class="meta-report-comparison-tile is-a is-custom">
          ...
          ${renderMetaReportComparisonRangeField("a",i,l,r)}
        </div>
        ...
        <div class="meta-report-comparison-tile is-b is-custom">
          ...
          ${renderMetaReportComparisonRangeField("b",c,d,r)}
        </div>
      </div>
  ...
  `:"",p=renderMetaReportMonthTables(e,{editable:t}),h=d?renderMetaReportChartsPanel({editable:t}):"",m=t&&d?renderMetaReportScenarioCard():"",$=d?renderMetaReportViewModeTabs():"";let w;return d?w=`
      ...
      ${m}
    `:w=`<div class="meta-report-content-main">${p}</div>`,`${r}${u}${$}${w}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${r?"":" is-hidden"}" id="${t}-fee-nested" ${h}>
          ...
          <div class="meta-report-fee-mode-pills">${g}</div>
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),$=META_REPORT_FEE_MODE_META.map(({value:b,icon:g,title:v,desc:S})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(b)}"${c===b?" checked":""} />
          <span class="meta-report-fee-mode-card-icon" aria-hidden="true">${esc(g)}</span>
