...
Resume the interrupted ${S}?`)&&(l=$.id,c=Number($.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const $=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),w=$.summary||$;l=w.runId,c=0,setFbLeadProgress({processed:0,total:w.inWindow||0,label:`Matching ${w.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:y||m?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:h,batchOffset:c,batchLimit:s})});const $=d.summary||d;l=$.runId,c=$.nextBatchOffset??$.batchOffset+($.batchProcessed||0),u=u.concat($.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const w=$.inWindow||c||y;a?(setFbLeadProgress({processed:c,total:w,complete:!$.hasMore}),r&&(r.innerHTML=renderFbLeadRunResults({...$,rows:u}))):updateGlobalApplyProgress({processed:c,total:w,written:$.updated||0,complete:!$.hasMore})}while((d.summary||d).hasMore);p=!0;const b=d.summary||d,f=b.inWindow||c||y,g=b.nextBatchOffset??c;if(a)setFbLeadProgress({processed:g,total:f,complete:!0}),markFbLeadPreviewReady(e,t,b.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(b.updated||0)>0),showToast((b.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:g,total:f,written:b.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const $=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete $[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify($))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const v=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=v.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(b){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${b.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((b.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(r=>r.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `}function syncMetaReportBaselineUi(e=[],t=null,a=null){const n=resolveMetaReportScenarioBaselineHints(e,t,a),o=document.getElementById("meta-report-budget-baseline");if(o){const r=o.value||metaReportsState.budgetBaseline;o.innerHTML=renderMetaReportBaselineSelectOptions(n,r),o.value=normalizeMetaReportBudgetBaseline(r)}}function renderMetaReportScenarioContextStrip(e){if(!e||e.insufficientData)return'<p class="meta-report-scenario-context" id="meta-report-scenario-context">Add at least two completed ad months to model a budget scenario.</p>';if(!e.projectionAllowed){const c=metaReportsState.activeMonthKey,d=c?metaMonthLabel(c):"this month";return`<p class="meta-report-scenario-context" id="meta-report-scenario-context">Budget estimates are not shown for completed months like ${esc(d)}. Select the current or an upcoming month tab.</p>`}const t=describeActiveMetaReportScenarioPills(),a=t.length?t.join(" \xB7 "):"Recent months only",n=e.projectionTargetMonthKey?metaMonthLabel(e.projectionTargetMonthKey):"selected month",o=e.trendDirection==="up"?`trend up ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:e.trendDirection==="down"?`trend down ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:"trend flat",r=e.prepared?.outliersAdjusted?` \xB7 ${e.prepared.outliersAdjusted} uneven month${e.prepared.outliersAdjusted===1?"":"s"} removed`:"",i={low:"Low confidence",medium:"Medium confidence",high:"High confidence"}[e.prepared?.confidence]||"",l=i?` \xB7 ${i} (${e.prepared?.monthsUsed||0} mo data)`:"";return`<p class="meta-report-scenario-context" id="meta-report-scenario-context"><strong>Estimate for ${esc(n)}:</strong> ${esc(a)} \xB7 last ${e.prepared?.monthsUsed||0} ad months \xB7 ${esc(o)}${esc(r)}${esc(l)}</p>`}function payloadHasMetaReportBottomline(e){return e?.settings?.metaReportShowBottomline===!1?!1:seriesHasBottomline(metaReportsState.chartSeries||buildMetaReportChartSeries(e))}function seriesHasBottomline(e=[]){return e.some(t=>t.poasKr!=null&&t.poasX!=null)}function renderMetaReportChartsPanel({editable:e=!1}={}){const t=metaReportsState.chartTab||"trend";return`
  ...
  `}function renderMetaReportComparisonCustomDatesPanel(e,t,a){const n=metaReportComparisonDateInputBounds(a),o=e?.startDate||n.min||"",r=e?.endDate||o,s=t?.startDate||n.min||"",i=t?.endDate||s;return`
    <div class="meta-report-comparison-duel is-custom-dates" id="meta-report-comparison-range-panel" role="group" aria-label="Custom compared periods">
      ${renderMetaReportComparisonCustomRangeSide("a",o,r,n)}
      <div class="meta-report-comparison-duel-divider" aria-hidden="true">vs</div>
  ...
  `:"",p=renderMetaReportMonthTables(e,{editable:t}),h=d?renderMetaReportChartsPanel({editable:t}):"",m=t&&d?renderMetaReportScenarioCard():"",y=d?renderMetaReportViewModeTabs():"";let b;return d?b=`
      ...
      ${m}
    `:b=`<div class="meta-report-content-main">${p}</div>`,`${s}${u}${y}${b}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `}const m=describeMetaReportFeeModeLabel(c,d,u),y=META_REPORT_FEE_MODE_META.map(({value:v,icon:$,title:w,desc:S})=>`
