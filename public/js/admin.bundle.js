...
Resume the interrupted ${S}?`)&&(l=w.id,c=Number(w.batchOffset)||0)}}if(a?setFbLeadProgress({processed:c,total:0,label:c>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),a&&c===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l})}),$=w.summary||w;l=$.runId,c=0,setFbLeadProgress({processed:0,total:$.inWindow||0,label:`Matching ${$.inWindow||0} Meta leads to GHL\u2026`})}else!a&&c===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:b||f?.updated||0});do{d=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:a,runId:l,previewRunId:a?null:v,batchOffset:c,batchLimit:r})});const w=d.summary||d;l=w.runId,c=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const $=w.inWindow||c||b;a?(setFbLeadProgress({processed:c,total:$,complete:!w.hasMore}),s&&(s.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:c,total:$,written:w.updated||0,complete:!w.hasMore})}while((d.summary||d).hasMore);p=!0;const h=d.summary||d,m=h.inWindow||c||b,g=h.nextBatchOffset??c;if(a)setFbLeadProgress({processed:g,total:m,complete:!0}),markFbLeadPreviewReady(e,t,h.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(h.updated||0)>0),showToast((h.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:g,total:m,written:h.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const y=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=y.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(h){a?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${h.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((h.message||(a?"FB lead preview failed.":"FB lead apply failed."))+(a?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{o&&(o.disabled=!1),fbLeadSyncState.applyingRunId=null,!p&&a&&hideFbLeadProgress(!0),!a&&!p&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const a=await adminFetch(`/api/fb-lead-sync/history/${e}`),n=fbLeadSyncState.clients.find(s=>s.clientId===a.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(a.run.rows||[],a.run.locationId||n?.locationId)}</div></td>`,t.hidden=!1;const o=document.getElementById(`fb-lead-audit-scroll-${e}`);o&&o.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(a){showToast(a.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(o=>o.clientId===e)),a=document.getElementById("fb-lead-history-mount");if(!a)return;const n=Object.fromEntries(fbLeadSyncState.clients.map(o=>[o.clientId,o.locationId]));a.innerHTML=t.length?renderFbLeadHistoryRows(t,n):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,a.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const a=document.getElementById("dashboard");if(!a)return;const n=await fetchStaffMe();if(!n){e||(a.innerHTML=`
  ...
  `:`<div class="meta-hub-switch is-disabled"><span class="meta-hub-switch-label">${esc(e)}</span><span class="meta-report-muted">\u2014</span></div>`}function metaReportFullUrl(e){return e?`${window.location.origin}${e}`:""}function normalizeMetaReportSlugInput(e){return String(e||"").trim().toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"-").replace(/-+/g,"-").replace(/^-|-$/g,"")}function resolveMetaReportShareSlug(e){const t=e.settings||{};if(t.metaReportSlug)return t.metaReportSlug;const n=String(e.reportUrl||"").replace(/^\/report\//,"").match(/^(.+)-(\d{4})$/);return n?n[1]:""}function resolveMetaReportShareCode(e){const a=String(e.reportUrl||"").replace(/^\/report\//,"").match(/-(\d{4})$/);return a?a[1]:""}function renderMetaReportShareSummary(e,{hidden:t=!1}={}){const n=!!(e.settings||{}).metaReportEnabled,o=resolveMetaReportShareSlug(e),s=resolveMetaReportShareCode(e),r=o?`${o}-${s||"\xB7\xB7\xB7\xB7"}`:"Not set";return`
  ...
  `}function renderMetaReportClientControlPanel(e){const t=e.settings||{},a=metaReportsState.clientShareExpanded,n=metaReportsState.clientReportSettingsExpanded,o=!!e.reportUrl,s=a||n,r=metaReportMonthsNeedingBackfill(e),i=r.length?`${ICON_SYNC} Backfill ${r.length} month${r.length===1?"":"s"} from Meta`:`${ICON_SYNC} Re-sync year from Meta`;return`
    <div class="meta-report-control-panel${a?" is-editing-share":""}${n?" is-editing-report":""}${t.metaReportEnabled?"":" is-share-disabled"}" id="meta-report-control-panel">
      ...
      <div class="meta-report-control-editors">
        ${o?renderMetaReportShareEditor(e):""}
  ...
  `}function renderMetaReportComparisonPeriodControls(e,t,a,n=null){const o=metaReportsState.comparisonMode||"mom";return o==="custom"?renderMetaReportComparisonCustomDatesPanel(e,t,a):o==="months"?renderMetaReportComparisonMonthPickersPanel(e,t,a):renderMetaReportComparisonSummaryBar(e,t,n)}function renderMetaReportComparisonPeriodTiles(e,t,a,n=null){return renderMetaReportComparisonPeriodControls(e,t,a,n)}function renderMetaReportComparisonChangeCell(e,t,a=!0,n=!0){const o=metaReportComparisonDeltaTone(e,t);if(!a&&!n||!a||!n||e==null||!Number.isFinite(e))return'<span class="meta-report-comparison-change is-neutral">\u2014</span>';const s=e>0?"\u2191":e<0?"\u2193":"\u2014",r=metaReportFormatDeltaPct(e);return`<span class="meta-report-comparison-change is-${o}">${esc(s)} ${esc(r)}</span>`}function renderMetaReportComparisonGroupTable(e,t,a,n,{highlightLastN:o=0}={}){if(!n||n.insufficientData||n.samePeriod)return"";const s=n.periodA.label,r=n.periodB.label,i=n.periodA.hasData!==!1,l=n.periodB.hasData!==!1,c=t.map((d,u)=>{const p=n.periodA.metrics[d.metricId],v=n.periodB.metrics[d.metricId],f=i&&l?metaReportDeltaPct(p,v):null;return`
      <tr class="${o>0&&u>=t.length-o?`is-highlight accent-${a}`:""}">
        ...
        <td class="meta-report-comparison-col-b">${esc(metaReportComparisonFmtValue(v,d.format,l))}</td>
        <td class="meta-report-comparison-col-change">${renderMetaReportComparisonChangeCell(f,d.higherIsBetter,i,l)}</td>
      </tr>
    `}).join("");return`
    <section class="meta-report-group">
      <div class="meta-report-group-head">
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:a=null,activeMonthKey:n=null,settings:o=null}={}){const s=n||e?.monthKey||null,r=s?metaReportMonthBounds(s):null,i=o||a?.settings||{},l=isMetaReportGhlListClient(i),c=resolveMetaToplineUiMode(e,metaReportsState.clientToplineUiMode),d=resolveMetaToplineSource(e),u=l&&c==="cenhub"&&d==="ghl",p=l&&c==="cenhub"&&d!=="ghl";let v="";if(t&&e&&!e.meta?.emptyMonth&&r){const A=u?" readonly":"",L=e.manualLeads??e.inputs?.manualLeads??"",k=l?renderMetaToplineModeSection({prefix:"meta-report",monthKey:s,monthPayload:e,settings:i,saving:metaReportsState.clientToplineSaving,uiPreference:metaReportsState.clientToplineUiMode}):"";v=`
    ...
    `}const f=n||e?.monthKey||null,b=t?f?`Report preview \xB7 ${metaMonthLabel(f)}`:"Report preview":f?`Report \xB7 ${metaMonthLabel(f)}`:"Report",h=metaReportsState.reportViewMode||"monthly",m=shouldShowMetaReportYearVisuals(e,a,{editable:t}),g=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${b}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",y=renderMetaReportMonthTables(e,{editable:t}),w=m?renderMetaReportChartsPanel({editable:t}):"",$=t&&m?renderMetaReportScenarioCard():"",S=m?renderMetaReportViewModeTabs():"";let I;return m?I=`
      <div class="meta-report-view-panel${h==="monthly"?" is-active":""}" data-report-view-panel="monthly" role="tabpanel"${h==="monthly"?"":" hidden"}>
        <div class="meta-report-content-layout">
          <div class="meta-report-content-main">${y}</div>
          ${w}
        </div>
      </div>
      <div class="meta-report-view-panel${h==="comparison"?" is-active":""}" data-report-view-panel="comparison" role="tabpanel"${h==="comparison"?"":" hidden"}>
        ${renderMetaReportComparisonView()}
      </div>
      ${$}
    `:I=`<div class="meta-report-content-main">${y}</div>`,`${v}${g}${S}${I}`}function renderMetaReportsHubPage(e){const t=e.clients||[],a=e.meta||{};return`
  ...
  `}function setMetaReportYearLoadingBanner(e,t,a){const n=document.querySelector(e);if(!n)return;let o=n.querySelector(".meta-report-year-loading-banner");a?(o||(o=document.createElement("span"),o.className="meta-report-year-loading-banner",o.setAttribute("aria-live","polite"),n.appendChild(o)),o.innerHTML=`<span class="meta-report-save-indicator-spinner"></span> Loading ${esc(t)}\u2026`,o.hidden=!1):o&&(o.hidden=!0)}function setMetaReportClientYearLoading(e,t){const a=document.querySelector(".meta-report-client-page"),n=document.getElementById("meta-report-year");if(t){n&&(n.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!0);const o=document.querySelector(".meta-report-tabs");o&&(o.innerHTML=renderMetaReportTabsSkeleton(),o.setAttribute("aria-busy","true"));const s=document.getElementById("meta-report-month-panel");s&&(s.innerHTML=renderMetaReportMonthPanelSkeleton(),s.classList.add("is-loading")),destroyMetaReportCharts()}else{n&&(n.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!1);const o=document.querySelector(".meta-report-tabs");o&&o.removeAttribute("aria-busy");const s=document.getElementById("meta-report-month-panel");s&&s.classList.remove("is-loading")}}function setMetaCvYearLoading(e,t){const a=document.querySelector(".sync-history-page.meta-reports-page"),n=document.getElementById("meta-cv-year"),o=document.getElementById("meta-cv-search");if(t){n&&(n.disabled=!0),o&&(o.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!0);const s=document.querySelector(".meta-cv-summary");s&&(s.classList.add("is-skeleton"),s.innerHTML=renderMetaCvSummarySkeleton());const r=document.querySelector(".meta-cv-legend");r&&r.setAttribute("aria-hidden","true");const i=document.getElementById("meta-cv-client-list");i&&(i.innerHTML=renderMetaCvListSkeleton(),i.setAttribute("aria-busy","true"));const l=document.getElementById("meta-cv-editor");l&&(l.innerHTML=renderMetaCvEditorYearSkeleton(),l.setAttribute("aria-busy","true"))}else{n&&(n.disabled=!1),o&&(o.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!1);const s=document.querySelector(".meta-cv-summary");s&&s.classList.remove("is-skeleton");const r=document.querySelector(".meta-cv-legend");r&&r.removeAttribute("aria-hidden");const i=document.getElementById("meta-cv-client-list");i&&i.removeAttribute("aria-busy");const l=document.getElementById("meta-cv-editor");l&&l.removeAttribute("aria-busy")}}function setMetaReportPublicYearLoading(e,t){const a=document.querySelector(".meta-reports-page--public"),n=document.getElementById("meta-report-year");if(t){n&&(n.disabled=!0),a&&a.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!0);const o=document.querySelector(".meta-reports-page--public .meta-report-tabs");o&&(o.innerHTML=renderMetaReportTabsSkeleton(),o.setAttribute("aria-busy","true"));const s=document.getElementById("meta-report-month-panel");s&&(s.innerHTML=renderMetaReportMonthPanelSkeleton(),s.classList.add("is-loading")),destroyMetaReportCharts()}else{n&&(n.disabled=!1),a&&a.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!1);const o=document.querySelector(".meta-reports-page--public .meta-report-tabs");o&&o.removeAttribute("aria-busy");const s=document.getElementById("meta-report-month-panel");s&&s.classList.remove("is-loading")}}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,a=document.getElementById("meta-report-year");a&&(a.onchange=async()=>{if(a.selectedOptions?.[0]?.disabled){a.value=String(metaReportsState.selectedYear);return}const o=metaReportsState.selectedYear,s=Number(a.value);if(s!==o){metaReportsState.selectedYear=s,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportClientYearLoading(s,!0);try{await loadMetaReportsClientPage({silent:!0})}catch(r){metaReportsState.selectedYear=o,a&&(a.value=String(o)),await loadMetaReportsClientPage({silent:!0}),showToast(r.message||"Load failed","error")}finally{setMetaReportClientYearLoading(s,!1)}}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report"),bindMetaReportShareEvents(e)}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e),isMetaReportGhlListClient(e.settings||{})&&bindMetaReportToplineEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const a=document.getElementById("meta-report-refresh-meta");a&&(a.onclick=async()=>{a.disabled=!0;try{const o=metaReportsState.activeMonthKey,s=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(s.monthPayload),showToast("Meta data refreshed","success")}catch(o){showToast(o.message||"Meta refresh failed","error")}finally{a.disabled=!1}});const n=document.getElementById("meta-report-save-month");n&&(n.onclick=async()=>{n.disabled=!0;try{const o=metaReportsState.activeMonthKey,s=t?.months?.[o],r=t?.settings||{},i=resolveMetaToplineUiMode(s,metaReportsState.clientToplineUiMode),l=isMetaReportGhlListClient(r)&&i==="cenhub"&&resolveMetaToplineSource(s)!=="ghl",c={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:document.getElementById("meta-report-won-leads")?.value===""?null:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:document.getElementById("meta-report-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-report-avg-profit")?.value===""?null:Number(document.getElementById("meta-report-avg-profit")?.value)};if(l){c.toplineSource="manual",c.manualOverride=!0;const u=document.getElementById("meta-report-manual-leads");u&&(c.manualLeads=u.value===""?null:Number(u.value))}const d=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(o)}`,{method:"PUT",body:JSON.stringify(c)});refreshMetaReportMonthPanel(d.monthPayload),showToast("Month saved","success")}catch(o){showToast(o.message||"Save failed","error")}finally{n.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{switchMetaReportMonthTab(t.getAttribute("data-meta-month-tab"),{editable:!1})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}const n=metaReportsState.selectedYear,o=Number(t.value);if(o!==n){metaReportsState.selectedYear=o,metaReportsState.activeMonthKey=null,metaReportsState.comparisonYearCache={},setMetaReportPublicYearLoading(o,!0);try{await loadPublicMetaReportPage({silent:!0})}catch(s){metaReportsState.selectedYear=n,t&&(t.value=String(n)),await loadPublicMetaReportPage({silent:!0}),showToast(s.message||"Load failed","error")}finally{setMetaReportPublicYearLoading(o,!1)}}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
    ...
    `}).join("")}function isMetaReportGhlListClient(e={}){return!!e.metaReportGhlDataEnabled}function resolveMetaToplineSource(e){return e?.dataSource||e?.toplineSource||"meta"}function classifyMetaReportMonthInputStatus(e,t=!1){if(!e)return"empty";const a=e.topline?.wonLeads!=null||e.inputs?.wonLeads!=null,n=e.topline?.avgLeadValue!=null||e.inputs?.avgLeadValue!=null,o=e.inputs?.avgProfitPerWon!=null;return a&&n&&(!t||o)?"complete":a||n||o?"partial":"empty"}function resolveMetaToplineUiMode(e,t=null){return t==="cenhub"||t==="meta"?t:resolveMetaToplineSource(e)==="meta"?"meta":"cenhub"}function monthHasCenhubSnapshot(e){if(resolveMetaToplineSource(e)==="ghl")return!0;const a=e?.ghlTotals||{};return!!(e?.ghlSyncedAt||a.leads!=null||a.wonLeads!=null)}function resolveMetaToplineStatus(e,t){const a=resolveMetaToplineSource(e);return t==="meta"?{badgeClass:"is-meta",label:"Using Meta Analytics",hint:null}:a==="ghl"?{badgeClass:"is-cenhub",label:"Using Cenhub",hint:null}:a==="manual"?{badgeClass:"is-manual",label:"Manual values",hint:null}:{badgeClass:"is-cenhub-empty",label:"No Cenhub data",hint:"Sync from Cenhub or enter values below, then save."}}function renderMetaToplineModeSection({prefix:e,monthKey:t,monthPayload:a,settings:n={},saving:o=!1,uiPreference:s=null}={}){if(!isMetaReportGhlListClient(n))return"";const r=resolveMetaToplineUiMode(a,s),i=resolveMetaToplineSource(a),l=resolveMetaToplineStatus(a,r),c=o?" disabled":"",d=n.hasGhl!==!1,u=r==="cenhub",p=r==="cenhub"&&i==="ghl";return`
    <div class="meta-topline-subsection" id="${esc(e)}-topline-section" data-meta-topline-prefix="${esc(e)}">
      <div class="meta-topline-subsection-head">
        <div class="meta-topline-subsection-label">Topline source</div>
        <span class="meta-topline-status-badge ${esc(l.badgeClass)}">${esc(l.label)}</span>
      </div>
      <div class="meta-topline-switcher" role="group" aria-label="Topline source">
        <button type="button" class="meta-topline-switcher-btn${r==="meta"?" is-active":""}" id="${esc(e)}-mode-meta" data-meta-topline-mode="meta"${c}>Meta Analytics</button>
        <button type="button" class="meta-topline-switcher-btn${r==="cenhub"?" is-active":""}" id="${esc(e)}-mode-cenhub" data-meta-topline-mode="cenhub"${c}>Cenhub</button>
      </div>
      ${l.hint?`<p class="meta-topline-hint">${esc(l.hint)}</p>`:""}
      ${u?`
        <div class="meta-cv-ghl-actions">
          <button type="button" class="admin-btn admin-btn--secondary" id="${esc(e)}-sync-ghl"${c||!d?" disabled":""}${d?"":' title="GHL not connected"'}>Sync with Cenhub</button>
          <button type="button" class="admin-btn admin-btn--ghost" id="${esc(e)}-sync-ghl-year"${c||!d?" disabled":""}>Sync year\u2026</button>
          <button type="button" class="admin-btn admin-btn--ghost" id="${esc(e)}-refresh-snapshot"${c||!d?" disabled":""}>Refresh Cenhub data</button>
          ${p?`<button type="button" class="admin-btn admin-btn--ghost" id="${esc(e)}-use-manual"${c}>Enter manually for this month</button>`:""}
        </div>
      `:""}
    </div>
  `}function bindMetaToplineSourceEvents({clientId:e,prefix:t,getMonthKey:a,getYear:n,getMonthRecord:o,setSaving:s,getUiPreference:r,setUiPreference:i,onMonthUpdated:l}){if(!e||!t)return;const c=document.getElementById(`${t}-mode-cenhub`);c&&(c.onclick=async()=>{if(c.disabled)return;const b=a?.(),h=o?.(b);resolveMetaToplineUiMode(h,r?.())!=="cenhub"&&(i&&i("cenhub"),l&&await l({},b,{refreshUiOnly:!0}))});const d=document.getElementById(`${t}-mode-meta`);d&&(d.onclick=async()=>{if(d.disabled)return;const b=a?.(),h=o?.(b);if(resolveMetaToplineUiMode(h,r?.())!=="meta"&&b){s&&s(!0);try{const g=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(b)}/use-meta`,{method:"POST",body:"{}"});i&&i(null),l&&await l(g,b),showToast("Using Meta analytics for this month","success")}catch(g){showToast(g.message||"Update failed","error")}finally{s&&s(!1)}}});const u=document.getElementById(`${t}-use-manual`);u&&(u.onclick=async()=>{if(u.disabled)return;const b=a?.();if(b){s&&s(!0);try{const h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(b)}/use-manual`,{method:"POST",body:"{}"});i&&i("cenhub"),l&&await l(h,b),showToast("Manual entry enabled for this month","success")}catch(h){showToast(h.message||"Could not switch to manual mode","error")}finally{s&&s(!1)}}});const p=document.getElementById(`${t}-sync-ghl`);p&&(p.onclick=async()=>{const b=a?.();if(!b)return;const h=o?.(b);let m=!1;if(h?.manualOverride&&(h?.toplineSource==="manual"||h?.dataSource==="manual")){if(!window.confirm("This month has manual values. Overwrite with Cenhub data?"))return;m=!0}s&&s(!0);try{const g=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(b)}/sync-ghl`,{method:"POST",body:JSON.stringify({overwriteManual:m})});i&&i("cenhub"),l&&await l(g,b),showToast("Synced from Cenhub snapshot","success")}catch(g){showToast(g.message||"Sync failed","error")}finally{s&&s(!1)}});const v=document.getElementById(`${t}-sync-ghl-year`);v&&(v.onclick=async()=>{s&&s(!0);try{const b=n?.()||metaReportsState.selectedYear,h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-preview?year=${encodeURIComponent(b)}`);let m=!0,g=!1;if((h.skipManual||[]).length){const w=window.prompt(`${h.skipManual.length} month(s) have manual values (${h.skipManual.join(", ")}).
Type "skip" to skip manual months, or "overwrite" to replace all:`,"skip");if(!w)return;w.trim().toLowerCase()==="overwrite"&&(m=!1,g=!0)}const y=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/sync-ghl-year?year=${encodeURIComponent(b)}`,{method:"POST",body:JSON.stringify({skipManual:m,overwriteManual:g})});i&&i(null),l&&await l(y,null,{yearSync:!0}),showToast(`Synced ${y.synced?.length||0} month(s)${y.skipped?.length?`, skipped ${y.skipped.length}`:""}`,"success")}catch(b){showToast(b.message||"Year sync failed","error")}finally{s&&s(!1)}});const f=document.getElementById(`${t}-refresh-snapshot`);f&&(f.onclick=async()=>{const b=a?.();if(b){s&&s(!0);try{const h=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(b)}/refresh-snapshot`,{method:"POST",body:"{}"});i&&i("cenhub"),l&&await l(h,b),showToast("Cenhub data refreshed and synced","success")}catch(h){showToast(h.message||"Refresh failed","error")}finally{s&&s(!1)}}})}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),a=e.monthKeys||[];return t.length?t.map(n=>`
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t,t&&(metaReportsState.clientShareExpanded=!1);const a=document.querySelector(".meta-report-bottomline-fee-settings--client"),n=document.getElementById(`${e}-fee-editor`);a?.classList.toggle("is-editing",t),a?.classList.toggle("is-collapsed",!t),n&&n.classList.toggle("is-hidden",!t);const o=document.getElementById("meta-report-share-summary"),s=document.getElementById("meta-report-share-editor");o&&(o.hidden=metaReportsState.clientShareExpanded),s&&(s.hidden=!metaReportsState.clientShareExpanded),syncMetaReportControlPanelUi()}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const a=document.getElementById(`${t}-fee-summary`);if(!a)return;const n=document.createElement("div");n.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const o=n.firstElementChild;o&&a.replaceWith(o);const s=document.getElementById(`${t}-fee-edit`);s&&(s.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:a=!1,variant:n="default",collapsible:o=!1,externalSummary:s=!1}={}){const r=!!e.metaReportShowBottomline,i=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",c=resolveMetaReportFeeMode(e),d=e.metaReportFeePercent??20,u=e.metaReportMarketingFeeAmount??0,p=metaReportUsesInlineFeeValue(t),v=`data-fee-mode="${esc(l)}" data-fee-enabled="${i?"true":"false"}" data-fee-percent="${esc(d)}" data-marketing-fee="${esc(u)}"`;if(n==="cv"){const y=describeMetaReportFeeModeLabel(c,d,u),w=META_REPORT_FEE_MODE_META.map(({value:$,title:S})=>`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${r?"":" is-hidden"}" id="${t}-fee-nested" ${v}>
  ...
  `}const f=describeMetaReportFeeModeLabel(c,d,u),b=META_REPORT_FEE_MODE_META.map(({value:y,icon:w,title:$,desc:S})=>`
        <label class="meta-report-fee-mode-card">
          ...
          <span class="meta-report-fee-mode-card-text">
            <span class="meta-report-fee-mode-card-title">${esc($)}</span>
            <small>${esc(S)}</small>
          </span>
          <span class="meta-report-fee-mode-card-radio" aria-hidden="true">${ICON_CHECK}</span>
        </label>`).join(""),h=o?metaReportsState.clientReportSettingsExpanded:!0,m=o?`
    ...
    <div class="meta-report-bottomline-fee-settings${o?" meta-report-bottomline-fee-settings--client":""}${o?h?" is-editing":" is-collapsed":""}">
      ${g}
      <div class="meta-report-bottomline-editor${o&&!h?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${r?"":" is-hidden"}" id="${t}-fee-nested" ${v}>
          <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
            <span class="meta-report-fee-status-value is-active">${esc(f)}</span>
          </div>
          <div class="meta-report-fee-mode-grid${a?" meta-report-fee-mode-grid--rows":""}">${b}
          </div>
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,a=(e.clients||[]).find(R=>R.clientId===t.selectedClientId),n=t.selectedMonthKey;if(!a||!n)return`
    ...
    `;const o=a.months?.[n]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(a,n,o);const s=t.editorPayload?.months?.[n]||null,r=a.metaReportShowBottomline,i=!!a.metaReportGhlDataEnabled,l=o.toplineSource||s?.dataSource||"meta",c=resolveMetaToplineUiMode(s||{dataSource:l},t.toplineUiMode),d=i&&c==="cenhub"&&l==="ghl",u=i&&c==="cenhub"&&l!=="ghl",p=o.wonLeads!=null?o.wonLeads:s?.inputs?.wonLeads??s?.topline?.wonLeads??"",v=o.avgLeadValue!=null?o.avgLeadValue:s?.inputs?.avgLeadValue??s?.topline?.avgLeadValue??"",f=o.avgProfitPerWon!=null?o.avgProfitPerWon:s?.inputs?.avgProfitPerWon??"",b=o.ghlLeads??s?.ghlTotals?.leads??"",h=o.ghlTotalRevenue??s?.ghlTotals?.totalRevenue??"",m=o.ghlTotalProfit??s?.ghlTotals?.totalProfit??"",g=metaReportsState.customValues.draftInputs,y=g?.wonLeads??p,w=g?.avgLeadValue??v,$=g?.avgProfitPerWon??f,S=g?.manualLeads??o.manualLeads??"",I=d?" readonly":"",A=d?" disabled":"",L=t.settingsSaving&&t.settingsSavingScope==="report",k=i?renderMetaToplineModeSection({prefix:"meta-cv",monthKey:n,monthPayload:s||{dataSource:l,ghlTotals:{leads:b!==""?b:null,totalRevenue:h!==""?h:null,totalProfit:m!==""?m:null},ghlSyncedAt:o.ghlSyncedAt||null},settings:{metaReportGhlDataEnabled:!0,hasGhl:a.hasGhl!==!1},saving:t.saving,uiPreference:t.toplineUiMode}):"",M=u?`
  ...
  `:"";return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      ${M}
      <label>Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(y)}"${I} />
      </label>
      ...
      <label>Avg profit per won${r?"":" (optional)"}
        <input type="number" step="any" id="meta-cv-avg-profit" value="${esc($)}"${I} />
      </label>
    </div>
    ${k}
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(o){showToast(o.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(a){showToast(a.message,"error")}finally{if(t){t.disabled=!1;const a=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${a}`:a}}}function getMetricsModelLabels(e,t=setupPipelines){const a=new Map((t||[]).map(n=>[n.id,n.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${a.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),a=!e.metricsModelSetAt||metricsModelChangeMode,n=a?" is-wizard":" is-locked",o=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",s=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",r=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return a?`
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:a=!1,forceFresh:n=!1}=t,o=!!n,s=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const r=!!document.getElementById("dashboard-content"),i=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const c=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!r)s.innerHTML=`
        ...
        `)}`;else if(!a){const d=document.getElementById("dashboard-content");d&&(d.innerHTML=renderKpiSkeleton()),i&&(i.disabled=!0,i.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let d=null;if(!availablePipelines.length){if(d=await bootstrapDashboardData(c),l!==fetchGeneration)return;availablePipelines=d.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,d.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const p=canReuseBootstrapDashboard(d)&&!o;let v,f;if(p?[f,v]=await Promise.all([fetchFacebookMetrics(c),Promise.resolve(d)]):[v,f]=await Promise.all([fetchDashboardData(availablePipelines,c,{forceFresh:o}),fetchFacebookMetrics(c)]),l!==fetchGeneration)return;cachedFacebookMetrics=f,cachedData=applyMarketingData(v,f),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(d){if(l!==fetchGeneration)return;if(d.name==="AbortError"){r?restoreDashboardContentAfterFailedFetch(r):s.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
