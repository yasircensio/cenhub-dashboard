(function(){const t=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);if(t[0]==="login"){document.body.dataset.dashboardMode="login",delete document.body.dataset.clientSlug;return}if(t[0]==="team"){document.body.dataset.dashboardMode="team",delete document.body.dataset.clientSlug;return}if(t[0]==="admin"){if(t[1]==="sync-history"&&(t[2]==="ghl"||t[2]==="meta")){document.body.dataset.dashboardMode=`sync-history-${t[2]}`,delete document.body.dataset.clientSlug;return}if(t[1]==="fb-lead-sync"){document.body.dataset.dashboardMode="fb-lead-sync",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]==="custom-values"){document.body.dataset.dashboardMode="meta-reports-custom",delete document.body.dataset.clientSlug;return}if(t[1]==="meta-reports"&&t[2]){document.body.dataset.dashboardMode="meta-reports-client",document.body.dataset.clientSlug=t[2];return}if(t[1]==="meta-reports"){document.body.dataset.dashboardMode="meta-reports",delete document.body.dataset.clientSlug;return}document.body.dataset.dashboardMode=t.length>=2?"admin":"hub",t.length>=2?document.body.dataset.clientSlug=t[1]:delete document.body.dataset.clientSlug;return}if(t[0]==="report"&&t[1]){document.body.dataset.dashboardMode="report",document.body.dataset.reportToken=t[1],delete document.body.dataset.clientSlug;return}t[0]&&t[0]!=="index.html"&&(document.body.dataset.dashboardMode="client",document.body.dataset.clientSlug=t[0])})();const DASHBOARD_MODE=document.body.dataset.dashboardMode||"client",IS_LOGIN_PAGE=DASHBOARD_MODE==="login",IS_TEAM_PAGE=DASHBOARD_MODE==="team",IS_ADMIN_HUB=DASHBOARD_MODE==="hub",IS_ADMIN_CLIENT=DASHBOARD_MODE==="admin",IS_ADMIN_SYNC_HISTORY_GHL=DASHBOARD_MODE==="sync-history-ghl",IS_ADMIN_SYNC_HISTORY_META=DASHBOARD_MODE==="sync-history-meta",IS_ADMIN_FB_LEAD_SYNC=DASHBOARD_MODE==="fb-lead-sync",IS_ADMIN_META_REPORTS=DASHBOARD_MODE==="meta-reports",IS_ADMIN_META_REPORTS_CLIENT=DASHBOARD_MODE==="meta-reports-client",IS_ADMIN_META_REPORTS_CUSTOM=DASHBOARD_MODE==="meta-reports-custom",IS_REPORT_VIEW=DASHBOARD_MODE==="report",REPORT_TOKEN=document.body.dataset.reportToken||"",IS_ADMIN_SYNC_HISTORY=IS_ADMIN_SYNC_HISTORY_GHL||IS_ADMIN_SYNC_HISTORY_META,IS_CLIENT_VIEW=DASHBOARD_MODE==="client",IS_PREVIEW=IS_CLIENT_VIEW&&!!new URLSearchParams(window.location.search).get("client"),IS_ADMIN=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||new URLSearchParams(window.location.search).get("view")==="admin",ADMIN_UI=IS_ADMIN_HUB||IS_ADMIN_CLIENT||IS_TEAM_PAGE||IS_ADMIN_SYNC_HISTORY||IS_ADMIN_FB_LEAD_SYNC||IS_ADMIN_META_REPORTS||IS_ADMIN_META_REPORTS_CLIENT||IS_ADMIN_META_REPORTS_CUSTOM||IS_ADMIN,LOADING_MSG=ADMIN_UI?"Loading dashboard data...":"Henter dashboard data...",RETRY_MSG=ADMIN_UI?"Try again":"Pr\xF8v igen";(function(){const t=document.getElementById("initial-loading-msg");t&&(t.textContent=LOADING_MSG)})();function resolveClientSlug(){if(document.body.dataset.clientSlug)return document.body.dataset.clientSlug;const e=window.location.pathname.replace(/^\/+|\/+$/g,"").split("/").filter(Boolean);return e[0]==="admin"&&e[1]==="meta-reports"&&e[2]&&e[2]!=="custom-values"?e[2]:e[0]==="admin"&&e[1]&&e[1]!=="meta-reports"?e[1]:e[0]&&e[0]!=="admin"&&e[0]!=="index.html"&&e[0]!=="report"?e[0]:new URLSearchParams(window.location.search).get("client")||"suntech-nordic"}const CLIENT_SLUG=resolveClientSlug();let tenantParams={},facebookClientId=CLIENT_SLUG,setupAccount=null,setupPipelines=[],metricsModelChangeMode=!1;function esc(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function renderBrandTopbar(e=""){const t=!!(e&&e.includes("staff-admin-chrome")),n=t||e&&e.includes("admin-topbar-link")?"/admin":"",a=n?`<a href="${n}" class="brand-logo-link" aria-label="Cenhub home"><img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" /></a>`:'<img class="brand-logo" src="/cenhub-logo-white.png" alt="Cenhub" width="167" height="41" />',s=t?`<button type="button" class="staff-nav-toggle" id="staff-nav-toggle" aria-expanded="false" aria-controls="staff-topbar-panel" aria-label="Open menu">
  ...
  `}function renderSyncHistoryRows(e,t){if(!e.length)return'<div class="sync-history-empty">No sync runs logged yet.</div>';const n=t==="meta"?"Spend / details":"Details",a=e.map(s=>{let o="\u2014";if(t==="meta"){const r=[];if(s.thisMonthSpend!=null&&r.push(`${fmtDkk(s.thisMonthSpend)} DKK this month`),s.spendDateStop){const i=formatSyncHistoryDate(s.spendDateStop);i&&r.push(`through ${i}`)}s.errorMessage&&r.push(s.errorMessage),o=r.join(" \xB7 ")||"\u2014"}else s.opportunityCount!=null?(o=`${s.opportunityCount} opportunities`,s.errorMessage&&(o+=` \xB7 ${s.errorMessage}`)):s.errorMessage&&(o=s.errorMessage);return`
      <tr>
...
${n}`))try{const s=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}`,{method:"DELETE"});showToast(`Cleared ${s.deleted||0} log entr${s.deleted===1?"y":"ies"}.`,"success"),await loadSyncHistoryPage(e)}catch(s){showToast(s.message||`Failed to clear ${t} sync log.`,"error")}}function renderSyncHistoryLoginPrompt(e){const t=e==="meta"?"Meta sync log":"GHL sync log";return`
  ...
  `}async function loadSyncHistoryPage(e,{silent:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){t||(n.innerHTML=renderSyncHistoryLoginPrompt(e));return}currentStaffUser=a,t||(n.innerHTML=`
    ...
    `);try{const s=await adminFetch(`/api/sync-history?type=${encodeURIComponent(e)}&limit=150`);n.innerHTML=renderSyncHistoryPage(e,s);const o=document.getElementById("sync-history-refresh");o&&(o.onclick=()=>loadSyncHistoryPage(e));const r=document.getElementById("sync-history-clear-log");r&&(r.onclick=()=>clearSyncHistoryLog(e)),syncHistoryRefreshTimer&&(clearInterval(syncHistoryRefreshTimer),syncHistoryRefreshTimer=null),syncHistoryRefreshTimer=window.setInterval(()=>{loadSyncHistoryPage(e,{silent:!0}).catch(()=>{})},6e4)}catch(s){n.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome(e==="meta"?"meta-sync":"ghl-sync"))}
...
${r} contact(s) will get Fb Lead id written in GHL. This writes live data.`)&&(fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:!0,previewRunId:n},await runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!1}))}async function refreshFbLeadClientRowsUi(){const e=document.getElementById("fb-lead-clients-mount"),t=document.getElementById("fb-lead-field-warnings-mount");e&&(e.innerHTML=renderFbLeadClientRows(fbLeadSyncState.clients)),t&&(t.innerHTML=renderFbLeadFieldWarnings(fbLeadSyncState.clients)),bindFbLeadSyncToggles()}function renderFbLeadClientActions(e){const t=esc(e.clientId),n=e.metaPageId?"":`<a class="admin-btn admin-btn--ghost admin-btn--small" href="/admin/${encodeURIComponent(e.clientId)}">Setup</a>`;return`
  ...
  `}function lastRunIsSuccessfulApply(e){if(!e||e.dryRun)return!1;const t=String(e.status||"").toLowerCase();return t==="success"||t==="ok"}function lastRunIsSuccessfulPreview(e){if(!e||!e.dryRun)return!1;const t=String(e.status||"").toLowerCase();return t==="success"||t==="ok"}function resolveFbLeadClientStats(e,t={}){const n=e?.displayStats||{},a=t.metaLeadCount90d??n.metaLeadCount90d??null,s=t.estimatedMissing??n.outstanding??null,o=t.estimatedMissing!=null?"Estimated contacts still missing Fb Lead id (from a Meta sample)":n.outstandingHint||"Estimated contacts still missing Fb Lead id (from a Meta sample)";return{metaLeads:a!=null?String(a):"\u2014",missing:s!=null?String(s):"\u2014",missingTitle:o}}function formatFbLeadLastRunLabel(e){if(!e)return"\u2014";const t=[e.dryRun?"preview":e.status,`${e.updated||0} ${e.dryRun?"would update":"updated"}`];return(e.skippedNoMatch??0)>0&&t.push(`${e.skippedNoMatch} no match`),t.join(" \xB7 ")}function renderFbLeadClientRows(e){return e.length?`
    <div class="sync-history-table-wrap">
      <table class="sync-history-table fb-lead-clients-table">
        ...
        <tbody>${e.map(n=>{const a=fbLeadSyncState.preflightByClient[n.clientId]||{},s=resolveFbLeadClientStats(n,a),o=formatFbLeadLastRunLabel(n.lastRun);return`
      <tr>
        <td>
  ...
  `}function renderFbLeadAuditTable(e,t){if(!e.length)return'<div class="sync-history-empty" style="padding:12px">No contact-level rows stored for this run.</div>';const n=e.map(s=>{const o=ghlContactUrl(t,s.contactId),r=o?`<a href="${esc(o)}" target="_blank" rel="noopener noreferrer">Open in GHL</a>`:"\u2014";return`
      <tr>
  ...
  `}function closeFbLeadRunPanel(){const e=document.getElementById("fb-lead-run-panel-mount");e&&(e.innerHTML=""),fbLeadSyncState.activeRun=null}function openFbLeadRunPanel(e,t){const n=fbLeadSyncState.clients.find(o=>o.clientId===e);if(!clientFbLeadFieldReady(n)){showToast(getClientFbLeadFieldHint(n),"error");return}const a=document.getElementById("fb-lead-run-panel-mount");if(!a)return;fbLeadSyncState.activeRun={clientId:e,mode:t,runId:null,previewOk:clientHasReadyPreview(fbLeadSyncState.clients.find(o=>o.clientId===e))},a.innerHTML=renderFbLeadRunPanel(e,t);const s=document.getElementById("fb-lead-preview-btn");s&&(s.onclick=()=>runFbLeadSyncBatch({clientId:e,mode:t,dryRun:!0})),a.scrollIntoView({behavior:"smooth",block:"nearest"})}function renderFbLeadRunResults(e){const t=e.rows||[];if(!t.length)return'<div class="sync-history-empty" style="padding:12px">No rows in this batch.</div>';const n=t.map(o=>`
    <tr>
  ...
  `).join(""),a=e.nextBatchOffset??(e.batchOffset||0)+(e.batchProcessed||0),s=e.inWindow||a;return`
    ...
    <p style="color:var(--text-soft);font-size:12px;margin-top:8px">
      ${a} / ${s} leads scanned \xB7 ${e.updated||0} would update/updated \xB7 ${e.errors||0} errors
    </p>
  `}function renderGlobalApplyProgress(e,t,n=0){const a=fbLeadSyncState.clients.find(r=>r.clientId===e),s=t==="backfill"?"90-day backfill":"recent sync",o=n>0?`${n} contact(s)`:"contacts";return`
    <div class="fb-lead-apply-progress-card" id="fb-lead-global-progress-card">
      <div class="fb-lead-apply-progress-header">
        <div>
          ...
          <div class="fb-lead-apply-progress-sub">${esc(a?.accountName||e)} \xB7 ${esc(o)}</div>
        </div>
  ...
  `}function startGlobalApplyProgress(e,t,n=0){const a=document.getElementById("fb-lead-apply-progress-mount");a&&(fbLeadSyncState.isApplying=!0,fbLeadSyncState.applyStartedAt=Date.now(),fbLeadSyncState.applyProgress=null,a.innerHTML=renderGlobalApplyProgress(e,t,n),a.scrollIntoView({behavior:"smooth",block:"nearest"}),fbLeadSyncState.applyWaitTimer&&clearInterval(fbLeadSyncState.applyWaitTimer),fbLeadSyncState.applyWaitTimer=window.setInterval(()=>{if(!fbLeadSyncState.isApplying)return;const s=Math.floor((Date.now()-fbLeadSyncState.applyStartedAt)/1e3),o=fbLeadSyncState.applyProgress;o?.total>0?updateGlobalApplyProgress({...o,waiting:!0,elapsed:s}):updateGlobalApplyProgress({waiting:!0,elapsed:s})},1e3))}function updateGlobalApplyProgress({processed:e,total:t,written:n,waiting:a=!1,elapsed:s=0,complete:o=!1,error:r=""}={}){const i=document.getElementById("fb-lead-global-progress-card"),l=document.getElementById("fb-lead-global-progress"),d=document.getElementById("fb-lead-global-progress-bar"),c=document.getElementById("fb-lead-global-progress-label"),u=document.getElementById("fb-lead-global-progress-stats"),m=document.getElementById("fb-lead-apply-status-badge");if(!i||!l||!d||!c)return;const b=fbLeadSyncState.applyProgress||{},p=e??b.processed??0,S=t??b.total??0,g=n??b.written??0;!o&&!r&&(S>0||p>0)&&(fbLeadSyncState.applyProgress={processed:p,total:S,written:g});const f=S>0||p>0;if(a&&!f&&!o){l.classList.add("is-indeterminate"),d.style.width="35%",u&&(u.textContent="Contacting Meta and GHL\u2026"),c.textContent=`Writing to GHL\u2026 (${s}s \u2014 do not close this page)`;return}l.classList.remove("is-indeterminate");const v=S>0?Math.min(100,Math.round(p/S*100)):o?100:0;d.style.width=`${v}%`,u&&(u.textContent=o?`${g} written to GHL \xB7 ${p} / ${S} contacts processed`:`${g} written \xB7 ${p} / ${S} contacts (${v}%)`),o?(i.classList.add("is-complete"),i.classList.remove("is-error"),c.textContent=`Complete \u2014 ${g} contact(s) updated in GHL`,m&&(m.textContent="Complete",m.classList.add("is-ok"),m.classList.remove("is-missing"))):r?(i.classList.add("is-error"),i.classList.remove("is-complete"),c.textContent=r,m&&(m.textContent="Failed",m.classList.remove("is-ok"),m.classList.add("is-missing"))):a&&f?c.textContent=p>=S?`Finishing up\u2026 (${s}s)`:`Batch in progress \u2014 ${p} / ${S} leads (${v}%) \xB7 ${s}s`:c.textContent=`Writing to GHL \u2014 ${p} / ${S} leads (${v}%)`}function stopGlobalApplyProgress({success:e=!0,keepVisibleMs:t=6e3}={}){fbLeadSyncState.isApplying=!1,fbLeadSyncState.applyProgress=null,fbLeadSyncState.applyWaitTimer&&(clearInterval(fbLeadSyncState.applyWaitTimer),fbLeadSyncState.applyWaitTimer=null),window.setTimeout(()=>{const n=document.getElementById("fb-lead-apply-progress-mount");n&&!fbLeadSyncState.isApplying&&(n.innerHTML="")},t)}function setFbLeadProgress({processed:e=0,total:t=0,complete:n=!1,label:a=""}={}){const s=document.getElementById("fb-lead-progress-wrap"),o=document.getElementById("fb-lead-progress-bar"),r=document.getElementById("fb-lead-progress-label");if(!s||!o)return;s.hidden=!1,s.classList.toggle("is-complete",n);const i=t>0?Math.min(100,Math.round(e/t*100)):n?100:0;o.style.width=`${i}%`,r&&(r.textContent=a||(n?`Complete \u2014 ${e} / ${t} leads (${i}%)`:`Processing \u2014 ${e} / ${t} leads (${i}%)`))}function hideFbLeadProgress(e=!0){const t=document.getElementById("fb-lead-progress-wrap"),n=document.getElementById("fb-lead-progress-bar");t&&e&&(t.hidden=!0,t.classList.remove("is-complete"),n&&(n.style.width="0%"))}function updateFbLeadHistoryClearButton(){const e=document.getElementById("fb-lead-history-clear-all");if(!e)return;const t=(fbLeadSyncState.historyRuns||[]).length>0;e.disabled=!t}function updateFbLeadCronSummaryUi(){const e=document.getElementById("fb-lead-cron-summary-24h");!e||!fbLeadSyncState.cronSummary24h||(e.outerHTML=renderFbLeadCronSummary24h(fbLeadSyncState.cronSummary24h))}async function refreshFbLeadHistoryUi(){await loadFbLeadHistory();const e=document.getElementById("fb-lead-history-mount");if(!e)return;const t=Object.fromEntries(fbLeadSyncState.clients.map(n=>[n.clientId,n.locationId]));e.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,t),updateFbLeadHistoryClearButton()}function bindFbLeadHistoryClearButton(){const e=document.getElementById("fb-lead-history-clear-all");e&&(e.onclick=()=>clearFbLeadSyncHistory());const t=document.getElementById("fb-lead-show-routine-cron");t&&(t.checked=fbLeadSyncState.showRoutineCronRuns,t.onchange=()=>{fbLeadSyncState.showRoutineCronRuns=t.checked;const n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=renderFbLeadHistoryRows(fbLeadSyncState.historyRuns,a)}),updateFbLeadHistoryClearButton()}async function clearFbLeadSyncHistory(){if(!(!isStaffAdmin()||!window.confirm(`Delete all FB lead sync run history?

This only clears the log \u2014 it does not change GHL contacts or Meta data.`)))try{const t=await adminFetch("/api/fb-lead-sync/history",{method:"DELETE"});showToast(`Cleared ${t.deleted||0} run(s).`,"success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(t){showToast(t.message||"Failed to clear FB lead sync log.","error")}}async function deleteFbLeadRun(e){if(!(!isStaffAdmin()||!window.confirm(`Delete run #${e} from history?`)))try{await adminFetch(`/api/fb-lead-sync/history/${e}`,{method:"DELETE"}),showToast("Run deleted.","success"),await refreshFbLeadHistoryUi(),await loadFbLeadSyncPage({silent:!0})}catch(n){showToast(n.message||"Failed to delete run.","error")}}async function runFbLeadSyncBatch({clientId:e,mode:t,dryRun:n,resumeRunId:a=null}){const s=document.getElementById("fb-lead-preview-btn"),o=document.getElementById("fb-lead-run-results");s&&(s.disabled=!0);const r=n?12:10,i=a!=null?fbLeadSyncState.historyRuns.find(g=>Number(g.id)===Number(a)):null;let l=i?.id||fbLeadSyncState.activeRun?.runId||null,d=i&&Number(i.batchOffset)||0,c=null,u=[],m=!1;const b=a??fbLeadSyncState.activeRun?.previewRunId??null,p=b!=null?fbLeadSyncState.historyRuns.find(g=>Number(g.id)===Number(b)):null,S=i?partialSyncRemaining(i):p?.updated??fbLeadSyncState.previewOkByClient[e]?.updated??0;n||(fbLeadSyncState.applyingRunId=b,startGlobalApplyProgress(e,t,S),await refreshFbLeadHistoryUi());try{if(!a){const w=findPartialSyncRun(e,t,n);if(w){const h=partialSyncRemaining(w),$=n?"preview":"apply";window.confirm(`A ${$} for ${e} was interrupted (${w.updated||0} processed, ${h} remaining).

Resume the interrupted ${$}?`)&&(l=w.id,d=Number(w.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const w=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),h=w.summary||w;l=h.runId,d=0,setFbLeadProgress({processed:0,total:h.inWindow||0,label:`Matching ${h.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:S||p?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:b,batchOffset:d,batchLimit:r})});const w=c.summary||c;l=w.runId,d=w.nextBatchOffset??w.batchOffset+(w.batchProcessed||0),u=u.concat(w.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const h=w.inWindow||d||S;n?(setFbLeadProgress({processed:d,total:h,complete:!w.hasMore}),o&&(o.innerHTML=renderFbLeadRunResults({...w,rows:u}))):updateGlobalApplyProgress({processed:d,total:h,written:w.updated||0,complete:!w.hasMore})}while((c.summary||c).hasMore);m=!0;const g=c.summary||c,f=g.inWindow||d||S,v=g.nextBatchOffset??d;if(n)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,g.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(g.updated||0)>0),showToast((g.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:g.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const w=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete w[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(w))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const y=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=y.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(g){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${g.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((g.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!m&&n&&hideFbLeadProgress(!0),!n&&!m&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(o=>o.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
        ${renderBrandTopbar("")}
        ${wrapDashboardShell(`
  ...
  `).join("")}function getMetaReportCalendarYear(e){const t=Number(e?.currentYear);return Number.isFinite(t)&&t>=2e3?t:new Date().getFullYear()}function getMetaReportYearOptions(e){const t=getMetaReportCalendarYear(e),n=t-1,a=new Map;Array.isArray(e?.years)&&e.years.forEach(o=>{const r=Number(o?.year);Number.isFinite(r)&&a.set(r,o.available!==!1)});const s=a.has(n)?a.get(n):e?.previousYearHasData===!0;return[{year:t,available:!0},{year:n,available:!!s}]}function renderMetaReportYearSelectOptions(e,{disableUnavailable:t=!1}={}){const n=Number(metaReportsState.selectedYear)||Number(e?.year);return getMetaReportYearOptions(e).map(({year:a,available:s})=>{const o=t&&!s;return`<option value="${a}"${Number(n)===a?" selected":""}${o?" disabled":""}>${a}${s?"":" (no data)"}</option>`}).join("")}function syncMetaReportSelectedYear(e,{disableUnavailable:t=!1}={}){const n=getMetaReportYearOptions(e),a=Number(metaReportsState.selectedYear),s=n.find(l=>l.year===a);if(s&&(!t||s.available)){metaReportsState.selectedYear=s.year;return}const o=Number(e?.year),r=n.find(l=>l.year===o);if(r&&(!t||r.available)){metaReportsState.selectedYear=r.year;return}const i=n.find(l=>l.available)||n[0];i&&(metaReportsState.selectedYear=i.year)}function refreshMetaReportMonthPanel(e){const t=metaReportsState.clientPayload;!t||!metaReportsState.activeMonthKey||(t.months=t.months||{},t.months[metaReportsState.activeMonthKey]=e,!document.getElementById("meta-report-month-panel"))||(syncMetaReportMonthPanelDom(e,getMetaReportMonthBodyOptions(t.settings,{editable:!0}),t),bindMetaReportsClientEditEvents(t.clientId,t),refreshMetaReportBackfillControl(t))}function refreshMetaReportBackfillControl(e){const t=document.querySelector(".meta-report-client-page .meta-report-toolbar-left"),n=document.querySelector(".meta-report-backfill-wrap"),a=metaReportMonthsNeedingBackfill(e),s=a.length?`${ICON_SYNC} Backfill ${a.length} month${a.length===1?"":"s"} from Meta`:`${ICON_SYNC} Re-sync year from Meta`,o=document.getElementById("meta-report-backfill");if(o)o.innerHTML=s,o.disabled=!1;else if(t){const r=document.createElement("div");r.className="meta-report-backfill-wrap",r.innerHTML=`
      <button type="button" class="admin-btn admin-btn--secondary admin-btn--small" id="meta-report-backfill">${s}</button>
      <span class="meta-report-backfill-progress" id="meta-report-backfill-progress"></span>
    `,t.appendChild(r),bindMetaReportBackfillButton(e.clientId)}n&&!o&&!t&&n.remove()}function updateMetaReportsClientContent(e){metaReportsState.clientPayload=e;const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null),syncMetaReportSelectedYear(e,{disableUnavailable:!0});const n=document.getElementById("meta-report-year");n&&(n.innerHTML=renderMetaReportYearSelectOptions(e,{disableUnavailable:!0})),syncMetaReportToolbarCopy(e),refreshMetaReportClientShareUi(e);const a=e.settings||{},s=e.months?.[metaReportsState.activeMonthKey]||null;syncMetaReportSettingsControls(a,s);const o=document.querySelector(".meta-report-tabs");if(o&&(o.innerHTML=renderMetaReportsClientTabs(e),bindMetaReportsClientTabEvents(e)),document.getElementById("meta-report-month-panel")&&(syncMetaReportMonthPanelDom(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(a,{editable:!0}),e),bindMetaReportsClientEditEvents(e.clientId,e)),refreshMetaReportBackfillControl(e),metaReportsState.chartSeries?.length){const i=buildMetaReportScenarioProjection(metaReportsState.chartSeries,e);metaReportsState.chartProjection=i,syncMetaReportBaselineUi(metaReportsState.chartSeries,i,e)}}function updatePublicMetaReportContent(e){metaReportsState.publicPayload=e,syncMetaReportPublicBranding(e);const t=e.monthKeys||[];metaReportsState.activeMonthKey&&t.includes(metaReportsState.activeMonthKey)||(metaReportsState.activeMonthKey=t[t.length-1]||null),syncMetaReportSelectedYear(e,{disableUnavailable:!0});const n=document.getElementById("meta-report-year");n&&(n.innerHTML=renderMetaReportYearSelectOptions(e,{disableUnavailable:!0}));const a=document.querySelector(".meta-report-tabs");a&&(a.innerHTML=t.map(o=>`
    ...
    `).join(""),bindPublicMetaReportTabEvents(e)),document.getElementById("meta-report-month-panel")&&syncMetaReportMonthPanelDom(e.months?.[metaReportsState.activeMonthKey],getMetaReportMonthBodyOptions(e.settings,{editable:!1}),e)}const META_MONTH_LABELS=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];function metaFmtKr(e){return`Dkr ${(Number(e)||0).toLocaleString("da-DK",{minimumFractionDigits:0,maximumFractionDigits:2})}`}function metaFmtNum(e,t=0){return(Number(e)||0).toLocaleString("da-DK",{minimumFractionDigits:t,maximumFractionDigits:t})}function metaReportFmtBaselineAmount(e){const t=Number(e)||0;return t<=0?"\u2014":metaFmtKr(t)}function renderMetaReportBaselinePill(e,t,{active:n=!1,title:a=""}={}){return`<span class="meta-report-baseline-pill${n?" is-active":""}"${a?` title="${esc(a)}"`:""}><span class="meta-report-baseline-pill-label">${esc(e)}</span><span class="meta-report-baseline-pill-value">${esc(metaReportFmtBaselineAmount(t))}</span></span>`}function metaFmtX(e){const t=Number(e);return Number.isFinite(t)?t===0?"0.00x":`${t.toFixed(2)}x`:"\u2014"}function metaMonthLabel(e){const t=Number(String(e||"").slice(5,7));return META_MONTH_LABELS[t-1]||e}function metaReportMonthBounds(e){const t=String(e||"").trim();if(!/^\d{4}-\d{2}$/.test(t))return{start:"",end:""};const[n,a]=t.split("-").map(Number),s=new Date(n,a,0).getDate(),o=String(a).padStart(2,"0");return{start:`${n}-${o}-01`,end:`${n}-${o}-${String(s).padStart(2,"0")}`}}function metaReportMonthPeriodAligned(e){if(!e?.monthKey)return!0;const t=metaReportMonthBounds(e.monthKey);return e.periodStart===t.start&&e.periodEnd===t.end}function resolveMetaReportActiveMonthPayload(e=null){const t=e||metaReportsState.clientPayload||metaReportsState.publicPayload,n=metaReportsState.activeMonthKey;return!t||!n?null:t.months?.[n]||null}function switchMetaReportMonthTab(e,{editable:t=!1}={}){if(!e)return;metaReportsState.activeMonthKey=e;const n=metaReportsState.clientPayload||metaReportsState.publicPayload;if(!n)return;document.querySelectorAll("[data-meta-month-tab]").forEach(s=>{s.classList.toggle("is-active",s.getAttribute("data-meta-month-tab")===e)});const{series:a}=resolveMetaReportChartSeries(n);a.length&&(metaReportsState.chartSeries=a),(async()=>{const s=n.months?.[e];if(t&&n.clientId&&s&&!metaReportMonthPeriodAligned(s))try{const o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(n.clientId)}/months/${encodeURIComponent(e)}`);o?.monthPayload&&(n.months[e]=o.monthPayload)}catch{}if(syncMetaReportMonthPanelDom(null,getMetaReportMonthBodyOptions(n.settings||{},{editable:t,yearPayload:n}),n),t&&n.clientId&&bindMetaReportsClientEditEvents(n.clientId,n),syncMetaReportSettingsControls(n.settings||{},resolveMetaReportActiveMonthPayload(n)),metaReportsState.chartSeries?.length){const o=buildMetaReportScenarioProjection(metaReportsState.chartSeries,n);metaReportsState.chartProjection=o,syncMetaReportBaselineUi(metaReportsState.chartSeries,o,n)}})()}function metaReportMonthsNeedingBackfill(e){return(e?.monthKeys||[]).filter(n=>!e.months?.[n]?.metaFetchedAt)}function metaReportTone(e){const t=Number(e);return!Number.isFinite(t)||t===0?"neutral":t>0?"positive":"negative"}function renderMetaReportHighlightStrip(e){if(!e||e.meta?.emptyMonth)return"";const t=Number(e.topline?.roasX)||0,n=[{label:"Ad spend",value:metaFmtKr(e.meta?.spend),tone:"neutral"},{label:"Leads",value:metaFmtNum(e.topline?.leads),tone:"neutral"},{label:"Return on ad spend",value:metaFmtX(t),tone:metaReportTone(t)}];if(e.bottomline){const a=Number(e.bottomline.feePercent)>0,s=a?e.bottomline.poiKr:e.bottomline.poasKr;n.push({label:a?"Profit on investment":"Profit on ad spend",value:metaFmtKr(s),tone:metaReportTone(s)})}else n.push({label:"Won leads",value:metaFmtNum(e.topline?.wonLeads),tone:"neutral"});return`
    <div class="meta-report-highlight-strip">
      ${n.map(({label:a,value:s,tone:o})=>`
  ...
  `}function renderMetaReportMetricTable(e,t,n="meta",{badge:a="",highlightLastN:s=0}={}){const o=t.map(([r,i,l],d)=>`
      <tr class="${s>0&&d>=t.length-s?`is-highlight accent-${n}`:""}">
  ...
  `}function getMetaReportMonthBodyOptions(e={},{editable:t=!1,yearPayload:n=null}={}){return{editable:t,yearPayload:n}}function shouldShowMetaReportYearVisuals(e,t,{editable:n=!1}={}){if(!e||e.meta?.emptyMonth||!t)return!1;const{series:a,demo:s}=resolveMetaReportChartSeries(t);return!(a.length<META_REPORT_CHART_MIN_POINTS||s&&!n)}const META_REPORT_CHART_MIN_POINTS=2,META_REPORT_SCENARIO_MAX_HISTORICAL=9,META_REPORT_SCENARIO_PROJECTED_MONTHS=4,META_REPORT_SPEND_CHART_TYPE_OPTIONS=[{value:"area",label:"Area"},{value:"bar",label:"Bar"}],META_REPORT_SCENARIO_MONTH_WINDOW_OPTIONS=[{value:"3",label:"Last 3 ad months"},{value:"6",label:"Last 6 ad months"},{value:"9",label:"Last 9 ad months"},{value:"12",label:"Last 12 ad months"},{value:"all",label:"All ad-active months"}],META_REPORT_SCENARIO_FIXED_ELASTICITY=.8,META_SCENARIO_TREND_R_THRESHOLD=.35,META_REPORT_SCENARIO_MODEL_PILLS=[{id:"scenarioSmoothUneven",label:"Remove uneven months",defaultOn:!0},{id:"scenarioBlendHistory",label:"Balance recent months",defaultOn:!1},{id:"scenarioIncludeTrend",label:"Follow trend",defaultOn:!1},{id:"scenarioCautionStrongMonths",label:"Cap hot streak",defaultOn:!1,requires:"scenarioIncludeTrend"}];function normalizeMetaReportSpendChartType(e){const t=String(e||"area").trim().toLowerCase();return META_REPORT_SPEND_CHART_TYPE_OPTIONS.some(n=>n.value===t)?t:"area"}function hydrateMetaReportSpendChartType(e){metaReportsState.spendChartType=normalizeMetaReportSpendChartType(e?.settings?.metaReportSpendChartType)}function normalizeMetaReportScenarioPill(e,t=!1){if(typeof e=="boolean")return e;if(e==null)return t;const n=String(e).trim().toLowerCase();return n==="true"||n==="1"?!0:n==="false"||n==="0"?!1:t}function normalizeMetaReportScenarioMonthWindow(e){const t=String(e||"6").trim().toLowerCase();return META_REPORT_SCENARIO_MONTH_WINDOW_OPTIONS.some(n=>n.value===t)?t:"6"}function normalizeMetaReportBudgetBaseline(e){const t=String(e||"year").trim().toLowerCase();return t==="month"||t==="last"?t:"year"}function normalizeMetaReportBudgetMultiplier(e){const t=Number.parseFloat(e);return Number.isFinite(t)?Math.min(3,Math.max(.5,Math.round(t*10)/10)):2}function getMetaReportScenarioPillsFromState(){return{smoothUneven:!!metaReportsState.scenarioSmoothUneven,blendHistory:!!metaReportsState.scenarioBlendHistory,includeTrend:!!metaReportsState.scenarioIncludeTrend,cautionStrongMonths:!!metaReportsState.scenarioCautionStrongMonths}}function applyMetaReportScenarioPillsToState(e={}){metaReportsState.scenarioSmoothUneven=!!e.smoothUneven,metaReportsState.scenarioBlendHistory=!!e.blendHistory,metaReportsState.scenarioIncludeTrend=!!e.includeTrend,metaReportsState.scenarioCautionStrongMonths=!!e.cautionStrongMonths}function describeActiveMetaReportScenarioPills(){return META_REPORT_SCENARIO_MODEL_PILLS.filter(e=>!!metaReportsState[e.id]&&(!e.requires||!!metaReportsState[e.requires])).map(e=>e.label)}function hydrateMetaReportScenarioSettings(e){const t=e?.settings||{};metaReportsState.budgetMultiplier=normalizeMetaReportBudgetMultiplier(t.metaReportBudgetMultiplier),metaReportsState.budgetBaseline=normalizeMetaReportBudgetBaseline(t.metaReportBudgetBaseline),metaReportsState.scenarioMonthWindow=normalizeMetaReportScenarioMonthWindow(t.metaReportScenarioMonthWindow),applyMetaReportScenarioPillsToState({smoothUneven:normalizeMetaReportScenarioPill(t.metaReportScenarioSmoothUneven,!0),blendHistory:normalizeMetaReportScenarioPill(t.metaReportScenarioBlendHistory,!1),includeTrend:normalizeMetaReportScenarioPill(t.metaReportScenarioIncludeTrend,!1),cautionStrongMonths:normalizeMetaReportScenarioPill(t.metaReportScenarioCautionStrongMonths,!1)})}function renderMetaReportScenarioModelPills(){return`
    <div class="meta-report-scenario-pill-section" id="meta-report-scenario-pills-wrap">
      <div class="meta-report-scenario-pill-row" role="group" aria-label="Forecast options">
        ${META_REPORT_SCENARIO_MODEL_PILLS.map(({id:e,label:t,requires:n})=>{const a=!!metaReportsState[e],s=n?!metaReportsState[n]:!1,o=s?'Turn on "Follow trend" first':"";return`
            <button type="button" class="meta-report-scenario-pill${a&&!s?" is-active":""}${s?" is-disabled":""}" data-scenario-pill="${esc(e)}" aria-pressed="${a&&!s}"${s?" disabled":""}${o?` title="${esc(o)}"`:""}>${esc(t)}</button>
          `}).join("")}
      </div>
    </div>
  ...
  `}function isMetaReportCensioClient(e){const t=String(e?.clientId||"").toLowerCase(),n=String(e?.accountName||"");return t==="censio"||/censio/i.test(n)}function monthHasChartData(e){if(!e||e.meta?.emptyMonth)return!1;if(!metaReportMonthPeriodAligned(e))return(Number(e.topline?.leads)||0)>0;const t=Number(e.meta?.spend)||0,n=Number(e.topline?.leads)||0;return t>0||n>0}function buildMetaReportChartSeries(e){const t=e?.monthKeys||[],n=e?.months||{},a=[];for(const s of t){const o=n[s];if(!o||o.meta?.emptyMonth||!monthHasChartData(o))continue;const r=Number(o.topline?.roasKr)||0,i=Number(o.topline?.roasX)||0,l=!!o.bottomline,d=l?Number(o.bottomline.poasKr)||0:null,c=l?Number(o.bottomline.poasX)||0:null;a.push({monthKey:s,label:metaMonthLabel(s),periodEnd:o.periodEnd||null,roasKr:r,roasX:i,poasKr:d,poasX:c,spend:Number(o.meta?.spend)||0,leads:Number(o.topline?.leads)||0,wonLeads:Number(o.topline?.wonLeads)||0,avgLeadValue:Number(o.topline?.avgLeadValue)||0,avgProfitPerWon:Number(o.inputs?.avgProfitPerWon)||0,hasData:!0})}return a}function getMetaReportDemoSeries(e,t,n,a){if(!isMetaReportCensioClient({clientId:e,accountName:t}))return null;const o=new Date,r=o.getFullYear(),i=o.getMonth()+1,l=Number(n)||r;let d=(a||[]).filter(c=>{const u=Number(String(c).slice(0,4)),m=Number(String(c).slice(5,7));return!(u!==l||l===r&&m>i)});if(!d.length){const c=l===r?Math.min(8,i):8;d=Array.from({length:c},(u,m)=>`${l}-${String(m+1).padStart(2,"0")}`)}return d.map((c,u)=>{const m=d.length>1?u/(d.length-1):0,b=Math.round(8e3+m*1e4),p=Math.round(2e3+m*15e3),S=Number((.5+m*2.5).toFixed(2)),g=Math.round(1500+m*12e3),f=Number((.4+m*2.2).toFixed(2)),v=Math.round(20+m*80),y=Math.max(1,Math.round(v*(.08+m*.04)));return{monthKey:c,label:metaMonthLabel(c),roasKr:p,roasX:S,poasKr:g,poasX:f,spend:b,leads:v,wonLeads:y,avgLeadValue:85e3,avgProfitPerWon:42e3,hasData:!0,demo:!0}})}function resolveMetaReportChartSeries(e){const t=buildMetaReportChartSeries(e);if(t.length>=META_REPORT_CHART_MIN_POINTS)return{series:t,demo:!1};if(isMetaReportCensioClient(e)){const n=getMetaReportDemoSeries(e.clientId,e.accountName,e.year||metaReportsState.selectedYear,e.monthKeys);if(n&&n.length>=META_REPORT_CHART_MIN_POINTS)return{series:n,demo:!0}}return{series:t,demo:!1}}function metaReportParseAmount(e){const t=Number.parseFloat(e);return Number.isFinite(t)?t:0}function metaReportRoundMoney(e){return Math.round(metaReportParseAmount(e)*100)/100}function metaReportRoundRatio(e){return Math.round(metaReportParseAmount(e)*1e8)/1e8}function metaReportAggregateSeriesEfficiency(e=[]){let t=0,n=0,a=0,s=0,o=0;for(const i of e){const l=metaReportParseAmount(i.spend),d=metaReportParseAmount(i.leads),c=metaReportParseAmount(i.wonLeads);l<=0&&d<=0||(t+=l,n+=d,a+=c,s+=c*metaReportParseAmount(i.avgLeadValue),o+=c*metaReportParseAmount(i.avgProfitPerWon))}const r=e.filter(i=>metaReportParseAmount(i.spend)>0).length;return{avgCpl:n>0?metaReportRoundMoney(t/n):0,winRate:n>0?metaReportRoundRatio(a/n):0,avgLeadValue:a>0?metaReportRoundMoney(s/a):0,avgProfitPerWon:a>0?metaReportRoundMoney(o/a):0,avgMonthlySpend:r>0?metaReportRoundMoney(t/r):0}}const META_SCENARIO_MIN_EFFICIENCY_MONTHS=2,META_SCENARIO_MIN_TREND_MONTHS=4,META_SCENARIO_MAD_MULTIPLIER=2.5,META_SCENARIO_MAX_MONTHLY_TREND=.03;function metaReportScenarioParseDate(e){if(!e)return null;const t=String(e).slice(0,10);if(!/^\d{4}-\d{2}-\d{2}$/.test(t))return null;const n=new Date(`${t}T12:00:00.000Z`);return Number.isNaN(n.getTime())?null:n}function metaReportScenarioIsIncompleteMonth(e,t=new Date){if(!metaReportScenarioParseDate(e.periodEnd))return!0;const a=t.toISOString().slice(0,10);return String(e.periodEnd).slice(0,10)>=a}function metaReportScenarioIsAdActive(e){return metaReportParseAmount(e.spend)>0&&metaReportParseAmount(e.leads)>0}function metaReportScenarioMedian(e=[]){const t=e.filter(a=>Number.isFinite(a)).slice().sort((a,s)=>a-s);if(!t.length)return 0;const n=Math.floor(t.length/2);return t.length%2===0?(t[n-1]+t[n])/2:t[n]}function metaReportScenarioMad(e=[]){const t=metaReportScenarioMedian(e);return metaReportScenarioMedian(e.map(n=>Math.abs(n-t)))}function metaReportScenarioLinearRegression(e=[]){const t=e.length;if(t<2)return{slope:0,intercept:0,r:0};let n=0,a=0,s=0,o=0,r=0;for(const m of e){const b=metaReportParseAmount(m.x),p=metaReportParseAmount(m.y);n+=b,a+=p,s+=b*p,o+=b*b,r+=p*p}const i=t*o-n*n;if(i===0)return{slope:0,intercept:a/t,r:0};const l=(t*s-n*a)/i,d=(a-l*n)/t,c=Math.sqrt((t*o-n*n)*(t*r-a*a)),u=c>0?(t*s-n*a)/c:0;return{slope:l,intercept:d,r:u}}function metaReportScenarioEnrichMonth(e){const t=metaReportParseAmount(e.spend),n=metaReportParseAmount(e.leads),a=metaReportParseAmount(e.wonLeads),s=metaReportParseAmount(e.avgLeadValue),o=metaReportParseAmount(e.avgProfitPerWon),r=metaReportRoundMoney(a*s);return{...e,spend:t,leads:n,wonLeads:a,avgLeadValue:s,avgProfitPerWon:o,cpl:n>0?metaReportRoundMoney(t/n):0,totalLeadValue:r,efficiencyIndex:t>0?r/t:0}}function metaReportPrepareScenarioSeries(e=[],{windowMonths:t="6",asOfDate:n=new Date,smoothOutliers:a=!0}={}){const s=normalizeMetaReportScenarioMonthWindow(t)==="all"?1/0:Number.parseInt(normalizeMetaReportScenarioMonthWindow(t),10)||6,o=e.filter(metaReportScenarioIsAdActive).filter(c=>!metaReportScenarioIsIncompleteMonth(c,n)).map(metaReportScenarioEnrichMonth),r=o.length,i=s===1/0?o:o.slice(-s);let l=0,d=i;if(a&&i.length>=3){const c=i.map(g=>metaReportParseAmount(g.cpl)),u=i.map(g=>metaReportParseAmount(g.leads)),m=metaReportScenarioMedian(c),b=metaReportScenarioMedian(u),p=metaReportScenarioMad(c),S=metaReportScenarioMad(u);d=i.map(g=>{const f={...g};let v=!1;if(p>0){const y=m-META_SCENARIO_MAD_MULTIPLIER*p,w=m+META_SCENARIO_MAD_MULTIPLIER*p,h=Math.min(w,Math.max(y,metaReportParseAmount(g.cpl)));h!==metaReportParseAmount(g.cpl)&&(f.cpl=metaReportRoundMoney(h),f.leads=Math.max(1,Math.round(metaReportParseAmount(g.spend)/h)),v=!0)}if(S>0){const y=b-META_SCENARIO_MAD_MULTIPLIER*S,w=b+META_SCENARIO_MAD_MULTIPLIER*S,h=Math.min(w,Math.max(y,metaReportParseAmount(g.leads)));Math.round(h)!==Math.round(metaReportParseAmount(g.leads))&&(f.leads=Math.max(1,Math.round(h)),v=!0)}return v&&(l+=1),f})}return{months:d,monthsAvailable:r,monthsUsed:d.length,outliersAdjusted:l,confidence:metaReportResolveScenarioConfidence(d.length)}}function metaReportScenarioRecencyEfficiency(e=[]){if(e.length<META_SCENARIO_MIN_EFFICIENCY_MONTHS)return null;let t=0,n=0,a=0,s=0,o=0,r=0;return e.forEach((i,l)=>{const d=l+1;t+=metaReportParseAmount(i.spend)*d,n+=metaReportParseAmount(i.leads)*d,a+=metaReportParseAmount(i.wonLeads)*d,s+=metaReportParseAmount(i.wonLeads)*metaReportParseAmount(i.avgLeadValue)*d,o+=metaReportParseAmount(i.wonLeads)*metaReportParseAmount(i.avgProfitPerWon)*d,r+=metaReportParseAmount(i.spend)}),{avgCpl:n>0?metaReportRoundMoney(t/n):0,winRate:n>0?metaReportRoundRatio(a/n):0,avgLeadValue:a>0?metaReportRoundMoney(s/a):0,avgProfitPerWon:a>0?metaReportRoundMoney(o/a):0,avgMonthlySpend:e.length>0?metaReportRoundMoney(r/e.length):0}}function metaReportScenarioPooledEfficiency(e=[]){return metaReportAggregateSeriesEfficiency(e)}function metaReportScenarioBlendEfficiency(e,t,n=.5){if(!e||!t)return e||t;const a=1-n;return{avgCpl:metaReportRoundMoney(e.avgCpl*n+t.avgCpl*a),winRate:metaReportRoundRatio(e.winRate*n+t.winRate*a),avgLeadValue:metaReportRoundMoney(e.avgLeadValue*n+t.avgLeadValue*a),avgProfitPerWon:metaReportRoundMoney(e.avgProfitPerWon*n+t.avgProfitPerWon*a),avgMonthlySpend:metaReportRoundMoney(e.avgMonthlySpend*n+t.avgMonthlySpend*a)}}function metaReportScenarioMonthlyTrend(e=[],t=.75,{dampenHotStreak:n=!1}={}){if(e.length<META_SCENARIO_MIN_TREND_MONTHS)return{trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};const a=e.map(c=>metaReportParseAmount(c.spend)).filter(c=>c>0),s=metaReportScenarioMedian(a)||1,o=e.map((c,u)=>{const m=metaReportParseAmount(c.spend),b=metaReportParseAmount(c.efficiencyIndex),p=b>0?b:1e-6,S=m>0?m:s;return{x:u,y:Math.log(p)-t*Math.log(S/s)}}),{slope:r,r:i}=metaReportScenarioLinearRegression(o);let l=Math.max(-META_SCENARIO_MAX_MONTHLY_TREND,Math.min(META_SCENARIO_MAX_MONTHLY_TREND,r));if(n){const c=e.map(b=>metaReportParseAmount(b.efficiencyIndex)).filter(b=>b>0),u=metaReportScenarioMedian(c),m=e.slice(-3).reduce((b,p)=>b+metaReportParseAmount(p.efficiencyIndex),0)/Math.min(3,e.length);u>0&&m>u*1.15&&l>0&&(l*=.5)}let d="flat";return Math.abs(l)>=.005&&(d=l>0?"up":"down"),{trendRate:l,trendDirection:d,trendRatePct:metaReportRoundRatio(l*100),trendR:metaReportRoundRatio(i)}}function metaReportScenarioCalibrateElasticity(){return META_REPORT_SCENARIO_FIXED_ELASTICITY}function metaReportResolveScenarioConfidence(e=0){const t=Number.parseInt(e,10)||0;return t<4?"low":t<7?"medium":"high"}function metaReportComputeScenarioEfficiency(e=[],{blendHistory:t=!1,includeTrend:n=!1,cautionStrongMonths:a=!1}={}){const s=META_REPORT_SCENARIO_FIXED_ELASTICITY,o=metaReportScenarioCalibrateElasticity(),r=metaReportScenarioRecencyEfficiency(e),i=metaReportScenarioPooledEfficiency(e);if(!r||r.avgCpl<=0)return{efficiency:null,elasticity:o,presetElasticity:s,trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};const l=t?metaReportScenarioBlendEfficiency(i,r,.5):r;let d={trendRate:0,trendDirection:"flat",trendRatePct:0,trendR:0};if(n){const c=metaReportScenarioMonthlyTrend(e,o,{dampenHotStreak:a});Math.abs(c.trendR)>=META_SCENARIO_TREND_R_THRESHOLD&&(d=c)}return{efficiency:l,elasticity:o,presetElasticity:s,...d}}function metaReportProjectBudgetAtSpend({baselineSpend:e,spend:t,efficiency:n,elasticity:a=.75,trendAdj:s=1,hasBottomline:o=!1}={}){const r=metaReportParseAmount(e),i=metaReportParseAmount(t);if(i<=0||r<=0||!n||n.avgCpl<=0)return{spend:0,leads:0,wonLeads:0,totalLeadValue:0,roasKr:0,roasX:0,poasKr:null,poasX:null};const l=r/n.avgCpl,d=i/r,c=metaReportParseAmount(s)>0?metaReportParseAmount(s):1,u=Math.max(0,Math.round(l*d**a*c)),m=Math.round(u*n.winRate),b=metaReportRoundMoney(m*n.avgLeadValue),p=metaReportRoundMoney(b-i),S=i>0?metaReportRoundRatio(p/i):0;let g=null,f=null;if(o&&n.avgProfitPerWon>0){const v=metaReportRoundMoney(m*n.avgProfitPerWon);g=metaReportRoundMoney(v-i),f=i>0?metaReportRoundRatio(g/i):0}return{spend:i,leads:u,wonLeads:m,totalLeadValue:b,roasKr:p,roasX:S,poasKr:g,poasX:f}}function metaReportSpendFromMonthKey(e=[],t=null,n=null){if(!n)return 0;const a=t?.months?.[n];if(a&&!a.meta?.emptyMonth&&metaReportMonthPeriodAligned(a)){const o=metaReportParseAmount(a.meta?.spend);if(o>0)return o}const s=e.find(o=>o.monthKey===n);return s?metaReportParseAmount(s.spend):0}function buildMetaReportScenarioProjection(e,t){const n=getMetaReportScenarioPillsFromState();return metaReportProjectBudgetScenario({series:e,multiplier:metaReportsState.budgetMultiplier,hasBottomline:payloadHasMetaReportBottomline(t),blendHistory:n.blendHistory,includeTrend:n.includeTrend,cautionStrongMonths:n.includeTrend&&n.cautionStrongMonths,monthWindow:metaReportsState.scenarioMonthWindow,baselineMode:metaReportsState.budgetBaseline,activeMonthKey:metaReportsState.activeMonthKey,smoothOutliers:n.smoothUneven})}function resolveMetaReportLastMonthBaseline(e=[],{activeMonthKey:t=null,asOfDate:n=new Date,payload:a=null}={}){const s=e.filter(d=>metaReportParseAmount(d.spend)>0).sort((d,c)=>String(d.monthKey).localeCompare(String(c.monthKey)));if(!s.length)return null;const o=t||metaReportMonthKeyFromDate(n);if(!o)return null;const r=o?metaReportAddMonthsToMonthKey(o,-1):null;if(!r)return null;const i=s.find(d=>d.monthKey===r),l=metaReportSpendFromMonthKey(e,a,r);return!i&&l<=0?null:{monthKey:r,label:metaMonthLabel(r),spend:l>0?l:metaReportParseAmount(i.spend)}}function resolveMetaReportScenarioBaselineSpend(e,t,{baselineMode:n="year",activeMonthKey:a=null,asOfDate:s=new Date,payload:o=null}={}){const r=t?.months||[];if(n==="month"&&a){const i=metaReportSpendFromMonthKey(e,o,a);if(i>0)return i;const l=r.find(d=>d.monthKey===a);return l&&metaReportParseAmount(l.spend)>0?metaReportParseAmount(l.spend):0}if(n==="last"){const i=resolveMetaReportLastMonthBaseline(e,{activeMonthKey:a,asOfDate:s,payload:o});return i?.spend>0?i.spend:0}if(n==="year"&&r.length>0){const i=r.reduce((l,d)=>l+metaReportParseAmount(d.spend),0);return metaReportRoundMoney(i/r.length)}return 0}function metaReportProjectBudgetScenario({series:e=[],baselineSpend:t,multiplier:n=1,hasBottomline:a=!1,blendHistory:s=!1,includeTrend:o=!1,cautionStrongMonths:r=!1,monthWindow:i="6",baselineMode:l="year",activeMonthKey:d=null,asOfDate:c=new Date,smoothOutliers:u=!0}={}){const m=metaReportPrepareScenarioSeries(e,{windowMonths:i,asOfDate:c,smoothOutliers:u}),b=metaReportComputeScenarioEfficiency(m.months,{blendHistory:s,includeTrend:o,cautionStrongMonths:r});let p=metaReportParseAmount(t);p||(p=resolveMetaReportScenarioBaselineSpend(e,m,{baselineMode:l,activeMonthKey:d,asOfDate:c}));const S=metaReportParseAmount(n)||1,{efficiency:g,elasticity:f,trendRate:v}=b;if(!g||g.avgCpl<=0||p<=0||m.monthsUsed<META_SCENARIO_MIN_EFFICIENCY_MONTHS)return{...b,prepared:m,baseline:metaReportProjectBudgetAtSpend({baselineSpend:0,spend:0,efficiency:g}),projected:metaReportProjectBudgetAtSpend({baselineSpend:0,spend:0,efficiency:g}),projectedConservative:null,projectedOptimistic:null,multiplier:S,baselineSpend:p,hasBottomline:a,insufficientData:!0};const y=Math.exp(v*1),w=metaReportProjectBudgetAtSpend({baselineSpend:p,spend:p,efficiency:g,elasticity:f,trendAdj:1,hasBottomline:a}),h=metaReportProjectBudgetAtSpend({baselineSpend:p,spend:metaReportRoundMoney(p*S),efficiency:g,elasticity:f,trendAdj:y,hasBottomline:a}),$=metaReportProjectBudgetAtSpend({baselineSpend:p,spend:metaReportRoundMoney(p*S),efficiency:g,elasticity:f,trendAdj:Math.exp(Math.min(v,0)*1),hasBottomline:a}),E=metaReportProjectBudgetAtSpend({baselineSpend:p,spend:metaReportRoundMoney(p*S),efficiency:g,elasticity:f,trendAdj:Math.exp(Math.max(v,0)*1),hasBottomline:a});return{...b,prepared:m,baseline:w,projected:h,projectedConservative:$,projectedOptimistic:E,multiplier:S,baselineSpend:p,hasBottomline:a,insufficientData:!1}}function metaReportSpendAxisTick(e){const t=Number(e)||0;return Math.abs(t)>=1e6?`${(t/1e6).toFixed(1)}M`:Math.abs(t)>=1e3?`${Math.round(t/1e3)}K`:t}function metaReportSpendMultiplierValue(e,t){const n=metaReportParseAmount(t),a=metaReportParseAmount(e);return n<=0?0:metaReportRoundRatio(a/n)}function resolveMetaReportSpendChartBaseline(e,t=null){const n=metaReportParseAmount(t?.baselineSpend);return n>0?n:metaReportAggregateSeriesEfficiency(e).avgMonthlySpend}function trimMetaReportScenarioSeries(e,t=META_REPORT_SCENARIO_MAX_HISTORICAL){const n=e.filter(a=>metaReportParseAmount(a.spend)>0);return n.length>t?n.slice(-t):n}function metaReportMonthKeyFromDate(e=new Date){const t=e instanceof Date?e:new Date(e);return Number.isNaN(t.getTime())?null:`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}`}function metaReportAddMonthsToMonthKey(e,t=0){const n=String(e||"").trim();if(!/^\d{4}-\d{2}$/.test(n))return null;const[a,s]=n.split("-").map(Number),o=new Date(a,s-1+t,1);return`${o.getFullYear()}-${String(o.getMonth()+1).padStart(2,"0")}`}function metaReportBuildScenarioProjectionMonthKeys(e=META_REPORT_SCENARIO_PROJECTED_MONTHS,t=new Date){const n=metaReportMonthKeyFromDate(t);if(!n||e<=0)return[];const a=[];for(let s=1;s<=e;s+=1)a.push(metaReportAddMonthsToMonthKey(n,s));return a}function buildMetaReportScenarioProjectionSteps(e,{months:t=META_REPORT_SCENARIO_PROJECTED_MONTHS,hasBottomline:n=!1,asOfDate:a=new Date,targetMultiplier:s=null}={}){if(!e||e.insufficientData)return[];const o=metaReportParseAmount(e.baselineSpend),r=metaReportParseAmount(s??e.multiplier)||1,i=e.efficiency,l=e.elasticity||.75,d=e.trendRate||0,c=metaReportBuildScenarioProjectionMonthKeys(t,a),u=[];for(let m=1;m<=t;m+=1){const b=r,p=metaReportRoundMoney(o*b),S=Math.exp(d*m),g=metaReportProjectBudgetAtSpend({baselineSpend:o,spend:p,efficiency:i,elasticity:l,trendAdj:S,hasBottomline:n}),f=c[m-1]||null;u.push({label:f?metaMonthLabel(f):`+${m} mo`,monthKey:f,spendMultiplier:metaReportRoundRatio(b),spend:g.spend,leads:g.leads,wonLeads:g.wonLeads,totalLeadValue:g.totalLeadValue,roasKr:g.roasKr,roasX:g.roasX,poasKr:g.poasKr,poasX:g.poasX})}return u}function metaReportDeltaPct(e,t){const n=metaReportParseAmount(t),a=metaReportParseAmount(e);return n?metaReportRoundRatio((a-n)/Math.abs(n)*100):null}function metaReportResolveBaselineSpend(e,t,n=null){if(n?.baselineSpend>0)return metaReportParseAmount(n.baselineSpend);const a=getMetaReportScenarioPillsFromState(),s=n?.prepared||metaReportPrepareScenarioSeries(t,{windowMonths:metaReportsState.scenarioMonthWindow,smoothOutliers:a.smoothUneven});return resolveMetaReportScenarioBaselineSpend(t,s,{baselineMode:metaReportsState.budgetBaseline,activeMonthKey:metaReportsState.activeMonthKey})}function resolveMetaReportScenarioChartSeries(e){const t=metaReportMonthKeyFromDate(new Date),n=e.filter(a=>!(metaReportParseAmount(a.spend)<=0||t&&a.monthKey&&a.monthKey>t));return trimMetaReportScenarioSeries(n)}function resolveMetaReportScenarioBaselineHints(e=[],t=null,n=null){const a=n||metaReportsState.clientPayload||null,s=getMetaReportScenarioPillsFromState(),o=t?.prepared||metaReportPrepareScenarioSeries(e,{windowMonths:metaReportsState.scenarioMonthWindow,smoothOutliers:s.smoothUneven}),r=metaReportsState.activeMonthKey,i=metaReportMonthKeyFromDate(new Date),l=a?.year||metaReportsState.selectedYear||(r?Number(String(r).slice(0,4)):null),d={activeMonthKey:r,asOfDate:new Date,payload:a},c=resolveMetaReportLastMonthBaseline(e,d),u=resolveMetaReportScenarioBaselineSpend(e,o,{baselineMode:"year",...d}),m=resolveMetaReportScenarioBaselineSpend(e,o,{baselineMode:"last",...d}),b=resolveMetaReportScenarioBaselineSpend(e,o,{baselineMode:"month",...d}),p=!!(i&&l&&String(i).startsWith(`${l}-`));return{windowAverage:u,windowMonthCount:o.monthsUsed||o.months.length||0,selectedMonthSpend:b,selectedMonthLabel:r?metaMonthLabel(r):"",activeKey:r,currentMonthSpend:p?metaReportSpendFromMonthKey(e,a,i):0,currentMonthLabel:p?metaMonthLabel(i):"",currentKey:p?i:null,lastMonthSpend:m,lastMonthLabel:c?.label||"",lastMonthKey:c?.monthKey||null}}function renderMetaReportBaselineSelectOptions(e=null,t="year"){const n=e?.windowAverage>0?metaReportFmtBaselineAmount(e.windowAverage):"\u2014",a=e?.lastMonthSpend>0?metaReportFmtBaselineAmount(e.lastMonthSpend):"\u2014",s=e?.selectedMonthSpend>0?metaReportFmtBaselineAmount(e.selectedMonthSpend):"\u2014",o=normalizeMetaReportBudgetBaseline(t),r=e?.lastMonthLabel?`Last month \xB7 ${e.lastMonthLabel}`:"Last month";return`
    <option value="year"${o==="year"?" selected":""}>Window average \xB7 ${esc(n)}</option>
    <option value="last"${o==="last"?" selected":""}>${esc(r)} \xB7 ${esc(a)}</option>
    <option value="month"${o==="month"?" selected":""}>Selected month \xB7 ${esc(s)}</option>
  `}function renderMetaReportBaselineStrip(e=null){if(!e)return"";const t=[],n=metaReportsState.budgetBaseline;if(e.windowAverage>0&&t.push(renderMetaReportBaselinePill("Window avg",e.windowAverage,{active:n==="year",title:"Average ad spend in the history window (completed months)"})),e.lastMonthSpend>0&&e.lastMonthLabel&&t.push(renderMetaReportBaselinePill(`Last month \xB7 ${e.lastMonthLabel}`,e.lastMonthSpend,{active:n==="last",title:"Ad spend for the month before the selected month tab"})),e.selectedMonthSpend>0&&e.selectedMonthLabel){const a=e.activeKey&&e.currentKey&&e.activeKey===e.currentKey;t.push(renderMetaReportBaselinePill(a?`Selected \xB7 ${e.selectedMonthLabel} (current)`:`Selected \xB7 ${e.selectedMonthLabel}`,e.selectedMonthSpend,{active:n==="month",title:"Ad spend for the selected month tab"}))}return e.currentMonthSpend>0&&e.currentMonthLabel&&e.activeKey!==e.currentKey&&t.push(renderMetaReportBaselinePill(`Current \xB7 ${e.currentMonthLabel}`,e.currentMonthSpend,{title:"Ad spend for the current calendar month"})),t.join("")}function syncMetaReportBaselineUi(e=[],t=null,n=null){const a=resolveMetaReportScenarioBaselineHints(e,t,n),s=document.getElementById("meta-report-baseline-strip");if(s){const r=renderMetaReportBaselineStrip(a);s.innerHTML=r,s.hidden=!r}const o=document.getElementById("meta-report-budget-baseline");if(o){const r=o.value||metaReportsState.budgetBaseline;o.innerHTML=renderMetaReportBaselineSelectOptions(a,r),o.value=normalizeMetaReportBudgetBaseline(r)}}function renderMetaReportScenarioContextStrip(e){if(!e||e.insufficientData)return'<p class="meta-report-scenario-context" id="meta-report-scenario-context">Add at least two completed ad months to model a budget scenario.</p>';const t=describeActiveMetaReportScenarioPills(),n=t.length?t.join(" \xB7 "):"Recent months only",a=e.trendDirection==="up"?`trend up ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:e.trendDirection==="down"?`trend down ${Math.abs(e.trendRatePct||0).toFixed(1)}%/mo`:"trend flat",s=e.prepared?.outliersAdjusted?` \xB7 ${e.prepared.outliersAdjusted} uneven month${e.prepared.outliersAdjusted===1?"":"s"} removed`:"",r={low:"Low confidence",medium:"Medium confidence",high:"High confidence"}[e.prepared?.confidence]||"",i=r?` \xB7 ${r} (${e.prepared?.monthsUsed||0} mo data)`:"";return`<p class="meta-report-scenario-context" id="meta-report-scenario-context"><strong>Active:</strong> ${esc(n)} \xB7 last ${e.prepared?.monthsUsed||0} ad months \xB7 ${esc(a)}${esc(s)}${esc(i)}</p>`}function payloadHasMetaReportBottomline(e){return e?.settings?.metaReportShowBottomline===!1?!1:seriesHasBottomline(metaReportsState.chartSeries||buildMetaReportChartSeries(e))}function seriesHasBottomline(e=[]){return e.some(t=>t.poasKr!=null&&t.poasX!=null)}function renderMetaReportChartsPanel({editable:e=!1}={}){const t=metaReportsState.chartTab||"trend";return`
  ...
  `}function renderMetaReportScenarioCard(){return`
    <section class="meta-report-scenario-card" id="meta-report-scenario-card">
      ...
      <div class="meta-report-scenario-baseline-section">
  ...
  `}function metaReportFormatDeltaPct(e){return e==null||!Number.isFinite(e)?"":`${e>0?"+":""}${e.toFixed(0)}%`}function metaReportScenarioDeltaTone(e=""){return String(e).startsWith("+")?"positive":String(e).startsWith("-")?"negative":"neutral"}function renderMetaReportScenarioOutcomeCard({label:e,projectedValue:t,projectedSub:n="",baselineValue:a="",delta:s=""}={}){const o=metaReportScenarioDeltaTone(s);return`
  ...
  `}function renderMetaReportScenarioDetailRow(e,t,n,a){const s=metaReportScenarioDeltaTone(a);return`
    <div class="meta-report-scenario-details-row">
      ...
      <span class="meta-report-scenario-delta${s==="positive"?" is-positive":s==="negative"?" is-negative":""}">${esc(a||"\u2014")}</span>
    </div>
  ...
  `:""}function renderMetaReportMonthBody(e,{editable:t=!1,yearPayload:n=null,activeMonthKey:a=null}={}){const s=a||e?.monthKey||null,o=s?metaReportMonthBounds(s):null;let r="";t&&e&&!e.meta?.emptyMonth&&o&&(r=`
    ...
    `);const i=a||e?.monthKey||null,l=t?i?`Report preview \xB7 ${metaMonthLabel(i)}`:"Report preview":i?`Report \xB7 ${metaMonthLabel(i)}`:"Report",d=e&&!e.meta?.emptyMonth?`
    <div class="meta-report-preview-card">
      <div class="meta-report-preview-card-label">${l}</div>
      ${renderMetaReportHighlightStrip(e)}
    </div>
  `:"",c=renderMetaReportMonthTables(e,{editable:t}),u=shouldShowMetaReportYearVisuals(e,n,{editable:t}),m=u?renderMetaReportChartsPanel({editable:t}):"",b=t&&u?renderMetaReportScenarioCard():"",p=u?`
  ...
  `:`
    <div class="meta-report-content-main">${c}</div>
  `;return`${r}${d}${p}`}function renderMetaReportsHubPage(e){const t=e.clients||[],n=e.meta||{};return`
    ...
    `)}
  `}function renderMetaReportsClientPage(e){const t=e.monthKeys||[],n=metaReportsState.activeMonthKey||t[t.length-1]||"",a=e.months?.[n]||null,s=e.settings||{},o=t.map(r=>`
  ...
  `).join("");return`
    ...
    <div class="sync-history-page meta-reports-page meta-report-client-page">
      ...
      <div class="meta-report-month-panel" id="meta-report-month-panel">
        ${renderMetaReportMonthBody(a,getMetaReportMonthBodyOptions(s,{editable:!0,yearPayload:e}))}
      </div>
    </div>
    `)}
  `}function metaReportClientDisplayName(e){return String(e?.accountName||e?.clientId||"").trim()||"Meta report"}function syncMetaReportPublicBranding(e){const t=metaReportClientDisplayName(e);document.title=`${t} \xB7 Censio Analytics`;const n=document.querySelector(".meta-report-public-hero h1");n&&(n.textContent=t)}function renderPublicMetaReportPage(e){const t=metaReportClientDisplayName(e),n=e.monthKeys||[],a=metaReportsState.activeMonthKey||n[n.length-1]||"",s=e.months?.[a]||null,o=n.map(r=>`
  ...
  `}function setMetaReportYearLoadingBanner(e,t,n){const a=document.querySelector(e);if(!a)return;let s=a.querySelector(".meta-report-year-loading-banner");n?(s||(s=document.createElement("span"),s.className="meta-report-year-loading-banner",s.setAttribute("aria-live","polite"),a.appendChild(s)),s.innerHTML=`<span class="meta-report-save-indicator-spinner"></span> Loading ${esc(t)}\u2026`,s.hidden=!1):s&&(s.hidden=!0)}function setMetaReportClientYearLoading(e,t){const n=document.querySelector(".meta-report-client-page"),a=document.getElementById("meta-report-year");if(t){a&&(a.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!0);const s=document.querySelector(".meta-report-tabs");s&&(s.innerHTML=renderMetaReportTabsSkeleton(),s.setAttribute("aria-busy","true"));const o=document.getElementById("meta-report-month-panel");o&&(o.innerHTML=renderMetaReportMonthPanelSkeleton(),o.classList.add("is-loading")),destroyMetaReportCharts()}else{a&&(a.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-report-client-page .meta-report-toolbar-left",e,!1);const s=document.querySelector(".meta-report-tabs");s&&s.removeAttribute("aria-busy");const o=document.getElementById("meta-report-month-panel");o&&o.classList.remove("is-loading")}}function setMetaCvYearLoading(e,t){const n=document.querySelector(".sync-history-page.meta-reports-page"),a=document.getElementById("meta-cv-year"),s=document.getElementById("meta-cv-search");if(t){a&&(a.disabled=!0),s&&(s.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!0);const o=document.querySelector(".meta-cv-summary");o&&(o.classList.add("is-skeleton"),o.innerHTML=renderMetaCvSummarySkeleton());const r=document.querySelector(".meta-cv-legend");r&&r.setAttribute("aria-hidden","true");const i=document.getElementById("meta-cv-client-list");i&&(i.innerHTML=renderMetaCvListSkeleton(),i.setAttribute("aria-busy","true"));const l=document.getElementById("meta-cv-editor");l&&(l.innerHTML=renderMetaCvEditorYearSkeleton(),l.setAttribute("aria-busy","true"))}else{a&&(a.disabled=!1),s&&(s.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-cv-toolbar-left",e,!1);const o=document.querySelector(".meta-cv-summary");o&&o.classList.remove("is-skeleton");const r=document.querySelector(".meta-cv-legend");r&&r.removeAttribute("aria-hidden");const i=document.getElementById("meta-cv-client-list");i&&i.removeAttribute("aria-busy");const l=document.getElementById("meta-cv-editor");l&&l.removeAttribute("aria-busy")}}function setMetaReportPublicYearLoading(e,t){const n=document.querySelector(".meta-reports-page--public"),a=document.getElementById("meta-report-year");if(t){a&&(a.disabled=!0),n&&n.classList.add("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!0);const s=document.querySelector(".meta-reports-page--public .meta-report-tabs");s&&(s.innerHTML=renderMetaReportTabsSkeleton(),s.setAttribute("aria-busy","true"));const o=document.getElementById("meta-report-month-panel");o&&(o.innerHTML=renderMetaReportMonthPanelSkeleton(),o.classList.add("is-loading")),destroyMetaReportCharts()}else{a&&(a.disabled=!1),n&&n.classList.remove("is-year-loading"),setMetaReportYearLoadingBanner(".meta-reports-page--public .meta-report-toolbar-left",e,!1);const s=document.querySelector(".meta-reports-page--public .meta-report-tabs");s&&s.removeAttribute("aria-busy");const o=document.getElementById("meta-report-month-panel");o&&o.classList.remove("is-loading")}}function bindMetaReportsClientChromeEvents(e){const t=e.clientId,n=document.getElementById("meta-report-year");n&&(n.onchange=async()=>{if(n.selectedOptions?.[0]?.disabled){n.value=String(metaReportsState.selectedYear);return}const s=metaReportsState.selectedYear,o=Number(n.value);if(o!==s){metaReportsState.selectedYear=o,metaReportsState.activeMonthKey=null,setMetaReportClientYearLoading(o,!0);try{await loadMetaReportsClientPage({silent:!0})}catch(r){metaReportsState.selectedYear=s,n&&(n.value=String(s)),await loadMetaReportsClientPage({silent:!0}),showToast(r.message||"Load failed","error")}finally{setMetaReportClientYearLoading(o,!1)}}}),bindMetaReportCopyButtons(),bindMetaReportBackfillButton(t),bindMetaReportBottomlineFeeEvents(t,"meta-report"),bindMetaReportShareEvents(e)}function bindMetaReportsClientEvents(e){bindMetaReportsClientChromeEvents(e),bindMetaReportsClientTabEvents(e),bindMetaReportsClientEditEvents(e.clientId,e)}function bindMetaReportsClientEditEvents(e,t){const n=document.getElementById("meta-report-refresh-meta");n&&(n.onclick=async()=>{n.disabled=!0;try{const s=metaReportsState.activeMonthKey,o=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}/refresh`,{method:"POST"});refreshMetaReportMonthPanel(o.monthPayload),showToast("Meta data refreshed","success")}catch(s){showToast(s.message||"Meta refresh failed","error")}finally{n.disabled=!1}});const a=document.getElementById("meta-report-save-month");a&&(a.onclick=async()=>{a.disabled=!0;try{const s=metaReportsState.activeMonthKey,o={periodStart:document.getElementById("meta-report-period-start")?.value,periodEnd:document.getElementById("meta-report-period-end")?.value,wonLeads:Number(document.getElementById("meta-report-won-leads")?.value),avgLeadValue:Number(document.getElementById("meta-report-avg-lead-value")?.value),avgProfitPerWon:Number(document.getElementById("meta-report-avg-profit")?.value)},r=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(s)}`,{method:"PUT",body:JSON.stringify(o)});refreshMetaReportMonthPanel(r.monthPayload),showToast("Month saved","success")}catch(s){showToast(s.message||"Save failed","error")}finally{a.disabled=!1}})}function bindPublicMetaReportTabEvents(e){document.querySelectorAll("[data-meta-month-tab]").forEach(t=>{t.onclick=()=>{switchMetaReportMonthTab(t.getAttribute("data-meta-month-tab"),{editable:!1})}})}function bindPublicMetaReportEvents(e){bindPublicMetaReportTabEvents(e);const t=document.getElementById("meta-report-year");t&&(t.onchange=async()=>{if(t.selectedOptions?.[0]?.disabled){t.value=String(metaReportsState.selectedYear);return}const a=metaReportsState.selectedYear,s=Number(t.value);if(s!==a){metaReportsState.selectedYear=s,metaReportsState.activeMonthKey=null,setMetaReportPublicYearLoading(s,!0);try{await loadPublicMetaReportPage({silent:!0})}catch(o){metaReportsState.selectedYear=a,t&&(t.value=String(a)),await loadPublicMetaReportPage({silent:!0}),showToast(o.message||"Load failed","error")}finally{setMetaReportPublicYearLoading(s,!1)}}})}async function loadMetaReportsHubPage(){const e=document.getElementById("dashboard");if(!e)return;const t=await fetchStaffMe();if(!t){e.innerHTML=`
    ...
    `}).join("")}function renderMetaCvClientRows(e){const t=getMetaCvFilteredClients(e),n=e.monthKeys||[];return t.length?t.map(a=>`
    ...
    `).join(""):'<div class="meta-report-empty" style="padding:32px 12px">No matching clients.</div>'}function monthStatusIsEmpty(e,t={}){return e?(e.status??classifyMetaCvMonthStatus({wonLeads:e.inputs?.wonLeads??e.topline?.wonLeads??e.wonLeads,avgLeadValue:e.inputs?.avgLeadValue??e.topline?.avgLeadValue??e.avgLeadValue,avgProfitPerWon:e.inputs?.avgProfitPerWon??e.avgProfitPerWon},!!t.metaReportShowBottomline))==="empty":!0}function metaReportEffectiveFeeEnabled(e,t=null){if(!t)return!!e?.metaReportFeeEnabled;const n=!!e?.metaReportShowBottomline;return(t.status??classifyMetaCvMonthStatus({wonLeads:t.inputs?.wonLeads??t.topline?.wonLeads??t.wonLeads,avgLeadValue:t.inputs?.avgLeadValue??t.topline?.avgLeadValue??t.avgLeadValue,avgProfitPerWon:t.inputs?.avgProfitPerWon??t.avgProfitPerWon},n))==="empty"?!1:!!e?.metaReportFeeEnabled}function resolveMetaReportFeeMode(e={}){return e.metaReportFeeEnabled?resolveMetaReportStoredFeeMode(e):""}const META_REPORT_FEE_MODE_META=[{value:"",icon:"\u2013",title:"None",desc:"No Censio fee"},{value:"performance",icon:"%",title:"Performance fee",desc:"% of profit (POAS)"},{value:"marketing",icon:"Dkr",title:"Marketing fee",desc:"Fixed Dkr amount"}];function metaReportFeeValueBlockHtml(e,t,n,a,{inline:s=!1}={}){return s&&t==="performance"?`
      ...
      </label>`:'<p class="meta-report-fee-value-empty">No amount needed \u2014 just save your selection.</p>'}function metaReportUsesInlineFeeValue(e){return e==="meta-cv"}function describeMetaReportFeeModeLabel(e,t,n,{feeEnabled:a=!0}={}){if(!a||e===""||e==null)return"No Censio fee";if(e==="performance"){const s=Number(t);return`Performance fee \xB7 ${Number.isFinite(s)?s:20}%`}if(e==="marketing"){const s=Number(n);return`Marketing fee \xB7 Dkr ${Number.isFinite(s)?s:0}`}return"No Censio fee"}function readMetaReportFeeDraftState(e){const t=document.getElementById(`${e}-fee-nested`);if(!t)return null;const n=t.querySelector(`input[name="${e}-fee-mode"]:checked`),a=n?n.value||"":t.dataset.feeMode||"";let s=Number(t.dataset.feePercent),o=Number(t.dataset.marketingFee);if(a==="performance"){const u=document.getElementById(`${e}-setting-fee-percent`),m=Number(u?.value);Number.isFinite(m)&&(s=m)}else if(a==="marketing"){const u=document.getElementById(`${e}-setting-marketing-fee`),m=Number(u?.value);Number.isFinite(m)&&(o=m)}const r=t.dataset.feeEnabled==="true"?resolveMetaReportStoredFeeMode({metaReportFeeEnabled:!0,metaReportFeeMode:t.dataset.feeMode||null,metaReportMarketingFeeAmount:t.dataset.marketingFee}):"",i=Number(t.dataset.feePercent),l=Number(t.dataset.marketingFee);return{appliedMode:r,appliedPercent:i,appliedMarketing:l,selectedMode:a,selectedPercent:s,selectedMarketing:o,isDirty:a!==r||(a==="performance"?s!==i:a==="marketing"?o!==l:!1),appliedLabel:describeMetaReportFeeModeLabel(r,i,l),selectedLabel:describeMetaReportFeeModeLabel(a,s,o)}}function metaReportFeeStatusHtml(e){return e.isDirty?`
  ...
  `}function setMetaReportClientSettingsExpanded(e,t){if(e!=="meta-report")return;metaReportsState.clientReportSettingsExpanded=!!t,t&&(metaReportsState.clientShareExpanded=!1);const n=document.querySelector(".meta-report-bottomline-fee-settings--client"),a=document.getElementById(`${e}-fee-editor`);n?.classList.toggle("is-editing",t),n?.classList.toggle("is-collapsed",!t),a&&a.classList.toggle("is-hidden",!t);const s=document.getElementById("meta-report-share-summary"),o=document.getElementById("meta-report-share-editor");s&&(s.hidden=metaReportsState.clientShareExpanded),o&&(o.hidden=!metaReportsState.clientShareExpanded),syncMetaReportControlPanelUi()}function updateMetaReportBottomlineFeeSummary(e={},t="meta-report"){const n=document.getElementById(`${t}-fee-summary`);if(!n)return;const a=document.createElement("div");a.innerHTML=renderMetaReportBottomlineFeeSummary(e,t);const s=a.firstElementChild;s&&n.replaceWith(s);const o=document.getElementById(`${t}-fee-edit`);o&&(o.onclick=()=>setMetaReportClientSettingsExpanded(t,!0))}function renderMetaReportBottomlineFeeSettings(e={},t="meta-report",{compact:n=!1,variant:a="default",collapsible:s=!1,externalSummary:o=!1}={}){const r=!!e.metaReportShowBottomline,i=!!e.metaReportFeeEnabled,l=e.metaReportFeeMode||"",d=resolveMetaReportFeeMode(e),c=e.metaReportFeePercent??20,u=e.metaReportMarketingFeeAmount??0,m=metaReportUsesInlineFeeValue(t),b=`data-fee-mode="${esc(l)}" data-fee-enabled="${i?"true":"false"}" data-fee-percent="${esc(c)}" data-marketing-fee="${esc(u)}"`;if(a==="cv"){const y=describeMetaReportFeeModeLabel(d,c,u),w=META_REPORT_FEE_MODE_META.map(({value:h,title:$})=>`
    ...
    <div class="meta-report-bottomline-fee-settings${s?" meta-report-bottomline-fee-settings--client":""}${s?g?" is-editing":" is-collapsed":""}">
      ...
      <div class="meta-report-bottomline-editor${s&&!g?" is-hidden":""}" id="${t}-fee-editor">
        ...
        <div class="meta-report-fee-nested${r?"":" is-hidden"}" id="${t}-fee-nested" ${b}>
          ...
          <div class="meta-report-fee-mode-grid${n?" meta-report-fee-mode-grid--rows":""}">${S}
          </div>
  ...
  `}function renderMetaReportSwitch(e,t,n,a=!1,{hideLabel:s=!1}={}){return`
    <div class="meta-report-switch">
      ...
      ${s?"":`<span class="meta-report-switch-label">${esc(n)}</span>`}
    </div>
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find(S=>S.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
      <div class="meta-cv-editor-empty">
        Select a client month pill to edit won leads, average lead value, and average profit.
      </div>
    `;const s=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,s);const o=t.editorPayload?.months?.[a]||null,r=n.metaReportShowBottomline,i=s.wonLeads!=null?s.wonLeads:o?.inputs?.wonLeads??o?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:o?.inputs?.avgLeadValue??o?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:o?.inputs?.avgProfitPerWon??"",c=metaReportsState.customValues.draftInputs,u=c?.wonLeads??i,m=c?.avgLeadValue??l,b=c?.avgProfitPerWon??d,p=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-cv-editor-grid">
      <label>Client
        <select id="meta-cv-client-select" class="admin-select">
          ${(e.clients||[]).map(S=>`
            <option value="${esc(S.clientId)}"${S.clientId===n.clientId?" selected":""}>${esc(S.accountName)}</option>
          `).join("")}
        </select>
      </label>
      <label>Month
        <select id="meta-cv-month-select" class="admin-select">
          ${(e.monthKeys||[]).map(S=>`
            <option value="${esc(S)}"${S===a?" selected":""}>${esc(metaMonthLabel(S))}</option>
          `).join("")}
        </select>
      </label>
      <label>Won leads
  ...
  `}function refreshMetaCvListAndEditor(){const e=metaReportsState.customValues.overview;if(!e)return;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e));const n=document.getElementById("meta-cv-editor");n&&(n.innerHTML=renderMetaCvEditor(e)),metaReportsState.customValues.draftInputs=null,bindMetaCvEditorEvents(),bindMetaCvListEvents()}function setMetaCvSavePending(e){metaReportsState.customValues.saving=e;const t=document.getElementById("meta-cv-save"),n=document.getElementById("meta-cv-save-indicator");n&&n.classList.toggle("is-visible",e),t&&(t.disabled=e||metaReportsState.customValues.loadingEditor||metaReportsState.customValues.settingsSaving,t.textContent=e?"Saving\u2026":"Save values")}function setMetaCvSettingsSaving(e,t="report"){metaReportsState.customValues.settingsSaving=e,metaReportsState.customValues.settingsSavingScope=e?t:null;const n=e&&t==="report";document.getElementById("meta-cv-settings-status")?.classList.toggle("is-visible",n),document.querySelector(".meta-cv-client-settings")?.classList.toggle("is-saving",n),["meta-cv-setting-bottomline"].forEach(o=>{const r=document.getElementById(o);r&&(r.disabled=n)}),document.querySelectorAll('input[name="meta-cv-fee-mode"]').forEach(o=>{o.disabled=n}),["meta-cv-setting-fee-percent","meta-cv-setting-marketing-fee"].forEach(o=>{const r=document.getElementById(o);r&&(r.disabled=n||r.disabled)});const s=document.getElementById("meta-cv-save");s&&(s.disabled=e||metaReportsState.customValues.saving||metaReportsState.customValues.loadingEditor)}function classifyMetaCvMonthStatus(e,t=!1){if(!e)return"empty";const n=e.wonLeads!=null,a=e.avgLeadValue!=null,s=e.avgProfitPerWon!=null;return n&&a&&(!t||s)?"complete":n||a||s?"partial":"empty"}function applyMetaCvSaveResult(e,t,n,a){const o=metaReportsState.customValues.overview?.clients?.find(l=>l.clientId===e),r=o?.months?.[t],i=a?.month||{};r&&(r.wonLeads=i.wonLeads??n.wonLeads,r.avgLeadValue=i.avgLeadValue??n.avgLeadValue,r.avgProfitPerWon=i.avgProfitPerWon??n.avgProfitPerWon,r.status=classifyMetaCvMonthStatus(r,!!o.metaReportShowBottomline),r.updatedAt=r.status==="empty"?null:i.updatedAt||new Date().toISOString()),a?.monthPayload&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:a.monthPayload}})}async function refreshMetaCvOverviewSilently(){const e=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(e.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=e;const t=document.getElementById("meta-cv-client-list");t&&(t.innerHTML=renderMetaCvClientRows(e),bindMetaCvListEvents());const n=document.querySelector(".meta-cv-summary");if(n&&e.summary){const a=e.summary;n.innerHTML=`
    ...
    `}}function collectMetaCvSaveBody(){return{wonLeads:document.getElementById("meta-cv-won-leads")?.value===""?null:Number(document.getElementById("meta-cv-won-leads")?.value),avgLeadValue:document.getElementById("meta-cv-avg-lead-value")?.value===""?null:Number(document.getElementById("meta-cv-avg-lead-value")?.value),avgProfitPerWon:document.getElementById("meta-cv-avg-profit")?.value===""?null:Number(document.getElementById("meta-cv-avg-profit")?.value)}}function syncMetaCvClientSettingsDom(e={}){syncMetaReportBottomlineFeeDom(e,"meta-cv")}async function saveMetaCvClientSettings(e,t,n){setMetaCvSettingsSaving(!0,"report");try{const a=metaReportsState.customValues.selectedMonthKey,s=await patchMetaReportSettings(e,{...t,monthKey:a||void 0}),r=metaReportsState.customValues.overview?.clients?.find(i=>i.clientId===e);r&&s.settings&&(r.metaReportShowBottomline=!!s.settings.metaReportShowBottomline,r.metaReportFeeEnabled=!!s.settings.metaReportFeeEnabled,r.metaReportFeeMode=s.settings.metaReportFeeMode||null,r.metaReportFeePercent=s.settings.metaReportFeePercent??20,r.metaReportMarketingFeeAmount=s.settings.metaReportMarketingFeeAmount??0),s.monthPayload&&a&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[a]:s.monthPayload}}),setMetaCvSettingsSaving(!1),syncMetaCvClientSettingsDom(s.settings||{}),showToast(describeMetaReportSettingsChange(t,s.settings||{}),"success")}catch(a){showToast(a.message||"Update failed","error"),n&&n(),setMetaCvSettingsSaving(!1)}}function bindMetaCvSettingsEvents(e){bindMetaReportBottomlineFeeEvents(e,"meta-cv",(t,n)=>saveMetaCvClientSettings(e,t,n))}async function selectMetaCvMonth(e,t,{reloadEditor:n=!0}={}){if(metaReportsState.customValues.selectedClientId=e,metaReportsState.customValues.selectedMonthKey=t,refreshMetaCvListAndEditor(),!!n){metaReportsState.customValues.loadingEditor=!0,refreshMetaCvListAndEditor();try{const a=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(e)}/months/${encodeURIComponent(t)}`);metaReportsState.customValues.selectedClientId===e&&metaReportsState.customValues.selectedMonthKey===t&&(metaReportsState.customValues.editorPayload={...metaReportsState.customValues.editorPayload||{},months:{...metaReportsState.customValues.editorPayload?.months||{},[t]:a.monthPayload}})}catch(a){showToast(a.message||"Failed to load month","error")}finally{metaReportsState.customValues.loadingEditor=!1,refreshMetaCvListAndEditor()}}}function bindMetaCvListEvents(){document.querySelectorAll("[data-meta-cv-month]").forEach(e=>{e.onclick=()=>{selectMetaCvMonth(e.getAttribute("data-meta-cv-client"),e.getAttribute("data-meta-cv-month"))}})}function bindMetaCvEditorEvents(){const e=metaReportsState.customValues.selectedClientId;e&&bindMetaCvSettingsEvents(e);const t=document.getElementById("meta-cv-client-select"),n=document.getElementById("meta-cv-month-select");t&&(t.onchange=()=>{const s=metaReportsState.customValues.selectedMonthKey||metaReportsState.customValues.overview?.monthKeys?.slice(-1)[0];s&&selectMetaCvMonth(t.value,s)}),n&&(n.onchange=()=>{const s=metaReportsState.customValues.selectedClientId;s&&selectMetaCvMonth(s,n.value)});const a=document.getElementById("meta-cv-save");a&&(a.onclick=async()=>{const s=metaReportsState.customValues.selectedClientId,o=metaReportsState.customValues.selectedMonthKey;if(!s||!o)return;const r=collectMetaCvSaveBody();setMetaCvSavePending(!0);try{const i=await adminFetch(`/api/meta-reports/clients/${encodeURIComponent(s)}/months/${encodeURIComponent(o)}`,{method:"PUT",body:JSON.stringify(r)});applyMetaCvSaveResult(s,o,r,i),metaReportsState.customValues.draftInputs=r,setMetaCvSavePending(!1),refreshMetaCvListAndEditor(),showToast("Custom values saved","success"),await refreshMetaCvOverviewSilently()}catch(i){showToast(i.message||"Save failed","error"),setMetaCvSavePending(!1)}})}function bindMetaReportsCustomValuesEvents(e){const t=document.getElementById("meta-cv-year");t&&(t.onchange=async()=>{const a=metaReportsState.selectedYear,s=Number(t.value);if(s!==a){metaReportsState.selectedYear=s,metaReportsState.customValues.selectedClientId=null,metaReportsState.customValues.selectedMonthKey=null,metaReportsState.customValues.editorPayload=null,setMetaCvYearLoading(s,!0);try{await loadMetaReportsCustomValuesPage({silent:!0})}catch(o){metaReportsState.selectedYear=a,t&&(t.value=String(a)),await loadMetaReportsCustomValuesPage({silent:!0,keepSelection:!0}),showToast(o.message||"Load failed","error")}finally{setMetaCvYearLoading(s,!1)}}});const n=document.getElementById("meta-cv-search");n&&(n.oninput=()=>{metaReportsState.customValues.searchQuery=n.value;const a=document.getElementById("meta-cv-client-list");a&&(a.innerHTML=renderMetaCvClientRows(e)),bindMetaCvListEvents()}),bindMetaCvListEvents(),bindMetaCvEditorEvents()}async function loadMetaReportsCustomValuesPage({silent:e=!1,keepSelection:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){window.location.href=`/login?next=${encodeURIComponent(window.location.pathname)}`;return}currentStaffUser=a,!metaReportsState.customValues.mounted&&!e&&(n.innerHTML=`
      ${renderBrandTopbar(renderStaffAdminChrome("meta-reports-custom"))}
      ${wrapDashboardShell('<div class="loading-state"><div class="spinner"></div><p>Loading custom values...</p></div>')}
    `);try{const s=await adminFetch(`/api/meta-reports/custom-values?year=${encodeURIComponent(metaReportsState.selectedYear)}`);metaReportsState.selectedYear=Number(s.year)||metaReportsState.selectedYear,metaReportsState.customValues.overview=s,t||(s.clients||[]).some(r=>r.clientId===metaReportsState.customValues.selectedClientId)||(metaReportsState.customValues.selectedClientId=null,metaReportsState.customValues.selectedMonthKey=null,metaReportsState.customValues.editorPayload=null),n.innerHTML=renderMetaReportsCustomValuesPage(s),metaReportsState.customValues.mounted=!0,metaReportsState.customValues.saving=!1,bindMetaReportsCustomValuesEvents(s)}catch(s){if(!metaReportsState.customValues.mounted)n.innerHTML=`
        ${renderBrandTopbar(renderStaffAdminChrome("meta-reports-custom"))}
        ${wrapDashboardShell(`<div class="error-state" style="padding:24px">${esc(s.message)}</div>`)}
      `;else{if(e)throw s;showToast(s.message||"Load failed","error")}}}function renderAdminHubPage(e){const t=e.length,n=currentStaffUser?`
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",o=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",r=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `}function getSetupProgressSteps(e){return[{id:"metrics",label:"Metrics",done:!!e.metricsModelSetAt},{id:"ghl",label:"GHL",done:!!(e.hasGhlToken&&e.locationId)},{id:"pipelines",label:"Pipelines",done:!!(e.newLeadsPipelineId&&e.salesPipelineId)},{id:"meta",label:"Meta",done:e.metaSyncStatus==="ok",partial:!!(e.metaAdAccountId&&e.metaSyncStatus!=="ok")}]}function renderSetupProgressStrip(e){const t=getSetupProgressSteps(e),n='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6L9 17l-5-5"/></svg>';return`
    <nav class="setup-progress" aria-label="Setup progress">
      ${t.map((a,s)=>{const o=a.done?" is-done":a.partial?" is-partial":"",r=a.done?`<span class="setup-progress-mark">${n}</span>`:`<span class="setup-progress-mark">${s+1}</span>`;return`
          <button type="button" class="setup-progress-step${o}" onclick="scrollToSetupSection('${a.id}')">
            ${r}
      ...
      <div class="checkbox-list">
        ${s.map(([o,r])=>`
          <label class="checkbox-item">
            <input type="checkbox"
              ${isVisible(t,o,a)?"checked":""}
              onchange="toggleDisplay('${t}', '${o}'${a?`, '${a}'`:""})" />
            <span>${r}</span>
          </label>
        `).join("")}
      </div>
  ...
  `:`
    ...
    `}function renderTable(e,t,n,a,s){const o=n.filter(r=>isVisible("columns",r.key,e));return o.length?`
  ...
  `}function renderDashboardContent(e){const{kpis:t,statusBreakdown:n,sourceReport:a,assigneeReport:s,pipelines:o,filters:r}=e,i=isVisible("sections","statusBreakdown");return`
