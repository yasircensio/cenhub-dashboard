...
Resume the interrupted ${y}?`)&&(l=p.id,d=Number(p.batchOffset)||0)}}if(n?setFbLeadProgress({processed:d,total:0,label:d>0?"Resuming preview\u2026":"Fetching Meta leads\u2026"}):updateGlobalApplyProgress({waiting:!0,elapsed:0}),n&&d===0){const p=await adminFetchWithRetry("/api/fb-lead-sync/prepare",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l})}),u=p.summary||p;l=u.runId,d=0,setFbLeadProgress({processed:0,total:u.inWindow||0,label:`Matching ${u.inWindow||0} Meta leads to GHL\u2026`})}else!n&&d===0&&updateGlobalApplyProgress({waiting:!1,processed:0,total:g||w?.updated||0});do{c=await adminFetchWithRetry("/api/fb-lead-sync/run",{method:"POST",body:JSON.stringify({clientId:e,mode:t,dryRun:n,runId:l,previewRunId:n?null:$,batchOffset:d,batchLimit:o})});const p=c.summary||c;l=p.runId,d=p.nextBatchOffset??p.batchOffset+(p.batchProcessed||0),b=b.concat(p.rows||[]),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.runId=l);const u=p.inWindow||d||g;n?(setFbLeadProgress({processed:d,total:u,complete:!p.hasMore}),i&&(i.innerHTML=renderFbLeadRunResults({...p,rows:b}))):updateGlobalApplyProgress({processed:d,total:u,written:p.updated||0,complete:!p.hasMore})}while((c.summary||c).hasMore);h=!0;const k=c.summary||c,f=k.inWindow||d||g,v=k.nextBatchOffset??d;if(n)setFbLeadProgress({processed:v,total:f,complete:!0}),markFbLeadPreviewReady(e,t,k.updated),fbLeadSyncState.activeRun&&(fbLeadSyncState.activeRun.previewOk=(k.updated||0)>0),showToast((k.updated||0)>0?"Preview complete \u2014 use Apply in Run history below.":"Preview complete \u2014 all contacts already have Fb Lead id. Nothing to apply.","success"),await refreshFbLeadHistoryUi(),document.getElementById("fb-lead-history-mount")?.scrollIntoView({behavior:"smooth",block:"nearest"});else{updateGlobalApplyProgress({processed:v,total:f,written:k.updated||0,complete:!0}),delete fbLeadSyncState.previewOkByClient[e];try{const p=JSON.parse(sessionStorage.getItem("fbLeadSyncPreviewOk")||"{}");delete p[e],sessionStorage.setItem("fbLeadSyncPreviewOk",JSON.stringify(p))}catch{}showToast("FB lead sync complete \u2014 Fb Lead id written to GHL","success"),stopGlobalApplyProgress({success:!0})}await refreshFbLeadHistoryUi();const m=await adminFetch("/api/fb-lead-sync");fbLeadSyncState.clients=m.clients||[],hydrateFbLeadPreviewState(fbLeadSyncState.clients),await refreshFbLeadClientRowsUi()}catch(k){n?hideFbLeadProgress(!0):(updateGlobalApplyProgress({error:`Failed \u2014 ${k.message||"Apply failed"}`}),stopGlobalApplyProgress({success:!1,keepVisibleMs:12e3})),showToast((k.message||(n?"FB lead preview failed.":"FB lead apply failed."))+(n?" Partial progress is saved \u2014 use Resume in Run history or try Preview again.":" Partial progress is saved \u2014 use Resume in Run history."),"error"),await refreshFbLeadHistoryUi()}finally{s&&(s.disabled=!1),fbLeadSyncState.applyingRunId=null,!h&&n&&hideFbLeadProgress(!0),!n&&!h&&(fbLeadSyncState.isApplying=!1,await refreshFbLeadHistoryUi())}}async function expandFbLeadRun(e){const t=document.getElementById(`fb-lead-run-audit-${e}`);if(t){if(!t.hidden){t.hidden=!0;return}try{const n=await adminFetch(`/api/fb-lead-sync/history/${e}`),a=fbLeadSyncState.clients.find(i=>i.clientId===n.run.clientId);t.innerHTML=`<td colspan="7"><div class="fb-lead-audit-scroll" id="fb-lead-audit-scroll-${e}">${renderFbLeadAuditTable(n.run.rows||[],n.run.locationId||a?.locationId)}</div></td>`,t.hidden=!1;const s=document.getElementById(`fb-lead-audit-scroll-${e}`);s&&s.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(n){showToast(n.message||"Failed to load run audit.","error")}}}function viewFbLeadClientHistory(e){const t=getFbLeadHistoryRunsForDisplay(fbLeadSyncState.historyRuns.filter(s=>s.clientId===e)),n=document.getElementById("fb-lead-history-mount");if(!n)return;const a=Object.fromEntries(fbLeadSyncState.clients.map(s=>[s.clientId,s.locationId]));n.innerHTML=t.length?renderFbLeadHistoryRows(t,a):`<div class="sync-history-empty">No runs for ${esc(e)} yet.</div>`,n.scrollIntoView({behavior:"smooth",block:"start"})}async function loadFbLeadSyncPage({silent:e=!1,fullPreflight:t=!1}={}){const n=document.getElementById("dashboard");if(!n)return;const a=await fetchStaffMe();if(!a){e||(n.innerHTML=`
    ...
    <div class="meta-report-bottomline-fee-settings meta-report-bottomline-fee-settings--cv">
      <div class="meta-cv-bottomline-panel">
        ...
        <div class="meta-report-fee-nested meta-cv-bottomline-body${s?"":" is-hidden"}" id="${t}-fee-nested" data-fee-mode="${esc(i)}" data-fee-percent="${esc(o)}" data-marketing-fee="${esc(r)}">
          ...
          <div class="meta-report-fee-mode-pills">${h}</div>
          <div class="meta-cv-fee-footer">
            <div class="meta-cv-fee-value-inline" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,i,o,r,{inline:!0})}</div>
            <button type="button" class="admin-btn admin-btn--primary admin-btn--small" id="${t}-fee-save" disabled>Saved</button>
          </div>
        </div>
      </div>
    </div>
  `}const d=describeMetaReportFeeModeLabel(i,o,r),c=META_REPORT_FEE_MODE_META.map(({value:b,icon:h,title:$,desc:w})=>`
        <label class="meta-report-fee-mode-card">
          <input type="radio" name="${t}-fee-mode" value="${esc(b)}"${i===b?" checked":""} />
          <span class="meta-report-fee-mode-card-icon" aria-hidden="true">${esc(h)}</span>
    ...
    <div class="meta-report-bottomline-fee-settings">
      ...
      <div class="meta-report-fee-nested${s?"":" is-hidden"}" id="${t}-fee-nested" data-fee-mode="${esc(i)}" data-fee-percent="${esc(o)}" data-marketing-fee="${esc(r)}">
        <div class="meta-report-fee-status" id="${t}-fee-status" aria-live="polite">
          <span class="meta-report-fee-status-value is-active">${esc(d)}</span>
        </div>
        <div class="meta-report-fee-mode-grid${n?" meta-report-fee-mode-grid--rows":""}">${c}
        </div>
        <div class="meta-report-fee-value" id="${t}-fee-value">${metaReportFeeValueBlockHtml(t,i,o,r,{inline:l})}</div>
  ...
  `}function syncMetaReportBottomlineFeeDom(e={},t="meta-report"){const n=!!e.metaReportShowBottomline,a=resolveMetaReportFeeMode(e)||"",s=e.metaReportFeePercent??20,i=e.metaReportMarketingFeeAmount??0,o=document.getElementById(`${t}-setting-bottomline`);o&&(o.checked=n);const r=document.getElementById(`${t}-fee-nested`);r?.classList.toggle("is-hidden",!n),r&&(r.dataset.feeMode=a,r.dataset.feePercent=String(s),r.dataset.marketingFee=String(i)),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(c=>{c.checked=c.value===a});const l=metaReportUsesInlineFeeValue(t),d=document.getElementById(`${t}-fee-value`);d&&(d.innerHTML=metaReportFeeValueBlockHtml(t,a,s,i,{inline:l}),bindMetaReportFeeValueInputEnterKey(t)),syncMetaReportFeeSelectionState(t)}function bindMetaReportFeeValueInputEnterKey(e){const t=document.getElementById(`${e}-setting-fee-percent`)||document.getElementById(`${e}-setting-marketing-fee`);t&&(t.onkeydown=n=>{n.key==="Enter"&&(n.preventDefault(),document.getElementById(`${e}-fee-save`)?.click())})}function bindMetaReportBottomlineFeeEvents(e,t,n){const a=n||((o,r)=>saveMetaReportClientSettings(e,o,r)),s=document.getElementById(`${t}-setting-bottomline`);s&&(s.onchange=()=>{const o=s.checked;a({metaReportShowBottomline:o},()=>{s.checked=!o})}),document.querySelectorAll(`input[name="${t}-fee-mode"]`).forEach(o=>{o.onchange=()=>{if(!o.checked)return;const r=document.getElementById(`${t}-fee-nested`),l=metaReportUsesInlineFeeValue(t),d=document.getElementById(`${t}-fee-value`);d&&r&&(d.innerHTML=metaReportFeeValueBlockHtml(t,o.value||"",r.dataset.feePercent??20,r.dataset.marketingFee??0,{inline:l}),bindMetaReportFeeValueInputEnterKey(t)),bindMetaReportFeeAmountInputs(t),syncMetaReportFeeSelectionState(t)}}),bindMetaReportFeeValueInputEnterKey(t),bindMetaReportFeeDraftTracking(t);const i=document.getElementById(`${t}-fee-save`);i&&(i.onclick=async()=>{if(i.disabled)return;const o=document.getElementById(`${t}-fee-nested`),r=o?.querySelector(`input[name="${t}-fee-mode"]:checked`),l=r&&r.value||null,d={metaReportFeeMode:l};if(l==="performance"){const b=document.getElementById(`${t}-setting-fee-percent`);let h=Number(b?.value);Number.isFinite(h)||(h=20),h=Math.min(100,Math.max(0,h)),b&&(b.value=String(h)),d.metaReportFeePercent=h}else if(l==="marketing"){const b=document.getElementById(`${t}-setting-marketing-fee`);let h=Number(b?.value);Number.isFinite(h)||(h=0),h=Math.max(0,h),b&&(b.value=String(h)),d.metaReportMarketingFeeAmount=h}const c=i.textContent;i.disabled=!0,i.textContent="Saving\u2026",i.classList.remove("is-dirty");try{await a(d,()=>syncMetaReportBottomlineFeeDom({metaReportShowBottomline:s?s.checked:!0,metaReportFeeMode:o?.dataset.feeMode||null,metaReportFeePercent:Number(o?.dataset.feePercent)||20,metaReportMarketingFeeAmount:Number(o?.dataset.marketingFee)||0},t)),syncMetaReportFeeSelectionState(t)}finally{i.disabled=!1,i.textContent=c,syncMetaReportFeeSelectionState(t)}})}function renderMetaReportSaveIndicator(e,t=!1){return`
  ...
  `}function renderMetaCvEditor(e){const t=metaReportsState.customValues,n=(e.clients||[]).find(g=>g.clientId===t.selectedClientId),a=t.selectedMonthKey;if(!n||!a)return`
    ...
    `;const s=n.months?.[a]||{};if(t.loadingEditor)return renderMetaCvEditorSkeleton(n,a,s);const i=t.editorPayload?.months?.[a]||null,o=n.metaReportShowBottomline,r=s.wonLeads!=null?s.wonLeads:i?.inputs?.wonLeads??i?.topline?.wonLeads??"",l=s.avgLeadValue!=null?s.avgLeadValue:i?.inputs?.avgLeadValue??i?.topline?.avgLeadValue??"",d=s.avgProfitPerWon!=null?s.avgProfitPerWon:i?.inputs?.avgProfitPerWon??"",c=metaReportsState.customValues.draftInputs,b=c?.wonLeads??r,h=c?.avgLeadValue??l,$=c?.avgProfitPerWon??d,w=t.settingsSaving&&t.settingsSavingScope==="report";return`
    ...
    <div class="meta-cv-editor-grid">
      ...
      <label>Won leads
        <input type="number" step="any" id="meta-cv-won-leads" value="${esc(b)}" />
      </label>
    ...
    <div class="meta-report-settings-group meta-report-settings-group--stacked meta-cv-client-settings${w?" is-saving":""}">
      ${renderMetaReportSaveIndicator("meta-cv-settings-status",w)}
...
This removes the account, GHL token, and all synced snapshot data. This cannot be undone.`))return;if(window.prompt(`Type "${e}" to confirm deletion:`)!==e){showToast("Deletion cancelled \u2014 slug did not match.","error");return}try{await adminFetch(`/api/clients/${encodeURIComponent(e)}`,{method:"DELETE"}),showToast(`Deleted ${e}`,"success"),IS_ADMIN_HUB?await loadAdminHub():window.location.href="/admin"}catch(s){showToast(s.message,"error")}}async function syncClient(e,t){t&&(t.disabled=!0,t.textContent="Syncing...");try{await adminFetch(`/api/clients/${encodeURIComponent(e)}/sync`,{method:"POST",body:"{}"}),showToast("Sync completed","success"),IS_ADMIN_HUB?await loadAdminHub():IS_ADMIN_CLIENT?(await loadSetupAccount(),accountCanPreviewDashboard(setupAccount)&&loadDashboard(!0,{background:!0})):loadDashboard(!0,{background:!0})}catch(n){showToast(n.message,"error")}finally{if(t){t.disabled=!1;const n=t.dataset.syncLabel||"Sync";t.innerHTML=t.dataset.syncLabel?`${ICON_SYNC} ${n}`:n}}}function getMetricsModelLabels(e,t=setupPipelines){const n=new Map((t||[]).map(a=>[a.id,a.name]));return e.dedupeEnabled?{typeLabel:"Funnel + deduplication",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId||"\u2014"}`}:e.winPipelineId?{typeLabel:"Simple (single win pipeline)",winLabel:`Win pipeline: ${n.get(e.winPipelineId)||e.winPipelineId}`}:{typeLabel:"Simple (no deduplication)",winLabel:"All won opportunities"}}const ICON_LAYER='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l9 5-9 5-9-5 9-5z"/><path d="M3 13l9 5 9-5"/></svg>',ICON_MERGE='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="18" cy="15" r="2.4"/><path d="M6 8.4V15.6"/><path d="M8.2 6.4C14 6.4 14 12.6 15.8 13.4"/></svg>',ICON_TARGET='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/></svg>',ICON_LOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 018 0v3.5"/></svg>',ICON_UNLOCK='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4.5" y="10.5" width="15" height="9" rx="2"/><path d="M8 10.5V7a4 4 0 017.8-1.2"/></svg>',ICON_TAG='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.6 12.7L12.7 20.6a2 2 0 01-2.8 0l-7-7a2 2 0 010-2.8L10.8 3H19a2 2 0 012 2v7.7z"/><circle cx="15" cy="8" r="1.2"/></svg>',ICON_HASH='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 9h14M5 15h14M10 3L8 21M16 3l-2 18"/></svg>',ICON_EDIT='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>',ICON_WARNING='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9L2.6 18a1.5 1.5 0 001.3 2.2h16.2a1.5 1.5 0 001.3-2.2L13.7 3.9a1.5 1.5 0 00-3.4 0z"/><path d="M12 9v4"/><path d="M12 16.5h.01"/></svg>';function renderMetricsModelPanel(e){const t=getMetricsModelLabels(e),n=!e.metricsModelSetAt||metricsModelChangeMode,a=n?" is-wizard":" is-locked",s=e.dedupeEnabled?"dedupe":e.winPipelineId?"pipeline":"simple",i=e.metricsModelLockedAt?`Locked since ${new Date(e.metricsModelLockedAt).toLocaleDateString("en-GB")}`:"Editable until first successful sync",o=e.metricsModelLockedAt?"metrics-model-badge is-locked":"metrics-model-badge";return n?`
  ...
  `,IS_ADMIN_CLIENT&&loadSetupAccount(),initDatePickers()}let isFetching=!1,pendingRefetch=!1,fetchGeneration=0,activeFetchController=null,fetchStartedAt=0;const FETCH_TIMEOUT_MS=9e4;function restoreDashboardContentAfterFailedFetch(e){!e||!cachedData||(cachedData=applyMarketingData(cachedData,cachedFacebookMetrics),updateDashboardContent(cachedData),updateFilterUi())}function setPresetButtonsDisabled(e){document.querySelectorAll("[data-preset]").forEach(t=>{t.disabled=e})}function resetFetchUiState(){isFetching=!1,setPresetButtonsDisabled(!1);const e=document.getElementById("apply-filters-btn");e&&(e.disabled=!1,e.textContent="Apply data filters")}function cancelActiveFetch(){activeFetchController&&(activeFetchController.abort(),activeFetchController=null)}function canReuseBootstrapDashboard(e){return!!(e?.kpis&&usesClientPipelineDefaults()&&!state.dateFrom&&!state.dateTo&&state.status==="all"&&state.source==="all"&&state.assignedTo==="all")}async function loadDashboard(e=!0,t={}){const{background:n=!1,forceFresh:a=!1}=t,s=!!a,i=document.getElementById("dashboard");if(isFetching&&(e||!cachedData))cancelActiveFetch(),fetchGeneration+=1,pendingRefetch=!1,resetFetchUiState();else if(isFetching){e&&(pendingRefetch=!0);return}const o=!!document.getElementById("dashboard-content"),r=document.getElementById("apply-filters-btn");if(e||!cachedData){const l=++fetchGeneration;activeFetchController=new AbortController;const d=activeFetchController.signal;if(isFetching=!0,fetchStartedAt=Date.now(),!o)i.innerHTML=`
        ...
        `)}`;else if(!n){const c=document.getElementById("dashboard-content");c&&(c.innerHTML=renderKpiSkeleton()),r&&(r.disabled=!0,r.textContent="Loading..."),setPresetButtonsDisabled(!0)}try{let c=null;if(!availablePipelines.length){if(c=await bootstrapDashboardData(d),l!==fetchGeneration)return;availablePipelines=c.filterOptions.pipelines||[],ensurePipelineDefaults(availablePipelines,c.account?.defaultPipelineIds)}if(!ensurePipelineSelectionBeforeFetch())throw new Error("Select at least one pipeline.");const h=canReuseBootstrapDashboard(c)&&!s;let $,w;if(h?[w,$]=await Promise.all([fetchFacebookMetrics(d),Promise.resolve(c)]):[$,w]=await Promise.all([fetchDashboardData(availablePipelines,d,{forceFresh:s}),fetchFacebookMetrics(d)]),l!==fetchGeneration)return;cachedFacebookMetrics=w,cachedData=applyMarketingData($,w),cachedData.account?.facebookClientId?facebookClientId=cachedData.account.facebookClientId:cachedData.account?.clientId&&(facebookClientId=cachedData.account.clientId),availablePipelines=cachedData.filterOptions.pipelines||availablePipelines,ensurePipelineDefaults(availablePipelines,cachedData?.account?.defaultPipelineIds),lastFetchedAt=cachedData.cachedAt?new Date(cachedData.cachedAt).getTime():Date.now()}catch(c){if(l!==fetchGeneration)return;if(c.name==="AbortError"){o?restoreDashboardContentAfterFailedFetch(o):i.innerHTML=`
            ${renderBrandTopbar(IS_ADMIN?'<a class="admin-topbar-link" href="/admin">Admin hub</a>':"")}
            ${wrapDashboardShell(`
