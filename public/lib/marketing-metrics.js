...
function getPeriodLabel(preset, timeZone = DEFAULT_TIMEZONE) {
  if (preset === 'month') {
    return formatMonthYearLabel(`${getCurrentMonthKey(timeZone)}-01`, timeZone);
  }
  if (preset === 'lastMonth') {
    return formatMonthYearLabel(`${getPreviousMonthKey(timeZone)}-01`, timeZone);
  }
  if (preset === 'year') {
    return getCurrentMonthKey(timeZone).slice(0, 4);
  }
...
function getPeriodLabelForRange(dateFrom, dateTo, timeZone = DEFAULT_TIMEZONE) {
  ...
  if (startYear === endYear && dateFrom === `${startYear}-01-01` && dateTo === `${startYear}-12-31`) {
    return startYear;
  }
...
function enrichKpisWithMarketing(kpis, fbMetrics, preset, options = {}) {
  ...
  if (preset === 'custom' && dateFrom && dateTo) {
    ...
    adSpendLabel = getPeriodLabel(preset, timeZone);
