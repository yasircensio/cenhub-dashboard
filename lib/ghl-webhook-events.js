...
const OPPORTUNITY_CREATE_EVENTS = ['OpportunityCreate', 'opportunity.create'];

async function countOpportunityCreates24hByClient() {
  const since24h = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();

  if (usePostgres()) {
    try {
      const rows = await query`
        SELECT client_id, COUNT(*)::int AS opportunities_created
        FROM ghl_webhook_events
        WHERE received_at >= ${since24h}
          AND client_id IS NOT NULL
          AND event_type IN ('OpportunityCreate', 'opportunity.create')
        GROUP BY client_id
      `;
      return rows.map((row) => ({
        clientId: row.client_id,
        opportunitiesCreated: Number(row.opportunities_created) || 0,
      }));
    } catch (error) {
  ...
  for (const entry of store.ghlWebhookEvents || []) {
    if (String(entry.received_at || '') < since24h) continue;
    if (!OPPORTUNITY_CREATE_EVENTS.includes(String(entry.event_type || ''))) continue;
    if (!entry.client_id) continue;
    grouped.set(entry.client_id, (grouped.get(entry.client_id) || 0) + 1);
  }
  return [...grouped.entries()].map(([clientId, opportunitiesCreated]) => ({
    clientId,
...
async function getGhlWebhookHealthSummary() {
  ...
  try {
    ...
    `;
    const row = rows[0] || {};
    const stuckReceived = row.stuck_received_count || 0;
    return {
      configured: true,
      ok: (row.failed_count || 0) === 0 && stuckReceived === 0,
      failedLast24h: row.failed_count || 0,
      processedLast24h: row.processed_count || 0,
      receivedLast24h: row.received_count || 0,
      stuckReceivedLast24h: stuckReceived,
