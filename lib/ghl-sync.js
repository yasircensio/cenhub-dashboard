...
async function fetchOpportunityById(token, opportunityId, locationId) {
  const data = await apiFetch(token, `/opportunities/${encodeURIComponent(opportunityId)}`);
  const opportunity = data.opportunity || data;
  if (!opportunity?.id) {
    throw new Error(`GHL returned no opportunity for id "${opportunityId}".`);
  }

  let customFieldDefinitions = [];
  if (locationId) {
    customFieldDefinitions = await fetchOpportunityCustomFieldDefinitions(token, locationId).catch(() => []);
  }

  const [enriched] = enrichOpportunityCustomFields([opportunity], customFieldDefinitions);
