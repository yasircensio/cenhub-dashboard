...
function isAllowedAdsCheckDownloadUrl(rawUrl) {
  ...
  return parsedUrl.protocol === 'https:' && isMetaCdnHost;
}
...
async function handleMetaReportsRequest(request, response) {
  ...
  try {
    ...
    if (pathInfo.kind === 'ads-check') {
      ...
      const result = await fetchAdAccountCreatives(account.metaAdAccountId, token, { activeOnly });
    ...
    if (pathInfo.kind === 'ads-check-download') {
      ...
      try {
        ...
        sendJson(response, 502, { error: error.message || 'Failed to reach Meta CDN.' });
        return;
      }
      if (!upstream.ok) {
        sendJson(response, 502, { error: `Meta CDN returned HTTP ${upstream.status}.` });
        return;
      }
      const buffer = Buffer.from(await upstream.arrayBuffer());
      const contentType = upstream.headers.get('content-type') || 'application/octet-stream';
      const safeFilename = (String(query.filename || 'meta-ad-file').replace(/[^a-zA-Z0-9._-]+/g, '_').slice(0, 120)) || 'meta-ad-file';
      const headers = {
        'Content-Type': contentType,
        'Content-Disposition': `attachment; filename="${safeFilename}"`,
        'Content-Length': String(buffer.length),
      };
      // Wait for the write to actually flush before returning — on Vercel's
      // serverless runtime the execution container can freeze as soon as the
      // handler's promise resolves, which was cutting off larger (video)
      // downloads that hadn't fully flushed yet while small images happened
      // to finish in time.
      await new Promise((resolve, reject) => {
        ...
        response.end(buffer, (error) => (error ? reject(error) : resolve()));
      });
      return;
    }
