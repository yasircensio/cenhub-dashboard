...
async function handleMetaReportsRequest(request, response) {
  ...
  try {
    ...
    if (pathInfo.kind === 'month-save') {
      if (method === 'GET') {
        // #region agent log (debug session b952dc, hypotheses H1-H5: request timing breakdown)
        global.__cvDebugTiming = [];
        const __dbgReqStart = Date.now();
        // #endregion agent log
        await requireStaffSession(request);
        const monthPayload = await buildSingleMonthPayload(pathInfo.clientId, pathInfo.monthKey, {
          includeUnpublished: true,
        });
        // #region agent log (debug session b952dc, hypotheses H1-H5)
        const __dbgTiming = {
          totalMs: Date.now() - __dbgReqStart,
          vercelRegion: process.env.VERCEL_REGION || null,
          queries: global.__cvDebugTiming,
        };
        global.__cvDebugTiming = null;
        sendJson(response, 200, { monthPayload, _debugTiming: __dbgTiming });
        return;
        // #endregion agent log
      }
      await requireStaffSession(request);
