...
const { applyStoredVideoUrls, getStoredVideo, hasBlobStore, listStoredVideos } = require('./meta-ads-blob');
...
async function handleMetaReportsRequest(request, response) {
  ...
  try {
    ...
    if (pathInfo.kind === 'ads-check') {
      ...
      const storedByVideoId = await storedPromise;
      result.creatives = applyStoredVideoUrls(result.creatives, storedByVideoId);
