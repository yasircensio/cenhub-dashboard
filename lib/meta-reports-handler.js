  ...
  try {
    ...
    if (pathInfo.kind === 'client-settings') {
      ...
      if (method === 'PATCH' || method === 'PUT') {
        ...
        if (body.monthKey) {
          const year = String(body.monthKey).slice(0, 4);
          const yearPayload = await buildClientYearPayload(pathInfo.clientId, year, {
            includeUnpublished: true,
          });
          monthPayload = yearPayload.months?.[body.monthKey] || null;
        }
    ...
    if (pathInfo.kind === 'month-save') {
      ...
      try {
        ...
        fs.appendFileSync(path.join(__dirname, '..', '.cursor', 'debug-b952dc.log'), `${JSON.stringify({
          ...
          runId: 'pre-fix',
          hypothesisId: 'B',
          location: 'lib/meta-reports-handler.js:month-save',
          message: 'month-save request body',
          data: {
            clientId: pathInfo.clientId,
            monthKey: pathInfo.monthKey,
            wonLeads: body.wonLeads,
            avgLeadValue: body.avgLeadValue,
            avgProfitPerWon: body.avgProfitPerWon,
            lineItemCount: Array.isArray(body.lineItems) ? body.lineItems.length : null,
          },
          timestamp: Date.now(),
        })}\n`);
      } catch {
      ...
      try {
        ...
        fs.appendFileSync(path.join(__dirname, '..', '.cursor', 'debug-b952dc.log'), `${JSON.stringify({
          ...
          hypothesisId: 'B',
          location: 'lib/meta-reports-handler.js:month-save',
          message: 'month-save persisted record',
          data: {
            clientId: pathInfo.clientId,
            monthKey: pathInfo.monthKey,
            wonLeads: month.wonLeads,
            avgLeadValue: month.avgLeadValue,
            avgProfitPerWon: month.avgProfitPerWon,
            lineItemCount: month.lineItems?.length || 0,
          },
          timestamp: Date.now(),
        })}\n`);
      } catch {
