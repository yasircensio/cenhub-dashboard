...
const { attachStoredVideos } = require('./meta-ads-blob');
...
async function handleMetaReportsRequest(request, response) {
  ...
  try {
    ...
    if (pathInfo.kind === 'ads-check') {
      ...
      const stored = await attachStoredVideos(account.clientId, result.creatives, { archiveLimit: 2 });
      result.creatives = stored.creatives;
      ...
      sendJson(response, 200, {
        ...
        blobConfigured: stored.blobConfigured,
        archivedNow: stored.archivedNow,
      });
      return;
    }
