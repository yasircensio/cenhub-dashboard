...
async function handleMetaReportsRequest(request, response) {
  ...
  try {
    ...
    if (pathInfo.kind === 'ads-check') {
      ...
      if (query.debugLibrary) {
        const { GRAPH_VERSION, graphFetch } = require('./meta-token');
        const acctId = String(account.metaAdAccountId || '').replace(/^act_/i, '');
        try {
          const images = await graphFetch(
            `https://graph.facebook.com/${GRAPH_VERSION}/act_${acctId}/adimages?fields=hash,name,url,permalink_url&limit=3`,
            token,
          );
          const videos = await graphFetch(
            `https://graph.facebook.com/${GRAPH_VERSION}/act_${acctId}/advideos?fields=id,title,source,picture,length&limit=3`,
            token,
          );
          libraryProbe = {
            imageKeys: images.data?.[0] ? Object.keys(images.data[0]) : [],
            imageHasUrl: Boolean(images.data?.[0]?.url),
            imageCount: images.data?.length || 0,
            videoKeys: videos.data?.[0] ? Object.keys(videos.data[0]) : [],
            videoHasSource: Boolean(videos.data?.[0]?.source),
            videoCount: videos.data?.length || 0,
          };
        } catch (error) {
          libraryProbe = { error: error.message };
        }
      }
      sendJson(response, 200, {
        ok: true,
