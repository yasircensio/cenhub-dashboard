...
async function handleMetaReportsRequest(request, response) {
  ...
  try {
    ...
    if (pathInfo.kind === 'ads-check-media') {
      const debugAdsCheckLog = (hypothesisId, message, data) => {
        const payload = {
          sessionId: 'f182bb',
          runId: 'pre-fix',
          hypothesisId,
          location: 'lib/meta-reports-handler.js:ads-check-media',
          message,
          data,
          timestamp: Date.now(),
        };
        // #region agent log
        try {
          require('fs').appendFileSync('/Users/apple/GHL Integration/.cursor/debug-f182bb.log', `${JSON.stringify(payload)}\n`);
        } catch {
          // Local log file is only available on this machine.
        }
        fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': 'f182bb' },
      ...
      try {
        ...
        debugAdsCheckLog('A', 'media unauthorized', {
          clientId: pathInfo.clientId,
          videoId: pathInfo.videoId || null,
