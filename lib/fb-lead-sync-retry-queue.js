...
let workerHeartbeatsTableEnsured = false;

async function ensureRetryWorkerHeartbeatsTable() {
  if (!usePostgres() || workerHeartbeatsTableEnsured) return;
  await query`
  ...
  `;
  workerHeartbeatsTableEnsured = true;
}

async function recordRetryWorkerHeartbeat({
  processed = 0,
  updated = 0,
  failed = 0,
  stillPending = 0,
  ...
  await ensureRetryWorkerHeartbeatsTable();
  await query`
    INSERT INTO fb_lead_retry_worker_heartbeats (processed, updated, failed, still_pending)
    VALUES (${processed}, ${updated}, ${failed}, ${stillPending})
  `;
  const cutoff = new Date(Date.now() - 48 * 60 * 60 * 1000).toISOString();
  await query`
    DELETE FROM fb_lead_retry_worker_heartbeats WHERE ran_at < ${cutoff}
  `;
}
