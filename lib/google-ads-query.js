...
async function searchGoogleAds(customerId, query, options = {}) {
  ...
  const loginCustomerId = options.loginCustomerId || config.loginCustomerId;
  const response = await fetch(url, {
    method: 'POST',
    headers: buildGoogleAdsHeaders(accessToken, config, loginCustomerId),
    body: JSON.stringify({ query: gaql }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    const { getGoogleAdsConfigDebug } = require('./google-ads-config');
    // #region agent log
    fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f182bb'},body:JSON.stringify({sessionId:'f182bb',runId:'pre-fix',hypothesisId:'C',location:'lib/google-ads-query.js:searchGoogleAds',message:'searchGoogleAds failed',data:{status:response.status,customerId:normalizeGoogleAdsCustomerId(customerId),apiVersion:config.apiVersion,configDebug:getGoogleAdsConfigDebug(config),errorMeta:extractGoogleAdsErrorMeta(data)},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
...
async function listAccessibleCustomerIds(options = {}) {
  ...
  const requestMeta = {
    ...
    developerTokenHeaderLength: String(headers['developer-token'] || '').length,
    authorizationLength: String(headers.Authorization || '').length,
    hasBearerPrefix: String(headers.Authorization || '').startsWith('Bearer '),
  };
  const response = await fetch(url, { method: 'GET', headers });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    const { getGoogleAdsConfigDebug, readEnv } = require('./google-ads-config');
    requestMeta.googleRequestId = response.headers.get('request-id') || response.headers.get('x-goog-request-id') || null;
    const mccId = normalizeGoogleAdsCustomerId(readEnv('GOOGLE_ADS_TEST_CUSTOMER_ID')) || '9103268801';
    let mccSearchProbe = null;
    try {
      const probeUrl = buildGoogleAdsSearchUrl(mccId, config);
      const probeHeaders = buildGoogleAdsHeaders(accessToken, config, mccId);
      ...
      mccSearchProbe = {
        ...
        ok: probeResponse.ok,
        googleRequestId: probeResponse.headers.get('request-id') || probeResponse.headers.get('x-goog-request-id') || null,
        errorMeta: probeResponse.ok ? null : extractGoogleAdsErrorMeta(probeData),
      };
    } catch (probeError) {
      mccSearchProbe = { customerId: mccId, ok: false, error: probeError.message || 'probe failed' };
    }
    // #region agent log
    fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f182bb'},body:JSON.stringify({sessionId:'f182bb',runId:'mcc-probe',hypothesisId:'L',location:'lib/google-ads-query.js:listAccessibleCustomerIds',message:'listAccessibleCustomers failed',data:{status:response.status,apiVersion:config.apiVersion,requestMeta,mccSearchProbe,configDebug:getGoogleAdsConfigDebug(config),errorMeta:extractGoogleAdsErrorMeta(data)},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    const error = new Error(parseGoogleAdsErrorBody(data, response.status));
    error.statusCode = response.status;
    error.details = data;
    ...
    error.mccSearchProbe = mccSearchProbe;
    throw error;
  }
