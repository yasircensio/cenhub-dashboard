...
async function listGoogleAdsMccCustomers(options = {}) {
  ...
  if (config.loginCustomerId) {
    try {
      const rows = await searchGoogleAds(config.loginCustomerId, `
SELECT
  customer_client.id,
...
`.trim(), options);
      rows.forEach((row) => {
        const account = customerClientRowToAccount(row);
        if (!account || account.manager || seen.has(account.googleCustomerId)) return;
        if (account.googleCustomerId === config.loginCustomerId) return;
        seen.add(account.googleCustomerId);
        accounts.push(account);
      });
    } catch {
      // Fall through to listAccessibleCustomers if MCC listing fails.
    }
  }
  ...
  if (!accounts.length) {
    ...
    for (const id of ids) {
      if (seen.has(id) || id === config.loginCustomerId) continue;
      try {
        const account = await fetchGoogleAdsCustomerName(id, options);
