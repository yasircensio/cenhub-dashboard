...
async function listAccessibleCustomerIds(options = {}) {
  ...
  if (!response.ok) {
    const { getGoogleAdsConfigDebug } = require('./google-ads-config');
    // #region agent log
    fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f182bb'},body:JSON.stringify({sessionId:'f182bb',runId:'token-shape',hypothesisId:'J',location:'lib/google-ads-query.js:listAccessibleCustomerIds',message:'listAccessibleCustomers failed',data:{status:response.status,apiVersion:config.apiVersion,requestMeta,configDebug:getGoogleAdsConfigDebug(config),errorMeta:extractGoogleAdsErrorMeta(data)},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    const error = new Error(parseGoogleAdsErrorBody(data, response.status));
    error.statusCode = response.status;
    error.details = data;
