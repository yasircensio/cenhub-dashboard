...
async function searchGoogleAds(customerId, query, options = {}) {
  ...
  const targetCustomerId = normalizeGoogleAdsCustomerId(customerId);
  const loginIdUsed = normalizeGoogleAdsCustomerId(loginCustomerId);
  // #region agent log
  fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f182bb'},body:JSON.stringify({sessionId:'f182bb',runId:'pre-fix',hypothesisId:'A',location:'lib/google-ads-query.js:searchGoogleAds',message:'search headers',data:{targetCustomerId,hasLoginCustomerIdHeader:Boolean(headers['login-customer-id']),loginIdUsed,loginMatchesTarget:Boolean(loginIdUsed&&loginIdUsed===targetCustomerId),configHasLoginCustomerId:Boolean(config.loginCustomerId),sentHeaderNames:Object.keys(headers)},timestamp:Date.now()})}).catch(()=>{});
  // #endregion
  ...
  if (!response.ok) {
    // #region agent log
    fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f182bb'},body:JSON.stringify({sessionId:'f182bb',runId:'pre-fix',hypothesisId:'A',location:'lib/google-ads-query.js:searchGoogleAds',message:'search failed',data:{targetCustomerId,loginIdUsed,status:response.status,errorMeta:extractGoogleAdsErrorMeta(data)},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
    ...
    error.searchDebug = {
      targetCustomerId,
      hasLoginCustomerIdHeader: Boolean(headers['login-customer-id']),
      loginIdUsed,
      loginMatchesTarget: Boolean(loginIdUsed && loginIdUsed === targetCustomerId),
      configHasLoginCustomerId: Boolean(config.loginCustomerId),
    };
    throw error;
  }
  return data.results || [];
...
async function resolveGoogleAdsLoginCustomerId(config, options = {}) {
  ...
  // #region agent log
  fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f182bb'},body:JSON.stringify({sessionId:'f182bb',runId:'post-fix',hypothesisId:'A',location:'lib/google-ads-query.js:resolveGoogleAdsLoginCustomerId',message:'resolved MCC login customer',data:{resolved:cachedManagerLoginCustomerId,source,accessibleCount:ids.length},timestamp:Date.now()})}).catch(()=>{});
  // #endregion
