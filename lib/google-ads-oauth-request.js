...
function readNamedQueryValue(query, name) {
  if (!query || typeof query !== 'object') return undefined;
  try {
    if (typeof query.get === 'function') {
      const fromGet = query.get(name);
      if (fromGet != null && fromGet !== '') return fromGet;
    }
  } catch {
    // ignore
  }
  try {
    ...
    if (Array.isArray(value) && value[0] != null && value[0] !== '') return value[0];
...
function collectVercelQuery(request = {}) {
  ...
  const knownKeys = ['path', 'code', 'state', 'error', 'scope', 'iss', 'authuser', 'prompt', 'hd', 'session_state', 'google_ads_oauth'];
  if (query && typeof query === 'object') {
    for (const key of knownKeys) {
      const value = readNamedQueryValue(query, key);
    ...
    try {
      ...
      // ignore non-enumerable query objects
    }
  }
