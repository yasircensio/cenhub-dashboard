...
async function buildClientYearPayload(clientId, year) {
  const client = await getGoogleAdsReportClient(clientId);
  if (!client) {
  ...
  monthKeys.forEach((monthKey, index) => {
    const payload = buildMonthPayload(client, records[index]);
