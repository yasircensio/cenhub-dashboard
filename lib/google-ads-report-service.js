...
const { resolveEffectiveGoogleAdsLeads } = require('./google-ads-leads-source');
...
function clientSettings(client) {
  ...
  return {
    ...
    googleAdsReportLeadsSource: fields.googleAdsReportLeadsSource,
    googleCustomerId: fields.googleCustomerId,
    googleCustomerLabel: fields.googleCustomerLabel,
    linkedMetaClientId: fields.linkedMetaClientId || null,
  };
}
...
function buildMonthPayload(client, monthRecord) {
  ...
  const rawGoogleConversions = periodAligned && monthRecord.googleConversions != null
    ? Number(monthRecord.googleConversions)
    : null;
  const resolvedLeads = resolveEffectiveGoogleAdsLeads({
    googleConversions: rawGoogleConversions,
    manualLeads: monthRecord.manualLeads,
  }, settings);
  const conversions = resolvedLeads.leads != null ? Number(resolvedLeads.leads) || 0 : 0;
  const googleConversions = rawGoogleConversions != null ? rawGoogleConversions : 0;
  ...
  return {
    ...
    ...metrics,
    leadsSource: resolvedLeads.mode,
    googleConversions: rawGoogleConversions,
    manualLeads: resolvedLeads.manualLeads,
    leadsOverridden: resolvedLeads.overridden,
    wonLeads: monthRecord.wonLeads,
    avgLeadValue: monthRecord.avgLeadValue,
    avgProfitPerWon: monthRecord.avgProfitPerWon,
  };
...
async function buildClientYearPayload(clientId, year, { includeUnpublished = true } = {}) {
  ...
  return attachLinkedPair(payload, { googleClientId: client.clientId });
}
