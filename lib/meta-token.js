...
function appendAccessTokenToUrl(url, accessToken) {
...
// META_SYSTEM_USER_TOKEN_V2 is used when set. Otherwise the old
// META_SYSTEM_USER_TOKEN is used, unchanged. Nothing else changes.
function resolveMetaEnvToken() {
  return process.env.META_SYSTEM_USER_TOKEN_V2 || process.env.META_SYSTEM_USER_TOKEN || '';
}
...
function resolveMetaAccessToken(account) {
  ...
  return {
    ...
    source: 'none',
    hint: null,
    reason: envCheck.reason || accountCheck.reason || 'Missing Meta system user token (set META_SYSTEM_USER_TOKEN_V2, META_SYSTEM_USER_TOKEN, or a per-client override).',
    ignoredAccountOverride: Boolean(accountToken),
  };
}
