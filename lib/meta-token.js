...
async function verifyMetaAccessToken(accessToken) {
  ...
  const url = `https://graph.facebook.com/${GRAPH_VERSION}/me/adaccounts?fields=account_id,name&limit=1`;
  try {
...
function resolveMetaAccessToken(account) {
  ...
  if (envCheck.ok) {
    return {
      ...
      accountOverrideIssue: accountCheck.ok ? null : accountCheck.reason,
    };
  }
  ...
  return {
    ...
    source: 'none',
    hint: null,
    reason: accountCheck.reason || envCheck.reason || 'Missing Meta system user token (set META_SYSTEM_USER_TOKEN or per-client override).',
    ignoredAccountOverride: Boolean(accountToken),
  };
}
