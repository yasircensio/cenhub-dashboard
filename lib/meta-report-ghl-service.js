...
async function syncGhlMonthsFromSnapshotSafe(clientId, monthKeys, {
  overwriteManual = false,
} = {}) {
  const synced = [];
  const skipped = [];
  const errors = [];
  const uniqueKeys = [...new Set((monthKeys || []).filter(Boolean))];

  for (const monthKey of uniqueKeys) {
    try {
      await syncGhlMonthFromSnapshot(clientId, monthKey, { overwriteManual });
      synced.push(monthKey);
    } catch (error) {
      if (error.statusCode === 409 && error.code === 'manual_override') {
        skipped.push({ monthKey, reason: 'manual_override' });
        continue;
      }
      if (error.statusCode === 404) {
        skipped.push({ monthKey, reason: 'no_data' });
        continue;
      }
      errors.push({ monthKey, message: error.message });
    }
...
async function autoSyncMetaReportGhlAfterSnapshot(clientId) {
  const account = await getAccount(clientId);
  if (!account?.metaReportEnabled || !account?.metaReportGhlDataEnabled) {
    return { skipped: true, reason: 'not_configured' };
  }
  if (account.metaReportToplineMode !== 'cenhub') {
    return { skipped: true, reason: 'meta_mode' };
  }

  const year = Number(getCurrentMonthKey(account.timezone).slice(0, 4));
  return syncGhlYearFromSnapshot(clientId, year, {
    skipManual: true,
    overwriteManual: false,
  });
}
...
function shouldListOnMetaCenhubPage(account) {
  return Boolean(account?.cenhubDashboard);
}
...
async function getGhlClientsPageData() {
  ...
  for (const client of clients) {
    ...
    if (!shouldListOnMetaCenhubPage(account)) continue;
...
async function setGhlListMembership(clientId, enabled) {
  ...
  if (enabled && !account.cenhubDashboard) {
    const error = new Error('This is not a Cenhub GHL client. Add it from the Clients hub first.');
    error.statusCode = 400;
    throw error;
  }
