...
} = require('./google-ads-oauth-handler');
...
function parseAuthPath(urlPath = '') {
  ...
  if (segments[0] === 'google-ads' && segments[1] === 'start' && segments.length === 2) {
    return { kind: 'google-ads-oauth-start' };
  }
  if (segments[0] === 'google-ads' && segments[1] === 'callback' && segments.length === 2) {
    return { kind: 'google-ads-oauth-callback' };
  }
  return { kind: 'unknown' };
}
...
async function handleAuthRequest(request, response) {
  const { wrapAuthRequest } = require('./google-ads-oauth-request');
  const pathname = String(request.url || request.path || '/api/auth').split('?')[0];
  request = wrapAuthRequest(request, pathname);
  ...
  try {
    ...
    if (pathInfo.kind === 'google-ads-oauth-start') {
      if (method !== 'GET') {
        sendJson(response, 405, { error: 'Method not allowed' });
        return;
      }
    ...
    if (pathInfo.kind === 'google-ads-oauth-callback') {
