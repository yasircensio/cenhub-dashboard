  ...
  if (usePostgres()) {
    await query`
      INSERT INTO accounts (
        ...
        metrics_model_changed_at, metrics_model_version, ready_for_ghl, created_at, updated_at
      ) VALUES (
        ...
        ${record.ready_for_ghl}, ${record.created_at}, ${record.updated_at}
      )
    `;
    await query`
...
async function updateAccount(clientId, input) {
  ...
  if (usePostgres()) {
    const currentRows = await query`SELECT ghl_token_encrypted FROM accounts WHERE client_id = ${existing.clientId} LIMIT 1`;
    const currentEncrypted = currentRows[0]?.ghl_token_encrypted || '';
    const tokenValue = ghlTokenEncrypted !== undefined ? ghlTokenEncrypted : currentEncrypted;
  ...
  store.accounts[existing.clientId] = { ...prev, ...updated };
  writeFileStore(store);
...
async function recoverStuckSyncStates(maxAgeMs = 10 * 60 * 1000) {
  ...
  try {
    if (usePostgres()) {
      const rows = await query`
        SELECT client_id, fetched_at, sync_started_at
        FROM sync_snapshots
        WHERE sync_status = 'syncing'
      `;
      for (const row of rows) {
        const startedAt = row.sync_started_at;
        if (startedAt && startedAt > threshold) continue;
        const nextStatus = row.fetched_at ? 'success' : 'error';
        const nextError = row.fetched_at
          ? null
          : 'Background sync did not finish. Click Sync again.';
        await query`
          UPDATE sync_snapshots
          SET sync_status = ${nextStatus}, sync_error = ${nextError}, sync_started_at = NULL
    ...
    if (/sync_started_at/.test(String(error.message || ''))) {
      ...
      for (const row of rows) {
        ...
        await query`
          UPDATE sync_snapshots
          SET sync_status = ${nextStatus}, sync_error = ${nextError}
          WHERE client_id = ${row.client_id}
        `;
        recovered += 1;
      }
  ...
  for (const [clientId, snapshot] of Object.entries(store.snapshots || {})) {
    ...
    const startedAt = snapshot.sync_started_at;
    if (startedAt && startedAt > threshold) continue;
    snapshot.sync_status = snapshot.fetched_at ? 'success' : 'error';
    snapshot.sync_error = snapshot.fetched_at
      ? null
      : 'Background sync did not finish. Click Sync again.';
    snapshot.sync_started_at = null;
    recovered += 1;
  }
  if (recovered) writeFileStore(store);
  return recovered;
}
