...
const RESERVED_SLUGS = new Set([
  ...
  'team', 'staff', 'users', 'settings', 'fb-lead-sync', 'sync-history',
  'meta-reports', 'google-ads', 'report',
...
function resolveMetaSystemUserToken(account) {
  ...
  return resolveMetaAccessToken(account).token;
}

function accountHasSavedMetaSystemUserToken(row) {
  return Boolean(String(row?.meta_system_user_token_encrypted || '').trim());
}

function accountHasMetaSystemUserToken(row) {
  return accountHasSavedMetaSystemUserToken(row)
    || Boolean(String(process.env.META_SYSTEM_USER_TOKEN || '').trim());
}

function resolveCenhubDashboardFlag(row = {}) {
  if (row.cenhub_dashboard != null) return Boolean(row.cenhub_dashboard);
  if (row.cenhubDashboard != null) return Boolean(row.cenhubDashboard);
  return Boolean(String(row.location_id || row.locationId || '').trim())
    || Boolean(String(row.ghl_token_encrypted || row.ghlToken || '').trim());
}

function filterCenhubHubClients(clients, filter = 'all', searchQuery = '') {
  let list = Array.isArray(clients) ? clients.slice() : [];
  if (filter === 'live') list = list.filter((client) => client.status === 'ready');
  if (filter === 'needs-setup') list = list.filter((client) => client.status && client.status !== 'ready');
  const query = String(searchQuery || '').trim().toLowerCase();
  if (query) {
    list = list.filter((client) => {
      const haystack = [client.accountName, client.clientId].filter(Boolean).join(' ').toLowerCase();
...
function rowToAccount(row, { includeSecrets = false } = {}) {
  ...
  const account = {
    ...
    cenhubDashboard: resolveCenhubDashboardFlag(normalized),
    ...
    hasSavedMetaSystemUserToken: accountHasSavedMetaSystemUserToken(normalized),
    hasEnvMetaSystemUserToken: Boolean(String(process.env.META_SYSTEM_USER_TOKEN || '').trim()),
    ...
    fbLeadSyncEnabled: Boolean(normalized.fb_lead_sync_enabled),
    ghlFbLeadFieldId: normalized.ghl_fb_lead_field_id || null,
    metaReportEnabled: Boolean(normalized.meta_report_enabled),
    metaReportShowBottomline: Boolean(normalized.meta_report_show_bottomline),
    metaReportFeeEnabled: Boolean(normalized.meta_report_fee_enabled),
    metaReportFeePercent: normalized.meta_report_fee_percent != null
      ? Number(normalized.meta_report_fee_percent)
      : 20,
    metaReportFeeMode: normalized.meta_report_fee_mode || null,
    metaReportMarketingFeeAmount: normalized.meta_report_marketing_fee_amount != null
      ? Number(normalized.meta_report_marketing_fee_amount)
      : 0,
    metaReportShowOther: normalized.meta_report_show_other != null
      ? Boolean(normalized.meta_report_show_other)
      : true,
    metaReportTableColumns: Number(normalized.meta_report_table_columns) === 2 ? 2 : 1,
    metaReportSpendChartType: require('./meta-report-chart-type').normalizeMetaReportSpendChartType(
      normalized.meta_report_spend_chart_type,
    ),
    metaReportScenarioMonthWindow: require('./meta-report-scenario-settings').normalizeMetaReportScenarioMonthWindow(
      normalized.meta_report_scenario_month_window,
    ),
    metaReportScenarioSmoothUneven: require('./meta-report-scenario-settings').normalizeScenarioPillValue(
      normalized.meta_report_scenario_smooth_uneven, true,
    ),
    metaReportScenarioBlendHistory: require('./meta-report-scenario-settings').normalizeScenarioPillValue(
      normalized.meta_report_scenario_blend_history, false,
    ),
    metaReportScenarioIncludeTrend: require('./meta-report-scenario-settings').normalizeScenarioPillValue(
      normalized.meta_report_scenario_include_trend, false,
    ),
    metaReportScenarioCautionStrongMonths: require('./meta-report-scenario-settings').normalizeScenarioPillValue(
      normalized.meta_report_scenario_caution_strong_months, false,
    ),
    metaReportBudgetMultiplier: require('./meta-report-scenario-settings').normalizeMetaReportBudgetMultiplier(
      normalized.meta_report_budget_multiplier,
    ),
    metaReportBudgetBaseline: require('./meta-report-scenario-settings').normalizeMetaReportBudgetBaseline(
      normalized.meta_report_budget_baseline,
    ),
    metaReportSlug: normalized.meta_report_slug || null,
    metaReportAccessToken: normalized.meta_report_access_token || null,
    metaReportDefaultWonLeads: normalized.meta_report_default_won_leads != null
      ? Number(normalized.meta_report_default_won_leads)
      : null,
    metaReportDefaultAvgLeadValue: normalized.meta_report_default_avg_lead_value != null
      ? Number(normalized.meta_report_default_avg_lead_value)
      : null,
    metaReportDefaultAvgProfitPerWon: normalized.meta_report_default_avg_profit_per_won != null
      ? Number(normalized.meta_report_default_avg_profit_per_won)
      : null,
    metaReportGhlDataEnabled: Boolean(normalized.meta_report_ghl_data_enabled),
    metaReportToplineMode: normalized.meta_report_topline_mode === 'cenhub' ? 'cenhub' : 'meta',
    metaReportExcelSheetUrl: normalized.meta_report_excel_sheet_url || null,
...
function toPublicSummary(account, snapshot = null) {
  ...
  return {
    ...
    cenhubDashboard: Boolean(account.cenhubDashboard),
...
async function listAccounts({ cenhubDashboardOnly = false } = {}) {
  await recoverStuckSyncStates();
  if (usePostgres()) {
    const rows = cenhubDashboardOnly
      ? await query`SELECT * FROM accounts WHERE cenhub_dashboard = TRUE ORDER BY account_name ASC`
      : await query`SELECT * FROM accounts ORDER BY account_name ASC`;
    const summaries = [];
    for (const row of rows) {
      const account = rowToAccount(row);
      if (cenhubDashboardOnly && !account.cenhubDashboard) continue;
  ...
  return Object.values(store.accounts)
    ...
    .filter((account) => !cenhubDashboardOnly || account.cenhubDashboard)
...
async function listCenhubDashboardClientIds() {
  if (usePostgres()) {
    const rows = await query`
      SELECT client_id FROM accounts
      WHERE cenhub_dashboard = TRUE
  ...
  return Object.values(store.accounts || {})
    .map((row) => rowToAccount(row))
    .filter((account) => account?.cenhubDashboard)
    .sort((a, b) => a.accountName.localeCompare(b.accountName))
...
async function createAccount(input) {
  ...
  const record = {
    ...
    cenhub_dashboard: Boolean(input.cenhubDashboard),
    ...
    meta_page_access_token_encrypted: input.metaPageAccessToken
      ...
      : '',
    fb_lead_sync_enabled: Boolean(input.fbLeadSyncEnabled),
    ghl_fb_lead_field_id: input.ghlFbLeadFieldId ? String(input.ghlFbLeadFieldId).trim() : null,
  ...
  if (usePostgres()) {
    await query`
      INSERT INTO accounts (
        ...
        metrics_model_changed_at, metrics_model_version, ready_for_ghl, cenhub_dashboard,
        meta_ad_account_id, meta_page_id, meta_pixel_id,
        meta_system_user_token_encrypted, meta_page_access_token_encrypted,
        fb_lead_sync_enabled, ghl_fb_lead_field_id,
        ...
        ${record.ready_for_ghl}, ${record.cenhub_dashboard},
        ...
        ${record.fb_lead_sync_enabled}, ${record.ghl_fb_lead_field_id},
...
async function updateAccount(clientId, input) {
  ...
  const updated = {
    ...
    cenhub_dashboard: input.cenhubDashboard != null
      ? Boolean(input.cenhubDashboard)
      : Boolean(existing.cenhubDashboard),
    ...
    fb_lead_sync_enabled: input.fbLeadSyncEnabled != null
      ? Boolean(input.fbLeadSyncEnabled)
      : existing.fbLeadSyncEnabled,
    ghl_fb_lead_field_id: input.ghlFbLeadFieldId !== undefined
      ? (input.ghlFbLeadFieldId ? String(input.ghlFbLeadFieldId).trim() : null)
      : existing.ghlFbLeadFieldId,
    meta_report_enabled: input.metaReportEnabled != null
      ? Boolean(input.metaReportEnabled)
      : Boolean(existing.metaReportEnabled),
    meta_report_show_bottomline: input.metaReportShowBottomline != null
      ? Boolean(input.metaReportShowBottomline)
      : Boolean(existing.metaReportShowBottomline),
    meta_report_fee_enabled: input.metaReportFeeEnabled != null
      ? Boolean(input.metaReportFeeEnabled)
      : Boolean(existing.metaReportFeeEnabled),
    meta_report_fee_percent: input.metaReportFeePercent != null
      ? Number(input.metaReportFeePercent)
      : (existing.metaReportFeePercent != null ? Number(existing.metaReportFeePercent) : 20),
    meta_report_fee_mode: input.metaReportFeeMode !== undefined
      ? (input.metaReportFeeMode || null)
      : (existing.metaReportFeeMode || null),
    meta_report_marketing_fee_amount: input.metaReportMarketingFeeAmount != null
      ? Number(input.metaReportMarketingFeeAmount)
      : (existing.metaReportMarketingFeeAmount != null ? Number(existing.metaReportMarketingFeeAmount) : 0),
    meta_report_show_other: input.metaReportShowOther != null
      ? Boolean(input.metaReportShowOther)
      : (existing.metaReportShowOther != null ? Boolean(existing.metaReportShowOther) : true),
    meta_report_table_columns: input.metaReportTableColumns != null
      ? (Number(input.metaReportTableColumns) === 2 ? 2 : 1)
      : (Number(existing.metaReportTableColumns) === 2 ? 2 : 1),
    meta_report_spend_chart_type: input.metaReportSpendChartType != null
      ? require('./meta-report-chart-type').normalizeMetaReportSpendChartType(input.metaReportSpendChartType)
      : require('./meta-report-chart-type').normalizeMetaReportSpendChartType(existing.metaReportSpendChartType),
    meta_report_scenario_month_window: input.metaReportScenarioMonthWindow != null
      ? require('./meta-report-scenario-settings').normalizeMetaReportScenarioMonthWindow(input.metaReportScenarioMonthWindow)
      : require('./meta-report-scenario-settings').normalizeMetaReportScenarioMonthWindow(existing.metaReportScenarioMonthWindow),
    meta_report_scenario_smooth_uneven: input.metaReportScenarioSmoothUneven != null
      ? require('./meta-report-scenario-settings').normalizeScenarioPillValue(input.metaReportScenarioSmoothUneven, true)
      : require('./meta-report-scenario-settings').normalizeScenarioPillValue(existing.metaReportScenarioSmoothUneven, true),
    meta_report_scenario_blend_history: input.metaReportScenarioBlendHistory != null
      ? require('./meta-report-scenario-settings').normalizeScenarioPillValue(input.metaReportScenarioBlendHistory, false)
      : require('./meta-report-scenario-settings').normalizeScenarioPillValue(existing.metaReportScenarioBlendHistory, false),
    meta_report_scenario_include_trend: input.metaReportScenarioIncludeTrend != null
      ? require('./meta-report-scenario-settings').normalizeScenarioPillValue(input.metaReportScenarioIncludeTrend, false)
      : require('./meta-report-scenario-settings').normalizeScenarioPillValue(existing.metaReportScenarioIncludeTrend, false),
    meta_report_scenario_caution_strong_months: input.metaReportScenarioCautionStrongMonths != null
      ? require('./meta-report-scenario-settings').normalizeScenarioPillValue(input.metaReportScenarioCautionStrongMonths, false)
      : require('./meta-report-scenario-settings').normalizeScenarioPillValue(existing.metaReportScenarioCautionStrongMonths, false),
    meta_report_budget_multiplier: input.metaReportBudgetMultiplier != null
      ? require('./meta-report-scenario-settings').normalizeMetaReportBudgetMultiplier(input.metaReportBudgetMultiplier)
      : require('./meta-report-scenario-settings').normalizeMetaReportBudgetMultiplier(existing.metaReportBudgetMultiplier),
    meta_report_budget_baseline: input.metaReportBudgetBaseline != null
      ? require('./meta-report-scenario-settings').normalizeMetaReportBudgetBaseline(input.metaReportBudgetBaseline)
      : require('./meta-report-scenario-settings').normalizeMetaReportBudgetBaseline(existing.metaReportBudgetBaseline),
    meta_report_slug: input.metaReportSlug !== undefined
      ? (input.metaReportSlug || null)
      : (existing.metaReportSlug || null),
    meta_report_access_token: input.metaReportAccessToken !== undefined
      ? (input.metaReportAccessToken || null)
      : (existing.metaReportAccessToken || null),
    meta_report_default_won_leads: input.metaReportDefaultWonLeads !== undefined
      ? input.metaReportDefaultWonLeads
      : existing.metaReportDefaultWonLeads,
    meta_report_default_avg_lead_value: input.metaReportDefaultAvgLeadValue !== undefined
      ? input.metaReportDefaultAvgLeadValue
      : existing.metaReportDefaultAvgLeadValue,
    meta_report_default_avg_profit_per_won: input.metaReportDefaultAvgProfitPerWon !== undefined
      ? input.metaReportDefaultAvgProfitPerWon
      : existing.metaReportDefaultAvgProfitPerWon,
    meta_report_ghl_data_enabled: input.metaReportGhlDataEnabled != null
      ? Boolean(input.metaReportGhlDataEnabled)
      : existing.metaReportGhlDataEnabled,
    meta_report_topline_mode: input.metaReportToplineMode != null
      ? (input.metaReportToplineMode === 'cenhub' ? 'cenhub' : 'meta')
      : (existing.metaReportToplineMode === 'cenhub' ? 'cenhub' : 'meta'),
    meta_report_excel_sheet_url: input.metaReportExcelSheetUrl !== undefined
      ? (input.metaReportExcelSheetUrl || null)
      : (existing.metaReportExcelSheetUrl || null),
  ...
  if (usePostgres()) {
    ...
    await query`
      UPDATE accounts SET
        ...
        cenhub_dashboard = ${updated.cenhub_dashboard},
        meta_ad_account_id = ${updated.meta_ad_account_id},
        meta_page_id = ${updated.meta_page_id},
        meta_pixel_id = ${updated.meta_pixel_id},
        fb_lead_sync_enabled = ${updated.fb_lead_sync_enabled},
        ghl_fb_lead_field_id = ${updated.ghl_fb_lead_field_id},
        meta_report_enabled = ${updated.meta_report_enabled},
        meta_report_show_bottomline = ${updated.meta_report_show_bottomline},
        meta_report_fee_enabled = ${updated.meta_report_fee_enabled},
        meta_report_fee_percent = ${updated.meta_report_fee_percent},
        meta_report_fee_mode = ${updated.meta_report_fee_mode},
        meta_report_marketing_fee_amount = ${updated.meta_report_marketing_fee_amount},
        meta_report_show_other = ${updated.meta_report_show_other},
        meta_report_table_columns = ${updated.meta_report_table_columns},
        meta_report_spend_chart_type = ${updated.meta_report_spend_chart_type},
        meta_report_scenario_month_window = ${updated.meta_report_scenario_month_window},
        meta_report_scenario_smooth_uneven = ${updated.meta_report_scenario_smooth_uneven},
        meta_report_scenario_blend_history = ${updated.meta_report_scenario_blend_history},
        meta_report_scenario_include_trend = ${updated.meta_report_scenario_include_trend},
        meta_report_scenario_caution_strong_months = ${updated.meta_report_scenario_caution_strong_months},
        meta_report_budget_multiplier = ${updated.meta_report_budget_multiplier},
        meta_report_budget_baseline = ${updated.meta_report_budget_baseline},
        meta_report_slug = ${updated.meta_report_slug},
        meta_report_access_token = ${updated.meta_report_access_token},
        meta_report_default_won_leads = ${updated.meta_report_default_won_leads},
        meta_report_default_avg_lead_value = ${updated.meta_report_default_avg_lead_value},
        meta_report_default_avg_profit_per_won = ${updated.meta_report_default_avg_profit_per_won},
        meta_report_ghl_data_enabled = ${updated.meta_report_ghl_data_enabled},
        meta_report_topline_mode = ${updated.meta_report_topline_mode},
        meta_report_excel_sheet_url = ${updated.meta_report_excel_sheet_url},
...
async function deleteAccount(clientId) {
  ...
  if (usePostgres()) {
    ...
    store.metaSyncRuns = (store.metaSyncRuns || []).filter((run) => run.client_id !== id);
    store.fbLeadSyncRuns = (store.fbLeadSyncRuns || []).filter((run) => run.client_id !== id);
    writeFileStore(store);
  }
...
async function logSyncRun(clientId, { status, errorMessage = null, opportunityCount = null, startedAt, finishedAt, source = 'unknown' }) {
  const id = normalizeClientId(clientId);
  const entry = {
    client_id: id,
  ...
  if (usePostgres()) {
    await query`
      INSERT INTO sync_runs (client_id, started_at, finished_at, status, error_message, opportunity_count, source)
      VALUES (${entry.client_id}, ${entry.started_at}, ${entry.finished_at}, ${entry.status}, ${entry.error_message}, ${entry.opportunity_count}, ${entry.source})
    `;
    return entry;
  }
