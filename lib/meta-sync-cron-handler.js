...
const { runRetentionMaintenance } = require('./sync-history-retention');
...
async function handleMetaSyncCronRequest(request, response) {
  ...
  try {
    ...
    let retention = null;
    try {
      retention = await runRetentionMaintenance();
    } catch (error) {
      retention = { error: error.message || 'Retention maintenance failed.' };
    }

    let metaReports = null;
    try {
      // Refresh all YTD report months (not only the current month) so lead-count
      // and insights fixes re-apply to historical months like April.
      const { refreshAllEnabledYearToDate } = require('./meta-report-service');
      metaReports = await refreshAllEnabledYearToDate();
    } catch (error) {
      metaReports = { error: error.message || 'Meta report refresh failed.' };
    }

    let googleAdsReports = null;
    try {
      const { refreshAllEnabledYearToDate } = require('./google-ads-report-service');
      googleAdsReports = await refreshAllEnabledYearToDate();
    } catch (error) {
      googleAdsReports = { error: error.message || 'Google Ads report refresh failed.' };
    }
