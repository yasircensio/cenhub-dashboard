...
async function fetchAdAccountCreatives(adAccountId, accessToken, { activeOnly = true, pageAccessToken = null } = {}) {
  const id = normalizeAdAccountId(adAccountId);
  if (!id) {
  ...
  await fillMissingVideosFromPage(videoIds, videoLibrary, pageAccessToken);
