...
function collectVideoIds(creative = {}) {
  const ids = [];
  const add = (value) => {
  ...
  add(creative.object_story_spec?.template_data?.video_id);
  const feedVideos = creative.asset_feed_spec?.videos;
  if (Array.isArray(feedVideos)) {
    for (const row of feedVideos) add(row?.video_id);
  }
  const children = creative.object_story_spec?.link_data?.child_attachments;
  if (Array.isArray(children)) {
    for (const row of children) add(row?.video_id);
  }
  return ids;
}

function extractVideoId(creative = {}) {
  return collectVideoIds(creative)[0] || null;
}

function classifyCreative(creative = {}) {
  const spec = creative.object_story_spec || {};
  const children = spec.link_data?.child_attachments;
  if (Array.isArray(children) && children.length > 1 && !collectVideoIds(creative).length) return 'carousel';
  if (
    collectVideoIds(creative).length
...
async function fillVideosFromPagePosts(ads, videoLibrary, pageAccessToken, { maxLookups = 10 } = {}) {
  if (!pageAccessToken) return videoLibrary;
  const pending = [];
  for (const ad of ads || []) {
    if (extractVideoId(ad.creative)) continue;
    const storyId = ad.creative?.effective_object_story_id || ad.creative?.object_story_id;
    if (!storyId) continue;
    pending.push({ ad, storyId });
    if (pending.length >= maxLookups) break;
  }
  await Promise.all(pending.map(async ({ ad, storyId }) => {
    try {
      const post = await graphFetch(
        `https://graph.facebook.com/${GRAPH_VERSION}/${encodeURIComponent(storyId)}?fields=id,type,source,attachments{media_type,type,url,target,media}`,
        pageAccessToken,
      );
      const attachment = Array.isArray(post.attachments?.data) ? post.attachments.data[0] : null;
      const videoId = attachment?.target?.id || null;
      const source = post.source || attachment?.media?.source || null;
      if (!source) return;
      const id = String(videoId || storyId);
      videoLibrary.set(id, {
        id,
        source,
        picture: attachment?.media?.image?.src || null,
      });
      if (videoId && ad.creative) ad.creative.video_id = String(videoId);
    } catch {
      // Page token may not cover this post.
    }
  }));
  return videoLibrary;
}
...
async function fetchAdAccountCreatives(adAccountId, accessToken, {
  ...
  const fields = [
    ...
    'campaign{id,name}',
    'adset{id,name}',
    'creative{id,name,title,body,thumbnail_url,image_url,object_type,video_id,image_hash,object_story_spec,asset_feed_spec,object_story_id,effective_object_story_id}',
  ].join(',');
  const params = new URLSearchParams({
    fields,
  ...
  const videoIds = [...new Set(ads.flatMap((ad) => collectVideoIds(ad.creative)))];
  ...
  const [imageLibrary] = await Promise.all([
    ...
    (async () => {
      await fillMissingVideosFromPage(missingVideoIds, videoLibrary, pageAccessToken, { maxLookups: 20 });
      await fillVideosFromPagePosts(ads, videoLibrary, pageAccessToken);
    })(),
  ]);
