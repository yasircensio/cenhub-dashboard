...
function summarizeAd(ad = {}, video = null) {
  ...
  return {
    ...
    videoId: creative.video_id || video?.id || null,
...
async function fetchAdAccountCreatives(adAccountId, accessToken, { activeOnly = true } = {}) {
  ...
  const videoIds = [...new Set(
    ads.map((ad) => ad.creative?.video_id).filter(Boolean),
  )].slice(0, MAX_VIDEO_LOOKUPS);
  const videos = new Map();
  const videoErrors = [];
  for (const videoId of videoIds) {
    try {
      videos.set(videoId, await fetchVideoDetails(videoId, accessToken));
    } catch (error) {
      videoErrors.push({ videoId, error: error.message || 'Video lookup failed' });
    }
  }

  const creatives = ads.map((ad) => summarizeAd(ad, videos.get(ad.creative?.video_id) || null));
  return {
