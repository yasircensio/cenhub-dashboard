...
async function buildCronSummary24h() {
  ...
  if (usePostgres()) {
    const rows = await query`
      SELECT r.client_id, a.account_name,
             COUNT(*)::int AS runs,
      ...
      GROUP BY r.client_id, a.account_name
    ...
    return {
      since24h,
      totalRuns: byClient.reduce((sum, row) => sum + row.runs, 0),
  ...
  for (const entry of readLocalRuns()) {
    ...
    if (!isAutomatedCronSource(entry.source)) continue;
    const clientId = entry.client_id;
    const row = grouped.get(clientId) || {
      clientId,
      accountName: accountNameById[clientId] || clientId,
      runs: 0,
      updated: 0,
      lastRunAt: null,
    };
    row.runs += 1;
    row.updated += Number(entry.updated_count) || 0;
    if (!row.lastRunAt || String(entry.started_at) > String(row.lastRunAt)) {
      row.lastRunAt = entry.started_at;
    }
    grouped.set(clientId, row);
  }
  const byClient = [...grouped.values()].sort((a, b) => a.accountName.localeCompare(b.accountName));
  return {
    since24h,
    totalRuns: byClient.reduce((sum, row) => sum + row.runs, 0),
    totalUpdated: byClient.reduce((sum, row) => sum + row.updated, 0),
