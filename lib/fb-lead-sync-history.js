...
const FB_LEAD_SYNC_SCHEDULE = 'Every hour (cron-job.org)';
const AUTOMATED_CRON_SOURCES = [
  'cron-job.org',
  'github-actions',
  'vercel-cron',
  'http-cron',
];
const AUTOMATED_CRON_SOURCE_SET = new Set(AUTOMATED_CRON_SOURCES);
  ...
  if (usePostgres()) {
    const rows = await query`
      ...
      WHERE r.started_at >= ${since24h}
        AND r.dry_run = FALSE
        AND r.source IN ('cron-job.org', 'github-actions', 'vercel-cron', 'http-cron')
