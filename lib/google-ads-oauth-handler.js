...
async function handleGoogleAdsOAuthCallback(request, response) {
  // #region agent log
  fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f182bb'},body:JSON.stringify({sessionId:'f182bb',location:'lib/google-ads-oauth-handler.js:C-callback-entry',message:'callback handler entry',data:{url:request.url,method:request.method,originalUrl:request.originalUrl,queryKeys:Object.keys(request.query||{}),hasQueryCode:Boolean((request.query||{}).code),xForwardedUri:request.headers?.['x-forwarded-uri'],xVercelUri:request.headers?.['x-vercel-forwarded-uri']},timestamp:Date.now(),hypothesisId:'A-B-C-D-E'})}).catch(()=>{});
  // #endregion
  const config = assertGoogleAdsOAuthConfig(getGoogleAdsConfig());
  const redirectUri = resolveGoogleAdsOAuthRedirectUri(request);
...
(function(){
  ...
  document.body.insertAdjacentHTML('afterbegin', '<p>Recovering authorization from the browser URL…</p>');
  fetch(window.location.pathname, {
    ...
    sendHtml(response, 400, renderOAuthResultHtml({
      ...
      body: `${describeOAuthCallbackFailure(request, state)}${debugDetails}${recoverScript}`,
    }));
    return;
  }
