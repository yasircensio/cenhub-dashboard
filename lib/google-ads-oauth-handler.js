...
async function handleGoogleAdsOAuthCallback(request, response) {
  ...
  if (!code) {
    ...
    const debugDetails = `<pre style="background:#f4f4f5;padding:12px;border-radius:8px;overflow:auto">url=${String(request?.url || '')}\nquery keys=${Object.keys(debugQuery).join(', ') || '(none)'}</pre>`;
    sendHtml(response, 400, renderOAuthResultHtml({
