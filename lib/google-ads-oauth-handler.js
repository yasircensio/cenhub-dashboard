...
async function handleGoogleAdsOAuthCallback(request, response) {
  ...
  if (!code || !state || !verifyOAuthState(state)) {
    sendHtml(response, 400, renderOAuthResultHtml({
      ...
      body: '<p>The authorization link expired or was opened in a different browser. Open <a href="/api/auth/google-ads/start">/api/auth/google-ads/start</a> again (while logged into admin) and complete Google sign-in without delay.</p>',
    }));
    return;
  }
