  ...
  try {
    ...
    if (statusCode === 401) {
      sendHtml(response, 401, renderOAuthResultHtml({
        ...
        body: '<p>Log into <a href="/admin">/admin</a>, then open <a href="/api/google-ads/oauth/start">/api/google-ads/oauth/start</a> again.</p>',
      }));
      return;
    }
...
async function handleGoogleAdsOAuthCallback(request, response) {
  ...
  if (!code || !state || !expectedState || state !== expectedState) {
    sendHtml(response, 400, renderOAuthResultHtml({
      ...
      body: '<p>Start again from <a href="/api/google-ads/oauth/start">/api/google-ads/oauth/start</a> while logged into admin.</p>',
    }));
    return;
  }
  ...
  try {
    ...
    if (!tokens.refresh_token) {
      sendHtml(response, 200, renderOAuthResultHtml({
        ...
        body: `
          ...
          <p><a href="/api/google-ads/oauth/start">Try again</a></p>
        `,
      }));
      return;
    }
    ...
    sendHtml(response, 500, renderOAuthResultHtml({
      ...
      body: `<p>${err.message || err}</p><p><a href="/api/google-ads/oauth/start">Try again</a></p>`,
    }));
  }
}
