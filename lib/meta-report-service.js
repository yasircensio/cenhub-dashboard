...
const { verifyMetaAccessToken } = require('./meta-token');
...
} = require('./meta-report-topline-mode');
const { restoreMetaInputsPatch } = require('./meta-report-topline-restore');
const {
  accountReportFields,
  clampReportYear,
...
function resolveMonthReportInputs(monthRecord, periodAligned, settings = {}) {
  const stored = monthRecord.toplineSource || 'meta';
  const effective = resolveEffectiveToplineSource(monthRecord, settings);
  ...
  const restoredMeta = (stored === 'manual' || stored === 'ghl') && effective === 'meta'
    ? restoreMetaInputsPatch(monthRecord)
    : {};

  return {
    source: 'meta',
    leads: periodAligned ? monthRecord.metaLeads : null,
    wonLeads: restoredMeta.wonLeads ?? monthRecord.wonLeads,
    avgLeadValue: restoredMeta.avgLeadValue ?? monthRecord.avgLeadValue,
    avgProfitPerWon: restoredMeta.avgProfitPerWon ?? monthRecord.avgProfitPerWon,
    ghlTotals: null,
  };
}
...
function buildMonthPayload(account, monthRecord, { includeUnpublished = false } = {}) {
  ...
  const resolved = resolveMonthReportInputs(monthRecord, periodAligned, settings);
  ...
  return {
    ...
    manualOverride: isActiveCenhubManualMonth(monthRecord, settings),
...
async function buildClientYearPayloadCore(clientId, year, options = {}) {
...
async function buildClientYearPayload(clientId, year, options = {}) {
  const payload = await buildClientYearPayloadCore(clientId, year, options);
  const { attachLinkedPair } = require('./report-link');
  return attachLinkedPair(payload, { metaClientId: payload.clientId });
}
...
async function getPublicReportPayload(token, year) {
  ...
  const { buildDualPublicReportPayload } = require('./report-link');
  const access = await requireReportAccess(token);
  if (access.reportKind === 'google-ads') {
    const dual = await buildDualPublicReportPayload({
      origin: 'google-ads',
      year,
      googleClientId: access.clientId,
    });
    if (dual) return dual;
    const { getPublicGoogleAdsReportPayload } = require('./google-ads-report-service');
    return getPublicGoogleAdsReportPayload(token, year);
  }
  const dual = await buildDualPublicReportPayload({
    origin: 'meta',
    year,
    metaClientId: access.clientId,
  });
  if (dual) return dual;
  return buildClientYearPayload(access.clientId, year, { includeUnpublished: false });
}
