...
function filterLeadsForApplyFromPreview(previewRows, cachedLeads) {
  const rows = Array.isArray(previewRows) ? previewRows : [];
  const leads = Array.isArray(cachedLeads) ? cachedLeads : [];
  const wouldUpdateIds = new Set(
    rows
      .filter((row) => row.status === 'would_update')
      .map((row) => String(row.metaLeadId)),
  );
  if (!wouldUpdateIds.size) return [];
  return leads.filter((lead) => wouldUpdateIds.has(String(lead.id)));
}
...
async function syncMetaLeadIdsToGhl(clientId, {
  ...
  const applyRunHasOwnCache = Boolean(cached?.leads?.length);
  ...
  if (cached?.leads?.length) {
    ...
    metaLeadCount = cached.metaLeadCount ?? cached.leads.length;
    if (!dryRun && previewRunId && !applyRunHasOwnCache) {
      const previewRun = await getFbLeadSyncRun(previewRunId);
      allInWindow = filterLeadsForApplyFromPreview(previewRun.rows, allInWindow);
    }
...
function extractContactEmailPhone(contact) {
  const raw = contact?.contact || contact || {};
  return {
    email: normalizeEmail(raw.email || raw.primaryEmail || ''),
    phone: String(raw.phone || raw.primaryPhone || '').trim(),
    name: raw.name || raw.contactName || '',
    contactId: raw.id || contact?.id || null,
  };
}

function metaLeadMatchesContact(lead, { email, phone }) {
  const parsed = parseFieldData(lead.field_data);
  const leadEmail = normalizeEmail(parsed.email);
  const leadPhone = normalizePhone(parsed.phone);
  if (email && leadEmail && email === leadEmail) return true;
  if (phone && leadPhone && normalizePhone(phone) === leadPhone) return true;
  return false;
}

function findMetaLeadForContact(contact, metaLeads) {
  const { email, phone } = extractContactEmailPhone(contact);
  if (!email && !phone) return null;
  return (metaLeads || []).find((lead) => metaLeadMatchesContact(lead, { email, phone })) || null;
}

function isRetryableSyncOutcome(status) {
  return status === 'no_meta_match' || status === 'error';
}

async function syncFbLeadIdForContact(clientId, contactId, {
  source = 'unknown',
  logHistory = true,
  ...
  const account = accountOverride || await getAccount(clientId, { includeSecrets: true });
  if (!account) {
    const error = new Error('Account not found.');
    error.statusCode = 404;
    throw error;
  ...
  if (!account?.ghlToken || !account?.locationId || !account.metaPageId) {
    return {
      clientId,
      contactId,
  ...
  const finishRun = async (patch) => {
    if (!logHistory || !activeRunId) return;
    await updateFbLeadSyncRun(activeRunId, {
      ...patch,
      inWindow: 1,
      batchOffset: 1,
  ...
  try {
    const full = await ghlRequest(account.ghlToken, 'GET', `/contacts/${contactId}`);
    const contact = full.contact || full;
    const { email, phone, name } = extractContactEmailPhone(contact);

    const existing = getFbLeadIdFromContact(contact, fbLeadFieldId);
    if (existing && !force) {
      const row = {
        contactId,
        email,
    ...
    if (!email && !phone) {
      const row = { contactId, email, phone, name, status: 'no_email_or_phone' };
      await finishRun({
        status: 'success',
        updated: 0,
        skippedHasId: 0,
    ...
    const meta = await fetchMetaLeads(clientId, { withFields: true });
    const recentLeads = filterLeadsByDays(meta.leads, days);
    const lead = findMetaLeadForContact(contact, recentLeads);

    if (!lead) {
      const row = { contactId, email, phone, name, status: 'no_meta_match' };
      await finishRun({
        status: 'success',
        metaLeadCount: meta.leadCount,
        updated: 0,
    ...
    const outcome = await evaluateLeadMatch(account, lead, { fbLeadFieldId, dryRun, force });
    const row = { ...outcome.row, contactId };
    await finishRun({
      status: outcome.errors > 0 ? 'error' : 'success',
      metaLeadCount: meta.leadCount,
      updated: outcome.updated,
      skippedHasId: outcome.skippedHasId,
      skippedNoMatch: outcome.skippedNoMatch,
      errors: outcome.errors,
      rows: row.status === 'updated' || row.status === 'error' ? [row] : [],
    ...
    return {
      ...
      status: row.status,
      updated: outcome.updated,
      retryable: isRetryableSyncOutcome(row.status),
      skipped: outcome.updated === 0 && !isRetryableSyncOutcome(row.status),
      row,
      error: row.error || null,
    };
  } catch (error) {
    if (logHistory && activeRunId) {
      await finishFbLeadSyncRun(activeRunId, {
        status: 'error',
    ...
    return {
      ...
      status: 'error',
      updated: 0,
      retryable: isRetryableGhlError(error),
      skipped: false,
      error: error.message,
    };
  }
