...
async function ghlRequest(token, method, path, body) {
  ...
  if (!response.ok) {
    throw new Error(`GHL ${method} ${path} → ${response.status}: ${json.message || text.slice(0, 200)}`);
  }
  return json;
}
...
async function getFbLeadSyncPreflight(clientId) {
  ...
  const fbLeadFieldId = resolveFbLeadFieldId(account);
  const result = {
...
async function syncMetaLeadIdsToGhl(clientId, {
  ...
  const fbLeadFieldId = fieldOverride || resolveFbLeadFieldId(account);
  ...
  const hasMore = batchOffset + batchLeads.length < allInWindow.length;
  ...
  if (logHistory && activeRunId) {
    await updateFbLeadSyncRun(activeRunId, {
      ...
      batchOffset: batchOffset + batchLeads.length,
      batchLimit,
      hasMore,
      ...
      status: hasMore ? 'running' : (summary.errors > 0 ? 'error' : 'success'),
