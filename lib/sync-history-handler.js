...
const { deleteMetaSyncRuns, listGhlSyncRuns, listMetaSyncRuns } = require('./sync-history');
...
async function handleSyncHistoryRequest(request, response) {
  ...
  try {
    ...
    if (method === 'DELETE') {
      ...
      if (type !== 'meta') {
        sendJson(response, 400, { error: 'Only Meta sync logs can be cleared (type=meta).' });
        return;
      }
