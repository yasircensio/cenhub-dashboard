...
const MIN_ELASTICITY = 0.65;
const MAX_ELASTICITY = 0.80;
...
function prepareScenarioSeries(series = [], {
  ...
  const { months, outliersAdjusted } = winsorizeMonths(windowed);
...
function computeMonthlyTrendRate(months = [], elasticity = 0.75) {
...
function calibrateElasticity(months = [], presetElasticity = 0.75) {
  if (months.length < MIN_TREND_MONTHS) return presetElasticity;

  const spends = months.map((point) => parseAmount(point.spend));
  const leads = months.map((point) => parseAmount(point.leads));
  const minSpend = Math.min(...spends);
  const maxSpend = Math.max(...spends);
  if (minSpend <= 0 || maxSpend <= 0) return presetElasticity;
  if ((maxSpend - minSpend) / minSpend < 0.25) return presetElasticity;

  const points = months
    .filter((point) => parseAmount(point.spend) > 0 && parseAmount(point.leads) > 0)
    .map((point) => ({
      x: Math.log(parseAmount(point.spend)),
      y: Math.log(parseAmount(point.leads)),
    }));

  if (points.length < MIN_TREND_MONTHS) return presetElasticity;

  const { slope, r } = linearRegression(points);
  if (Math.abs(r) < 0.25) return presetElasticity;

  return roundRatio(Math.max(MIN_ELASTICITY, Math.min(MAX_ELASTICITY, slope)));
}

function computeScenarioEfficiency(months = [], {
  trendMethod = 'recency_weighted',
  elasticityPreset = 'balanced',
} = {}) {
  const method = normalizeMetaReportScenarioTrendMethod(trendMethod);
  const presetElasticity = resolveScenarioElasticity(elasticityPreset);
  const elasticity = calibrateElasticity(months, presetElasticity);
  ...
  let efficiency = recencyWeighted;
  let trendInfo = { trendRate: 0, trendDirection: 'flat', trendRatePct: 0, trendR: 0 };

  if (method === 'robust_trend') {
    trendInfo = computeMonthlyTrendRate(months, elasticity);
  } else if (method === 'conservative_blend') {
    efficiency = blendEfficiency(pooled, recencyWeighted, 0.5);
    const fullTrend = computeMonthlyTrendRate(months, elasticity);
    if (Math.abs(fullTrend.trendR) >= TREND_R_THRESHOLD) {
      trendInfo = fullTrend;
    }
  }
...
function projectScenario({
  ...
  const prepared = prepareScenarioSeries(series, { windowMonths: monthWindow, asOfDate });
  const efficiencyResult = computeScenarioEfficiency(prepared.months, { trendMethod, elasticityPreset });
  ...
  const conservativeElasticity = Math.max(MIN_ELASTICITY, elasticity - 0.05);
  const optimisticElasticity = Math.min(MAX_ELASTICITY, elasticity + 0.05);
  const projectedConservative = projectScenarioAtSpend({
    baselineSpend: resolvedBaselineSpend,
    spend: roundMoney(resolvedBaselineSpend * mult),
    efficiency,
    elasticity: conservativeElasticity,
  ...
  const projectedOptimistic = projectScenarioAtSpend({
    ...
    spend: roundMoney(resolvedBaselineSpend * mult),
    efficiency,
    elasticity: optimisticElasticity,
...
function buildScenarioProjectionSteps(projection, {
  ...
  const targetMult = parseAmount(projection.multiplier) || 1;
  ...
  for (let step = 1; step <= months; step += 1) {
    const stepMult = Math.min(targetMult, 1 + step);
