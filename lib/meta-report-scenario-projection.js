...
const SCENARIO_SPEND_RAMP_CAP = 2;
...
function scenarioPointHasTopline(point) {
  return parseAmount(point.spend) > 0
    && parseAmount(point.leads) > 0
    && parseAmount(point.wonLeads) > 0
    && parseAmount(point.avgLeadValue) > 0;
}

function scenarioPointHasBottomlineInputs(point) {
  return scenarioPointHasTopline(point) && parseAmount(point.avgProfitPerWon) > 0;
}

function isIncompleteMonth(point, asOfDate = new Date(), { hasBottomline = false } = {}) {
  const periodEnd = parseDateOnly(point.periodEnd);
  if (!periodEnd) return true;
  const today = formatDateOnly(asOfDate);
  if (String(point.periodEnd).slice(0, 10) < today) return false;
  const hasScenarioInputs = hasBottomline
    ? scenarioPointHasBottomlineInputs(point)
    : scenarioPointHasTopline(point);
  return !hasScenarioInputs;
}
...
function winsorizeMonths(months = []) {
  ...
  const adjusted = months.map((point) => {
    const next = { ...point };
    let changed = false;
    const originalLeads = parseAmount(point.leads);
    ...
    if (leadMad > 0) {
      ...
      if (Math.round(cappedLeads) !== Math.round(originalLeads)) {
    ...
    if (changed) {
      outliersAdjusted += 1;
      const monthWinRate = originalLeads > 0 ? parseAmount(point.wonLeads) / originalLeads : 0;
      const rescaledLeads = parseAmount(next.leads);
      next.wonLeads = Math.max(0, Math.round(rescaledLeads * monthWinRate));
      next.totalLeadValue = roundMoney(next.wonLeads * parseAmount(point.avgLeadValue));
      const spend = parseAmount(next.spend);
      next.efficiencyIndex = spend > 0 ? next.totalLeadValue / spend : 0;
    }
...
function prepareScenarioSeries(series = [], {
  ...
  const adActive = series
    ...
    .filter((point) => !isIncompleteMonth(point, asOfDate, { hasBottomline }))
...
function efficiencyFromPreparedMonth(point) {
  const leads = parseAmount(point.leads);
  const wonLeads = parseAmount(point.wonLeads);
  return {
    avgCpl: parseAmount(point.cpl),
    winRate: leads > 0 ? roundRatio(wonLeads / leads) : 0,
    avgLeadValue: parseAmount(point.avgLeadValue),
    avgProfitPerWon: parseAmount(point.avgProfitPerWon),
    avgMonthlySpend: parseAmount(point.spend),
  };
}

function resolveScenarioProjectionEfficiency(prepared, recencyEfficiency, {
  hasBottomline = false,
  activeMonthKey = null,
  asOfDate = new Date(),
} = {}) {
  const months = prepared?.months || [];
  if (!months.length || !recencyEfficiency) return recencyEfficiency;
  const last = months[months.length - 1];
  const targetKey = activeMonthKey || monthKeyFromDate(asOfDate);
  if (!targetKey || last.monthKey !== targetKey) return recencyEfficiency;
  const hasInputs = hasBottomline
    ? scenarioPointHasBottomlineInputs(last)
    : scenarioPointHasTopline(last);
  if (!hasInputs) return recencyEfficiency;
  return blendEfficiency(efficiencyFromPreparedMonth(last), recencyEfficiency, 0.65);
}
...
function resolveLastMonthBaseline(series = [], {
  activeMonthKey = null,
  asOfDate = new Date(),
} = {}) {
  const referenceKey = activeMonthKey || monthKeyFromAsOfDate(asOfDate);
  if (!referenceKey) return null;

  const previousKey = addMonthsToMonthKey(referenceKey, -1);
  if (!previousKey) return null;

  const fromSeries = series.find((point) => point.monthKey === previousKey);
  const spendFromSeries = fromSeries ? parseAmount(fromSeries.spend) : 0;
  if (spendFromSeries > 0) {
    return {
      monthKey: previousKey,
      label: fromSeries.label || monthLabelFromMonthKey(previousKey),
      spend: spendFromSeries,
    };
  }
  ...
  const previous = sorted.find((point) => point.monthKey === previousKey);
  if (!previous) return null;

  return {
    monthKey: previousKey,
    label: previous.label || monthLabelFromMonthKey(previousKey),
    spend: parseAmount(previous.spend),
  };
}

function prepareBaselineSpendWindow(series = [], {
  windowMonths = '6',
  asOfDate = new Date(),
} = {}) {
  const windowLimit = resolveScenarioMonthWindowLimit(windowMonths);
  const today = asOfDate.toISOString().slice(0, 10);
  const withSpend = series
    .filter((point) => parseAmount(point.spend) > 0)
    .filter((point) => {
      const periodEnd = String(point.periodEnd || '').slice(0, 10);
      return !periodEnd || periodEnd < today;
    });
  return windowLimit === Infinity ? withSpend : withSpend.slice(-windowLimit);
}

function resolveWindowAverageBaselineSpend(spendMonths = []) {
  if (!spendMonths.length) return 0;
  const total = spendMonths.reduce((sum, point) => sum + parseAmount(point.spend), 0);
  return total > 0 ? roundMoney(total / spendMonths.length) : 0;
}

function resolvePreparedWindowAverageSpend(preparedMonths = []) {
  if (!preparedMonths.length) return 0;
  const total = preparedMonths.reduce((sum, point) => sum + parseAmount(point.spend), 0);
  return total > 0 ? roundMoney(total / preparedMonths.length) : 0;
}

function resolveLatestPreparedSpend(preparedMonths = []) {
  for (let index = preparedMonths.length - 1; index >= 0; index -= 1) {
    const spend = parseAmount(preparedMonths[index]?.spend);
...
function resolveScenarioBaselineSpend(series, preparedMonths, {
  ...
  if (baselineMode === 'month' && activeMonthKey) {
    ...
    if (fromSeries && parseAmount(fromSeries.spend) > 0) {
      spend = parseAmount(fromSeries.spend);
    } else {
      const month = preparedMonths.find((point) => point.monthKey === activeMonthKey);
      if (month && parseAmount(month.spend) > 0) {
        spend = parseAmount(month.spend);
      }
    }
  } else if (baselineMode === 'last') {
    const lastMonth = resolveLastMonthBaseline(series, { activeMonthKey, asOfDate });
    if (lastMonth?.spend > 0) spend = lastMonth.spend;
  } else if (baselineMode === 'year') {
    const spendWindow = prepareBaselineSpendWindow(series, { windowMonths: monthWindow, asOfDate });
    spend = resolveWindowAverageBaselineSpend(spendWindow);
    if (spend <= 0) spend = resolvePreparedWindowAverageSpend(preparedMonths);
  }

  if (spend <= 0) spend = resolvePreparedWindowAverageSpend(preparedMonths);
  if (spend <= 0) spend = resolveLatestPreparedSpend(preparedMonths);
...
function projectScenario({
  ...
  const resolvedBaselineSpend = baselineSpend != null
    ...
    : resolveScenarioBaselineSpend(series, prepared.months, {
      baselineMode,
      activeMonthKey,
      asOfDate,
      monthWindow,
  ...
  let { efficiency, elasticity, trendRate } = efficiencyResult;
  efficiency = resolveScenarioProjectionEfficiency(prepared, efficiency, {
    hasBottomline,
    activeMonthKey,
    asOfDate,
  });

  const insufficientData = !efficiency
    || efficiency.avgCpl <= 0
    || resolvedBaselineSpend <= 0
    || prepared.monthsUsed < MIN_EFFICIENCY_MONTHS;
  ...
  if (insufficientData) {
    return {
      ...
      insufficientReason: prepared.monthsUsed < MIN_EFFICIENCY_MONTHS
        ? 'need_more_months'
        : resolvedBaselineSpend <= 0
          ? 'need_baseline_spend'
          : 'need_efficiency',
...
function resolveScenarioStepMultiplier(stepIndex, targetMultiplier = 1) {
  const targetMult = parseAmount(targetMultiplier) || 1;
  const step = Number.parseInt(stepIndex, 10) || 1;
  if (targetMult <= SCENARIO_SPEND_RAMP_CAP) return targetMult;
  if (step <= 1) return SCENARIO_SPEND_RAMP_CAP;
  return targetMult;
}
...
function buildScenarioProjectionSteps(projection, {
  ...
  for (let step = 1; step <= months; step += 1) {
    const stepMult = resolveScenarioStepMultiplier(step, targetMult);
...
module.exports = {
  ...
  resolveLatestPreparedSpend,
  resolvePreparedWindowAverageSpend,
  resolveScenarioBaselineSpend,
  resolveScenarioConfidence,
  resolveScenarioStepMultiplier,
