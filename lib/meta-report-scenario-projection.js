...
function isScenarioProjectionAllowed(targetMonthKey, asOfDate = new Date()) {
  const currentKey = monthKeyFromAsOfDate(asOfDate);
  if (!targetMonthKey || !currentKey) return false;
  return String(targetMonthKey) >= String(currentKey);
}

function scenarioProjectionStepOffset(targetMonthKey, asOfDate = new Date()) {
  const currentKey = monthKeyFromAsOfDate(asOfDate);
  if (!targetMonthKey || !currentKey || String(targetMonthKey) < String(currentKey)) return null;
  let offset = 0;
  let cursor = currentKey;
  while (cursor && cursor <= targetMonthKey) {
    offset += 1;
    if (cursor === targetMonthKey) break;
    cursor = addMonthsToMonthKey(cursor, 1);
  }
  return offset > 0 ? offset : null;
}

function resolveScenarioProjectionTarget(targetMonthKey, asOfDate = new Date()) {
  const stepOffset = scenarioProjectionStepOffset(targetMonthKey, asOfDate);
  const allowed = stepOffset != null && isScenarioProjectionAllowed(targetMonthKey, asOfDate);
  return {
    allowed,
    targetMonthKey: allowed ? targetMonthKey : null,
    stepOffset: allowed ? stepOffset : null,
  };
}

function buildScenarioProjectionMonthKeys(targetMonthKey, { allowed = true } = {}) {
  if (!allowed || !targetMonthKey) return [];
...
function buildScenarioProjectionSteps(projection, {
  ...
  const resolvedTarget = targetMonthKey ?? projection.projectionTargetMonthKey ?? null;
  const allowed = projectionAllowed ?? projection.projectionAllowed ?? true;
  if (!allowed || !resolvedTarget) return [];

  const resolvedStepOffset = stepOffset ?? projection.projectionStepOffset
    ?? scenarioProjectionStepOffset(resolvedTarget, asOfDate);
  if (!resolvedStepOffset) return [];
  ...
  const spend = roundMoney(baseSpend * targetMult);
  const trendAdj = Math.exp(trendRate * resolvedStepOffset);
  ...
  return [{
    label: monthLabelFromMonthKey(resolvedTarget),
    monthKey: resolvedTarget,
    spendMultiplier: roundRatio(targetMult),
    spend: metrics.spend,
    leads: metrics.leads,
...
module.exports = {
  ...
  isScenarioProjectionAllowed,
  resolveScenarioProjectionTarget,
  scenarioProjectionStepOffset,
