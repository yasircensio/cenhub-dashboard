...
function resolveLastMonthBaseline(series = [], asOfDate = new Date()) {
  const currentKey = monthKeyFromAsOfDate(asOfDate);
  const previousKey = currentKey ? addMonthsToMonthKey(currentKey, -1) : null;

  if (previousKey) {
    const previous = series.find((point) => point.monthKey === previousKey);
    if (previous && parseAmount(previous.spend) > 0) {
      return {
        monthKey: previousKey,
        label: monthLabelFromMonthKey(previousKey),
        spend: parseAmount(previous.spend),
      };
    }
  }

  const priorMonths = series
    .filter((point) => parseAmount(point.spend) > 0 && (!currentKey || point.monthKey < currentKey))
    .sort((left, right) => String(left.monthKey).localeCompare(String(right.monthKey)));

  if (!priorMonths.length) return null;

  const last = priorMonths[priorMonths.length - 1];
  return {
    monthKey: last.monthKey,
    label: last.label || monthLabelFromMonthKey(last.monthKey),
    spend: parseAmount(last.spend),
  };
}
...
function resolveScenarioBaselineSpend(series, preparedMonths, {
  ...
  if (baselineMode === 'last') {
    const lastMonth = resolveLastMonthBaseline(series, asOfDate);
