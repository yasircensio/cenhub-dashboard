...
function readLocalMainStoreFull() {
  if (!fs.existsSync(DATA_FILE)) {
    return { syncRuns: [], metaSyncRuns: [], metaReportGhlSyncRuns: [], ghlWebhookEvents: [] };
  }
  try {
...
function readLocalMainStore() {
  ...
  return {
    ...
    metaSyncRuns: store.metaSyncRuns,
    metaReportGhlSyncRuns: store.metaReportGhlSyncRuns,
  };
}
...
function writeLocalMainStore(patch) {
  ...
  if (patch.metaReportGhlSyncRuns) fullStore.metaReportGhlSyncRuns = patch.metaReportGhlSyncRuns;
...
async function pruneMetaReportGhlSyncHistory({ maxAgeDays = SYNC_HISTORY_RETENTION_DAYS } = {}) {
  const cutoffIso = getSyncHistoryCutoffIso(maxAgeDays);
  ...
  store.metaReportGhlSyncRuns = store.metaReportGhlSyncRuns.filter(
    (row) => String(row.started_at || '') >= cutoffIso,
  );
  writeLocalMainStore({ metaReportGhlSyncRuns: store.metaReportGhlSyncRuns });
  return { deleted: before - store.metaReportGhlSyncRuns.length, cutoffIso };
}
...
async function pruneAllSyncHistory({ maxAgeDays = SYNC_HISTORY_RETENTION_DAYS } = {}) {
  ...
  const metaReportGhl = await pruneMetaReportGhlSyncHistory({ maxAgeDays });
  const fb = await pruneFbLeadSyncHistory({ maxAgeDays });
  return {
    retentionDays: maxAgeDays,
    cutoffIso: ghl.cutoffIso,
    deleted: ghl.deleted + meta.deleted + metaReportGhl.deleted + fb.deleted,
    ghl,
    meta,
    metaReportGhl,
    fb,
