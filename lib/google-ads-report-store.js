...
async function getGoogleAdsReportsDashboard({ filter = 'all' } = {}) {
  ...
  try {
    ...
    const tokenLength = partnerFetch.debug?.developerTokenLength || 0;
    partnerFetch.error = googleCode === 'DEVELOPER_TOKEN_INVALID'
      ? `Google Ads rejected this developer token (DEVELOPER_TOKEN_INVALID). CenHub sent ${tokenLength} characters from GOOGLE_ADS_DEVELOPER_TOKEN. Google’s published developer tokens are 22 characters. After View token, use Copy (do not retype), paste into a text editor, and confirm the character count is 22. Then replace the Vercel value and redeploy.`
      : (error.message || 'Failed to load Google Ads accounts.');
    
    // Add additional diagnostic information if the token is 20 characters
    if (googleCode === 'DEVELOPER_TOKEN_INVALID' && tokenLength === 20) {
      partnerFetch.error += ' Note: If your token is exactly 20 characters in the API Center, it might be an older format or there might be a display issue in the Google Ads UI. Please ensure you are copying the entire string.';
    }
    partnerFetch.debug = {
      ...(partnerFetch.debug || {}),
      ...
      mccSearchProbe: error.mccSearchProbe || null,
    };
    // #region agent log
    fetch('http://127.0.0.1:7412/ingest/8036624f-bbd1-4142-b516-bb72c323b06c',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'f182bb'},body:JSON.stringify({sessionId:'f182bb',runId:'mcc-probe',hypothesisId:'L',location:'lib/google-ads-report-store.js:getGoogleAdsReportsDashboard',message:'Google Ads listing threw',data:{error:partnerFetch.error,debug:partnerFetch.debug},timestamp:Date.now()})}).catch(()=>{});
    // #endregion
  }
