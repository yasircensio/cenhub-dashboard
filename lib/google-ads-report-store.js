...
} = require('./google-ads-leads-source');
const {
...
function clientRowToRecord(row) {
  ...
  return {
    ...
    leadsSource: normalizeGoogleAdsLeadsSource(row.leads_source),
...
function clientPublicFields(client) {
  return {
    ...
    googleAdsReportLeadsSource: normalizeGoogleAdsLeadsSource(client.leadsSource),
...
async function listGoogleAdsReportClients({ filter = 'all', hydrateShareLinks = true } = {}) {
  let rows = [];
  if (!usePostgres()) {
...
async function updateGoogleAdsReportSettings(clientId, input = {}) {
  ...
  const patch = {
    ...
    leads_source: existing.leadsSource,
    linked_meta_client_id: existing.linkedMetaClientId,
  };

  if (input.googleAdsReportLeadsSource !== undefined) {
    patch.leads_source = normalizeGoogleAdsLeadsSource(input.googleAdsReportLeadsSource);
  }
  ...
  const rows = await query`
    UPDATE google_ads_report_clients SET
      ...
      leads_source = ${patch.leads_source},
...
async function saveGoogleAdsMonthRecord(clientId, monthKey, input = {}) {
  ...
  const manualLeads = input.manualLeads !== undefined
    ? resolveManualLeadsToStore(
      input.manualLeads,
      existing.googleConversions,
      existing.manualLeads,
      client.leadsSource,
    )
    : existing.manualLeads;
...
function classifyGoogleAdsCustomInputStatus(row, { requireProfit = false } = {}) {
  if (!row) return 'empty';
...
function googleAdsCustomInputsUpdatedAt(row, status) {
  if (status === 'empty') return null;
  return row?.updated_at || row?.updatedAt || null;
}

async function listGoogleAdsMonthInputRowsForYear(year) {
  const numericYear = Number(year);
  if (!Number.isFinite(numericYear)) return [];
  const prefix = `${numericYear}-`;

  if (!usePostgres()) {
    return Object.values(readFileStore().months || {})
  ...
  return query`
    SELECT
      ...
      m.google_conversions,
      m.google_spend,
      m.manual_leads,
      m.updated_at
    FROM google_ads_report_months m
    WHERE m.month_key LIKE ${`${prefix}%`}
  `;
}

function shouldListOnGoogleAdsCustomValuesPage(client) {
  if (!client?.clientId || !(client.googleCustomerId || client.google_customer_id)) return false;
  if (client.googleAdsReportEnabled === false || client.enabled === false) return false;
  return true;
}

async function getGoogleAdsCustomValuesOverview(year, { timeZone } = {}) {
  ...
  const clients = (await listGoogleAdsReportClients({ filter: 'enabled', hydrateShareLinks: false }))
    .filter(shouldListOnGoogleAdsCustomValuesPage);
  const monthRows = await listGoogleAdsMonthInputRowsForYear(resolvedYear);
  ...
  const clientPayloads = clients.map((client) => {
    const clientMonths = byClient.get(String(client.clientId)) || new Map();
    const months = {};
    ...
    for (const monthKey of monthKeys) {
      ...
      const status = classifyGoogleAdsCustomInputStatus(row, {
        requireProfit: Boolean(client.showBottomline),
      });
      const updatedAt = googleAdsCustomInputsUpdatedAt(row, status);
...
module.exports = {
  ...
  getGoogleAdsCustomValuesOverview,
