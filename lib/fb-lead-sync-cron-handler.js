...
const CRON_SCHEDULE_LABEL = 'Manual / debug only (webhook + daily reconcile in production)';

function isLegacyCronAllowed() {
  if (process.env.FB_LEAD_LEGACY_CRON_DEBUG === '1') return true;
...
async function handleFbLeadSyncCronRequest(request, response) {
  ...
  if (!isLegacyCronAllowed()) {
    sendJson(response, 410, {
      error: 'Legacy hourly FB lead sync cron is disabled in production.',
      hint: 'Use webhook-first sync with /api/fb-lead-sync-retries (Vercel cron every 5m) and daily reconcile via /api/ghl-sync-cron. Set FB_LEAD_LEGACY_CRON_DEBUG=1 temporarily to re-enable for debugging.',
    });
    return;
  }
