...
async function logMetaSyncRun(clientId, {
  ...
  if (usePostgres()) {
    try {
      ...
      debugIngest('sync-history.js:logMetaSyncRun', 'direct insert ok', {
        rowId: Number(rowId),
        clientId: id || 'system',
        source: entry.source,
        status: entry.status,
...
const DEFAULT_PRODUCTION_LOG_URL = 'https://cenhub-dashboard.vercel.app/api/meta-sync-log';

async function logMetaSyncRunRemote(clientId, payload = {}) {
  const url = (process.env.META_SYNC_LOG_URL || DEFAULT_PRODUCTION_LOG_URL).replace(/\/$/, '');
  const secret = process.env.CRON_SECRET;
  debugIngest('sync-history.js:logMetaSyncRunRemote', 'remote log attempt', {
    url,
  ...
  if (!secret) {
    debugIngest('sync-history.js:logMetaSyncRunRemote', 'CRON_SECRET missing on caller', {
      vercelEnv: process.env.VERCEL_ENV || null,
    }, 'H2');
    throw new Error(
      'CRON_SECRET is not configured. Set the same CRON_SECRET on Vercel Production (and Preview) so Inngest cron runs can write to the Meta sync log.',
    );
  }
  ...
  if (!response.ok) {
    debugIngest('sync-history.js:logMetaSyncRunRemote', 'remote log API failed', {
      status: response.status,
      error: body?.error || text || null,
    }, 'H1');
    throw new Error(
      `Meta sync log API failed (${response.status}): ${body?.error || text || 'unknown error'}`,
    );
  }

  debugIngest('sync-history.js:logMetaSyncRunRemote', 'remote log ok', {
    status: response.status,
    rowId: body?.id || null,
  }, 'H1');
...
async function logMetaSyncRunForInngest(clientId, payload = {}) {
  const useDb = usePostgres();
  debugIngest('sync-history.js:logMetaSyncRunForInngest', 'choose log path', {
    useDb,
  ...
  if (useDb) {
    const row = await logMetaSyncRun(clientId, payload);
    debugIngest('sync-history.js:logMetaSyncRunForInngest', 'direct db log ok', {
      rowId: row?.id || null,
      clientId: clientId || 'system',
    }, 'H3');
    return row;
  }
  return logMetaSyncRunRemote(clientId, payload);
}

const GHL_SYNC_SCHEDULE_LABEL = 'Daily at 3:00 Copenhagen (Vercel cron, 01:00 UTC)';
...
async function listGhlSyncRuns({ clientId = null, limit = DEFAULT_LIMIT } = {}) {
  ...
  if (usePostgres()) {
    ...
    return {
      type: 'ghl',
      runs,
      summary: buildSummary(runs, GHL_SYNC_SCHEDULE_LABEL),
    };
  }
...
const META_SYNC_SCHEDULE_LABEL = 'Daily at 4:00 UTC (Vercel cron)';
...
async function listMetaSyncRuns({ clientId = null, limit = DEFAULT_LIMIT } = {}) {
  ...
  if (usePostgres()) {
    ...
    return {
      type: 'meta',
      runs,
      summary: buildSummary(runs, META_SYNC_SCHEDULE_LABEL),
    };
  }
...
async function deleteGhlSyncRuns({ clientId = null } = {}) {
  if (usePostgres()) {
    if (clientId) {
    ...
    const rows = await query`DELETE FROM sync_runs RETURNING id`;
  ...
  if (clientId) {
    const id = normalizeClientId(clientId);
    store.syncRuns = (store.syncRuns || []).filter((row) => row.client_id !== id);
  } else {
    store.syncRuns = [];
  }
  writeLocalStore(store);
  return { deleted: before - (store.syncRuns || []).length, clientId: clientId || null };
}

async function deleteMetaSyncRuns({ clientId = null } = {}) {
  if (usePostgres()) {
    if (clientId) {
      const id = normalizeClientId(clientId);
      const rows = await query`
    ...
    const rows = await query`DELETE FROM meta_sync_runs RETURNING id`;
    return { deleted: rows.length };
  }
  ...
  if (clientId) {
    const id = normalizeClientId(clientId);
    store.metaSyncRuns = (store.metaSyncRuns || []).filter((row) => row.client_id !== id);
  } else {
    store.metaSyncRuns = [];
  }
  writeLocalStore(store);
  return { deleted: before - (store.metaSyncRuns || []).length, clientId: clientId || null };
}
...
module.exports = {
  ...
  listMetaSyncRuns,
  logMetaSyncRun,
  logMetaSyncRunRemote,
  logMetaSyncRunForInngest,
  deleteGhlSyncRuns,
  deleteMetaSyncRuns,
};
