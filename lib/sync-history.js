...
function buildSummary(runs, schedule) {
  ...
  const lastSuccess = runs.find((run) => run.status === 'success' || run.status === 'ok') || null;
  return {
...
async function logMetaSyncRun(clientId, {
  ...
  const entry = {
    ...
    spend_date_stop: spendDateStop,
    metrics_client_id: metricsClientId,
  };
  ...
  if (usePostgres()) {
    const rows = await query`
      ...
      VALUES (
        ${entry.client_id}, ${entry.started_at}, ${entry.finished_at}, ${entry.status}, ${entry.source},
...
async function listMetaSyncRuns({ clientId = null, limit = DEFAULT_LIMIT } = {}) {
  const cappedLimit = normalizeLimit(limit);
  const schedule = process.env.META_SYNC_CRON || '*/2 * * * *';
  const vercelCron = '*/2 * * * * (Vercel /api/meta-sync-cron)';
