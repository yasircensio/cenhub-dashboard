...
const { getAccount, listClientIds, suggestSlugFromName, normalizeClientId } = require('./account-store');
...
async function isReportTokenTaken(token, excludeClientId = null) {
  ...
  if (excluded && rows[0].client_id === excluded) return false;
  return true;
}

async function generateReportAccessToken(accountName, { excludeClientId } = {}) {
  const slug = reportSlugFromAccountName(accountName, excludeClientId);
...
async function rotateReportAccessToken(currentToken, accountName, excludeClientId) {
  const parsed = parseHybridReportToken(currentToken);
  const slug = parsed?.slug || reportSlugFromAccountName(accountName, excludeClientId);
