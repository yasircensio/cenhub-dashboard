...
async function recordMetaSyncRun(clientId, payload, skipHistoryLog = false) {
...
async function syncMetaMetrics(clientId, options = {}) {
  ...
  const skipHistoryLog = Boolean(options.skipHistoryLog);
  ...
  if (!verified.ok) {
    ...
    if (resolved.source === 'env') {
      message += ` Using Vercel env token${resolved.hint ? ` (${resolved.hint})` : ''}. Re-copy the System User token from Meta Business Settings into META_SYSTEM_USER_TOKEN_V2 (or META_SYSTEM_USER_TOKEN), redeploy, and sync again.`;
    } else if (resolved.source === 'account') {
      message += ' Clear the saved token override in admin and use META_SYSTEM_USER_TOKEN_V2 on Vercel instead.';
    }
    const finishedAt = new Date().toISOString();
