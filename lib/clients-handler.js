...
const { queueSyncAll } = require('./sync-batch');
...
async function handleClientsRequest(request, response) {
  ...
  try {
      ...
      if (method === 'POST') {
        ...
        if (body.action === 'sync-all') {
          ...
          sendJson(response, 200, {
            ...
            fallback: true,
            results,
            message: 'Inngest is not configured — synced clients sequentially in this request.',
          });
          return;
        }
        await requireAdminClientsAuth(request);
    ...
    if (pathInfo.kind === 'sync') {
      ...
      await requireClientsAuth(request);
      const result = await syncAccount(pathInfo.clientId);
      sendJson(response, 200, result);
      return;
    }
