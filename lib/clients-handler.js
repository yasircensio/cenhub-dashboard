...
const { INLINE_SYNC_MAX_CLIENTS, syncAllGhlInline, syncAllInline } = require('./sync-batch');
...
async function handleClientsRequest(request, response) {
  ...
  try {
    ...
    if (pathInfo.kind === 'slugs') {
      ...
      await requireClientsAuth(request);
      const clients = await listAccounts({ cenhubDashboardOnly: true });
    ...
    if (pathInfo.kind === 'root') {
      ...
      if (method === 'POST') {
        ...
        if (body.action === 'sync-all') {
          ...
          const clientIds = await listCenhubDashboardClientIds();
          const results = clientIds.length <= INLINE_SYNC_MAX_CLIENTS
            ? await syncAllInline(clientIds, { source: 'admin' })
            : await syncAllGhlInline(clientIds, { source: 'admin' });
    ...
    if (pathInfo.kind === 'account') {
      ...
      if (method === 'PUT') {
        await requireClientsAuth(request);
        const body = parseJsonBody(request.body);
        if (body.accountName !== undefined && !String(body.accountName).trim()) {
          sendJson(response, 400, { error: 'Dashboard heading (accountName) cannot be empty.' });
          return;
        }
        await updateAccount(pathInfo.clientId, {
          ...
          fbLeadSyncEnabled: body.fbLeadSyncEnabled,
          ghlFbLeadFieldId: body.ghlFbLeadFieldId,
        });
        sendJson(response, 200, { account: await getAccountDetail(pathInfo.clientId) });
        return;
      }
