...
function getBundlinjePipelineIds(afterSalePipelineId) {
  return afterSalePipelineId ? new Set([afterSalePipelineId]) : new Set();
}

function sumWonBundlinje(opportunities, filters, pipelineIds) {
  if (!pipelineIds.size) return 0;
  const hasDateFilter = Boolean(filters.dateFrom || filters.dateTo);
  let total = 0;

  for (const opportunity of opportunities) {
    if (!pipelineIds.has(opportunity.pipelineId)) continue;
    if (opportunity.status !== 'won') continue;
...
function aggregateOpportunities(
  ...
  const bundlinjePipelineIds = getBundlinjePipelineIds(afterSalePipelineId);
  const hasDateFilter = Boolean(filters.dateFrom || filters.dateTo);
  ...
  if (hasDateFilter) {
    ...
    wonBundlinje = sumWonBundlinje(allOpportunities, filters, bundlinjePipelineIds);
  } else {
    wonRevenue = 0;
