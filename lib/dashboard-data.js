...
function getDashboardReadSource() {
  const configured = String(process.env.DASHBOARD_READ_SOURCE || '').trim().toLowerCase();

  if (process.env.VERCEL_ENV === 'production') {
    // Rollback: set both DASHBOARD_READ_SOURCE=live and DASHBOARD_LIVE_ROLLBACK=1 on Vercel.
    if (configured === 'live' && String(process.env.DASHBOARD_LIVE_ROLLBACK || '').trim() === '1') {
      return 'live';
    }
    return 'snapshot';
  }
...
async function loadDataSource(accountContext, query = {}) {
  ...
  const readSource = getDashboardReadSource();
  ...
  if (readSource === 'snapshot') {
    ...
    if (hasUsableSnapshot(snapshot)) {
      return snapshotToDataSource(snapshot, forceFresh ? 'snapshot-refresh' : 'snapshot');
    }

    const error = new Error(
      `No synced GHL snapshot for "${clientId}". Run Sync GHL data from admin, or wait for the daily sync.`,
    );
