...
function countUniqueWonClients(opportunities) {
  ...
  return contactIds.size + opportunityIds.size;
}

function countClientsWon(rawOpportunities, afterSalePipelineId, filters) {
  if (!afterSalePipelineId) {
    return countUniqueWonClients(rawOpportunities.filter((opportunity) => opportunity.status === 'won'));
  }
  ...
  for (const opportunity of rawOpportunities) {
    if (opportunity.pipelineId !== afterSalePipelineId) continue;
    if (opportunity.status !== 'won') continue;
...
function sumWonBundlinje(opportunities, filters) {
  const hasDateFilter = Boolean(filters.dateFrom || filters.dateTo);
  let total = 0;
...
function buildPeriodKpis(baseOpportunities, filters) {
  ...
  const clientsWon = countUniqueWonClients(wonInPeriod);
...
function aggregateOpportunities(
  ...
  if (hasDateFilter) {
    ...
    wonBundlinje = sumWonBundlinje(allOpportunities, filters);
  } else {
    wonRevenue = 0;
  ...
  clientsWon = countClientsWon(
    rawOpportunities,
    dedupeStats?.afterSalePipelineId || null,
    filters,
  );
