...
function getProfit(opportunity, profitFieldId = DEFAULT_PROFIT_FIELD_ID) {
  ...
  const field = fields.find((customField) =>
    customField.id === profitFieldId
    || customField.fieldKey === `opportunity.${profitFieldId}`
    || customField.fieldKey === profitFieldId
...
async function loadDataSource(accountContext) {
  const readSource = process.env.DASHBOARD_READ_SOURCE || 'snapshot';
  const snapshot = await getSnapshot(accountContext.clientId);
  const hasSnapshot = Boolean(
    snapshot?.fetched_at
    && (snapshot.opportunities?.length || snapshot.pipelines?.length),
  );

  if (readSource === 'snapshot' && hasSnapshot) {
    return {
      source: 'snapshot',
  ...
  return {
    source: readSource === 'snapshot' ? 'live-fallback' : 'live',
...
async function getDashboardData(query = {}) {
  ...
  const dataSource = await loadDataSource(accountContext);
  const {
