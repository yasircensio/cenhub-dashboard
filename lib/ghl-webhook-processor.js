const { getAccount, getSnapshot } = require('./account-store');
const { fetchOpportunityById } = require('./ghl-sync');
const { markGhlWebhookProcessed } = require('./ghl-webhook-events');
const { triggerFbLeadSyncForOpportunity } = require('./fb-lead-sync-webhook-hook');
const { triggerMetaReportGhlSyncForOpportunity } = require('./meta-report-ghl-webhook-hook');
...
async function processGhlOpportunityWebhook(payload) {
  ...
  if (isDeleteEvent(eventType)) {
    ...
    const removedOpportunity = (snapshot?.opportunities || []).find(
      (row) => String(row?.id) === String(opportunityId),
    );
    const result = await removeOpportunityFromSnapshot(account.clientId, opportunityId);
    const metaSyncStartedAt = new Date().toISOString();
    try {
      const metaSyncResult = await triggerMetaReportGhlSyncForOpportunity(account, removedOpportunity, {
        deleted: true,
        opportunityId,
      });
      await safeRecordMetaReportGhlSyncRun(account.clientId, {
        trigger: 'webhook',
        source: 'ghl-webhook',
        result: metaSyncResult,
        startedAt: metaSyncStartedAt,
      ...
      console.error('[ghl-webhook] Meta report GHL sync hook failed:', error.message);
      await safeRecordMetaReportGhlSyncRun(account.clientId, {
        trigger: 'webhook',
        source: 'ghl-webhook',
        result: null,
  ...
  try {
    await triggerFbLeadSyncForOpportunity(account, opportunity, {
      eventType,
      webhookId,
    });
  } catch (error) {
    console.error('[ghl-webhook] FB lead sync hook failed:', error.message);
  }

  const metaSyncStartedAt = new Date().toISOString();
  try {
    const metaSyncResult = await triggerMetaReportGhlSyncForOpportunity(account, opportunity);
    await safeRecordMetaReportGhlSyncRun(account.clientId, {
      trigger: 'webhook',
      source: 'ghl-webhook',
      result: metaSyncResult,
