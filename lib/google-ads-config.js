const GOOGLE_ADS_SCOPE = 'https://www.googleapis.com/auth/adwords';
const DEFAULT_GOOGLE_ADS_API_VERSION = 'v22';
const MIN_GOOGLE_ADS_API_MAJOR = 22;

function readEnv(name) {
  return String(process.env[name] || '').trim();
}

function normalizeGoogleAdsCustomerId(value) {
  const digits = String(value || '').replace(/\D/g, '');
  return digits.length === 10 ? digits : null;
}

function normalizeGoogleAdsApiVersion(value) {
  const match = String(value || '').trim().match(/^v?(\d+)/i);
  if (!match) return DEFAULT_GOOGLE_ADS_API_VERSION;
  const major = Number(match[1]);
  if (!Number.isFinite(major) || major < MIN_GOOGLE_ADS_API_MAJOR) {
    return DEFAULT_GOOGLE_ADS_API_VERSION;
  }
  return `v${major}`;
}

function formatGoogleAdsCustomerId(value) {
  const id = normalizeGoogleAdsCustomerId(value);
  if (!id) return null;
  return `${id.slice(0, 3)}-${id.slice(3, 6)}-${id.slice(6)}`;
}

function getGoogleAdsConfig(options = {}) {
  const developerToken = readEnv(options.developerTokenEnv || 'GOOGLE_ADS_DEVELOPER_TOKEN');
  const clientId = readEnv(options.clientIdEnv || 'GOOGLE_ADS_CLIENT_ID');
  const clientSecret = readEnv(options.clientSecretEnv || 'GOOGLE_ADS_CLIENT_SECRET');
  const refreshToken = readEnv(options.refreshTokenEnv || 'GOOGLE_ADS_REFRESH_TOKEN');
  const loginCustomerId = normalizeGoogleAdsCustomerId(
    readEnv(options.loginCustomerIdEnv || 'GOOGLE_ADS_LOGIN_CUSTOMER_ID'),
  );
  const apiVersion = normalizeGoogleAdsApiVersion(
    readEnv(options.apiVersionEnv || 'GOOGLE_ADS_API_VERSION'),
  );

  return {
    developerToken,
    clientId,
    clientSecret,
    refreshToken,
    loginCustomerId,
    apiVersion,
    scope: GOOGLE_ADS_SCOPE,
  };
}

function getGoogleAdsConfigDebug(config = getGoogleAdsConfig()) {
  const token = config.developerToken || '';
  const raw = String(process.env.GOOGLE_ADS_DEVELOPER_TOKEN || '');
  return {
    apiVersion: config.apiVersion,
    hasDeveloperToken: Boolean(token),
    developerTokenLength: token.length,
    rawEnvLength: raw.length,
    trimmedDiffersFromRaw: raw !== token,
    startsWithQuote: /^['"]/.test(token) || /^['"]/.test(raw),
    looksLikeOauthClientId: /apps\.googleusercontent\.com/i.test(token),
    looksLikeOauthSecret: /^GOCSPX-/i.test(token),
    hasRefreshToken: Boolean(config.refreshToken),
    refreshTokenLength: (config.refreshToken || '').length,
    hasLoginCustomerId: Boolean(config.loginCustomerId),
  };
}

function describeGoogleAdsDeveloperTokenProblem(config = getGoogleAdsConfig()) {
  const length = (config.developerToken || '').length;
  if (!length) {
    return 'GOOGLE_ADS_DEVELOPER_TOKEN is missing in Vercel.';
  }
  if (length !== 22) {
    return `GOOGLE_ADS_DEVELOPER_TOKEN is ${length} characters; Google Ads developer tokens are 22. Copy the full token from Google Ads API Center (ads.google.com/aw/apicenter), update the Vercel env var, and redeploy.`;
  }
  return null;
}

function assertGoogleAdsOAuthConfig(config = getGoogleAdsConfig()) {
  const missing = [];
  if (!config.clientId) missing.push('GOOGLE_ADS_CLIENT_ID');
  if (!config.clientSecret) missing.push('GOOGLE_ADS_CLIENT_SECRET');
  if (missing.length) {
    const error = new Error(`Missing Google Ads OAuth env: ${missing.join(', ')}`);
    error.code = 'GOOGLE_ADS_CONFIG';
    throw error;
  }
  return config;
}

function assertGoogleAdsQueryConfig(config = getGoogleAdsConfig()) {
  assertGoogleAdsOAuthConfig(config);
  const missing = [];
  if (!config.developerToken) missing.push('GOOGLE_ADS_DEVELOPER_TOKEN');
  if (!config.refreshToken) missing.push('GOOGLE_ADS_REFRESH_TOKEN');
  if (missing.length) {
    const error = new Error(`Missing Google Ads query env: ${missing.join(', ')}`);
    error.code = 'GOOGLE_ADS_CONFIG';
    throw error;
  }
  return config;
}

function resolveGoogleAdsOAuthRedirectUri(request = null) {
  const explicit = readEnv('GOOGLE_ADS_OAUTH_REDIRECT_URI');
  if (explicit) return explicit.replace(/\/$/, '');

  const requestPath = String(request?.url || request?.path || '').split('?')[0];
  const host = request?.headers?.['x-forwarded-host'] || request?.headers?.host || '';
  const proto = request?.headers?.['x-forwarded-proto'] || 'https';
  if (host && requestPath.includes('/api/auth/google-ads/callback')) {
    return `${proto}://${host}/api/auth/google-ads/callback`;
  }

  const appBase = readEnv('APP_BASE_URL')
    || readEnv('DASHBOARD_PUBLIC_URL')
    || (readEnv('VERCEL_URL') ? `https://${readEnv('VERCEL_URL')}` : '');
  if (appBase) {
    return `${appBase.replace(/\/$/, '')}/api/auth/google-ads/callback`;
  }

  if (host) {
    return `${proto}://${host}/api/auth/google-ads/callback`;
  }

  return 'https://analytics.censio.dk/api/auth/google-ads/callback';
}

module.exports = {
  GOOGLE_ADS_SCOPE,
  DEFAULT_GOOGLE_ADS_API_VERSION,
  MIN_GOOGLE_ADS_API_MAJOR,
  readEnv,
  normalizeGoogleAdsCustomerId,
  normalizeGoogleAdsApiVersion,
  formatGoogleAdsCustomerId,
  getGoogleAdsConfig,
  getGoogleAdsConfigDebug,
  describeGoogleAdsDeveloperTokenProblem,
  assertGoogleAdsOAuthConfig,
  assertGoogleAdsQueryConfig,
  resolveGoogleAdsOAuthRedirectUri,
};
