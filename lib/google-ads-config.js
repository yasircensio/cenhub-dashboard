...
const MIN_GOOGLE_ADS_API_MAJOR = 25;
...
function normalizeGoogleAdsApiVersion(value) {
  const match = String(value || '').trim().match(/^v?(\d+)/i);
  if (!match) return DEFAULT_GOOGLE_ADS_API_VERSION;
  const major = Number(match[1]);
  if (!Number.isFinite(major) || major < MIN_GOOGLE_ADS_API_MAJOR) {
...
function getGoogleAdsConfig(options = {}) {
  ...
  const apiVersion = normalizeGoogleAdsApiVersion(
    readEnv(options.apiVersionEnv || 'GOOGLE_ADS_API_VERSION'),
  );
...
function getTokenShape(token) {
  ...
  let nonAsciiCount = 0;
  let asciiHyphenCount = 0;
  let asciiUnderscoreCount = 0;
  const otherCharCodes = [];
  for (let i = 0; i < value.length; i += 1) {
    const code = value.charCodeAt(i);
    if (code > 127) nonAsciiCount += 1;
    if (code === 45) asciiHyphenCount += 1;
    else if (code === 95) asciiUnderscoreCount += 1;
    else if (
      !((code >= 48 && code <= 57) || (code >= 65 && code <= 90) || (code >= 97 && code <= 122))
    ) {
      otherCharCodes.push(code);
    }
  }
  return {
    officialExpectedLength: 22,
    lengthShortBy: Math.max(0, 22 - value.length),
    nonAsciiCount,
    asciiHyphenCount,
    asciiUnderscoreCount,
    otherCharCodes: otherCharCodes.slice(0, 8),
    firstCharCode: value ? value.charCodeAt(0) : null,
    lastCharCode: value ? value.charCodeAt(value.length - 1) : null,
  };
}

function getGoogleAdsConfigDebug(config = getGoogleAdsConfig()) {
  const token = config.developerToken || '';
  const raw = String(process.env.GOOGLE_ADS_DEVELOPER_TOKEN || '');
  return {
    apiVersion: config.apiVersion,
    hasDeveloperToken: Boolean(token),
    developerTokenLength: token.length,
    rawEnvLength: raw.length,
    trimmedDiffersFromRaw: raw !== token,
    startsWithQuote: /^['"]/.test(token) || /^['"]/.test(raw),
    looksLikeOauthClientId: /apps\.googleusercontent\.com/i.test(token),
    looksLikeOauthSecret: /^GOCSPX-/i.test(token),
    hasHyphen: token.includes('-'),
    hasUnderscore: token.includes('_'),
    hasRefreshToken: Boolean(config.refreshToken),
    refreshTokenLength: (config.refreshToken || '').length,
    hasLoginCustomerId: Boolean(config.loginCustomerId),
    tokenShape: getTokenShape(token),
  };
}
...
function resolveGoogleAdsOAuthRedirectUri(request = null) {
  ...
  if (host && requestPath.includes('/api/auth/google-ads/callback')) {
  ...
  if (appBase) {
    return `${appBase.replace(/\/$/, '')}/api/auth/google-ads/callback`;
  }
