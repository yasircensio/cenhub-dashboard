...
async function syncAccount(clientId, options = {}) {
  ...
  try {
    ...
    try {
      const { autoSyncMetaReportGhlAfterSnapshot } = require('./meta-report-ghl-service');
      const metaSyncResult = await autoSyncMetaReportGhlAfterSnapshot(clientId);
      const { safeRecordMetaReportGhlSyncRun } = require('./meta-report-ghl-sync-history');
      await safeRecordMetaReportGhlSyncRun(clientId, {
        trigger: 'full_snapshot',
        source: 'full-ghl-sync',
        result: metaSyncResult,
        startedAt: metaSyncStartedAt,
        finishedAt: new Date().toISOString(),
        context: { parentSource: source },
      });
    } catch (error) {
      console.error('[sync-service] Meta report GHL auto-sync failed:', error.message);
      try {
        const { safeRecordMetaReportGhlSyncRun } = require('./meta-report-ghl-sync-history');
        await safeRecordMetaReportGhlSyncRun(clientId, {
          trigger: 'full_snapshot',
        ...
        console.error('[sync-service] Meta report GHL sync log failed:', logError.message);
      }
    }
    return {
      success: true,
