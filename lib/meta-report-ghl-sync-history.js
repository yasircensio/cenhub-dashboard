...
let lastMetaReportGhlPruneAt = 0;

async function maybePruneMetaReportGhlSyncHistory() {
  const now = Date.now();
  if (now - lastMetaReportGhlPruneAt < PRUNE_INTERVAL_MS) return null;
  lastMetaReportGhlPruneAt = now;
  try {
    return await pruneMetaReportGhlSyncHistory();
  } catch (error) {
    console.error('[meta-report-ghl-sync-history] Retention prune failed:', error.message || error);
    return null;
  }
}
...
async function logMetaReportGhlSyncRun(clientId, payload = {}) {
  ...
  if (usePostgres()) {
    ...
    await maybePruneMetaReportGhlSyncHistory();
