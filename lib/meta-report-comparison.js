...
function resolveComparisonPresetDates({
  ...
  const anchorMonth = monthsMap[anchorKey] || null;

  if (normalizedMode === 'mom') {
    const previousKey = addMonthsToMonthKey(anchorKey, -1);
    const previousMonth = previousKey ? monthsMap[previousKey] || null : null;
    return {
      mode: normalizedMode,
      periodA: monthPayloadDates(anchorMonth, anchorKey),
      periodB: previousKey ? monthPayloadDates(previousMonth, previousKey) : null,
    };
  }
  ...
  if (normalizedMode === 'ytd') {
    ...
    const anchorDates = monthPayloadDates(anchorMonth, anchorKey);
    const priorMonth = monthsMap[`${priorYear}-${month}`] || null;
    return {
      mode: normalizedMode,
      periodA: {
        startDate: `${year}-01-01`,
        endDate: anchorDates.endDate,
        startMonthKey: `${year}-01`,
        endMonthKey: anchorKey,
      },
      periodB: {
        startDate: `${priorYear}-01-01`,
        endDate: monthPayloadDates(priorMonth, `${priorYear}-${month}`).endDate,
        startMonthKey: `${priorYear}-01`,
        endMonthKey: `${priorYear}-${month}`,
      },
    };
  }
...
function buildComparison({
  ...
  const aggA = aggregateDateRange(mapA, normalizedA.startDate, normalizedA.endDate);
  const aggB = aggregateDateRange(mapB, normalizedB.startDate, normalizedB.endDate);
