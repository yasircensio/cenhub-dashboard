const { listClientIds, recoverStuckSyncStates } = require('./account-store');
const { syncAllGhlInline } = require('./sync-batch');
const { runDailyFbLeadReconciliation } = require('./fb-lead-sync-reconcile');
...
async function handleGhlSyncCronRequest(request, response) {
  ...
  try {
    ...
    let fbLeadReconcile = null;
    try {
      fbLeadReconcile = await runDailyFbLeadReconciliation({ clientIds, source: 'daily-reconcile' });
    } catch (error) {
      fbLeadReconcile = { success: false, error: error.message || 'Daily FB lead reconcile failed.' };
    }
