...
function rowToMonthRecord(row) {
  ...
  return {
    ...
    toplineSource: row.topline_source || null,
    manualOverride: Boolean(row.manual_override),
    ghlLeads: row.ghl_leads != null ? Number(row.ghl_leads) : null,
    ghlWonLeads: row.ghl_won_leads != null ? Number(row.ghl_won_leads) : null,
    ghlTotalRevenue: row.ghl_total_revenue != null ? Number(row.ghl_total_revenue) : null,
    ghlTotalProfit: row.ghl_total_profit != null ? Number(row.ghl_total_profit) : null,
    ghlSyncedAt: row.ghl_synced_at || null,
    manualLeads: row.manual_leads != null ? Number(row.manual_leads) : null,
    metaSavedWonLeads: row.meta_saved_won_leads != null ? Number(row.meta_saved_won_leads) : null,
    metaSavedAvgLeadValue: row.meta_saved_avg_lead_value != null ? Number(row.meta_saved_avg_lead_value) : null,
    metaSavedAvgProfitPerWon: row.meta_saved_avg_profit_per_won != null
      ? Number(row.meta_saved_avg_profit_per_won)
      : null,
...
function accountReportFields(account) {
  ...
  return {
    ...
    metaReportExcelSheetUrl: normalizeMetaReportExcelSheetUrl(account.metaReportExcelSheetUrl),
    hasGhl: Boolean(account.hasGhlToken && account.locationId),
  };
}
...
async function listMetaReportClients({ filter = 'all' } = {}) {
  ...
  for (const clientId of ids) {
    ...
    if (filter === 'meta-only' && account.metaReportGhlDataEnabled && account.metaReportToplineMode === 'cenhub') continue;
    clients.push({
  ...
  const { mapMetaToGoogleLinks } = require('./report-link');
  const googleLinks = await mapMetaToGoogleLinks();
  for (const client of clients) {
    const link = googleLinks.get(String(client.clientId));
    if (!link) continue;
    client.linkedGoogleClientId = link.linkedGoogleClientId;
    client.linkedGoogleAccountName = link.linkedGoogleAccountName;
    client.linkedGoogleEnabled = link.linkedGoogleEnabled;
  }
...
async function getMetaReportsDashboard({ filter = 'all' } = {}) {
  ...
  if (filter === 'meta-only') {
    clients = merged.filter((row) => !(row.metaReportGhlDataEnabled && row.metaReportToplineMode === 'cenhub'));
  }
...
async function updateMetaReportSettings(clientId, input = {}) {
  ...
  if (input.metaReportExcelSheetUrl !== undefined) {
    const normalized = normalizeMetaReportExcelSheetUrl(input.metaReportExcelSheetUrl);
    if (String(input.metaReportExcelSheetUrl || '').trim() && !normalized) {
      const error = new Error('Invalid Excel sheet URL.');
      error.statusCode = 400;
      throw error;
    }
    patch.metaReportExcelSheetUrl = normalized;
  }
...
async function saveMonthRecord(clientId, monthKey, input = {}) {
  ...
  const toplineSource = input.toplineSource !== undefined ? input.toplineSource : existing.toplineSource;
  const manualOverride = input.manualOverride != null
    ? Boolean(input.manualOverride)
    : existing.manualOverride;
  const manualLeads = input.manualLeads !== undefined ? input.manualLeads : existing.manualLeads;
  const settings = accountReportFields(account);
  const preserveCenhubManual = isCenhubManualStored(existing)
    && settings.metaReportToplineMode !== 'cenhub'
    && input.toplineSource === undefined
    && input.manualOverride == null;

  let resolvedWonLeads = wonLeads;
  let resolvedAvgLeadValue = avgLeadValue;
  let resolvedAvgProfitPerWon = avgProfitPerWon;
  let metaSavedWonLeads = input.metaSavedWonLeads !== undefined
    ? input.metaSavedWonLeads
    : existing.metaSavedWonLeads;
  let metaSavedAvgLeadValue = input.metaSavedAvgLeadValue !== undefined
    ? input.metaSavedAvgLeadValue
    : existing.metaSavedAvgLeadValue;
  let metaSavedAvgProfitPerWon = input.metaSavedAvgProfitPerWon !== undefined
    ? input.metaSavedAvgProfitPerWon
    : existing.metaSavedAvgProfitPerWon;

  if (preserveCenhubManual) {
    resolvedWonLeads = existing.wonLeads;
    resolvedAvgLeadValue = existing.avgLeadValue;
    resolvedAvgProfitPerWon = existing.avgProfitPerWon;
    if (input.wonLeads !== undefined) metaSavedWonLeads = input.wonLeads;
    if (input.avgLeadValue !== undefined) metaSavedAvgLeadValue = input.avgLeadValue;
    if (input.avgProfitPerWon !== undefined) metaSavedAvgProfitPerWon = input.avgProfitPerWon;
  }
  ...
  if (!usePostgres()) {
    ...
    const next = {
      ...
      won_leads: resolvedWonLeads,
      avg_lead_value: resolvedAvgLeadValue,
      avg_profit_per_won: resolvedAvgProfitPerWon,
      topline_source: toplineSource,
      manual_override: manualOverride,
      manual_leads: manualLeads,
      meta_saved_won_leads: metaSavedWonLeads,
      meta_saved_avg_lead_value: metaSavedAvgLeadValue,
      meta_saved_avg_profit_per_won: metaSavedAvgProfitPerWon,
      published,
      updated_at: new Date().toISOString(),
    };
  ...
  const rows = await query`
    UPDATE meta_report_months SET
      ...
      manual_override = ${manualOverride},
      manual_leads = ${manualLeads},
      meta_saved_won_leads = ${metaSavedWonLeads ?? null},
      meta_saved_avg_lead_value = ${metaSavedAvgLeadValue ?? null},
      meta_saved_avg_profit_per_won = ${metaSavedAvgProfitPerWon ?? null},
      published = ${published},
      updated_at = NOW()
    WHERE client_id = ${id} AND month_key = ${key}
    RETURNING *
...
async function saveGhlMonthRecord(clientId, monthKey, aggregated = null, options = {}) {
  const account = await getAccount(clientId);
  if (!account) {
    const error = new Error('Account not found.');
    error.statusCode = 404;
  ...
  const bounds = monthBoundsIso(key);
  const toplineSource = options.toplineSource
    ?? (aggregated ? 'ghl' : existing.toplineSource);
  ...
  if (aggregated) {
    patch.ghlLeads = aggregated.leads;
    patch.ghlWonLeads = aggregated.wonLeads;
    patch.ghlTotalRevenue = aggregated.totalRevenue;
    patch.ghlTotalProfit = aggregated.totalProfit;
    patch.wonLeads = aggregated.wonLeads;
    patch.avgLeadValue = aggregated.avgLeadValue;
    patch.avgProfitPerWon = aggregated.avgProfitPerWon;
    patch.ghlSyncedAt = new Date().toISOString();
    const { buildMetaInputsSnapshot, shouldSnapshotMetaInputs } = require('./meta-report-topline-restore');
    if (shouldSnapshotMetaInputs(existing)) {
  ...
  if (!usePostgres()) {
    ...
    const next = {
      ...
      topline_source: patch.toplineSource,
      manual_override: patch.manualOverride,
      published: patch.published,
      updated_at: new Date().toISOString(),
    };
    if (aggregated) {
      next.ghl_leads = patch.ghlLeads;
      next.ghl_won_leads = patch.ghlWonLeads;
      next.ghl_total_revenue = patch.ghlTotalRevenue;
      next.ghl_total_profit = patch.ghlTotalProfit;
      next.won_leads = patch.wonLeads;
      next.avg_lead_value = patch.avgLeadValue;
      next.avg_profit_per_won = patch.avgProfitPerWon;
      next.ghl_synced_at = patch.ghlSyncedAt;
      if (patch.metaSavedWonLeads != null) next.meta_saved_won_leads = patch.metaSavedWonLeads;
      if (patch.metaSavedAvgLeadValue != null) next.meta_saved_avg_lead_value = patch.metaSavedAvgLeadValue;
      if (patch.metaSavedAvgProfitPerWon != null) {
        next.meta_saved_avg_profit_per_won = patch.metaSavedAvgProfitPerWon;
      }
    }
    store.months[storeKey] = next;
    writeFileStore(store);
  ...
  if (aggregated) {
    const rows = await query`
      UPDATE meta_report_months SET
        ...
        topline_source = ${patch.toplineSource},
        manual_override = ${patch.manualOverride},
        ghl_leads = ${patch.ghlLeads},
        ghl_won_leads = ${patch.ghlWonLeads},
        ghl_total_revenue = ${patch.ghlTotalRevenue},
        ghl_total_profit = ${patch.ghlTotalProfit},
        ghl_synced_at = NOW(),
        won_leads = ${patch.wonLeads},
        avg_lead_value = ${patch.avgLeadValue},
        avg_profit_per_won = ${patch.avgProfitPerWon},
        meta_saved_won_leads = ${patch.metaSavedWonLeads ?? null},
        meta_saved_avg_lead_value = ${patch.metaSavedAvgLeadValue ?? null},
        meta_saved_avg_profit_per_won = ${patch.metaSavedAvgProfitPerWon ?? null},
        published = ${patch.published},
        updated_at = NOW()
      WHERE client_id = ${id} AND month_key = ${key}
      RETURNING *
  ...
  if (!usePostgres()) {
    ...
    return Object.values(store.months || {})
      ...
      .map((row) => ({
        ...
        topline_source: row.topline_source,
        manual_override: row.manual_override,
        ghl_leads: row.ghl_leads,
        ghl_won_leads: row.ghl_won_leads,
        ghl_total_revenue: row.ghl_total_revenue,
        ghl_total_profit: row.ghl_total_profit,
        ghl_synced_at: row.ghl_synced_at,
        manual_leads: row.manual_leads,
...
async function getCustomValuesOverview(year, { timeZone } = {}) {
  ...
  const clientPayloads = clients.map((client) => {
    ...
    return {
      ...
      metaReportGhlDataEnabled: Boolean(client.metaReportGhlDataEnabled),
      metaReportToplineMode: client.metaReportToplineMode === 'cenhub' ? 'cenhub' : 'meta',
      hasGhl: Boolean(client.hasGhl),
