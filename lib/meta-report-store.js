...
async function ensureMetaReportShareLink(clientId, { enable = true } = {}) {
  ...
  if (account.metaReportAccessToken) return account;

  const { updateAccount } = require('./account-store');
  const patch = {
    metaReportAccessToken: await generateReportAccessToken(
      account.accountName,
...
async function updateMetaReportSettings(clientId, input = {}) {
  ...
  if (input.rotateAccessToken) {
    ...
    patch.metaReportAccessToken = await generateReportAccessToken(
      account.accountName,
      { excludeClientId: clientId },
    );
  }
