...
function accountReportFields(account) {
  const feeMode = account.metaReportFeeMode || (
    account.metaReportFeeEnabled ? 'performance' : null
  );
  return {
    metaReportEnabled: Boolean(account.metaReportEnabled),
    metaReportShowBottomline: Boolean(account.metaReportShowBottomline),
    metaReportFeeEnabled: Boolean(feeMode),
    metaReportFeeMode: feeMode,
...
async function saveMonthRecord(clientId, monthKey, input = {}) {
  ...
  const id = normalizeClientId(clientId);
  const key = parseMonthKey(monthKey);
  const existing = await ensureMonthRecord(id, key, account, account.timezone);
...
async function replaceLineItems(clientId, monthKey, items = []) {
  let refreshed = await getMonthRecord(clientId, monthKey);
  if (!refreshed) {
    const account = await getAccount(clientId);
    refreshed = await ensureMonthRecord(clientId, monthKey, account, account?.timezone);
  }
  if (!refreshed?.id) {
    const error = new Error('Month record not found.');
    error.statusCode = 404;
...
function classifyCustomInputStatus(row, { requireProfit = false } = {}) {
  ...
  const lineItemCount = Number(row.lineItemCount || row.line_item_count || 0) || 0;
  const filledCore = hasWon && hasAvgLead && (!requireProfit || hasProfit);
  if (filledCore) return 'complete';
  if (hasWon || hasAvgLead || hasProfit || lineItemCount > 0) return 'partial';
  return 'empty';
}
...
function effectiveFeeEnabledForMonth(settings, monthRecord) {
  const status = classifyCustomInputStatus({
    ...
    line_item_count: monthRecord?.lineItems?.length || 0,
...
async function listMonthInputRowsForYear(year) {
  ...
  if (!usePostgres()) {
    ...
    return Object.values(store.months || {})
      ...
      .map((row) => ({
        ...
        line_item_count: Array.isArray(store.lineItems?.[row.id])
          ? store.lineItems[row.id].length
          : 0,
      }));
  }
  ...
  return query`
    SELECT
      ...
      (
        SELECT COUNT(*)::int
        FROM meta_report_line_items li
        WHERE li.meta_report_month_id = m.id
      ) AS line_item_count
...
async function getCustomValuesOverview(year, { timeZone } = {}) {
  ...
  const clientPayloads = clients.map((client) => {
    ...
    return {
      ...
      metaReportFeeMode: client.metaReportFeeMode || (client.metaReportFeeEnabled ? 'performance' : null),
      ...
      metaReportMarketingFeeAmount: client.metaReportMarketingFeeAmount != null
        ...
        : 0,
      metaReportShowOther: client.metaReportShowOther !== false,
      months,
