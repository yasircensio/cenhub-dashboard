...
function computeMetaReportMetrics(input = {}) {
  ...
  const feeEnabled = Boolean(input.feeEnabled);
  ...
  // Performance fee applies only to positive POAS (no negative "fee credit" on losses).
  const censioFee = feeEnabled
    ? roundMoney(Math.max(0, poasKr) * (feePercent / 100))
    : 0;

  const customLineItems = lineItems.map((row, index) => ({
    id: row.id ?? null,
  ...
  const customLineItemsTotal = roundMoney(
    customLineItems.reduce((sum, row) => sum + parseAmount(row.amount), 0),
  );
  // Sheet (Jan): POI = POAS - (fee + other costs); POI % (x) = POI / spend
  const poiKr = roundMoney(poasKr - censioFee - customLineItemsTotal);
  ...
  return {
    ...
    bottomline: showBottomline ? {
      ...
      poasX,
      censioFee,
      feePercent: feeEnabled ? feePercent : 0,
      poiKr,
      poiX,
    } : null,
    customLineItems,
