...
function projectMetaReportBudgetAtSpend(spend, efficiency, hasBottomline = false) {
  ...
  const leads = roundMoney(amount / efficiency.avgCpl);
  const wonLeads = roundMoney(leads * efficiency.winRate);
...
function projectMetaReportBudgetScenario({
  series = [],
  baselineSpend,
  multiplier = 1,
  hasBottomline = false,
} = {}) {
  const efficiency = aggregateMetaReportSeriesEfficiency(series);
  const baseSpend = parseAmount(baselineSpend);
  const mult = parseAmount(multiplier) || 1;
  const baseline = projectMetaReportBudgetAtSpend(baseSpend, efficiency, hasBottomline);
  const projected = projectMetaReportBudgetAtSpend(roundMoney(baseSpend * mult), efficiency, hasBottomline);
  ...
  return {
    ...
    hasBottomline,
    insufficientData: efficiency.avgCpl <= 0 || baseSpend <= 0,
  };
}
