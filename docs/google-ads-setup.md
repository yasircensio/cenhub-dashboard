...
Do not use `/api/google-ads/oauth/*` or `/api/google-ads-oauth-callback` — those 404 on the custom domain.
...
| Variable | Value |
|----------|--------|
| `GOOGLE_ADS_DEVELOPER_TOKEN` | API Center → View token (Explorer Access is fine for reporting) |
